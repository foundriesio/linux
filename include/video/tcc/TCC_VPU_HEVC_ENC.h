...
typedef struct hevc_enc_init_t
{
	...
	codec_addr_t m_BitstreamBufferAddr[2];     //!< The start physical[0] and virtual[1] address of output bitstream buffer into which encoder puts bitstream. (multiple of 1024)
    int m_iBitstreamBufferSize;			            //!< The size of the buffer in bytes pointed by output bitstreamBuffer. This value must be a multiple of 1024. The maximum size is 16383 x 1024 bytes.
 ...
 *     TCC_VPU_HEVC_ENC( VPU_ENC_ENCODE               , codec_handle_t*     , hevc_dec_input_t*    , hevc_dec_output_t*);
