...
#define VPU_HEVC_ENC_WORK_CODE_BUF_SIZE	(VPU_HEVC_ENC_SIZE_BIT_WORK + (VPU_HEVC_ENC_WORKBUF_SIZE*VPU_HEVC_ENC_MAX_NUM_INSTANCE))  // 16MB


// Calculation formula = FrameCount * Align1M( ( Align256(width) * Align64(height) * 69 / 32 + 20*1024 ) )
// The size of frame buffers is calculated by CAL_VPU_HEVC_ENC_FRAME_BUF_SIZE macro.
// The size of one frame buffer is calculated by CAL_VPU_HEVC_ENC_ONE_FRAME_BUF_SIZE macro.
...
#endif
#define CAL_VPU_HEVC_ENC_ONE_FRAME_BUF_SIZE(width, height) ALIGNED_SIZE_VPU_HEVC_ENC( (ALIGNED_SIZE_VPU_HEVC_ENC((width), 256)*ALIGNED_SIZE_VPU_HEVC_ENC((height), 64)*69/32 + (20*1024)), (1024*1024))
#define CAL_VPU_HEVC_ENC_FRAME_BUF_SIZE(width, height, frame_count)   ( CAL_VPU_HEVC_ENC_ONE_FRAME_BUF_SIZE(width, height) * (frame_count) )
