...
typedef struct dec_initial_info_t
{
	...
	int m_iMjpg_MinFrameBufferSize[4];  //!< minimum frame buffer size for JPEG only
	                                    ...
	                                    //!< 2 ( 1/4 Scaling Down )
	                                    //!< 3 ( 1/8 Scaling Down )
	unsigned int m_Reserved[33];        //! Reserved.
} dec_initial_info_t;
...
typedef struct dec_buffer_t
{
	...
	unsigned int m_Vp8MbDataSaveBufferAddr; //!< start address and size of mb save buffer which the decoder can save mb data : multiple of 4
	...
	int m_iMJPGScaleRatio;                  //!< JPEG Scaling Ratio
	                                        ...
	                                        //!< 2 ( 1/4 Scaling Down )
	                                        //!< 3 ( 1/8 Scaling Down )
	unsigned int m_Reserved[24];            //! Reserved.
} dec_buffer_t;
...
typedef struct dec_buffer2_t
{
	...
	unsigned int    m_addrAvcSliceSaveBuffer;       //!< start address and size of slice save buffer which the VP8 decoder can save slice RBSP : multiple of 4
