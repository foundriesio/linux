...
static int tcc_audio_probe(struct platform_device *pdev)
{
	...
	//	if (ret)
	//		return ret;
	amp_mute = of_get_named_gpio(pdev->dev.of_node, "amp_mute-gpios", 0);
	if (gpio_is_valid(amp_mute)) {
		gpio_request(amp_mute, "AMP_MUTE");
		gpio_direction_output(amp_mute, 1);
	}
	amp_stby = of_get_named_gpio(pdev->dev.of_node, "amp_stby-gpios", 0);
	if (gpio_is_valid(amp_stby)) {
		gpio_request(amp_stby, "AMP_STBY");
		gpio_direction_output(amp_stby, 1);
	}
...
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:tcc-audio-lcn2");
