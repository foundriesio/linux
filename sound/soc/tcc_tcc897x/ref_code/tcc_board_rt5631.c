...
#undef alsa_dbg
#if 1
#define alsa_dbg(fmt,arg...)    printk("<=tcc_board_rt5631== alsa-debug == "fmt,##arg)
#else
...
static void hp_mute()
{
	...
	{
#if defined(CONFIG_COBY_MID9729)
	...
	{
#if defined(CONFIG_COBY_MID9729)
		gpio_set_value(TCC_GPE(13), 0); // pad name gpio_e(17)
#else	 
		gpio_set_value(TCC_GPE(19), 0); // pad name gpio_e(15)
#endif		
        printk("@@@@@@@@@@@@@@@@@ %s() \n", __func__);
	}
...
static void hp_un_mute()
{
	...
	{
#if defined(CONFIG_COBY_MID9729)
		gpio_set_value(TCC_GPE(13), 1); // pad name gpio_e(17)
#else	
		gpio_set_value(TCC_GPE(19), 1); // pad name gpio_e(15)
#endif		
        printk("@@@@@@@@@@@@@@@@@ %s() \n", __func__);
	}
#endif
...
static const struct snd_soc_dapm_widget tcc_rt5631_dapm_widgets[] = {
...
static const struct snd_kcontrol_new rt5631_tcc_controls[] = {
...
*/
/*
 * Logic for a rt5631 as connected on a Sharp SL-C7x0 Device
 */
static int tcc_rt5631_init(struct snd_soc_pcm_runtime *rtd)
{
	int i, ret;
	struct snd_soc_codec *codec = rtd->codec;
	struct snd_soc_dapm_context *dapm = &codec->dapm;
...
//    snd_soc_dapm_enable_pin(dapm, "MICIN");
	...
	ret = snd_soc_add_controls(codec, rt5631_tcc_controls,
				ARRAY_SIZE(rt5631_tcc_controls));
	if (ret)
		return ret;


	/* Add tcc specific widgets */
	snd_soc_dapm_new_controls(dapm, tcc_rt5631_dapm_widgets,
				  ARRAY_SIZE(tcc_rt5631_dapm_widgets));
...
static struct snd_soc_dai_link tcc_dai[] = {
	{
        ...
        .codec_name = "rt5631 I2C Codec.0-001a",
        .codec_dai_name = "rt5631-hifi",
        .init = tcc_rt5631_init,
        .ops = &tcc_ops,
	},
#if !defined(CONFIG_SND_TCC_DAI2SPDIF)
#if defined(CONFIG_ARCH_TCC892X) || defined(CONFIG_CHIP_TCC8925S) // Planet 20121025 SPDIF exists only on channel01
    {
        .name = "TCC",
        .stream_name = "IEC958",
 ...
 * New drivers should register the rt5631 I2C device in the machine 
 * setup code (under arch/arm for ARM systems). 
 */
static int rt5631_i2c_register(void)
{
...
static int __init tcc_init_rt5631(void)
{
...
#else		
        tcc_gpio_config(TCC_GPG(15), GPIO_FN(0));
        gpio_request(TCC_GPG(15), "HP_MUTE_CTL");
  
        gpio_direction_output(TCC_GPG(15), 0);   // HeadPhone mute
#endif		
        tcc_hp_hw_mute(false);
        tcc_spk_hw_mute(false);
    }
	...
	gpio_set_value(TCC_GPE(14), 1); // pad name gpio_e(18)

    /* h/w mute control */
    if(machine_is_m805_893x()) {
#if defined(CONFIG_COBY_MID9729)
        tcc_gpio_config(TCC_GPE(13), GPIO_FN(0)); // pad name gpio_e(17)
        gpio_request(TCC_GPE(13), "HP_MUTE_CTL");
  
        gpio_direction_output(TCC_GPE(13), 0);   // HeadPhone mute
#else		
        tcc_gpio_config(TCC_GPG(19), GPIO_FN(0));  // pad name gpio_e(16)
        gpio_request(TCC_GPG(19), "HP_MUTE_CTL");
  
        gpio_direction_output(TCC_GPG(19), 0);   // HeadPhone mute
#endif		
        tcc_hp_hw_mute(false);
        tcc_spk_hw_mute(false);
    }
    ...
    alsa_dbg("=======end func: tcc_init_rt5631\n");
	return ret;
}

static void __exit tcc_exit_rt5631(void)
{
	platform_device_unregister(tcc_snd_device);
}

module_init(tcc_init_rt5631);
module_exit(tcc_exit_rt5631);

/* Module information */
MODULE_AUTHOR("linux <linux@telechips.com>");
MODULE_DESCRIPTION("ALSA SoC TCCxx Board (RT5631)");
MODULE_LICENSE("GPL");
