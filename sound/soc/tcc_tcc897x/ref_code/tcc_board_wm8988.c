...
static void spk_mute(void)
{
	...
	if(machine_is_m805_892x())
	{
		if(system_rev == 0x2002 || system_rev == 0x2003 || system_rev == 0x2004 || system_rev == 0x2005 || system_rev == 0x2006 || system_rev == 0x2007) {
			...
			#if defined(CONFIG_CHIP_TCC8923)
	...
	{
		gpio_set_value(TCC_GPE(14), 0); // pad name gpio_e(18)
	}
#endif
...
int tcc_hp_is_valid(void)
{
	...
	if(machine_is_m805_893x())
	{
		return gpio_get_value(TCC_GPE(12)); // pad name gpio_e(16)
	}
#endif
...
static int __init tcc_init_wm8988(void)
{
    ...
    if( !(machine_is_m57te() || machine_is_m801() || machine_is_m801_88() || machine_is_m803() || machine_is_m805_892x() || machine_is_m805_893x()) ) {
        alsa_dbg("\n\n\n\n%s() do not execution....\n\n", __func__);
        return 0;
    }
	...
	if(machine_is_m805_893x())
	{
		tcc_gpio_config(TCC_GPE(14), GPIO_FN(0)); // pad name gpio_e(18)
		gpio_request(TCC_GPE(14), "SPK_MUTE_CTL");
		gpio_direction_output(TCC_GPE(14), 0);	 // Speaker mute
			
		tcc_gpio_config(TCC_GPE(13), GPIO_FN(0)); // pad name gpio_e(17)
		gpio_request(TCC_GPE(13), "HP_MUTE_CTL");
		gpio_direction_output(TCC_GPE(13), 1);	 // HeadPhone mute
