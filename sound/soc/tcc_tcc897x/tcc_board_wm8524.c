...
static struct snd_soc_dai_link tcc_dai[] = {
	...
	},
	{
		.name = "TCC-SPDIF-CH1",
		.stream_name = "IEC958",
		.codec_dai_name = "IEC958",//temporary
...
static int tcc_audio_probe(struct platform_device *pdev)
{
	...
	int ret, codec_aif, mute_ang,mute_ctl, i;
	...
	for (i=0 ; i<card->num_links ; i++) {
		...
		if(!strcmp(tcc_dai[i].codec_of_node->name,"dummy")){
			printk("%s (%d) dummy codec is used. change init for dummy",__func__,i);
			tcc_dai[i].codec_dai_name = "IEC958";
			tcc_dai[i].init = tcc_iec958_dummy_init;
		}
