...
static void tcc_cdif_shutdown(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	...
	struct tcc_runtime_data *prtd = dev_get_drvdata(dai->dev);
	void __iomem *pdai_reg = prtd->ptcc_reg->dai_reg;
	unsigned int reg_value=0, bEn=0;
...
static int tcc_cdif_resume(struct device *dev)
{
	...
	if(prtd->dai_port)    
		...
		tca_cdif_port_mux(prtd->id, prtd->dai_port);
#elif defined(CONFIG_ARCH_TCC802X)
	tca_cdif_port_mux(padma_reg, prtd->dai_port);
#endif
#endif	
...
#if defined(CONFIG_ARCH_TCC802X)
	struct audio_i2s_port *pcdif_port;
#endif
	u32 clk_rate;
	int ret = 0;
...
#elif defined(CONFIG_ARCH_TCC802X)
	pcdif_port = (struct audio_i2s_port *)kmalloc(sizeof(struct audio_i2s_port), GFP_KERNEL);
	if(pcdif_port == NULL) {
		alsa_dai_dbg(prtd->id, "pcdif_port kmalloc has some error.\n");
		ret= -1;
		goto err_reg_component;
	}
	prtd->dai_port = pcdif_port;
	memset(pcdif_port, 255, sizeof(struct audio_i2s_port));
	of_property_read_u8_array(pdev->dev.of_node, "clk-mux", pcdif_port->clk, 
			of_property_count_elems_of_size(pdev->dev.of_node, "clk-mux", sizeof(char)));
	of_property_read_u8_array(pdev->dev.of_node, "dain-mux", pcdif_port->dain, 
