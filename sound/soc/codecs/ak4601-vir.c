...
#define dprintk(msg...)    pr_info(LOG_TAG  msg)
#else
#define dprintk(msg...)    do {} while (0)
#endif

#define eprintk(msg...)    pr_err(KERN_ERR LOG_TAG  msg)
...
struct ak4601_priv {
	...
	//adc1,2 - L/R, adcm
	unsigned int adc_input_voltage_setting[ADC_COUNT][STEREO_COUNT];
...
// TODO :  We should consider a7 send the init and changed
// values to a53 through mbox
static int ak4601_vir_mixer_init(struct ak4601_priv *ak4601)
{
	...
	ak4601->mic_in_vol[LEFT_CHANNEL] = ak4601->mic_in_vol[RIGHT_CHANNEL] =
	    0;

	ak4601->adc_vol[ADC1_INDEX][LEFT_CHANNEL] = 255;
	ak4601->adc_vol[ADC1_INDEX][RIGHT_CHANNEL] = 0;
	ak4601->adc_vol[ADC2_INDEX][LEFT_CHANNEL] =
	    ak4601->adc_vol[ADC2_INDEX][RIGHT_CHANNEL] = 207;
	//adcm-right is dummy
	ak4601->adc_vol[ADCM_INDEX][LEFT_CHANNEL] =
		ak4601->adc_vol[ADCM_INDEX][RIGHT_CHANNEL] = 207;
	ak4601->dac_vol[DAC1_INDEX][LEFT_CHANNEL] =
	    ak4601->dac_vol[DAC1_INDEX][RIGHT_CHANNEL] = 231;
	ak4601->dac_vol[DAC2_INDEX][LEFT_CHANNEL] =
	    ak4601->dac_vol[DAC2_INDEX][RIGHT_CHANNEL] = 231;
	ak4601->dac_vol[DAC3_INDEX][LEFT_CHANNEL] =
	    ak4601->dac_vol[DAC3_INDEX][RIGHT_CHANNEL] = 231;

	for (i = 0; i < ADC_COUNT; i++) {
		ak4601->adc_mute[i] = AK4601_VIRTUAL_OFF;
		ak4601->adc_input_voltage_setting[i][LEFT_CHANNEL] =
		    AK4601_VIRTUAL_2_3_Vpp;
		ak4601->adc_input_voltage_setting[i][RIGHT_CHANNEL] =
		    AK4601_VIRTUAL_2_3_Vpp;
	}
...
static char const *ak4601_vir_adc_input_vol_texts[] = {
	"2.3Vpp/plus or minus2.3Vpp",
	"2.83Vpp/plus or minus2.83Vpp",
};

static const struct soc_enum ak4601_vir_adc_input_vol_enum[] = {
	AK4601_VIR_SOC_ENUM_SINGLE_EXT(
		ARRAY_SIZE(ak4601_vir_adc_input_vol_texts),
		ak4601_vir_adc_input_vol_texts, 0x00),	//ADC1 L
	AK4601_VIR_SOC_ENUM_SINGLE_EXT(
		ARRAY_SIZE(ak4601_vir_adc_input_vol_texts),
		ak4601_vir_adc_input_vol_texts, 0x01),	//ADC1 R
	AK4601_VIR_SOC_ENUM_SINGLE_EXT(
		ARRAY_SIZE(ak4601_vir_adc_input_vol_texts),
		ak4601_vir_adc_input_vol_texts, 0x10),	//ADC2 L
	AK4601_VIR_SOC_ENUM_SINGLE_EXT(
		ARRAY_SIZE(ak4601_vir_adc_input_vol_texts),
		ak4601_vir_adc_input_vol_texts, 0x11),	//ADC2 R
	AK4601_VIR_SOC_ENUM_SINGLE_EXT(
		ARRAY_SIZE(ak4601_vir_adc_input_vol_texts),
		ak4601_vir_adc_input_vol_texts, 0x20),	//ADCM
};

static char const *ak4601_vir_source_select_texts[] = {
	"ALL0",
...
//temp
static unsigned int const ak4601_vir_source_select_values[] = {
	0x00,
	0x01, 0x02, 0x03, 0x04,
	0x05, 0x06, 0x07, 0x08,
...
SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(ak4601_vir_source_select_texts),
		    ak4601_vir_source_select_texts);

static const struct soc_enum ak4601_vir_mixerb_ch1_source_selector_enum =
SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(ak4601_vir_source_select_texts),
		    ak4601_vir_source_select_texts);

static char const *ak4601_vir_adc2_select_texts[] = {
	"AIN2", "AIN3", "AIN4", "AIN5" };
static const struct soc_enum ak4601_vir_adc2_mux_enum =
SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(ak4601_vir_adc2_select_texts),
		    ak4601_vir_adc2_select_texts);
				  ...
				  struct snd_ctl_elem_info *uinfo)
{
	...
	//dprintk("%s : pv = 0x%8x, max = 0x%08x,
	//	val = 0x%08x\n", __func__, pv, max, value);
...
static int set_codec_value_to_mbox(struct mbox_audio_device *mbox_audio_dev,
...
static int get_codec_value_from_mbox(struct mbox_audio_device *mbox_audio_dev,
				     unsigned int param, unsigned int *value)
{
	struct mbox_audio_data_header_t *header;
...
static int get_mic_input_volume(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	...
	param =
	    (value ==
	     0) ? (unsigned int)AK4601_VIRTUAL_001_MIC_INPUT_VOLUME_L
	    : (unsigned int)AK4601_VIRTUAL_002_MIC_INPUT_VOLUME_R;
...
static int set_mic_input_volume(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct ak4601_priv *ak4601 = snd_soc_codec_get_drvdata(codec);
	...
	set_codec_value_to_mbox(ak4601->mbox_audio_dev, param,
				(unsigned int)ak4601->mic_in_vol[value]);

	return 0;
}

static int get_adc_volume(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct ak4601_priv *ak4601 = snd_soc_codec_get_drvdata(codec);
	...
	if (adc_val < ADC_COUNT && lr_val < STEREO_COUNT) {
		ucontrol->value.integer.value[0] =
		    ak4601->adc_vol[adc_val][lr_val];
	} else {
		eprintk
		    ("%s : private_value error : adc_val = %d, lr_val = %d\n",
		     __func__, adc_val, lr_val);
		return -EINVAL;
	}
#else
	unsigned int param = 0;
...
static int set_adc_volume(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct ak4601_priv *ak4601 = snd_soc_codec_get_drvdata(codec);
	...
	set_codec_value_to_mbox(ak4601->mbox_audio_dev, param,
				(unsigned int)ak4601->adc_vol[adc_val][lr_val]);
...
static int get_dac_volume(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct ak4601_priv *ak4601 = snd_soc_codec_get_drvdata(codec);
...
static int set_dac_volume(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct ak4601_priv *ak4601 = snd_soc_codec_get_drvdata(codec);
	...
	set_codec_value_to_mbox(ak4601->mbox_audio_dev, param,
				(unsigned int)ak4601->dac_vol[dac_val][lr_val]);
...
static int get_adc_mute(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct ak4601_priv *ak4601 = snd_soc_codec_get_drvdata(codec);
	...
	if (kcontrol->private_value < ADC_COUNT) {
		ucontrol->value.integer.value[0] =
		    ak4601->adc_mute[kcontrol->private_value];
	} else {
...
static int set_adc_mute(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct ak4601_priv *ak4601 = snd_soc_codec_get_drvdata(codec);
	...
	set_codec_value_to_mbox(ak4601->mbox_audio_dev, param,
		(unsigned int)ak4601->adc_mute[kcontrol->private_value]);
...
static int get_dac_mute(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct ak4601_priv *ak4601 = snd_soc_codec_get_drvdata(codec);
	...
	if (kcontrol->private_value < DAC_COUNT) {
		ucontrol->value.integer.value[0] =
		    ak4601->dac_mute[kcontrol->private_value];
	} else {
		eprintk("%s : error- private_value = %lu\n", __func__,
			kcontrol->private_value);
		return -EINVAL;
...
static int set_dac_mute(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct ak4601_priv *ak4601 = snd_soc_codec_get_drvdata(codec);
	...
	set_codec_value_to_mbox(ak4601->mbox_audio_dev, param,
		(unsigned int)ak4601->dac_mute[kcontrol->private_value]);
...
static int get_sdout_enable(struct snd_kcontrol *kcontrol,
			    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct ak4601_priv *ak4601 = snd_soc_codec_get_drvdata(codec);
	...
	if (kcontrol->private_value < SDOUT_COUNT) {
		ucontrol->value.integer.value[0] =
		    ak4601->sdout_enable[kcontrol->private_value];
	} else {
		eprintk("%s : error- private_value = %lu\n", __func__,
			kcontrol->private_value);
		return -EINVAL;
...
static int set_sdout_enable(struct snd_kcontrol *kcontrol,
			    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct ak4601_priv *ak4601 = snd_soc_codec_get_drvdata(codec);
	...
	set_codec_value_to_mbox(ak4601->mbox_audio_dev, param,
		(unsigned int)ak4601->sdout_enable[kcontrol->private_value]);
...
static int get_adc_input_voltage_setting(struct snd_kcontrol *kcontrol,
					 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct ak4601_priv *ak4601 = snd_soc_codec_get_drvdata(codec);
	...
	if (adc_val < ADC_COUNT && lr_val < STEREO_COUNT) {
		ucontrol->value.enumerated.item[0] =
		    ak4601->adc_input_voltage_setting[adc_val][lr_val];
	} else {
		eprintk
		    ("%s : private_value error : adc_val = %d, lr_val = %d\n",
		     __func__, adc_val, lr_val);
	...
	case ADCM_INDEX:
		param =
		    (unsigned int)AK4601_VIRTUAL_024_ADCM_INPUT_VOLTAGE_SETTING;
		break;
	}
	...
	dprintk("%s : get val = %u, adc_val = %u, lr_val = %u, done\n",
		__func__, ucontrol->value.enumerated.item[0], adc_val,
		lr_val);
	return 0;
}

static int set_adc_input_voltage_setting(struct snd_kcontrol *kcontrol,
					 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct ak4601_priv *ak4601 = snd_soc_codec_get_drvdata(codec);
	...
	set_codec_value_to_mbox(ak4601->mbox_audio_dev, param,
				(unsigned int)
				ak4601->adc_input_voltage_setting[adc_val]
				[lr_val]);

	dprintk("%s : set val = %u, adc_val = %u, lr_val = %u, done\n",
		__func__,
		ak4601->adc_input_voltage_setting[adc_val][lr_val], adc_val,
		lr_val);
	return 0;
}

static int get_sdout1_source_selector(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct ak4601_priv *ak4601 = snd_soc_codec_get_drvdata(codec);
	...
	get_codec_value_from_mbox(ak4601->mbox_audio_dev, (unsigned int)
...
static int set_sdout1_source_selector(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct ak4601_priv *ak4601 = snd_soc_codec_get_drvdata(codec);
	...
	set_codec_value_to_mbox(ak4601->mbox_audio_dev, (unsigned int)
				AK4601_VIRTUAL_028_SDOUT1_SOURCE_SELECTOR,
				(unsigned int)ak4601->sdout1_source_selector);

	dprintk("%s : value = %u, done\n", __func__,
		ak4601->sdout1_source_selector);
	return 0;

}

static int get_mixerb_ch1_source_selector(struct snd_kcontrol *kcontrol,
					  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct ak4601_priv *ak4601 = snd_soc_codec_get_drvdata(codec);
...
static int set_mixerb_ch1_source_selector(struct snd_kcontrol *kcontrol,
					  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct ak4601_priv *ak4601 = snd_soc_codec_get_drvdata(codec);
	...
	set_codec_value_to_mbox(ak4601->mbox_audio_dev, (unsigned int)
				AK4601_VIRTUAL_029_MIXER_B_CH_1_SOURCE_SELECTOR,
				(unsigned int)
				ak4601->mixerb_ch1_source_selector);
...
static int get_adc2_mux(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct ak4601_priv *ak4601 = snd_soc_codec_get_drvdata(codec);
	...
	get_codec_value_from_mbox(ak4601->mbox_audio_dev,
				  (unsigned int)AK4601_VIRTUAL_030_ADC2_MUX,
				  &mbox_value);
	ucontrol->value.enumerated.item[0] = mbox_value;
#endif
...
static int set_adc2_mux(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct ak4601_priv *ak4601 = snd_soc_codec_get_drvdata(codec);
	...
	set_codec_value_to_mbox(ak4601->mbox_audio_dev,
				(unsigned int)AK4601_VIRTUAL_030_ADC2_MUX,
				(unsigned int)ak4601->adc2_mux);

	dprintk("%s : value = %u, done\n", __func__, ak4601->adc2_mux);
...
static const struct snd_kcontrol_new ak4601_vir_snd_controls[] = {
	...
	AK4601_VIR_SINGLE_EXT("MIC Input Volume L", 0x00, 0x0F,
			      get_mic_input_volume, set_mic_input_volume),
	AK4601_VIR_SINGLE_EXT("MIC Input Volume R", 0x01, 0x0F,
			      get_mic_input_volume, set_mic_input_volume),

	AK4601_VIR_SINGLE_EXT("ADC1 Digital Volume L", 0x00, 0xFF,
			      get_adc_volume, set_adc_volume),
	AK4601_VIR_SINGLE_EXT("ADC1 Digital Volume R", 0x01, 0xFF,
			      get_adc_volume, set_adc_volume),
	AK4601_VIR_SINGLE_EXT("ADC2 Digital Volume L", 0x10, 0xFF,
			      get_adc_volume, set_adc_volume),
	AK4601_VIR_SINGLE_EXT("ADC2 Digital Volume R", 0x11, 0xFF,
			      get_adc_volume, set_adc_volume),
	AK4601_VIR_SINGLE_EXT("ADCM Digital Volume", 0x20, 0xFF, get_adc_volume,
			      set_adc_volume),

	AK4601_VIR_SINGLE_EXT("DAC1 Digital Volume L", 0x00, 0xFF,
			      get_dac_volume, set_dac_volume),
	AK4601_VIR_SINGLE_EXT("DAC1 Digital Volume R", 0x01, 0xFF,
			      get_dac_volume, set_dac_volume),
	AK4601_VIR_SINGLE_EXT("DAC2 Digital Volume L", 0x10, 0xFF,
			      get_dac_volume, set_dac_volume),
	AK4601_VIR_SINGLE_EXT("DAC2 Digital Volume R", 0x11, 0xFF,
			      get_dac_volume, set_dac_volume),
	AK4601_VIR_SINGLE_EXT("DAC3 Digital Volume L", 0x20, 0xFF,
			      get_dac_volume, set_dac_volume),
	AK4601_VIR_SINGLE_EXT("DAC3 Digital Volume R", 0x21, 0xFF,
			      get_dac_volume, set_dac_volume),

	SOC_SINGLE_BOOL_EXT("ADC1 Mute", ADC1_INDEX, get_adc_mute,
			    set_adc_mute),
	SOC_SINGLE_BOOL_EXT("ADC2 Mute", ADC2_INDEX, get_adc_mute,
			    set_adc_mute),
	SOC_SINGLE_BOOL_EXT("ADCM Mute", ADCM_INDEX, get_adc_mute,
			    set_adc_mute),

	SOC_SINGLE_BOOL_EXT("DAC1 Mute", DAC1_INDEX, get_dac_mute,
			    set_dac_mute),
	SOC_SINGLE_BOOL_EXT("DAC2 Mute", DAC2_INDEX, get_dac_mute,
			    set_dac_mute),
	SOC_SINGLE_BOOL_EXT("DAC3 Mute", DAC3_INDEX, get_dac_mute,
			    set_dac_mute),

	SOC_ENUM_EXT("ADC1 Lch Input Voltage Setting",
		     ak4601_vir_adc_input_vol_enum[0],
		     get_adc_input_voltage_setting,
		     set_adc_input_voltage_setting),
	SOC_ENUM_EXT("ADC1 Rch Input Voltage Setting",
		     ak4601_vir_adc_input_vol_enum[1],
		     get_adc_input_voltage_setting,
		     set_adc_input_voltage_setting),
	SOC_ENUM_EXT("ADC2 Lch Input Voltage Setting",
		     ak4601_vir_adc_input_vol_enum[2],
		     get_adc_input_voltage_setting,
		     set_adc_input_voltage_setting),
	SOC_ENUM_EXT("ADC2 Rch Input Voltage Setting",
		     ak4601_vir_adc_input_vol_enum[3],
		     get_adc_input_voltage_setting,
		     set_adc_input_voltage_setting),
	SOC_ENUM_EXT("ADCM Input Voltage Setting",
		     ak4601_vir_adc_input_vol_enum[4],
		     get_adc_input_voltage_setting,
		     set_adc_input_voltage_setting),

	SOC_SINGLE_BOOL_EXT("SDOUT1 Output Enable", SDOUT1_INDEX,
			    get_sdout_enable, set_sdout_enable),
	SOC_SINGLE_BOOL_EXT("SDOUT2 Output Enable", SDOUT2_INDEX,
			    get_sdout_enable, set_sdout_enable),
	SOC_SINGLE_BOOL_EXT("SDOUT3 Output Enable", SDOUT3_INDEX,
			    get_sdout_enable, set_sdout_enable),

	//a7 process belows with dapm
	SOC_ENUM_EXT("SDOUT1 Source Selector",
		     ak4601_vir_sdout1_source_selector_enum,
		     get_sdout1_source_selector, set_sdout1_source_selector),
	SOC_ENUM_EXT("MixerB Ch1 Source Selector",
		     ak4601_vir_mixerb_ch1_source_selector_enum,
		     get_mixerb_ch1_source_selector,
		     set_mixerb_ch1_source_selector),
	SOC_ENUM_EXT("ADC2 MUX", ak4601_vir_adc2_mux_enum, get_adc2_mux,
		     set_adc2_mux),
};
...
#define AK4601_DAC_FORMATS \
	(SNDRV_PCM_FMTBIT_S16_LE|SNDRV_PCM_FMTBIT_S24_LE| \
	SNDRV_PCM_FMTBIT_S32_LE)
#define AK4601_ADC_FORMATS \
	(SNDRV_PCM_FMTBIT_S16_LE|SNDRV_PCM_FMTBIT_S24_LE| \
	SNDRV_PCM_FMTBIT_S32_LE)

int ak4601_vir_set_tdm_slot(struct snd_soc_dai *dai, unsigned int tx_mask,
...
struct snd_soc_codec_driver soc_codec_dev_ak4601_virtual = {
	...
	.component_driver = {
			     ...
			     .num_controls =
			     ARRAY_SIZE(ak4601_vir_snd_controls),
			     },
};
...
static int ak4601_vir_codec_probe(struct platform_device *pdev)
{
	...
	ak4601 =
	    devm_kzalloc(&pdev->dev, sizeof(struct ak4601_priv), GFP_KERNEL);
	...
	dprintk("[%s] global_audio_dev dev_name:%s, mbox_name:%s, dev_num:%d\n",
		__func__, ak4601->mbox_audio_dev->dev_name,
		ak4601->mbox_audio_dev->mbox_name,
		ak4601->mbox_audio_dev->dev_num);

	//in here, pdev->dev is set to codec->component->dev
	ret = snd_soc_register_codec(&pdev->dev,
				     &soc_codec_dev_ak4601_virtual,
				     &ak4601_virtual_dai, 1);
	if (ret < 0) {
	...
	dprintk("%s : snd_soc_resister_codec Successfull!!!!, mbox = %p\n",
		__func__, ak4601->mbox_audio_dev);
	return ret;
}
