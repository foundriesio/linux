...
#if 0
#define alsa_dbg(f, a...)  pr_info("== alsa-debug == Dummy Dac " f, ##a)
#else
#define alsa_dbg(f, a...)
#endif
...
static void dummy_shutdown(struct snd_pcm_substream *substream,
			   struct snd_soc_dai *cpu_dai)
{
}
...
static int dummy_set_dai_sysclk(struct snd_soc_dai *codec_dai, int clk_id,
				unsigned int freq, int dir)
{
	return 0;
}

static int dummy_set_dai_fmt(struct snd_soc_dai *codec_dai, unsigned int fmt)
{
	return 0;
}
...
static int tcc_dummy_codec_probe(struct platform_device *pdev)
{
	...
	alsa_dbg("%s\n", __func__);
	ret =
	    snd_soc_register_codec(&pdev->dev, &soc_codec_dev_tcc_dummy,
