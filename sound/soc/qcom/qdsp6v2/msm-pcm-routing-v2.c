...
int msm_pcm_routing_reg_phy_stream(int fedai_id, int perf_mode, int dspst_id, int stream_type)
{
	...
	pr_err("DEBUG::: %s %d %d %d %d\n", __func__, fedai_id, perf_mode, dspst_id, stream_type);
	//pr_err("port id %d path_type %d sample rate %d channels %d bits per sample %d \n", session->port_id, session->path_type, session->sample_rate, session->channels, session->bits_per_sample, 0, 0);
...
//	pr_err("%s: setting idx bit of fe:%d, type: %d, be:%d COPP idx %d\n", __func__, fedai_id, session_type, i , copp_idx);
...
struct msm_routing_session_data *msm_pcm_routing_get_session_data(struct msm_routing_data *data, int port_id, int port_type)
{
	...
	for (i = 0; i < MAX_SESSIONS; i++) {
		pr_err("Checking session %s: %d \n", __func__, i);
		if (port_id == data->sessions[i].port_id) {
			pr_err("Matching .... %d port id with %d \n", port_id, data->sessions[i].port_id);
...
struct msm_routing_session_data *get_session_from_fedai_id(struct msm_routing_data *data, int fedai_id)
{
	...
	for (i = 0; i < MAX_SESSIONS; i++) {
		...
		if (fedai_id == data->sessions[i].fedai_id) {
			pr_err("Matching .... %d port id with %d \n", fedai_id, data->sessions[i].fedai_id);
			return &data->sessions[i];
		}
	}
...
//		session_type = SESSION_TYPE_RX;
	...
	pr_err("%s: port id %d session  BE Sample Rate (%d) format (%d) be_id %d\n",
