...
#define vir_i2s_dbg(f, a...)\
	pr_info("[DEBUG][VIR_I2S] " f, ##a)
#else
#define vir_i2s_dbg(f, a...)
#endif
#define vir_i2s_err(f, a...)\
	pr_err("[ERROR][VIR_I2S] " f, ##a)
...
static int tcc_vir_i2s_set_dai_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	struct tcc_vir_i2s_t *vi2s =
	    (struct tcc_vir_i2s_t *)snd_soc_dai_get_drvdata(dai);
	int ret = 0;
	...
	default:
		vir_i2s_err("[%s][%d][dev-%d] format does not supported\n",
...
static int tcc_vir_i2s_startup(
...
static void tcc_vir_i2s_shutdown(
	struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	vir_i2s_dbg("%s\n", __func__);
...
static int tcc_vir_i2s_hw_params(
	struct snd_pcm_substream *substream,
...
static int tcc_vir_i2s_hw_free(
	struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
	vir_i2s_dbg("%s\n", __func__);
...
static int tcc_vir_i2s_trigger(
...
const struct snd_soc_component_driver tcc_vir_i2s_component_drv = {
	.name = "tcc-i2s",
};
	...
	struct tcc_vir_i2s_t *vi2s)
{
	...
	of_property_read_u32(
		pdev->dev.of_node,
		"playback-channel-max",
		&play_ch_max);
	vir_i2s_dbg("vi2s dev-%d playback-channel-max : %u\n",
		vi2s->dev_id,
		play_ch_max);
	of_property_read_u32(pdev->dev.of_node, "capture-channel-max",
		     &cap_ch_max);
	...
	if (vi2s->pi2s_dai_drv != NULL) {
		...
		vi2s->pi2s_dai_drv->playback.stream_name =
		    tcc_vir_i2s_dai_drv.playback.stream_name;
		vi2s->pi2s_dai_drv->playback.channels_min =
		    tcc_vir_i2s_dai_drv.playback.channels_min;
		if (play_ch_max != 0) {
			vi2s->pi2s_dai_drv->playback.channels_max = play_ch_max;
		} else {
			vi2s->pi2s_dai_drv->playback.channels_max =
			    tcc_vir_i2s_dai_drv.playback.channels_max;
		}
		vi2s->pi2s_dai_drv->playback.rates =
		    tcc_vir_i2s_dai_drv.playback.rates;
		vi2s->pi2s_dai_drv->playback.formats =
		    tcc_vir_i2s_dai_drv.playback.formats;

		vi2s->pi2s_dai_drv->capture.stream_name =
		    tcc_vir_i2s_dai_drv.capture.stream_name;
		vi2s->pi2s_dai_drv->capture.channels_min =
		    tcc_vir_i2s_dai_drv.capture.channels_min;
		if (cap_ch_max != 0) {
			vi2s->pi2s_dai_drv->capture.channels_max = cap_ch_max;
		} else {
			vi2s->pi2s_dai_drv->capture.channels_max =
			    tcc_vir_i2s_dai_drv.capture.channels_max;
		}
		vi2s->pi2s_dai_drv->capture.rates =
		    tcc_vir_i2s_dai_drv.capture.rates;
		vi2s->pi2s_dai_drv->capture.formats =
		    tcc_vir_i2s_dai_drv.capture.formats;
	} else {
		vir_i2s_err("snd_soc_dai_driver setting failed\n");
	}
}
...
static int tcc_vir_i2s_probe(struct platform_device *pdev)
{
	...
	vi2s = (struct tcc_vir_i2s_t *)devm_kzalloc(&pdev->dev,
		sizeof(struct tcc_vir_i2s_t), GFP_KERNEL);

	if (vi2s == NULL)
		return -ENOMEM;

	ptcc_vir_i2s_dai_drv =
		(struct snd_soc_dai_driver *)devm_kzalloc(&pdev->dev,
			sizeof(struct snd_soc_dai_driver), GFP_KERNEL);

	if (ptcc_vir_i2s_dai_drv == NULL)
		return -ENOMEM;
...
//	if ((ret = devm_snd_soc_register_component(&pdev->dev,
//		&tcc_vir_i2s_component_drv, &tcc_vir_i2s_dai_drv[
//		vi2s->direction_type], 1)) < 0) {

	ret = devm_snd_soc_register_component(&pdev->dev,
		&tcc_vir_i2s_component_drv, vi2s->pi2s_dai_drv, 1);

	if (ret < 0) {
		vir_i2s_err("devm_snd_soc_register_component failed\n");
		goto error;
	}
...
static const struct of_device_id tcc_vir_i2s_of_match[] = {
