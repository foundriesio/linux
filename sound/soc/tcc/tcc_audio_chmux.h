	...
	uint32_t port)
{
	...
	if (port <= IOBUSCFG_DAI_PORT_MAX) {
		chmux_writel(
			VAL2FLD(IOBUS_CFG_CHMUX_SEL, (1 << port)),
			iobuscfg_reg + offset);
	}
}

static inline void iobuscfg_cdif_chmux(
	void __iomem *iobuscfg_reg,
	int32_t id,
	uint32_t port)
{
	ptrdiff_t offset =
	    (id == 0) ? IOBUS_CFG_CDIF0_CHMUX : IOBUS_CFG_CDIF1_CHMUX;
				      ...
				      uint32_t port)
{
	...
	uint32_t pos =
	    (id == 0) ?
		(uint32_t) IOBUS_CFG_DAI_CHMUX_71CH_0_SEL_Pos : (id == 1) ?
		(uint32_t) IOBUS_CFG_DAI_CHMUX_71CH_1_SEL_Pos : (id == 2) ?
		(uint32_t) IOBUS_CFG_DAI_CHMUX_71CH_2_SEL_Pos : (id == 3) ?
		(uint32_t) IOBUS_CFG_DAI_CHMUX_SRCH_0_SEL_Pos : (id == 4) ?
		(uint32_t) IOBUS_CFG_DAI_CHMUX_SRCH_1_SEL_Pos : (id == 5) ?
		(uint32_t) IOBUS_CFG_DAI_CHMUX_SRCH_2_SEL_Pos :
		(uint32_t) IOBUS_CFG_DAI_CHMUX_SRCH_3_SEL_Pos;

	uint32_t mask =
		(id == 0) ? IOBUS_CFG_DAI_CHMUX_71CH_0_SEL_Msk :
		...
		IOBUS_CFG_DAI_CHMUX_SRCH_3_SEL_Msk;
	...
	uint32_t port)
{
	...
	uint32_t pos =
		(id == 0) ? (uint32_t)IOBUS_CFG_DAI_CHMUX_71CH_0_SEL_Pos :
		...
		(id == 6) ? (uint32_t)IOBUS_CFG_DAI_CHMUX_SRCH_3_SEL_Pos :
		(uint32_t)IOBUS_CFG_DAI_CHMUX_71CH_3_SEL_Pos;
	...
	uint32_t mask =
		...
		(id == 4) ? IOBUS_CFG_DAI_CHMUX_SRCH_1_SEL_Msk :
		(id == 5) ? IOBUS_CFG_DAI_CHMUX_SRCH_2_SEL_Msk :
		(id == 6) ? IOBUS_CFG_DAI_CHMUX_SRCH_3_SEL_Msk :
		IOBUS_CFG_DAI_CHMUX_71CH_3_SEL_Msk;
...
static inline void iobuscfg_cdif_chmux(
	void __iomem *iobuscfg_reg, int32_t id,
	uint32_t port)
{
	...
	uint32_t port)
{
	...
	uint32_t pos =
	    (id == 0) ? (uint32_t) IOBUS_CFG_SPDIF_CHMUX_71CH0_0_SEL_Pos :
		...
		(id == 5) ? (uint32_t) IOBUS_CFG_SPDIF_CHMUX_SRCH1_2_SEL_Pos :
		(uint32_t) IOBUS_CFG_SPDIF_CHMUX_SRCH1_3_SEL_Pos;

	uint32_t mask =
		(id == 0) ? IOBUS_CFG_SPDIF_CHMUX_71CH0_0_SEL_Msk :
		...
		(id == 5) ? IOBUS_CFG_SPDIF_CHMUX_SRCH1_2_SEL_Msk :
		IOBUS_CFG_SPDIF_CHMUX_SRCH1_3_SEL_Msk;
	...
	struct tcc_gfb_spdif_port *port)
{
	...
	pcfg3 |=
		VAL2FLD(PCFG3_SPDIF_TX, port->port[0]) |
	    VAL2FLD(PCFG3_SPDIF_RX, port->port[1]);

	chmux_writel(pcfg3, pcfg_reg + TCC_AUDIO_PCFG3_OFFSET);
}

static inline void tcc_gfb_cdif_portcfg(
	void __iomem *pcfg_reg,
	struct tcc_gfb_cdif_port *port)
{
	uint32_t pcfg2 = readl(pcfg_reg + TCC_AUDIO_PCFG2_OFFSET);
	uint32_t pcfg3 = readl(pcfg_reg + TCC_AUDIO_PCFG3_OFFSET);
	...
	pcfg3 |=
		VAL2FLD(PCFG3_CD_BCLK, port->port[0]) |
