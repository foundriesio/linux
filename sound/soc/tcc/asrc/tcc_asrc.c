...
uint32_t get_fifo_in_phys_addr(uint32_t asrc_reg_phys, int asrc_ch)
{
...
uint32_t get_fifo_out_phys_addr(uint32_t asrc_reg_phys, int asrc_ch)
{
...
void tcc_asrc_reg_backup(void __iomem *asrc_reg, struct asrc_reg_t *regs)
{
	regs->enable           = readl(asrc_reg + TCC_ASRC_ENABLE_OFFSET);
	regs->inport_ctrl      = readl(asrc_reg + TCC_ASRC_INPORT_CTRL_OFFSET);
	regs->outport_ctrl     = readl(asrc_reg + TCC_ASRC_OUTPORT_CTRL_OFFSET);
	regs->src_init_zero_sz = readl(asrc_reg + TCC_ASRC_SRC_INIT_ZERO_SZ_OFFSET);
	regs->src_opt_buf_lvl  = readl(asrc_reg + TCC_ASRC_SRC_OPT_BUF_LVL_OFFSET);
	regs->ext_io_fmt       = readl(asrc_reg + TCC_ASRC_EXT_IO_FMT_OFFSET);

	regs->period_sync_cnt0 = readl(asrc_reg + TCC_ASRC_PERIOD_SYNC_CNT0_OFFSET);
	regs->period_sync_cnt1 = readl(asrc_reg + TCC_ASRC_PERIOD_SYNC_CNT1_OFFSET);
	regs->period_sync_cnt2 = readl(asrc_reg + TCC_ASRC_PERIOD_SYNC_CNT2_OFFSET);
	regs->period_sync_cnt3 = readl(asrc_reg + TCC_ASRC_PERIOD_SYNC_CNT3_OFFSET);

	regs->rate_config0     = readl(asrc_reg + TCC_ASRC_SRC_RATE0_OFFSET);
	regs->rate_config1     = readl(asrc_reg + TCC_ASRC_SRC_RATE1_OFFSET);
	regs->rate_config2     = readl(asrc_reg + TCC_ASRC_SRC_RATE2_OFFSET);
	regs->rate_config3     = readl(asrc_reg + TCC_ASRC_SRC_RATE3_OFFSET);

	regs->mix_in_en        = readl(asrc_reg + TCC_ASRC_MIX_IN_EN_OFFSET);
	regs->samp_timing	   = readl(asrc_reg + TCC_ASRC_SAMP_TIMING_OFFSET);

	regs->vol_gain0        = readl(asrc_reg + TCC_ASRC_VOL_GAIN0_OFFSET);
	regs->vol_gain1        = readl(asrc_reg + TCC_ASRC_VOL_GAIN1_OFFSET);
	regs->vol_gain2        = readl(asrc_reg + TCC_ASRC_VOL_GAIN2_OFFSET);
	regs->vol_gain3        = readl(asrc_reg + TCC_ASRC_VOL_GAIN3_OFFSET);

	regs->vol_ramp_gain0   = readl(asrc_reg + TCC_ASRC_VOL_RAMP_GAIN0_OFFSET);
	regs->vol_ramp_gain1   = readl(asrc_reg + TCC_ASRC_VOL_RAMP_GAIN1_OFFSET);
	regs->vol_ramp_gain2   = readl(asrc_reg + TCC_ASRC_VOL_RAMP_GAIN2_OFFSET);
	regs->vol_ramp_gain3   = readl(asrc_reg + TCC_ASRC_VOL_RAMP_GAIN3_OFFSET);

	regs->vol_ramp_dn_cfg0 = readl(asrc_reg + TCC_ASRC_VOL_RAMP_UP_CFG0_OFFSET);
	regs->vol_ramp_dn_cfg1 = readl(asrc_reg + TCC_ASRC_VOL_RAMP_UP_CFG1_OFFSET);
	regs->vol_ramp_dn_cfg2 = readl(asrc_reg + TCC_ASRC_VOL_RAMP_UP_CFG2_OFFSET);
	regs->vol_ramp_dn_cfg3 = readl(asrc_reg + TCC_ASRC_VOL_RAMP_UP_CFG3_OFFSET);

	regs->vol_ramp_up_cfg0 = readl(asrc_reg + TCC_ASRC_VOL_RAMP_DN_CFG0_OFFSET);
	regs->vol_ramp_up_cfg1 = readl(asrc_reg + TCC_ASRC_VOL_RAMP_DN_CFG1_OFFSET);
	...
	regs->fifo_in_ctrl3    = readl(asrc_reg + TCC_ASRC_FIFO_IN3_CTRL_OFFSET);

	regs->fifo_out_ctrl0   = readl(asrc_reg + TCC_ASRC_FIFO_OUT0_CTRL_OFFSET);
	regs->fifo_out_ctrl1   = readl(asrc_reg + TCC_ASRC_FIFO_OUT1_CTRL_OFFSET);
	regs->fifo_out_ctrl2   = readl(asrc_reg + TCC_ASRC_FIFO_OUT2_CTRL_OFFSET);
	regs->fifo_out_ctrl3   = readl(asrc_reg + TCC_ASRC_FIFO_OUT3_CTRL_OFFSET);

	regs->fifo_misc_ctrl   = readl(asrc_reg + TCC_ASRC_FIFO_MISC_CTRL_OFFSET);
}

void tcc_asrc_reg_restore(void __iomem *asrc_reg, struct asrc_reg_t *regs)
{
	...
	writel(regs->src_init_zero_sz, asrc_reg + TCC_ASRC_SRC_INIT_ZERO_SZ_OFFSET);
	writel(regs->src_opt_buf_lvl , asrc_reg + TCC_ASRC_SRC_OPT_BUF_LVL_OFFSET);
	writel(regs->ext_io_fmt      , asrc_reg + TCC_ASRC_EXT_IO_FMT_OFFSET);

	writel(regs->period_sync_cnt0, asrc_reg + TCC_ASRC_PERIOD_SYNC_CNT0_OFFSET);
	writel(regs->period_sync_cnt1, asrc_reg + TCC_ASRC_PERIOD_SYNC_CNT1_OFFSET);
	writel(regs->period_sync_cnt2, asrc_reg + TCC_ASRC_PERIOD_SYNC_CNT2_OFFSET);
	writel(regs->period_sync_cnt3, asrc_reg + TCC_ASRC_PERIOD_SYNC_CNT3_OFFSET);
	...
	writel(regs->vol_ramp_dn_cfg0, asrc_reg + TCC_ASRC_VOL_RAMP_UP_CFG0_OFFSET);
	writel(regs->vol_ramp_dn_cfg1, asrc_reg + TCC_ASRC_VOL_RAMP_UP_CFG1_OFFSET);
	writel(regs->vol_ramp_dn_cfg2, asrc_reg + TCC_ASRC_VOL_RAMP_UP_CFG2_OFFSET);
	writel(regs->vol_ramp_dn_cfg3, asrc_reg + TCC_ASRC_VOL_RAMP_UP_CFG3_OFFSET);

	writel(regs->vol_ramp_up_cfg0, asrc_reg + TCC_ASRC_VOL_RAMP_DN_CFG0_OFFSET);
	writel(regs->vol_ramp_up_cfg1, asrc_reg + TCC_ASRC_VOL_RAMP_DN_CFG1_OFFSET);
