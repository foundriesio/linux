...
/*
 * Codec command
 * unsigned int cmd[0] = usage | cmd_type | tx instance num | msg_size (3) ;
 * 8bit | 8bit | 8bit | 8bit
 * unsigned int cmd[1] = integer mapped control command string
 * unsigned int cmd[2] = interger mapped value,
 * if using MBOX_AUDIO_USAGE_REQUEST, do not use it
...
#define CODEC_COMMAND_MAPPER(cvalue, cname, ctype) \
{ .value = cvalue, .name = cname, .type = ctype}
