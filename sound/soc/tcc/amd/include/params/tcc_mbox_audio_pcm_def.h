 ...
 * unsigned int cmd[0] =
 * usage | cmd_type | tx instance num | msg_size (5) ; 8bit | 8bit | 8bit | 8bit
 ...
 * unsigned int cmd[5] = PCM_VALUE_MUX_SOURCE_MAIN
 * (or PCM_VALUE_MUX_SOURCE_INTERNAL_0 or
 * PCM_VALUE_MUX_SOURCE_INTERNAL_1 for tuner, aux..)
 */
...
/*
 * example 3 : for position command
 * (A7S -> A53) for each playback or capture
 * unsigned int cmd[0] =
 * usage | cmd_type | tx instance num | msg_size (3) ; 8bit | 8bit | 8bit | 8bit
...
/*
 * example 4 : for position command
 * (A7S -> A53) for all playbacks concurrently (burst)
 * unsigned int cmd[0] =
 * usage | cmd_type | tx instance num | msg_size (6) ; 8bit | 8bit | 8bit | 8bit
 ...
 * unsigned int cmd[0] =
 * usage | cmd_type | tx instance num | msg_size (4) ; 8bit | 8bit | 8bit | 8bit
 * unsigned int cmd[1] = PCM_VOLUME
 * unsigned int cmd[2] = PCM_VALUE_DEVICE_0_1
 * unsigned int cmd[3] = PCM_VALUE_DIRECTION_TX
 * unsigned int cmd[4] = integer | fraction (not supported) | mode ;
 *                       16bit |  8bit | 8bit ;
 *                       "integer" value is short considering +,-
 *                       value to set , if using MBOX_AUDIO_USAGE_REQUEST,
 *                       do not use it. reply value is set here
 */

/*
 * example 6 : for balance/fade command
 * unsigned int cmd[0] = usage | cmd_type | tx instance num | msg_size (2) ;
 *                       8bit | 8bit | 8bit | 8bit
 * unsigned int cmd[1] = PCM_FADER
 * unsigned int cmd[2] = balance | fade ;
 *                       16bit | 16bit ; balance, fade is short
 *                       value to set , if using MBOX_AUDIO_USAGE_REQUEST,
 *                       do not use it. reply value is set here
 */
 ...
 * unsigned int cmd[0] = usage | cmd_type | tx instance num | msg_size (4) ;
 * 8bit | 8bit | 8bit | 8bit
 ...
 * unsigned int cmd[4] = PCM_VALUE_MUX_SOURCE_MAIN
 * (PCM_VALUE_MUX_SOURCE_INTERNAL_0 or PCM_VALUE_MUX_SOURCE_INTERNAL_1)
 *                    value to set , if using MBOX_AUDIO_USAGE_REQUEST,
 *					  do not use it. reply value is set here
 */
 ...
 * unsigned int cmd[2] = chime index to set type
 * (0 or 1 : CHIME_IDX_DOOR or CHIME_IDX_TRUNK
 ...
 * unsigned int cmd[2] = PCM_VALUE_DEVICE_0_1
 * unsigned int cmd[3] = PCM_VALUE_DIRECTION_TX
 * unsigned int cmd[4] = integer
 * (ramp time in ms. It is an approximation due to be calculated by integer).
...
enum pcm_command_id {
	PCM_ACTION = 0,
	// to send start, stop command with device inform (ex hw0:0)
	PCM_PARAMS,
	// to send overal param (device, tx or rx, buf address, buf size)
	PCM_BUFFER_ADDRESS,
	// to send buffer address only (with device inform)
	PCM_BUFFER_SIZE,
	// to send buffer size only (with device inform)
	PCM_POSITION_DATA,
	// to send position data during pcm playing (A7S -> A53)
	PCM_VOLUME,
	// to send tcc volume command at each pcm device
	PCM_FADER,
	// to send tcc fade/balance for mixed tx pcm
	PCM_MUX,
	// to send mux switching to playback
	PCM_CHIME_TYPE,
	// to send tcc chime type at each chime index
	PCM_INPUT_SOURCE,
	// (Not official) to send which input source is played
	PCM_MUTE_RAMP_TIME,
	// (Not official) to send ramp time in ms for mute/unmute
	// (calculated as an approximation)
	PCM_VOLUME_RAMP_TIME,
	// (Not official) to send ramp time in ms when changing volume
	// (calculated as an approximation)
};
