 ...
 * because there are many transmission about the position inform
 * during pcm writing or reading at each device.
 * The number of devices are totally 4 (3 tx and 1 rx).
 * So, we estimates 5 queues
 * (4 queues are for i/o device and 1 queue is for command.)
 */
 ...
 *   struct mbox_audio_effect_backup_data_t
 *   am3d_power_base_data[EFFECT_TYPE_COUNT];
 *   };
 */
...
/*
 * TODO : we should choose the size of msg_size
 * (1. unsigned int, 2. unsigned short)
 */
};
...
typedef void (*mbox_audio_work_handler) (void *device_info,
...
struct mbox_audio_client_t {
	...
	int is_used;
	int is_client_user;
	// 0: kernel (use callback), 1: user (use polling)
};
...
struct mbox_audio_device {
	...
	int mbox_audio_ready;
	//after initialize, this value is changed as DRV_STATUS_READY
...
int tcc_mbox_audio_send_command(struct mbox_audio_device *audio_dev,
				...
				unsigned int *msg,
				struct mbox_audio_tx_reply_data_t *reply);
int tcc_mbox_audio_send_command_r5(struct mbox_audio_device *audio_dev,
				struct mbox_audio_data_header_t *header,
				unsigned int *msg,
				struct mbox_audio_tx_reply_data_t *reply);
