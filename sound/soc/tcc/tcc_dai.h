...
static inline void tcc_dai_set_i2s_tdm_mode(void __iomem *base_addr, uint32_t slots, bool lateMode)
{
	...
	mccr0 |= ((uint32_t)half_frame_len << (uint32_t) MCCR0_FRAME_END_POSTION_Pos);
...
static inline void tcc_dai_set_dsp_tdm_mode(void __iomem *base_addr, uint32_t slot_width, bool late)
{
	...
	damr |= (DAMR_DAI_SYNC_IIS_DSP_TDM | DAMR_DSP_OR_TDM_MODE);
	damr |= (slot_width == (uint32_t)24) ? ((uint32_t) DAMR_DSP_WORD_LEN_24BIT) : ((uint32_t) DAMR_DSP_WORD_LEN_16BIT);

	mccr0 |= ((uint32_t)(DSP_TDM_MODE_FRAME_LENGTH-1) << (uint32_t) MCCR0_FRAME_SIZE_Pos);
...
#else
static inline uint32_t tcc_dai_get_mclk_div(void __iomem *base_addr, bool tdm_mode)
#endif
{
	uint32_t mccr0 = readl(base_addr + TCC_DAI_MCCR0_OFFSET);
#if defined(CONFIG_ARCH_TCC803X) || defined(CONFIG_ARCH_TCC805X) || defined(CONFIG_ARCH_TCC806X) || defined(CONFIG_ARCH_TCC899X) || defined(CONFIG_ARCH_TCC901X)
...
#else
static inline uint32_t tcc_dai_get_bclk_ratio(void __iomem *base_addr, bool tdm_mode)
#endif
{
#if defined(CONFIG_ARCH_TCC803X) || defined(CONFIG_ARCH_TCC805X) || defined(CONFIG_ARCH_TCC806X) || defined(CONFIG_ARCH_TCC899X) || defined(CONFIG_ARCH_TCC901X)
	uint32_t dclkdiv = readl(base_addr + TCC_DAI_DCLKDIV_OFFSET);
