...
static int dmem_insert_entry(struct dmem_hashbucket *bucket,
		struct dmem_entry *entry, struct dmem_entry **dupentry)
{
...
static struct dmem_hashbucket *dmem_find_hashbucket(struct dmem *dmem,
		struct dmem_pool *pool, void *key)
{
...
int gcma_frontswap_store(unsigned type, pgoff_t offset, struct page *page)
{
	int ret;
 ...
 * Returns non-zero if failed.
 */
int gcma_frontswap_load(unsigned type, pgoff_t offset, struct page *page)
{
	int ret;
...
void gcma_cleancache_invalidate_page(int pool_id,
		struct cleancache_filekey fkey, pgoff_t offset)
{
	struct cleancache_dmem_key key;
	unsigned long flags;
