...
extern struct sync_timeline *sync_timeline_create(
	const char *name);
extern int sw_sync_create_fence(
...
extern void hdmi_set_activate_callback(
	void(*)(int, int),
	int,
	int);
...
static unsigned int hdmi_base_address;
extern int tca_vioc_displayblock_pre_ctrl_set(
	struct tcc_dp_device *dp_device);
#endif

extern void tcafb_activate_var(
	struct tccfb_info *fbi,
	struct fb_var_screeninfo *var);

/* Debugging stuff */
//#define DEBUG_FB
//#define DEBUG_FB_SCREEN

#ifdef DEBUG_FB
#define dprintk(msg...) pr_info("[DBG][FB] " msg)
#else
#define dprintk(msg...)
#endif

#ifdef DEBUG_FB_SCREEN
#define sprintk(msg...) pr_info("[DBG][FB_S] " msg)
#else
#define sprintk(msg...)
#endif
...
extern void tcc_scrshare_set_sharedBuffer(
	unsigned int addr,
...
extern void tcc_video_info_backup(
	VSYNC_CH_TYPE type,
	struct tcc_lcdc_image_update *input_image);
extern int tcc_video_check_last_frame(
	struct tcc_lcdc_image_update *ImageInfo);
#endif

extern void tca_vioc_displayblock_powerOff(
	struct tcc_dp_device *pDisplayInfo);
extern void tca_vioc_displayblock_disable(
	struct tcc_dp_device *pDisplayInfo);
#if defined(CONFIG_ARCH_TCC)
extern void tca_vioc_displayblock_extra_set(
	struct tcc_dp_device *pDisplayInfo,
	struct tcc_fb_extra_data *tcc_fb_extra_data);
#endif
extern void tca_vioc_displayblock_timing_set(
	unsigned int outDevice,
	struct tcc_dp_device *pDisplayInfo,
	struct lcdc_timimg_parms_t *mode);
extern void tca_vioc_displayblock_ctrl_set(
	unsigned int outDevice,
	struct tcc_dp_device *pDisplayInfo,
	stLTIMING *pstTiming,
	stLCDCTR *pstCtrl);
extern int tca_fb_pan_display(
	struct fb_var_screeninfo *var,
	struct fb_info *info);
extern void tca_fb_activate_var(
	unsigned int dma_addr,
	struct fb_var_screeninfo *var,
	struct tcc_dp_device *pdp_data);
extern void tccfb1_set_par(
	struct tccfb_info *fbi,
	struct fb_var_screeninfo *var);
...
extern int tca_fb_mouse_move(
	unsigned int width,
	unsigned int height,
	tcc_mouse *mouse,
	struct tcc_dp_device *pdp_data);
#endif
extern void tca_fb_resize_set_value(
	tcc_display_resize resize_value,
	TCC_OUTPUT_TYPE output);
extern void tca_fb_output_attach_resize_set_value(
	tcc_display_resize resize_value);
extern int tca_fb_divide_set_mode(
	struct tcc_dp_device *pdp_data,
	char enable,
	char mode);
 ...
 *  16 - 16bpp(rgb565)
 */
#define SCREEN_DEPTH_MAX (32)

const unsigned int default_scn_depth[CONFIG_FB_TCC_DEVS_MAX] = {
	(32), /* fb0, Layer0: bottom, (32 or 16) */
	(32), /* fb1, Layer1: middle */
	(16)  /* fb2, Layer2: top    */
};

#ifdef CONFIG_OF
static const struct of_device_id tccfb_of_match[] = {
...
static struct lcd_panel *display_ext_panel;

static int lcd_video_started;

TCC_OUTPUT_TYPE	Output_SelectMode = TCC_OUTPUT_NONE;

static char  HDMI_pause;
static char HDMI_video_mode;

unsigned int HDMI_video_width;
unsigned int HDMI_video_height;
unsigned int HDMI_video_hz;

unsigned int fb_chromakey_control_enabled;
...
extern int tcc_fb_swap_vpu_frame(
	struct tcc_dp_device *pdp_data,
...
static void tcc_fb_update_regs(
	...
	struct tcc_fenc_reg_data *regs)
{
	...
	if ((system_rev != 0) && (regs->var.reserved[3] != 0)) {
		BaseAddr = tccfb->map_dma +
			regs->var.xres * regs->var.yoffset
				* (regs->var.bits_per_pixel / 8)
			+ regs->var.reserved[1];
	} else
	#endif
	{
	...
	if (tccfb->pdata.Sdp_data.FbPowerState) {
		tca_fb_activate_var(BaseAddr, &regs->var,
			&tccfb->pdata.Sdp_data);
	}
	#endif
	#endif
...
static void send_vsync_event(struct work_struct *work)
{
	char buf[64];
	char *envp[] = { NULL, NULL };
	struct tccfb_info *fbdev =
		container_of(work, struct tccfb_info, vsync_work);
...
void tccfb_extoutput_activate(int fb, int stage)
{
	...
	if (ptccfb_info->pdata.Mdp_data.DispDeviceType == TCC_OUTPUT_HDMI) {
		...
		/* prevent KCS warning */
	} else if (ptccfb_info->pdata.Sdp_data.DispDeviceType
		== TCC_OUTPUT_HDMI) {
		pdp_data = &ptccfb_info->pdata.Sdp_data;
	} else {
		if (stage == STAGE_OUTPUTSTARTER) {
			pdp_data = &ptccfb_info->pdata.Mdp_data;
			pr_info("[INF][FB] %s: Output(%d)\n", __func__,
				ptccfb_info->pdata.Mdp_data.DispDeviceType);
		} else {
			pr_err("[INF][FB] %s: can't find HDMI voic display block\n",
				__func__);
		}
	}
	...
	if (pdp_data != NULL && pdp_data->FbPowerState) {
		if (stage == STAGE_OUTPUTSTARTER) {
			...
			#endif
			{
				BaseAddr = ptccfb_info->map_dma +
					ptccfb_info->fb->var.xres *
					ptccfb_info->fb->var.yoffset *
					(ptccfb_info->fb->var.bits_per_pixel /
					8);
			}

			tca_fb_activate_var(BaseAddr, &ptccfb_info->fb->var,
				pdp_data);
			...
			BaseAddr = ptccfb_info->map_dma +
				ptccfb_info->fb->var.xres *
				ptccfb_info->fb->var.yoffset *
				(ptccfb_info->fb->var.bits_per_pixel / 8);
			...
			if ((pdp_data->FbBaseAddr != 0)
				&& !(pdp_data->DispOrder == DD_MAIN)) {
				tca_fb_activate_var(pdp_data->FbBaseAddr,
					&ptccfb_info->fb->var, pdp_data);
			} else {
				tca_fb_activate_var(BaseAddr,
					&ptccfb_info->fb->var, pdp_data);
			}
			#endif
			...
			// For Video Display using TCC_LCDC_HDMI_DISPLAY
			tcc_vsync_hdmi_start(pdp_data, &lcd_video_started);
			#else
			#ifdef CONFIG_VIDEO_TCC_VOUT
			if (tcc_vout_get_status(pdp_data->ddc_info.blk_num)
				!= TCC_VOUT_RUNNING)
			#endif
			{
				if (tcc_vsync_isVsyncRunning(VSYNC_MAIN) ||
					tcc_vsync_isVsyncRunning(VSYNC_SUB0)) {
					tcc_vsync_hdmi_start(pdp_data,
						&lcd_video_started);
				}
			}
	...
	int specific_pclk)
{
	...
	case TCC_OUTPUT_HDMI:
		...
		tca_vioc_displayblock_ctrl_set(VIOC_OUTCFG_HDMI, pdp_data,
			pstTiming, pstCtrl);
		break;
	...
	case TCC_OUTPUT_COMPOSITE:
		...
		tca_vioc_displayblock_ctrl_set(VIOC_OUTCFG_SDVENC, pdp_data,
			pstTiming, pstCtrl);
		break;
	...
	case TCC_OUTPUT_COMPONENT:
		...
		pdp_data->FbUpdateType = FB_SC_RDMA_UPDATE;
		tca_vioc_displayblock_powerOn(pdp_data, specific_pclk);
		tca_vioc_displayblock_ctrl_set(VIOC_OUTCFG_HDVENC, pdp_data,
			pstTiming, pstCtrl);
		break;
	}
...
#if defined(CONFIG_ARCH_TCC)
void tccfb_output_starter_extra_data(
	char output_type,
	struct tcc_fb_extra_data *tcc_fb_extra_data)
{
	struct fb_info *info;
...
static int tccfb_ioctl(
	struct fb_info *info,
	unsigned int cmd,
	unsigned long arg)
{
	struct tccfb_info *ptccfb_info =
		(info != NULL) ? (struct tccfb_info *)info->par : NULL;
	...
	if ((info->node < 0) || (info->node >= CONFIG_FB_TCC_DEVS)) {
	...
	case GET_UMP_SECURE_ID_BUF(0):
	{
		if (ump_wrapped_buffer[info->node][0] == 0) {
			...
			ump_wrapped_buffer[info->node][0] =
	...
	case GET_UMP_SECURE_ID_BUF(1):
	{
		if (ump_wrapped_buffer[info->node][1] == 0) {
			ump_memory_description.addr = info->fix.smem_start;
			ump_memory_description.size = info->fix.smem_len;
			ump_wrapped_buffer[info->node][1] =
				ump_dd_handle_create_from_phys_blocks(
					&ump_memory_description, 1);
		}
	...
	case GET_UMP_SECURE_ID_BUF(2):
	{
		if (ump_wrapped_buffer[info->node][2] == 0) {
			...
			ump_memory_description.size = info->fix.smem_len;
			ump_wrapped_buffer[info->node][2] =
				ump_dd_handle_create_from_phys_blocks(
					&ump_memory_description, 1);
		}
	...
	case TCC_LCD_FBIOPUT_VSCREENINFO:
	{
		...
		if (!regs) {
			//pr_err("[ERR][FB] fb fence sync couldn't allocate\n");
			mutex_unlock(&ptccfb_info->output_lock);
			return -ENOMEM;
		}
		...
		if (copy_to_user(
			(struct fb_var_screeninfo __user *)arg, &var_info,
			sizeof(struct fb_var_screeninfo))) {
			ret = -EFAULT;
			break;
		}
	...
	case TCC_LCDC_SET_OUTPUT_RESIZE_MODE:
	{
		...
		if (copy_from_user((void *)&resize_value, (const void *)arg,
			sizeof(tcc_display_resize))) {
			return -EFAULT;
		}

		//pr_debug("%s: TCC_LCDC_SET_OUTPUT_RESIZE_MODE mode=%d\n",
		//	__func__, mode);

		tca_fb_resize_set_value(resize_value, TCC_OUTPUT_MAX);

		if ((ptccfb_info->pdata.Mdp_data.DispDeviceType
				== TCC_OUTPUT_HDMI)
			|| (ptccfb_info->pdata.Mdp_data.DispDeviceType
			...
			//pr_err("TCC_LCDC_SET_OUTPUT_RESIZE_MODE
			//Can't find display device, Main:%d, Sub:%d\n",
			//	ptccfb_info->pdata.Mdp_data.DispDeviceType,
			//	ptccfb_info->pdata.Sdp_data.DispDeviceType);
		//}
	...
	case TCC_SECONDARY_OUTPUT_RESIZE_MODE_STB:
	{
		...
		pr_info("[INF][FB] TCC_SECONDARY_OUTPUT_RESIZE_MODE_STB, mode=left:%d, right:%d, up:%d, down:%d\n",
			...
			resize_value.resize_up, resize_value.resize_down);
	...
	case TCC_LCDC_SET_OUTPUT_ATTACH_RESIZE_MODE:
	{
		...
		//dprintk("%s: TCC_LCDC_SET_OUTPUT_ATTACH_RESIZE_MODE(%d)\n",
		//	__func__, mode);
	...
	case TCC_LCDC_SET_HDMI_R2YMD:
	{
		...
		if (copy_from_user((void *)&r2ymd, (const void *)arg,
			sizeof(r2ymd))) {
			return -EFAULT;
		}

		if ((ptccfb_info->pdata.Mdp_data.FbPowerState != true)
			|| (ptccfb_info->pdata.Mdp_data.DispDeviceType
				== TCC_OUTPUT_HDMI)) {

			pdp_data = &ptccfb_info->pdata.Mdp_data;

		} else if ((ptccfb_info->pdata.Sdp_data.FbPowerState != true)
			|| (ptccfb_info->pdata.Sdp_data.DispDeviceType
				== TCC_OUTPUT_HDMI)) {
			...
			pr_err("%s: hdmi power on: can't find HDMI voic display block.\n",
				__func__);
		}

		if (pdp_data && ((r2ymd >> 16)
			== TCC_LCDC_SET_HDMI_R2YMD_MAGIC)) {
			VIOC_DISP_SetR2YMD(pdp_data->ddc_info.virt_addr,
				(unsigned char)(r2ymd & 0xFF));
		}
	}
		break;
	#endif
	...
	case TCC_LCDC_HDMI_START:
	{
		...
		if (ptccfb_info != NULL) {
			...
			} else if ((ptccfb_info->pdata.Sdp_data.FbPowerState
					!= true)
				|| (ptccfb_info->pdata.Sdp_data.DispDeviceType
					== TCC_OUTPUT_HDMI)) {
			...
			if (pdp_data != NULL) {
				if (pdp_data->FbPowerState == true) {
					...
					tca_vioc_displayblock_disable(pdp_data);
					tca_vioc_displayblock_powerOff(
						pdp_data);
				}
	...
	case TCC_LCDC_HDMI_TIMING:
	{
		...
		if (copy_from_user((void *)&lcdc_timing, (const void *)arg,
			sizeof(struct lcdc_timimg_parms_t))) {
			return -EFAULT;
		}
		...
		if (ptccfb_info != NULL) {
			...
			if (pdp_data != NULL) {
				tca_vioc_displayblock_timing_set(
					VIOC_OUTCFG_HDMI, pdp_data,
					&lcdc_timing);
				...
				if (DV_PATH_DIRECT & vioc_get_path_type()) {
					...
					pr_info("[INF][FB] %s TCC_LCDC_HDMI_TIMING DV mode\n",
						__func__);
					hdmi_set_activate_callback(
						tccfb_extoutput_activate,
						info->node, STAGE_FB);
				} else {
					/* Remove Callaback */
				...
				if (display_ext_panel != NULL &&
					...
					if (!strncmp("HDMI_480TU",
						display_ext_panel->name, 10)) {
						skip_activate = 0;
					}
				}
				#endif
				...
				if (skip_activate && pdp_data->FbPowerState) {
					VIOC_DISP_TurnOn(
						pdp_data->ddc_info.virt_addr);
				}
				#endif
			#endif

				if (!skip_activate) {
					tccfb_extoutput_activate(info->node,
						STAGE_FB);
				}

				if (display_ext_panel != NULL) {
					if (display_ext_panel->set_power
						!= NULL) {
						display_ext_panel->set_power(
							display_ext_panel,
	...
	case TCC_LCDC_HDMI_DISPLAY:
	{
		...
		if (copy_from_user((void *)&ImageInfo, (const void *)arg,
		...
		if (ImageInfo.enable != 0) {
			...
			if (ptccfb_info->image_enable != 0) {
				if (ptccfb_info->swap_buf_status ==
					SWAP_BUF_START) {
					if (ptccfb_info->swap_buf_id >=
						ImageInfo.buffer_unique_id) {

						dprintk("0x%X (%d >= %d)\n",
						ptccfb_info->swap_buf_status,
						ptccfb_info->swap_buf_id,
						ImageInfo.buffer_unique_id);
		...
		if (ptccfb_info->pdata.Mdp_data.DispDeviceType
			...
			//pr_err("TCC_LCDC_HDMI_DISPLAY Can't find HDMI output,
			//Main:%d, Sub :%d\n",
			//ptccfb_info->pdata.Mdp_data.DispDeviceType,
			//ptccfb_info->pdata.Sdp_data.DispDeviceType);
		//}
		...
		if (pdp_data && Output_SelectMode == TCC_OUTPUT_HDMI) {
			...
			if (ImageInfo.enable != 0) {
				...
				if (ImageInfo.output_toMemory) {
					tcc_video_clear_last_frame(
						ImageInfo.Lcdc_layer, 1);
				}

				if (tcc_video_check_last_frame(&ImageInfo)
					<= 0) {
	...
	case TCC_LCDC_HDMI_CHECK:
	{
		...
		if (HDMI_pause || ((screen_width < screen_height)
			&& (!HDMI_video_mode))) {
			ret_mode = 1;
			dprintk("TCC_LCDC_HDMI_CHECK: %d : %d %d\n",
	...
	case TCC_LCDC_HDMI_MODE_SET:
	{
		...
		dprintk("%s: TCC_LCDC_HDMI_MODE_SET [%d] vd:%d osm:%d\n",
		...
		case TCC_HDMI_VIDEO_END:
			...
			//TCC_OUTPUT_FB_CaptureVideoImg(0);
				...
				&& defined(CONFIG_HDMI_DISPLAY_LASTFRAME)
	...
	case TCC_LCDC_HDMI_GET_SIZE:
	{
		...
		dprintk("%s: TCC_LCDC_HDMI_GET_SIZE: HDMI_video wxh: %dx%d\n",
			__func__, HDMI_video_width, HDMI_video_height);

		if (copy_to_user((tcc_display_size *)arg, &HdmiSize,
			sizeof(HdmiSize))) {
			return -EFAULT;
		}
	}
	...
	case TCC_LCDC_HDMI_SET_SIZE:
	{
		...
		if (copy_from_user((void *)&HdmiSize, (const void *)arg,
			sizeof(tcc_display_size))) {
			return -EFAULT;
		}
		...
		dprintk("%s: TCC_LCDC_HDMI_SET_SIZE: HDMI_video wxh:%dx%d\n",
			__func__, HDMI_video_width, HDMI_video_height);
	}
		break;
	...
	case TCC_LCDC_HDMI_EXTRA_SET:
	{
		...
		if (copy_from_user((void *)&tcc_fb_extra_data,
			(const void *)arg,
			sizeof(struct tcc_fb_extra_data))) {
			return -EFAULT;
		}
		...
		if (ptccfb_info->pdata.Mdp_data.DispDeviceType
			...
			pr_err("%s: hdmi timing setting: can't find HDMI voic display block\n",
				__func__);
		}

		if (pdp_data != NULL) {
			tca_vioc_displayblock_extra_set(pdp_data,
				&tcc_fb_extra_data);
		}
	}
		break;
	...
	case TCC_LCDC_HDMI_DISPDEV_ID:
	{
		...
		if (ptccfb_info != NULL) {
			...
			if (pdp_data != NULL) {
				...
				pr_err("%s: TCC_LCDC_HDMI_DISPDEV_ID: can't find HDMI voic display block\n",
					__func__);
			}

			if (copy_to_user((int *)arg, &dispdev_id,
				sizeof(int))) {
				return -EFAULT;
			}
		}
	...
	case TCC_COMPONENT_FBIOPUT_VSCREENINFO:
	{
		...
		if (copy_from_user((void *)&sc_info, (const void *)arg,
			sizeof(external_fbioput_vscreeninfo))) {
			return -EFAULT;
		}
		...
		if (copy_to_user((external_fbioput_vscreeninfo *)arg,
			(void *)&sc_info,
			sizeof(external_fbioput_vscreeninfo))) {
			return -EFAULT;
		}
	...
	#ifdef CONFIG_TCC_SCREEN_SHARE
		tcc_scrshare_set_sharedBuffer(BaseAddr, sc_info.width,
			sc_info.height, TCC_LCDC_IMG_FMT_RGB888);
	#endif
	}
		break;
	...
	case TCC_LCDC_COMPOSITE_CHECK:
	{
		...
		if (copy_to_user((void *)arg, &composite_detect,
			sizeof(unsigned int))) {
			return -EFAULT;
		}
	}
	...
	case TCC_LCDC_COMPOSITE_MODE_SET:
	{
		...
		if (copy_from_user((void *)&composite_mode, (const void *)arg,
			sizeof(LCDC_COMPOSITE_MODE))) {
			return -EFAULT;
		}
		...
		if (composite_mode == LCDC_COMPOSITE_UI_MODE) {
			...
			#ifdef CONFIG_ANDROID
			tca_vsync_video_display_disable(); // @inbest: org code
			#else
			#ifdef CONFIG_VIDEO_TCC_VOUT
	...
	case TCC_LCDC_COMPONENT_CHECK:
	{
		...
		if (copy_to_user((void *)arg, &component_detect,
			sizeof(unsigned int))) {
			return -EFAULT;
		}
	}
	...
	case TCC_LCDC_COMPONENT_MODE_SET:
	{
		...
		if (copy_from_user((void *)&component_mode, (const void *)arg,
			sizeof(LCDC_COMPONENT_MODE))) {
			return -EFAULT;
		}
		...
		if (component_mode == LCDC_COMPONENT_UI_MODE) {
			...
			#ifdef CONFIG_ANDROID
			tca_vsync_video_display_enable(); // @alank: org code
			#else
			#ifdef CONFIG_VIDEO_TCC_VOUT
			if (tcc_vout_get_status(1) != TCC_VOUT_RUNNING)
			#endif
			...
			//---> @alank: for v4l2 vout driver
			#ifdef CONFIG_ANDROID
			tca_vsync_video_display_disable(); // @alank: org code
			#else
			#ifdef CONFIG_VIDEO_TCC_VOUT
			if (tcc_vout_get_status(1) != TCC_VOUT_RUNNING)
			#endif
	...
	case TCC_LCDC_FBCHANNEL_ONOFF:
	{
		...
		if (copy_from_user((void *)&onOff, (const void *)arg,
			sizeof(unsigned int))) {
			return -EFAULT;
			}
	...
	case TCC_LCDC_MOUSE_SHOW:
	{
		...
		if (pdp_data != NULL) {
			if (enable == 0) {
				VIOC_RDMA_SetImageDisable(
				pdp_data->rdma_info[RDMA_MOUSE].virt_addr);
			}
		}
	}
		break;
	...
	case TCC_LCDC_MOUSE_MOVE:
	{
		...
		if (copy_from_user((void *)&mouse, (const void *)arg,
			sizeof(tcc_mouse))) {
			return -EFAULT;
		}
		...
		if (pdp_data != NULL) {
			tca_fb_mouse_move(ptccfb_info->fb->var.xres,
				ptccfb_info->fb->var.yres, &mouse, pdp_data);
		}
	}
		break;
	...
	case TCC_LCDC_MOUSE_ICON:
	{
		...
		if (copy_from_user((void *)&mouse_icon, (const void *)arg,
			sizeof(tcc_mouse_icon))) {
			return -EFAULT;
		}
	...
	case TCC_LCDC_3D_UI_ENABLE:
	{
		...
		if (copy_from_user((void *)&mode, (const void *)arg,
			sizeof(unsigned int))) {
			return -EFAULT;
		}
	...
	{
		unsigned int display_type = 0; /* Support HDMI/CVBS/COMPONENT */

		#if defined(CONFIG_FB_TCC_COMPOSITE) \
			&& defined(CONFIG_FB_TCC_COMPONENT)
		display_type = 0; /* Support HDMI/CVBS/COMPONENT */
		#endif

		#if defined(CONFIG_FB_TCC_COMPOSITE) \
			&& !defined(CONFIG_FB_TCC_COMPONENT)
		display_type = 1; /* Support HDMI/CVBS */
		#endif

		#if !defined(CONFIG_FB_TCC_COMPOSITE) \
			&& !defined(CONFIG_FB_TCC_COMPONENT)
		display_type = 2; /* Support HDMI */
		#endif


		if (copy_to_user((void *)arg, &display_type,
			sizeof(unsigned int))) {
			return -EFAULT;
		}
	...
	case TCC_LCDC_DISPLAY_END:
	{
		...
		ImageInfo.enable = 0;
		tca_scale_display_update(&ptccfb_info->pdata.Mdp_data,
			(struct tcc_lcdc_image_update *)&ImageInfo);
	...
	case FBIO_DISABLE:
	{
		...
		if (copy_from_user((void *)&type, (const void *)arg,
			sizeof(unsigned int))) {
			return -EFAULT;
		}
		...
		if (pdp_data->rdma_info[RDMA_FB].virt_addr) {
			...
			sc_num = VIOC_CONFIG_GetScaler_PluginToRDMA(
				pdp_data->rdma_info[RDMA_FB].blk_num);
			if (sc_num > 0) {
			...
			VIOC_RDMA_SetImageSize(
				pdp_data->rdma_info[RDMA_FB].virt_addr, 0, 0);
	...
	case TCC_LCDC_FB_CHROMAKEY_CONTROL_KERNEL:
	{
		...
		if (cmd == TCC_LCDC_FB_CHROMAKEY_CONTROL_KERNEL) {
			if (memcpy((void *)&chromakey_ctrl,
				(const void *)arg,
				sizeof(lcdc_chromakey_params)) == NULL) {
				return -EFAULT;
			}
		} else {
			if (copy_from_user((void *)&chromakey_ctrl,
				(const void *)arg,
				sizeof(lcdc_chromakey_params))) {
				return -EFAULT;
			}
		}
		...
		if (pdp_data->wmixer_info.virt_addr) {
			...
			VIOC_WMIX_SetChromaKey(pdp_data->wmixer_info.virt_addr,
				chromakey_ctrl.layer_num,
				chroma_en, key_r, key_g, key_b,
	...
	case GET_2D_COMPRESSION_FB_INFO:
	{
		...
		if (copy_to_user((void *)arg, &(pmap_fb_video.base),
	...
	case CHECK_2D_COMPRESSION_EN:
	{
		...
		if (copy_from_user((void *)&ts_address, (const void *)arg,
			sizeof(unsigned int))) {
			return -EFAULT;
		}
		...
		if (bMark_ffff && CONFIG_ADDITIONAL_TS_BUFFER_LINES != 0) {
			...
			remap_addr = (unsigned int *)ioremap_nocache(ts_address,
				4096);
		...
		if (copy_to_user((void *)arg, &(ts_address),
			sizeof(unsigned int))) {
			return -EFAULT;
		}
	}
	...
	case TCC_LCDC_GET_DISP_FU_STATUS:
	{
		...
		if (copy_to_user((void *)arg, &fu_count,
			sizeof(unsigned int))) {
			return -EFAULT;
		}
	...
	case TCC_LCDC_SET_COLOR_ENHANCE:
	{
		...
		if (copy_from_user((void *)&params, (const void *)arg,
			sizeof(struct lcdc_colorenhance_params))) {
			return -EFAULT;
		}
		...
		if (pdp_data) {
			if (pdp_data->ddc_info.virt_addr) {
				...
				pr_info("[INF][FB] TCC_LCDC_SET_COLOR_ENHANCE lcdc:0x%x contrast:0x%x saturation:0x%x brightness:0x%x hue:0x%x\n",
					params.lcdc_type, params.contrast,
					params.saturation, params.brightness,
					params.hue);

				if (params.hue < 0x100) {
					VIOC_DISP_SetCENH_hue(
						pdp_data->ddc_info.virt_addr,
						params.hue);
					VIOC_DISP_DCENH_hue_onoff(
						pdp_data->ddc_info.virt_addr,
						1);
				} else {
					VIOC_DISP_DCENH_hue_onoff(
				...
				if ((params.brightness >= 0x400)
					&& (params.saturation >= 0x400)
					&& (params.contrast >= 0x400)) {
					VIOC_DISP_DCENH_onoff(
						pdp_data->ddc_info.virt_addr,
						0);
				} else {
					if (params.brightness < 0x400) {
						VIOC_DISP_SetCENH_brightness(
						pdp_data->ddc_info.virt_addr,
						params.brightness);
					}
					if (params.saturation < 0x400) {
						VIOC_DISP_SetCENH_saturation(
						pdp_data->ddc_info.virt_addr,
						params.saturation);
					}
					if (params.contrast < 0x400) {
						VIOC_DISP_SetCENH_contrast(
						pdp_data->ddc_info.virt_addr,
						params.contrast);
					}
				...
				pr_info("[INF][FB] TCC_LCDC_SET_COLOR_ENHANCE lcdc:0x%x contrast:0x%x , brightness:0x%x hue:0x%x\n",
					params.lcdc_type, params.contrast,
					params.brightness, params.hue);

				VIOC_DISP_SetColorEnhancement(
					pdp_data->ddc_info.virt_addr,
					(signed char)params.contrast,
					(signed char)params.brightness,
					(signed char)params.hue);
				#endif
			}
		}
	}
	...
	case TCC_LCDC_GET_COLOR_ENHANCE:
	{
		...
		if (pdp_data) {
			if (pdp_data->ddc_info.virt_addr) {
					...
					|| defined(CONFIG_ARCH_TCC899X) \
					|| defined(CONFIG_ARCH_TCC901X)
				VIOC_DISP_GetCENH_hue(
					pdp_data->ddc_info.virt_addr,
					(unsigned int *)&params.hue);
				VIOC_DISP_GetCENH_brightness(
					pdp_data->ddc_info.virt_addr,
					(unsigned int *)&params.brightness);
				VIOC_DISP_GetCENH_saturation(
					pdp_data->ddc_info.virt_addr,
					(unsigned int *)&params.saturation);
				VIOC_DISP_GetCENH_contrast(
					pdp_data->ddc_info.virt_addr,
					(unsigned int *)&params.contrast);
				VIOC_DISP_GetCENH_hue_onoff(
					pdp_data->ddc_info.virt_addr,
					(unsigned int *)&params.check_hue_onoff
					);
				VIOC_DISP_GetCENH_onoff(
					pdp_data->ddc_info.virt_addr,
					(unsigned int *)&params.check_colE_onoff
					);

				pr_info("[INF][FB] TCC_LCDC_GET_COLOR_ENHANCE lcdc:0x%x hue:0x%x onoff:%d\n",
					params.lcdc_type, params.hue,
					params.check_hue_onoff);
				pr_info("[INF][FB] TCC_LCDC_GET_COLOR_ENHANCE lcdc:0x%x contrast:0x%x saturation:0x%x brightness:0x%x onoff:%d\n",
					params.lcdc_type, params.contrast,
					params.saturation, params.brightness,
					params.check_colE_onoff);
				#else

				VIOC_DISP_GetColorEnhancement(
					pdp_data->ddc_info.virt_addr,
					(signed char *)&params.contrast,
					(signed char *)&params.brightness,
					(signed char *)&params.hue);
	...
	case TCC_LCDC_FB_SWAP_VPU_FRAME:
	{
		...
		TempImage = kmalloc(sizeof(struct tcc_lcdc_image_update),
			GFP_KERNEL);
		if (!TempImage) {
			//pr_err("TCC_LCDC_FB_SWAP_VPU_FRAME kmalloc\n");
			return -EFAULT;
		}

		ret = tcc_fb_swap_vpu_frame(pdp_data, &WmixerInfo,
			TempImage, VSYNC_MAIN);
	...
	enum dma_data_direction direction)
{
	...
	debug_dma_map_sg(NULL, table->sgl, table->nents,
...
static void *fb_ion_dma_buf_map(
...
static void *dmabuf_kmap_atomic(
	struct dma_buf *buf,
	unsigned long page)
{
	return NULL;
...
static void dmabuf_kunmap_atomic(
	struct dma_buf *buf,
	unsigned long page,
	void *vaddr)
{
...
static int __init tccfb_map_video_memory(
	struct tccfb_info *fbi,
	int plane)
{
	...
	if (pmap_fb_video.size == 0) {
		fbi->map_size  = fbi->fb->var.xres_virtual
			* fbi->fb->var.yres_virtual
			* (fbi->fb->var.bits_per_pixel / 8);
		fbi->map_cpu = dma_alloc_writecombine(fbi->dev, fbi->map_size,
			&fbi->map_dma, GFP_KERNEL);
		pr_info("%s: fbi=%p dma:0x%x size:%08x\n",
			__func__, fbi, fbi->map_dma, fbi->map_size);
	} else {
		fbi->map_dma = pmap_fb_video.base;
		fbi->map_size = pmap_fb_video.size;
		fbi->map_cpu = ioremap_nocache(fbi->map_dma, fbi->map_size);
		pr_info("%s: plane:%d fbi:%p dma:0x%x cpu:%p size:%08x\n",
			__func__, plane, fbi, fbi->map_dma,
	...
	if (fbi->map_cpu) {
		...
		if (hdmi_base_address != 0 && pWDMA != NULL && pSC != NULL) {
			...
			VIOC_SC_SetDstSize(pSC, fbi->fb->var.xres,
				fbi->fb->var.yres);
			VIOC_SC_SetOutSize(pSC, fbi->fb->var.xres,
				fbi->fb->var.yres);
			VIOC_CONFIG_PlugIn(scnum, VIOC_WDMA |
				get_vioc_index(fbi->pdata.Mdp_data.DispNum));

			VIOC_WDMA_SetImageFormat(pWDMA,
				(fbi->fb->var.bits_per_pixel == 32)
				? TCC_LCDC_IMG_FMT_RGB888
				: TCC_LCDC_IMG_FMT_RGB565);
			VIOC_WDMA_SetImageSize(pWDMA, fbi->fb->var.xres,
				fbi->fb->var.yres);
			VIOC_WDMA_SetImageOffset(pWDMA,
				(fbi->fb->var.bits_per_pixel == 32)
				? TCC_LCDC_IMG_FMT_RGB888
				: TCC_LCDC_IMG_FMT_RGB565,
				fbi->fb->var.xres);
		...
		#else
		#if !defined(CONFIG_LOGO) && defined(CONFIG_PLATFORM_AVN) \
...
static int tcc_dp_dt_parse_data(struct tccfb_info *info)
{
	...
	if (info->dev->of_node) {
		...
		if (of_property_read_u32(info->dev->of_node,
			"telechips,fbdisplay_num", &info->pdata.lcdc_number)) {
			pr_err("[ERR][FB] could not find  telechips,fbdisplay_nubmer\n");
			ret = -ENODEV;
		}

		if (info->pdata.lcdc_number) {
			main_np = of_find_node_by_name(info->dev->of_node,
				"fbdisplay1");
			extend_np = of_find_node_by_name(info->dev->of_node,
				"fbdisplay0");
		} else {
			main_np = of_find_node_by_name(info->dev->of_node,
				"fbdisplay0");
		...
		memcpy(&info->pdata.Sdp_data.ddc_info,
			&info->pdata.Mdp_data.ddc_info,
			sizeof(info->pdata.Mdp_data.ddc_info));
		info->pdata.Sdp_data.FbPowerState = 0;
		memcpy(&info->pdata.Sdp_data.wmixer_info,
			&info->pdata.Mdp_data.wmixer_info,
			sizeof(info->pdata.Mdp_data.wmixer_info));
		memcpy(&info->pdata.Sdp_data.rdma_info[RDMA_FB],
			&info->pdata.Mdp_data.rdma_info[1],
			sizeof(info->pdata.Mdp_data.rdma_info[0]));
		#endif

		// Set rdma arbitor resgister and disable disp emergency
		// flag according to SOC guide for TCC896x
		rdma_arbitor_np = of_parse_phandle(info->dev->of_node,
			"telechips,rdma_arbitor", 0);
		if (rdma_arbitor_np) {
			VIOC_DISP_EmergencyFlagDisable(
				info->pdata.Mdp_data.ddc_info.virt_addr);
			VIOC_DISP_EmergencyFlagDisable(
				info->pdata.Sdp_data.ddc_info.virt_addr);
...
static const char tccfb_driver_name[] = "tccfb";
static int tccfb_probe(struct platform_device *pdev)
{
	...
	//	of_match_device(tccfb_of_match, &pdev->dev);
	...
	if (display_ext_panel != NULL) {
		pr_info("\x1b[1;38m [INF][FB] Extended panel is %s %s %d x %d \x1b[0m\n",
			display_ext_panel->manufacturer,
			display_ext_panel->name,
			display_ext_panel->xres,
			display_ext_panel->yres);
	}
	...
	for (plane = 0; plane < CONFIG_FB_TCC_DEVS; plane++) {
		...
		fbinfo_reg = framebuffer_alloc(sizeof(struct tccfb_info),
			&pdev->dev);
		...
		fbinfo_reg->var.yres_virtual = fbinfo_reg->var.yres
						* FB_NUM_BUFFERS;
		fbinfo_reg->var.bits_per_pixel = default_scn_depth[plane];
		fbinfo_reg->fix.line_length = fbinfo_reg->var.xres
					* fbinfo_reg->var.bits_per_pixel / 8;

#if defined(CONFIG_ADDITIONAL_TS_BUFFER_LINES)
		fbinfo_reg->var.reserved[0] = CONFIG_ADDITIONAL_TS_BUFFER_LINES;
#endif

		fbinfo_reg->pseudo_palette = devm_kzalloc(&pdev->dev,
		...
		if (plane == 0)	{ // top layer
			if (fb_prepare_logo(fbinfo_reg, FB_ROTATE_UR)) {
				...
				fb_alloc_cmap_gfp(&fbinfo->cmap, 16, 0,
					GFP_KERNEL);
				fb_alloc_cmap_gfp(&fbinfo_reg->cmap, 16, 0,
					GFP_KERNEL);
				#endif
	...
	if (display_ext_panel != NULL && display_ext_panel->init != NULL) {
		display_ext_panel->init(display_ext_panel,
			&info->pdata.Sdp_data);
	}
...
static int tccfb_restore(struct device *dev)
{
	...
	#if defined(__CONFIG_HIBERNATION)
	struct platform_device *fb_device = container_of(dev,
