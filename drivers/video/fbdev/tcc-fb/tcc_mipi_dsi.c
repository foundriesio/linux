...
{
	struct tcc_db_dsi_data *dsi =
		(struct tcc_db_dsi_data *)tcc_db_get_outdata(db);

	pr_info("%s %s: LINE:%d  %s\n", COLORWRN_COLOR, __func__, __LINE__,
	       NORMAL_COLOR);
...
static void tcc_db_dsi_enable(struct lcd_panel *db)
{
	...
	if (dsi->enabled) {
		pr_err("ERROR %s: mipi dsi enabled:\n", __func__);
		goto fail;
	}
...
#error not define PMS frequency
		...
		pHwMIPI_DSI->CLKCTRL = (0x1 << 31) // TxRequestHsClk
			...
			| (0x1 << 0); // EscPrescaler
		udelay(200);
		pr_info("%s %s: LINE:%d  %s\n", COLORWRN_COLOR, __func__,
		       __LINE__, NORMAL_COLOR);
		...
		while (((pHwMIPI_DSI->STATUS >> 8) & 0x1) != 0x0)
			; // wait for StopstateClk = 0
		// while (((pHwMIPI_DSI->STATUS>>0)&0xf) != 0x0); // wait for
		// StopstateDat = 0
		...
		while (((pHwMIPI_DSI->STATUS >> 9) & 0x1) != 0x1)
			; // wait for UlpsClk = 1
		while (((pHwMIPI_DSI->STATUS >> 4) & 0xf) != 0xf)
			; // wait for UlpsDat = 0xf
		sim_value(pHwMIPI_DSI->STATUS);
		// exit ulps mode
		pHwMIPI_DSI->ESCMODE = (0x0 << 21) // STOPstate_cnt
			| (0x0 << 20)		   // ForceStopstate
		...
		while (((pHwMIPI_DSI->STATUS >> 9) & 0x1) != 0x0)
			; // wait for UlpsClk = 0
		while (((pHwMIPI_DSI->STATUS >> 4) & 0xf) != 0x0)
			; // wait for UlpsDat = 0
		sim_value(pHwMIPI_DSI->STATUS);
		udelay(200);
...
static void tcc_db_dsi_disable(struct lcd_panel *db)
{
	...
	if (!dsi->enabled) {
		pr_err("ERROR %s: mipi dsi disabled:\n", __func__);
		goto fail;
	}
