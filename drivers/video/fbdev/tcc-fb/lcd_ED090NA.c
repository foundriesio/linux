...
static int
ed090na_panel_init(struct lcd_panel *panel, struct tcc_dp_device *fb_pdata)
{
	...
	struct lcd_panel *panel, int on, struct tcc_dp_device *fb_pdata)
{
	...
	if (on) {
		...
		VIOC_DDICONFIG_LVDS_SetPath(
			pDDICfg, LVDS_TX_DATA(0), 0x15141312);
		VIOC_DDICONFIG_LVDS_SetPath(
			pDDICfg, LVDS_TX_DATA(1), 0x0B0A1716);
		VIOC_DDICONFIG_LVDS_SetPath(
			pDDICfg, LVDS_TX_DATA(2), 0x0F0E0D0C);
		VIOC_DDICONFIG_LVDS_SetPath(
			pDDICfg, LVDS_TX_DATA(3), 0x05040302);
		VIOC_DDICONFIG_LVDS_SetPath(
			pDDICfg, LVDS_TX_DATA(4), 0x1A190706);
		VIOC_DDICONFIG_LVDS_SetPath(
			pDDICfg, LVDS_TX_DATA(5), 0x1F1E1F18);
		VIOC_DDICONFIG_LVDS_SetPath(
			pDDICfg, LVDS_TX_DATA(6), 0x1F1E1F1E);
		VIOC_DDICONFIG_LVDS_SetPath(
			pDDICfg, LVDS_TX_DATA(7), 0x1F1E1F1E);
		VIOC_DDICONFIG_LVDS_SetPath(
			pDDICfg, LVDS_TX_DATA(8), 0x001E1F1E);

		if (panel->clk_freq >= LVDS_VCO_45MHz
		    && panel->clk_freq < LVDS_VCO_60MHz) {
			M = 7;
			P = 7;
			S = 0;
			VSEL = 0;
...
#ifdef CONFIG_OF
static const struct of_device_id ed090na_of_match[] = {
