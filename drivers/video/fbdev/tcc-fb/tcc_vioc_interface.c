...
//#define DEBUG_VIOC
#ifdef DEBUG_VIOC
#define dprintk(msg...)	pr_info("[DBG][VIOC_I] " msg)
#else
#define dprintk(msg...)
#endif
...
#if defined(CONFIG_VIOC_AFBCDEC)
static unsigned int afbc_dec_1st_cfg;
static unsigned int afbc_dec_vioc_id = VIOC_FBCDEC0;
static unsigned int afbc_wdma_need_on; /* 0: no, 1: need */
#define AFBC_MAX_SURFACE (2)
#endif

#if defined(__CONFIG_HIBERNATION)
//extern unsigned int do_hibernation; // tccfb.h
...
#include "./vioc/tcc_composite.h" // declare tcc_plugout_for_composite()
#include "./vioc/tcc_component.h" // declare tcc_plugout_for_component()

#ifdef CONFIG_VIDEO_DISPLAY_SWAP_VPU_FRAME
extern void tcc_video_frame_backup(
...
void tca_fb_dump_underrun_state(void)
{
	...
	pr_info("[INF][VIOC_I] %s: %d'th\n", __func__, underrun_idx);
...
void tca_fb_mem_scale_init(void)
{
	...
	pr_info("[INF][VIOC_I] %s scaler buffer:0x%x 0x%x g2d buffer : 0x%x, 0x%x size:%llu\n",
		__func__, fb_scaler_pbuf0, fb_scaler_pbuf1,
		fb_g2d_pbuf0, fb_g2d_pbuf1, pmap.size);
}
...
#define DD0_DMA_CONNECT_CHECK(PlugInOut) ( \
	(PlugInOut.connect_device == VIOC_SC_RDMA_00) \
	|| (PlugInOut.connect_device == VIOC_SC_RDMA_01) \
	|| (PlugInOut.connect_device == VIOC_SC_RDMA_02) \
	|| (PlugInOut.connect_device == VIOC_SC_RDMA_03) \
	|| (PlugInOut.connect_device == VIOC_SC_WDMA_00) \
	)

#define DD1_DMA_CONNECT_CHECK(PlugInOut) ( \
	(PlugInOut.connect_device == VIOC_SC_RDMA_04) \
	|| (PlugInOut.connect_device == VIOC_SC_RDMA_05) \
	|| (PlugInOut.connect_device == VIOC_SC_RDMA_06) \
	|| (PlugInOut.connect_device == VIOC_SC_RDMA_07) \
 ...
 */
void vioc_display_device_reset(
	unsigned int device_num,
	struct tcc_dp_device *pDisplayDevice)
{
	...
	unsigned char WMIXER_N, RDMA_N[RDMA_MAX_NUM];

	VIOC_PlugInOutCheck VIOC_PlugIn[VIOC_VIQE + 1];
	unsigned char VIOC_PlugIn_reset[get_vioc_index(VIOC_VIQE0) + 1];
	...
	unsigned int vioc_compress[RDMA_MAX_NUM] = {0,}; // MC, DTRC, DEC100
	...
	for (i = 0; i < RDMA_MAX_NUM; i++) {
		#ifdef CONFIG_VIOC_MAP_DECOMP
		if ((vioc_compress[i] >= VIOC_MC0)
			&& (vioc_compress[i] < (VIOC_MC0 + VIOC_MC_MAX))) {
			tca_map_convter_onoff(vioc_compress[i], 0, 0);
		}
		#endif

		#ifdef CONFIG_VIOC_DTRC_DECOMP
		if ((vioc_compress[i] >= VIOC_DTRC0)
			&& (vioc_compress[i] < (VIOC_DTRC0 + VIOC_DTRC_MAX))) {
	...
	if (DISPBackup != NULL) {
		...
		pr_err("[ERR][VIOC_I] %s: DISPBackup is null\n", __func__);
	}
	...
	if (WMIXBackup != NULL) {
		...
		pr_err("[ERR][VIOC_I] %s: WMIXBackup is null\n", __func__);
	}
	...
	for (i = 0; i < (get_vioc_index(VIOC_SCALER3)+1); i++) {
		...
		if (VIOC_PlugIn_reset[i] && (pSC_Addr[i] != NULL)) {
			...
			memcpy((void *)pSC_Backup[i], (void *)pSC_Addr[i],
				0x20);
		}
	}
	...
	if (VIOC_PlugIn_reset[get_vioc_index(VIOC_VIQE0)]
		...
		if (ViqeBackup != NULL) {
			...
			pr_err("[ERR][VIOC_I] %s: ViqeBackup is null\n",
				__func__);
		}
	}
	...
	for (i = 0; i < RDMA_MAX_NUM; i++) {
		// rdma reset
		VIOC_CONFIG_SWReset(RDMA_N[i], VIOC_CONFIG_RESET);
	}
	...
	for (i = 0; i < RDMA_MAX_NUM; i++) {
		...
		if ((vioc_compress[i] >= VIOC_MC0)
			&& (vioc_compress[i] < (VIOC_MC0 + VIOC_MC_MAX))) {
			// MC
			VIOC_CONFIG_SWReset(vioc_compress[i],
				VIOC_CONFIG_RESET);
		}
		#endif
		#ifdef CONFIG_VIOC_DTRC_DECOMP
	...
	if (VIOC_PlugIn_reset[get_vioc_index(VIOC_SCALER0)]) {
		VIOC_CONFIG_SWReset(VIOC_SCALER0,
			VIOC_CONFIG_RESET);
	}
	if (VIOC_PlugIn_reset[get_vioc_index(VIOC_SCALER1)]) {
		VIOC_CONFIG_SWReset(VIOC_SCALER1,
			VIOC_CONFIG_RESET);
	}
	if (VIOC_PlugIn_reset[get_vioc_index(VIOC_SCALER2)]) {
		VIOC_CONFIG_SWReset(VIOC_SCALER2,
			VIOC_CONFIG_RESET);
	}
	if (VIOC_PlugIn_reset[get_vioc_index(VIOC_SCALER3)]) {
		VIOC_CONFIG_SWReset(VIOC_SCALER3,
			VIOC_CONFIG_RESET);
	}
	if (VIOC_PlugIn_reset[get_vioc_index(VIOC_VIQE0)]) {
		VIOC_CONFIG_SWReset(VIOC_VIQE0,
			VIOC_CONFIG_RESET);
	}
	...
	for (i = 0; i < RDMA_MAX_NUM; i++) {
		// rdma reset release
		VIOC_CONFIG_SWReset(RDMA_N[i], VIOC_CONFIG_CLEAR);
	}
	...
	VIOC_CONFIG_SWReset(WMIXER_N, VIOC_CONFIG_RESET);   // wmix normal
	VIOC_CONFIG_SWReset(device_num, VIOC_CONFIG_RESET); // wdma normal
	VIOC_CONFIG_SWReset(device_num, VIOC_CONFIG_RESET); // disp normal
	...
	for (i = 0; i < (get_vioc_index(VIOC_SCALER3)+i); i++) {
		if (VIOC_PlugIn_reset[i] && (pSC_Addr[i] != NULL)) {
			memcpy((void *)pSC_Addr[i], (void *)pSC_Backup[i],
				0x20);
	...
	for (i = 0; i < RDMA_MAX_NUM; i++) {
		memcpy((void *)pRDMA_Addr[i], (void *)RDMABackup[i],
			0x20);
	}

	if (WMIXBackup != NULL) {
		memcpy((void *)pWMIX_Addr, (void *)WMIXBackup,
			0x20);
	}
	...
	pr_info("[INF][VIOC_I] VIOC_sc0:%d VIOC_sc1:%d VIOC_sc2:%d VIOC_sc3:%d VIOC_VIQE:%d\n",
		VIOC_PlugIn_reset[0], VIOC_PlugIn_reset[1],
		VIOC_PlugIn_reset[2], VIOC_PlugIn_reset[3],
		VIOC_PlugIn_reset[4]);
}
#endif
...
irqreturn_t tca_main_display_handler(int irq, void *dev_id)
{
	...
	if (dev_id == NULL) {
		pr_err("[ERR][VIOC_I] %s irq: %d dev_id:%p\n",
			__func__, irq, dev_id);
		return IRQ_HANDLED;
	}

	#if defined(CONFIG_VIOC_AFBCDEC)
	afbcDec_status = VIOC_AFBCDec_GetStatus(
		VIOC_AFBCDec_GetAddress(afbc_dec_vioc_id));
	...
	if (afbcDec_status & AFBCDEC_IRQ_ALL) {
		...
		if (afbcDec_status & AFBCDEC_IRQ_DECODE_ERR_MASK) {
			...
			fbdev->pdata.Mdp_data.rdma_info[RDMA_FB].virt_addr,
			&rdma_enable);

			if (rdma_enable) {
				VIOC_RDMA_SetImageDisable(
					fbdev->pdata.Mdp_data.rdma_info[
						RDMA_FB].virt_addr);
			}

			VIOC_CONFIG_SWReset(afbc_dec_vioc_id,
				VIOC_CONFIG_RESET);
			VIOC_CONFIG_SWReset(afbc_dec_vioc_id,
				VIOC_CONFIG_CLEAR);
	...
	if (VIOC_CONFIG_DV_GET_EDR_PATH()) {
		...
		if (status & DV_FB_INT) {
			...
			if (fbdev->pdata.Mdp_data.pandisp_sync.state == 0) {
				...
				&fbdev->pdata.Mdp_data.pandisp_sync.waitq);
			}
		}
	...
	//	__func__, fbdev->pdata.Mdp_data.pandisp_sync.state,
	//	dispblock_status, fbdev);

	if (dispblock_status & (1 << VIOC_DISP_INTR_RU)) {
		vioc_intr_clear(fbdev->pdata.lcdc_number,
			(1 << VIOC_DISP_INTR_RU));
		...
		if (VIOC_DISP_Get_TurnOnOff(
			fbdev->pdata.Mdp_data.ddc_info.virt_addr)
		) {
			if (dispblock_status & (1<<VIOC_DISP_INTR_FU)) {
				vioc_intr_disable(irq, fbdev->pdata.lcdc_number,
					(1<<VIOC_DISP_INTR_FU));
				vioc_intr_clear(fbdev->pdata.lcdc_number,
					(1<<VIOC_DISP_INTR_FU));

			#if defined(CONFIG_DUMP_DISPLAY_UNDERRUN) \
				&& defined(CONFIG_VIOC_DOLBY_VISION_EDR)
				if (underrun_idx++ < 1) {
					pr_err("[ERR][VIOC_I] FIFO UNDERRUN(%d) status(0x%x) main(%d)\n",
					underrun_idx, dispblock_status,
					fbdev->pdata.Mdp_data.DispDeviceType);
				...
				#if defined(CONFIG_VIOC_RESET_FOR_UNDERRUN)
					if (fbdev->pdata.Mdp_data.DispDeviceType
						== TCC_OUTPUT_HDMI) {
						hdmi_stop();
					}
	...
	if (dispblock_status & (1<<VIOC_DISP_INTR_DD)) {
		...
		if (fbdev->pdata.Mdp_data.disp_dd_sync.state == 0) {
			...
			wake_up_interruptible(
				&fbdev->pdata.Mdp_data.disp_dd_sync.waitq);
		}

		#if defined(CONFIG_VIOC_RESET_FOR_UNDERRUN)
		if (VIOCFifoUnderRun) {
			vioc_display_device_reset(fbdev->pdata.lcdc_number,
				&fbdev->pdata.Mdp_data);
			VIOC_DISP_TurnOn(
				fbdev->pdata.Mdp_data.ddc_info.virt_addr);

			pr_info("[INF][VIOC_I] %s DISABEL DONE Lcdc_num:%d VIOCFifoUnderRun:0x%x\n",
				__func__, fbdev->pdata.lcdc_number,
				VIOCFifoUnderRun);
		...
		pr_info("[INF][VIOC_I] %s DISABEL DONE Lcdc_num:%d 0x%p  STATUS:0x%x\n",
			__func__, fbdev->pdata.lcdc_number,
			fbdev->pdata.Mdp_data.ddc_info.virt_addr,
			dispblock_status);
		#endif
...
static void _tca_vioc_intr_onoff(char on, int irq, char lcdc_num)
{
	...
	#endif
	{
		if (on) {
			...
			vioc_intr_disable(irq, lcdc_num,
...
int tca_main_interrupt_reg(char SetClear, struct tccfb_info *info)
{
	...
	//pr_info("[INF][VIOC_I] %s en:%d lcdc_num:%d %d INT_VIOC_DEV1:%d\n",
	//	__func__, SetClear, info->pdata.lcdc_number,
	//	info->pdata.Mdp_data.ddc_info.irq_num, INT_VIOC_DEV1);

	#if defined(CONFIG_FB_TCC_USE_VSYNC_INTERRUPT)
	if (SetClear) {
		vioc_intr_clear(info->pdata.lcdc_number, 0x39);

		ret = request_irq(info->pdata.Mdp_data.ddc_info.irq_num,
			tca_main_display_handler, IRQF_SHARED,
			"TCC_MAIN_D", info);

		_tca_vioc_intr_onoff(ON, info->pdata.Mdp_data.ddc_info.irq_num,
		...
		_tca_vioc_intr_onoff(OFF, info->pdata.Mdp_data.ddc_info.irq_num,
			info->pdata.lcdc_number);
...
static irqreturn_t tca_sub_display_handler(int irq, void *dev_id)
{
	...
	if (dispblock_status & VIOC_DISP_IREQ_RU_MASK) {
		...
		if (pDisplayInfo->pandisp_sync.state == 0) {
			...
			wake_up_interruptible(
				&pDisplayInfo->pandisp_sync.waitq);
		}
		...
		if (VIOC_DISP_Get_TurnOnOff(pDisplayInfo->ddc_info.virt_addr)) {
			if (dispblock_status & (1<<VIOC_DISP_INTR_FU)) {
			#if defined(CONFIG_VIOC_FIFO_UNDER_RUN_COMPENSATION)
				pr_err("[ERR][VIOC_I] FIFO UNDERRUN status(0x%x) sub\n",
					dispblock_status);
			#endif
				vioc_intr_clear(pDisplayInfo->DispNum,
					(1<<VIOC_DISP_INTR_FU));
				//vioc_intr_disable(irq, pDisplayInfo->DispNum,
				//	(1<<VIOC_DISP_INTR_FU));
			}
		} else {
			//Don't care FIFO under-run, when DISP is off.
	...
	if (dispblock_status & (1<<VIOC_DISP_INTR_DD)) {
		...
		if (pDisplayInfo->disp_dd_sync.state == 0) {
			...
			wake_up_interruptible(
				&pDisplayInfo->disp_dd_sync.waitq);
		}

		pr_info("[INF][VIOC_I] %s DISABEL DONE Lcdc_num:%d 0x%p  STATUS:0x%x\n",
			__func__, pDisplayInfo->DispNum,
			pDisplayInfo->ddc_info.virt_addr,
			dispblock_status);
		#endif
	}
...
#if defined(CONFIG_TCC_VSYNC_DRV_CONTROL_LUT)
static void tca_fb_wait_for_video_rdma_eofr(
	struct tcc_dp_device *pdp_data)
{
	...
	if (pdp_data != NULL) {
		...
		if (video_rdma_irq_id != 0) {
			vioc_intr_clear(video_rdma_irq_id,
				(1 << VIOC_RDMA_INTR_EOFR));

			do {
				irq_rdma_status =
					vioc_intr_get_status(video_rdma_irq_id);
...
void tca_vioc_displayblock_clock_select(
	struct tcc_dp_device *pDisplayInfo,
	int clk_src_hdmi_phy)
{
#if defined(CONFIG_TCC_HDMI_DRIVER_V2_0)
	...
	if (pDisplayInfo != NULL && pDDICONFIG != NULL) {
		if (pDisplayInfo->DispDeviceType == TCC_OUTPUT_HDMI) {
			if (clk_src_hdmi_phy) {
				//pr_info("%s: disp dev uses hdmi phy clk\n",
				//	__func__);
				...
				if (!IS_ERR(pDisplayInfo->ddc_clock)) {
					...
					//pr_info("disp dev uses peri clk %lu\n"
					//	, __func__, hdmi_pixel_clock);

					clk_set_rate(pDisplayInfo->ddc_clock,
						hdmi_pixel_clock);
					clk_prepare_enable(
						pDisplayInfo->ddc_clock);
				}
			...
			VIOC_DDICONFIG_SetPeriClock(pDDICONFIG,
				pDisplayInfo->DispNum, clk_src_hdmi_phy);

			#if defined(CONFIG_VIOC_DOLBY_VISION_EDR)
			/* Set DV clock source is L0_LCLK */
			if (DV_PATH_DIRECT & vioc_get_path_type()) {
				pr_info("[INF][VIOC_I] dv select %s clk\n",
					clk_src_hdmi_phy?"hdmiphy":"periclk");
				VIOC_DDICONFIG_SetPeriClock(pDDICONFIG, 3,
					clk_src_hdmi_phy);
			} else if (VIOC_DDICONFIG_GetPeriClock(pDDICONFIG, 3)
				&& clk_src_hdmi_phy == 0) {
				pr_info("[INF][VIOC_I] dv select peri clk\n");
				VIOC_DDICONFIG_SetPeriClock(pDDICONFIG, 3,
					clk_src_hdmi_phy);
			}
			#endif
			...
			hdmi_pixel_clock =
				clk_get_rate(pDisplayInfo->ddc_clock);
			//pr_info("[INF][VIOC_I] L%d_LCLK is %luHz\n",
	...
	int specific_pclk)
{
	...
	if (pDisplayInfo->DispDeviceType == TCC_OUTPUT_HDMI) {
		...
		np_output = of_find_compatible_node(NULL, NULL,
			"telechips,tcc897x-hdmi");
		#endif
	...
	if (np_output != NULL) {
		of_property_read_u32(np_output, "vioc-frequency",
			&vioc_clock_value);

		/* Component and Composite clocks vary
		 * with chip and resolution.
		 */
		if (specific_pclk > 0) {
	...
	pr_info("[INF][VIOC_I] %s lcdc:%d device:%d lcdc_clock:%d\n", __func__,
		get_vioc_index(pDisplayInfo->ddc_info.blk_num),
		pDisplayInfo->DispDeviceType, vioc_clock_value);

	clk_prepare_enable(pDisplayInfo->vioc_clock);
	clk_prepare_enable(pDisplayInfo->ddc_clock);

	if (pDisplayInfo->DispOrder != DD_MAIN)	{
		ret = request_irq(pDisplayInfo->ddc_info.irq_num,
			tca_sub_display_handler, IRQF_SHARED,
			"SUB_LCDC", pDisplayInfo);

		if (ret < 0) {
			pr_err("[ERR][VIOC_I] %s:%d  ret:%d\n",
				__func__, __LINE__, ret);
		} else {
			_tca_vioc_intr_onoff(ON, pDisplayInfo->ddc_info.irq_num,
				pDisplayInfo->DispNum);
		}
	}
}

void tca_vioc_displayblock_powerOff(
	struct tcc_dp_device *pDisplayInfo)
{
	#if defined(CONFIG_ARCH_TCC898X) || defined(CONFIG_ARCH_TCC899X) \
		|| defined(CONFIG_ARCH_TCC803X) || defined(CONFIG_ARCH_TCC901X)
	...
	pr_info("[INF][VIOC_I] %s lcdc: %d, type: %d\n", __func__,
		get_vioc_index(pDisplayInfo->ddc_info.blk_num),
		pDisplayInfo->DispDeviceType);

	#if defined(CONFIG_VIOC_DOLBY_VISION_EDR)
	if (VIOC_CONFIG_DV_GET_EDR_PATH()
		&& pDisplayInfo->DispDeviceType == TCC_OUTPUT_HDMI
		&& pDisplayInfo->DispNum == 0) {
		//vioc_v_dv_block_off(); msleep(30);
		//VIOC_V_DV_SWReset(0);
		_tca_vioc_intr_onoff(OFF, pDisplayInfo->ddc_info.irq_num,
			pDisplayInfo->DispNum);
		VIOC_V_DV_Power(0);
	}
	#endif
	...
	if (g2d_filp.private_data != NULL) {
		g2d_release((struct inode *)&g2d_inode,
...
void tca_vioc_displayblock_disable(
	struct tcc_dp_device *pDisplayInfo)
{
	int scaler_num = 0;
	...
	pr_info("[INF][VIOC_I] %s lcdc:%d onoff:%d\n", __func__,
		get_vioc_index(pDisplayInfo->ddc_info.blk_num),
	...
	if (VIOC_CONFIG_DV_GET_EDR_PATH() &&
		...
		_tca_vioc_intr_onoff(OFF, pDisplayInfo->ddc_info.irq_num,
			pDisplayInfo->DispNum);
		VIOC_V_DV_GetInterruptPending(VIOC_DV_VEDR_GetAddress(VDV_CFG),
			&status);
		VIOC_V_DV_ClearInterrupt(VIOC_DV_VEDR_GetAddress(VDV_CFG),
			status);
	...
	if (tcc_vout_get_status(pDisplayInfo->ddc_info.blk_num)
		!= TCC_VOUT_RUNNING) {
		VIOC_RDMA_SetImageDisableNW(
			pDisplayInfo->rdma_info[RDMA_VIDEO_3D].virt_addr);
		VIOC_RDMA_SetImageDisableNW(
			pDisplayInfo->rdma_info[RDMA_VIDEO].virt_addr);
	}
	#else
	{
		int i = 0;
		...
		for (i = 0; i < RDMA_MAX_NUM; i++) {
			VIOC_RDMA_SetImageDisableNW(
				pDisplayInfo->rdma_info[i].virt_addr);
		}
	}
	#endif
	...
	if (VIOC_DISP_Get_TurnOnOff(pDisplayInfo->ddc_info.virt_addr)) {
		...
		#endif
		{
			...
			if (wait_event_interruptible_timeout(
				pDisplayInfo->disp_dd_sync.waitq,
				pDisplayInfo->disp_dd_sync.state == 1,
				msecs_to_jiffies(30)) == 0) {
				pr_info("[INF][VIOC_I] %s: Timeout DISABLE DONE wait for ms\n",
					__func__);
			}
			#else
	...
	if (pDisplayInfo->FbUpdateType == FB_MVC_UPDATE) {
		pr_info("[INF][VIOC_I] %s : pDisplayInfo->FbUpdateType = %d\n",
			__func__, pDisplayInfo->FbUpdateType);

		// scaler plug in status check
		if (VIOC_CONFIG_GetScaler_PluginToRDMA(
			pDisplayInfo->rdma_info[RDMA_3D].blk_num)
			>= VIOC_SCALER0) {
			scaler_num = VIOC_CONFIG_GetScaler_PluginToRDMA(
				pDisplayInfo->rdma_info[RDMA_3D].blk_num);
			...
			pr_info("[INF][VIOC_I] %s : Scaler-%d is reset\n",
	...
	// scaler plug in status check
	if (VIOC_CONFIG_GetScaler_PluginToRDMA(
		pDisplayInfo->rdma_info[RDMA_FB].blk_num) >= VIOC_SCALER0) {
		scaler_num = VIOC_CONFIG_GetScaler_PluginToRDMA(
			pDisplayInfo->rdma_info[RDMA_FB].blk_num);
		VIOC_CONFIG_PlugOut(scaler_num);
		VIOC_CONFIG_SWReset(scaler_num, VIOC_CONFIG_RESET);
		VIOC_CONFIG_SWReset(scaler_num, VIOC_CONFIG_CLEAR);
		pr_info("[INF][VIOC_I] %s : UI Scaler-%d is reset\n",
			__func__, get_vioc_index(scaler_num));
	}
	...
	if (pDisplayInfo->FbUpdateType == FB_DIVIDE_UPDATE) {
		...
		if (VIOC_CONFIG_GetScaler_PluginToRDMA(
			...
			VIOC_CONFIG_SWReset(scaler_num, VIOC_CONFIG_RESET);
			VIOC_CONFIG_SWReset(scaler_num, VIOC_CONFIG_CLEAR);
			pr_info("[INF][VIOC_I] %s : 3D Scaler-%d is reset\n",
				__func__, get_vioc_index(scaler_num));
		}
	}

#if defined(CONFIG_TCC_DISPLAY_MODE_DUAL_HDMI_CVBS) \
	|| defined(CONFIG_TCC_DISPLAY_MODE_DUAL_ALL) \
	|| defined(CONFIG_TCC_DISPLAY_LCD_CVBS) \
	|| defined(CONFIG_TCC_DISPLAY_MODE_DUAL_HDMI_CC)

	if (pDisplayInfo->FbUpdateType == FB_DIVIDE_UPDATE) {
		VIOC_CONFIG_SWReset(pDisplayInfo->wmixer_info.blk_num,
			VIOC_CONFIG_RESET);
		VIOC_CONFIG_SWReset(pDisplayInfo->wmixer_info.blk_num,
			VIOC_CONFIG_CLEAR);
		VIOC_CONFIG_SWReset(pDisplayInfo->rdma_info[RDMA_FB].blk_num,
			VIOC_CONFIG_RESET);
		VIOC_CONFIG_SWReset(pDisplayInfo->rdma_info[RDMA_FB].blk_num,
			VIOC_CONFIG_CLEAR);
		VIOC_CONFIG_SWReset(pDisplayInfo->rdma_info[RDMA_3D].blk_num,
			VIOC_CONFIG_RESET);
		VIOC_CONFIG_SWReset(pDisplayInfo->rdma_info[RDMA_3D].blk_num,
			VIOC_CONFIG_CLEAR);
		VIOC_CONFIG_SWReset(pDisplayInfo->wdma_info.blk_num,
			VIOC_CONFIG_RESET);
		VIOC_CONFIG_SWReset(pDisplayInfo->wdma_info.blk_num,
			VIOC_CONFIG_CLEAR);
	}
...
static void tca_vioc_displaytiming_dump(struct tcc_dp_device *pDisplayInfo)
{
	...
	unsigned int reg = __raw_readl(pDISP+DCTRL);
	unsigned int val, val1, val2, val3, val4;

	pr_debug("[DBG][VIOC_I] FB Timing Dump\n");
	pr_debug("[DBG][VIOC_I] SYNC VSYNC(%s), HSYNC(%s), DE(%s) PCLK(%s)\n",
		(reg & (1<<13))?"Low":"High",  (reg & (1<<12))?"Low":"High",
		(reg & (1<<14))?"Low":"High", (reg & (1<<1))?"Low":"High");
	...
	pr_info("[INF][VIOC_I] HSYNC ACTIVE(%d) DELAY(%d) SYNC(%d) BLANK(%d)\n",
		val,  val3, val1,  val4);
	...
	pr_info("[DBG][VIOC_I] VSYNC ACTIVE(%d) DELAY(%d) SYNC(%d) BLANK(%d)\n",
		val,  val3, val1,  val4);
}
#endif

#if defined(CONFIG_ARCH_TCC)
void tca_vioc_displayblock_extra_set(
	struct tcc_dp_device *pDisplayInfo,
	struct tcc_fb_extra_data *tcc_fb_extra_data)
{
	...
	if (!tcc_fb_extra_data) {
		pr_err("[ERR][VIOC_I] %s tcc_fb_extra_data is null\n",
			__func__);
		return;
	}
	if (!pDisplayInfo || !pDisplayInfo->ddc_info.virt_addr) {
		pr_err("[ERR][VIOC_I] %s invalid param\n", __func__);
		return;
	}
	...
	vioc_v_dv_set_output_color_format(tcc_fb_extra_data->pxdw,
		tcc_fb_extra_data->swapbf);
	#endif
	...
	if (pxdw_bit <= 0xF) {
		...
		case 21:
			/* "20-bit YCbCr * {Y,Cb} a {Y,Cr}" to
			 * "16-bit * YCbCr * {Y,Cb} a {Y,Cr}"
			 */
			tcc_fb_extra_data->pxdw = 8;
			break;
		case 23:
			/* "30-bit RGB101010" to "24-bit RGB888" */
			tcc_fb_extra_data->pxdw = 12;
			break;
		case 27:
			/* YCbCr 420 * {Cb,Y0,Y1} a {Cr,Y2,Y3} is not support */
			pr_err("[ERR][VIOC_I] %s pxdw %d is out of ranger\n",
	...
	pr_info("[INF][VIOC_I] %s pxdw(%d), swapbf(%d), r2ymd(%d), r2y(%d)\n",
		__func__, tcc_fb_extra_data->pxdw, tcc_fb_extra_data->swapbf,
		tcc_fb_extra_data->r2ymd, tcc_fb_extra_data->r2y);
	...
	if (pDisplayInfo->FbUpdateType == FB_DIVIDE_UPDATE
		...
		if (pRDMA != NULL) {
			VIOC_RDMA_SetImageR2YEnable(pRDMA, 1);
			VIOC_RDMA_SetImageY2REnable(pRDMA, 0);
			VIOC_RDMA_SetImageR2YMode(pRDMA,
				tcc_fb_extra_data->r2ymd);
		}
	}
...
void tca_vioc_displayblock_timing_set(
	unsigned int outDevice,
	struct tcc_dp_device *pDisplayInfo,
	struct lcdc_timimg_parms_t *mode)
{
	...
	//pr_info("[INF][VIOC_I] #### DV EDR Mode ? format(%d),
	//reg(0x%x)/meta(0x%x), outDevice(%d)/Disp_0(%p =? %p)\n",
	//	mode->format, mode->dv_reg_phyaddr, mode->dv_md_phyaddr,
	//	outDevice, pDISP, VIOC_DISP_GetAddress(0));

	if ((outDevice == VIOC_OUTCFG_HDMI)
		&& (pDISP == VIOC_DISP_GetAddress(0))) {

		if (mode->dv_reg_phyaddr) {
			pr_info("[INF][VIOC_I] #### DV EDR Mode!!! format(%d) phy(0x%x) noYUV422(%d) vsvdb(0x%x) LL(%d)\n",
				mode->format, mode->dv_reg_phyaddr,
				mode->dv_noYUV422_SDR, mode->dv_vsvdb_size,
				mode->dv_ll_mode);
			voic_v_dv_set_hdmi_timming(mode, 1,
				mode->dv_hdmi_clk_khz);
			vioc_v_dv_set_stage(DV_STANDBY);
			vioc_v_dv_set_mode(mode->dv_ll_mode,
				(unsigned char *)mode->dv_vsvdb,
				mode->dv_vsvdb_size);
			VIOC_V_DV_Power(1);
		} else {
			VIOC_CONFIG_DV_SET_EDR_PATH(0);
			pr_info("[INF][VIOC_I] #### Non-DV EDR Mode!!! stage(%d) mode(0x%x) path(%d) out(%d)\n",
				vioc_v_dv_get_stage(), vioc_v_dv_get_mode(),
				vioc_get_path_type(), vioc_get_out_type());
		}
	}
	#endif

	#if defined(CONFIG_FB_REG_DUMP)
	pr_info("[INF][VIOC_I] DUMP FB\n");
	pr_info(" id     = %d\n", mode->id);
	pr_info(" iv     = %d\n", mode->iv);
	pr_info(" ih     = %d\n", mode->ih);
	pr_info(" ip     = %d\n", mode->ip);
	...
	pr_info("[INF][VIOC_I] %s width=%d, height=%d\n",
	...
	#endif
	{
		VIOC_OUTCFG_SetOutConfig(outDevice,
			pDisplayInfo->ddc_info.blk_num);
	}

	VIOC_RDMA_GetImageEnable(pDisplayInfo->rdma_info[RDMA_FB].virt_addr,
		&rdma_en);

	if (rdma_en) {
		VIOC_RDMA_SetImageDisable(
			pDisplayInfo->rdma_info[RDMA_FB].virt_addr);
	}

#if defined(CONFIG_HDMI_FB_ROTATE_90) || defined(CONFIG_HDMI_FB_ROTATE_180) \
	|| defined(CONFIG_HDMI_FB_ROTATE_270)
	pDisplayInfo->FbUpdateType = FB_SC_G2D_RDMA_UPDATE;
		...
		#else
		/* YUV output */
		pDisplayInfo->FbUpdateType = FB_SC_M2M_RDMA_UPDATE;
		#endif
	#else
	...
	pr_info("[INF][VIOC_I] mode->framepacking = %d\n", mode->framepacking);

	if (mode->framepacking == 1) {
		//pDisplayInfo->MVCMode = 1;
		pDisplayInfo->FbUpdateType = FB_MVC_UPDATE;
		pr_info("[INF][VIOC_I] >>FB_MVC_UPDATE\n");
		...
		pr_info("[INF][VIOC_I] >>FB_DIVIDE_UPDATE\n");
	} else {
...
// patch for stb presentation window.
	...
	/* This code used to prevent kernel panic when FbUpdateType is
	 * FB_RDMA_UPDATE
	 */
	...
	if (pDisplayInfo->FbUpdateType == FB_SC_RDMA_UPDATE) {
		VIOC_CONFIG_PlugIn(pDisplayInfo->sc_num0,
			pDisplayInfo->rdma_info[RDMA_FB].blk_num);
	...
	} else if (pDisplayInfo->FbUpdateType == FB_DIVIDE_UPDATE
		|| pDisplayInfo->FbUpdateType == FB_MVC_UPDATE) {
		...
		if (pDisplayInfo->FbUpdateType == FB_DIVIDE_UPDATE
			&& divide_data.fbtype == FB_SC_M2M_RDMA_UPDATE) {
			if (!scaler_filp) {
				pr_info("[INF][VIOC_I] Prepare Scaler driver with SC1\n");
				scaler_filp =
					filp_open(DEV_SCALER1, O_RDWR, 0666);
				if (IS_ERR(scaler_filp)) {
					scaler_filp = NULL;
					pr_info("[INF][VIOC_I] Failed Open SC1\n");
				}
			}
		} else {
			VIOC_CONFIG_PlugIn(pDisplayInfo->sc_num0,
				pDisplayInfo->rdma_info[RDMA_FB].blk_num);
			VIOC_CONFIG_PlugIn(pDisplayInfo->sc_num1,
				pDisplayInfo->rdma_info[RDMA_3D].blk_num);
		}
	}
	...
	pr_info("[INF][VIOC_I] %s displayN:%d non-interlaced:%d w:%d h:%d FbUpdateType:%d\n",
...
int tca_vioc_displayblock_pre_ctrl_set(
	struct tcc_dp_device *dp_device)
{
	int ret = -1;
	unsigned int sc_num;
	...
	do {
		...
		if (get_vioc_index(dp_device->ddc_info.blk_num) == DD_MAIN) {
			viocmg_set_wmix_ovp(VIOCMG_CALLERID_FB,
				dp_device->wmixer_info.blk_num,
				viocmg_get_main_display_ovp());
		} else
		#endif
		{
			VIOC_WMIX_SetOverlayPriority(pWMIX, 24);
		...
		if (dp_device->FbUpdateType != FB_ATTACH_UPDATE) {
			#if defined(CONFIG_HDMI_FB_ROTATE_90) \
				|| defined(CONFIG_HDMI_FB_ROTATE_180) \
				|| defined(CONFIG_HDMI_FB_ROTATE_270)
			...
			/* YUV output */
			dp_device->FbUpdateType = FB_SC_M2M_RDMA_UPDATE;
			//!CONFIG_TCC_VIOC_DISP_PATH_INTERNAL_CS_YUV-
			#endif
			//CONFIG_TCC_OUTPUT_COLOR_SPACE_YUV ||
			//CONFIG_TCC_COMPOSITE_COLOR_SPACE_YUV -
			#else
			...
			//CONFIG_TCC_COMPOSITE_COLOR_SPACE_YUV) -
			#endif
			//!(CONFIG_HDMI_FB_ROTATE_90 ||
			//CONFIG_HDMI_FB_ROTATE_180 ||
			//CONFIG_HDMI_FB_ROTATE_270) -
			#endif
			...
			if (dp_device->FbUpdateType == FB_SC_RDMA_UPDATE) {
				sc_num = VIOC_CONFIG_GetScaler_PluginToRDMA(
		...
		#if defined(CONFIG_VIOC_DOLBY_VISION_EDR)
		if (vioc_v_dv_get_stage() == DV_OFF ||
			(vioc_v_dv_get_stage() != DV_OFF && dp_device->DispNum
			!= 0))
		#endif
		{
			_tca_vioc_intr_onoff(ON, dp_device->ddc_info.irq_num,
				dp_device->DispNum);
		}
...
void tca_vioc_displayblock_ctrl_set(
	unsigned int outDevice,
	struct tcc_dp_device *pDisplayInfo,
	stLTIMING *pstTiming,
	stLCDCTR *pstCtrl)
{
	...
	if (skip_display_device) {
		...
		VIOC_RDMA_GetImageEnable(
			pDisplayInfo->rdma_info[RDMA_FB1].virt_addr, &rdma_en);

		if (rdma_en) {
			VIOC_RDMA_SetImageDisable(
				pDisplayInfo->rdma_info[RDMA_FB1].virt_addr);
		}

		VIOC_CONFIG_SWReset(pDisplayInfo->rdma_info[RDMA_FB1].blk_num,
			VIOC_CONFIG_RESET);
		VIOC_CONFIG_SWReset(pDisplayInfo->rdma_info[RDMA_FB1].blk_num,
			VIOC_CONFIG_CLEAR);
	}
	...
	if (skip_display_device) {
		...
		VIOC_CONFIG_SWReset(pDisplayInfo->ddc_info.blk_num,
			VIOC_CONFIG_RESET);
		VIOC_CONFIG_SWReset(pDisplayInfo->ddc_info.blk_num,
			VIOC_CONFIG_CLEAR);
	...
	if (get_vioc_index(pDisplayInfo->ddc_info.blk_num) == DD_MAIN) {
		viocmg_set_wmix_ovp(VIOCMG_CALLERID_FB,
			pDisplayInfo->wmixer_info.blk_num,
			viocmg_get_main_display_ovp());
	} else
	#endif
	{
	...
	if (pDisplayInfo->FbUpdateType != FB_ATTACH_UPDATE) {
		...
		if (pDisplayInfo->FbUpdateType == FB_SC_RDMA_UPDATE) {
			VIOC_CONFIG_PlugOut(pDisplayInfo->sc_num0);
			// prevent display under-run
			VIOC_CONFIG_SWReset(pDisplayInfo->sc_num0,
				VIOC_CONFIG_RESET);
			VIOC_CONFIG_SWReset(pDisplayInfo->sc_num0,
				VIOC_CONFIG_CLEAR);
			VIOC_CONFIG_SWReset(
		...
		} else  if (pDisplayInfo->FbUpdateType
			== FB_SC_M2M_RDMA_UPDATE) {
			if (!scaler_filp) {
				scaler_filp = filp_open(DEV_SCALER1, O_RDWR,
					0666);
			...
			g2d_open((struct inode *)&g2d_inode,
					(struct file *)&g2d_filp);
	...
	#if defined(CONFIG_VIOC_DOLBY_VISION_EDR)
	if (vioc_v_dv_get_stage() == DV_OFF || (vioc_v_dv_get_stage() != DV_OFF
		&& pDisplayInfo->DispNum != 0))
	#endif
	{
		_tca_vioc_intr_onoff(ON, pDisplayInfo->ddc_info.irq_num,
...
static void tca_vioc_configure_DEC100(
	...
	void __iomem *pRDMA)
{
	...
	unsigned int ts_address = base_addr +
	...
	if (CONFIG_ADDITIONAL_TS_BUFFER_LINES != 0) {
		...
		if (var->reserved[2] != 0) {
			...
			remap_addr = (unsigned int *)ioremap_nocache(
				ts_address, 4096);
	...
	if (bSet_Comp) {
		VIOC_RDMA_DEC_CTRL(pRDMA, ts_address,
			(var->xres * var->yres / 64) / 2, nAlpha, 0xf);

		// decompress mode : 1, bypass mode : 0
		VIOC_RDMA_DEC_EN(pRDMA, 1);
		//pr_info("[INF][VIOC_I] => FBC for DEC100 - 0x%x/0x%x - %d\n",
...
#if defined(CONFIG_VIOC_AFBCDEC)
static void
tca_vioc_configure_AFBCDEC(
	unsigned int vioc_dec_id, unsigned int base_addr,
	void __iomem *pRDMA, void __iomem *pWDMA,
	unsigned int fmt, unsigned int rdmaPath,
	unsigned char bSet_Comp, unsigned int bSplitMode,
	unsigned int bWideMode,
	unsigned int width, unsigned int height)
{
	...
	if (bSet_Comp) {
		if (!afbc_dec_1st_cfg) {
			if (VIOC_WDMA_IsImageEnable(pWDMA)
			...
			VIOC_AFBCDec_SurfaceCfg(pAFBC_Dec, base_addr, fmt,
				width, height, 0, bSplitMode, bWideMode,
				VIOC_AFBCDEC_SURFACE_0, 1);
			VIOC_AFBCDec_SetContiDecEnable(pAFBC_Dec, 1);
			VIOC_AFBCDec_SetSurfaceN(pAFBC_Dec,
				VIOC_AFBCDEC_SURFACE_0, 1);
			VIOC_AFBCDec_SetIrqMask(pAFBC_Dec, 1, AFBCDEC_IRQ_ALL);
			VIOC_AFBCDec_TurnOn(pAFBC_Dec,
				VIOC_AFBCDEC_SWAP_DIRECT);
			afbc_dec_1st_cfg = 1;
		} else{
			VIOC_AFBCDec_SurfaceCfg(pAFBC_Dec, base_addr, fmt,
				width, height, 0, bSplitMode, bWideMode,
	...
	struct tcc_dp_device *pdp_data)
{
	...
	VIOC_DISP_GetSize(pdp_data->ddc_info.virt_addr,
		&lcd_width, &lcd_height);

	//pr_info("[INF][VIOC_I] %s: %d base:0x%x %dx%d offs:%d lcd:%d - %d\n",
	//	__func__, pdp_data->ddc_info.blk_num, base_addr,
	//	var->xres, var->yres, var->yoffset, lcd_width, lcd_height);
	...
	if (var->reserved[3] == 0) {
		tca_vioc_configure_AFBCDEC(afbc_dec_vioc_id, base_addr,
			pRDMA, pWDMA, fmt, blk_num, var->reserved[3], 1, 0,
			img_width, img_height);

		/* TCC803X Android - change UI layer to layer1, disable layer0
		 * , AFBCDec enable - prevent FIFO under-run
		 */
		...
		VIOC_RDMA_SetImageDisable(
			pdp_data->rdma_info[RDMA_FB].virt_addr);
		#endif
	}
	#endif
	...
	#endif
	{
			...
			|| defined(CONFIG_ARCH_TCC899X) \
			|| defined(CONFIG_ARCH_TCC901X)
		VIOC_WMIX_SetChromaKey(pWMIX, lcd_layer, chroma_en,
			chromaR, chromaG, chromaB, 0x3FF, 0x3FF, 0x3FF);
		#else
		VIOC_WMIX_SetChromaKey(pWMIX, lcd_layer, chroma_en,
			chromaR, chromaG, chromaB, 0xF8, 0xFC, 0xF8);
		#if defined(CONFIG_TCC_VIOC_DISP_PATH_INTERNAL_CS_YUV)
		VIOC_WMIX_SetChromaKey(pWMIX, lcd_layer, chroma_en,
			chromaY, chromaU, chromaV, 0xF8, 0xFC, 0xF8);
		#endif
		#endif
	}
	...
	/* temp pjj because fifo under have to enable rdma
	 * before display device on.
	 */
...
void tca_fb_rdma_transparent_active_var(
	unsigned int base_addr,
	struct fb_var_screeninfo *var_original,
	struct tcc_dp_device *pdp_data)
{
	unsigned int  fmt = 0;
	unsigned int lcd_width, lcd_height, img_width, img_height;
	...
	#endif
	{
			...
			|| defined(CONFIG_ARCH_TCC899X) \
			|| defined(CONFIG_ARCH_TCC901X)
		VIOC_WMIX_SetChromaKey(pWMIX, lcd_layer, 1,
			chromaR, chromaG, chromaB, 0, 0, 0);
		#else
		VIOC_WMIX_SetChromaKey(pWMIX, lcd_layer, 1,
			chromaR, chromaG, chromaB, 0, 0, 0);
		#if defined(CONFIG_TCC_VIOC_DISP_PATH_INTERNAL_CS_YUV)
		VIOC_WMIX_SetChromaKey(pWMIX, lcd_layer, 1,
			chromaY, chromaU, chromaV, 0, 0, 0);
		#endif
		#endif
	}
...
void tca_fb_rdma_active_var_rbase(struct tcc_dp_device *pdp_data)
{
	...
	VIOC_RDMA_SetImageRBase(pRDMA, __raw_readl(pRDMA+RDMABASE0),
		__raw_readl(pRDMA+RDMABASE1), __raw_readl(pRDMA+RDMABASE2));
	VIOC_RDMA_SetImage3DMode(pRDMA, rdma_3d_mode);
	VIOC_RDMA_SetImageEnable(pRDMA);
}
#endif

void tca_fb_sc_rdma_active_var(
	unsigned int base_addr,
	struct fb_var_screeninfo *var,
	struct tcc_dp_device *pdp_data)
{
	...
	#if 0
	pr_info("[INF][VIOC_I] %s: display order:%d block:%d base_addr:0x%x w:%d h:%d offset:%d lcd:%d - %d\n",
		__func__, pdp_data->DispOrder, pdp_data->ddc_info.blk_num,
		base_addr, var->xres, var->yres, var->yoffset,
		lcd_width, lcd_height);
	pr_info("[INF][VIOC_I] scaler num %d  l:%d r : %d u:%d d:%d\n",
		pdp_data->sc_num0, pResize->resize_left,
		pResize->resize_right, pResize->resize_up,
		pResize->resize_down);
	#endif
	...
	if (divide_data.enable) {
		...
		VIOC_RDMA_SetImageY2RMode(pRDMA, 0x2);
	} else {
		pSC = VIOC_SC_GetAddress(pdp_data->sc_num0);
	}
	...
	// default framebuffer
	VIOC_WMIX_SetPosition(pdp_data->wmixer_info.virt_addr,
		lcd_layer, lcd_pos_x, lcd_pos_y);
	...
	VIOC_RDMA_SetImageSize(pRDMA, img_width, img_height);
	VIOC_RDMA_SetImageOffset(pRDMA, fmt, var->xres_virtual);
	...
	if (var->reserved[3] != 0) {
		// BGR
		VIOC_RDMA_SetImageRGBSwapMode(pRDMA, 0x5);
	} else {
		// RGB
		VIOC_RDMA_SetImageRGBSwapMode(pRDMA, 0x0);
	}
	#endif
#else
	VIOC_RDMA_SetImageBase(pRDMA, base_addr, 0, 0);
	...
	#if defined(CONFIG_VIOC_AFBCDEC)
	tca_vioc_configure_AFBCDEC(afbc_dec_vioc_id, base_addr, pRDMA, pWDMA,
		fmt, blk_num, var->reserved[3], 1, 0, img_width, img_height);
	...
	unsigned int dest_y)
{
	...
	//pr_info("[INF][VIOC_I] %s: base:0x%x %dx%d offset:%d lcd:%d - %d\n",
	//	__func__, base_addr, var->xres, var->yres,
	//	var->yoffset, dest_x, dest_y);
	...
	if (scaler_filp) {
		scaler_filp->f_op->unlocked_ioctl(scaler_filp,
			TCC_SCALER_IOCTRL_KERENL, (unsigned long)&fbscaler);
	}

	return (unsigned int)fbscaler.dest_Yaddr;
}

void tca_fb_sc_m2m_rdma_active_var(
	unsigned int base_addr,
	struct fb_var_screeninfo *var,
	struct tcc_dp_device *pdp_data)
{
	...
	if (divide_data.enable && divide_data.count == 1) {
		...
		#if defined(CONFIG_TCC_OUTPUT_COLOR_SPACE_YUV) \
			|| defined(CONFIG_TCC_COMPOSITE_COLOR_SPACE_YUV)
		targetAddr = tca_fb_sc_m2m(base_addr, var,
			lcd_width, lcd_height);
		#else
...
unsigned int tca_fb_g2d_m2m(
	unsigned int base_addr,
	struct fb_var_screeninfo *var,
	unsigned int g2d_rotate)
{
	...
	if (g2d_buf_index) {
		// destination image address
		g2d_p.tgt0 = fb_g2d_pbuf1;
	} else {
		// destination image address
		g2d_p.tgt0 = fb_g2d_pbuf0;
	}
	...
	g2d_ioctl((struct file *)&g2d_filp, TCC_GRP_ROTATE_IOCTRL_KERNEL,
		(unsigned long)&g2d_p);

	return g2d_p.tgt0;
}

void tca_fb_sc_g2d_rdma_active_var(
	unsigned int base_addr,
	struct fb_var_screeninfo *var,
	struct tcc_dp_device *pdp_data)
{
	...
	if ((lcd_width * lcd_height) > (var->xres * var->yres)) {
		...
		targetAddr = tca_fb_sc_m2m(targetAddr, &tar_var,
			lcd_width, lcd_height);
	} else {
		targetAddr = tca_fb_sc_m2m(base_addr, var,
			lcd_width, lcd_height);
	...
	struct tcc_dp_device *pdp_data)
{
	...
	if ((!lcd_width) || (!lcd_height)) {
		dprintk("%s: err %dx%d bpp=%d LCD %dx%d\n", __func__,
			var->xres, var->yres, var->bits_per_pixel,
			lcd_width, lcd_height);
		return;
	}

	dprintk("%s %dx%d, bpp:%d\n", __func__,
		var->xres, var->yres, var->bits_per_pixel);
	...
	VIOC_SC_SetDstSize(pSC0, lcd_width, lcd_height);
	VIOC_SC_SetOutSize(pSC0, lcd_width, lcd_height);
	VIOC_SC_SetUpdate(pSC0);

	VIOC_SC_SetSrcSize(pSC1, img_width, img_height);
	VIOC_SC_SetDstSize(pSC1, lcd_width, lcd_height);
	VIOC_SC_SetOutSize(pSC1, lcd_width, lcd_height);
	VIOC_SC_SetUpdate(pSC1);

	//--------------------------------------
	// set RDMA for 1st UI
	//--------------------------------------
	VIOC_RDMA_SetImageUVIEnable(pdp_data->rdma_info[RDMA_FB].virt_addr,
		true);

	// size
	VIOC_RDMA_SetImageSize(pdp_data->rdma_info[RDMA_FB].virt_addr,
		img_width, img_height);

	// format
	VIOC_RDMA_SetImageFormat(pdp_data->rdma_info[RDMA_FB].virt_addr, ifmt);
	if (ifmt >= TCC_LCDC_IMG_FMT_444SEP
		&& ifmt <= TCC_LCDC_IMG_FMT_YUV422ITL1)	{
		VIOC_RDMA_SetImageY2REnable(
			pdp_data->rdma_info[RDMA_FB].virt_addr, true);
		/* Y2RMode Default 0 (Studio Color) */
		VIOC_RDMA_SetImageY2RMode(
			pdp_data->rdma_info[RDMA_FB].virt_addr, 0x2);
	} else {
		VIOC_RDMA_SetImageY2REnable(
			pdp_data->rdma_info[RDMA_FB].virt_addr, 0);
	}

	VIOC_RDMA_SetImageIntl(pdp_data->rdma_info[RDMA_FB].virt_addr,
		interlace_output);

	//offset
	VIOC_RDMA_SetImageOffset(pdp_data->rdma_info[RDMA_FB].virt_addr,
		ifmt, img_width);

	// alpha & chroma key color setting
	VIOC_RDMA_SetImageAlphaSelect(pdp_data->rdma_info[RDMA_FB].virt_addr,
		true);
	VIOC_RDMA_SetImageAlphaEnable(pdp_data->rdma_info[RDMA_FB].virt_addr,
		true);

	// Update base addr
	VIOC_RDMA_SetImageBase(pdp_data->rdma_info[RDMA_FB].virt_addr,
		base_addr, 0, 0);

	//--------------------------------------
	// set RDMA for 2nd UI
	//--------------------------------------
	VIOC_RDMA_SetImageUVIEnable(pdp_data->rdma_info[RDMA_3D].virt_addr,
		true);

	// size
	VIOC_RDMA_SetImageSize(pdp_data->rdma_info[RDMA_3D].virt_addr,
		img_width, img_height);
	...
	if (ifmt >= TCC_LCDC_IMG_FMT_444SEP
		&& ifmt <= TCC_LCDC_IMG_FMT_YUV422ITL1) {
		VIOC_RDMA_SetImageY2REnable(
			pdp_data->rdma_info[RDMA_3D].virt_addr, true);
		/* Y2RMode Default 0 (Studio Color) */
		VIOC_RDMA_SetImageY2RMode(
			pdp_data->rdma_info[RDMA_3D].virt_addr, 0x2);
	} else {
		VIOC_RDMA_SetImageY2REnable(
			pdp_data->rdma_info[RDMA_3D].virt_addr, 0);
	}

	VIOC_RDMA_SetImageIntl(pdp_data->rdma_info[RDMA_3D].virt_addr,
		interlace_output);

	//offset
	VIOC_RDMA_SetImageOffset(pdp_data->rdma_info[RDMA_3D].virt_addr,
		ifmt, img_width);

	// alpha & chroma key color setting
	VIOC_RDMA_SetImageAlphaSelect(pdp_data->rdma_info[RDMA_3D].virt_addr,
		true);
	VIOC_RDMA_SetImageAlphaEnable(pdp_data->rdma_info[RDMA_3D].virt_addr,
		true);

	// Update base addr
	VIOC_RDMA_SetImageBase(pdp_data->rdma_info[RDMA_3D].virt_addr,
		base_addr, 0, 0);
	...
	VIOC_WMIX_SetPosition(pdp_data->wmixer_info.virt_addr, 1, 0,
		lcd_height + iVBlank);

	#if defined(CONFIG_ARCH_TCC898X) || defined(CONFIG_ARCH_TCC899X) \
		|| defined(CONFIG_ARCH_TCC901X)
	VIOC_WMIX_SetBGColor(pdp_data->wmixer_info.virt_addr,
	...
	#if defined(CONFIG_ARCH_TCC898X) || defined(CONFIG_ARCH_TCC899X) \
		|| defined(CONFIG_ARCH_TCC901X)
	VIOC_WMIX_SetChromaKey(pdp_data->wmixer_info.virt_addr, 0, chroma_en,
		chromaR, chromaG, chromaB, 0x3FF, 0x3FF, 0x3FF);
	VIOC_WMIX_SetChromaKey(pdp_data->wmixer_info.virt_addr, 1, chroma_en,
		chromaR, chromaG, chromaB, 0x3FF, 0x3FF, 0x3FF);
	#else
	VIOC_WMIX_SetChromaKey(pdp_data->wmixer_info.virt_addr, 0, chroma_en,
		chromaR, chromaG, chromaB, 0xF8, 0xFC, 0xF8);
...
void tca_fb_attach_update(
	struct tcc_dp_device *pMdp_data,
	struct tcc_dp_device *pSdp_data)
{
	...
	VIOC_DISP_GetSize(pSdp_data->ddc_info.virt_addr,
		&img_width, &img_height);

	/* default framebuffer */
	VIOC_WMIX_SetPosition(pSdp_data->wmixer_info.virt_addr, nRDMA_Attached,
		0, 0);

	/* overlay setting */
	#if defined(CONFIG_ARCH_TCC898X) || defined(CONFIG_ARCH_TCC899X) \
		|| defined(CONFIG_ARCH_TCC901X)
	VIOC_WMIX_SetChromaKey(pSdp_data->wmixer_info.virt_addr, nRDMA_Attached,
		0, 0, 0, 0, 0x3FF, 0x3FF, 0x3FF);
	#else
	VIOC_WMIX_SetChromaKey(pSdp_data->wmixer_info.virt_addr, nRDMA_Attached,
		0, 0, 0, 0, 0xF8, 0xFC, 0xF8);
	#endif
	VIOC_RDMA_SetImageFormat(pSdp_data->rdma_info[nRDMA_Attached].virt_addr,
		TCC_LCDC_IMG_FMT_RGB888);
	VIOC_RDMA_SetImageOffset(pSdp_data->rdma_info[nRDMA_Attached].virt_addr,
		TCC_LCDC_IMG_FMT_RGB888, img_width);
	VIOC_RDMA_SetImageSize(pSdp_data->rdma_info[nRDMA_Attached].virt_addr,
		img_width, img_height);
	VIOC_RDMA_SetImageBase(pSdp_data->rdma_info[nRDMA_Attached].virt_addr,
		base_addr, 0, 0);

	VIOC_RDMA_SetImageAlphaSelect(
		pSdp_data->rdma_info[nRDMA_Attached].virt_addr, 0);
	...
	base_addr = attach_data.pbuf[attach_data.index];
	VIOC_WDMA_SetImageBase(pMdp_data->wdma_info.virt_addr,
		base_addr+attach_posx * 4 + img_width * attach_posy * 4,
		0, 0);
	...
	if (attach_data.update_started == 0) {
		...
		if (0)/*VIOC_CONFIG_DV_GET_EDR_PATH())*/ {
			...
			tcc_set_lut_csc_coeff(0x0DA706A4, 0x0F810FB4,
				0x0FF60488, 0x0F980FEC, 0x479); // 2020 to 709
			tcc_set_lut_plugin(nLut_component,
				pSdp_data->rdma_info[nRDMA_Attached].blk_num);
			tcc_set_lut_enable(nLut_component, true);
			pr_info("[INF][VIOC_I] ### DV-PLUG-LUT for attach : 0x%x\n",
				pSdp_data->rdma_info[nRDMA_Attached].blk_num);

			VIOC_RDMA_SetImageRGBSwapMode(
				pSdp_data->rdma_info[nRDMA_Attached].virt_addr,
				4);
		}
		#endif
...
static irqreturn_t tca_fb_attach_handler(int irq, void *dev_id)
{
	...
	if (!is_vioc_intr_activatied(attach_intr.vioc_intr->id,
		(1<<attach_intr.vioc_intr->bits))) {
		return IRQ_NONE;
	}
	...
	vioc_intr_clear(attach_intr.vioc_intr->id,
		(1<<attach_intr.vioc_intr->bits));
...
void tca_fb_attach_start(struct tccfb_info *info)
{
	...
	pr_info("[INF][VIOC_I] %s\n", __func__);

	/* set the buffer for attached output */
	if (pmap_get_info("output_attach", &pmap) < 0) {
		pr_info("[INF][VIOC_I] %s-%d : output_attach allocation is failed.\n",
			__func__, __LINE__);
		return;
	}

	if (pmap.size == 0)      {
		pr_info("[INF][VIOC_I] attach buffer cann't alloc from pmap :\n");
		return;
	}
	...
	vioc_intr_disable(pMdp_data->wdma_info.irq_num,
		attach_intr.vioc_intr->id, (1<<attach_intr.vioc_intr->bits));
	ret = request_irq(pMdp_data->wdma_info.irq_num, tca_fb_attach_handler,
		IRQF_SHARED, "TCC_WDMA", &attach_intr);
	vioc_intr_enable(pMdp_data->wdma_info.irq_num,
		attach_intr.vioc_intr->id, (1<<attach_intr.vioc_intr->bits));
	...
	for (i = 0; i < MAX_ATTACH_BUFF_CNT; i++) {
		...
		//pr_info("[INF][VIOC_I] %s-%d attach buffer[%d] = 0x%x\n",
		//	__func__, __LINE__, i, attach_data.pbuf[i]);
	}

	attach_wd -= (output_attach_resize_data.resize_left
		+ output_attach_resize_data.resize_right) * 8;
	attach_ht -= (output_attach_resize_data.resize_up
		+ output_attach_resize_data.resize_down) * 4;
	attach_posx = output_attach_resize_data.resize_left * 8;
	attach_posy = output_attach_resize_data.resize_up * 4;
	...
	VIOC_WDMA_SetImageBase(pWDMA,
		(unsigned int)attach_data.pbuf[0] + attach_posx * 4
		+ attach_wd*attach_posy * 4, 0, 0);
	...
	if (VIOC_CONFIG_DV_GET_EDR_PATH()
		...
		pr_info("[INF][VIOC_I] ### DV-ATTACH : %d => %dx%d -> %dx%d, 0x%x\n",
				attach_data.flag, main_wd, main_ht,
				attach_wd, attach_ht,
				attach_data.pbuf[0] + attach_posx*4
					+ attach_wd * attach_posy * 4);

		if (vioc_get_path_type() == DV_PATH_DIRECT_VIN_WDMA) {
			VIOC_DISP_TurnOnOff_With_DV(
				pMdp_data->ddc_info.virt_addr, 1);
		}
	}
#endif
...
int tca_fb_attach_stop(struct tccfb_info *info)
{
	...
	if (attach_data.plugin) {
		if (VIOC_CONFIG_PlugOut(scaler_num)
			!= VIOC_PATH_DISCONNECTED) {
			if (pMdp_data->DispNum) {
				VIOC_CONFIG_SWReset(scaler_num,
					VIOC_CONFIG_RESET);
				VIOC_CONFIG_SWReset(scaler_num,
					VIOC_CONFIG_CLEAR);
				VIOC_CONFIG_SWReset(VIOC_WDMA01,
					VIOC_CONFIG_RESET);
				VIOC_CONFIG_SWReset(VIOC_WDMA01,
					VIOC_CONFIG_CLEAR);
			} else {
				...
				VIOC_CONFIG_SWReset(scaler_num,
					VIOC_CONFIG_CLEAR);
				VIOC_CONFIG_SWReset(VIOC_WDMA00,
					VIOC_CONFIG_RESET);
				VIOC_CONFIG_SWReset(VIOC_WDMA00,
					VIOC_CONFIG_CLEAR);
			}
...
#ifdef CONFIG_DIRECT_MOUSE_CTRL
unsigned char tca_fb_mouse_data_out(
	unsigned int data_in,
	unsigned char yuv,
	unsigned char threshold_use)
{
...
void tca_fb_mouse_set_icon(tcc_mouse_icon *mouse_icon)
{
	...
	/* 0: HDMI, 1: DVI */
	hdmi_mode = (mouse_icon->option & 0x00ff) >> 0;
	/* 1: HDMI, 2: CVBS, 3:Component */
	output_mode = (mouse_icon->option & 0xff00) >> 8;
	...
	if (output_mode == OUTPUT_COMPOSITE) {
		...
		for (i = 0; i < mouse_icon->height; i++) {
			for (j = 0; j < mouse_icon->width; j++) {
				...
				wY_tmp = (wYADDR << 9) + (wYMULR * wR)
					+ (wYMULG * wG) + (wYMULB * wB);
				wU_tmp = (wUADDR << 9) - (wUMULR * wR)
					- (wUMULG * wG) + (wUMULB * wB);
				wV_tmp = (wVADDR << 9) + (wVMULR * wR)
					- (wVMULG * wG) - (wVMULB * wB);

				if (output_mode == OUTPUT_COMPOSITE) {
					wY = tca_fb_mouse_data_out(wY_tmp,
						0, 1);
				} else {
					wY = tca_fb_mouse_data_out(wY_tmp,
						0, 0);
			...
			#else
			for (k = 0;
				k < (mouse_cursor_width - mouse_icon->width);
				k++) {
			#endif
				*(dst+3) = 0x00; // Alpha
				*(dst+2) = 0x00; // Y
	...
	struct tcc_dp_device *pdp_data)
{
	...
	unsigned int lcd_width, lcd_height, lcd_w_pos, lcd_h_pos;
	...
	if ((pDISPBase == NULL) || (pWMIXBase == NULL) || (pRDMABase == NULL)) {
		//pr_err("%s: VIOC is not valid(R:0x%08x W:0x%08x D:0x%08x)\n",
	...
	VIOC_RDMA_SetImageOffset(pRDMABase, TCC_LCDC_IMG_FMT_RGB888,
		mouse_data.width);
	...
	#if !defined(CONFIG_TCC_M2M_USE_INTERLACE_OUTPUT)
	if (interlace_output) {
		VIOC_WMIX_SetPosition(pWMIXBase, wmix_channel,
			lcd_w_pos, (lcd_h_pos / 2));
	} else {
		VIOC_WMIX_SetPosition(pWMIXBase, wmix_channel,
			lcd_w_pos, lcd_h_pos);
	}
	...
	#if defined(CONFIG_TCC_VIOCMG)
	viocmg_set_wmix_ovp(VIOCMG_CALLERID_FB, pdp_data->wmixer_info.blk_num,
		8);
	#else
	...
	// image address
	if (mouse_data.index) {
		VIOC_RDMA_SetImageBase(pRDMABase,
			(unsigned int)mouse_data.pbuf1, 0, 0);
	} else {
		VIOC_RDMA_SetImageBase(pRDMABase,
			(unsigned int)mouse_data.pbuf2, 0, 0);
	}
	...
	VIOC_RDMA_SetImageEnable(pRDMABase);
		...
		&& !defined(CONFIG_TCC_DISPLAY_MODE_AUTO_DETECT)
	tca_fb_attach_update_flag();
	#endif
...
void tca_fb_resize_set_value(
	tcc_display_resize resize_value,
	TCC_OUTPUT_TYPE output)
{
	pr_info("[INF][VIOC_I] %s : resize_up = %d, resize_down = %d, resize_left = %d, resize_right = %d\n",
		__func__, resize_value.resize_up, resize_value.resize_down,
		resize_value.resize_left, resize_value.resize_right);

	#ifdef CONFIG_PRESENTATION_SECONDAY_DISPLAY_RESIZE_STB
	if ((output == TCC_OUTPUT_COMPONENT)
		|| (output == TCC_OUTPUT_COMPOSITE)) {
		memcpy((void *)&secondary_display_resize_data,
			(void *)&resize_value, sizeof(tcc_display_resize));
	} else
	#endif
	{
		memcpy((void *)&resize_data, (void *)&resize_value,
			sizeof(tcc_display_resize));
	}
}

void tca_fb_output_attach_resize_set_value(
	tcc_display_resize resize_value)
{
	...
	memcpy((void *)&output_attach_resize_data, (void *)&resize_value,
		sizeof(tcc_display_resize));
}

int tca_fb_divide_set_mode(
	struct tcc_dp_device *pdp_data,
	char enable,
	char mode)
{
	...
	if (enable) {
		...
		VIOC_CONFIG_SWReset(pdp_data->rdma_info[RDMA_3D].blk_num,
			VIOC_CONFIG_RESET);
		VIOC_CONFIG_SWReset(pdp_data->rdma_info[RDMA_3D].blk_num,
			VIOC_CONFIG_CLEAR);
		VIOC_CONFIG_PlugIn(pdp_data->sc_num1,
			pdp_data->rdma_info[RDMA_3D].blk_num);
	} else {
		pdp_data->FbUpdateType = divide_data.fbtype;

		VIOC_RDMA_SetImageDisable(
			pdp_data->rdma_info[RDMA_3D].virt_addr);

		//scaler plug in status check
		if (VIOC_CONFIG_GetScaler_PluginToRDMA(
			pdp_data->rdma_info[RDMA_3D].blk_num) != -1
		) {
			pr_info("[INF][VIOC_I] Scaler-%d plug out\n",
				get_vioc_index(pdp_data->sc_num1));
			VIOC_CONFIG_PlugOut(pdp_data->sc_num1);
			VIOC_CONFIG_SWReset(pdp_data->sc_num1,
				VIOC_CONFIG_RESET);
			VIOC_CONFIG_SWReset(pdp_data->sc_num1,
				VIOC_CONFIG_CLEAR);
		}
	...
	pr_info("[INF][VIOC_I] %s, enable=%d, mode=%d, fbtype=%d\n",
	...
	struct tcc_lcdc_image_update *ImageInfo)
{
	...
	if ((pdp_data == NULL) || (ImageInfo == NULL)) {
		pr_err("[ERR][VIOC_I] %s pdp_data:%p ImageInfo:%p\n",
	...
	if (ImageInfo->private_data.optional_info[VID_OPT_HAVE_DOLBYVISION_INFO]
		!= 0) {
		return;
	}
#endif
	...
	if (ImageInfo->on_the_fly) {
		iSCType = tca_get_scaler_num(pdp_data->DispDeviceType,
		...
		if (VIOC_CONFIG_GetScaler_PluginToRDMA(
			...
			if (((ImageInfo->crop_right - ImageInfo->crop_left)
			...
			#endif
			{
				...
				VIOC_RDMA_SetImageDisable(
					pdp_data->rdma_info[
					ImageInfo->Lcdc_layer].virt_addr);
			...
			if (ImageInfo->private_data.optional_info[
				VID_OPT_HAVE_MC_INFO] != 0
			) {
				if (VIOC_CONFIG_DMAPath_Support()) {
					int component_num =
						VIOC_CONFIG_DMAPath_Select(
						pdp_data->rdma_info[
						ImageInfo->Lcdc_layer].blk_num);

					if ((component_num < VIOC_MC0) ||
						(component_num >
						(VIOC_MC0 + VIOC_MC_MAX))
					) {
						pr_info("[INF][VIOC_I] %s Map Converter-%d\n",
							__func__,
							nDeCompressor_Main);

						VIOC_CONFIG_DMAPath_UnSet(
							component_num);
			...
			if (ImageInfo->private_data.optional_info[
				...
				if (VIOC_CONFIG_DMAPath_Support()) {
					...
					if ((component_num < VIOC_DTRC0) ||
						...
						pr_info("[INF][VIOC_I] %s DTRC converter-%d\n",
							__func__,
							nDeCompressor_Main);
		...
		if (VIOC_CONFIG_GetScaler_PluginToRDMA(
			...
			iSCType =
			(unsigned int)VIOC_CONFIG_GetScaler_PluginToRDMA(
			pdp_data->rdma_info[ImageInfo->Lcdc_layer].blk_num);
			...
			if (VIOC_CONFIG_DMAPath_Support()) {
				...
				pr_info("[INF][VIOC_I] %s Scaler-%d is plug out component_num:%x RDMA:%d\n",
					__func__,
				...
				if ((component_num >= VIOC_MC0) &&
					...
					tca_map_convter_onoff(component_num,
						0, 1);
					VIOC_CONFIG_DMAPath_UnSet(
						component_num);
				...
				if ((component_num >= VIOC_DTRC0) &&
					...
					tca_dtrc_convter_onoff(component_num,
						0, 1);
					VIOC_CONFIG_DMAPath_UnSet(
						component_num);
					// disable it to prevent system-hang
				...
				if ((component_num < VIOC_RDMA00) &&
					(component_num >
					(VIOC_RDMA00 + VIOC_RDMA_MAX))
				) {
					void __iomem *pVRDMA =
						VIOC_RDMA_GetAddress(
						component_num);
				...
				VIOC_CONFIG_DMAPath_Set(pdp_data->rdma_info[
					ImageInfo->Lcdc_layer].blk_num,
					pdp_data->rdma_info[
					ImageInfo->Lcdc_layer].blk_num);
			} else {
			...
			VIOC_RDMA_SetImageDisable(
			pdp_data->rdma_info[ImageInfo->Lcdc_layer].virt_addr);

			//VIOC_CONFIG_SWReset(pdp_data->rdma_info[
			//	ImageInfo->Lcdc_layer].blk_num,
			//	VIOC_CONFIG_RESET);
			//VIOC_CONFIG_SWReset(pdp_data->rdma_info[
			//	ImageInfo->Lcdc_layer].blk_num,
...
int tca_edr_path_configure(void)
{
	...
	if (vioc_v_dv_get_stage() != DV_OFF) {
		...
		VIOC_DISP_GetSize(pdp_data->ddc_info.virt_addr,
			&lcd_width, &lcd_height);
		lcd_width -= (pResize->resize_left + pResize->resize_right)
			* 8;
		lcd_height -= (pResize->resize_up + pResize->resize_down) * 4;
		lcd_pos_x = pResize->resize_left * 8;
		lcd_pos_y = pResize->resize_up * 4;
		...
		VIOC_DISP_GetSize(VIOC_DISP_GetAddress(0),
			&lcd_width, &lcd_height);

		//msleep(100);//Temp!!
		VIOC_V_DV_Base_Configure(lcd_pos_x, lcd_pos_y,
			lcd_width, lcd_height);
		...
		pr_info("[INF][VIOC_I] ### V_DV 1st Configuration :: (0x%x - 0x%x)\n",
	...
	unsigned int *ratio)
{
	...
	if (Src_ImageInfo->enable == 0) {
		...
		if (Src_ImageInfo->private_data.dolbyVision_info.el_offset[0]
			== 0x00) {
			DV_PROC_CHECK = 0;
			dvprintk("%s-%d : DV_PROC_CHECK(%d)\n",
				__func__, __LINE__, DV_PROC_CHECK);
		...
		if (Src_ImageInfo->private_data.dolbyVision_info.el_frame_width
			== 0) {
			pr_info("[INF][VIOC_I] ^@New^^^^^^^^^^^^^ @@@ %d/%d, %03d :: TS: %04d  %d bpp #BL(0x%x, %dx%d (%dx%d), 0x%x fmt) #EL(0x%x, %dx%d (%dx%d)) #OSD(0x%x/0x%x) #Reg(0x%x) #Meta(0x%x)\n",
				0, 0,
				Src_ImageInfo->private_data.optional_info[
					VID_OPT_BUFFER_ID],
				Src_ImageInfo->private_data.optional_info[
					VID_OPT_TIMESTAMP],
				Src_ImageInfo->private_data.optional_info[
					VID_OPT_BIT_DEPTH],
				Src_ImageInfo->addr0,
				Src_ImageInfo->Frame_width,
				Src_ImageInfo->Frame_height,
				Src_ImageInfo->crop_right
					- Src_ImageInfo->crop_left,
				Src_ImageInfo->crop_bottom
					- Src_ImageInfo->crop_top,
				Src_ImageInfo->fmt,
		Src_ImageInfo->private_data.dolbyVision_info.el_offset[0],
		Src_ImageInfo->private_data.dolbyVision_info.el_buffer_width,
		Src_ImageInfo->private_data.dolbyVision_info.el_buffer_height,
		Src_ImageInfo->private_data.dolbyVision_info.el_frame_width,
		Src_ImageInfo->private_data.dolbyVision_info.el_frame_height,
		Src_ImageInfo->private_data.dolbyVision_info.osd_addr[0],
		Src_ImageInfo->private_data.dolbyVision_info.osd_addr[1],
		Src_ImageInfo->private_data.dolbyVision_info.reg_addr,
		Src_ImageInfo->private_data.dolbyVision_info.md_hdmi_addr);
		}

		*ratio = ((Src_ImageInfo->crop_right -
			Src_ImageInfo->crop_left) /
		Src_ImageInfo->private_data.dolbyVision_info.el_frame_width);
		...
		// buffer resolution
		El_ImageInfo->Frame_width
		= El_ImageInfo->private_data.dolbyVision_info.el_buffer_width;
		El_ImageInfo->Frame_height
		= El_ImageInfo->private_data.dolbyVision_info.el_buffer_height;

		// display-region
		El_ImageInfo->Image_width
			= Src_ImageInfo->Image_width / (*ratio);
		El_ImageInfo->Image_height
			= Src_ImageInfo->Image_height / (*ratio);

		dvprintk("%s(%d) EL Disp-W(%d = %d/%d) ratio(%d = (%d-%d)/%d\n",
			__func__, __LINE__,
			El_ImageInfo->Image_width, Src_ImageInfo->Image_width,
			*ratio, *ratio,
			Src_ImageInfo->crop_right, Src_ImageInfo->crop_left,
		Src_ImageInfo->private_data.dolbyVision_info.el_frame_width);

		// crop
		El_ImageInfo->crop_left = El_ImageInfo->crop_top = 0;
		El_ImageInfo->crop_right
		= El_ImageInfo->private_data.dolbyVision_info.el_frame_width;
		El_ImageInfo->crop_bottom
		= El_ImageInfo->private_data.dolbyVision_info.el_frame_height;

		// buffer address
		El_ImageInfo->addr0
		= El_ImageInfo->private_data.dolbyVision_info.el_offset[0];
		El_ImageInfo->addr1
		= El_ImageInfo->private_data.dolbyVision_info.el_offset[1];
		El_ImageInfo->addr2
		= El_ImageInfo->private_data.dolbyVision_info.el_offset[2];

		// no map-converter usage
		El_ImageInfo->private_data.optional_info[VID_OPT_HAVE_MC_INFO]
			= 0;
...
void tca_edr_el_display_update(
	struct tcc_dp_device *pdp_data,
	struct tcc_lcdc_image_update *ImageInfo)
{
	void __iomem *pSC;
	...
	dvprintk("%d/%d %03d/%03d TS:%04ld %dbpp BL(0x%x, %dx%d-%dx%d 0x%x)\n",
		bStep_Check, DEF_DV_CHECK_NUM, nFrame,
		ImageInfo->private_data.optional_info[VID_OPT_BUFFER_ID],
		ImageInfo->private_data.optional_info[VID_OPT_TIMESTAMP],
		...
		ImageInfo->fmt,
	dvprintk("EL(0x%x, %dx%d (%dx%d)) OSD(0x%x/0x%x) Reg:0x%x Meta:0x%x\n",
		ImageInfo->private_data.dolbyVision_info.el_offset[0],
		ImageInfo->private_data.dolbyVision_info.el_frame_width,
		ImageInfo->private_data.dolbyVision_info.el_frame_height,
		ImageInfo->private_data.dolbyVision_info.el_buffer_width,
		ImageInfo->private_data.dolbyVision_info.el_buffer_height,
		ImageInfo->private_data.dolbyVision_info.osd_addr[0],
		ImageInfo->private_data.dolbyVision_info.osd_addr[1],
		ImageInfo->private_data.dolbyVision_info.reg_addr,
		ImageInfo->private_data.dolbyVision_info.md_hdmi_addr);


	if (tca_edr_el_configure(ImageInfo, &el_ImageInfo, &ratio) < 0) {
		return;
		/* prevent KCS warning */
	}

	//pr_info("[INF][VIOC_I] %s enable:%d layer:%d addr:0x%x ts:%d fmt:%d
	//Fw:%d Fh:%d Iw:%d Ih:%d fmt:%d onthefly:%d map:%d\n",
	//	__func__, el_ImageInfo.enable, el_ImageInfo.Lcdc_layer,
	//	el_ImageInfo.addr0, el_ImageInfo.time_stamp, el_ImageInfo.fmt,
	//	el_ImageInfo.Frame_width, el_ImageInfo.Frame_height,
	//	el_ImageInfo.Image_width, el_ImageInfo.Image_height,
	//	el_ImageInfo.fmt, el_ImageInfo.on_the_fly,
	//	el_ImageInfo.private_data.optional_info[VID_OPT_HAVE_MC_INFO]);

	if ((el_ImageInfo.Lcdc_layer >= RDMA_MAX_NUM)
		|| (el_ImageInfo.fmt > TCC_LCDC_IMG_FMT_MAX)) {
		pr_err("[ERR][VIOC_I] LCD :: lcdc:%d, enable:%d, layer:%d, addr:0x%x, fmt:%d, Fw:%d, Fh:%d, Iw:%d, Ih:%d, fmt:%d onthefly:%d\n",
		       ...
		       el_ImageInfo.enable, el_ImageInfo.Lcdc_layer,
		       el_ImageInfo.addr0, el_ImageInfo.fmt,
		       el_ImageInfo.Frame_width, el_ImageInfo.Frame_height,
		       el_ImageInfo.Image_width, el_ImageInfo.Image_height,
		       el_ImageInfo.fmt, el_ImageInfo.on_the_fly);
		return;
	...
	if (!el_ImageInfo.enable) {
		...
		if (VIOC_CONFIG_DMAPath_Support()) {
			int component_num = VIOC_CONFIG_DMAPath_Select(
				pdp_data->rdma_info[
				el_ImageInfo.Lcdc_layer].blk_num);
			...
			if (component_num < 0) {
				...
				void __iomem *pVRDMA =
					VIOC_RDMA_GetAddress(component_num);
				VIOC_RDMA_SetImageDisable(pVRDMA);
				VIOC_CONFIG_DMAPath_UnSet(component_num);
			}
			...
			VIOC_CONFIG_DMAPath_Set(
			pdp_data->rdma_info[el_ImageInfo.Lcdc_layer].blk_num,
			pdp_data->rdma_info[el_ImageInfo.Lcdc_layer].blk_num);
		}

		VIOC_RDMA_SetImageDisable(
			pdp_data->rdma_info[el_ImageInfo.Lcdc_layer].virt_addr);
	...
	if ((!lcd_width) || (!lcd_height)) {
		pr_err("[ERR][VIOC_I] %s LCD :: Error :: lcd_width %d, lcd_height %d\n",
	...
	if (el_ImageInfo.on_the_fly) {
		...
		VIOC_SC_SetDstSize(pSC, el_ImageInfo.Image_width,
			el_ImageInfo.Image_height);
		VIOC_SC_SetOutSize(pSC, el_ImageInfo.Image_width,
			el_ImageInfo.Image_height);
		...
		if (!(__raw_readl(pDISP+DCTRL) & DCTRL_NI_MASK)) {
			...
			if (((el_ImageInfo.crop_right - el_ImageInfo.crop_left)
				== el_ImageInfo.Image_width)
				&& ((el_ImageInfo.crop_bottom
					- el_ImageInfo.crop_top)
					== el_ImageInfo.Image_height)) {
				VIOC_SC_SetBypass(pSC, ON);
		...
		if (VIOC_CONFIG_GetScaler_PluginToRDMA(
			pdp_data->rdma_info[el_ImageInfo.Lcdc_layer].blk_num)
			== -1) {
			VIOC_RDMA_SetImageDisable(pdp_data->rdma_info[
				el_ImageInfo.Lcdc_layer].virt_addr);
			...
			VIOC_CONFIG_PlugIn(iSCType, pdp_data->rdma_info[
				el_ImageInfo.Lcdc_layer].blk_num);
		}
	} else {
		if (VIOC_CONFIG_GetScaler_PluginToRDMA(
	...
	if (VIOC_CONFIG_GetScaler_PluginToRDMA(
		pdp_data->rdma_info[el_ImageInfo.Lcdc_layer].blk_num) != -1) {
		VIOC_SC_SetUpdate(pSC);
	}
	...
	VIOC_WMIX_SetPosition(pdp_data->wmixer_info.virt_addr,
		el_ImageInfo.Lcdc_layer,
		el_ImageInfo.offset_x, el_ImageInfo.offset_y);
	VIOC_WMIX_SetUpdate(pdp_data->wmixer_info.virt_addr);

	//map converter
	#ifdef CONFIG_VIOC_MAP_DECOMP
	if (el_ImageInfo.private_data.optional_info[VID_OPT_HAVE_MC_INFO]
		!= 0
	) {
		if (VIOC_CONFIG_DMAPath_Support()) {
			int component_num = VIOC_CONFIG_DMAPath_Select(
	...
	if (ImageInfo->private_data.optional_info[VID_OPT_HAVE_DTRC_INFO]
		...
		tca_dtrc_convter_set(VIOC_DTRC0 + nDeCompressor_Main,
			&el_ImageInfo, 1);
	} else
	#endif
	{
		...
		if (el_ImageInfo.fmt == TCC_LCDC_IMG_FMT_COMP) {
		...
		#endif
		else if (el_ImageInfo.fmt >= TCC_LCDC_IMG_FMT_444SEP
			&& el_ImageInfo.fmt <= TCC_LCDC_IMG_FMT_YUV422ITL1) {
			#if defined(CONFIG_TCC_VIOC_DISP_PATH_INTERNAL_CS_YUV)
			VIOC_RDMA_SetImageY2REnable(
			pdp_data->rdma_info[el_ImageInfo.Lcdc_layer].virt_addr,
			false);
			...
			VIOC_RDMA_SetImageY2RMode(
			pdp_data->rdma_info[el_ImageInfo.Lcdc_layer].virt_addr,
			0x2); /* Y2RMode Default 0 (Studio Color) */
		...
		if (el_ImageInfo.one_field_only_interlace
			...
			|| (el_ImageInfo.private_data.optional_info[
				VID_OPT_BIT_DEPTH] == 10)
		#endif
		)		{
			VIOC_RDMA_SetImageOffset(pdp_data->rdma_info[
				el_ImageInfo.Lcdc_layer].virt_addr,
				el_ImageInfo.fmt,
				el_ImageInfo.Frame_width * 2);
		}
		#ifdef CONFIG_VIOC_10BIT
		else if (el_ImageInfo.private_data.optional_info[
			VID_OPT_BIT_DEPTH] == 11) {
			VIOC_RDMA_SetImageOffset(pdp_data->rdma_info[
				el_ImageInfo.Lcdc_layer].virt_addr,
				el_ImageInfo.fmt,
				el_ImageInfo.Frame_width * 125 / 100);
		}
		#endif
		else {
			VIOC_RDMA_SetImageOffset(pdp_data->rdma_info[
				el_ImageInfo.Lcdc_layer].virt_addr,
				el_ImageInfo.fmt, el_ImageInfo.Frame_width);
		}

		#ifdef CONFIG_VIOC_10BIT
		if (el_ImageInfo.private_data.optional_info[VID_OPT_BIT_DEPTH]
			== 10) {
		...
		VIOC_RDMA_SetImageSize(
			pdp_data->rdma_info[el_ImageInfo.Lcdc_layer].virt_addr,
			el_ImageInfo.crop_right - el_ImageInfo.crop_left,
			el_ImageInfo.crop_bottom - el_ImageInfo.crop_top);

		if (!(((el_ImageInfo.crop_left == 0) &&
			(el_ImageInfo.crop_right == el_ImageInfo.Frame_width))
			&&  ((el_ImageInfo.crop_top == 0)
			&& (el_ImageInfo.crop_bottom ==
				el_ImageInfo.Frame_height)))) {
			...
			dprintk("Crop left:%d right:%d top:%d bot:%d\n",
				el_ImageInfo.crop_left,
				el_ImageInfo.crop_right,
				el_ImageInfo.crop_top,
				el_ImageInfo.crop_bottom);

			tccxxx_GetAddress(el_ImageInfo.fmt,
				(unsigned int)el_ImageInfo.addr0,
				el_ImageInfo.Frame_width,
				el_ImageInfo.Frame_height,
				el_ImageInfo.crop_left, el_ImageInfo.crop_top,
				&addr_Y, &addr_U, &addr_V);

			if (el_ImageInfo.one_field_only_interlace) {
				VIOC_RDMA_SetImageSize(pdp_data->rdma_info[
					el_ImageInfo.Lcdc_layer].virt_addr,
					el_ImageInfo.crop_right
						- el_ImageInfo.crop_left,
					(el_ImageInfo.crop_bottom
						- el_ImageInfo.crop_top) / 2);
			} else {
				VIOC_RDMA_SetImageSize(pdp_data->rdma_info[
					el_ImageInfo.Lcdc_layer].virt_addr,
					el_ImageInfo.crop_right
			...
			VIOC_RDMA_SetImageBase(pdp_data->rdma_info[
				el_ImageInfo.Lcdc_layer].virt_addr,
				addr_Y, addr_U, addr_V);
		} else {
			...
			VIOC_RDMA_SetImageBase(pdp_data->rdma_info[
				el_ImageInfo.Lcdc_layer].virt_addr,
				el_ImageInfo.addr0, el_ImageInfo.addr1,
				el_ImageInfo.addr2);
		}
		...
		if (VIOC_CONFIG_DV_GET_EDR_PATH()) {
			...
			if (el_ImageInfo.Lcdc_layer == RDMA_VIDEO_SUB) {
			/*|| el_ImageInfo.Lcdc_layer == RDMA_LASTFRM) {*/
				if (el_ImageInfo.Lcdc_layer == RDMA_LASTFRM
					&& (el_ImageInfo.fmt
					>= TCC_LCDC_IMG_FMT_444SEP
					&& el_ImageInfo.fmt
				...
				VIOC_V_DV_SetSize(NULL, pdp_data->rdma_info[
					el_ImageInfo.Lcdc_layer].virt_addr,
					el_ImageInfo.offset_x/ratio,
					el_ImageInfo.offset_y/ratio,
...
extern void VIOC_RDMA_SetImageUpdate_for_CertiTest(
	void __iomem *reg,
	unsigned int sw,
	unsigned int sh,
	unsigned int nBase0);
...
void tca_edr_vioc_set(
	unsigned int nRDMA,
	void __iomem *pRDMA,
	struct dolby_layer_str_t stDolby_Layer,
	struct tcc_dp_device *pdp_data,
	struct tcc_lcdc_image_update *ImageInfo,
	unsigned int width,
	unsigned int height,
	enum DV_DISP_TYPE type,
	unsigned int el_ratio)
{
	...
	if (type == EDR_BL &&
		...
		#if defined(CONFIG_TCC_VIOC_DISP_PATH_INTERNAL_CS_YUV)
		//tca_map_convter_set(VIOC_MC0 + pdp_data->ddc_info.blk_num,
		//	ImageInfo, 0);
		#else
		//tca_map_convter_set(VIOC_MC0 + pdp_data->ddc_info.blk_num,
		//	ImageInfo, 1);
	...
	#endif
	{
		...
		if (stDolby_Layer.bit10) {
			/* YUV 10bit(16bit) support */
			VIOC_RDMA_SetDataFormat(pRDMA, 0x1, 0x1);
			VIOC_RDMA_SetImageOffset(pRDMA, stDolby_Layer.format,
				stDolby_Layer.buffer_width*2);
		} else
		#endif
		{
			/* YUV 8bit support */
			VIOC_RDMA_SetDataFormat(pRDMA, 0x0, 0x0);
			VIOC_RDMA_SetImageOffset(pRDMA, stDolby_Layer.format,
				stDolby_Layer.buffer_width);
		}

		VIOC_RDMA_SetImageFormat(pRDMA, stDolby_Layer.format);
		VIOC_RDMA_SetImageSize(pRDMA, stDolby_Layer.frame_width,
			stDolby_Layer.frame_height);
		VIOC_RDMA_SetImageBase(pRDMA, stDolby_Layer.offset[0],
			stDolby_Layer.offset[1], stDolby_Layer.offset[2]);
		...
		if (ImageInfo->Lcdc_layer == RDMA_FB) {
			VIOC_RDMA_SetImageUpdate_for_CertiTest(pRDMA,
				stDolby_Layer.frame_width,
				stDolby_Layer.frame_height,
				stDolby_Layer.offset[0]);
		} else
		#endif
		{
			VIOC_RDMA_SetImageUpdate(pRDMA);
	...
	VIOC_SC_SetDstSize(pSC, target_width, target_height);
	VIOC_SC_SetOutSize(pSC, target_width, target_height);
...
static unsigned int nINT_Total, nDisp_Try, nDisp_Proc, nDV_Updated;
static unsigned int nCopyed_Max;
...
void tca_edr_display_update(
	struct tcc_dp_device *pdp_data,
	struct tcc_lcdc_image_update *ImageInfo)
{
	void __iomem *pRDMA = NULL;
	unsigned int nRDMA = 0;
	struct dolby_layer_str_t stDolby_BL, stDolby_EL;
	struct dolby_layer_str_t stDolby_OSD, stDolby_OSD_1;
	unsigned int ratio = 1;
	...
	if (!ImageInfo->enable) {
		...
		tca_edr_vioc_set(nRDMA, pRDMA, stDolby_BL, pdp_data, ImageInfo,
			Hactive, Vactive, EDR_BL, ratio);
		...
		nRDMA = pdp_data->rdma_info[ImageInfo->Lcdc_layer].blk_num;
		pRDMA = pdp_data->rdma_info[ImageInfo->Lcdc_layer].virt_addr;
		tca_edr_vioc_set(nRDMA, pRDMA, stDolby_EL, pdp_data, ImageInfo,
			Hactive, Vactive, EDR_EL, ratio);

		pr_info("[INF][VIOC_I] %s @ :: RDMA for EDR is diabled\n",
			__func__);

		ImageInfo->Lcdc_layer = RDMA_FB1;
		nRDMA = pdp_data->rdma_info[ImageInfo->Lcdc_layer].blk_num;
		pRDMA = pdp_data->rdma_info[ImageInfo->Lcdc_layer].virt_addr;
		tca_edr_vioc_set(nRDMA, pRDMA, stDolby_OSD_1, pdp_data,
			ImageInfo, Hactive, Vactive,
			/*EDR_OSD1*/RDMA_FB1, ratio);

		pr_info("[INF][VIOC_I] %s @ :: Finish Proc => nINT_Total(%03d) nDisp_Try(%03d) nDisp_Proc(%03d) nDV_Updated(%03d)\n",
			__func__, nINT_Total, nDisp_Try,
			nDisp_Proc, nDV_Updated);
		...
		for (i = 0; i < nCopyed_Max; i++) {
			if (nAccumulated_ImageInfo[i].private_data.dolbyVision_info.osd_addr[0] != USE_OWN_FB) {
				if (i != nAccumulated_ImageInfo[i].private_data.optional_info[VID_OPT_BUFFER_ID]) {
					pr_info("[INF][VIOC_I] ^@ ID[%03d] Gap[---] :######################: is not displayed!!!!\n",
						i);
				} else {
					pr_info("[INF][VIOC_I] ^@ ID[%03d] Gap[%03d] F[%01d/%01d] TS[%04ld/%04ld] %d bpp #Type[%d/%d] #BL(MC(%d), 0x%x, %dx%d (%dx%d), 0x%x fmt) #EL(0x%x, %dx%d (%dx%d)) #OSD(0x%x/0x%x) #Reg(0x%x) #Meta(0x%x)\n",
						nAccumulated_ImageInfo[i].private_data.optional_info[VID_OPT_BUFFER_ID], nAccumulated_ImageInfo[i].private_data.optional_info[VID_OPT_RESERVED_1],
						...
						nAccumulated_ImageInfo[i].time_stamp, nAccumulated_ImageInfo[i].sync_time, nAccumulated_ImageInfo[i].private_data.optional_info[VID_OPT_BIT_DEPTH],
						vioc_get_out_type(), nAccumulated_ImageInfo[i].private_data.dolbyVision_info.reg_out_type, nAccumulated_ImageInfo[i].private_data.optional_info[VID_OPT_HAVE_MC_INFO],
						...
						nAccumulated_ImageInfo[i].private_data.dolbyVision_info.el_offset[0], nAccumulated_ImageInfo[i].private_data.dolbyVision_info.el_frame_width, nAccumulated_ImageInfo[i].private_data.dolbyVision_info.el_frame_height,
						nAccumulated_ImageInfo[i].private_data.dolbyVision_info.el_buffer_width, nAccumulated_ImageInfo[i].private_data.dolbyVision_info.el_buffer_height, nAccumulated_ImageInfo[i].private_data.dolbyVision_info.osd_addr[0],
						nAccumulated_ImageInfo[i].private_data.dolbyVision_info.osd_addr[1], nAccumulated_ImageInfo[i].private_data.dolbyVision_info.reg_addr, nAccumulated_ImageInfo[i].private_data.dolbyVision_info.md_hdmi_addr);
				}
			}
		}
		...
		nCopyed_Max = 0;
		for (i = 0; i < 240; i++) {
			nAccumulated_ImageInfo[i].private_data.optional_info[
				VID_OPT_BUFFER_ID] = 0xffff;
		}
		...
		ImageInfo->private_data.optional_info[VID_OPT_RESERVED_1] =
			time_gap_ms;

		if (ImageInfo->private_data.dolbyVision_info.osd_addr[0]
			!= USE_OWN_FB) {
			if (ImageInfo->private_data.optional_info[
				VID_OPT_BUFFER_ID] < 240) {
				memcpy(&nAccumulated_ImageInfo[
		...
		stDolby_BL.buffer_width =
			ImageInfo->private_data.optional_info[
				VID_OPT_BUFFER_WIDTH];
		stDolby_BL.buffer_height =
			ImageInfo->private_data.optional_info[
				VID_OPT_BUFFER_HEIGHT];
		stDolby_BL.frame_width =
			ImageInfo->private_data.optional_info[
				VID_OPT_FRAME_WIDTH];
		stDolby_BL.frame_height =
			ImageInfo->private_data.optional_info[
				VID_OPT_FRAME_HEIGHT];
		stDolby_BL.bit10 =
			ImageInfo->private_data.optional_info[
				VID_OPT_BIT_DEPTH] == 10 ? 1 : 0;

		stDolby_EL.offset[0] =
			ImageInfo->private_data.dolbyVision_info.el_offset[0];
		stDolby_EL.offset[1] =
			ImageInfo->private_data.dolbyVision_info.el_offset[1];
		stDolby_EL.offset[2] =
			ImageInfo->private_data.dolbyVision_info.el_offset[2];
		stDolby_EL.format = TCC_LCDC_IMG_FMT_YUV420ITL0;

		stDolby_EL.buffer_width =
		ImageInfo->private_data.dolbyVision_info.el_buffer_width;

		stDolby_EL.buffer_height =
		ImageInfo->private_data.dolbyVision_info.el_buffer_height;

		stDolby_EL.frame_width =
		ImageInfo->private_data.dolbyVision_info.el_frame_width;

		stDolby_EL.frame_height =
		ImageInfo->private_data.dolbyVision_info.el_frame_height;
		...
		stDolby_OSD.buffer_width =
			ImageInfo->private_data.dolbyVision_info.osd_width;
		stDolby_OSD.buffer_height =
			ImageInfo->private_data.dolbyVision_info.osd_height;
		stDolby_OSD.frame_width =
			ImageInfo->private_data.dolbyVision_info.osd_width;
		stDolby_OSD.frame_height =
			ImageInfo->private_data.dolbyVision_info.osd_height;
		stDolby_OSD.bit10 = 0;

		stDolby_OSD_1.offset[0] =
			ImageInfo->private_data.dolbyVision_info.osd_addr[1];
		stDolby_OSD_1.format = TCC_LCDC_IMG_FMT_RGB888;
		stDolby_OSD_1.buffer_width =
			ImageInfo->private_data.dolbyVision_info.osd_width;
		stDolby_OSD_1.buffer_height =
			ImageInfo->private_data.dolbyVision_info.osd_height;
		stDolby_OSD_1.frame_width =
			ImageInfo->private_data.dolbyVision_info.osd_width;
		stDolby_OSD_1.frame_height =
			ImageInfo->private_data.dolbyVision_info.osd_height;
		stDolby_OSD_1.bit10 = 0;

		if (stDolby_EL.offset[0] != 0x00 && stDolby_EL.frame_width != 0
			&& stDolby_EL.frame_height != 0) {
			/* prevent KCS warning */
		...
		if (ratio == 0) {
			pr_info("[INF][VIOC_I] ID[%d] BL(%dx%d) EL(0x%x %dx%d)\n",
				ImageInfo->private_data.optional_info[
					VID_OPT_BUFFER_ID],
				stDolby_BL.frame_width, stDolby_BL.frame_height,
				stDolby_EL.offset[0], stDolby_EL.frame_width,
				stDolby_EL.frame_height);
			return;
		}

		dvprintk("%s @1 sc(%d) reg(0x%x) md(0x%x) %d bpp\n",
			__func__,
			vioc_v_dv_get_sc(),
			ImageInfo->private_data.dolbyVision_info.reg_addr,
			ImageInfo->private_data.dolbyVision_info.md_hdmi_addr,
			stDolby_BL.bit10,
		dvprintk(" @BL(0x%x, %d x %d) @EL(0x%x, %d x %d) @OSD(0x%x)\n",
			stDolby_BL.offset[0],
			stDolby_BL.frame_width, stDolby_BL.frame_height,
			stDolby_EL.offset[0],
			stDolby_EL.frame_width, stDolby_EL.frame_height,
			stDolby_OSD.offset[0]);

		if (stDolby_BL.offset[0] != 0x00 && stDolby_BL.frame_width != 0
			&& stDolby_BL.frame_height != 0) {
			...
			if (!VIOC_CONFIG_DV_GET_EDR_PATH()) {
				VIOC_WMIX_SetPosition(
					pdp_data->wmixer_info.virt_addr,
					ImageInfo->Lcdc_layer, 0, 0);
				VIOC_WMIX_SetUpdate(
					pdp_data->wmixer_info.virt_addr);
			}
			...
			tca_edr_vioc_set(nRDMA, pRDMA, stDolby_BL, pdp_data,
				ImageInfo, stDolby_BL.frame_width,
				stDolby_BL.frame_height, EDR_BL, ratio);
		}
		...
		if (stDolby_EL.offset[0] != 0x00 && stDolby_EL.frame_width != 0
			...
			tca_edr_vioc_set(nRDMA, pRDMA, stDolby_EL, pdp_data,
				ImageInfo, stDolby_EL.frame_width,
				stDolby_EL.frame_height, EDR_EL, ratio);
		}

		#ifdef CONFIG_VIOC_DOLBY_VISION_CERTIFICATION_TEST_UI
		if (stDolby_OSD.offset[0] != 0x00 && stDolby_OSD.offset[0]
			!= USE_OWN_FB && stDolby_OSD.frame_width != 0
			&& stDolby_OSD.frame_height != 0) {
			...
			tca_edr_vioc_set(nRDMA, pRDMA, stDolby_OSD, pdp_data,
				ImageInfo, stDolby_OSD.frame_width,
				stDolby_OSD.frame_height,
				/*EDR_OSD3*/RDMA_FB, ratio);
		}

		if (stDolby_OSD_1.offset[0] != 0x00
			&& stDolby_OSD_1.frame_width != 0
			&& stDolby_OSD_1.frame_height != 0) {
			...
			tca_edr_vioc_set(nRDMA, pRDMA, stDolby_OSD_1, pdp_data,
				ImageInfo, stDolby_OSD_1.frame_width,
				stDolby_OSD_1.frame_height,
				/*EDR_OSD1*/RDMA_FB1, ratio);
		...
		if (VIOC_CONFIG_DV_GET_EDR_PATH()) {
			#ifdef CONFIG_VIOC_MAP_DECOMP
			if (stDolby_EL.offset[0] != 0x00
				&& stDolby_EL.frame_width != 0
				&& stDolby_EL.frame_height != 0) {

				VIOC_RDMA_SetImageEnable(
					pdp_data->rdma_info[
					RDMA_VIDEO_SUB].virt_addr);
			}
			...
			ImageInfo->private_data.dolbyVision_info.reg_out_type) {
					if (stDolby_BL.offset[0] != 0x00
						&& stDolby_BL.frame_width != 0
						&& stDolby_BL.frame_height != 0
					) {
						VIOC_RDMA_SetImageEnable(
							pdp_data->rdma_info[
							RDMA_VIDEO].virt_addr);
					}
		...
		ImageInfo->private_data.optional_info[VID_OPT_CONTENT_TYPE],
					...
					pr_err("[ERR][VIOC_I] 0 Dolby Out type mismatch (%d != %d)\n",
						vioc_get_out_type(),
			ImageInfo->private_data.dolbyVision_info.reg_out_type);
				}
			}
...
#endif	//CONFIG_VIOC_DOLBY_VISION_CERTIFICATION_TEST
...
static void tca_scale_display_update_internal(
	struct tcc_dp_device *pdp_data,
	struct tcc_lcdc_image_update *ImageInfo,
	int process_lut_plugin)
{
	void __iomem *pSC;
	int iSCType;
	unsigned int lcd_width = 0, lcd_height = 0;
	...
	if (!ImageInfo->enable) {
		...
		if (VIOC_CONFIG_DV_GET_EDR_PATH()) {
			...
			vioc_v_dv_prog(dv_md_phyaddr, dv_reg_phyaddr,
				ATTR_SDR, 1);
			return;
		}
		#endif
		...
		if (ImageInfo->private_data.optional_info[
			...
			if (ImageInfo->private_data.dolbyVision_info.osd_addr[0]
				!= USE_OWN_FB) {
				VIOC_RDMA_PreventEnable_for_UI(1,
			ImageInfo->private_data.dolbyVision_info.osd_addr[0]
				== 0x00 ? 1 : 0);
			}
			#endif
	...
	if (!ImageInfo->enable)	{
		...
		if (VIOC_CONFIG_DMAPath_Support()) {
			...
			pdp_data->rdma_info[ImageInfo->Lcdc_layer].blk_num);
				...
				&& (defined(CONFIG_ARCH_TCC803X) \
				|| defined(CONFIG_ARCH_TCC805X))
			VIOC_CONFIG_MCPath(pdp_data->wmixer_info.blk_num,
			pdp_data->rdma_info[ImageInfo->Lcdc_layer].blk_num);
			#endif
		}
	 ...
	 */
	VIOC_WMIX_SetPosition(pdp_data->wmixer_info.virt_addr,
		ImageInfo->Lcdc_layer, ImageInfo->offset_x,
		ImageInfo->offset_y);
	VIOC_WMIX_SetUpdate(pdp_data->wmixer_info.virt_addr);
	...
	if (ImageInfo->private_data.optional_info[VID_OPT_HAVE_DTRC_INFO]
		...
		if (VIOC_CONFIG_DMAPath_Support()) {
			...
			if ((component_num < VIOC_DTRC0) ||
				...
				//tca_dtrc_convter_swreset(VIOC_DTRC0
				//	+ nDeCompressor_Main);
	...
	VIOC_RDMA_SetImageIntl(
		pdp_data->rdma_info[ImageInfo->Lcdc_layer].virt_addr, 0);

	#if defined(CONFIG_TCC_VSYNC_DRV_CONTROL_LUT)
	if (process_lut_plugin) {
		VIOC_RDMA_GetImageEnable(
			pdp_data->rdma_info[RDMA_VIDEO].virt_addr,
			&rdma_en);

		if (!rdma_en  && !ImageInfo->deinterlace_mode) {
			tca_map_convter_wait_done(VIOC_MC0 +
				nDeCompressor_Main);
		} else if (rdma_en) {
	...
	if (process_lut_plugin) {
		...
		if (!rdma_en && !ImageInfo->deinterlace_mode) {
			tca_map_convter_onoff(VIOC_MC0
				+ nDeCompressor_Main, 0, 0);
		} else if (rdma_en) {
			VIOC_RDMA_SetImageDisableNW(
			pdp_data->rdma_info[RDMA_VIDEO].virt_addr);
		}
	...
	if (VIOC_CONFIG_DV_GET_EDR_PATH()) {
		...
		if (ImageInfo->Lcdc_layer == RDMA_VIDEO
			...
			if (ImageInfo->Lcdc_layer == RDMA_LASTFRM &&
				(ImageInfo->fmt >= TCC_LCDC_IMG_FMT_444SEP &&
				ImageInfo->fmt <= TCC_LCDC_IMG_FMT_YUV422ITL1)
			) {
				VIOC_RDMA_SetImageY2REnable(
					pdp_data->rdma_info[
					ImageInfo->Lcdc_layer].virt_addr, true);

				VIOC_RDMA_SetImageY2RMode(
					pdp_data->rdma_info[
					ImageInfo->Lcdc_layer].virt_addr, 0x2);
			}

			if (vioc_get_out_type() ==
		ImageInfo->private_data.dolbyVision_info.reg_out_type
			) {
				VIOC_V_DV_SetSize(NULL,
					pdp_data->rdma_info[
					ImageInfo->Lcdc_layer].virt_addr,
					ImageInfo->offset_x,
					ImageInfo->offset_y,
	...
	struct tcc_lcdc_image_update *ImageInfo)
{
	...
	dprintk("%s en:%d layer:%d fmt:%d Fw:%d Fh:%d Iw:%d Ih:%d otf:%d\n",
		__func__, ImageInfo->enable, ImageInfo->Lcdc_layer,
	...
	VIOC_RDMA_SetImageOffset(pRDMABase0, ImageInfo->fmt,
		ImageInfo->Frame_width);
	...
	VIOC_RDMA_SetImageSize(pRDMABase0, ImageInfo->Image_width,
		ImageInfo->Image_height);
	VIOC_RDMA_SetImageBase(pRDMABase0, ImageInfo->addr0,
		ImageInfo->addr1, ImageInfo->addr2);
	...
	VIOC_RDMA_SetImageOffset(pRDMABase1, ImageInfo->fmt,
		ImageInfo->Frame_width);
	VIOC_RDMA_SetImageFormat(pRDMABase1, ImageInfo->fmt);

	// position
	VIOC_WMIX_SetPosition(pWMIXBase, ImageInfo->Lcdc_layer, 0,
		ImageInfo->Frame_height + iVBlank);

	VIOC_RDMA_SetImageSize(pRDMABase1, ImageInfo->Image_width,
		ImageInfo->Image_height);
	VIOC_RDMA_SetImageBase(pRDMABase1, ImageInfo->MVC_Base_addr0,
		ImageInfo->MVC_Base_addr1, ImageInfo->MVC_Base_addr2);
	...
	struct tcc_dp_device *pdp_data)
{
	...
	scN = VIOC_CONFIG_GetScaler_PluginToRDMA(
		pdp_data->rdma_info[layer].blk_num);
	...
	#endif
	{
			...
			|| defined(CONFIG_ARCH_TCC899X) \
			|| defined(CONFIG_ARCH_TCC901X)
		VIOC_WMIX_SetChromaKey(pWMIX, layer, chroma_en,
			chromaR, chromaG, chromaB, 0x3FF, 0x3FF, 0x3FF);
		#else
		VIOC_WMIX_SetChromaKey(pWMIX, layer, chroma_en,
			chromaR, chromaG, chromaB, 0xF8, 0xFC, 0xF8);
	...
	struct fb_var_screeninfo *var)
{
	...
	if ((lcd_width != var->xres) || (lcd_height != var->yres)) {
		...
		if (VIOC_CONFIG_GetScaler_PluginToRDMA(
			pdp_data->rdma_info[rdmaN].blk_num) < 0) {

			VIOC_PlugInOutCheck VIOC_PlugIn;

			VIOC_CONFIG_Device_PlugState(VIOC_SCALER0,
				&VIOC_PlugIn);
			VIOC_RDMA_GetImageEnable(
				pdp_data->rdma_info[rdmaN].virt_addr, &en_rdma);

			if (!en_rdma && !VIOC_PlugIn.enable) {
				pr_info("[INF][VIOC_I] fb %d : display scaler 1 plug in\n",
					fbi->fb->node);
				VIOC_CONFIG_PlugIn(VIOC_SCALER0,
					pdp_data->rdma_info[rdmaN].blk_num);
			} else {
				pr_err("[ERR][VIOC_I] fb%d: scaler 1 plug in error RDMA enable:%d  SCALER enable:%d\n",
					fbi->fb->node, en_rdma,
					VIOC_PlugIn.enable);
			}
		}
	...
	int interlaced)
{
	...
	if (VIOC_CONFIG_DMAPath_Support()) {
		...
		if ((component_num >= VIOC_MC0)
			&& (component_num <= (VIOC_MC0 + VIOC_MC_MAX))) {
			pr_info("[INF][VIOC_I] tcc_video_rdma[%d]_off for last-frame :: mc_num %u plug out\n",
				ImageInfo->Lcdc_layer,
		...
		if ((component_num >= VIOC_DTRC0)
			&& (component_num <= (VIOC_DTRC0 + VIOC_DTRC_MAX))) {
			pr_info("[INF][VIOC_I] tcc_video_rdma[%d]_off for last-frame :: dtrc_num %u plug out\n",
				ImageInfo->Lcdc_layer,
				get_vioc_index(component_num));
		...
		VIOC_CONFIG_DMAPath_Set(
			...
			&& (defined(CONFIG_ARCH_TCC803X) \
			|| defined(CONFIG_ARCH_TCC805X))
		if (ImageInfo->private_data.optional_info[VID_OPT_HAVE_MC_INFO]
			!= 0) {
			pr_info("[INF][VIOC_I] tcc_video_rdma[%d]_off for last-frame :: Map convter plug out\n",
				ImageInfo->Lcdc_layer);

			tca_map_convter_onoff(VIOC_MC0 + nDeCompressor_Main,
				0, 1);
			VIOC_CONFIG_MCPath(pdp_data->wmixer_info.blk_num,
			pdp_data->rdma_info[ImageInfo->Lcdc_layer].blk_num);
		}
	...
	if (!interlaced && (outputMode == TCC_OUTPUT_LCD
		|| outputMode == TCC_OUTPUT_HDMI)) {
		...
		if (VIOC_CONFIG_GetScaler_PluginToRDMA(
			...
			!= -1) {
			// scaler plug in status check
			pr_info("[INF][VIOC_I] tcc_video_rdma[%d]_off for last-frame :: scaler %u plug out\n",
				ImageInfo->Lcdc_layer,
				get_vioc_index(iSCType));
...
void tca_fb_wait_for_vsync(struct tcc_dp_device *pdata)
{
	...
	if (pdata->pandisp_sync.state == 0) {
		ret = wait_event_interruptible_timeout(
			pdata->pandisp_sync.waitq,
			pdata->pandisp_sync.state == 1,
			msecs_to_jiffies(50));
	}

	if (!ret) {
		pr_info("[INF][VIOC_I] %s(%d) tcc_setup_interrupt timed_out!!\n",
			__func__, ret);
	}
	#endif
}
	...
	struct tcc_dp_device *pdp_data)
{
	...
	if (pdp_data->DispDeviceType == TCC_OUTPUT_HDMI) {
		if ((!VIOC_CONFIG_DV_GET_EDR_PATH() && vioc_v_dv_get_stage()
			!= DV_OFF)
		|| (VIOC_CONFIG_DV_GET_EDR_PATH() && vioc_v_dv_get_stage()
			!= DV_RUN)
		) {
			return;
		}
	...
	if (VIOC_CONFIG_DV_GET_EDR_PATH() && pdp_data->DispDeviceType
		== TCC_OUTPUT_HDMI && bUse_GAlpha) {
		...
		VIOC_RDMA_SetImageEnable(
			pdp_data->rdma_info[RDMA_FB].virt_addr);
		//pr_info("[INF][VIOC_I] Global Alpha for Logo\n");
	}
	#endif
	...
	struct fb_var_screeninfo *var)
{
	...
	if ((system_rev != 0) && (var->reserved[3] != 0)) {
		BaseAddr = fbi->map_dma + var->xres * var->yoffset
			* (var->bits_per_pixel/8) + var->reserved[1];
	} else
	#endif
	{
		BaseAddr = fbi->map_dma + var->xres * var->yoffset
	...
	if (fbi->pdata.Mdp_data.FbPowerState) {
		...
		&& !defined(CONFIG_TCC_DISPLAY_MODE_USE)
	...
	struct fb_info *info)
{
	...
	if (info->node == 0) {
		...
		if ((fbi->pdata.Mdp_data.FbPowerState)
			...
			if (var->activate & FB_ACTIVATE_VBL) {
				...
				if (fbi->pdata.Mdp_data.pandisp_sync.state == 0
					...
					ret = wait_event_interruptible_timeout(
					fbi->pdata.Mdp_data.pandisp_sync.waitq,
					fbi->pdata.Mdp_data.pandisp_sync.state
						== 1,
					msecs_to_jiffies(50));
				}
		...
		base_addr = fbi->map_dma + var->xres * var->yoffset
			* (var->bits_per_pixel/8);
		...
		tca_fb_rdma_pandisplay(RDMA_FB1, base_addr, var,
			&fbi->pdata.Mdp_data);
	}
...
int tca_fb_suspend(
	struct device *dev,
	struct lcd_panel *disp_panel,
	struct lcd_panel *ext_panel)
{
	...
	if (info->fb->node == 0) {
		...
		ret = wait_event_interruptible_timeout(
			pdp_data->disp_dd_sync.waitq,
			pdp_data->disp_dd_sync.state == 1,
			msecs_to_jiffies(30));

		if (ret == 0) {
			pr_info("[INF][VIOC_I] %s: timeout DD\n",
				__func__);
		}
	} else {
	...
	_tca_vioc_intr_onoff(OFF, pdp_data->ddc_info.irq_num,
		info->pdata.lcdc_number);
	...
	struct lcd_panel *ext_panel)
{
	...
	_tca_vioc_intr_onoff(ON, pdp_data->ddc_info.irq_num,
		fbi->pdata.lcdc_number);
...
int tca_fb_init(struct tccfb_info *fbi)
{
	...
	if (ViocScaler_np == NULL) {
		pr_err("[ERR][VIOC_I] %s: cann't find scaler\n",
			__func__);
	}

	ViocConfig_np = of_find_compatible_node(NULL, NULL,
		"telechips,vioc_config");
	if (ViocConfig_np == NULL) {
		pr_err("[ERR][VIOC_I] %s: cann't find vioc config\n",
			__func__);
	}
	...
	Gmap_cpu = dma_alloc_writecombine(0, MOUSE_CURSOR_BUFF_SIZE,
		&Gmap_dma, GFP_KERNEL);
	...
	attach_intr.vioc_intr = kzalloc(sizeof(struct vioc_intr_type),
		GFP_KERNEL);
	...
	//void __iomem *pRDMA =
	//	fbi->pdata.Mdp_data.rdma_info[RDMA_FB].virt_addr;
