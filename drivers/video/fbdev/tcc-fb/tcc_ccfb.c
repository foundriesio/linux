...
static int tccxxx_ccfb_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	if (range_is_allowed(vma->vm_pgoff, vma->vm_end - vma->vm_start) < 0) {
		pr_err("ccfb: this address is not allowed\n");
		return -EAGAIN;
	}
...
static int tccxxx_ccfb_act_clock(ccfb_dev_config_t *dev, int lcdc_num)
{
	...
	if ((dev == NULL) || ((lcdc_num < 0) && (lcdc_num >= 2))) {
		pr_err("Invalid parameters for lcdc(%d, %p)\n",
		       lcdc_num, dev);
		return -EINVAL;
	}
	...
	if (IS_ERR(dev->pLcdcClk[lcdc_num])) {
		pr_err("[ERR][CCFB] %s clock get fail.\n",
		       pDevName[lcdc_num]);
		return -EIO;
	}
	...
	if (lcdc_num == 0) {
		dev->pCurLcdc = VIOC_RDMA_GetAddress(1);
		dev->pCurWMix = VIOC_WMIX_GetAddress(0);
	} else {
		dev->pCurLcdc = VIOC_RDMA_GetAddress(5);
		dev->pCurWMix = VIOC_WMIX_GetAddress(1);
	}
...
static int tccxxx_ccfb_get_config(ccfb_dev_config_t *dev, void *arg)
{
	...
	if (copy_to_user((unsigned long *)arg, &state, sizeof(unsigned long)))
		return -EFAULT;
...
tccxxx_ccfb_set_config(ccfb_dev_config_t *dev, struct ccfb_config_t *arg)
{
	...
	if (copy_from_user(
		    (void *)&cfg, (void *)arg, sizeof(struct ccfb_config_t)))
		return -EFAULT;
	...
	if (ret == 0) {
		...
		VIOC_WMIX_SetPosition(
			dev->pCurWMix, 1, cfg.res.disp_x, cfg.res.disp_y);

		// size
		VIOC_RDMA_SetImageSize(
			dev->pCurLcdc, cfg.res.disp_w, cfg.res.disp_h);

		if (cfg.res.disp_fmt == CCFB_IMG_FMT_RGB565) {
			VIOC_RDMA_SetImageFormat(
				dev->pCurLcdc, VIOC_IMG_FMT_RGB565);
			...
			if (cfg.res.chroma_en) {
				VIOC_WMIX_SetChromaKey(
					dev->pCurWMix, 1, cfg.res.chroma_en,
					cfg.res.chroma_R, cfg.res.chroma_G,
					cfg.res.chroma_B, 0xF8, 0xFC, 0xF8);
			}
		} else {
			if (cfg.res.disp_fmt == CCFB_IMG_FMT_ARGB4444) {
				// ARGB4444 format
				VIOC_RDMA_SetImageFormat(
					dev->pCurLcdc, VIOC_IMG_FMT_ARGB4444);
				VIOC_RDMA_SetImageOffset(
					dev->pCurLcdc, VIOC_IMG_FMT_ARGB4444,
					cfg.res.mem_w);
				...
				// ARGB8888 format
				VIOC_RDMA_SetImageFormat(
					dev->pCurLcdc, VIOC_IMG_FMT_ARGB8888);
				VIOC_RDMA_SetImageOffset(
					dev->pCurLcdc, VIOC_IMG_FMT_ARGB8888,
					cfg.res.mem_w);
			}
			...
			VIOC_RDMA_SetImageAlphaSelect(
				dev->pCurLcdc, 1); // ASEL set
			VIOC_RDMA_SetImageAlphaEnable(
				dev->pCurLcdc, 1); // AEN set
		}
...
static int tccxxx_ccfb_disp_update(ccfb_dev_config_t *dev, unsigned int *arg)
{
	...
	if (copy_from_user(
		    (void *)&cur_addr, (void *)arg, sizeof(unsigned int))) {
		pr_err("[ERR][CCFB] error\n");
		return -EFAULT;
	}
	...
	if (dev->pUiLcdc->LIC & Hw28) {
		pr_warn("==>>> [%s] WARNING : UI is enabled.\n", __func__);
	...
	// position (full screen update)
	//VIOC_WMIX_SetPosition(dev->pCurWMix, 1, g_ccfg_cfg.res.disp_x,
	//g_ccfg_cfg.res.disp_y);
	VIOC_WMIX_SetPosition(dev->pCurWMix, 1, 10, 10);

	// size
	VIOC_RDMA_SetImageSize(dev->pCurLcdc, g_ccfg_cfg.res.disp_w,
	g_ccfg_cfg.res.disp_h);

	//ARGB 32bit format
	VIOC_RDMA_SetImageFormat(dev->pCurLcdc, VIOC_IMG_FMT_ARGB8888);
	VIOC_RDMA_SetImageOffset(dev->pCurLcdc, VIOC_IMG_FMT_ARGB8888,
	g_ccfg_cfg.res.mem_w);
...
tccxxx_ccfb_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case CCFB_GET_CONFIG:
		ret = tccxxx_ccfb_get_config(dev, (unsigned long *)arg);
		break;

	case CCFB_SET_CONFIG:
		if (dev->cur_state == CCFB_STATE_OPENED) {
			ret = tccxxx_ccfb_set_config(
				dev, (struct ccfb_config_t *)arg);
			if (ret == 0)
				dev->cur_state = CCFB_STATE_PAUSE;
		} else if (
			(dev->cur_state == CCFB_STATE_PREPARE)
			|| (dev->cur_state == CCFB_STATE_PAUSE)) {
			ret = tccxxx_ccfb_set_config(
				dev, (struct ccfb_config_t *)arg);
			if (ret == 0)
	...
	case CCFB_DISP_ENABLE:
		if (dev->cur_state == CCFB_STATE_OPENED)
			dev->cur_state = CCFB_STATE_PREPARE;
		else if (dev->cur_state == CCFB_STATE_PAUSE)
			dev->cur_state = CCFB_STATE_RUNNING;
		break;
	...
	case CCFB_DISP_DISABLE:
		if (dev->cur_state == CCFB_STATE_RUNNING) {
			...
			dev->cur_state = CCFB_STATE_PAUSE;
		} else if (dev->cur_state == CCFB_STATE_PREPARE) {
			ret = tccxxx_ccfb_lcdc_disable(dev);
...
static int tccxxx_ccfb_release(struct inode *inode, struct file *file)
{
	...
	if (dev->pCurWMix != NULL) {
		if (g_ccfg_cfg.res.chroma_en) {
			VIOC_WMIX_SetChromaKey(
				dev->pCurWMix, 1, 0, 0, 0, 0, 0xF8, 0xFC, 0xF8);
		}
	}
...
static const struct file_operations tcc_ccfb_fops = {
	.owner = THIS_MODULE,
