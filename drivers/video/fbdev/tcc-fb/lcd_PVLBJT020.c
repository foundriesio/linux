...
static int pvlbjt020_panel_init(
...
static int pvlbjt020_set_power(
	struct lcd_panel *panel,
	int on,
	struct tcc_dp_device *fb_pdata)
{
	...
	if (on) {
		...
		if (gpio_is_valid(lvds_pvlbjt020.gpio.stby)) {
			gpio_set_value_cansleep(
				lvds_pvlbjt020.gpio.stby, 0);
		}

		if (gpio_is_valid(lvds_pvlbjt020.gpio.reset)) {
			gpio_set_value_cansleep(
				lvds_pvlbjt020.gpio.reset, 0);
		}

		if (gpio_is_valid(lvds_pvlbjt020.gpio.display_on)) {
			gpio_set_value_cansleep(
				lvds_pvlbjt020.gpio.display_on, 0);
		}

		if (gpio_is_valid(lvds_pvlbjt020.gpio.power_on)) {
			gpio_set_value_cansleep(
				lvds_pvlbjt020.gpio.power_on, 0);
		}
	...
	if (on) {
		...
		status = VIOC_LVDS_PHY_CheckStatus(lvds_pvlbjt020.main_port,
			lvds_pvlbjt020.sub_port);
		if (!(status & 0x1)) {
...
{
	if (np) {
		lvds_pvlbjt020.gpio.power_on = of_get_named_gpio(np,
			"power-on-gpios", 0);
		if (!gpio_is_valid(lvds_pvlbjt020.gpio.power_on)) {
		...
		lvds_pvlbjt020.gpio.reset = of_get_named_gpio(np,
			"reset-gpios", 0);
		if (!gpio_is_valid(lvds_pvlbjt020.gpio.reset)) {
		...
		lvds_pvlbjt020.gpio.display_on = of_get_named_gpio(np,
			"display-on-gpios", 0);
		if (!gpio_is_valid(lvds_pvlbjt020.gpio.display_on)) {
			...
			gpio_request(lvds_pvlbjt020.gpio.display_on,
				"lvds_display");
			gpio_direction_output(lvds_pvlbjt020.gpio.display_on,
				1);
		}

		lvds_pvlbjt020.gpio.stby = of_get_named_gpio(np,
			"lvds-stby-gpios", 0);
		if (!gpio_is_valid(lvds_pvlbjt020.gpio.stby)) {
		...
		lvds_pvlbjt020.gpio.power = of_get_named_gpio(np,
			"lvds-power-gpios", 0);
...
static int pvlbjt020_probe(struct platform_device *pdev)
{
	...
	of_property_read_u32(pdev->dev.of_node, "second-display",
		&second_display);
	...
	if (of_property_read_u32_index(pdev->dev.of_node, "ports", 0,
		&lvds_pvlbjt020.main_port) < 0) {
	...
	pr_info("%s lvds - main_port: %d\n", __func__,
		lvds_pvlbjt020.main_port);

	if (of_property_read_u32_index(pdev->dev.of_node, "ports", 1,
		&lvds_pvlbjt020.sub_port) < 0) {
	...
	if (of_property_read_u32_index(pdev->dev.of_node, "mux_select", 1,
		&lvds_pvlbjt020.lcdc_select) < 0) {
...
#ifdef CONFIG_OF
static const struct of_device_id pvlbjt020_of_match[] = {
