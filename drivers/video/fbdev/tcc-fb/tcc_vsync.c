...
volatile void __iomem *pExtFrame_RDMABase;
...
#if 0
#define lut_prinfo(msg, ...) pr_info("\x1b[1;38m[DBG][VSYNC] \x1b[0m" msg, ##__VA_ARGS__)
#else
#define lut_prinfo(msg, ...)
#endif
...
#if 0
#define vprintk(msg...) pr_info("[DBG][VSYNC] " msg)
#else
#define vprintk(msg...)
#endif

#if 0
#define dprintk(msg...) pr_info("[DBG][VSYNC] " msg)
#else
#define dprintk(msg...)
#endif

#if 0
#define dprintk_ext(msg...) pr_info("[DBG][VSYNC-EXT] " msg)
#else
#define dprintk_ext(msg...)
#endif

#if 0
#define dprintk_drop(msg...) pr_info("[DBG][VSYNC] " msg)
#else
#define dprintk_drop(msg...)
#endif

#if 0
#define dprintk_dv_transition(msg...) pr_info("[DBG][VSYNC-DV] " msg)
#else
#define dprintk_dv_transition(msg...)
#endif

#if defined(CONFIG_VIOC_DOLBY_VISION_CERTIFICATION_TEST)
#undef dprintk_drop(msg...)
#define dprintk_drop(msg...) pr_info("[DBG][VSYNC] " msg)

#undef dprintk_dv_transition(msg...)
#define dprintk_dv_transition(msg...)
#endif
...
#if 0
static int testToggleBit1;
static int testToggleBit;
#endif
...
static int vsync_process_video_lut(struct tcc_video_disp *p, struct tcc_lcdc_image_update *pImage, enum VSYNC_CH_TYPE type)
{
	int ret = -1;
...
static int tcc_vsync_set_max_buffer(struct tcc_vsync_buffer_t *buffer_t, int buffer_count)
{
...
static int _tcc_vsync_print_all_buffers(struct tcc_vsync_buffer_t *buffer_t)
{
	int i = 0;
...
static int tcc_vsync_push_buffer(struct tcc_video_disp *p, struct tcc_vsync_buffer_t *buffer_t, struct tcc_lcdc_image_update *inputData)
{
...
static inline void *tcc_vsync_get_buffer(struct tcc_vsync_buffer_t *buffer_t, int offset)
{
	int readIdx;
...
static inline int tcc_vsync_buffer_check(struct tcc_video_disp *p, unsigned int ref_buffer_id)
{
	...
	if (p->isVsyncRunning != EM_VSYNC_RUNNING) {
		vprintk("vsync is not started!! return\n");
		return -1;
	}
...
static inline int tcc_vsync_clean_buffer(struct tcc_vsync_buffer_t *buffer_t)
{
...
static int tcc_vsync_pop_all_buffer(struct tcc_video_disp *p, int caller_line)
{
...
static int tcc_vsync_is_full_buffer(struct tcc_vsync_buffer_t *buffer_t)
{
...
static inline int tcc_vsync_is_empty_buffer(struct tcc_vsync_buffer_t *buffer_t)
{
	if (atomic_read(&buffer_t->valid_buff_count) > 0)
		return 0;
	else
...
static void tcc_vsync_push_render_frame_for_duplicated_interlace(struct tcc_video_disp *p)
{
	int readCount = 0;
...
static void tcc_vsync_check_interlace_output(struct tcc_video_disp *p, struct tcc_dp_device *pdp_data)
{
...
static int tcc_vsync_bypass_frame(struct tcc_video_disp *p, struct tcc_dp_device *pdp_data, struct tcc_lcdc_image_update *pImage)
{
...
static void tcc_vsync_display_update(struct tcc_video_disp *p)
{
	...
	for (i = 0; i < readable_buff_cnt; i++) {
		...
		if (time_gap >= 0) {
			if (tcc_vsync_pop_buffer(&p->vsync_buffer) < 0) {
				if (popped_count > 0) {
					dprintk_drop("[%d] %s:0 skipped buffer(%d) and no display\n", p->type, p->pIntlNextImage->Lcdc_layer == RDMA_VIDEO ? "M" : "S", popped_count);
				}
				return;
			}
			popped_count++;
	...
	if (popped_count > 1) {
		dprintk_drop("[%d] %s:1 skipped buffer(p:[%d]) r:[%d -> %d])\n", p->type,
...
//#endif
		...
		if (p->skipFrameStatus) {
			dprintk("skip status(%d) %d/0x%x => %d/0x%x, en: %d layer: %d out: %d\n",
				p->skipFrameStatus, pPrevImage->buffer_unique_id,
				pPrevImage->addr0, pNextImage->buffer_unique_id,
				pNextImage->addr0, pNextImage->enable,
				pNextImage->Lcdc_layer, Output_SelectMode);
		}
...
static void tcc_vsync_display_update_forDeinterlaced(struct tcc_video_disp *p)
{
	...
	if (p->nDeinterProcCount == 0) {
		...
		readable_buff_cnt--;
		if (readable_buff_cnt < 0) {
			dprintk_drop("[%d] %s: no available buffer(%d)\n", p->type, p->pIntlNextImage->Lcdc_layer == RDMA_VIDEO ? "M" : "S", readable_buff_cnt);
		}
		...
		for (i = 0; i < readable_buff_cnt; i++) {
			...
			vprintk("[%d/%d] ID[%d] p->pIntlNextImage->time_stamp : %d / %d, %d\n",
				i, readable_buff_cnt, p->pIntlNextImage->buffer_unique_id,
				p->pIntlNextImage->time_stamp, p->pIntlNextImage->sync_time, current_time);
...
int display_vsync(struct tcc_video_disp *p)
{
	...
	if (tcc_vsync_is_empty_buffer(&p->vsync_buffer) == 0) {
		...
		if( (p->isVsyncRunning == EM_VSYNC_RUNNING) && (0 >= tcc_video_get_readable_count(p)))
			return -1;
	}
...
static void tcc_vsync_unplugged_int_start(irq_handler_t handler)
{
	if (!vsync_unpluged_int_timer) {
		vsync_unpluged_int_timer = tcc_register_timer(NULL, 16000, handler); // 16.667ms
...
static void tcc_vsync_reset_syncTime(struct tcc_video_disp *p, int currentTime, enum VSYNC_CH_TYPE type)
{
...
static int tcc_vsync_calculate_syncTime(struct tcc_video_disp *p, int currentTime, enum VSYNC_CH_TYPE type)
{
	int diffTime;
	int avgTime;
	int base_time;
...
static int tcc_vsync_push_preprocess(struct tcc_video_disp *p, struct tcc_dp_device *pdp_data, struct tcc_lcdc_image_update *input_image_info, enum VSYNC_CH_TYPE type)
{
	...
	if(!pdp_data->FbPowerState || !lcd_width || !lcd_height || !VIOC_DISP_Get_TurnOnOff(pDISPBase) || (p->isVsyncRunning != EM_VSYNC_RUNNING)) {
...
static void tcc_vsync_push_set_outputMode(struct tcc_video_disp *p, struct tcc_dp_device *pdp_data, struct tcc_lcdc_image_update *input_image_info)
{
...
static int tcc_vsync_push_preprocess_deinterlacing(struct tcc_video_disp *p, struct tcc_dp_device *pdp_data, struct tcc_lcdc_image_update *input_image_info)
{
...
static int tcc_vsync_push_bypass_frame(struct tcc_video_disp *p, struct tcc_dp_device *pdp_data, struct tcc_lcdc_image_update *input_image_info, enum VSYNC_CH_TYPE type)
{
...
static int tcc_vsync_push_check_error(struct tcc_video_disp *p, struct tcc_dp_device *pdp_data, struct tcc_lcdc_image_update *input_image_info)
{
...
static int tcc_vsync_push_process(struct tcc_video_disp *p, struct tcc_dp_device *pdp_data, struct tcc_lcdc_image_update *input_image_info, enum VSYNC_CH_TYPE type)
{
...
static int tcc_vsync_start(struct tcc_video_disp *p, struct tcc_lcdc_image_update *input_image_info, enum VSYNC_CH_TYPE type)
{
...
{
	#if defined(CONFIG_HDMI_DISPLAY_LASTFRAME)
	pr_info("[INF][VSYNC] ### END_VSYNC_%d started:%d SelectMode:%d LastFrame[%d]\n", p->type, p->isVsyncRunning, Output_SelectMode, tccvid_lastframe[p->type].LastImage.enable);
	#else
	...
	if (p->isVsyncRunning == EM_VSYNC_RUNNING) {
		...
		if (tcc_vsync_get_isVsyncRunning(p->type) && !video_display_disable_check) {
		...
		#ifdef USE_VSYNC_TIMER_INT_FOR_UNPLUGED_OUTPUT
		if ((tcc_vsync_isVsyncRunning(VSYNC_MAIN) == 0) && (tcc_vsync_isVsyncRunning(VSYNC_SUB0) == 0))
	...
	#endif
	{
		//if ((p->type == VSYNC_MAIN) || ((p->type == VSYNC_SUB0) && (!keep_display)))
...
static void print_vsync_input(char *str, struct tcc_lcdc_image_update *pIn)
{
	...
	pr_info("^@New^^^^^^^^^^^^^ @@@ %d/%d, %03d :: TS: %04d / %04d  %d bpp Attr(%d), #BL(0x%x, %dx%d (%dx%d), 0x%x fmt) #EL(0x%x, %dx%d (%dx%d)) #OSD(0x%x/0x%x) #Reg(0x%x) #Meta(0x%x)\n",
...
//#endif
//#ifdef CONFIG_VIOC_DTRC_DECOMP
//	if (inFframeInfo->private_data.optional_info[VID_OPT_HAVE_DTRC_INFO] == 0x1)
...
#if defined(CONFIG_HDMI_DISPLAY_LASTFRAME)
void tcc_video_info_backup(enum VSYNC_CH_TYPE type, struct tcc_lcdc_image_update *input_image)
{
#if 0
	if ((input_image->codec_id != tccvid_lastframe[type].LastImage.codec_id)
			...
			|| ((input_image->sync_time + 200) < tccvid_lastframe[type].LastImage.sync_time)
		...
		if ((input_image->sync_time + 200) < tccvid_lastframe[type].LastImage.sync_time) {
...
static int tcc_fill_black_lastframe(struct tcc_lcdc_image_update *lastUpdated, unsigned int dest_addr, unsigned int dest_size, enum VSYNC_CH_TYPE type)
{
...
int tcc_origin_video_off_for_last_frame(struct tcc_video_disp *p, struct tcc_dp_device *pdp_data, struct tcc_lcdc_image_update *lastUpdated, enum VSYNC_CH_TYPE type, int ch_disable)
{
#ifdef TCC_VIDEO_DISPLAY_DEINTERLACE_MODE
	if (p->deinterlace_mode && !p->interlace_bypass_lcdc) {
		if (!p->output_toMemory) {
...
//	}
	...
	if (((type == VSYNC_MAIN && !(p->output_toMemory && p->m2m_mode)) || (type == VSYNC_SUB0 && !(p->output_toMemory && p->m2m_mode)))
		...
		if (p->outputMode == TCC_OUTPUT_HDMI || p->outputMode == TCC_OUTPUT_COMPOSITE || p->outputMode == TCC_OUTPUT_COMPONENT
			...
			ret = tcc_move_video_last_frame(lastUpdated, p->deinterlace_mode, target_address, target_size, LAST_FRAME_FORMAT);
			if (ret < 0) {
				goto Screen_off;
			}
...
Screen_off:
		...
		if (p->outputMode == TCC_OUTPUT_HDMI || p->outputMode == TCC_OUTPUT_COMPOSITE || p->outputMode == TCC_OUTPUT_COMPONENT
			...
			if (0) {/* if (type == VSYNC_SUB0 && tccvid_lastframe[type].support && (p->output_toMemory && p->m2m_mode)) */
				if (tcc_fill_black_lastframe(lastUpdated, target_address, target_size, type) >= 0) {
					lastUpdated->enable = 1;
					lastUpdated->on_the_fly = 0;
...
int tcc_display_ext_frame(struct tcc_lcdc_image_update *lastUpdated, char bInterlaced, int output_mode)
{
...
#ifdef CONFIG_VIOC_DOLBY_VISION_EDR
	if (VIOC_CONFIG_DV_GET_EDR_PATH() && (vioc_get_video_attribute() == ATTR_DV_WITHOUT_BC)) {
	...
	if (lastUpdated->enable) {
		...
		ret = tcc_move_video_last_frame(&iImage, bInterlaced, dest_addr, (tccvid_lastframe[type].pmapBuff.size / 2), LAST_FRAME_FORMAT);
		if (ret < 0) {
			ret = -1;
			goto Error;
		}
...
int tcc_video_swap_vpu_frame(struct tcc_video_disp *p, int idx, struct WMIXER_INFO_TYPE *WmixerInfo, struct tcc_lcdc_image_update *TempImage, enum VSYNC_CH_TYPE type)
{
	int ret = 0;
	...
	if ( p->outputMode == Output_SelectMode ) {
		if(p->isVsyncRunning != EM_VSYNC_RUNNING || tccvid_lastframe[type].CurrImage.enable == 0 || tccvid_lastframe[type].enabled || tccvid_lastframe[type].CurrImage.Lcdc_layer != RDMA_VIDEO){
			if(tccvid_lastframe[type].CurrImage.Lcdc_layer != RDMA_VIDEO){
				...
				pr_info("[INF][VSYNC] ---->[%d] return tcc_video_swap_vpu_frame :: reason %d/%d/%d \n", type, p->isVsyncRunning, tccvid_lastframe[type].CurrImage.enable, tccvid_lastframe[type].enabled);
			}
			ret = -1;
			if (p->isVsyncRunning == EM_VSYNC_RUNNING && (tccvid_lastframe[type].CurrImage.enable || tccvid_lastframe[type].enabled)) {
...
static int tcc_vsync_push_main_process(unsigned int cmd, struct tcc_video_disp *p, struct tcc_lcdc_image_update *input_image, struct tcc_dp_device *dp_device, enum VSYNC_CH_TYPE type)
{
	int ret = 0;
	int err_type = 0;
...
static long tcc_vsync_do_ioctl(unsigned int cmd, unsigned long arg, enum VSYNC_CH_TYPE type)
{
	int ret = 0;
	struct tcc_video_disp *p = &tccvid_vsync[type];
	...
	{
		struct tcc_lcdc_image_update *input_image = kmalloc(sizeof(struct tcc_lcdc_image_update), GFP_KERNEL);
		...
		if (cmd == TCC_LCDC_VIDEO_START_VSYNC_KERNEL) {
			if (memcpy((void *)input_image, (const void *)arg, sizeof(struct tcc_lcdc_image_update)) == NULL)
				ret = -EFAULT;
		} else {
	...
	case TCC_LCDC_VIDEO_END_VSYNC_KERNEL:
	{
		...
		if (cmd == TCC_LCDC_VIDEO_END_VSYNC_KERNEL) {
			if (memcpy((void *)&keep_display, (const void *)arg, sizeof(unsigned int)) == NULL) {
	...
	case TCC_LCDC_VIDEO_SWAP_VPU_FRAME:
	{
		...
		struct WMIXER_INFO_TYPE WmixerInfo;
		struct tcc_lcdc_image_update *TempImage = kmalloc(sizeof(struct tcc_lcdc_image_update), GFP_KERNEL);
		...
		// if (!g_is_vsync1_opened)
	{
		...
		if (copy_from_user((void *)&lastframe_info, (const void *)arg, sizeof(struct stTcc_last_frame)))
			...
			struct tcc_lcdc_image_update *lastUpdated = kmalloc(sizeof(struct tcc_lcdc_image_update), GFP_KERNEL);
			...
			ret = tcc_video_last_frame((void *)p, lastframe_info, lastUpdated, type);
			if (ret > 0)
				p->deinterlace_mode = 0;
	...
	case TCC_LCDC_SET_M2M_LASTFRAME:
	{
		...
		dprintk("[%d] TCC_LCDC_SET_M2M_LASTFRAME\n", type);
		dprintk("[%d] LastFrame=%d Output_SelectMode=%d\n", type, tccvid_lastframe[type].enabled, Output_SelectMode);
	...
	case TCC_LCDC_VIDEO_CTRL_EXT_FRAME_KERNEL:
	{
		...
		if (cmd == TCC_LCDC_VIDEO_CTRL_EXT_FRAME_KERNEL) {
			if (memcpy((void *)&enable, (const void *)arg, sizeof(unsigned int)) == NULL)
				ret = -EFAULT;
			else {
	...
	case TCC_LCDC_VIDEO_RESTRICT_EXT_FRAME_KERNEL:
		...
		if (cmd == TCC_LCDC_VIDEO_RESTRICT_EXT_FRAME_KERNEL) {
			if (memcpy((void *)&restricted_ExtFrame, (const void *)arg, sizeof(unsigned int)) == NULL)
				goto Error;
		} else {
	...
	case TCC_LCDC_VIDEO_SET_STATUS_PAUSE_KERNEL:
		...
		if (cmd == TCC_LCDC_VIDEO_SET_STATUS_PAUSE_KERNEL) {
			if (memcpy((void *)&p->push_ext_status_paused, (const void *)arg, sizeof(unsigned int)) == NULL) {
				pr_err("[ERR][VSYNC]  copy err");
				ret = 0;
			}
		} else {
	...
	case TCC_LCDC_VIDEO_RDMA_DISABLE:
		if (copy_from_user((void *)&vsync_rdma_off[type], (const void *)arg, sizeof(unsigned int))) {
			ret = -EFAULT;
		}
...
void tcc_vsync_set_firstFrameFlag(struct tcc_video_disp *p, int firstFrameFlag)
{
...
#else
	if(type == VSYNC_MAIN && tccvid_vsync[VSYNC_SUB0].isVsyncRunning != EM_VSYNC_RUNNING)
		return (tccvid_vsync[type].isVsyncRunning == EM_VSYNC_RUNNING) ? 1 : 0;
	else if(type != VSYNC_MAIN && tccvid_vsync[VSYNC_MAIN].isVsyncRunning != EM_VSYNC_RUNNING)
		return (tccvid_vsync[type].isVsyncRunning == EM_VSYNC_RUNNING) ? 1 : 0;
#endif
	return 0;
}
...
static int tcc_vsync1_open(struct inode *inode, struct file *filp)
{
	...
	if (pmap_get_info("fb_wmixer1", &tccvid_lastframe[VSYNC_SUB0].pmapBuff) < 0) {
...
static int tcc_vsync0_open(struct inode *inode, struct file *filp)
{
	...
	if (pmap_get_info("fb_wmixer", &tccvid_lastframe[VSYNC_MAIN].pmapBuff) < 0) {
