 ...
 */
#if defined(CONFIG_VIOC_DOLBY_VISION_CERTIFICATION_TEST) \
	&& defined(DV_PREVENT_FRAME_DROP_AND_DUPLICATION)
...
static int vsync_process_video_lut(
	tcc_video_disp *p,
	...
	VSYNC_CH_TYPE type)
{
	...
	if (lut_status == (VIOC_RDMA00 + UI_LUT_PLUGIN_CH)) {
		...
		output = 1;
		//pr_debug("display is HDR\n");
	} else {
		/* Display is SDR : SDR to SDR */
		output = 0;
		//pr_debug("display is SDR\n");
	}
	...
	if (pImage->private_data.optional_info[VID_OPT_HAVE_USERDATA] == 1
		...
		/* Content is HDR */
		content = 1;
		//pr_debug("content is HDR\n");
	} else {
		/* Content is SDR */
		content = 0;
		//pr_debug("content is SDR\n");
	}
	...
	if (vsync_lut_into.content != content
		...
		if (content == 0 && output == 1) {
			/* SDR -> HDR */
			pr_debug("%s contents is SDR, display is HDR >> plugin video_lut\n",
				__func__);

			if (!vsync_lut_into.lastframe_mode) {
				lut_drv_api_set_plugin(VIDEO_LUT, 1,
					VIDEO_LUT_PLUGIN_CH);
			} else {
				pr_debug("%s skip.. lastframe is enabled\n",
					__func__);
			}
		} else {
			pr_debug("%s contents is %s, display is %s >> unplug video_lut\n",
				__func__,
				(content == 1)?"HDR":"SDR",
				(output == 1)?"HDR":"SDR");
			lut_drv_api_set_plugin(VIDEO_LUT, 0,
				VIDEO_LUT_PLUGIN_CH);
		}
...
static int tcc_vsync_set_max_buffer(tcc_vsync_buffer_t *buffer_t,
	int buffer_count)
{
...
static int tcc_vsync_push_buffer(tcc_video_disp *p,
	tcc_vsync_buffer_t *buffer_t,
...
static inline void *tcc_vsync_get_buffer(
	tcc_vsync_buffer_t *buffer_t, int offset)
{
	int readIdx;
	...
	if (p->outputMode == Output_SelectMode) {
		...
		if (p->output_toMemory && p->m2m_mode) {
			...
			case TCC_OUTPUT_HDMI:
				...
				if (tcc_vsync_mvc_status
					&& pNextImage->MVCframeView == 1
					&& pNextImage->MVC_Base_addr0) {

					//pr_info("[INF][VSYNC] %d,%d,0x%08x\n",
					//	tcc_vsync_mvc_status,
					//	pNextImage->MVCframeView,
					//	pNextImage->MVC_Base_addr0);
...
#if defined(CONFIG_VIOC_DOLBY_VISION_EDR) \
	&& defined(USE_DOLBY_VISION_SUB_INTERRUPT)
	...
	struct tcc_lcdc_image_update *input_image_info)
{
	...
	if (p->firstFrameFlag && (layer == RDMA_VIDEO)) {
		int pixelmapper_plug;
		int plug_status = -1;

		pixelmapper_plug = CheckPixelMapPathSelection(
		...
		if (pixelmapper_plug >= 0) {
			...
			if (VIOC_CONFIG_Device_PlugState(VIOC_PIXELMAP0,
				&VIOC_PlugIn) == VIOC_DEVICE_CONNECTED) {
				if (VIOC_PlugIn.connect_device ==
					get_vioc_index(
					pdp_data->rdma_info[layer].blk_num)
			...
			if (VIOC_CONFIG_Device_PlugState(VIOC_PIXELMAP1,
				&VIOC_PlugIn) == VIOC_DEVICE_CONNECTED) {
				if (VIOC_PlugIn.connect_device ==
					get_vioc_index(
					pdp_data->rdma_info[layer].blk_num)
			...
			if (plug_status == -1) {
				ret = VIOC_CONFIG_PlugIn(VIOC_PIXELMAP0,
					pdp_data->rdma_info[layer].blk_num);
				if (ret < 0) {
					pr_err("[ERR][VSYNC] %s pixel_mapper_%d plug in fail\n",
						__func__,
						get_vioc_index(VIOC_PIXELMAP0));
				}
	...
	VSYNC_CH_TYPE type)
{
	...
	default: /* EM_VSYNC_DISABLED or EM_VSYNC_PREPARE */
		...
		if (type == 0 && (p->video_frame_rate > 0)
			&& (p->video_frame_rate <= display_hz / 2)) {
			p->duplicateUseFlag = 1;
		}
...
#if  defined(CONFIG_HDMI_DISPLAY_LASTFRAME) \
	|| defined(CONFIG_VIDEO_DISPLAY_SWAP_VPU_FRAME)
	...
	unsigned int length)
{
	...
	for (type = VSYNC_MAIN; type < VSYNC_MAX; type++) {
		if ((start_phyaddr >= tccvid_lastframe[type].pmapBuff.base)
			&& (end_phyaddr
			<= (tccvid_lastframe[type].pmapBuff.base
			+ tccvid_lastframe[type].pmapBuff.size
			- 1)))

			return pmap_is_cma_alloc(
				&tccvid_lastframe[type].pmapBuff);
	}
	...
	unsigned int target_format)
{
	...
	inFframeInfo->private_data.optional_info[VID_OPT_HAVE_DTRC_INFO] != 0) {
		#ifdef CONFIG_VIOC_DTRC_DECOMP
		if (inFframeInfo->private_data.dtrcConv_info.m_iBitDepthLuma
			!= 8) {
	...
	if (ret > 0 && compressed_type == VID_OPT_HAVE_MC_INFO) {
		...
		vSrc_addr = (unsigned char *)ioremap_nocache(
			mapConv_info->m_FbcYOffsetAddr[PA],
	...
	#ifdef CONFIG_VIOC_DTRC_DECOMP
	if (ret > 0 && compressed_type == VID_OPT_HAVE_DTRC_INFO) {
		unsigned int LumaTblSize = 0, ChromaTblSize = 0;
		unsigned char *vSrc_addr, *vDst_addr;
		unsigned int pDst_addr = target_addr + szStream;
		vp9_compressed_info_t *dtrcConv_info;

		dtrcConv_info = &inFframeInfo->private_data.dtrcConv_info;
		...
		LumaTblSize = dtrcConv_info->m_iCompressionTableLumaSize;
		ChromaTblSize = dtrcConv_info->m_iCompressionTableChromaSize;

		dbg("%s: pre-processing(%dx%d - BitDepth(%d)\n",
			__func__,
			inFframeInfo->Frame_width, inFframeInfo->Frame_height,
			dtrcConv_info->m_iBitDepthLuma);
		dbg(" Y(0x%x-0x%x / 0x%x), Cb(0x%x-0x%x / 0x%x)\n",
			inFframeInfo->addr0,
			dtrcConv_info->m_CompressedY[PA],
			dtrcConv_info->m_CompressionTableLuma[PA],
			inFframeInfo->addr1,
			dtrcConv_info->m_CompressedCb[PA],
			dtrcConv_info->m_CompressionTableChroma[PA]);
		...
		if (vDst_addr != NULL) {
			...
			_tcc_vsync_release_virtaddr((void *)vDst_addr,
				pDst_addr, PAGE_ALIGN(512 * 1024));
			inFframeInfo->addr0 = dtrcConv_info->m_CompressedY[PA]
				= WmixerInfo->dst_y_addr;
			inFframeInfo->addr1 = dtrcConv_info->m_CompressedCb[PA]
				= WmixerInfo->dst_u_addr;
			dtrcConv_info->m_CompressionTableLuma[PA]
				= pDst_addr;
			dtrcConv_info->m_CompressionTableChroma[PA]
			...
			pr_info("[INF][VSYNC] %s: pre-processing(%dx%d - BitDepth(%d) Y(0x%x-0x%x / 0x%x), Cb(0x%x-0x%x / 0x%x) :: End(0x%x)\n",
				__func__, inFframeInfo->Frame_width,
				inFframeInfo->Frame_height,
				dtrcConv_info->m_iBitDepthLuma,
				inFframeInfo->addr0,
...
//
//	pDest_Vaddr =
//		(unsigned char *)ioremap_nocache(dest_addr, PAGE_ALIGN(nLen));
//
//	if (pDest_Vaddr != NULL) {
	...
	int dest_fmt)
{
	...
	#if defined(CONFIG_VIOC_10BIT) \
		&& defined(CONFIG_USE_FORCE_8BIT_OUT_LASTFRAME)
	fbmixer.dst_fmt_ext_info = 8;
	#endif
	...
	|| lastUpdated->private_data.optional_info[VID_OPT_HAVE_DTRC_INFO] != 0
		...
		!= 0) {
			...
			memcpy(&fbmixer.dtrcConv_info,
				&lastUpdated->private_data.dtrcConv_info,
				sizeof(vp9_compressed_info_t));
		}
		#endif
	}
	#endif
	...
	if (file) {
		...
		ret = file->f_op->unlocked_ioctl(file,
			TCC_WMIXER_VIOC_INFO_KERNEL,
			(unsigned long)&wmixer_id_info);
	...
	int output_mode)
{
	...
	if (lastUpdated->enable) {
		...
		#ifdef CONFIG_VIOC_DTRC_DECOMP
		if (iImage.private_data.optional_info[VID_OPT_HAVE_DTRC_INFO]
			!= 0) {
			iImage.private_data.optional_info[
				VID_OPT_HAVE_DTRC_INFO] = 0;
		}
		#endif
...
TCC_VSYNC_OPEN_ERROR:
	...
	case TCC_LCDC_VIDEO_PUSH_RENDER_INFO_KERNEL:
	{
		...
		if (cmd == TCC_LCDC_VIDEO_PUSH_RENDER_INFO_KERNEL) {
			if (memcpy((void *)&p->push_ext_infoframe,
				(const void *)arg,
				sizeof(struct tcc_lcdc_image_update)) == NULL) {
				pr_err("[ERR][VSYNC]  copy err");
			...
			if (copy_from_user((void *)&p->push_ext_infoframe,
				(const void *)arg,
				sizeof(struct tcc_lcdc_image_update))) {
				pr_err("[ERR][VSYNC]  copy err");
		...
		memcpy(&input_image->private_data, private_data,
			sizeof(TCC_PLATFORM_PRIVATE_PMEM_INFO));
...
void tcc_vsync_set_firstFrameFlag(tcc_video_disp *p,
	int firstFrameFlag)
{
	...
	int *lcd_video_started)
{
	...
	if (video_display_disable_check) {
		...
		|| defined(CONFIG_TCC_OUTPUT_ATTACH)
...
static int tcc_vsync_parse_dt(struct device_node *np)
{
	...
	memset(&vsync_vioc1_disp, 0x00,
		sizeof(struct tcc_vsync_display_info_t));
...
static int tcc_vsync_probe(struct platform_device *pdev)
{
	...
	#if defined(CONFIG_VSYNC_DRV_ALWAYS_ACCEPT_START_VSYNC)
	prepare_input_image_info[VSYNC_MAIN] = devm_kzalloc(&pdev->dev,
		sizeof(struct tcc_lcdc_image_update), GFP_KERNEL);
	prepare_input_image_info[VSYNC_SUB0] = devm_kzalloc(&pdev->dev,
		sizeof(struct tcc_lcdc_image_update), GFP_KERNEL);
...
static int __init tcc_vsync_init(void)
{
	...
	device_create(vsync_class, NULL, MKDEV(DevMajorNum, 0), NULL,
		DEVICE_NAME);
#endif
