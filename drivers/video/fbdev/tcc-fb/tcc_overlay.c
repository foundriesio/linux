...
static void tcc_overlay_configure_AFBCDEC(volatile void __iomem *pAFBC_Dec, unsigned int afbc_dec_id,
										...
										unsigned int base_addr, unsigned int fmt,
...
static int tcc_overlay_display_video_buffer(overlay_video_buffer_t buffer_cfg, struct overlay_drv_type *overlay_drv)
{
	...
	if(overlay_drv->rdma[layer].reg && overlay_drv->wmix.reg) {
		...
		dprintk("AFBC :: old_afbc(0x%p, %d), need(%d with %d), Layer(%d -> %d) \n", overlay_drv->afbc_dec.reg, get_vioc_index(overlay_drv->afbc_dec.id),
					buffer_cfg.afbc_dec_need, buffer_cfg.afbc_dec_num, overlay_drv->layer_nlast, layer);
	#endif

		if( (overlay_drv->afbc_dec.reg != NULL)
			 &&	(buffer_cfg.afbc_dec_need == 0 || (get_vioc_index(overlay_drv->afbc_dec.id) != buffer_cfg.afbc_dec_num) || (layer != overlay_drv->layer_nlast)))
		{
			volatile void __iomem *reg = overlay_drv->rdma[layer].reg;
			unsigned int id = overlay_drv->rdma[layer].id;

			if(layer != overlay_drv->layer_nlast){
				reg = overlay_drv->rdma[overlay_drv->layer_nlast].reg;
				id = overlay_drv->rdma[overlay_drv->layer_nlast].id;
			}

			tcc_overlay_configure_AFBCDEC(overlay_drv->afbc_dec.reg, overlay_drv->afbc_dec.id,
											reg, id,
											0/*for plug-out*/, 0, 0,
											buffer_cfg.addr, buffer_cfg.cfg.format,
											1, 0,
...
#endif
		VIOC_RDMA_SetImageSize(overlay_drv->rdma[layer].reg, buffer_cfg.cfg.width, buffer_cfg.cfg.height);
		VIOC_RDMA_SetImageFormat(overlay_drv->rdma[layer].reg, buffer_cfg.cfg.format);
		VIOC_RDMA_SetImageOffset(overlay_drv->rdma[layer].reg, buffer_cfg.cfg.format, buffer_cfg.cfg.width);
		...
		if(base1 == 0)
			base1 = GET_ADDR_YUV42X_spU(buffer_cfg.addr,  buffer_cfg.cfg.width, buffer_cfg.cfg.height);
		if(base2 == 0)
		{
			if(buffer_cfg.cfg.format == TCC_LCDC_IMG_FMT_444SEP )
				base2 		= (unsigned int)GET_ADDR_YUV42X_spU(base1, buffer_cfg.cfg.width, buffer_cfg.cfg.height);
			else if(buffer_cfg.cfg.format == TCC_LCDC_IMG_FMT_YUV422SP )
				base2 		= (unsigned int)GET_ADDR_YUV422_spV(base1, buffer_cfg.cfg.width, buffer_cfg.cfg.height);
			else
				base2 		= (unsigned int)GET_ADDR_YUV420_spV(base1, buffer_cfg.cfg.width, buffer_cfg.cfg.height);
		}
		...
		if(buffer_cfg.afbc_dec_need != 0) {
			...
			tcc_overlay_configure_AFBCDEC(overlay_drv->afbc_dec.reg, overlay_drv->afbc_dec.id,
											overlay_drv->rdma[layer].reg, overlay_drv->rdma[layer].id,
...
static long tcc_overlay_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		...
		case OVERLAY_PUSH_VIDEO_BUFFER:
		{
			...
			return tcc_overlay_display_video_buffer(overBuffCfg, overlay_drv);
		}
		...
		case OVERLAY_SET_LAYER:
			{
				...
				if(copy_from_user(&overlay_layer, (unsigned int *)arg, sizeof(unsigned int)))
					return -EFAULT;
...
//		VIOC_PlugInOutCheck VIOC_PlugIn;
		...
		if(overlay_drv->wmix.reg) {
			VIOC_WMIX_SetPosition(overlay_drv->wmix.reg, layer, 0, 0);
			VIOC_WMIX_SetUpdate(overlay_drv->wmix.reg);
		}
#if 0	/* TODO */
...
{	
	...
	{
		of_property_read_u32_index(pdev->dev.of_node, "rdmas", (overlay_drv->fb_dd_num == 0) ? i+1 : (OVERLAY_LAYER_MAX+i+1), &index);
		if(tmp_node) {
	...
	of_property_read_u32_index(pdev->dev.of_node, "wmixs", (overlay_drv->fb_dd_num == 0) ? 1 : 2, &index);
	if(tmp_node) {
