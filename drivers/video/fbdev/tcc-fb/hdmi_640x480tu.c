...
static int hdmi480tu_panel_init(struct lcd_panel *panel, struct tcc_dp_device *fb_pdata)
{
        int ret = -1;
...
static int hdmi480tu_set_power(struct lcd_panel *panel, int on, struct tcc_dp_device *fb_pdata)
{
        int ret = -1;
...
static int hdmi480ptu_probe(struct platform_device *pdev)
{
        ...
        dev = devm_kzalloc(&pdev->dev, sizeof(struct hdmi480tu_dev), GFP_KERNEL);
        if(dev == NULL) {
...
#ifdef CONFIG_OF
static struct of_device_id hdmi480ptu_of_match[] = {
