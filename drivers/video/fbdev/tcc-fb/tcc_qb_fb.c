...
int fb_quickboot_lastframe_display(void)
{
	...
	QB_Lastframe_cpu = dma_alloc_writecombine(
		0, size, &QB_Lastframe_dma, GFP_KERNEL); // virtual

	if (!QB_Lastframe_cpu) {
		pr_warn("%s Can not alloc back_up\n", __func__);
		err = -ENOMEM;
		goto free_memory;
	}

	dprintk(" %s back_up alloc addr = [0x%p] size = [%x]\n", __func__,
		QB_Lastframe_cpu, size);
...
void fb_quickboot_lastframe_display_release(void)
{
	...
	dma_free_writecombine(
		0, info->var.xres * info->var.yres * 4, QB_Lastframe_cpu,
		QB_Lastframe_dma);
}
EXPORT_SYMBOL(fb_quickboot_lastframe_display_release);

static void memset32(void *_ptr, unsigned short val, unsigned int count)
{
	char *ptr = _ptr;
	char b = val & 0x001f;
	char g = (val & 0x07e0) >> 5;
...
int fb_quickboot_progress_bar(int percent)
{
	...
	if (!mem_init) {
		...
		bar_addr_start = (char *)(info->screen_base)
			+ (info->var.xres * info->var.yoffset
			   * (info->var.bits_per_pixel / 8));
		QB_BAR_ADDR = pfbi->map_dma
			+ (info->var.xres * info->var.yoffset
			   * (info->var.bits_per_pixel / 8));
	...
	dprintk(" @@@ (%d, %d) ~ (%d, %d)\n", bar.sx, bar.sy, bar.width,
		bar.height);

	if (percent <= 100) {
		bar_addr =
			(bar_addr_start
			 + ((img_width * bar.sy * 4) + (bar.sx * 4)));
		bar_addr_width = (bar_addr + ((bar.width / 100) * 4));
		bar_addr_height =
			(bar_addr
			 + ((img_width * bar.height * 4)
			    + ((bar.width / 100) * 4)));
		...
		VIOC_RDMA_SetImageBase(
			pfbi->pdata.Mdp_data.rdma_info[RDMA_FB].virt_addr,
			QB_BAR_ADDR, 0, 0);
		...
		dprintk(" @@@ (%d, %d) ~ (%d, %d) percent = %d\n", bar.sx,
			bar.sy, bar.width, bar.height, percent);

		if (pfbi->pdata.Sdp_data.FbPowerState) {
			//tca_fb_activate_var(QB_BAR_ADDR,
			//&info->var, &pfbi->pdata.Sdp_data);
...
int fb_hibernation_logo(char *rle_filename)
{
	...
	BaseAddr = fbi->map_dma
		+ (info->var.xres * info->var.yoffset
		   * (info->var.bits_per_pixel
		      / 8)); // physical mem address setting for hdmi

	if (fbi->pdata.Sdp_data.FbPowerState) {
		//		tca_fb_activate_var(BaseAddr,  &info->var,
		//&fbi->pdata.Sdp_data);
...
int fb_hibernation_enter(void)
{
	...
	// back up framebuffer
	FrameSize = info->fb->var.xres * info->fb->var.yres
	...
	for (i = 0; i < info->fb->var.yres_virtual / info->fb->var.yres; i++) {
		...
		if (info->backup_map_cpu[i] != 0) {
			memcpy(info->backup_map_cpu[i],
			       info->map_cpu + Addroffset, FrameSize);
		} else {
...
int fb_quickboot_display_fb_resume(struct tccfb_info *info)
{
	...
	base_addr = info->map_dma
		+ (info->fb->var.xres * info->fb->var.yoffset
		   * (info->fb->var.bits_per_pixel / 8));
...
#endif
	{
		VIOC_WMIX_SetPosition(
			pdp_data->wmixer_info.virt_addr, imgch, 0, 0);
		VIOC_RDMA_SetImageFormat(
			pdp_data->rdma_info[RDMA_FB].virt_addr,
			TCC_LCDC_IMG_FMT_RGB888);
		VIOC_RDMA_SetImageOffset(
			pdp_data->rdma_info[RDMA_FB].virt_addr,
			TCC_LCDC_IMG_FMT_RGB888, info->fb->var.xres); // offset
		VIOC_RDMA_SetImageSize(
			pdp_data->rdma_info[RDMA_FB].virt_addr,
			info->fb->var.xres, info->fb->var.yres); // size
		VIOC_RDMA_SetImageBase(
			pdp_data->rdma_info[RDMA_FB].virt_addr, base_addr, 0,
			0);
#if defined(CONFIG_PLATFORM_AVN)
		VIOC_RDMA_SetImageAlphaSelect(
			pdp_data->rdma_info[RDMA_FB].virt_addr, 1);
...
void fb_quickboot_resume(struct tccfb_info *info)
{
	...
	pr_info("[INF][QB_FB] %s do_hibernation:%d do_hibernate_boot:%d size:%d  %d  FrameSize:%d\n",
		...
		ARRAY_SIZE(info->backup_map_cpu) /
		sizeof(info->backup_map_cpu[0]),
		FrameSize);
	...
	if (do_hibernation) {
		if (do_hibernate_boot) {
			FrameSize = info->fb->var.xres * info->fb->var.yres
				* info->fb->var.bits_per_pixel / 8;
			for (i = 0; i < sizeof(info->backup_map_cpu)
				     / sizeof(info->backup_map_cpu[0]);
			     i++) {
				pr_info("[INF][QB_FB] %d %p\n", i,
					info->backup_map_cpu[i]);
				if (info->backup_map_cpu[i]) {
					memcpy(info->map_cpu + (i * FrameSize),
					       info->backup_map_cpu[i],
					       FrameSize);
			...
			viocmg_set_wmix_ovp(
				VIOCMG_CALLERID_FB,
				pdp_data->wmixer_info.blk_num, 8);
#else
#if defined(CONFIG_PLATFORM_AVN)
