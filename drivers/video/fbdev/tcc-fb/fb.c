...
#endif
	{
		block_status = vioc_intr_get_status(get_vioc_index(
			par->pdata.ddc_info.blk_num));

		if (block_status & (0x1<<VIOC_DISP_INTR_RU)) {
			vioc_intr_clear(get_vioc_index(
				par->pdata.ddc_info.blk_num),
...
#endif
	{
		vioc_intr_clear(get_vioc_index(par->pdata.ddc_info.blk_num),
...
static int fbX_prepare_m2m(struct fb_info *info)
{
	...
	do {
		...
		if (par->pdata.FbUpdateType == FBX_ATTACH_UPDATE) {
			name = kasprintf(GFP_KERNEL, "/dev/attach%d",
				get_vioc_index(par->pdata.ddc_info.blk_num));
		} else {
			name = kasprintf(GFP_KERNEL, "/dev/scaler%d",
		...
		if (par->pdata.FbUpdateType == FBX_ATTACH_UPDATE) {
			name = kasprintf(GFP_KERNEL, "fb%d_video",
				info->node);
		} else {
			name = kasprintf(GFP_KERNEL, "fb_scaler%d",
				get_vioc_index(par->pdata.scaler_info.blk_num));
		}
	...
	struct fbX_par *par)
{
	...
	if (par->pdata.FbUpdateType == FBX_M2M_RDMA_UPDATE) {
		...
		scaler.dest_ImgWidth = par->pdata.region.width;   // dest w
		scaler.dest_ImgHeight = par->pdata.region.height; // dest h
		...
		if (par->pdata.filp) {
			par->pdata.filp->f_op->unlocked_ioctl(par->pdata.filp,
			TCC_SCALER_IOCTRL_KERENL, (unsigned long)&scaler);
		}
		...
		VIOC_WMIX_SetPosition(par->pdata.wmixer_info.virt_addr,
			channel, par->pdata.region.x, par->pdata.region.y);
		VIOC_WMIX_SetChromaKey(par->pdata.wmixer_info.virt_addr,
			channel, 1/*ON*/, 0x0, 0x0, 0x0, 0xF8, 0xFC, 0xF8);
	...
	unsigned int height)
{
	...
	if (bSet_Comp) {
		if (!fbdc_dec_1st_cfg) {
			...
			VIOC_PVRIC_FBDC_SetBasicConfiguration(pFBDC, base_addr,
				fmt, width, height, 0);
	...
	struct fbX_par *par)
{
	...
	if (par->pdata.scaler_info.virt_addr) {
		if (VIOC_CONFIG_GetScaler_PluginToRDMA(
			...
			VIOC_CONFIG_PlugIn(par->pdata.scaler_info.blk_num,
				par->pdata.rdma_info.blk_num);
		}
	}

	#if defined(CONFIG_VIOC_PVRIC_FBDC)
	tca_vioc_configure_FBCDEC(par->pdata.fbdc_info.blk_num, dma_addr,
		par->pdata.fbdc_info.virt_addr,
		par->pdata.rdma_info.virt_addr,
		par->pdata.wdma_info.virt_addr,
		format, par->pdata.rdma_info.blk_num,
	...
	if (par->pdata.FbWdmaPath  == 0) {
		...
		VIOC_WMIX_SetPosition(par->pdata.wmixer_info.virt_addr,
			channel, par->pdata.region.x, par->pdata.region.y);
		VIOC_WMIX_SetChromaKey(par->pdata.wmixer_info.virt_addr,
			channel, 1, 0x0, 0x0, 0x0, 0xF8, 0xFC, 0xF8);
		VIOC_WMIX_SetUpdate(par->pdata.wmixer_info.virt_addr);
	}

	if (par->pdata.scaler_info.virt_addr) {
		VIOC_SC_SetBypass(par->pdata.scaler_info.virt_addr, 0);
		VIOC_SC_SetDstSize(par->pdata.scaler_info.virt_addr,
			par->pdata.region.width, par->pdata.region.height);
		VIOC_SC_SetOutSize(par->pdata.scaler_info.virt_addr,
			par->pdata.region.width, par->pdata.region.height);
	...
	struct fb_info *info)
{
	...
	if (var->reserved[3] != 0) {
		...
		dma_addr = par->map_dma + (var->xres * var->yoffset
	...
	//pr_info("[INF][FBX] %s: fb%d addr:0x%08x - %s updateType:0x%x
	//yoffset:%d reserved:%d\n",
	...
	case FBX_ATTACH_UPDATE:
		if (par->pdata.filp == NULL) {
			fbX_prepare_m2m(info);
			fbX_m2m_activate_var((par->map_dma
...
static void *fb_ion_dma_buf_kmap(
...
struct dma_buf *new_tccfb_dmabuf_export(struct fb_info *info)
{
	...
	#else
	dmabuf = dma_buf_export(info, &new_fb_dma_buf_ops,
...
static void dmabuf_unmap_atomic(
	struct dma_buf *buf,
	unsigned long page,
	void *vaddr)
{
...
static int dmabuf_mmap(struct dma_buf *buf, struct vm_area_struct *vma)
{
	...
	return remap_pfn_range(vma, vma->vm_start,
		priv->fix.smem_start >> PAGE_SHIFT,
...
static int __init fb_map_video_memory(struct fb_info *info)
{
	...
	if (ret || resource_size(&res) == 0) {
		par->map_size  = info->var.xres_virtual * info->var.yres_virtual
			* (info->var.bits_per_pixel / 8);
		par->map_cpu = dma_alloc_writecombine(info->dev, par->map_size,
			&par->map_dma, GFP_KERNEL);
...
static inline void fb_unmap_video_memory(struct fb_info *info)
{
	...
	if (par->map_cpu) {
		dma_free_writecombine(info->dev, par->map_size,
			par->map_cpu, par->map_dma);
	}
}
...
#if !defined(CONFIG_VIOC_PVRIC_FBDC)
	if (par->pdata.FbUpdateType == FBX_OVERLAY_UPDATE) {
		vioc_intr_clear(VIOC_INTR_RD0+get_vioc_index(
			par->pdata.rdma_info.blk_num),
			(0x1 << VIOC_RDMA_INTR_EOFR));
...
static int fb_register_isr(struct fb_info *info)
{
	...
	if (par->pdata.FbUpdateType == FBX_OVERLAY_UPDATE) {
		...
		if (request_irq(par->pdata.rdma_info.irq_num,
			fbX_display_handler, IRQF_SHARED, info->fix.id, info)
			< 0) {
...
static int fb_dt_parse_data(struct fb_info *info)
{
	...
	if (info->dev->of_node != NULL) {
		if (of_property_read_u32(info->dev->of_node, "xres",
			&info->var.xres)) {
		...
		if (of_property_read_u32(info->dev->of_node, "yres",
			&info->var.yres)) {
		...
		if (of_property_read_u32(info->dev->of_node, "bpp",
		...
		if (of_property_read_u32(info->dev->of_node, "update-type",
			&par->pdata.FbUpdateType)) {
		...
		if (of_property_read_u32_index(info->dev->of_node,
			"device-priority", 0, &par->pdata.FbDeviceType)) {
			pr_err("[ERR][FBX] error in %s: can nod find update-type\n",
				__func__);
			ret = -ENODEV;
			goto err_dt_parse;
		...
		if (np != NULL) {
			of_property_read_u32_index(info->dev->of_node,
				"telechips,disp", property_idx,
				&par->pdata.ddc_info.blk_num);
			...
			par->pdata.vioc_clock = of_clk_get_by_name(np,
				"ddi-clk");
			BUG_ON(par->pdata.vioc_clock == NULL);
			par->pdata.FbWdmaPath = 0;
		} else {
			np = of_parse_phandle(info->dev->of_node,
				"telechips,wdma", 0);
		...
		of_property_read_u32_index(info->dev->of_node, "telechips,fbdc",
			property_idx, &par->pdata.fbdc_info.blk_num);
		par->pdata.fbdc_info.virt_addr =
		...
		of_property_read_u32_index(info->dev->of_node, "telechips,rdma",
			property_idx, &par->pdata.rdma_info.blk_num);
		par->pdata.rdma_info.virt_addr =
		...
		if (np) {
			...
			if (of_property_read_u32(np, "height",
	...
	const char *buf, size_t count)
{
	...
	if (par->pdata.FbLayerOrder != value) {
		...
		par->pdata.FbLayerOrder = value;
		VIOC_WMIX_SetOverlayPriority(par->pdata.wmixer_info.virt_addr,
			par->pdata.FbLayerOrder);
		VIOC_WMIX_SetUpdate(par->pdata.wmixer_info.virt_addr);
	}
...
static DEVICE_ATTR(ovp, 0664, fbX_ovp_show, fbX_ovp_store);
...
static int __init fbX_probe(struct platform_device *pdev)
{
...
#endif
	if (of_property_read_u32(pdev->dev.of_node, "no-kernel-logo",
		&no_kernel_logo)) {
	...
	if (panel_node && par->panel) {
		dev_info(info->dev, "[INFO][FBX] %s fb%d has framebuffer panel\n",
			__func__,
		...
		if (of_property_read_u32(
			...
			dev_info(info->dev, "[INFO][FBX] %s lcdc-mux-select=%d from remote node\n",
				__func__, par->pdata.lcdc_mux);
		} else {
			dev_err(info->dev, "[ERROR][FBX] %s can not found lcdc-mux-select from remote node\n",
				__func__);
			retval = -ENODEV;
			goto err_fb_free;
		}
	...
	snprintf(fbX_fix.id, sizeof(fbX_fix.id), "telechips,fb%d",
		of_alias_get_id(info->dev->of_node, "fb"));
	info->fix = fbX_fix;

	info->var.nonstd = 0;
	info->var.activate = par->pdata.FbUpdateType ==
		FBX_NOWAIT_UPDATE ? FB_ACTIVATE_NOW : FB_ACTIVATE_VBL;
	...
	info->pseudo_palette = devm_kzalloc(&pdev->dev,
		sizeof(unsigned int) * 16, GFP_KERNEL);
	...
	if (no_kernel_logo) {
		...
		if (fb_prepare_logo(info, FB_ROTATE_UR)) {
			...
			// So, we use fb_alloc_cmap_gfp function
			// (fb_default_camp(default_16_colors))
...
#if !defined(CONFIG_TCC803X_CA7S) && !defined(CONFIG_TCC805X_CA53Q)
	fbX_activate_var((par->map_dma + (info->var.xres * info->var.yoffset
		* info->var.bits_per_pixel/8)), &info->var, info->par);
#endif
	fbX_set_par(info);
...
static int fbx_turn_on_resource(struct fb_info *info)
{
	...
	if (par->panel) {
		...
		if (!IS_ERR_OR_NULL(par->pdata.ddc_clock)) {
			...
			dev_info(info->dev, "%s set ddc clock to %dHz\n",
...
static int fbx_turn_off_resource(struct fb_info *info)
{
	...
	if (scnum >= VIOC_SCALER0) {
		...
		dev_info(info->dev, "[INFO][FBX] reset rdma_%d with Scaler-%d\n",
			get_vioc_index(par->pdata.rdma_info.blk_num),
	...
	dev_info(info->dev, "[INFO][FBX] reset rdma_%d\n",
		get_vioc_index(par->pdata.rdma_info.blk_num));
