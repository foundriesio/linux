...
static int
at070tn93_panel_init(struct lcd_panel *panel, struct tcc_dp_device *fb_pdata)
{
...
static int at070tn93_set_power(
...
static int at070tn93_parse_dt(struct device_node *np)
{
	if (np) {
		if (of_property_read_u32(
			    np, "lcd-gpio-port",
			    &lcd_at070tn93.gpio_port_num)) {
			pr_err("[ERR][LCD] lcd-gpio-port\n");
		}

		lcd_at070tn93.power_on =
		...
		lcd_at070tn93.reset =
			of_get_named_gpio(np, "reset-on-gpios", 0);
		...
		pr_debug(
			"[DBG][LCD] %s  power : %d display :%d\n", __func__,
			lcd_at070tn93.power_on, lcd_at070tn93.display_on);
...
#ifdef CONFIG_OF
static const struct of_device_id at070tn93_of_match[] = {
