...
/**
 * If power gpio of ADV7613 set to low, hotplug detect pin of HDMI also goes to
 * low. If you do not use hot plug detect pin of HDMI port, please unmask the
 * below define.
 */
...
/* gpio */
	...
	int power_status;
	/* 0 : turn off by fb driver, 1: turn on by fb
	 * driver, 2: turn off by external app, 3: turn on
	 * by external app
	 */
...
static struct i2c_data_reg main_data_reg[] = {
	...
	{DEFAULT_MAIN_ADDR, 0xF4,
	 0x80}, /* ;default 0x00, CEC Map Address set to 0x80 */
	{DEFAULT_MAIN_ADDR, 0xF5,
	 0x7C}, /* ;default 0x00, Infoframe Map Address set to 0x7C */
	{DEFAULT_MAIN_ADDR, 0xF8,
	 0x4C}, /* ;default 0x00, DPLL Map Address set to 0x4C */
	{DEFAULT_MAIN_ADDR, 0xF9,
	 0x64}, /* ;default 0x00, KSV Map Address set to 0x64 */
	{DEFAULT_MAIN_ADDR, 0xFA,
	 0x6C}, /* ;default 0x00, EDID Map Address set to 0x6C */
	{DEFAULT_MAIN_ADDR, 0xFB,
	 0x68}, /* ;default 0x00, HDMI Map Address set to 0x68 */
	{DEFAULT_MAIN_ADDR, 0xFD,
	 0x44}, /* ;default 0x00, CP Map Address set to 0x44 */
	{DEFAULT_MAIN_ADDR, 0xE9,
	 0xC0}, /* ;default 0x00, LVDS Map Address set to 0xC0 */
	{DEFAULT_MAIN_ADDR, 0x00, 0x12},
	 /* ;default 0x08, [5:0] - VID_STD[5:0] = 6'b010010 - WXGA
	  * 1360x768p@60Hz
	  */
	{DEFAULT_MAIN_ADDR, 0x01,
	 0x06}, /* ;default 0x06, [3:0] - Prim_Mode[3:0] = 4'b0110 - HDMI-Gr */
	{DEFAULT_MAIN_ADDR, 0x02, 0xF2},
	/* ;default 0xF0, [7:4] - INP_COLOR_SPACE[3:0] = 4'b1111 -
	 * color space determined by HDMI block, [1] RGB_OUT - 1'b1 - RGB
	 * color space output
	 */
	{DEFAULT_MAIN_ADDR, 0x03, 0x42},
	/* ;default 0x00, [7:0] - OP_FORMAT_SEL[7:0] = 8'b01000010 -
	 * 36-bit 4:4:4 SDR Mode 0
	 */
	{DEFAULT_MAIN_ADDR, 0x04, 0x63}, /* ;enable CP timing adjustment */
	{DEFAULT_MAIN_ADDR, 0x05, 0x20},
	/* ;default 0x2C  [2] - AVCODE_INSERT_EN = 1'b0 added 0x20
	 * disable data blanking for script reload
	 */
	{DEFAULT_MAIN_ADDR, 0x0C,
	 0x42}, /* ;default 0x62, [5] - POWER_DOWN = 1'b0 - Powers up part  */
	{DEFAULT_MAIN_ADDR, 0x15, 0xAE},
	 /* ;default 0xBE, [4] - TRI_AUDIO = 1'b0 = untristate
	  * Audio , {3] - TRI_LLC = 1'b1 = tristate LLC, Bit{1] -
	  * TRI_PIX = 1'b1 = Tristate Pixel Pads
	  */
	{0x00, 0x00, 0x00}
};

struct i2c_data_reg hdmi_rd_info[] = {{DEFAULT_HDMI_ADDR, 0xea, 0x20},
...
static struct i2c_data_reg cp_data_reg[] = {
	{DEFAULT_CP_ADDR, 0x6C,
	 0x00}, /* ;default 0x10, ADI Recommended write */
...
static struct i2c_data_reg ksv_data_reg[] = {
	{DEFAULT_KSV_ADDR, 0x40, 0x81},
	/* ;default 0x83, BCAPS[7:0] - Disable HDCP 1.1 features */
	{0x00, 0x00, 0x00}
};

static struct i2c_data_reg hdmi_data_reg[] = {
	{DEFAULT_HDMI_ADDR, 0x03,
	 0x98}, /* ;default 0x18, ADI Recommended write */
	{DEFAULT_HDMI_ADDR, 0x10,
	 0xA5}, /* ;default 0x25, ADI Recommended write */
	{DEFAULT_HDMI_ADDR, 0x1B,
	 0x08}, /* ;default 0x18, ADI Recommended write */
	{DEFAULT_HDMI_ADDR, 0x45,
	 0x04}, /* ;default 0x00, ADI Recommended write */
	{DEFAULT_HDMI_ADDR, 0x97,
	 0xC0}, /* ;default 0x80, ADI Recommended write */
	{DEFAULT_HDMI_ADDR, 0x3D,
	 0x10}, /* ;default 0x00, ADI Recommended write */
	{DEFAULT_HDMI_ADDR, 0x3E,
	 0x7B}, /* ;default 0x79, ADI recommended write */
	{DEFAULT_HDMI_ADDR, 0x3F,
	 0x5E}, /* ;default 0x63, ADI Recommended Write */
	{DEFAULT_HDMI_ADDR, 0x4E,
	 0xFE}, /* ;default 0x7B, ADI recommended write */
	{DEFAULT_HDMI_ADDR, 0x4F,
	 0x08}, /* ;default 0x63, ADI recommended write */
	{DEFAULT_HDMI_ADDR, 0x57,
	 0xA3}, /* ;default 0x30, ADI recommended write */
	{DEFAULT_HDMI_ADDR, 0x58,
	 0x07}, /* ;default 0x01, ADI recommended write */
	{DEFAULT_HDMI_ADDR, 0x6F,
	 0x08}, /* ;default 0x00, ADI Recommended write */
	{DEFAULT_HDMI_ADDR, 0x83,
	 0xFE}, /* ;default 0xFF, ADI recommended write */
	{DEFAULT_HDMI_ADDR, 0x86,
	 0x9B}, /* ;default 0x00, ADI recommended write */
	{DEFAULT_HDMI_ADDR, 0x85,
	 0x10}, /* ;default 0x16, ADI recommended write */
	{DEFAULT_HDMI_ADDR, 0x89,
	 0x01}, /* ;default 0x00, ADI recommended write */
	{DEFAULT_HDMI_ADDR, 0x9B,
	 0x03}, /* ;default 0x0B, ADI Recommended write */
	{DEFAULT_HDMI_ADDR, 0x9C,
	 0x80}, /* ;default 0x08, ADI Recommended write */
	{DEFAULT_HDMI_ADDR, 0x9C,
	 0xC0}, /* ;default 0x08, ADI Recommended write */
	{DEFAULT_HDMI_ADDR, 0x9C,
	 0x00}, /* ;default 0x08, ADI Recommended write */
	{0x00, 0x00, 0x00}
};

static struct i2c_data_reg lvds_data_reg[] = {
	{DEFAULT_LVDS_ADDR, 0x40, 0x08},
	 /* ;default 0x02, Bit [0] tx_mode_itu656 - 1'b0 = OLDI
	  * mode, Bit [1] tx_pdn - 1'b0 = LVDS TX powered on, Bit
	  * [3] tx_pll_en - 1'b1 =  power up LVDS PLL
	  */
	{DEFAULT_LVDS_ADDR, 0x43,
	 0x03}, /* ;default 0x00, ADI Recommended write */
	{DEFAULT_LVDS_ADDR, 0x44, 0x00}, /* ;default 0x00, PLL GEAR < 200MHz */
	{DEFAULT_LVDS_ADDR, 0x45,
	 0x04}, /* ;default 0x1E, ADI Recommended write */
	{DEFAULT_LVDS_ADDR, 0x46,
	 0x53}, /* ;default 0x77, ADI Recommended write */
	{DEFAULT_LVDS_ADDR, 0x47,
	 0x03}, /* ;default 0x02, ADI Recommended write */
	{DEFAULT_LVDS_ADDR, 0x4C, 0x19},
	 /* ;default 0x71, Bit [6] tx_oldi_hs_pol - 1'b0 = HS
	  * Polarity Neg, Bit [5] tx_oldi_vs_pol - 1'b0 = VS
	  * Polarity Neg, Bit [4] tx_oldi_de_pol - 1'b1 =  DE
	  * Polarity Pos, Bit [3] tx_enable_ns_mapping - 1'b0 =
	  * normal oldi 8-bit mapping, Bit [2]
	  * tx_656_all_lanes_enable - 1'b0 = disable 656 data on
	  * lanes ,Bit [1] tx_oldi_balanced_mode - 1'b0 = Non DC
	  * balanced ,Bit [0] tx_color_mode - 1'b1 = 8-bit mode
	  */
	{
		DEFAULT_LVDS_ADDR,
		0x4E, 0x04}, /* ;default 0x08, Bit [3:2] tx_color_depth[1:0] -
			      * 2'b01 = 8-bit Mode,Bit [1] tx_int_res - 1'b0 =
			      * Res bit 0,Bit [0] tx_mux_int_res - 1'b0 =
			      * disable programming of res bit
			      */
	{0x00, 0x00, 0x00}
};

static int
adv7613_panel_init(struct lcd_panel *panel, struct tcc_dp_device *fb_pdata)
{
	int ret = -1;
...
static int
adv763_check(struct adv7613_dev *dev, struct i2c_data_reg *i2c_data_reg)
{
	int ret = -1;
	...
	do {
		...
		for (loop = 0;
		     !(i2c_data_reg[loop].addr == 0
		       ...
		       && i2c_data_reg[loop].val == 0);
		     loop++) {
			if (i2c_data_reg[loop].addr == DEFAULT_DELAY_ADDR) {
				mdelay(i2c_data_reg[loop].val);
			} else {
				retry = 3;
				do {
					regmap_read(
						regmap, i2c_data_reg[loop].reg,
						&val);
					if (val != i2c_data_reg[loop].val) {
						pr_info(" WARNING [%02d] A_0x%02x R_0x%02x 0x%02x : 0x%02x\r\n",
							loop,
							i2c_data_reg[loop].addr,
...
static int
adv763_update(struct adv7613_dev *dev, struct i2c_data_reg *i2c_data_reg)
{
	int ret = -1;
	int val, loop, retry = 0;
	struct regmap *regmap;
...
static int
hdmi_adv7613_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	int ret = -ENOMEM;
	...
	dev = devm_kzalloc(
		&client->dev, sizeof(struct adv7613_dev), GFP_KERNEL);
	if (dev == NULL)
		goto api_end;
	...
	of_property_read_u32(
		np, "main-slave", (u32 *)&dev->i2c_slave_address.main);
	of_property_read_u32(
		np, "cec-slave", (u32 *)&dev->i2c_slave_address.cec);
	of_property_read_u32(
		np, "info-slave", (u32 *)&dev->i2c_slave_address.info);
	of_property_read_u32(
		np, "dpll-slave", (u32 *)&dev->i2c_slave_address.dpll);
	of_property_read_u32(
		np, "ksv-slave", (u32 *)&dev->i2c_slave_address.ksv);
	of_property_read_u32(
		np, "edid-slave", (u32 *)&dev->i2c_slave_address.edid);
	of_property_read_u32(
		np, "hdmi-slave", (u32 *)&dev->i2c_slave_address.hdmi);
	of_property_read_u32(np, "cp-slave", (u32 *)&dev->i2c_slave_address.cp);
	of_property_read_u32(
		np, "lvds-slave", (u32 *)&dev->i2c_slave_address.lvds);
	...
	pr_info("hdmi-ext-panel = %d\r\n"
		...
		"cp-slave       = 0x%02x\r\n"
		"lvds-slave     = 0x%02x\r\n\r\n",
		hdmi_ext_panel, dev->i2c_slave_address.main,
		dev->i2c_slave_address.cec, dev->i2c_slave_address.info,
		dev->i2c_slave_address.dpll, dev->i2c_slave_address.ksv,
		dev->i2c_slave_address.edid, dev->i2c_slave_address.hdmi,
		dev->i2c_slave_address.cp, dev->i2c_slave_address.lvds);
#endif

	dev->i2c_regmap =
		devm_regmap_init_i2c(client, &hdmi_adv7613_regmap_config);
