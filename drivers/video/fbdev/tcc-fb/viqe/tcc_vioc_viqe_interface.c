...
static int not_exist_viqe1;
static int gUse_MapConverter;
#ifdef CONFIG_VIOC_DTRC
static int gUse_DtrcConverter;
#endif
...
#define iprintk(msg...) pr_info("[DBG][VIQE] " msg)
#define dvprintk(msg...) pr_info("[DBG][VIQE] DV_M2M: " msg)
#else
#define dprintk(msg...)
#define iprintk(msg...)
#define dvprintk(msg...)
...
void TCC_VIQE_DI_PlugInOut_forAlphablending(int plugIn)
{
	if (gUse_sDeintls) {
		if (plugIn) {
			...
			VIOC_CONFIG_PlugIn(viqe_common_info.gVIOC_Deintls,
				main_m2m_info.gVIQE_RDMA_num_m2m);
		} else {
		...
		if (plugIn) {
			VIOC_CONFIG_PlugIn(viqe_common_info.gVIOC_VIQE1,
				main_m2m_info.gVIQE_RDMA_num_m2m);
			VIOC_RDMA_SetImageIntl(main_m2m_info.pRDMABase_m2m, 1);
		} else {
...
void TCC_VIQE_DI_Init(struct VIQE_DI_TYPE *viqe_arg)
{
	...
	framebufHeight = 1088;
#else
	framebufWidth = ((viqe_arg->srcWidth - viqe_arg->crop_left
		- viqe_arg->crop_right) >> 3) << 3; // 8bit align
	framebufHeight = ((viqe_arg->srcHeight - viqe_arg->crop_top
		- viqe_arg->crop_bottom) >> 2) << 2; // 4bit align
#endif

	pr_info("[INF][VIQE] %s, W:%d, H:%d, FMT:%s, OddFirst:%d, %s-%d\n",
		__func__,
		framebufWidth, framebufHeight,
		(img_fmt?"YUV422":"YUV420"), viqe_arg->OddFirst,
		viqe_arg->use_sDeintls ? "S-Deintls" : "VIQE",
		viqe_arg->use_Viqe0 ? 0 : 1);
	...
	if (viqe_arg->use_sDeintls) {
		...
		if (not_exist_viqe1 || viqe_arg->use_Viqe0) {
			...
			if ((gPMEM_VIQE_SIZE / 2) < (imgSize * 4)) {
				...
				deintl_dma_base0 =
					gPMEM_VIQE_BASE + (gPMEM_VIQE_SIZE / 2);
			}
		}
		...
		VIOC_VIQE_SetControlRegister(pVIQE, framebufWidth,
			framebufHeight, img_fmt);
		VIOC_VIQE_SetDeintlRegister(pVIQE, img_fmt, top_size_dont_use,
...
void TCC_VIQE_DI_Run(struct VIQE_DI_TYPE *viqe_arg)
{
	...
	if (gUse_sDeintls) {
		...
		if (VIOC_CONFIG_Device_PlugState(nVIQE, &VIOC_PlugIn)
			== VIOC_DEVICE_INVALID) {
			return;
		}
		...
		if (viqe_arg->DI_use) {
			if (gbfield_30Hz) { // end fied of bottom field
				...
				// if you want to change the base address,
				// you call the RDMA SetImageBase function
				// in this line.
				gbfield_30Hz = 0;
			} else {
			...
			/* Y2RMode Default 0 (Studio Color) */
			VIOC_RDMA_SetImageY2RMode(pRDMA, 0x02);
			VIOC_VIQE_SetControlMode(pVIQE,
				OFF, OFF, OFF, OFF, OFF);
			VIOC_RDMA_SetImageIntl(pRDMA, 0);
			gFrmCnt_30Hz = 0;
		}
		...
		if (viqe_arg->DI_use) {
			...
			if (!gusingDI_S) {
				VIOC_CONFIG_PlugIn(
					viqe_common_info.gVIOC_Deintls, nRDMA);
				gusingDI_S = 1;
			}
		} else {
			VIOC_RDMA_SetImageIntl(pRDMA, 0);
	...
	struct tcc_lcdc_image_update *input_image, int type)
{
	...
	memcpy(&sub_m2m_image[type], input_image,
	...
	struct tcc_lcdc_image_update *input_image)
{
	...
	if (input_image->Lcdc_layer == RDMA_VIDEO_SUB) {
		...
		VIOC_VIQE_SetControlRegister(viqe_common_info.pVIQE1,
			framebufWidth, framebufHeight, img_fmt);
		VIOC_VIQE_SetDeintlRegister(viqe_common_info.pVIQE1,
			img_fmt, top_size_dont_use,
			framebufWidth, framebufHeight, gDI_mode_60Hz,
			deintl_dma_base0, deintl_dma_base1,
			deintl_dma_base2, deintl_dma_base3);
		VIOC_VIQE_SetControlEnable(viqe_common_info.pVIQE1,
			OFF, OFF, OFF, OFF, ON);

		VIOC_CONFIG_PlugIn(viqe_common_info.gVIOC_VIQE1,
			sub_m2m_info.gVIQE_RDMA_num_m2m);
		...
		VIOC_RDMA_SetImageIntl(main_m2m_info.pRDMABase_m2m, 1);
		VIOC_RDMA_SetImageBfield(main_m2m_info.pRDMABase_m2m,
			input_image->odd_first_flag);
		...
		#endif
		{
			...
			VIOC_VIQE_IgnoreDecError(viqe_common_info.pVIQE0,
				ON, ON, ON);
			VIOC_VIQE_SetControlRegister(viqe_common_info.pVIQE0,
				framebufWidth, framebufHeight, img_fmt);
			VIOC_VIQE_SetDeintlRegister(viqe_common_info.pVIQE0,
				img_fmt, top_size_dont_use,
				framebufWidth, framebufHeight, gDI_mode_60Hz,
				deintl_dma_base0, deintl_dma_base1,
				deintl_dma_base2, deintl_dma_base3);
			VIOC_VIQE_SetControlEnable(viqe_common_info.pVIQE0,
				OFF, OFF, OFF, OFF, ON);

			VIOC_CONFIG_PlugIn(viqe_common_info.gVIOC_VIQE0,
				main_m2m_info.gVIQE_RDMA_num_m2m);
...
m2m_init_scaler:
	if (input_image->Lcdc_layer == RDMA_VIDEO_SUB) {
		...
		if (input_image->private_data.dolbyVision_info.el_offset[0]
			!= 0x00) {
			struct tcc_lcdc_image_update ImageInfo_sub;

			memcpy(&ImageInfo_sub, input_image,
				sizeof(struct tcc_lcdc_image_update));
...
void TCC_VIQE_DI_Run60Hz_M2M(
	struct tcc_lcdc_image_update *input_image, int reset_frmCnt)
{
	VIOC_PlugInOutCheck VIOC_PlugIn;
	unsigned int JudderCnt = 0;
	...
	if (!input_image->deinterlace_mode) {
		if (input_image->Lcdc_layer == RDMA_VIDEO_SUB) {
			...
			VIOC_RDMA_SetImageY2REnable(main_m2m_info.pRDMABase_m2m,
				false);
			/* Y2RMode Default 0 (Studio Color) */
			VIOC_RDMA_SetImageY2RMode(main_m2m_info.pRDMABase_m2m,
				0x02);
			VIOC_RDMA_SetImageIntl(main_m2m_info.pRDMABase_m2m, 0);
		...
		if (input_image->Lcdc_layer == RDMA_VIDEO_SUB) {
			...
			if (gVIQE1_Init_State == 0) {
				pr_err("[ERR][VIQE] %s VIQE1 isn't init\n",
					__func__);
				return;
			}

			VIOC_CONFIG_Device_PlugState(
				viqe_common_info.gVIOC_VIQE1, &VIOC_PlugIn);
			if (VIOC_PlugIn.enable == 0) {
				pr_err("[ERR][VIQE] %s VIQE1 isn't pluged\n",
					__func__);
				return;
			}
			...
			VIOC_RDMA_SetImageY2REnable(sub_m2m_info.pRDMABase_m2m,
				false);
			VIOC_RDMA_SetImageY2RMode(sub_m2m_info.pRDMABase_m2m,
				0x02); /* Y2RMode Default 0 (Studio Color) */
			...
			if (input_image->frameInfo_interlace) {
				...
				VIOC_RDMA_SetImageIntl(
					sub_m2m_info.pRDMABase_m2m, 1);
			} else {
				VIOC_VIQE_SetDeintlMode(viqe_common_info.pVIQE1,
			...
			viqe_lock = 1;
			if (gVIQE_Init_State == 0)
				TCC_VIQE_DI_Init60Hz_M2M(gOutputMode,
					input_image);

			#endif

			if (gVIQE_Init_State == 0) {
				pr_err("[ERR][VIQE] %s VIQE isn't init\n",
					__func__);
				return;
			}
			...
			if (VIOC_PlugIn.enable) {
				...
				pr_err("[ERR][VIQE] %s VIQE isn't pluged\n",
					__func__);
				return;
			}
			...
			if (input_image->frameInfo_interlace) {
				...
				VIOC_VIQE_SetControlMode(
					...
					OFF, OFF, OFF, OFF, ON);
				VIOC_RDMA_SetImageIntl(
					main_m2m_info.pRDMABase_m2m, 1);
			} else {
				VIOC_VIQE_SetDeintlMode(viqe_common_info.pVIQE0,
					VIOC_VIQE_DEINTL_MODE_BYPASS);
	...
	struct tcc_lcdc_image_update *input_image)
{
	...
	if (atomic_read(&viqe_sub_m2m_init) == 0) {
		...
		if (TCC_VIQE_Scaler_Init_Buffer_M2M() < 0) {
			pr_err("[ERR][VIQE] %s-%d: memory alloc is failed\n",
				__func__, __LINE__);
			goto m2m_init_scaler;
		}
		...
		if (viqe_common_info.gBoard_stb == 1) {
			if (input_image->Lcdc_layer == RDMA_VIDEO_SUB) {
				VIOC_RDMA_SetImageDisable(
					sub_m2m_info.pRDMABase_m2m);
			} else if (input_image->Lcdc_layer == RDMA_VIDEO) {
				VIOC_RDMA_SetImageDisable(
					main_m2m_info.pRDMABase_m2m);
			}
		}
...
int TCC_VIQE_Scaler_Init_Buffer_M2M(void)
{
	...
	if (pmap_get_info("viqe", &pmap_info) < 0) {
...
#ifdef CONFIG_TCC_VIQE_FOR_SUB_M2M
	if (pmap_get_info("viqe1", &pmap_info) < 0) {
	...
	pr_info("[INF][VIQE] pmap :: 0: 0x%x/0x%x, 1: 0x%x/0x%x\n",
		gPMEM_VIQE_BASE, gPMEM_VIQE_SIZE,
		gPMEM_VIQE_SUB_BASE, gPMEM_VIQE_SUB_SIZE);
#else
	pr_info("[INF][VIQE] pmap :: 0: 0x%x/0x%x, 1: 0x%x/0x%x\n",
		gPMEM_VIQE_BASE, gPMEM_VIQE_SIZE, 0, 0);
#endif

	if (pmap_get_info("overlay", &pmap_info) < 0) {
	...
	if (pmap_info.size) {
		szBuffer = (u32)pmap_info.size / BUFFER_CNT_FOR_M2M_MODE;

		for (buffer_cnt = 0;
			buffer_cnt < BUFFER_CNT_FOR_M2M_MODE;
			buffer_cnt++) {
			overlay_buffer[0][buffer_cnt] =
				(unsigned int)pmap_info.base +
				(szBuffer * buffer_cnt);
	...
	if (pmap_get_info("overlay1", &pmap_info) < 0) {
...
unsigned int TCC_VIQE_Scaler_Get_Buffer_M2M(
	struct tcc_lcdc_image_update *input_image)
{
...
unsigned int TCC_VIQE_Scaler_Sub_Get_Buffer_M2M(
	struct tcc_lcdc_image_update *input_image)
{
	unsigned int buffer_addr = 0;
	...
	struct tcc_lcdc_image_update *input_image)
{
	...
	if (!scaler->data->irq_reged) {
		...
		ret = request_irq(scaler->irq, TCC_VIQE_Scaler_Handler60Hz_M2M,
			IRQF_SHARED, "m2m-main", scaler);
		if (ret) {
			pr_err("[ERR][VIQE] failed to acquire scaler1 request_irq\n");
			/* prevent KCS warning */
		}

		vioc_intr_enable(scaler->irq, scaler->vioc_intr->id,
...
#ifdef CONFIG_VIOC_MAP_DECOMP
		if (!gUse_MapConverter &&
		input_image->private_data.optional_info[VID_OPT_HAVE_MC_INFO]
		!= 0) {
...
#endif
#ifdef CONFIG_VIOC_DTRC
		if (!gUse_DtrcConverter &&
		input_image->private_data.optional_info[VID_OPT_HAVE_DTRC_INFO]
		!= 0) {
...
{
	if (layer == RDMA_VIDEO) {
		memcpy((struct tcc_lcdc_image_update *)input_image,
			scaler->info, sizeof(struct tcc_lcdc_image_update));
	} else {
		memcpy((struct tcc_lcdc_image_update *)input_image,
			&scaler_sub->info,
			sizeof(struct tcc_lcdc_image_update));
	...
	struct tcc_lcdc_image_update *input_image)
{
	...
	memcpy(scaler->info, (struct tcc_lcdc_image_update *)input_image,
		sizeof(struct tcc_lcdc_image_update));
	memcpy(&output_m2m_image, (struct tcc_lcdc_image_update *)input_image,
		sizeof(struct tcc_lcdc_image_update));
	#ifdef CONFIG_VIOC_MAP_DECOMP
	...
	if (scaler->info->private_data.optional_info[VID_OPT_HAVE_MC_INFO] != 0
		&& !(gUse_MapConverter&MAIN_USED)) {
	...
	if (scaler->info->private_data.optional_info[VID_OPT_HAVE_DTRC_INFO]
		!= 0 && !(gUse_DtrcConverter&MAIN_USED)) {
	...
	if ((gUse_DtrcConverter & MAIN_USED) &&
		scaler->info->private_data.optional_info[VID_OPT_HAVE_DTRC_INFO]
		!= 0) {
	...
	if (compressed_type == VID_OPT_HAVE_MC_INFO) {
		...
		VIOC_SC_SetDstSize(pSC_SCALERBase, scaler->info->Image_width,
			scaler->info->Image_height);
		VIOC_SC_SetOutSize(pSC_SCALERBase, scaler->info->Image_width,
			scaler->info->Image_height);
		VIOC_SC_SetOutPosition(pSC_SCALERBase, 0, 0);
		if (scaler->sc->id !=
			VIOC_CONFIG_GetScaler_PluginToRDMA(scaler->rdma->id)) {
			// rdma
			if (__raw_readl(pSC_RDMABase+RDMACTRL)
				& RDMACTRL_IEN_MASK) {
	...
	if (compressed_type == 0) {
		...
		if (gUse_MapConverter&MAIN_USED) {
			if (VIOC_CONFIG_DMAPath_Support()) {
				...
				VIOC_CONFIG_DMAPath_Set(scaler->rdma->id,
					scaler->rdma->id);
			} else {
				#if defined(CONFIG_ARCH_TCC803X) \
					|| defined(CONFIG_ARCH_TCC805X)
				VIOC_CONFIG_MCPath(scaler->wmix->id,
					scaler->rdma->id);
				#endif
			}
			gUse_MapConverter &= ~(MAIN_USED);
		...
		if (0) {//(scaler->info->interlaced) {
			VIOC_RDMA_SetImageSize(pSC_RDMABase,
				(scaler->info->crop_right
					- scaler->info->crop_left),
				(scaler->info->crop_bottom
					- scaler->info->crop_top) / 2);
			VIOC_RDMA_SetImageOffset(pSC_RDMABase,
				scaler->info->fmt,
				scaler->info->Frame_width*2);
		}
		...
		== 11) {
			...
			VIOC_RDMA_SetImageOffset(pSC_RDMABase,
				scaler->info->fmt,
				scaler->info->Frame_width*125/100);
		}
		#endif//
		else {
			VIOC_RDMA_SetImageSize(pSC_RDMABase,
				...
				(scaler->info->crop_bottom
					- scaler->info->crop_top));
			VIOC_RDMA_SetImageOffset(pSC_RDMABase,
				scaler->info->fmt, scaler->info->Frame_width);
		}
		...
		if (scaler->info->fmt >= VIOC_IMG_FMT_444SEP) {
			...
			scaler->info->crop_left =
				(scaler->info->crop_left >> 3) << 3;
			scaler->info->crop_right =
				scaler->info->crop_left + crop_width;
			scaler->info->crop_right =
				scaler->info->crop_left +
				(scaler->info->crop_right
					- scaler->info->crop_left);
			tccxxx_GetAddress(scaler->info->fmt,
				(unsigned int)scaler->info->addr0,
				scaler->info->Frame_width,
				scaler->info->Frame_height,
				scaler->info->crop_left,
				scaler->info->crop_top,
				&pSrcBase0, &pSrcBase1, &pSrcBase2);
		}
		VIOC_RDMA_SetImageY2REnable(pSC_RDMABase, 0);
		VIOC_RDMA_SetImageBase(pSC_RDMABase, (unsigned int)pSrcBase0,
			(unsigned int)pSrcBase1, (unsigned int)pSrcBase2);

		#ifdef CONFIG_VIOC_10BIT
		if (scaler->info->private_data.optional_info[VID_OPT_BIT_DEPTH]
			== 10) {
			/* YUV 10bit support */
	...
	VIOC_WMIX_SetSize(pSC_WMIXBase, scaler->info->Image_width,
		scaler->info->Image_height);
	VIOC_WMIX_SetUpdate(pSC_WMIXBase);

	VIOC_WDMA_SetImageFormat(pSC_WDMABase, dest_fmt);
	VIOC_WDMA_SetImageSize(pSC_WDMABase, scaler->info->Image_width,
		scaler->info->Image_height);
	VIOC_WDMA_SetImageOffset(pSC_WDMABase, dest_fmt,
		scaler->info->Image_width);
	if (scaler->info->dst_addr0 == 0) {
		scaler->info->dst_addr0 =
			TCC_VIQE_Scaler_Get_Buffer_M2M(input_image);
		/* prevent KCS warning */
	}
	...
	if ((DV_PROC_CHECK & DV_DUAL_MODE) == DV_DUAL_MODE) {
		...
		if (tca_edr_el_configure(input_image,
			&el_ImageInfo, &ratio) >= 0) {
	...
	struct tcc_lcdc_image_update *input_image)
{
	...
	if (!scaler_sub->data->irq_reged) {
		...
		vioc_intr_clear(scaler_sub->vioc_intr->id,
			scaler_sub->vioc_intr->bits);
		ret = request_irq(scaler_sub->irq,
			TCC_VIQE_Scaler_Sub_Handler60Hz_M2M, IRQF_SHARED,
			"m2m-sub", scaler_sub);

		if (ret) {
			pr_info("failed to acquire scaler3 request_irq\n");
			/* prevent KCS warning */
		}

		vioc_intr_enable(scaler_sub->irq, scaler_sub->vioc_intr->id,
			scaler_sub->vioc_intr->bits);
	...
	struct tcc_lcdc_image_update *input_image)
{
	...
	if (input_image->mosaic_mode) {
...
#ifdef CONFIG_VIOC_10BIT
		if (input_image->private_data.optional_info[VID_OPT_BIT_DEPTH]
			== 10) {
			VIOC_RDMA_SetImageSize(pSC_RDMABase,
				(input_image->crop_right
					- input_image->crop_left),
				(input_image->crop_bottom
					- input_image->crop_top));
			VIOC_RDMA_SetImageOffset(pSC_RDMABase,
				input_image->fmt, input_image->Frame_width * 2);
		} else if (
		...
		== 11) {
			...
			VIOC_RDMA_SetImageOffset(pSC_RDMABase,
				input_image->fmt,
				input_image->Frame_width * 125 / 100);
		}
#endif
		else {
			VIOC_RDMA_SetImageSize(pSC_RDMABase,
				...
				(input_image->crop_bottom
					- input_image->crop_top));
			VIOC_RDMA_SetImageOffset(pSC_RDMABase, input_image->fmt,
				input_image->Frame_width);
		}
		...
		if (input_image->fmt >= VIOC_IMG_FMT_444SEP) {
			dprintk("%s():  src addr is not allocate\n", __func__);
			input_image->crop_left =
				(input_image->crop_left >> 3) << 3;
			input_image->crop_right =
				input_image->crop_left + crop_width;
			input_image->crop_right =
				input_image->crop_left +
				(input_image->crop_right
					- input_image->crop_left);
			tccxxx_GetAddress(input_image->fmt,
				(unsigned int)input_image->addr0,
				input_image->Frame_width,
				input_image->Frame_height,
		...
		VIOC_SC_SetDstSize(pSC_SCALERBase,
			input_image->buffer_Image_width,
			input_image->buffer_Image_height);
		...
		VIOC_CONFIG_WMIXPath(scaler_sub->rdma->id, 0);
		...
		if (input_image->dst_addr0) {
			...
			tccxxx_GetAddress(dest_fmt,
				(unsigned int)input_image->dst_addr0,
				input_image->Image_width,
				input_image->Image_height,
				((input_image->buffer_offset_x
					- input_image->offset_x) >> 1) << 1,
				((input_image->buffer_offset_y
					- input_image->offset_y) >> 1) << 1,
				&pDstBase0, &pDstBase1, &pDstBase2);
			VIOC_WDMA_SetImageBase(pSC_WDMABase, pDstBase0,
				pDstBase1, pDstBase2);
		}
		VIOC_WDMA_SetImageR2YEnable(pSC_WDMABase, 0);
		VIOC_WDMA_SetImageEnable(pSC_WDMABase, 0);
	...
	struct tcc_lcdc_image_update *input_image)
{
	...
	memcpy(scaler_sub->info,
		(struct tcc_lcdc_image_update *)input_image,
		sizeof(struct tcc_lcdc_image_update));
	memcpy(&output_m2m_image_sub,
		(struct tcc_lcdc_image_update *)input_image,
		sizeof(struct tcc_lcdc_image_update));
#ifdef CONFIG_VIOC_MAP_DECOMP
	output_m2m_image_sub.private_data.optional_info[VID_OPT_HAVE_MC_INFO]
		= 0;
	output_m2m_image_sub.private_data.optional_info[VID_OPT_BIT_DEPTH] = 0;
#endif
#ifdef CONFIG_VIOC_DTRC
	output_m2m_image_sub.private_data.optional_info[VID_OPT_HAVE_DTRC_INFO]
		= 0;
	output_m2m_image_sub.private_data.optional_info[VID_OPT_BIT_DEPTH] = 0;
#endif
	...
	if (scaler_sub->info->private_data.optional_info[VID_OPT_HAVE_MC_INFO]
		!= 0 && !(gUse_MapConverter&SUB_USED)) {
	...
	if (scaler_sub->info->private_data.optional_info[VID_OPT_HAVE_DTRC_INFO]
		!= 0 && !(gUse_DtrcConverter&SUB_USED)) {
	...
	scaler_sub->info->private_data.optional_info[VID_OPT_HAVE_DTRC_INFO]
		!= 0) {
		compressed_type = VID_OPT_HAVE_DTRC_INFO;
	}
	#endif
	...
	scaler_sub->info->private_data.optional_info[VID_OPT_HAVE_MC_INFO]
		!= 0) {
		compressed_type = VID_OPT_HAVE_MC_INFO;
	}
	#endif
	...
	if (compressed_type == VID_OPT_HAVE_MC_INFO) {
		...
		if (scaler_sub->sc->id !=
			...
			if (__raw_readl(pSC_RDMABase+RDMACTRL)
				& RDMACTRL_IEN_MASK)
				VIOC_RDMA_SetImageDisable(pSC_RDMABase);
			VIOC_CONFIG_PlugIn(scaler_sub->sc->id,
				scaler_sub->rdma->id);
		}
		VIOC_SC_SetUpdate(pSC_SCALERBase);
	...
	if (compressed_type == VID_OPT_HAVE_DTRC_INFO) {
		...
		if (VIOC_CONFIG_DMAPath_Support()) {
			VIOC_CONFIG_DMAPath_UnSet(scaler_sub->rdma->id);
			VIOC_CONFIG_DMAPath_Set(scaler_sub->rdma->id,
				VIOC_DTRC1);
		}
	...
	if (compressed_type == 0) {
		...
		if (gUse_MapConverter&SUB_USED) {
			if (VIOC_CONFIG_DMAPath_Support()) {
				...
				#if defined(CONFIG_ARCH_TCC803X) \
				|| defined(CONFIG_ARCH_TCC805X)
				VIOC_CONFIG_MCPath(scaler_sub->wmix->id,
					scaler_sub->rdma->id);
				#endif
			}
			gUse_MapConverter &= ~(SUB_USED);
		...
		if (0) {//(scaler_sub->info->interlaced) {
			VIOC_RDMA_SetImageSize(pSC_RDMABase,
				(scaler_sub->info->crop_right
					- scaler_sub->info->crop_left),
				(scaler_sub->info->crop_bottom
					- scaler_sub->info->crop_top) / 2);
			VIOC_RDMA_SetImageOffset(pSC_RDMABase,
				scaler_sub->info->fmt,
				scaler_sub->info->Frame_width*2);
		}
		#ifdef CONFIG_VIOC_10BIT
		else if (
		scaler_sub->info->private_data.optional_info[VID_OPT_BIT_DEPTH]
		== 10) {
			VIOC_RDMA_SetImageSize(pSC_RDMABase,
				(scaler_sub->info->crop_right
					- scaler_sub->info->crop_left),
				(scaler_sub->info->crop_bottom
					- scaler_sub->info->crop_top));
			VIOC_RDMA_SetImageOffset(pSC_RDMABase,
				scaler_sub->info->fmt,
				scaler_sub->info->Frame_width * 2);
		} else if (
		...
		== 11) {
			...
			VIOC_RDMA_SetImageOffset(pSC_RDMABase,
				scaler_sub->info->fmt,
				scaler_sub->info->Frame_width * 125 / 100);
		}
		#endif
		else {
			VIOC_RDMA_SetImageSize(pSC_RDMABase,
		...
		if (scaler_sub->info->fmt >= VIOC_IMG_FMT_444SEP) {
			...
			dprintk("%s():  src addr is not allocate\n", __func__);
			scaler_sub->info->crop_left =
				(scaler_sub->info->crop_left >> 3) << 3;
			scaler_sub->info->crop_right =
				scaler_sub->info->crop_left + crop_width;
			scaler_sub->info->crop_right =
				scaler_sub->info->crop_left
				+ (scaler_sub->info->crop_right
				- scaler_sub->info->crop_left);
			tccxxx_GetAddress(scaler_sub->info->fmt,
				(unsigned int)scaler_sub->info->addr0,
				scaler_sub->info->Frame_width,
				scaler_sub->info->Frame_height,
				scaler_sub->info->crop_left,
				scaler_sub->info->crop_top,
				&pSrcBase0, &pSrcBase1, &pSrcBase2);
		}
		...
		if (scaler_sub->info->mosaic_mode) {
			VIOC_SC_SetDstSize(pSC_SCALERBase,
				scaler_sub->info->buffer_Image_width,
				scaler_sub->info->buffer_Image_height);
			VIOC_SC_SetOutSize(pSC_SCALERBase,
				scaler_sub->info->buffer_Image_width,
				scaler_sub->info->buffer_Image_height);
		} else
	...
	VIOC_CONFIG_WMIXPath(scaler_sub->rdma->id, 1);
	...
	if (scaler_sub->info->mosaic_mode) {
		...
		if (scaler_sub->info->dst_addr0 == 0)
			scaler_sub->info->dst_addr0 =
			TCC_VIQE_Scaler_Sub_Get_Buffer_M2M(input_image);
		if (scaler_sub->info->dst_addr0) {
			unsigned int pDstBase0, pDstBase1, pDstBase2;

			pDstBase0 = pDstBase1 = pDstBase2 = 0;
			tccxxx_GetAddress(dest_fmt,
				(unsigned int)scaler_sub->info->dst_addr0,
				scaler_sub->info->Image_width,
				scaler_sub->info->Image_height,
				((scaler_sub->info->buffer_offset_x
					- scaler_sub->info->offset_x) >> 1)
					<< 1,
				((scaler_sub->info->buffer_offset_y
					- scaler_sub->info->offset_y) >> 1)
					<< 1,
				&pDstBase0, &pDstBase1, &pDstBase2);
			VIOC_WDMA_SetImageBase(pSC_WDMABase, pDstBase0,
				pDstBase1, pDstBase2);
...
#endif
	{
		...
		VIOC_WDMA_SetImageOffset(pSC_WDMABase,
			dest_fmt, scaler_sub->info->Image_width);
		if (scaler_sub->info->dst_addr0 == 0) {
			scaler_sub->info->dst_addr0 =
				TCC_VIQE_Scaler_Sub_Get_Buffer_M2M(input_image);
		}
		VIOC_WDMA_SetImageBase(pSC_WDMABase,
			(unsigned int)scaler_sub->info->dst_addr0,
			(unsigned int)scaler_sub->info->dst_addr1,
			(unsigned int)scaler_sub->info->dst_addr2);
	}
	...
	dvprintk("%s-%d : Done(0x%x)\n", __func__, __LINE__, DV_PROC_CHECK);
	if ((DV_PROC_CHECK & DV_DUAL_MODE) == DV_DUAL_MODE) {
		output_m2m_image.private_data.dolbyVision_info.el_offset[0]
			= output_m2m_image_sub.addr0;
		output_m2m_image.private_data.dolbyVision_info.el_buffer_width
			= output_m2m_image_sub.Frame_width;
		output_m2m_image.private_data.dolbyVision_info.el_buffer_height
			= output_m2m_image_sub.Frame_height;
		output_m2m_image.private_data.dolbyVision_info.el_frame_width
			= (output_m2m_image_sub.crop_right
				- output_m2m_image_sub.crop_left);
		output_m2m_image.private_data.dolbyVision_info.el_frame_height
			= (output_m2m_image_sub.crop_bottom
				- output_m2m_image_sub.crop_top);
	}
#endif
...
{
	dprintk("%s():  In -open(%d), irq(%d)\n", __func__,
		scaler_sub->data->dev_opened, scaler_sub->data->irq_reged);
	...
	if (scaler_sub->data->dev_opened == 0) {
		if (scaler_sub->data->irq_reged) {
			vioc_intr_disable(scaler_sub->irq,
				scaler_sub->vioc_intr->id,
				scaler_sub->vioc_intr->bits);
	...
	int irq, void *client_data)
{
	...
	if (is_vioc_intr_activatied(scaler->vioc_intr->id,
		scaler->vioc_intr->bits) == false) {
		//dprintk("%s-%d\n", __func__, __LINE__);
		return IRQ_NONE;
	}

	if ((__raw_readl(pSC_WDMABase + WDMAIRQSTS_OFFSET))
...
#endif
		{
			...
			if ((DV_PROC_CHECK & DV_DUAL_MODE) == DV_DUAL_MODE) {
				...
				dvprintk("%s-%d : Done(0x%x)\n",
					__func__, __LINE__, DV_PROC_CHECK);
				if ((DV_PROC_CHECK & DV_ALL_DONE)
					== DV_ALL_DONE) {
					TCC_VIQE_Display_Update60Hz_M2M(
						&output_m2m_image);
				}
			} else
#endif
	...
	int irq, void *client_data)
{
	...
	if (is_vioc_intr_activatied(scaler_sub->vioc_intr->id,
		scaler_sub->vioc_intr->bits) == false) {
		return IRQ_NONE;
	}
...
#ifdef CONFIG_USE_SUB_MULTI_FRAME
			if (scaler_sub->info->mosaic_mode
				&& !list_empty(&video_queue_list)) {
				struct video_queue_t *q;

				q = video_queue_pop();
				if (q) {
					TCC_VIQE_Scaler_Sub_Repeat60Hz_M2M(
						&sub_m2m_image[q->type]);
	...
	struct tcc_lcdc_image_update *input_image)
{
	...
	dprintk("%s: FB:%dx%d SRC:%dx%d DST:%dx%d(%d,%d) %d, %s, %d\n",
		...
		input_image->odd_first_flag);
	//pr_info("%s: %dx%d, %dx%d, %d, %s, %d\n",
	//	__func__, framebufWidth, framebufHeight,
	//	input_image->Image_width, input_image->Image_height,
	//	input_image->fmt, (gViqe_fmt_60Hz ? "YUV422" : "YUV420"),
	//	input_image->odd_first_flag);

	VIOC_DISP_GetSize(pViqe_60hz_info->pDISPBase_60Hz,
	...
	if (gDeintlS_Use_60Hz) {
		VIOC_RDMA_SetImageY2REnable(pViqe_60hz_info->pRDMABase_60Hz,
			true);
		/* Y2RMode Default 0 (Studio Color) */
		VIOC_RDMA_SetImageY2RMode(pViqe_60hz_info->pRDMABase_60Hz,
			0x02);
	} else {
		VIOC_RDMA_SetImageY2REnable(pViqe_60hz_info->pRDMABase_60Hz,
			false);
	...
	VIOC_RDMA_SetImageOffset(pViqe_60hz_info->pRDMABase_60Hz,
		input_image->fmt, framebufWidth);
	VIOC_RDMA_SetImageFormat(pViqe_60hz_info->pRDMABase_60Hz,
		input_image->fmt);
	VIOC_RDMA_SetImageSize(pViqe_60hz_info->pRDMABase_60Hz,
		framebufWidth, framebufHeight);
	VIOC_RDMA_SetImageIntl(pViqe_60hz_info->pRDMABase_60Hz, 1);
	VIOC_RDMA_SetImageBfield(pViqe_60hz_info->pRDMABase_60Hz,
		input_image->odd_first_flag);
	...
	if (gDeintlS_Use_60Hz) {
		...
		VIOC_VIQE_SetControlRegister(pVIQE_Info, framebufWidth,
			framebufHeight, gViqe_fmt_60Hz);
		VIOC_VIQE_SetDeintlRegister(pVIQE_Info, gViqe_fmt_60Hz,
			top_size_dont_use, framebufWidth, framebufHeight,
			gDI_mode_60Hz, deintl_dma_base0,
			deintl_dma_base1, deintl_dma_base2, deintl_dma_base3);
		// for bypass path on progressive frame
		//VIOC_VIQE_SetDenoise(pVIQE_Info, gViqe_fmt_60Hz,
		//	framebufWidth, framebufHeight, 1, 0,
		//	deintl_dma_base0, deintl_dma_base1);
		...
		if ((VIOC_CONFIG_DV_GET_EDR_PATH()
			|| vioc_v_dv_get_stage() != DV_OFF)
			&& (input_image->Lcdc_layer == RDMA_VIDEO)) {
			y2r_on = false;
		}
	#endif

		VIOC_VIQE_SetImageY2REnable(pVIQE_Info, y2r_on);
		VIOC_CONFIG_PlugIn(nVIOC_VIQE,
			pViqe_60hz_info->gVIQE_RDMA_num_60Hz);
	}
	...
	if (input_image->on_the_fly) {
		...
		VIOC_CONFIG_PlugIn(pViqe_60hz_info->gSCALER_num_60Hz,
			pViqe_60hz_info->gVIQE_RDMA_num_60Hz);
	...
	VIOC_WMIX_SetPosition(pViqe_60hz_info->pWMIXBase_60Hz,
		gLcdc_layer_60Hz,
		input_image->offset_x, input_image->offset_y);
...
void TCC_VIQE_DI_Swap60Hz(int mode)
{
	...
	dprintk("%s: %d, %d, %d, %d\n", __func__,
		gOutputMode, gLcdc_layer_60Hz, VIOC_PlugIn.enable, mode);
	...
	int reset_frmCnt)
{
...
#ifdef CONFIG_VIOC_DOLBY_VISION_EDR
	if (
	input_image->private_data.optional_info[VID_OPT_HAVE_DOLBYVISION_INFO]
	!= 0) {
		if (!bStep_Check) {
			nFrame++;
		} else {
	...
	if (!gDeintlS_Use_60Hz) {
		...
		if (viocmg_lock_viqe(VIOCMG_CALLERID_VOUT) < 0) {
			...
			VIOC_RDMA_GetImageEnable(
				pViqe_60hz_info->pRDMABase_60Hz, &enable);
			if (enable) {
				VIOC_RDMA_SetImageDisable(
					pViqe_60hz_info->pRDMABase_60Hz);
			}
		...
		if (!plug_status.enable
			|| plug_status.connect_statue != VIOC_PATH_CONNECTED) {
			reset_frmCnt = VIQE_RESET_RECOVERY;
		}
	}
	#endif

	if (reset_frmCnt != VIQE_RESET_NONE
		...
		dprintk("VIQE%d 3D -> 2D :: %d -> %dx%d\n",
			get_vioc_index(nVIOC_VIQE), reset_frmCnt,
	...
	if (frmCnt_60hz == 0 && (gDeintlS_Use_60Hz == 0)) {
		...
		// you have to set physical address for using DMA register.
		// If 2D(spatial) mode, these registers are ignored
		imgSize = (input_image->Frame_width *
			(input_image->Frame_height / 2) * 4 * 3 / 2);
		...
		VIOC_VIQE_IgnoreDecError(pVIQE_Info, ON, ON, ON);
		VIOC_VIQE_SetControlRegister(pVIQE_Info,
			input_image->Frame_width,
			input_image->Frame_height, gViqe_fmt_60Hz);
		VIOC_VIQE_SetDeintlRegister(pVIQE_Info, gViqe_fmt_60Hz,
			top_size_dont_use, input_image->Frame_width,
			input_image->Frame_height, gDI_mode_60Hz,
			deintl_dma_base0, deintl_dma_base1,
			deintl_dma_base2, deintl_dma_base3);
		// for bypass path on progressive frame
		//VIOC_VIQE_SetDenoise(pVIQE_Info, gViqe_fmt_60Hz,
		//	srcWidth, srcHeight, 1, 0,
		//	deintl_dma_base0, deintl_dma_base1);
	...
	tccxxx_GetAddress(gImg_fmt_60Hz, (unsigned int)input_image->addr0,
		input_image->Frame_width, input_image->Frame_height,
		crop_left, crop_top, &addr_Y, &addr_U, &addr_V);
	VIOC_RDMA_SetImageSize(pViqe_60hz_info->pRDMABase_60Hz,
		cropWidth, cropHeight);
	VIOC_RDMA_SetImageBase(pViqe_60hz_info->pRDMABase_60Hz,
		addr_Y, addr_U, addr_V);

#ifdef CONFIG_VIOC_10BIT
	if (input_image->private_data.optional_info[VID_OPT_BIT_DEPTH] == 10) {
		VIOC_RDMA_SetDataFormat(pViqe_60hz_info->pRDMABase_60Hz,
			0x1, 0x1);	/* YUV 10bit support */
	} else if (input_image->private_data.optional_info[VID_OPT_BIT_DEPTH]
		== 11) {
		VIOC_RDMA_SetDataFormat(pViqe_60hz_info->pRDMABase_60Hz,
			0x3, 0x0);	/* YUV real 10bit support */
	} else {
		VIOC_RDMA_SetDataFormat(pViqe_60hz_info->pRDMABase_60Hz,
			0x0, 0x0);	/* YUV 8bit support */
	}

	if (input_image->private_data.optional_info[VID_OPT_BIT_DEPTH] == 10) {
		VIOC_RDMA_SetImageOffset(pViqe_60hz_info->pRDMABase_60Hz,
			gImg_fmt_60Hz, input_image->Frame_width * 2);
	} else if (input_image->private_data.optional_info[VID_OPT_BIT_DEPTH]
		== 11) {
		VIOC_RDMA_SetImageOffset(pViqe_60hz_info->pRDMABase_60Hz,
			gImg_fmt_60Hz, input_image->Frame_width * 125 / 100);
	} else
#endif
	{
		VIOC_RDMA_SetImageOffset(pViqe_60hz_info->pRDMABase_60Hz,
			gImg_fmt_60Hz, input_image->Frame_width);
	}
	...
	if (input_image->odd_first_flag) {
		...
		input_image->Lcdc_layer, input_image->on_the_fly,
		addr_Y, addr_U,
		);
	...
	if (gDeintlS_Use_60Hz) {
		...
		if (input_image->frameInfo_interlace) {
			VIOC_RDMA_SetImageIntl(pViqe_60hz_info->pRDMABase_60Hz,
				1);
		...
		if (input_image->frameInfo_interlace) {
			...
			VIOC_RDMA_SetImageY2REnable(
				pViqe_60hz_info->pRDMABase_60Hz, false);
			/* Y2RMode Default 0 (Studio Color) */
			VIOC_RDMA_SetImageY2RMode(
				pViqe_60hz_info->pRDMABase_60Hz, 0x02);
			if (frmCnt_60hz >= 3) {
				VIOC_VIQE_SetDeintlMode(pVIQE_Info,
					VIOC_VIQE_DEINTL_MODE_3D);
#ifndef USE_DEINTERLACE_S_IN60Hz
				// Process JUDDER CNTS
				JudderCnt = (input_image->Frame_width + 64)
					/ 64 - 1;
				VIOC_VIQE_SetDeintlJudderCnt(pVIQE_Info,
					JudderCnt);
#endif
			...
			VIOC_VIQE_SetControlMode(
				pVIQE_Info, OFF, OFF, OFF, OFF, ON);
			VIOC_RDMA_SetImageIntl(
				pViqe_60hz_info->pRDMABase_60Hz, 1);
		} else {
			VIOC_RDMA_SetImageY2REnable(
				pViqe_60hz_info->pRDMABase_60Hz, true);
			/* Y2RMode Default 0 (Studio Color) */
			VIOC_RDMA_SetImageY2RMode(
				pViqe_60hz_info->pRDMABase_60Hz, 0x02);
			VIOC_VIQE_SetControlMode(pVIQE_Info,
				OFF, OFF, OFF, OFF, OFF);
			VIOC_RDMA_SetImageIntl(
				pViqe_60hz_info->pRDMABase_60Hz, 0);
			if (input_image->Lcdc_layer == RDMA_VIDEO) {
				atomic_set(&gFrmCnt_60Hz, 0);
#ifdef CONFIG_TCC_VIQE_FOR_SUB_M2M
			} else if (input_image->Lcdc_layer == RDMA_VIDEO_SUB) {
	...
	if (input_image->on_the_fly) {
		if (VIOC_CONFIG_GetScaler_PluginToRDMA(
			pViqe_60hz_info->gVIQE_RDMA_num_60Hz)
			!= pViqe_60hz_info->gSCALER_num_60Hz) {
			VIOC_CONFIG_PlugIn(pViqe_60hz_info->gSCALER_num_60Hz,
				pViqe_60hz_info->gVIQE_RDMA_num_60Hz);
			VIOC_SC_SetBypass(pViqe_60hz_info->pSCALERBase_60Hz,
				OFF);
		}
	...
	if (VIOC_CONFIG_DV_GET_EDR_PATH()) {
		...
		if (input_image->Lcdc_layer == RDMA_VIDEO
			|| input_image->Lcdc_layer == RDMA_LASTFRM) {

			if (input_image->Lcdc_layer == RDMA_LASTFRM &&
				(input_image->fmt >= TCC_LCDC_IMG_FMT_444SEP
				&&
				input_image->fmt <= TCC_LCDC_IMG_FMT_YUV422ITL1)
			) {
				VIOC_RDMA_SetImageY2REnable(
					pViqe_60hz_info->pRDMABase_60Hz, true);
				VIOC_RDMA_SetImageY2RMode(
					pViqe_60hz_info->pRDMABase_60Hz, 0x2);
			}

			if (vioc_get_out_type() ==
		input_image->private_data.dolbyVision_info.reg_out_type) {
				VIOC_V_DV_SetSize(NULL,
					pViqe_60hz_info->pRDMABase_60Hz,
					input_image->offset_x,
					input_image->offset_y,
					Hactive, Vactive);
				vioc_v_dv_prog(
		input_image->private_data.dolbyVision_info.md_hdmi_addr,
...
void TCC_VIQE_DI_DeInit60Hz(void)
{
	...
	VIOC_CONFIG_PlugOut(pViqe_60hz_info->gSCALER_num_60Hz);
	VIOC_CONFIG_SWReset(pViqe_60hz_info->gSCALER_num_60Hz,
		VIOC_CONFIG_RESET);
	VIOC_CONFIG_SWReset(pViqe_60hz_info->gSCALER_num_60Hz,
		VIOC_CONFIG_CLEAR);
	VIOC_CONFIG_SWReset(pViqe_60hz_info->gVIQE_RDMA_num_60Hz,
		VIOC_CONFIG_RESET);
	VIOC_CONFIG_SWReset(pViqe_60hz_info->gVIQE_RDMA_num_60Hz,
		VIOC_CONFIG_CLEAR);
...
unsigned int tcc_viqe_commom_dt_parse(
	struct device_node *np,
	struct tcc_viqe_common_virt_addr_info_t *viqe_common_info)
{
	struct device_node *viqe_node0, *viqe_node1, *deintls_node;

	#ifdef CONFIG_OF
	viqe_node0 = of_parse_phandle(np, "telechips,viqe,0", 0);
	of_property_read_u32_index(np, "telechips,viqe,0", 1,
		&viqe_common_info->gVIOC_VIQE0);
	if (!viqe_node0) {
	...
	of_property_read_u32_index(np, "telechips,viqe,1", 1,
	...
	deintls_node = of_find_compatible_node(NULL, NULL,
		"telechips,vioc_deintls");
	of_property_read_u32(np, "vioc_deintls",
		&viqe_common_info->gVIOC_Deintls);
	...
	iprintk("Deintls(0x%x) Viqe0(0x%x) Viqe1(0x%x) gBoard_stb(%d)\n",
		viqe_common_info->gVIOC_Deintls, viqe_common_info->gVIOC_VIQE0,
		viqe_common_info->gVIOC_VIQE1, viqe_common_info->gBoard_stb);
	#endif

	return 0;
}

unsigned int tcc_viqe_60hz_dt_parse(
	struct device_node *np,
	struct tcc_viqe_60hz_virt_addr_info_t *viqe_60hz_info)
{
	...
	of_property_read_u32_index(np, "telechips,rdma,60", 1,
		&viqe_60hz_info->gVIQE_RDMA_num_60Hz);
	if (!rdma_node) {
		pr_err("[ERR][VIQE] could not find viqe_video rdma_60hz node\n");
	} else {
		viqe_60hz_info->pRDMABase_60Hz =
	...
	of_property_read_u32_index(np, "telechips,sc", 1,
		&viqe_60hz_info->gSCALER_num_60Hz);
	if (!scaler_node) {
		pr_err("[ERR][VIQE] could not find viqe_video scaler node\n");
	} else {
		viqe_60hz_info->pSCALERBase_60Hz =
			VIOC_SC_GetAddress(viqe_60hz_info->gSCALER_num_60Hz);
	...
	iprintk("Reg_Base: RDMA/SC/WMIX/DISP = 0x%p / 0x%p / 0x%p / 0x%p\n",
		viqe_60hz_info->pRDMABase_60Hz,
		viqe_60hz_info->pSCALERBase_60Hz,
		viqe_60hz_info->pWMIXBase_60Hz,
		viqe_60hz_info->pDISPBase_60Hz);
...
// ok
int tcc_viqe_m2m_dt_parse(
	struct device_node *main_deIntl, struct device_node *sub_deIntl,
	struct device_node *sc_main, struct device_node *sc_sub,
	struct tcc_viqe_m2m_virt_addr_info_t *main_m2m_info,
	struct tcc_viqe_m2m_virt_addr_info_t *sub_m2m_info)
{
	struct device_node *main_deintl_rdma_node, *sub_deintl_rdma_node;
	struct device_node *sc_rdma_node, *sc_wmixer_node;
	struct device_node *sc_scaler_node, *sc_wdma_node;
	int ret = -ENOMEM;

	//Main :: m2m_deIntl_info
	main_deintl_rdma_node = of_parse_phandle(main_deIntl,
		"telechips,main_rdma,m2m", 0);
	of_property_read_u32_index(main_deIntl, "telechips,main_rdma,m2m", 1,
		&main_m2m_info->gVIQE_RDMA_num_m2m);
	if (!main_deintl_rdma_node) {
		pr_err("[ERR][VIQE] could not find viqe_video main_rdma_m2m node\n");
	} else {
		main_m2m_info->pRDMABase_m2m =
			VIOC_RDMA_GetAddress(main_m2m_info->gVIQE_RDMA_num_m2m);
...
#ifdef CONFIG_TCC_VIQE_FOR_SUB_M2M
	// VIQE1
	sub_deintl_rdma_node = of_parse_phandle(sub_deIntl,
		"telechips,sub_rdma,m2m", 0);
	of_property_read_u32_index(sub_deIntl, "telechips,sub_rdma,m2m", 1,
		&sub_m2m_info->gVIQE_RDMA_num_m2m);
	if (!sub_deintl_rdma_node) {
		pr_err("[ERR][VIQE] could not find viqe_video sub_rdma_m2m node\n");
	} else {
		sub_m2m_info->pRDMABase_m2m =
			VIOC_RDMA_GetAddress(sub_m2m_info->gVIQE_RDMA_num_m2m);
	...
	scaler = kzalloc(sizeof(struct tcc_viqe_m2m_scaler_type_t),
		GFP_KERNEL);
	if (scaler == NULL) {
	...
	scaler->info = kzalloc(sizeof(struct tcc_lcdc_image_update),
		GFP_KERNEL);
	if (scaler->info == NULL) {
	...
	scaler->data = kzalloc(sizeof(struct tcc_viqe_m2m_scaler_data),
		GFP_KERNEL);
	if (scaler->data == NULL) {
	...
	scaler->vioc_intr = kzalloc(sizeof(struct vioc_intr_type),
		GFP_KERNEL);
	...
	scaler->rdma = kzalloc(sizeof(struct tcc_viqe_m2m_scaler_vioc),
		GFP_KERNEL);
	if (scaler->rdma == NULL) {
	...
	scaler->wmix = kzalloc(sizeof(struct tcc_viqe_m2m_scaler_vioc),
		GFP_KERNEL);
	if (scaler->wmix == NULL) {
		//pr_err("[ERR][VIQE] fail scaler->wmix kzalloc\n");
		return ret;
	}
	scaler->sc = kzalloc(sizeof(struct tcc_viqe_m2m_scaler_vioc),
		GFP_KERNEL);
	if (scaler->sc == NULL) {
	...
	scaler->wdma = kzalloc(sizeof(struct tcc_viqe_m2m_scaler_vioc),
		GFP_KERNEL);
	if (scaler->wdma == NULL) {
	...
	of_property_read_u32_index(sc_main, "m2m_sc_rdma", 1,
		&scaler->rdma->id);
	if (!sc_rdma_node) {
	...
	of_property_read_u32_index(sc_main, "m2m_sc_scaler", 1,
		&scaler->sc->id);
	if (!sc_scaler_node) {
	...
	of_property_read_u32_index(sc_main, "m2m_sc_wdma", 1,
		&scaler->wdma->id);
	if (!sc_wdma_node) {
		...
		scaler->irq = irq_of_parse_and_map(sc_wdma_node,
			get_vioc_index(scaler->wdma->id));
		scaler->vioc_intr->id   = VIOC_INTR_WD0
			+ get_vioc_index(scaler->wdma->id);
	...
	of_property_read_u32(sc_main, "m2m_sc_settop_support",
		&scaler->settop_support);

	//Sub :: m2m_deIntl_scaler
	scaler_sub = kzalloc(sizeof(struct tcc_viqe_m2m_scaler_type_t),
		GFP_KERNEL);
	if (scaler_sub == NULL) {
	...
	scaler_sub->info = kzalloc(sizeof(struct tcc_lcdc_image_update),
		GFP_KERNEL);
	...
	scaler_sub->data = kzalloc(sizeof(struct tcc_viqe_m2m_scaler_data),
		GFP_KERNEL);
	...
	scaler_sub->vioc_intr = kzalloc(sizeof(struct vioc_intr_type),
		GFP_KERNEL);
	...
	scaler_sub->rdma = kzalloc(sizeof(struct tcc_viqe_m2m_scaler_vioc),
		GFP_KERNEL);
	...
	scaler_sub->wmix = kzalloc(sizeof(struct tcc_viqe_m2m_scaler_vioc),
		GFP_KERNEL);
	if (scaler_sub->wmix == NULL) {
		//pr_err("[ERR][VIQE] fail scaler_sub->wmix kzalloc\n");
		return ret;
	}
	scaler_sub->sc = kzalloc(sizeof(struct tcc_viqe_m2m_scaler_vioc),
		GFP_KERNEL);
	...
	scaler_sub->wdma = kzalloc(sizeof(struct tcc_viqe_m2m_scaler_vioc),
		GFP_KERNEL);
	...
	of_property_read_u32_index(sc_sub, "m2m_sc_rdma", 1,
		&scaler_sub->rdma->id);
	if (!sc_rdma_node) {
		pr_err("could not find viqe_video rdma node of scaler_sub\n");
	} else {
		scaler_sub->rdma->reg =
			VIOC_RDMA_GetAddress(scaler_sub->rdma->id);
	...
	of_property_read_u32_index(sc_sub, "m2m_sc_wmix", 1,
		&scaler_sub->wmix->id);
	if (!sc_wmixer_node) {
		pr_err("[ERR][VIQE] could not find viqe_video wmixer node of scaler_sub\n");
	} else {
		scaler_sub->wmix->reg =
			VIOC_WMIX_GetAddress(scaler_sub->wmix->id);
		//of_property_read_u32(sc_sub, "m2m_sc_wmix_path",
		//	&scaler_sub->wmix->path);
	...
	of_property_read_u32_index(sc_sub, "m2m_sc_scaler", 1,
		&scaler_sub->sc->id);
	if (!sc_scaler_node) {
	...
	of_property_read_u32_index(sc_sub, "m2m_sc_wdma", 1,
		&scaler_sub->wdma->id);
	if (!sc_wdma_node) {
		pr_err("[ERR][VIQE] could not find viqe_video wdma node of scaler_sub\n");
	} else {
		scaler_sub->wdma->reg =
			VIOC_WDMA_GetAddress(scaler_sub->wdma->id);
		scaler_sub->irq = irq_of_parse_and_map(sc_wdma_node,
			get_vioc_index(scaler_sub->wdma->id));
		scaler_sub->vioc_intr->id = VIOC_INTR_WD0
			+ get_vioc_index(scaler_sub->wdma->id);
	...
	of_property_read_u32(sc_sub, "m2m_sc_settop_support",
		&scaler_sub->settop_support);
	...
	iprintk("Intr  : (%d)0x%08x / (%d)0x%08x\n",
		scaler->vioc_intr->id, scaler->irq,
		scaler_sub->vioc_intr->id, scaler_sub->irq);
...
static int tcc_viqe_probe(struct platform_device *pdev)
{
	...
	struct device_node *viqe_common, *lcd_60hz_node;
	struct device_node *external_60Hz_node, *external_sub_60Hz_node;
	struct device_node *m2m_deintl_main_node, *m2m_deintl_sub_node;
	struct device_node *m2m_scaler_main_node, *m2m_scaler_sub_node;

#ifdef CONFIG_OF
	//viqe_common
	viqe_common = of_find_node_by_name(dev->of_node,
	...
	//viqe_60hz_info
	lcd_60hz_node = of_find_node_by_name(dev->of_node,
		"tcc_video_viqe_lcd");
	external_60Hz_node = of_find_node_by_name(dev->of_node,
		"tcc_video_viqe_external");
	external_sub_60Hz_node = of_find_node_by_name(dev->of_node,
	...
	tcc_viqe_60hz_dt_parse(external_sub_60Hz_node,
		&viqe_60hz_external_sub_info);

	//m2m_viqe
	m2m_deintl_main_node = of_find_node_by_name(dev->of_node,
	"tcc_video_main_m2m");
	m2m_deintl_sub_node = of_find_node_by_name(dev->of_node,
		"tcc_video_sub_m2m");

	m2m_scaler_main_node = of_find_node_by_name(dev->of_node,
		"tcc_video_scaler_main_m2m");
	m2m_scaler_sub_node = of_find_node_by_name(dev->of_node,
		"tcc_video_scaler_sub_m2m");

	if (!m2m_deintl_main_node
		|| !m2m_scaler_main_node
		|| !m2m_deintl_sub_node
		|| !m2m_scaler_sub_node) {
	...
	tcc_viqe_m2m_dt_parse(m2m_deintl_main_node, m2m_deintl_sub_node,
		m2m_scaler_main_node, m2m_scaler_sub_node,
		&main_m2m_info, &sub_m2m_info);
#endif
