	...
	unsigned int fill_mode)
{
	...
	value |=
		((fill_mode << WDMACTRL_FMT10FILL_SHIFT)
		 | (fmt_type << WDMACTRL_FMT10_SHIFT));

	__raw_writel(value, reg + WDMACTRL_OFFSET);
}
EXPORT_SYMBOL(VIOC_WDMA_SetDataFormat);
#endif //
