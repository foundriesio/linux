...
void sarin_sync_polarity_set(
	uint hs_active_low, uint vs_active_low, uint field_bfield_low,
	uint de_active_low, uint gen_field_en, uint pxclk_pol)
{
#if 0
...
void sarin_ctrl_set(
	uint conv_en, uint hsde_connect_en, uint vs_mask, uint fmt,
	uint data_order)
{
#if 0
...
void sarin_size_set(
	uint width, uint height, uint offs_width, uint offs_height,
	uint offs_height_intl)
{
	...
	offset_intl = (offs_height_intl << SAR_IN_OFFS_INTL_HEIGHT_SHIFT)
		& SAR_IN_OFFS_INTL_HEIGHT_MASK;
...
void VIOC_SAR_Set(unsigned int width, unsigned int height)
{
	...
	// vs_active_low, uint field_bfield_low,
	// uint de_active_low, uint
	// gen_field_en, uint
	// pxclk_pol);
	...
	sarin_size_set(
		width, height, offs_width, offs_height, offs_height_intl);
...
void VIOC_SARIF_SetTimingParam(stLTIMING *pTimeParam)
{
	...
	//	Horizon
	value =
		(((pTimeParam->lpc) << DHTIME1_LPC_SHIFT)
		 | (pTimeParam->lpw << DHTIME1_LPW_SHIFT));
	__raw_writel(value, reg + DHTIME1);

	value =
		(((pTimeParam->lswc) << DHTIME2_LSWC_SHIFT)
...
static int __init vioc_sar_init(void)
{
	...
	ViocSarIn_np =
		of_find_compatible_node(NULL, NULL, "telechips,vioc_sar");
