...
#if defined(CONFIG_VIOC_AFBCDEC)
int VIOC_CONFIG_AFBCDECPath(
	unsigned int AFBCDecPath, unsigned int rdmaPath, unsigned int on)
{
	unsigned long value = 0;
	...
	if (on)
		value =
			(__raw_readl(reg)
			 & ~((0x1 << select)
			     << CFG_AFBC_DEC_SEL_AXSM_SEL_SHIFT));
	else
		value =
			(__raw_readl(reg)
			 | ((0x1 << select)
	...
	case VIOC_AFBCDEC0:
		if (on) {
			...
			value |=
				(select
				 << CFG_AFBC_DEC_SEL_AD_PATH_SEL00_SHIFT);
		} else {
	...
	case VIOC_AFBCDEC1:
		if (on) {
			...
			value |=
				(select
				 << CFG_AFBC_DEC_SEL_AD_PATH_SEL01_SHIFT);
		} else {
	...
	// pr_debug("[DBG][VIOC_CONFIG] %s RDMA(%d) with AFBC(%d) :: On(%d)-
	// 0x%08lx \n", __func__, get_vioc_index(rdmaPath),
	// get_vioc_index(AFBCDecPath), on, value);
	__raw_writel(value, reg);
	return VIOC_PATH_CONNECTED;

error:
	pr_err("[ERR][VIOC_CONFIG] %s, in error, AFBCDecPath(%d) :0x%x rdmaPath:0x%x cfg_reg:0x%x\n",
	       __func__, on, get_vioc_index(AFBCDecPath), rdmaPath,
	       __raw_readl(reg));
	WARN_ON(1);
	return VIOC_DEVICE_INVALID;
}
