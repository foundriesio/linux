...
extern int vioc_mgr_queue_work(unsigned int command, unsigned int blk, unsigned int data0, unsigned int data1, unsigned int data2);
#endif
...
static volatile void __iomem *pIREQ_reg = NULL;
...
int CheckPixelMapPathSelection(unsigned int component)
{
	...
	switch (get_vioc_type(component)) {
	case get_vioc_type(VIOC_RDMA):
		ret = vioc_config_pixel_mapper_rdma_sel[get_vioc_index(component)];
		break;
	case get_vioc_type(VIOC_VIN):
...
static int Check2dFilterPathSelection(unsigned int component)
{
	...
	case get_vioc_type(VIOC_RDMA):
		...
		break;
	case get_vioc_type(VIOC_VIN):
		ret = vioc_config_2d_filter_vin_sel[get_vioc_index(component)/2];
		break;
	case get_vioc_type(VIOC_WDMA):
...
int VIOC_CONFIG_PlugIn(unsigned int component, unsigned int select)
{
	...
	while (1) {
		mdelay(1);
		loop--;
		value = ((__raw_readl(reg) & CFG_PATH_STS_MASK) >>
			 CFG_PATH_STS_SHIFT);
		if (value == VIOC_PATH_CONNECTED)
			break;
		if (loop < 1) {
...
#ifdef CONFIG_VIOC_2D_FILTER
int VIOC_CONFIG_FILTER2D_PlugIn(unsigned int component, unsigned int select, unsigned int front)
{
	unsigned long value = 0;
	unsigned int loop = 0;
	volatile void __iomem *reg;
...
const unsigned int bypassDMA[] = {
	...
	/* VIDEO-IN */
	VIOC_VIN00,
	#if defined(CONFIG_ARCH_TCC898X) || defined(CONFIG_ARCH_TCC803X) || defined(CONFIG_ARCH_TCC805X)
	VIOC_VIN10,
	#endif
	#if defined(CONFIG_ARCH_TCC803X) || defined(CONFIG_ARCH_TCC805X)
	VIOC_VIN20,
...
int VIOC_CONFIG_WMIXPath(unsigned int component_num, unsigned int mode)
{
	...
	if(support_bypass)
	{
		...
		switch(get_vioc_type(component_num))
		{
			...
			#endif
						break;
			#if defined(CONFIG_ARCH_TCC803X) || defined(CONFIG_ARCH_TCC802X) || defined(CONFIG_ARCH_TCC897X) || defined(CONFIG_ARCH_TCC805X)
		...
		{
			value = __raw_readl(config_reg + CFG_MISC0_OFFSET) & ~(1<<shift_vin_rdma_path);

			if(get_vioc_type(component_num) == get_vioc_type(VIOC_RDMA))
	...
	dprintk("%s-%d :: ERROR This component(0x%x) doesn't support mixer bypass(%d) mode, %d/%d!! \n",
			__func__, __LINE__, component_num, support_bypass, shift_mix_path, shift_vin_rdma_path);
	return -1;
}
...
void VIOC_CONFIG_WMIXPathReset(unsigned int component_num, unsigned int mode)
{
	...
	if(support_bypass)
	{
		...
		if(shift_mix_path != 0xFF)
		{
			value = __raw_readl(config_reg + CFG_WMIX_PATH_SWR_OFFSET) & ~(1<<shift_mix_path);
			if (mode)
				value |= 1<<shift_mix_path;
...
int VIOC_CONFIG_MCPath(unsigned int component, unsigned int mc)
{
	...
	case VIOC_WMIX2:
		if (get_vioc_type(mc) == get_vioc_type(VIOC_MC)) {
			...
			value |= ((0x1 << CFG_PATH_MC_RD11_SHIFT) |
	...
	case VIOC_WMIX3:
		if (get_vioc_type(mc) == get_vioc_type(VIOC_MC)) {
			...
			value |= ((0x1 << CFG_PATH_MC_RD13_SHIFT) |
	...
	case VIOC_WMIX4:
		if (get_vioc_type(mc) == get_vioc_type(VIOC_MC)) {
			...
			value |= ((0x1 << CFG_PATH_MC_RD15_SHIFT) |
	...
	case VIOC_WMIX5:
		if (get_vioc_type(mc) == get_vioc_type(VIOC_MC)) {
			...
			value |= ((0x1 << CFG_PATH_MC_RD16_SHIFT) |
	...
	case VIOC_WMIX6:
		if (get_vioc_type(mc) == get_vioc_type(VIOC_MC)) {
			...
			value |= ((0x1 << CFG_PATH_MC_RD17_SHIFT) |
...
void VIOC_CONFIG_SWReset_RAW(unsigned int component, unsigned int mode)
{
	...
	case get_vioc_type(VIOC_DISP):
		...
		value |= (mode << (PWR_BLK_SWR1_DEV_SHIFT +
				   get_vioc_index(component)));
	...
	case get_vioc_type(VIOC_FIFO):
		...
		value |= (mode << (PWR_BLK_SWR1_FIFO_SHIFT +
				   get_vioc_index(component)));
		__raw_writel(value, (reg + PWR_BLK_SWR1_OFFSET));
		break;
	...
	case get_vioc_type(VIOC_RDMA):
		...
		value |= (mode << (PWR_BLK_SWR0_RDMA_SHIFT +
				   get_vioc_index(component)));
	...
	case get_vioc_type(VIOC_VIN):
		...
		value |= (mode << (PWR_BLK_SWR0_VIN_SHIFT +
				   get_vioc_index(component)));
		__raw_writel(value, (reg + PWR_BLK_SWR0_OFFSET));
#else /* CONFIG_ARCH_TCC803X || CONFIG_ARCH_TCC805X*/
	...
	case get_vioc_type(VIOC_MC):
		...
		value |= (mode << (PWR_BLK_SWR1_MC_SHIFT +
				   get_vioc_index(component)));
		__raw_writel(value, (reg + PWR_BLK_SWR1_OFFSET));
		break;
#endif
	...
	case get_vioc_type(VIOC_DEINTLS):
		...
		value |= (mode << (PWR_BLK_SWR1_DINTS_SHIFT +
				   get_vioc_index(component)));
		__raw_writel(value, (reg + PWR_BLK_SWR1_OFFSET));
		break;
	...
	case get_vioc_type(VIOC_PIXELMAP):
		...
		value |= (mode << (PWR_BLK_SWR2_PM_SHIFT +
				   get_vioc_index(component)));
		__raw_writel(value, reg + PWR_BLK_SWR2_OFFSET);
		break;
#endif
	...
	case get_vioc_type(VIOC_SAR):
		...
		value |= (mode << (PWR_BLK_SWR2_SAR_SHIFT +
				   get_vioc_index(component)));
		__raw_writel(value, reg + PWR_BLK_SWR2_OFFSET);
		break;
#endif
	...
	case get_vioc_type(VIOC_DV_IN):
		...
		value |= (mode << (PWR_BLK_SWR2_DV_IN_SHIFT +
				   get_vioc_index(component)));
		__raw_writel(value, reg + PWR_BLK_SWR2_OFFSET);
		break;
#endif
	...
	case get_vioc_type(VIOC_CINTPL):
		...
		value |= (mode << (PWR_BLK_SWR3_CIN_SHIFT +
				   get_vioc_index(component)));
		__raw_writel(value, reg + PWR_BLK_SWR2_OFFSET);
		break;
#endif
	...
	case get_vioc_type(VIOC_F2D):
		...
		value |= (mode << (PWR_BLK_SWR3_F2D_SHIFT +
				   get_vioc_index(component)));
		__raw_writel(value, reg + PWR_BLK_SWR3_OFFSET);
		break;
#endif
	...
	case get_vioc_type(VIOC_V_DV):
		...
		value |= (mode << (PWR_BLK_SWR4_V_DV_SHIFT +
				   get_vioc_index(component)));
		__raw_writel(value, reg + PWR_BLK_SWR4_OFFSET);
		break;
	...
	case get_vioc_type(VIOC_DV_DISP):
		value = (__raw_readl(reg + PWR_BLK_SWR4_OFFSET) &
			 ~(PWR_BLK_SWR4_OSD_MASK|PWR_BLK_SWR4_EDR_EL_MASK|PWR_BLK_SWR4_EDR_BL_MASK));
		value |= (mode << (PWR_BLK_SWR4_EDR_BL_SHIFT +
				   get_vioc_index(component)));
		__raw_writel(value, reg + PWR_BLK_SWR4_OFFSET);
		break;
#endif
...
*/
int VIOC_CONFIG_Device_PlugState(unsigned int component,
...
int VIOC_CONFIG_GetScaler_PluginToRDMA(unsigned int RdmaNum)
{
	...
	for (i = get_vioc_index(VIOC_SCALER0); i <= VIOC_SCALER_MAX; i++) {
		if (VIOC_CONFIG_Device_PlugState((VIOC_SCALER0 + i),
						 &VIOC_PlugIn) ==
		    VIOC_DEVICE_INVALID)
			continue;
...
int VIOC_CONFIG_GetRdma_PluginToComponent(unsigned int ComponentNum /*Viqe, Mc, Dtrc*/)
{
	VIOC_PlugInOutCheck VIOC_PlugIn;

	if (VIOC_CONFIG_Device_PlugState(ComponentNum,
			 &VIOC_PlugIn) == VIOC_DEVICE_INVALID)
			 return -1;
...
int VIOC_CONFIG_DV_SET_EDR_PATH(unsigned int edr_on)
{
	...
	value = (__raw_readl(reg + CFG_PATH_EDR_OFFSET) &
...
#ifdef CONFIG_VIOC_DTRC_DECOMP
		case get_vioc_type(VIOC_DTRC):
			rdma_component = VIOC_CONFIG_GetRdma_PluginToComponent(component);
			break;
#endif
#ifdef CONFIG_VIOC_MAP_DECOMP
		case get_vioc_type(VIOC_MC):
...
int VIOC_CONFIG_DMAPath_Select(unsigned int path)
{
	...
	for(i = 0; i < VIOC_RDMA_MAX; i ++)
	{
		...
		if((value & CFG_PATH_EN_MASK) && ((value & CFG_PATH_SEL_MASK) == (get_vioc_index(path))))
...
int VIOC_CONFIG_DMAPath_Set(unsigned int path, unsigned int dma)
{
	...
	if (cfg_path_reg == NULL){
		dprintk("%s-%d :: ERROR :: cfg_path_reg for dma(0x%x) is NULL \n", __func__, __LINE__, dma);
		return -1;
	}
	...
	dprintk("R:: %s CFG_RDMA:0x%p = 0x%x \n", __func__, cfg_path_reg, __raw_readl(cfg_path_reg));
...
void VIOC_CONFIG_DMAPath_Iint(void)
{
	...
	if( VIOC_CONFIG_DMAPath_Support())
	{
		for(i = 0; i < VIOC_RDMA_MAX; i++)
		{
			cfg_path_reg = (void __iomem *)CalcAddressViocComponent(VIOC_RDMA00 + i);

			if(!cfg_path_reg)
				continue;
			VIOC_CONFIG_DMAPath_Set(VIOC_RDMA00 + i, VIOC_RDMA00 + i);
		}
	}
}
...
int VIOC_CONFIG_LCDPath_Select(unsigned int lcdx_sel, unsigned int lcdx_if)
{
	...
	sel_lcd[0] = (val & CFG_MISC1_LCD0_SEL_MASK) >> CFG_MISC1_LCD0_SEL_SHIFT;
	sel_lcd[1] = (val & CFG_MISC1_LCD1_SEL_MASK) >> CFG_MISC1_LCD1_SEL_SHIFT;
	sel_lcd[2] = (val & CFG_MISC1_LCD2_SEL_MASK) >> CFG_MISC1_LCD2_SEL_SHIFT;
#if defined(CONFIG_ARCH_TCC805X)
	sel_lcd[3] = (val & CFG_MISC1_LCD3_SEL_MASK) >> CFG_MISC1_LCD3_SEL_SHIFT;
#endif
...
#if defined(CONFIG_ARCH_TCC805X)
	val &= ~(CFG_MISC1_LCD3_SEL_MASK | CFG_MISC1_LCD2_SEL_MASK | CFG_MISC1_LCD1_SEL_MASK | CFG_MISC1_LCD0_SEL_MASK);
...
#else
	val &= ~(CFG_MISC1_LCD2_SEL_MASK | CFG_MISC1_LCD1_SEL_MASK | CFG_MISC1_LCD0_SEL_MASK);
	...
	dprintk("%s, CFG_MISC1: 0x%08x\n", __func__, __raw_readl(pIREQ_reg + CFG_MISC1_OFFSET));
