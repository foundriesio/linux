...
static int __init test_sar_wdma_api(unsigned int loop)
{
	...
	unsigned int mSarPath_TotalCnt = 0, mSarSize_TotalCnt = 0,
		     mSarSRCSize_TotalCnt = 0, i = 0;
	int err_plug;
	...
	err_plug = VIOC_CONFIG_PlugOut(
		VIOC_SCALER + scaler_n); // plugin position in scaler
	if (err_plug < 0)
	...
	VIOC_RDMA_SetImageOffset(
		UIrdma_reg, (dest_width * 4), (dest_width * 4));
...
static int __init test_disp_api(unsigned int Nrdma_n)
{
	...
	VIOC_RDMA_SetDataFormat(rdma_reg, 0, 0);
#if 0
	pr_info("scaler setting\n");
	VIOC_CONFIG_PlugIn(VIOC_SCALER + scaler_n, VIOC_RDMA + rdma_n);
	// plugin position in scaler
	...
	//height);	VIOC_WMIX_SetUpdate(wmix_reg);
