...
void VIOC_DV_IN_SetInterlaceMode(unsigned int intl_en, unsigned int intpl_en)
{
	unsigned long val;
...
void VIOC_DV_IN_SetCaptureModeEnable(unsigned int cap_en)
{
	unsigned long val;
...
/* Image size setting */
void VIOC_DV_IN_SetImageSize(unsigned int width, unsigned int height)
{
	unsigned long val;
...
/* VIN Enable/Disable */
void VIOC_DV_IN_SetTunneling(unsigned int tunneling_en)
{
	unsigned long val;
...
/* VIN Enable/Disable */
void VIOC_DV_IN_SetSwap(unsigned int mode)
{
	unsigned long val;
...
void VIOC_DV_IN_Configure(
	unsigned int in_width, unsigned int in_height, unsigned int in_fmt,
	unsigned int bTunneling)
{
...
void VIOC_DV_IN_DUMP(void)
{
	...
	while (cnt < 0x20) {
		pr_debug(
			"[DBG][DV_IN] %p: 0x%08x 0x%08x 0x%08x 0x%08x\n",
			pReg + cnt, __dvin_reg_r(pReg + cnt),
			__dvin_reg_r(pReg + cnt + 0x4),
