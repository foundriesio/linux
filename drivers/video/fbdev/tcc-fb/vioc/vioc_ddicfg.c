...
#define dprintk(msg...)							\
	do {								\
		if (debug) {						\
			pr_info("\e[33m[DBG][DDICFG]\e[0m " msg);	\
		}							\
	} while (0)
	...
	volatile void __iomem *reg, unsigned int reset_enable)
{
	...
	val |= ((reset_enable & 0x1) << HDMI_CTRL_RESET_PHY_SHIFT);
	__raw_writel(val, reg + HDMI_CTRL);
}
	...
	volatile void __iomem *reg, unsigned int reset_enable)
{
	...
	val |= ((reset_enable & 0x1) << HDMI_CTRL_RESET_LINK_SHIFT);
	__raw_writel(val, reg + HDMI_CTRL);
}
	...
	volatile void __iomem *reg, unsigned int reset)
{
	...
	if (reset)
		val |= ((0x1 << MIPI_CFG_GEN_PX_RST_SHIFT)
			| (0x1 << MIPI_CFG_GEN_APB_RST_SHIFT));
	__raw_writel(val, reg + MIPI_CFG);
}
...
void VIOC_DDICONFIG_DUMP(void)
{
	...
	while (cnt < 0x50) {
		pr_debug(
			"[DBG][DDICFG] 0x%p: 0x%08x 0x%08x 0x%08x 0x%08x\n",
