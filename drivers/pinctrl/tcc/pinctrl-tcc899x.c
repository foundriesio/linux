/*
 * Telechips TCC899x pinctrl driver
 *
...
inline static int tcc899x_set_eint(void __iomem *base, unsigned bit, int extint, struct tcc_pinctrl *pctl)
{
	...
	int irq_size = tcc899x_pinctrl_soc_data.irq->size;
#ifdef CONFIG_ARM64
...
static int tcc899x_gpio_get(void __iomem *base, unsigned offset)
{
	unsigned int data;
...
static void tcc899x_gpio_set(void __iomem *base, unsigned offset, int value)
{
	unsigned int data;
...
static void tcc899x_gpio_pinconf_extra(void __iomem *base, unsigned offset, int value, unsigned base_offset)
{
...
static void tcc899x_gpio_input_buffer_set(void __iomem *base, unsigned offset, int value)
{
	tcc899x_gpio_pinconf_extra(base, offset, value, (unsigned)GPIO_INPUT_BUFFER_ENABLE);
}

static int tcc899x_gpio_set_direction(void __iomem *base, unsigned offset,
				      int input)
{
...
static int tcc899x_gpio_get_direction(void __iomem *base, u32 offset)
{
    void __iomem *reg = base + GPIO_OUTPUT_ENABLE;
    u32 data;
...
static int tcc899x_gpio_set_function(void __iomem *base, unsigned offset,
				      int func)
{
...
static int tcc899x_gpio_get_drive_strength(void __iomem *base, unsigned offset)
{
...
static int tcc899x_gpio_set_drive_strength(void __iomem *base, unsigned offset,
					   int value)
{
	void __iomem *reg = base + GPIO_DRIVE_STRENGTH + (offset/16)*4;
	int data;
...
static void tcc899x_gpio_pull_enable(void __iomem *base, unsigned offset,
				     int enable)
{
	tcc899x_gpio_pinconf_extra(base, offset, enable, (unsigned)GPIO_PULL_ENABLE);
}

static void tcc899x_gpio_pull_select(void __iomem *base, unsigned offset,
				     int up)
{
	tcc899x_gpio_pinconf_extra(base, offset, up, GPIO_PULL_SELECT);
}

static void tcc899x_gpio_input_type(void __iomem *base, unsigned offset, int value)
{
	tcc899x_gpio_pinconf_extra(base, offset, value, (unsigned)GPIO_INPUT_TYPE);
}

static void tcc899x_gpio_slew_rate(void __iomem *base, unsigned offset, int value)
{
...
set_gpio_to_irq_finish:
	tcc899x_gpio_set_function(base, offset, 0);
	tcc899x_gpio_set_direction(base, offset, 1);
	return match[i].irq;
}
...
static int tcc899x_pinconf_get(void __iomem *base, unsigned offset, int param)
{
	int ret;

	switch (param) {
	case TCC_PINCONF_PARAM_DRIVE_STRENGTH:
		ret = tcc899x_gpio_get_drive_strength(base, offset);
		break;
			...
			int config, struct tcc_pinctrl *pctl)
{
	switch (param) {
	case TCC_PINCONF_PARAM_DRIVE_STRENGTH:
		if (tcc899x_gpio_set_drive_strength(base, offset, config) < 0)
			return -EINVAL;
		break;

	case TCC_PINCONF_PARAM_NO_PULL:
		tcc899x_gpio_pull_enable(base, offset, 0);
		break;

	case TCC_PINCONF_PARAM_PULL_UP:
		tcc899x_gpio_pull_select(base, offset, 1);
		tcc899x_gpio_pull_enable(base, offset, 1);
		break;

	case TCC_PINCONF_PARAM_PULL_DOWN:
		tcc899x_gpio_pull_select(base, offset, 0);
		tcc899x_gpio_pull_enable(base, offset, 1);
		break;
	...
	case TCC_PINCONF_PARAM_OUTPUT_LOW:
		tcc899x_gpio_set(base, offset, 0);
		tcc899x_gpio_set_direction(base, offset, 0);
		break;

	case TCC_PINCONF_PARAM_OUTPUT_HIGH:
		tcc899x_gpio_set(base, offset, 1);
		tcc899x_gpio_set_direction(base, offset, 0);
		break;

	case TCC_PINCONF_PARAM_INPUT_BUFFER_ENABLE:
	case TCC_PINCONF_PARAM_INPUT_BUFFER_DISABLE:
		tcc899x_gpio_input_buffer_set(base, offset, param % 2);
		break;

	case TCC_PINCONF_PARAM_SCHMITT_INPUT:
	case TCC_PINCONF_PARAM_CMOS_INPUT:
		tcc899x_gpio_input_type(base, offset, param % 2);
		break;

	case TCC_PINCONF_PARAM_SLOW_SLEW:
	case TCC_PINCONF_PARAM_FAST_SLEW:
		tcc899x_gpio_slew_rate(base, offset, param % 2);
		break;
	}
	return 0;
}

static struct tcc_pinconf tcc899x_pin_configs[] = {
...
static struct tcc_pinctrl_ops tcc899x_ops = {
	.gpio_get = tcc899x_gpio_get,
	.gpio_set = tcc899x_gpio_set,
	.gpio_get_direction = tcc899x_gpio_get_direction,
	.gpio_set_function = tcc899x_gpio_set_function,
	.gpio_set_direction = tcc899x_gpio_set_direction,
	.pinconf_get = tcc899x_pinconf_get,
	.pinconf_set = tcc899x_pinconf_set,
	.to_irq = tcc899x_gpio_to_irq,
};

static int tcc899x_pinctrl_probe(struct platform_device *pdev)
{
	tcc899x_pinctrl_soc_data.pin_configs = tcc899x_pin_configs;
	tcc899x_pinctrl_soc_data.nconfigs = ARRAY_SIZE(tcc899x_pin_configs);
	tcc899x_pinctrl_soc_data.ops = &tcc899x_ops;
...
static const struct of_device_id tcc899x_pinctrl_of_match[] = {
	{
		.compatible = "telechips,tcc899x-pinctrl",
		.data = &tcc899x_pinctrl_soc_data },
	{ },
};

static struct platform_driver tcc899x_pinctrl_driver = {
	.probe		= tcc899x_pinctrl_probe,
	.driver		= {
		.name	= "tcc899x-pinctrl",
		.owner	= THIS_MODULE,
		.of_match_table = of_match_ptr(tcc899x_pinctrl_of_match),
	},
};

static int __init tcc899x_pinctrl_drv_register(void)
{
	return platform_driver_register(&tcc899x_pinctrl_driver);
}
postcore_initcall(tcc899x_pinctrl_drv_register);

static void __exit tcc899x_pinctrl_drv_unregister(void)
{
	platform_driver_unregister(&tcc899x_pinctrl_driver);
}
module_exit(tcc899x_pinctrl_drv_unregister);

MODULE_DESCRIPTION("Telechips TCC899X pinctrl driver");
MODULE_LICENSE("GPL");
