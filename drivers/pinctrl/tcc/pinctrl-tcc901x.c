...
inline static int tcc901x_set_eint(void __iomem *base, unsigned bit, int extint)
{
...
static int tcc901x_gpio_to_irq(void __iomem *base, unsigned offset)
{
	int i;
	struct extintr_match_ *match = (struct extintr_match_ *)tcc901x_pinctrl_soc_data.irq->data;
	int irq_size = tcc901x_pinctrl_soc_data.irq->size;
	...
	for (i=0; i < irq_size ; i++) {
		if (!match[i].used) {
			if (tcc901x_set_eint(base, offset, i) == 0)
				goto set_gpio_to_irq_finish;
			else
				break;
		}
