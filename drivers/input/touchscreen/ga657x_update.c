...
static u8 gup_get_ic_fw_msg(struct i2c_client *client)
{
	...
	update_msg.ic_fw_msg.vid = (buf[GTP_ADDR_LENGTH + 4] << 16) |
				   (buf[GTP_ADDR_LENGTH + 5] << 8) |
				   buf[GTP_ADDR_LENGTH + 6];
	dev_dbg(&client->dev, "IC version id:%06x", update_msg.ic_fw_msg.vid);
...
s32 gup_enter_update_mode(struct i2c_client *client)
{
	...
	/* step2:RST output high reset guitar */
...
void gup_leave_update_mode(struct i2c_client *client)
{
	...
	client->addr = 0x5d; /* i2c addr maybe changed, restore to 0x5d. */
				...
				struct st_fw_head *fw_head, u8 *path)
{
	...
	memset(fw_head, 0, sizeof(*fw_head));
	memcpy(fw_head->hw_info, update_msg.fw_data, 4);
	memcpy(fw_head->pid, update_msg.fw_data + 4, 6);
	fw_head->vid = (update_msg.fw_data[10 + 0] << 16) |
			(update_msg.fw_data[10 + 1] << 8) |
			update_msg.fw_data[10 + 2];
...
s32 gup_update_proc(void *dir)
{
	...
	if (gup_get_ic_msg(ts->client, GUP_REG_HW_INFO, buf, 1) == FAIL) {
		i2c_connect_client->addr = 0x14;
		if (gup_get_ic_msg(ts->client, GUP_REG_HW_INFO, buf, 1) == FAIL) {
			dev_err(&ts->client->dev, "[update_proc]i2c test failed.");
			goto file_fail;
		}
		/* firmware is broken. */
...
file_fail:
	...
	ts->client->addr = 0x5d; /* i2c addr maybe changed, restore to 0x5d. */
	update_msg.fw_data = NULL;
	update_msg.fw_total_len = 0;
	release_firmware(update_msg.fw);
