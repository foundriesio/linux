			 ...
			 int *key_value)
{
	...
	u8 point_data[2 + 1 + 8 * GTP_MAX_TOUCH_ID + 2] = {
			GTP_READ_COOR_ADDR >> 8, GTP_READ_COOR_ADDR & 0xFF };

	ret = gtp_i2c_read(ts->client, point_data, 13);
	if (ret < 0) {
	...
	if (touch_num > 1) {
		u8 buf[8 * GTP_MAX_TOUCH_ID] = {
					  (GTP_READ_COOR_ADDR + 11) >> 8,
					  (GTP_READ_COOR_ADDR + 11) & 0xff };
		...
		memcpy(&point_data[13], &buf[2], 8 * (touch_num - 1));
	}

	/* panel have touch key */
	/* 0x20_UPKEY 0X10_DOWNKEY 0X40_ALLKEYDOWN */
	*key_value = point_data[3 + 8 * touch_num] +
			(point_data[3 + 8 * touch_num + 1] << 8);
...
static void gtp_powerkey_handler(struct goodix_ts_data *ts)
{
	unsigned char buf[4] = {0x81, 0x4E};
	if (gtp_i2c_read(ts->client, buf, sizeof(buf)) > 0) {
		if (buf[2] == 0x55 && buf[3] == 0xAA) {
			input_report_key(ts->input_dev, KEY_POWER, 1);
			input_sync(ts->input_dev);
			input_report_key(ts->input_dev, KEY_POWER, 0);
			input_sync(ts->input_dev);
			gtp_work_control_enable(ts, false);
			dev_dbg(&ts->client->dev, "powerkey wakeup.");
		} else {
			dev_err(&ts->client->dev,
				"The chip doesn't work in sleep mode, enter sleep mode again.");
			gtp_enter_sleep(ts);
		}
	}
}
...
static void gtp_work_func(struct goodix_ts_data *ts)
{
	...
	/* powerkey */
	if (ts->pdata->has_powerkey && test_bit(SLEEP_MODE, &ts->flags)) {
		gtp_powerkey_handler(ts);
		return;
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE, "GT%s_%06x_%d\n",
...
static int gtp_create_file(struct goodix_ts_data *ts)
{
	...
	gtp_config_proc = proc_create(GTP_CONFIG_PROC_FILE, 0664,
				      NULL, &config_proc_ops);
	if (!gtp_config_proc)
		dev_err(&client->dev, "create_proc_entry %s failed\n",
			GTP_CONFIG_PROC_FILE);
	else
...
exit_free_config_proc:
	remove_proc_entry(GTP_CONFIG_PROC_FILE, gtp_config_proc);
	return -ENODEV;
}
...
s32 gtp_get_fw_info(struct i2c_client *client, struct goodix_fw_info *fw_info)
{
	...
	fw_info->version = (buf[GTP_ADDR_LENGTH + 4] << 16) |
			   (buf[GTP_ADDR_LENGTH + 5] << 8) |
			   (buf[GTP_ADDR_LENGTH + 6]);
	dev_info(&client->dev, "IC Version: %s_%06x\n",
...
static int gtp_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	for (index = 0; index < ts->pdata->key_nums; index++) {
		if (ts->pdata->key_map[index] == KEY_POWER) {
			ts->pdata->has_powerkey = 1;
			break;
		}
	}
	if (ts->pdata->slide_wakeup || ts->pdata->has_powerkey) {
		dev_info(&client->dev, "irq wake enabled\n");
...
static const struct of_device_id gtp_match_table[] = {
	...
	{.compatible = "goodix,ga6571",},
	{.compatible = "goodix,ga6572",},
	{ },
};
#endif
...
static int __init gtp_init(void)
{
	...
	pr_info("ga657x driver installing..\n");
...
MODULE_DESCRIPTION("GA657X serials touch controller Driver");
