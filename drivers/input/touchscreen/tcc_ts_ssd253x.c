 ...
 *   Description :
     modify to control gpio and update register table for Telechips EVB.
...
void ssd253x_reset(struct ssl_ts_priv *ssl_priv, s32 ms)
{
	gpio_direction_output(ssl_priv->reset_gpio, 0);
	msleep(ms);
	gpio_direction_input(ssl_priv->reset_gpio);
	usleep_range(25, 30);
}
...
static void ssd253x_i2c_reset(struct ssl_ts_priv *ssl_priv)
{
	gpio_direction_output(ssl_priv->reset_gpio, 0);
	usleep_range(25, 30);
	gpio_set_value_cansleep(ssl_priv->reset_gpio, GPIO_LOW);
	usleep_range(25, 30);
	gpio_set_value_cansleep(ssl_priv->reset_gpio, GPIO_HIGH);
	udelay(30);
}
#endif
...
void deviceWakeUp(struct ssl_ts_priv *ssl_priv)
{
	...
	StartWork(ssl_priv);
#ifdef CONFIG_ARCH_TCC
	gpio_direction_output(ssl_priv->int_gpio_num, 0);
	usleep_range(25, 30);
...
err0:
	kfree(ssl_priv);
	dev_err(dev, "%s : failed to run driver(%d).\n", __func__, error);
	return error;
}
...
.driver = {
		.name = "ssd253x",
		.of_match_table = of_match_ptr(ssd253x_ts_of_match),
