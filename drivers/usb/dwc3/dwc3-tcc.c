...
#endif
		read_data =
		    tcc->dwc3_phy->read_u30phy_reg(tcc->dwc3_phy, 0x1024);
#if defined(CONFIG_ARCH_TCC803X) || defined(CONFIG_ARCH_TCC805X)
	else
		read_data =
		    tcc->dwc3_phy->read_u30phy_reg(tcc->dwc3_phy, 0x1024);
...
#endif
		read_data =
		    tcc->dwc3_phy->read_u30phy_reg(tcc->dwc3_phy, 0x1006);
#if defined(CONFIG_ARCH_TCC803X) || defined(CONFIG_ARCH_TCC805X)
	else
		read_data =
		    tcc->dwc3_phy->read_ss_u30phy_reg(tcc->dwc3_phy, 0x1006);
#endif
			      ...
			      struct device_attribute *attr, char *buf)
{
...
#endif
	{
		struct USBPHYCFG *pUSBPHYCFG =
		    (struct USBPHYCFG *)(tcc->dwc3_phy
			->get_base(tcc->dwc3_phy));
		...
		struct USBSSPHYCFG *pUSBPHYCFG =
		    (struct USBSSPHYCFG *)(tcc->dwc3_phy
			->get_base(tcc->dwc3_phy));
			       ...
			       const char *buf, size_t count)
{
...
#endif
	{
		struct USBPHYCFG *pUSBPHYCFG =
		    (struct USBPHYCFG *)(tcc->dwc3_phy
			->get_base(tcc->dwc3_phy));
		/* val range is 0x0 ~ 0xF */
		BITCSET(reg, TXVRT_MASK, val << TXVRT_SHIFT);
