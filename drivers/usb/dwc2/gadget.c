...
static void dwc2_hsotg_init_fifo(struct dwc2_hsotg *hsotg)
{
	...
	dev_dbg(hsotg->dev,
		"[DEBUG][USB] FIFOs reset, timeout at %d\n",
		timeout);
}
...
static void dwc2_gadget_start_isoc_ddma(struct dwc2_hsotg_ep *hs_ep)
{
	...
	if (list_empty(&hs_ep->queue)) {
		...
		dev_dbg(hsotg->dev,
			"[DEBUG][USB] %s: No requests in queue\n",
			__func__);
		return;
	}
					 ...
					 struct dwc2_hsotg_req *hs_req)
{
	...
	dev_dbg(hsotg->dev,
		"[DEBUG][USB] %s: %s: status=%d actual-length=%d\n",
		__func__,
...
int dwc2_hsotg_set_test_mode(struct dwc2_hsotg *hsotg, int testmode)
{
	...
	dev_info(hsotg->dev,
		"[INFO][USB] @0x%lX: 0x%08X\n",
		(unsigned long)(hsotg->regs + DCTL), dctl);
					 ...
					 struct usb_ctrlrequest *ctrl)
{
	...
	dev_dbg(hsotg->dev,
		"[DEBUG][USB] %s: USB_REQ_GET_STATUS\n",
		__func__);

	if (!ep0->dir_in) {
		dev_warn(hsotg->dev,
			"[WARN][USB] %s: direction out?\n",
			__func__);
		return -EINVAL;
	}
...
static void dwc2_hsotg_enqueue_setup(struct dwc2_hsotg *hsotg)
{
	...
	dev_dbg(hsotg->dev,
		"[DEBUG][USB] %s: queueing setup request\n",
		__func__);
				       ...
				       int result)
{
	if (!hs_req) {
		dev_dbg(hsotg->dev,
			"[DEBUG][USB] %s: nothing to complete?\n",
			__func__);
		return;
	}
...
static void dwc2_gadget_complete_isoc_request_ddma(struct dwc2_hsotg_ep *hs_ep)
{
	...
	while ((desc_sts & DEV_DMA_BUFF_STS_MASK) >>
		...
		if (!hs_req) {
			dev_warn(hsotg->dev,
				"[WARN][USB] %s: ISOC EP queue empty\n",
				__func__);
			return;
		}
		ureq = &hs_req->req;
				   ...
				   struct dwc2_hsotg_ep *hs_ep)
{
	...
	if (!size_left && hs_req->req.actual < hs_req->req.length) {
		dev_dbg(hsotg->dev,
			"[DEBUG][USB] %s trying more for req...\n",
			__func__);
		dwc2_hsotg_start_req(hsotg, hs_ep, hs_req, true);
		return;
	}
			     ...
			     int dir_in)
{
	...
	if (ints & DXEPINT_BACK2BACKSETUP)
		dev_dbg(hsotg->dev,
			"[DEBUG][USB] %s: B2BSetup/INEPNakEff\n",
			__func__);

	if (ints & DXEPINT_BNAINTR) {
		dev_dbg(hsotg->dev,
			"[DEBUG][USB] %s: BNA interrupt\n",
			__func__);
		if (hs_ep->isochronous)
...
static void dwc2_gadget_handle_incomplete_isoc_out(struct dwc2_hsotg *hsotg)
{
	...
	dev_dbg(hsotg->dev,
		"[DEBUG][USB] %s: GINTSTS_INCOMPL_SOOUT\n",
		__func__);
#if 1
	daintmsk = dwc2_readl(hsotg->regs + DAINTMSK);
	...
	for (idx = 1; idx < hsotg->num_of_eps; idx++) {
		...
		if ((epctrl & DXEPCTL_EPENA) &&
			...
			dev_dbg(hsotg->dev,
				"[DEBUG][USB] %s: GINTSTS_INCOMPL_SOUT +\n",
				__func__);
				   ...
				   struct dwc2_hsotg_ep *hs_ep)
{
	...
	dev_dbg(hsotg->dev,
		"[DEBUG][USB] %s: stopping transfer on %s\n", __func__,
		hs_ep->name);
				...
				const struct usb_endpoint_descriptor *desc)
{
	...
	if (dir_in != hs_ep->dir_in) {
		dev_err(hsotg->dev,
			"[ERROR][USB] %s: direction mismatch!\n",
			__func__);
		return -EINVAL;
	}
...
static int dwc2_hsotg_ep_disable(struct usb_ep *ep)
{
	...
	if (hsotg->op_state != OTG_STATE_B_PERIPHERAL) {
		dev_err(hsotg->dev,
			"[ERROR][USB] %s: called in host mode?\n",
			__func__);
		return -EINVAL;
	}
...
static int dwc2_hsotg_ep_sethalt(struct usb_ep *ep, int value, bool now)
{
	...
	if (hs_ep->isochronous) {
		dev_err(hs->dev,
			"[ERROR][USB] %s is Isochronous Endpoint\n",
			ep->name);
		return -EINVAL;
	}
				...
				struct usb_gadget_driver *driver)
{
	...
	if (!driver->setup) {
		dev_err(hsotg->dev,
			"[ERROR][USB] %s: missing entry points\n",
			__func__);
		return -EINVAL;
	}
...
static int dwc2_hsotg_pullup(struct usb_gadget *gadget, int is_on)
{
	...
	dev_dbg(hsotg->dev,
		"[DEBUG][USB] %s: is_on: %d op_state: %d\n",
		__func__, is_on,
		hsotg->op_state);
...
static void dwc2_hsotg_dump(struct dwc2_hsotg *hsotg)
{
	...
	for (idx = 1; idx < hsotg->num_of_eps; idx++) {
		...
		dev_info(dev,
			"[INFO][USB] DPTx[%d] FSize=%d, StAddr=0x%08x\n",
			idx,
...
int dwc2_gadget_init(struct dwc2_hsotg *hsotg)
{
	...
	if (ret) {
		dev_err(hsotg->dev,
			"[ERROR][USB] Hardware configuration failed: %d\n",
			ret);
		return ret;
	}
