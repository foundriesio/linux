...
static struct usb_request *usbf_ep_alloc_request(struct usb_ep *_ep, gfp_t gfp_flags)
{
	struct f_req *req;
...
static int usbf_ep_queue(struct usb_ep *_ep, struct usb_request *_req, gfp_t gfp_flags)
{
	struct f_endpoint *ep = container_of(_ep, struct f_endpoint, ep);
	struct f_drv *chip = ep->chip;
...
static void usbf_ep_reset(struct f_endpoint *ep)
{
...
static uint32_t usbf_tasklet_flush_fifo(struct f_drv *chip)
{
	struct f_status s;
	int i;
	u32 int_status = 0;
...
static irqreturn_t usbf_irq(int irq, void *_chip)
{
	...
	struct f_regs *regs = chip->regs;
	u32 sysbint = readl(&regs->sysbint);
...
static int usbf_get_frame(struct usb_gadget *_gadget)
{
	return -EOPNOTSUPP;
}
...
static int usbf_debug_init(struct f_drv *chip)
{
	...
	chip->debugfs_regs = debugfs_create_file("regs", 0444,
						 chip->debugfs_root, NULL, &_fops);

	chip->debugfs_eps = debugfs_create_file("ep_trace", 0644,
