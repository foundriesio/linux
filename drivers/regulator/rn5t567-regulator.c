/*
 * Regulator driver for Ricoh RN5T567 PMIC
...
static int rn5t618_regulator_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < RN5T618_REG_NUM; i++) {
		...
		rdev = devm_regulator_register(&pdev->dev,
					       ...
					       &config);
		if (IS_ERR(rdev)) {
			dev_err(&pdev->dev, "failed to register %s regulator, err %ld\n",
				rn5t618_regulators[i].name, PTR_ERR(rdev));
			return PTR_ERR(rdev);
		}
	}
