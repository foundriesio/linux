...
static void stpmic1_reset_boost(struct boost_data *usb_data)
{
	...
	mutex_lock(&usb_data->boost_rdev->mutex);
	...
	if (otg_on) {
		mutex_lock(&usb_data->vbus_otg_rdev->mutex);
	...
	if (sw_out_on) {
		mutex_lock(&usb_data->sw_out_rdev->mutex);
	...
	if (otg_on) {
		...
		mutex_unlock(&usb_data->vbus_otg_rdev->mutex);
	}

	if (sw_out_on) {
		regulator_enable_regmap(usb_data->sw_out_rdev);
		mutex_unlock(&usb_data->sw_out_rdev->mutex);
	}

	mutex_unlock(&usb_data->boost_rdev->mutex);
}
