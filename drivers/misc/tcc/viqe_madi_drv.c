...
#ifdef EN_MADI_VERIFICATION
#define dprintk(fmt, args...) pr_info("\e[38m[DBG][MADI] "fmt"\e[0m", ## args)
#else
...
static irqreturn_t viqe_madi_handler(int irq, void *client_data)
{
	...
	if (enable & raw & MADI_INT_ACTIVATED) {
		unsigned int sts_active = __madi_dreg_r(reg +
			MADITIMMING_GEN_CFG_STS_ACTIVE_OFFSET);
		sts_active += 1;
		__madi_dreg_w(sts_active, reg +
			MADITIMMING_GEN_CFG_STS_ACTIVE_OFFSET);
	...
	if (enable & raw & MADI_INT_DEACTIVATED) {
		...
		dprintk("%s:  block_operating(%d), block_waiting(%d), cmd_count(%d), poll_count(%d)\n",
			...
			viqe_madi->data->cmd_count,
			viqe_madi->data->poll_count);
...
static long viqe_madi_ioctl(struct file *filp, unsigned int cmd,
	unsigned long arg)
{
	struct miscdevice *misc = (struct miscdevice *)filp->private_data;
	struct viqe_madi_type *viqe_madi = dev_get_drvdata(misc->parent);
	...
	{
		if (copy_from_user(&viqe_madi->info->init, (void *)arg,
			sizeof(struct stVIQE_MADI_INIT_TYPE))) {
			...
			if (viqe_madi->info->init.src_fmt
				== VIQE_MADI_YUV420_inter
				|| viqe_madi->info->init.src_fmt
				== VIQE_MADI_YUV422_inter) {
				viqe_madi->info->Coffset_bottom =
					viqe_madi->info->init.src_ImgWidth;
			} else {
				viqe_madi->info->Coffset_bottom =
					viqe_madi->info->init.src_ImgWidth / 2;
			}

			VIQE_MADI_Ctrl_Enable(MADI_READY);

			VIQE_MADI_Gen_Timming(
				viqe_madi->info->init.src_winRight
				- viqe_madi->info->init.src_winLeft,
				viqe_madi->info->init.src_winBottom
				- viqe_madi->info->init.src_winTop);

			//alank
			g_width = viqe_madi->info->init.src_winRight
				- viqe_madi->info->init.src_winLeft;
			g_height = viqe_madi->info->init.src_winBottom
				- viqe_madi->info->init.src_winTop;

			VIQE_MADI_SetBasicConfiguration(
			...
			VIQE_MADI_Set_SrcImgBase(MADI_ADDR_0,
				0x00100000 << COMMON_ADDR_SHIFT,
				0x00300000 << COMMON_ADDR_SHIFT);
			VIQE_MADI_Set_SrcImgBase(MADI_ADDR_1,
				(0x00100000 << COMMON_ADDR_SHIFT)
				+ viqe_madi->info->Yoffset_bottom,
				(0x00300000 << COMMON_ADDR_SHIFT)
				+ viqe_madi->info->Coffset_bottom);
			VIQE_MADI_Set_SrcImgBase(MADI_ADDR_2,
				0x00140000 << COMMON_ADDR_SHIFT,
				0x00340000 << COMMON_ADDR_SHIFT);
			VIQE_MADI_Set_SrcImgBase(MADI_ADDR_3,
				(0x00140000 << COMMON_ADDR_SHIFT)
				+ viqe_madi->info->Yoffset_bottom,
				(0x00340000 << COMMON_ADDR_SHIFT)
				+ viqe_madi->info->Coffset_bottom);
	...
	case TCC_VIQE_MADI_DEINIT:
	{
		...
		viqe_madi->info->curr_src_index =
			viqe_madi->info->curr_out_index = 0;
	#endif
	}
		break;
	...
	case TCC_VIQE_MADI_PROC:
	{
		...
		if (copy_from_user(&proc_info, (void *)arg,
			sizeof(struct stVIQE_MADI_PROC_TYPE))) {
			pr_err("[ERR][MADI] %s: Error copy_from_user(%d)\n",
				__func__, cmd);
			ret = -EFAULT;
		} else {
			unsigned int i = 0;
			unsigned int cur_src_index =
				viqe_madi->info->curr_src_index;
			...
			for (i = MADI_ADDR_0; i < MADI_ADDR_MAX; i++) {
				VIQE_MADI_Set_TargetImgBase(i,
					proc_info.dest_Aaddr,
					proc_info.dest_Yaddr,
					proc_info.dest_Caddr);
			}
	...
	case TCC_VIQE_MADI_GET_RESULT:
	{
		...
		if (viqe_madi->info->first_frame
			...
			result.dest_Yaddr = result.dest_Uaddr =
				result.dest_Vaddr = 0x00;
		} else {
			reg = VIQE_MADI_GetAddress(VMADI_TIMMING);
			cfg_code = __madi_dreg_r(reg +
				MADITIMMING_GEN_CFG_CODE_OFFSET);
			cur_alpha_index = (cfg_code+3)&0x3;
			cur_yc_index = (cur_alpha_index+3)&0x3;

			VIQE_MADI_Get_TargetImgBase(
				(enum MadiADDR_Type)cur_yc_index,
				&result.dest_Yaddr, &result.dest_Uaddr);
			//result.dest_Yaddr =
			//	viqe_madi->info->out_Yaddr[cur_yc_index];
			//result.dest_Uaddr =
			//	viqe_madi->info->out_Caddr[cur_yc_index];
			result.dest_Vaddr = 0x00;
		}
		...
		dprintk("TCC_VIQE_MADI_GET_RESULT(0x%x) :: [%d] = 0x%x/0x%x\n",
			viqe_madi->irq_status, viqe_madi->info->first_frame,
			result.dest_Yaddr, result.dest_Uaddr);

		if (copy_to_user((unsigned int *)arg, &result,
			sizeof(struct stVIQE_MADI_RESULT_TYPE))) {
			ret = -EFAULT;
		}
	#endif
	}
	...
	case 0x10001:
	{
		...
		if (copy_from_user(&viqe_madi->info->init, (void *)arg,
			...
			VIQE_MADI_Set_SrcImgBase(MADI_ADDR_1,
				...
				0x00340000 << COMMON_ADDR_SHIFT);
			VIQE_MADI_Set_SrcImgBase(MADI_ADDR_2,
				0x00180000 << COMMON_ADDR_SHIFT,
			0x00380000 << COMMON_ADDR_SHIFT);
			VIQE_MADI_Set_SrcImgBase(MADI_ADDR_3,
				0x001C0000 << COMMON_ADDR_SHIFT,
				0x003C0000 << COMMON_ADDR_SHIFT);
			VIQE_MADI_Set_TargetImgBase(MADI_ADDR_0,
				0x00500000 << COMMON_ADDR_SHIFT,
				0x00800000 << COMMON_ADDR_SHIFT,
				0x00c00000 << COMMON_ADDR_SHIFT);
			VIQE_MADI_Set_TargetImgBase(MADI_ADDR_1,
				0x00500800 << COMMON_ADDR_SHIFT,
				0x00840000 << COMMON_ADDR_SHIFT,
				0x00c40000 << COMMON_ADDR_SHIFT);
			VIQE_MADI_Set_TargetImgBase(MADI_ADDR_2,
				0x00501000 << COMMON_ADDR_SHIFT,
				0x00880000 << COMMON_ADDR_SHIFT,
				0x00c80000 << COMMON_ADDR_SHIFT);
			VIQE_MADI_Set_TargetImgBase(MADI_ADDR_3,
				0x00501800 << COMMON_ADDR_SHIFT,
				0x008c0000 << COMMON_ADDR_SHIFT,
				0x00cc0000 << COMMON_ADDR_SHIFT);
			VIQE_MADI_Ctrl_Enable(MADI_ON);
...
#ifdef CONFIG_COMPAT
static long viqe_madi_compat_ioctl(struct file *file, unsigned int cmd,
	unsigned long arg)
{
...
static int viqe_madi_release(struct inode *inode, struct file *filp)
{
	...
	if (viqe_madi->data->dev_opened == 0) {
		...
		if (viqe_madi->data->block_operating) {
			...
			ret = wait_event_interruptible_timeout(
				viqe_madi->data->cmd_wq,
				viqe_madi->data->block_operating == 0,
				msecs_to_jiffies(200));
			if (ret <= 0) {
				pr_info("%s(%d): timed_out block_operation:%d, cmd_count:%d\n",
					__func__, ret,
					viqe_madi->data->block_waiting,
					viqe_madi->data->cmd_count);
			}
		...
		viqe_madi->data->block_operating =
			viqe_madi->data->block_waiting = 0;
		viqe_madi->data->poll_count =
			viqe_madi->data->cmd_count = 0;
	}
...
//		clk_prepare_enable(viqe_madi->ddi_madi_clk);
	...
	if (!viqe_madi->data->irq_reged) {
		ret = request_irq(viqe_madi->irq, viqe_madi_handler,
			IRQF_SHARED, viqe_madi->misc->name, viqe_madi);
...
static int viqe_madi_probe(struct platform_device *pdev)
{
	...
	viqe_madi->info = kzalloc(sizeof(struct viqe_madi_info_type),
		GFP_KERNEL);
	if (viqe_madi->info == 0)
		goto err_info_alloc;
	...
	viqe_madi->vioc_intr = kzalloc(sizeof(struct vioc_intr_type),
		GFP_KERNEL);
...
{
	struct viqe_madi_type *viqe_madi =
		(struct viqe_madi_type *)platform_get_drvdata(pdev);
