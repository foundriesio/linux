...
void tca_sdr_gpio_schmitt(unsigned int base_addr, int pin_num, bool en)
{
	...
	if ((pin_num >= 0) && (pin_num < 32)) {
		...
		pr_err("[%s] ERROR! Invalid input [pin_num:%d]\n",
			__func__, pin_num);
	}
...
void tca_sdr_gpio_set(char port_num, int mode, bool en)
{
	...
	unsigned int gpio_alphabet[] = {
		0, 17, 47, 77, 99, 131, 163, 183, 195, 206, 238};
	unsigned int gpio_offset[] = {
		0, 0x40, 0x80, 0xC0, 0x100, 0x140, 0x180,
		0x240, 0x400, 0x440, 0x480};
	...
	if (port_num > 237) {
		...
		for (i = 0; i < (sizeof(gpio_alphabet)-1); i++) {
			...
			if ((port_num >= gpio_alphabet[i]) &&
				(port_num < gpio_alphabet[i+1])) {
				if (mode == 0) {
					tca_sdr_gpio_func(base_addr,
						port_num-gpio_alphabet[i],
						(en) ? 0xf : 0);
				} else if (mode == 1) {
					tca_sdr_gpio_schmitt(base_addr,
						port_num-gpio_alphabet[i], en);
				} else {
					pr_err("[%s] ERROR! Invalid input [mode:%d]\n",
						__func__, mode);
				}
#if 0   //for debug
				if ((base_addr&0xFFF) <= 0x180)
					pr_info("GPIO_%c_%d[pin_num:%d]\n",
						('A'+i),
						port_num-gpio_alphabet[i],
						port_num);
				else if ((base_addr&0xFFF) == 0x240)
					pr_info("GPIO_SD0_%d[pin_num:%d]\n",
						port_num-gpio_alphabet[i],
						port_num);
				else if ((base_addr&0xFFF) == 0x400)
					pr_info("GPIO_H_%d[pin_num:%d]\n",
					port_num-gpio_alphabet[i], port_num);
				else if ((base_addr&0xFFF) == 0x440)
					pr_info("GPIO_K_%d[pin_num:%d]\n",
					port_num-gpio_alphabet[i], port_num);
#endif
				break;
			}
...
void tca_sdr_port_mux(void *pDAIBaseAddr, struct HS_I2S_PORT *port)
{
	void __iomem *pcfg_reg = pDAIBaseAddr - DAI_OFFSET + PCFG_OFFSET;
	struct HS_I2S_PORT *sdr_port = port;
	unsigned int reg_value = 0;
	int i = 0;

	for (i = 0; i < 3; i++) {
		//GPIO FNC set
		//pr_info("%s clk[%d]=%d ", __func__,i, sdr_port->clk[i]);
		tca_sdr_gpio_set(sdr_port->clk[i], 0, 1);
	}
	for (i = 0; i < 4; i++) {
		//GPIO FNC set
		//pr_info("%s dain[%d]=%d ", __func__,i, sdr_port->dain[i]);
		tca_sdr_gpio_set(sdr_port->dain[i], 0, 1);
	}

	reg_value = (sdr_port->dain[0]|(sdr_port->dain[1]<<8)|
		(sdr_port->dain[2]<<16)|(sdr_port->dain[3]<<24));
	/* DAI_DI0 | DAI_DI1 | DAI_DI2 | DAI_DI3 */
	writel(reg_value, pcfg_reg + PCFG0);

	reg_value = (sdr_port->clk[1]|(sdr_port->clk[2]<<8)|
		(0xFF<<16)|(0xFF<<24));
