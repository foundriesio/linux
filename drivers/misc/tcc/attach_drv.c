...
static int attach_drv_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	if (range_is_allowed(vma->vm_pgoff, vma->vm_end - vma->vm_start) < 0) {
		pr_err(
			"[ERR][ATTACH] %s: %s: Address range is not allowed.\n",
...
static void attach_drv_update(struct attach_drv_type *attach)
{
	...
	unsigned int channel =
		(attach->rdma.id > VIOC_RDMA03) ?
		get_vioc_index((attach->rdma.id - VIOC_RDMA04)) :
		get_vioc_index(attach->rdma.id);
	unsigned int idx = atomic_read(&attach->buf_idx);

	VIOC_RDMA_SetImageSize(attach->rdma.reg,
		attach_info->img_width, attach_info->img_height);
	VIOC_RDMA_SetImageFormat(attach->rdma.reg, attach_info->fmt);
	VIOC_RDMA_SetImageOffset(attach->rdma.reg,
		attach_info->fmt, attach_info->img_width);
	...
	VIOC_RDMA_SetImageBase(attach->rdma.reg,
			attach_info->addr_y[idx],
			attach_info->addr_u[idx], attach_info->addr_v[idx]);

	VIOC_WMIX_SetPosition(attach->wmix.reg, channel,
...
static void attach_drv_ctrl(struct attach_drv_type *attach)
{
	...
	VIOC_SC_SetDstSize(attach->sc.reg,
		attach_info->img_width, attach_info->img_height);
	VIOC_SC_SetOutSize(attach->sc.reg,
		attach_info->img_width, attach_info->img_height);
	VIOC_CONFIG_PlugIn(attach->sc.id, attach->wdma.id);
	VIOC_SC_SetUpdate(attach->sc.reg);

	VIOC_WDMA_SetImageOffset(attach->wdma.reg,
		attach_info->fmt, attach_info->img_width);
	VIOC_WDMA_SetImageFormat(attach->wdma.reg,
		attach_info->fmt);
	VIOC_WDMA_SetImageSize(attach->wdma.reg,
		attach_info->img_width, attach_info->img_height);
	...
	if (attach_info->mode == ATTACH_CAPTURE_MODE) {
		if (wait_event_interruptible_timeout(attach->poll_wq,
			(atomic_read(&attach->block_operating) == 0),
			msecs_to_jiffies(500)) <= 0) {
			atomic_set(&attach->block_operating, 0);
			pr_err(
				"[ERR][ATTACH] %s: %s time-out\n",
				__func__, attach->misc->name);
		}
	}
}
...
static irqreturn_t attach_drv_handler(int irq, void *client_data)
{
	...
	if (is_vioc_intr_activatied(attach->vioc_intr->id,
		attach->vioc_intr->bits) == false)
		return IRQ_NONE;
	vioc_intr_clear(attach->vioc_intr->id, attach->vioc_intr->bits);
	...
	if (attach->info->mode == ATTACH_PREVIEW_MODE) {
		...
		VIOC_WDMA_SetImageBase(attach->wdma.reg,
				attach->info->addr_y[idx],
				attach->info->addr_u[idx],
				attach->info->addr_v[idx]);
		VIOC_WDMA_SetImageEnable(attach->wdma.reg, 0/*OFF*/);
...
static long attach_drv_ioctl(struct file *filp,
	unsigned int cmd, unsigned long arg)
{
	...
	if (atomic_read(&attach->block_operating)) {
		...
		ret = wait_event_interruptible_timeout(
				attach->cmd_wq,
				(atomic_read(&attach->block_waiting) == 0),
				msecs_to_jiffies(200));
		if (ret <= 0) {
			atomic_set(&attach->block_waiting, 0);
			pr_err(
				"[ERR][ATTACH] %s: %s timed_out block_waiting:%d!!\n",
					__func__, attach->misc->name,
					atomic_read(&attach->block_waiting));
		}
		ret = 0;
	}
	...
	case TCC_ATTACH_IOCTRL_KERNEL:
		if (cmd == TCC_ATTACH_IOCTRL_KERNEL) {
			memcpy(attach->info, (struct ATTACH_INFO_TYPE *)arg,
				sizeof(struct ATTACH_INFO_TYPE));
		} else {
			if (copy_from_user(attach->info,
				(struct ATTACH_INFO_TYPE *)arg,
				sizeof(struct ATTACH_INFO_TYPE))) {
				pr_err(
					"[ERR][ATTACH] %s: Not Supported copy_from_user(%d).\n",
					__func__, cmd);
				ret = -EFAULT;
			}
		}
		...
		if (attach->info->mode) {
			...
			if (cmd == TCC_ATTACH_GET_BUF_IOCTRL_KERNEL) {
				...
				if (copy_to_user((unsigned long *)arg,
					...
					pr_err(
						"[ERR][ATTACH] %s: Not Supported copy_to_user(%d).\n",
						__func__, cmd);
					ret = -EFAULT;
				}
			}
			...
			pr_err(
				"[ERR][ATTACH] %s: %s driver is not running(%d).\n",
					__func__, attach->misc->name,
					attach->info->mode);
			ret = -EBUSY;
		}
	...
	default:
		pr_err(
			"[ERR][ATTACH] %s: not supported %s IOCTL(0x%x).\n",
...
#ifdef CONFIG_COMPAT
static long attach_drv_compat_ioctl(struct file *file,
	unsigned int cmd, unsigned long arg)
{
...
static int attach_drv_release(struct inode *inode, struct file *filp)
{
	...
	if (atomic_read(&attach->dev_opened) == 0) {
		if (atomic_read(&attach->block_operating)) {
			atomic_set(&attach->block_waiting, 1);
		if (wait_event_interruptible_timeout(attach->cmd_wq,
			(atomic_read(&attach->block_waiting) == 0),
		...
		if (atomic_read(&attach->irq_reged)) {
			vioc_intr_clear(attach->vioc_intr->id,
				attach->vioc_intr->bits);
			vioc_intr_disable(attach->irq,
				attach->vioc_intr->id, attach->vioc_intr->bits);
...
static int attach_drv_open(struct inode *inode, struct file *filp)
{
	...
	if (atomic_read(&attach->dev_opened) == 0) {
		...
		if (atomic_read(&attach->irq_reged) == 0) {
			...
			vioc_intr_clear(attach->vioc_intr->id,
				attach->vioc_intr->bits);
			ret = request_irq(attach->irq, attach_drv_handler,
				IRQF_SHARED, attach->misc->name, attach);
			if (ret) {
				if (attach->clk)
					clk_disable_unprepare(attach->clk);
				pr_err(
					"[ERR][ATTACH] %s: failed to aquire %s request_irq.\n",
					__func__, attach->misc->name);
				return -EFAULT;
			}
			vioc_intr_enable(attach->irq,
				attach->vioc_intr->id, attach->vioc_intr->bits);
...
static void attach_drv_parse_dt(struct attach_drv_type *attach,
	struct device_node *node)
{
	...
	if (dev_np) {
		...
		pr_err(
			"[ERR][ATTACH] %s: could not find rdma node of %s driver.\n",
			__func__, attach->misc->name);
		attach->rdma.reg = NULL;
	}
	...
	if (dev_np) {
		...
		pr_err(
			"[ERR][ATTACH] %s: could not find wmix node of %s driver.\n",
			__func__, attach->misc->name);
	...
	if (dev_np) {
		...
		pr_err(
			"[ERR][ATTACH] %s: could not find attach node of %s driver.\n",
			__func__, attach->misc->name);
		attach->sc.reg = NULL;
	}
	...
	if (dev_np) {
		...
		attach->irq =
			irq_of_parse_and_map(dev_np, get_vioc_index(index));
		attach->vioc_intr->id   =
			VIOC_INTR_WD0 + get_vioc_index(attach->wdma.id);
		attach->vioc_intr->bits = VIOC_WDMA_IREQ_EOFR_MASK;
	} else {
		pr_err(
			"[ERR][ATTACH] %s: could not find wdma node of %s driver.\n",
			__func__, attach->misc->name);
...
static int attach_drv_probe(struct platform_device *pdev)
{
	...
	pr_info(
		"[INF][ATTACH] %s: id:%d, Attach Driver Initialized\n",
		pdev->name, attach->id);
	return 0;
...
{
	struct attach_drv_type *attach =
		(struct attach_drv_type *)platform_get_drvdata(pdev);
