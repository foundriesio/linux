...
static void tcc_shmem_reset(struct device *dev)
{
	...
	if (shdev->core_num == 0) {
		tcc_shmem_writel(0, shdev->base + TCC_SHM_A53_REQ_OFFSET);
	} else {
		tcc_shmem_writel(0, shdev->base + TCC_SHM_A72_REQ_OFFSET);
	}
	...
	if (tcc_shm_valid != 0) {
		...
		if (desc_pos->core_num == 0) {
			...
			tcc_shmem_writel((1 << desc_pos->bit_place),
					 desc_pos->a53_dist_reg + (0x204 +
								   (4 *
								    desc_pos->pending_offset)));
		} else {
			tcc_shmem_writel((1 << port),
					 desc_pos->base +
					 TCC_SHM_A53_READ_REQ_OFFSET);
			tcc_shmem_writel((1 << desc_pos->bit_place),
					 desc_pos->a72_dist_reg + (0x204 +
								   (4 *
								    desc_pos->pending_offset)));
		}
			      ...
			      char *buf)
{
	...
	received_num =
	    tcc_shmem_receive_port(dev, 2, SHM_RECV_BUF_NUM, received_data);
	printk("%s\n", received_data);
			       ...
			       size_t count)
{
	...
	tcc_shmem_transfer_port_nodev(2, count, trans_data);
