...
#if 0
struct timer_list tcc_shm_timer;
#endif
...
static long tcc_shmem_ioctl(struct file *flip, unsigned int cmd, unsigned long arg);
static int tcc_shmem_release(struct inode *inode, struct file *file);
static int32_t tcc_shm_receive_port(struct device *dev, int32_t port, uint32_t size, char *data);
static int32_t tcc_shm_create_desc_handler(struct device *dev);
static int32_t tcc_shm_del_desc_handler(struct device *dev);
static int32_t tcc_shmem_create_desc_req(struct device *dev, char* name, uint32_t size);
...
static irqreturn_t tcc_shmem_handler(int irq, void *dev)
{
	...
	if(tcc_shm_valid == 0){
		...
		if(shdev->core_num == 0) {
			val = tcc_shmem_readl(shdev->base+TCC_SHM_A53_REQ_OFFSET);
			read_req = tcc_shmem_readl(shdev->base+TCC_SHM_A53_READ_REQ_OFFSET);
		} else {
			val = tcc_shmem_readl(shdev->base+TCC_SHM_A72_REQ_OFFSET);
			read_req = tcc_shmem_readl(shdev->base+TCC_SHM_A72_READ_REQ_OFFSET);
		}
		...
		if(shdev->irq_mode) {
			if(read_req){
				for(i = 0; i < 32; i++){
					if(read_req & (1<<i)) {
						if(shdev->que_next_pos > shdev->que_pos) {
							for(j = shdev->que_pos; j < shdev->que_next_pos; j++) {
								if(shdev->read_que[j]==i) {
									port_exist = 1;
									break;
								}
							}
						} else if(shdev->que_next_pos < shdev->que_pos) {
							for(j = shdev->que_pos; j < 32 ; j++) {
								if(shdev->read_que[j]==i) {
									port_exist = 1;
									break;
								}
							...
							for(j = 0; j < shdev->que_next_pos; j++) {
								if(shdev->read_que[j]==i) {
									port_exist = 1;
									break;
								}
						...
						if(port_exist != 1) {
							...
							if(shdev->que_next_pos == 31) {
								shdev->read_que[shdev->que_next_pos] = i;
								shdev->que_next_pos = 0;

							} else {
								shdev->read_que[shdev->que_next_pos] = i;
								shdev->que_next_pos += 1;
							}
...
static void tcc_shm_tasklet_handler(unsigned long data) {
	...
	if(shdev->port_req) {
		tcc_shm_create_desc_handler(shdev->dev);
		//printk("%s: tcc shared mem. desc. creation\n", __func__);
	}

	if(shdev->port_del_req) {
		tcc_shm_del_desc_handler(shdev->dev);
	...
	if(shdev->read_req) {
		...
		while(!(shdev->que_pos==shdev->que_next_pos)) {
			list_for_each_entry_safe(desc_pos, desc_pos_s, &tcc_shm_list, list) {
				if(desc_pos->port_num == shdev->read_que[shdev->que_pos]) {
					port_found = 1;
					break;
				}
			}
			...
			if(port_found != 1) {
				...
				received_data = devm_kzalloc(dev, sizeof(char)*30, GFP_KERNEL);
				do {
					received_num = tcc_shm_receive_port(dev, desc_pos->port_num, 30, received_data);
					if(received_num < 0) {
						printk("%s: no port_exist!_1\n", __func__);
					}
					if(desc_pos->shm_callback.callback_func) {
						//printk("%s: callback executed\n", __func__);
						if(received_num > 0) {
							desc_pos->shm_callback.callback_func(desc_pos->shm_callback.data, received_data, received_num);
						}
					} else {
				...
				if(shdev->core_num == 0) {
					tcc_shmem_writel((tcc_shmem_readl(shdev->base+TCC_SHM_A53_READ_REQ_OFFSET)&~(1<<desc_pos->port_num)), shdev->base+TCC_SHM_A53_READ_REQ_OFFSET);
				} else {
					tcc_shmem_writel((tcc_shmem_readl(shdev->base+TCC_SHM_A72_READ_REQ_OFFSET)&~(1<<desc_pos->port_num)), shdev->base+TCC_SHM_A72_READ_REQ_OFFSET);
				}
...
static void tcc_shm_port_req_tasklet_handler(unsigned long data) {
	...
	if(shdev->core_num == 0) { //only maincore create and request port
		list_for_each_entry_safe(req_port_pos, req_port_pos_s, &tcc_shm_req_name, list) {
			tcc_shmem_create_desc_req(dev, req_port_pos->name, req_port_pos->size);
...
#if 0
static void tcc_shm_timer_func(unsigned long args) {
	struct tcc_shm_data *shdev = (struct tcc_shm_data *)args;
	uint32_t val1, val2;
	struct device *dev = shdev->dev;

	val1 = tcc_shmem_readl(shdev->base+TCC_SHM_A53_REQ_OFFSET)&TCC_SHM_ENABLED;
	val2 = tcc_shmem_readl(shdev->base+TCC_SHM_A72_REQ_OFFSET)&TCC_SHM_ENABLED;

	//printk("%s : val1 : %d val2 : %d\n", __func__, val1, val2);

	if((val1 != 0)&&(val2 !=0)) {
		tcc_shm_valid = 1;
	} else {
		mod_timer(&tcc_shm_timer, jiffies+msecs_to_jiffies(100));
	}
}
#endif
...
int32_t tcc_shmem_register_callback(int32_t port, struct tcc_shm_callback shm_callback) {
	...
	if(tcc_shm_valid != 0) {
		...
		if(shm_callback.callback_func) {
			desc_pos->shm_callback.callback_func = shm_callback.callback_func;
		} else {
...
static int32_t tcc_shm_receive_port(struct device *dev, int32_t port, uint32_t size, char *data) {
	...
	if(tcc_shm_valid != 0) {
		...
		if(shdev->core_num == 0) {
			memcpy_fromio(&head, shdev->base+desc_pos->offset_72+desc_pos->size+HEAD_OFFSET_53, sizeof(uint32_t));
			memcpy_fromio(&tail, shdev->base+desc_pos->offset_72+desc_pos->size+TAIL_OFFSET_53, sizeof(uint32_t));
			//printk("%s:1 head : 0x%x tail : 0x%x addr : 0x%px\n", __func__, head, tail, shdev->base+desc_pos->offset_72+desc_pos->size+TAIL_OFFSET_53);
		} else {
			memcpy_fromio(&head, shdev->base+desc_pos->offset_72+desc_pos->size+HEAD_OFFSET_72, sizeof(uint32_t));
			memcpy_fromio(&tail, shdev->base+desc_pos->offset_72+desc_pos->size+TAIL_OFFSET_72, sizeof(uint32_t));
			//printk("%s:2 head : 0x%x tail : 0x%x addr : 0x%px\n", __func__, head, tail, shdev->base+desc_pos->offset_72+desc_pos->size+TAIL_OFFSET_72);
		}
		...
		if(post_tail < (((desc_pos->size+1)/2))) {
			cnt = size;
			if(shdev->core_num == 0) {
				memcpy_fromio(data, shdev->base+desc_pos->offset_53+tail, cnt);
				memcpy_toio(shdev->base+desc_pos->offset_72+desc_pos->size+TAIL_OFFSET_53, &post_tail, sizeof(uint32_t));
			} else {
				memcpy_fromio(data, shdev->base+desc_pos->offset_72+tail, cnt);
				memcpy_toio(shdev->base+desc_pos->offset_72+desc_pos->size+TAIL_OFFSET_72, &post_tail, sizeof(uint32_t));
			}
		} else {
			...
			if(shdev->core_num == 0) {
				memcpy_fromio(data, shdev->base+desc_pos->offset_53+tail, cnt);
				memcpy_fromio(data+cnt, shdev->base+desc_pos->offset_53, left_cnt);
				memcpy_toio(shdev->base+desc_pos->offset_72+desc_pos->size+TAIL_OFFSET_53, &post_tail, sizeof(uint32_t));
			} else {
				memcpy_fromio(data, shdev->base+desc_pos->offset_72+tail, cnt);
				memcpy_fromio(data+cnt, shdev->base+desc_pos->offset_72, left_cnt);
				memcpy_toio(shdev->base+desc_pos->offset_72+desc_pos->size+TAIL_OFFSET_72, &post_tail, sizeof(uint32_t));
			}
		}
		return size;
...
int32_t tcc_shm_transfer_port_nodev(int32_t port, uint32_t size, char *data) {
	//static int32_t tcc_shm_transfer_port(struct device *dev, int32_t port, uint32_t size, char *data) {
	struct tcc_shm_desc *desc_pos, *desc_pos_s;
	uint32_t head, post_head, cnt, left_cnt, port_found = 0;

	//printk("%s: %d %d %s\n", __func__, port, size, data);
	if(tcc_shm_valid != 0) {
		...
		if(desc_pos->core_num == 0) {
			memcpy_fromio(&head, desc_pos->base+desc_pos->offset_72+desc_pos->size+HEAD_OFFSET_72, sizeof(uint32_t));
		} else {
			memcpy_fromio(&head, desc_pos->base+desc_pos->offset_72+desc_pos->size+HEAD_OFFSET_53, sizeof(uint32_t));
		}
		...
		if(post_head < (((desc_pos->size+1)/2))) {
			cnt = size;
			if(desc_pos->core_num == 0) {
				memcpy_toio(desc_pos->base+desc_pos->offset_72+head, data, cnt);
				memcpy_toio(desc_pos->base+desc_pos->offset_72+desc_pos->size+HEAD_OFFSET_72, &post_head, sizeof(uint32_t));
			} else {
				memcpy_toio(desc_pos->base+desc_pos->offset_53+head, data, cnt);
				memcpy_toio(desc_pos->base+desc_pos->offset_72+desc_pos->size+HEAD_OFFSET_53, &post_head, sizeof(uint32_t));
			}
		} else {
			...
			if(left_cnt > (((desc_pos->size+1)/2))) {
				printk("%s: size of packet is too big to transfer through shared memory. \
			...
			if(desc_pos->core_num == 0) {
				memcpy_toio(desc_pos->base+desc_pos->offset_72+head, data, cnt);
				memcpy_toio(desc_pos->base+desc_pos->offset_72, data+cnt, left_cnt);
				memcpy_toio(desc_pos->base+desc_pos->offset_72+desc_pos->size+HEAD_OFFSET_72, &post_head, sizeof(uint32_t));
			} else {
				memcpy_toio(desc_pos->base+desc_pos->offset_53+head, data, cnt);
				memcpy_toio(desc_pos->base+desc_pos->offset_53, data+cnt, left_cnt);
				memcpy_toio(desc_pos->base+desc_pos->offset_72+desc_pos->size+HEAD_OFFSET_53, &post_head, sizeof(uint32_t));
			}
		}
		...
		if(desc_pos->core_num == 0) {
			tcc_shmem_writel((1<<port), desc_pos->base+TCC_SHM_A72_READ_REQ_OFFSET);
			tcc_shmem_writel((1<<desc_pos->bit_place), desc_pos->a53_dist_reg+(0x204+(4*desc_pos->pending_offset)));
		} else {
			tcc_shmem_writel((1<<port), desc_pos->base+TCC_SHM_A53_READ_REQ_OFFSET);
			tcc_shmem_writel((1<<desc_pos->bit_place), desc_pos->a72_dist_reg+(0x204+(4*desc_pos->pending_offset)));
		}
...
EXPORT_SYMBOL(tcc_shm_transfer_port_nodev);
...
static int32_t tcc_shm_transfer_port(struct device *dev, int32_t port, uint32_t size, char *data) {
	...
	if(tcc_shm_valid != 0) {
		...
		if(post_head < (((desc_pos->size+1)/2))) {
			cnt = size;
			if(shdev->core_num == 0) {
				memcpy_toio(shdev->base+desc_pos->offset_72+head, data, cnt);
				memcpy_toio(shdev->base+desc_pos->offset_72+desc_pos->size+HEAD_OFFSET_72, &post_head, sizeof(uint32_t));
			} else {
				memcpy_toio(shdev->base+desc_pos->offset_53+head, data, cnt);
				memcpy_toio(shdev->base+desc_pos->offset_72+desc_pos->size+HEAD_OFFSET_53, &post_head, sizeof(uint32_t));
			}
		} else {
			cnt = (((desc_pos->size+1)/2)) - head;
			left_cnt = post_head - (((desc_pos->size+1)/2));
			...
			if(shdev->core_num == 0) {
				memcpy_toio(shdev->base+desc_pos->offset_72+head, data, cnt);
				memcpy_toio(shdev->base+desc_pos->offset_72, data+cnt, left_cnt);
				memcpy_toio(shdev->base+desc_pos->offset_72+desc_pos->size+HEAD_OFFSET_72, &post_head, sizeof(uint32_t));
			} else {
				memcpy_toio(shdev->base+desc_pos->offset_53+head, data, cnt);
				memcpy_toio(shdev->base+desc_pos->offset_53, data+cnt, left_cnt);
				memcpy_toio(shdev->base+desc_pos->offset_72+desc_pos->size+HEAD_OFFSET_53, &post_head, sizeof(uint32_t));
			}
		}
		return 0;
...
static int32_t tcc_shm_del_desc_handler(struct device *dev) {
	...
	if(shdev->core_num == 0) {
		tcc_shmem_writel(tcc_shmem_readl(shdev->base)&~TCC_SHM_PORT_DEL_REQ, shdev->base+TCC_SHM_A53_REQ_OFFSET);
	} else {
		tcc_shmem_writel(tcc_shmem_readl(shdev->base)&~TCC_SHM_PORT_DEL_REQ, shdev->base+TCC_SHM_A72_REQ_OFFSET);
	}
...
static int32_t tcc_shm_create_desc_handler(struct device *dev) {
	...
	memcpy_fromio(desc_new->name, shdev->base+TCC_SHM_NAME_OFFSET, name_size);

	//printk("%s: name : %s size : %d\n", __func__, desc_new->name, name_size);
	...
	if(shdev->core_num == 0) {
		tcc_shmem_writel(tcc_shmem_readl(shdev->base+TCC_SHM_A53_REQ_OFFSET)&~TCC_SHM_PORT_REQ, shdev->base+TCC_SHM_A53_REQ_OFFSET);
	} else {
		tcc_shmem_writel(tcc_shmem_readl(shdev->base+TCC_SHM_A72_REQ_OFFSET)&~TCC_SHM_PORT_REQ, shdev->base+TCC_SHM_A72_REQ_OFFSET);
	}

	//printk("%s: new port : %d, size : %d, offset : 0x%x\n", __func__, desc_new->port_num, desc_new->size, desc_new->offset_72);
		...
		if(port_found == 0) {
			list_for_each_entry_safe(req_port_pos, req_port_pos_s, &tcc_shm_req_name, list) {
				...
				if(req_port_pos->name != NULL) {
					if(strcmp(req_port_pos->name, name) == 0) {
						port_found =1;
						break;
					}
				}
			...
			if(port_found == 0) {
				req_port_new = kzalloc(sizeof(struct tcc_shm_req_port), GFP_KERNEL);
				req_port_new->name = kzalloc(strlen(name), GFP_KERNEL);
				memcpy(req_port_new->name, name, strlen(name));
				req_port_new->size = size;
				//printk("%s: name : %s size : %d\n", __func__, req_port_new->name, req_port_new->size);
				list_add_tail(&req_port_new->list, &tcc_shm_req_name);
...
static int32_t tcc_shmem_create_desc_req(struct device *dev, char* name, uint32_t size) {
	...
	if(tcc_shm_valid != 0) {
		...
		if((val & TCC_SHM_PORT_REQ)||(val & TCC_SHM_PORT_DEL_REQ)) {
			printk("%s: port creation and deletion req. is processing!\n", __func__);
			return -1;
		} else {
			if(shdev->core_num == 0) {
				tcc_shmem_writel(tcc_shmem_readl(shdev->base+TCC_SHM_A72_REQ_OFFSET)|TCC_SHM_PORT_REQ, shdev->base+TCC_SHM_A72_REQ_OFFSET);
		...
		desc_new = devm_kzalloc(dev, sizeof(struct tcc_shm_desc), GFP_KERNEL);
		desc_new->name = devm_kzalloc(dev, strlen(name), GFP_KERNEL);
		...
		if(total_port == 0) {
			...
			port_list = devm_kzalloc(dev, sizeof(struct tcc_shm_port_list)*total_port, GFP_KERNEL);
			mem_map = devm_kzalloc(dev, sizeof(struct tcc_shm_mem_map)*total_port, GFP_KERNEL);
			...
			for(i = 0; i < total_port - 1; i++) {
				for(j = 0; j < total_port - 1 - i; j++){
					if(port_list[j].port_num > port_list[j + 1].port_num) {
						temp_port = port_list[j].port_num;
						port_list[j].port_num = port_list[j + 1].port_num;
			...
			for(i = 0; i < total_port; i++) {
				if(i==0) {
					...
					if(port_list[i].port_num != (port_list[i-1].port_num + 1)) {
						desc_new->port_num = port_list[i-1].port_num + 1;
						break;
					}
				}

				if(i == total_port-1) {
					desc_new->port_num = port_list[i].port_num+1;
				}
			}
			...
			//for(i = 0; i < total_port; i++) {
			//printk("%s: memmap offset : 0x%x memmap size : 0x%x port_num : %d\n", __func__, mem_map[i].offset, mem_map[i].size, mem_map[i].port_num);
			//}

			for(i = 0; i < total_port; i++) {
				if(i==0) {
					if((uintptr_t)mem_map[i].offset > TCCSHM_BASE_OFFSET_VAL) { // ground memory address : shdev->reg_phy + TCCSHM_BASE_OFFSET_VAL
						if(((uintptr_t)mem_map[i].offset - (TCCSHM_BASE_OFFSET_VAL)) > (desc_new->size + 0xf)) {
				...
				} else if(((mem_map[i].offset) - (mem_map[i-1].offset + mem_map[i-1].size + 0xf)) > (desc_new->size + 0xf)) {
					desc_new->offset_72 = (mem_map[i-1].offset + mem_map[i-1].size) + 1 + 0xf;// allocate memory just next to previous memory. plus '1'. reserve 0xf for tail and head of Tx and Rx. plus '0xf'.
					desc_new->offset_53 = desc_new->offset_72 + ((desc_new->size+1)/2);
				...
				if(i==total_port-1) {
					if((mem_map[i].offset + mem_map[i].size) < shdev->reg_phy + shdev->size) {
						if(((shdev->reg_phy + shdev->size) - ((uintptr_t)mem_map[i].offset + mem_map[i].size + 0xf)) > (desc_new->size + 0xf)) {
							desc_new->offset_72 = (mem_map[i].offset + mem_map[i].size) + 1 + 0xf; // allocate memory just next to previous memory. plus '1'. reserve 0xf for tail and head of Tx and Rx. plus '0xf'.
							desc_new->offset_53 = desc_new->offset_72 + ((desc_new->size+1)/2);
							//printk("%s: offset set 3 : 0x%x, offset : 0x%x, size : 0x%x\n", __func__, desc_new->offset_72, mem_map[i].offset, mem_map[i].size);
							break;
						}
					}
				}
		...
		tcc_shmem_writel(desc_new->offset_72, shdev->base+TCC_SHM_BASE_OFFSET);
		tcc_shmem_writel(desc_new->size, shdev->base+TCC_SHM_SIZE_OFFSET);
		tcc_shmem_writel(desc_new->port_num, shdev->base+TCC_SHM_PORT_OFFSET);
		memset_io(shdev->base+TCC_SHM_NAME_OFFSET, 0, TCC_SHM_NAME_SPACE);
		memcpy_toio(shdev->base+TCC_SHM_NAME_OFFSET, desc_new->name, strlen(desc_new->name));
		tcc_shmem_writel(strlen(desc_new->name), shdev->base+TCC_SHM_NAME_SIZE);
		val = 0;
		memcpy_toio(shdev->base+desc_new->offset_72+desc_new->size+HEAD_OFFSET_72, &val, sizeof(uint32_t));
		memcpy_toio(shdev->base+desc_new->offset_72+desc_new->size+TAIL_OFFSET_72, &val, sizeof(uint32_t));
		memcpy_toio(shdev->base+desc_new->offset_72+desc_new->size+HEAD_OFFSET_53, &val, sizeof(uint32_t));
		memcpy_toio(shdev->base+desc_new->offset_72+desc_new->size+TAIL_OFFSET_53, &val, sizeof(uint32_t));

		//printk("%s: new port : %d, size : %d offset : 0x%x head: 0x%px\n", __func__, desc_new->port_num, desc_new->size, desc_new->offset_72, shdev->base+desc_new->offset_72+desc_new->size);

		if(shdev->core_num == 0) {
			tcc_shmem_writel((1<<shdev->bit_place), shdev->a53_dist_reg+(0x204+(4*shdev->pending_offset)));
		} else {
			tcc_shmem_writel((1<<shdev->bit_place), shdev->a72_dist_reg+(0x204+(4*shdev->pending_offset)));
		}
...
static int32_t tcc_shmem_deletion_desc_req(struct device *dev, int32_t port) {
	...
	if(tcc_shm_valid != 0) {
		...
		if((val & TCC_SHM_PORT_REQ)||(val & TCC_SHM_PORT_DEL_REQ)) {
			...
			if(shdev->core_num == 0) {
				tcc_shmem_writel(tcc_shmem_readl(shdev->base+TCC_SHM_A72_REQ_OFFSET)|TCC_SHM_PORT_DEL_REQ, shdev->base+TCC_SHM_A72_REQ_OFFSET);
			} else {
				tcc_shmem_writel(tcc_shmem_readl(shdev->base+TCC_SHM_A53_REQ_OFFSET)|TCC_SHM_PORT_DEL_REQ, shdev->base+TCC_SHM_A53_REQ_OFFSET);
			}
		}
...
static ssize_t shm_cre_req(struct device *dev, struct device_attribute *attr, char *buf)
{
	int val=0;
...
static ssize_t shm_del_req(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int32_t ret, input;
...
static ssize_t shm_trans_read(struct device *dev, struct device_attribute *attr, char *buf)
{
	char *received_data;
	...
	received_num = tcc_shm_receive_port(dev, 0, 30, received_data);
	//printk("%s\n", received_data);
...
static ssize_t shm_trans_write(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	char *trans_data;
	...
	tcc_shm_transfer_port(dev, 0, count, trans_data);
...
static DEVICE_ATTR(tcc_shmem_trans, S_IRUGO|S_IWUSR, shm_trans_read, shm_trans_write);
...
static int tcc_shmem_probe(struct platform_device *pdev) {
	...
	reg = devm_ioremap(&pdev->dev, reg_phy, size);
	ret = request_irq(irq, (irq_handler_t)tcc_shmem_handler, IRQF_SHARED, "TCC_SHMEM", (void *)&pdev->dev);

	shdev = devm_kzalloc(&pdev->dev, sizeof(struct tcc_shm_data), GFP_KERNEL);
	shdev->base = reg;
	shdev->dev = dev;
	tasklet_init(&tcc_shm_tasklet, tcc_shm_tasklet_handler, (unsigned long)shdev);

	of_property_read_u32_index(np, "core-num", 0, &core_num);
	//printk("jyp core num : %d\n", core_num);
	...
	//printk("Major = %d, Minor = %d\n", MAJOR(tcc_shmem_devt), MINOR(tcc_shmem_devt));
	...
	tasklet_init(&tcc_shm_port_req_tasklet, tcc_shm_port_req_tasklet_handler, (unsigned long)shdev);

#if 0
	init_timer(&tcc_shm_timer);
	tcc_shm_timer.function = tcc_shm_timer_func;
	tcc_shm_timer.data = (unsigned long)shdev;
	mod_timer(&tcc_shm_timer, jiffies+msecs_to_jiffies(100));
#endif
