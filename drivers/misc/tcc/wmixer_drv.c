...
static int wmixer_drv_ctrl(struct wmixer_drv_type *wmixer)
{
    ...
    dprintk("%s() : name:%s \n", __func__, wmixer->misc->name);
    dprintk("Src  : addr:0x%x 0x%x 0x%x  fmt:%d (swap:ext=%d/%d), crop(%d,%d ~ %dx%d) \n", wmix_info->src_y_addr, wmix_info->src_u_addr, wmix_info->src_v_addr, wmix_info->src_fmt, wmix_info->src_rgb_swap, wmix_info->src_fmt_ext_info,
                    wmix_info->src_win_left, wmix_info->src_win_top, wmix_info->src_win_right, wmix_info->src_win_bottom);
    dprintk("Dest : addr:0x%x 0x%x 0x%x  fmt:%d (swap:ext=%d/%d), crop(%d,%d ~ %dx%d)\n", wmix_info->dst_y_addr, wmix_info->dst_u_addr, wmix_info->dst_v_addr, wmix_info->dst_fmt, wmix_info->dst_rgb_swap, wmix_info->dst_fmt_ext_info,
                    wmix_info->dst_win_left, wmix_info->dst_win_top, wmix_info->dst_win_right, wmix_info->dst_win_bottom);
    ...
    if(wmix_info->src_fmt_ext_info == 0x10)
    {
        ...
        if(VIOC_CONFIG_DMAPath_Support()) {
            int component_num = VIOC_CONFIG_DMAPath_Select(wmixer->rdma0.id);
        ...
        tca_map_convter_driver_set(VIOC_MC0 + gMC_NUM, wmix_info->src_img_width, wmix_info->src_img_height,
                wmix_info->src_win_left, wmix_info->src_win_top, (wmix_info->src_win_right - wmix_info->src_win_left), (wmix_info->src_win_bottom - wmix_info->src_win_top), 0, &wmix_info->mapConv_info);
    ...
    if(wmix_info->src_fmt_ext_info == 0x20)
    {
        ...
        if(VIOC_CONFIG_DMAPath_Support()) {
            ...
            if((VIOC_DTRC0 + gDTRC_NUM) != component_num){
                VIOC_CONFIG_DMAPath_UnSet(component_num);
                tca_dtrc_convter_swreset(VIOC_DTRC0 + gDTRC_NUM);
            }

            VIOC_CONFIG_DMAPath_Set(wmixer->rdma0.id, (VIOC_DTRC0 + gDTRC_NUM));
        }
        tca_dtrc_convter_driver_set(VIOC_DTRC0 + gDTRC_NUM, (wmix_info->src_win_right - wmix_info->src_win_left), (wmix_info->src_win_bottom - wmix_info->src_win_top),
                wmix_info->src_win_left, wmix_info->src_win_top, 0, &wmix_info->dtrcConv_info);
    ...
    #endif//
    {
        ...
        if(wmix_info->src_fmt_ext_info == 10)
            ...
            VIOC_RDMA_SetImageOffset(pWMIX_rdma_base, wmix_info->src_fmt, wmix_info->src_img_width*125/100);
        else
        #endif
    ...
    if(wmix_info->dst_fmt_ext_info == 10)
        VIOC_WDMA_SetImageOffset(pWMIX_wdma_base, wmix_info->dst_fmt, wmix_info->dst_img_width*2);
    else if(wmix_info->dst_fmt_ext_info == 11)
        VIOC_WDMA_SetImageOffset(pWMIX_wdma_base, wmix_info->dst_fmt, wmix_info->dst_img_width*125/100);
    else
#endif
...
static int wmixer_drv_alpha_scaling_ctrl(struct wmixer_drv_type *wmixer)
{
    ...
    dprintk("etc  : mc_num:%d, fmt_info:%d/%d, inter:%d. \n", aps_info->mc_num, aps_info->src_fmt_ext_info, aps_info->dst_fmt_ext_info, aps_info->interlaced);
#endif
    ...
    dprintk("=> Src  : addr:0x%x, 0x%x, 0x%x,  fmt:%d. \n", pSrcBase0, pSrcBase1, pSrcBase2, aps_info->src_fmt);
    dprintk("=> Dst  : addr:0x%x, 0x%x, 0x%x,  fmt:%d. \n", pDstBase0, pDstBase1, pDstBase2, aps_info->dst_fmt);
    ...
    if(aps_info->src_fmt_ext_info == 0x10) {
        ...
        tca_map_convter_driver_set(VIOC_MC0 + gMC_NUM, aps_info->src_img_width, aps_info->src_img_height,
            aps_info->src_win_left, aps_info->src_win_top, (aps_info->src_win_right - aps_info->src_win_left), (aps_info->src_win_bottom - aps_info->src_win_top),0, &aps_info->mapConv_info);
        tca_map_convter_onoff(VIOC_MC0 + gMC_NUM, 1, 0);
    }
    else
#endif
    ...
    if(aps_info->src_fmt_ext_info == 0x20) {
        ...
        tca_dtrc_convter_driver_set((VIOC_DTRC0 + gDTRC_NUM), (aps_info->src_win_right - aps_info->src_win_left), (aps_info->src_win_bottom - aps_info->src_win_top),
            aps_info->src_win_left, aps_info->src_win_top, 0, &aps_info->dtrcConv_info);
        tca_dtrc_convter_onoff((VIOC_DTRC0 + gDTRC_NUM), 1, 0);
    }
    else
#endif
    {
        ...
        if(aps_info->src_fmt_ext_info == 10)
            ...
            VIOC_RDMA_SetDataFormat(pWMIX_rdma_base, 0x3, 0x0); /* YUV real 10bit support */
        else
            VIOC_RDMA_SetDataFormat(pWMIX_rdma_base, 0x0, 0x0);
    #endif
        ...
        if (aps_info->interlaced
            ...
            VIOC_RDMA_SetImageOffset(pWMIX_rdma_base, aps_info->src_fmt, aps_info->src_img_width*125/100);
    #endif
        else
	...
	if(!system_rev && aps_info->src_fmt_ext_info == 0x10)
	{
		...
		VIOC_SC_SetDstSize(pWMIX_sc_base, (aps_info->dst_win_right - aps_info->dst_win_left), (aps_info->dst_win_bottom - aps_info->dst_win_top)+plus_height);
	}
	else
#endif
		VIOC_SC_SetDstSize(pWMIX_sc_base, (aps_info->dst_win_right - aps_info->dst_win_left), (aps_info->dst_win_bottom - aps_info->dst_win_top));
    VIOC_CONFIG_PlugIn(wmixer->sc.id, wmixer->rdma0.id);
    VIOC_SC_SetUpdate(pWMIX_sc_base);

    VIOC_CONFIG_WMIXPath(wmixer->rdma0.id, 0); // wmixer op is by-pass mode.
    VIOC_WMIX_SetSize(pWMIX_wmix_base, (aps_info->dst_win_right - aps_info->dst_win_left), (aps_info->dst_win_bottom - aps_info->dst_win_top));
    VIOC_WMIX_SetUpdate(pWMIX_wmix_base);
    ...
    if(aps_info->dst_fmt_ext_info == 10)
        VIOC_WDMA_SetImageOffset(pWMIX_wdma_base, aps_info->dst_fmt, aps_info->dst_img_width*2);
    else if(aps_info->dst_fmt_ext_info == 11)
        VIOC_WDMA_SetImageOffset(pWMIX_wdma_base, aps_info->dst_fmt, aps_info->dst_img_width*125/100);
    else
#endif
    ...
    VIOC_WDMA_SetImageSize(pWMIX_wdma_base, (aps_info->dst_win_right - aps_info->dst_win_left), (aps_info->dst_win_bottom - aps_info->dst_win_top));
...
static int wmixer_drv_alpha_mixing_ctrl(struct wmixer_drv_type *wmixer)
{
    ...
    VIOC_WMIX_ALPHA_SetAlphaValue(pWMIX_wmix_base, apb_info->src0_layer, apb_info->src0_alpha0, apb_info->src0_alpha1);
    VIOC_WMIX_ALPHA_SetAlphaSelection(pWMIX_wmix_base, apb_info->src0_layer, apb_info->src0_asel);
    VIOC_WMIX_ALPHA_SetROPMode(pWMIX_wmix_base, apb_info->src0_layer, apb_info->src0_rop_mode);
    // set to layer1 of WMIX.
    //VIOC_WMIX_ALPHA_SetAlphaValueControl(pWMIX_wmix_base, apb_info->src1_layer, apb_info->region, apb_info->src1_acon0, apb_info->src1_acon1);
    //VIOC_WMIX_ALPHA_SetColorControl(pWMIX_wmix_base, apb_info->src1_layer, apb_info->region, apb_info->src1_ccon0, apb_info->src1_ccon1);
...
#ifdef CONFIG_COMPAT
static long wmixer_drv_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
...
static int wmixer_drv_release(struct inode *inode, struct file *filp)
{
    ...
    if (wmixer->data->dev_opened == 0) {
        ...
        if(wmixer->data->irq_reged) {
            vioc_intr_clear(wmixer->vioc_intr->id, wmixer->vioc_intr->bits);
            vioc_intr_disable(wmixer->irq, wmixer->vioc_intr->id, wmixer->vioc_intr->bits);
        ...
        if(VIOC_CONFIG_DMAPath_Support()) {
            ...
            if((component_num >= VIOC_MC0) && (component_num <= (VIOC_MC0 + VIOC_MC_MAX))) {
                ...
                tca_map_convter_swreset(VIOC_MC0 + gMC_NUM);
                VIOC_CONFIG_DMAPath_Set(wmixer->rdma0.id, wmixer->rdma0.id);
            }
            #endif
            #ifdef CONFIG_VIOC_DTRC_DECOMP
...
static int wmixer_drv_open(struct inode *inode, struct file *filp)
{
    ...
    dprintk("%s_open IN:  %d'th, block(%d/%d), cmd(%d), irq(%d :: %d/0x%x). \n", wmixer->misc->name, wmixer->data->dev_opened,              \
            wmixer->data->block_operating, wmixer->data->block_waiting, wmixer->data->cmd_count, wmixer->data->irq_reged, wmixer->vioc_intr->id, wmixer->vioc_intr->bits);
...
#ifdef CONFIG_COMPAT
    .compat_ioctl   = wmixer_drv_compat_ioctl, // for 32 bit OMX ioctl
#endif
...
static int wmixer_drv_probe(struct platform_device *pdev)
{
    ...
    if(dev_np) {
        if(!of_property_read_u32_index(pdev->dev.of_node, "rdmas", 1, &index)) {
    ...
    dev_np = of_parse_phandle(pdev->dev.of_node, "rdmas", 0);
    if(dev_np) {
        if(!of_property_read_u32_index(pdev->dev.of_node, "rdmas", 2, &index)) {
    ...
    if(dev_np) {
        ...
        wmixer->irq = irq_of_parse_and_map(dev_np, get_vioc_index(index));
