...
#include <video/tcc/tcc_mem_ioctl.h>
	...
	defined(CONFIG_ARCH_TCC570X) || defined(CONFIG_ARCH_TCC802X)
...
#if 0
#define dprintk(msg...)	pr_info("[DBG][SCALER] " msg)
#else
#define dprintk(msg...)
#endif

#if 0
#define plug_debug(msg...) pr_info("[DBG][SCALER] " msg)
#else
...
struct vioc_filter2d_block_res_type {
	...
	// after Mixer(2 : before scaler,  3 : after scaler)
	unsigned int path;
	unsigned int level;
};
	...
	struct file *filp, struct vm_area_struct *vma)
{
	if (range_is_allowed(vma->vm_pgoff,
		...
		pr_err("[ERR][SCALER] %s(): This address is not allowed.\n",
			__func__);
		return -EAGAIN;
	}
	...
	if (remap_pfn_range(vma, vma->vm_start, vma->vm_pgoff,
		...
		pr_err("[ERR][SCALER] %s(): Virtual address page port error.\n",
			__func__);
		return -EAGAIN;
	}
...
static int tcc_scaler_drv_common_register_interrupt(
	struct scaler_common_drv_type *scaler, unsigned int bOn)
{
	dprintk("%s INTR ID:%d bit :%d\n",
		__func__, scaler->vioc_intr->id, scaler->vioc_intr->bits);
	synchronize_irq(scaler->irq);
	vioc_intr_clear(scaler->vioc_intr->id, scaler->vioc_intr->bits);

	if (bOn)
		vioc_intr_enable(scaler->irq,
			scaler->vioc_intr->id, scaler->vioc_intr->bits);

	return 0;
}

int tcc_scaler_drv_common_set_path_resource(
	...
	struct tc_scaler_path_set *vioc_resorce)
{
	if (scaler->sc.id >= VIOC_SCALER) {
		...
		pr_info("[INF][SCALER] SC(0x%x) Plugged-Out\n", scaler->sc.id);
	} else {
		pr_info("[INF][SCALER] SC(0x%x) No Plugged-Out\n",
			scaler->sc.id);
	}
	...
	if (vioc_resorce->afbc_dec_need) {
		scaler->afbc_dec.id =
			VIOC_AFBCDEC0 + vioc_resorce->afbc_dec_num;
		scaler->afbc_dec.reg =
			VIOC_AFBCDec_GetAddress(scaler->afbc_dec.id);
		VIOC_CONFIG_AFBCDECPath(
			scaler->afbc_dec.id, scaler->rdma.id, 1);
	} else {
		scaler->afbc_dec.reg = NULL;
	}
#endif
	...
	if ((scaler->pixel_mapper_n >= VIOC_PIXELMAP) &&
		(scaler->pixel_mapper_n < (VIOC_PIXELMAP + VIOC_PIXELMAP_MAX)))
		VIOC_CONFIG_PlugOut(scaler->pixel_mapper_n);

	scaler->pixel_mapper_n = -1;

	if (vioc_resorce->pixel_mapper_n >= 0)
		scaler->pixel_mapper_n =
			VIOC_PIXELMAP + vioc_resorce->pixel_mapper_n;
#endif
	...
	if (vioc_resorce->wdma_num < 5)
		...
		scaler->wmix.id =
			VIOC_WMIX + 5 + (vioc_resorce->wdma_num - 5)/2;
	...
	pr_info("[INF][SCALER] Comp_Num:: rdma:0x%x wmixer:0x%x wdma:0x%x sc:0x%x sc_path:%s mixer_bypass(%d)\n ",
			scaler->rdma.id, scaler->wmix.id, scaler->wdma.id,
			scaler->sc.id, scaler->sc.path == 0 ?
			"front" : "rear", scaler->sc.mixer_bypass);
	dprintk("Register:: rdma:0x%p wmixer:0x%p wdma:0x%p sc:0x%p\n ",
		scaler->rdma.reg, scaler->wmix.reg,
		scaler->wdma.reg, scaler->sc.reg);
	pr_info("[INF][SCALER] INTR ID:%d bit :%d\n ",
		scaler->vioc_intr->id, scaler->vioc_intr->bits);
...
int tcc_scaler_drv_common_set_filter2d(struct scaler_common_drv_type *scaler,
	struct tc_scaler_filter2D_set *filter_resorce)
{
#if defined(CONFIG_VIOC_2D_FILTER)
	if (filter_resorce->filter2d_need) {
		plug_debug("%s en : %d num:%d  pos:%d  level:%d\n",
			__func__, filter_resorce->filter2d_need,
			filter_resorce->filter2d_num,
			filter_resorce->filter2d_pos,
			filter_resorce->filter2d_level);
		scaler->filter2d.id =
			VIOC_F2D + filter_resorce->filter2d_num;
		scaler->filter2d.reg =
			VIOC_Filter2D_GetAddress(scaler->filter2d.id);
		scaler->filter2d.path =
			filter_resorce->filter2d_pos;
		scaler->filter2d.level =
			filter_resorce->filter2d_level;
	} else {
		scaler->filter2d.id =
			VIOC_F2D + filter_resorce->filter2d_num;
...
static int tcc_scaler_drv_common_set_path_selection(
	struct scaler_common_drv_type *scaler)
{
	...
	if (VIOC_CONFIG_DMAPath_Support()) {
		...
		if (scaler->rdma.id == VIOC_RDMA02 ||
			scaler->rdma.id == VIOC_RDMA03 ||
			scaler->rdma.id == VIOC_RDMA07 ||
			scaler->rdma.id >= VIOC_RDMA11)
			component_num =
		...
		if (component_num < 0)
			pr_info("[INF][SCALER] %s: RDMA :0x%x dma path selection none\n",
	...
	unsigned int fmt, unsigned int width, unsigned int height)
{
	...
	VIOC_AFBCDec_SurfaceCfg(pAFBC_Dec, base_addr,
		fmt, width, height, 0, bSplitMode,
		bWideMode, VIOC_AFBCDEC_SURFACE_0);
	VIOC_AFBCDec_SetContiDecEnable(pAFBC_Dec, 0);
	VIOC_AFBCDec_SetSurfaceN(pAFBC_Dec, VIOC_AFBCDEC_SURFACE_0, 1);
	VIOC_AFBCDec_SetIrqMask(pAFBC_Dec, 0, AFBCDEC_IRQ_ALL);
...
static int tcc_component_plug_in_set(struct scaler_common_drv_type *scaler)
{
	...
	if (scaler->pixel_mapper_n > 0) {
		...
		pr_info(
			"[INF][SCALER] pixel mapper 0x%x  plug in to RDMA :0x%x\n",
			scaler->pixel_mapper_n, scaler->rdma.id);

		if (support_plug >= 0)
			ret = VIOC_CONFIG_PlugIn(
				(unsigned int)scaler->pixel_mapper_n,
				scaler->rdma.id);
	}
	#endif//
	...
	if (scaler->filter2d.reg != NULL) {
		...
		if (scaler->filter2d.path == 0)
			ret = VIOC_CONFIG_FILTER2D_PlugIn(
				scaler->filter2d.id, scaler->rdma.id, 0);
		else if (scaler->filter2d.path == 1)
			ret = VIOC_CONFIG_FILTER2D_PlugIn(
				scaler->filter2d.id, scaler->rdma.id, 1);
		else if (scaler->filter2d.path == 2)
			ret = VIOC_CONFIG_FILTER2D_PlugIn(
				scaler->filter2d.id, scaler->wdma.id, 0);
		else
			ret = VIOC_CONFIG_FILTER2D_PlugIn(
				scaler->filter2d.id, scaler->wdma.id, 1);

		if (ret >= 0) {
			if (scaler->filter2d.level <= STRENGTH_0) {
				strength = scaler->filter2d.level;
				filt2d_mode(
					scaler->filter2d.id, 1, 1,
					1, 0, 0, 0, 0, 0, 0);
				filt2d_coeff_hpf(
					scaler->filter2d.id,
				...
				strength =
					...
					% (SMODE_02 + 1));
				filt2d_coeff_set(
					scaler->filter2d.id, strength);
				filt2d_mode(scaler->filter2d.id,
					0, 0, 0, 0, 0, 0, 1, 1, 1);
			}
	...
	#endif//
	if (ret < 0)
		pr_err("%s :  dma: %x  pixel:%x ",
			__func__, scaler->rdma.id, scaler->pixel_mapper_n);
...
// vioc component plut in setting without scaler.
	...
	dprintk(
		"src   add : 0x%x 0x%x 0x%x, fmt :0x%x IMG:(%d %d)%d %d %d %d    interlace: %d\n",
		scaler->info->src_Yaddr,
		scaler->info->src_Uaddr,
		scaler->info->src_Vaddr,
		scaler->info->src_fmt,
	...
	dprintk(
		"  dest  add : 0x%x 0x%x 0x%x, fmt :0x%x IMG:(%d %d)%d %d %d %d\n",
		scaler->info->dest_Yaddr,
	...
	#endif//
	{
		...
		if (scaler->info->src_fmt >= VIOC_IMG_FMT_444SEP) {
			// address limitation!!
			scaler->info->src_winLeft =
				(scaler->info->src_winLeft >> 3) << 3;
			scaler->info->src_winRight =
				scaler->info->src_winLeft + crop_width;
			scaler->info->src_winRight =
				scaler->info->src_winLeft +
				(scaler->info->src_winRight -
				scaler->info->src_winLeft);
			tccxxx_GetAddress(scaler->info->src_fmt,
	...
	if (scaler->sc.mixer_bypass) {
		bypass_mixer = 1;
		if (VIOC_CONFIG_WMIXPath(scaler->rdma.id, 0) < 0)
			bypass_mixer = 0;
	} else {
		//VIOC_CONFIG_WMIXPath(scaler->rdma.id, 1);
		bypass_mixer = 0;
	...
	if (scaler->info->wdma_aligned_offset)
		VIOC_WDMA_SetImageOffset_withYV12(pSC_WDMABase,
		scaler->info->dest_ImgWidth);
	else
		VIOC_WDMA_SetImageOffset(
			pSC_WDMABase, scaler->info->dest_fmt,
			scaler->info->dest_ImgWidth);
	#else
...
#if defined(CONFIG_VIOC_AFBCDEC)
	tcc_scaler_drv_configure_AFBCDEC(
		scaler->afbc_dec.reg, scaler->info->src_Yaddr,
	...
	VIOC_WDMA_SetIreqStatus(pSC_WDMABase, 0xFFFFFFFF);
...
{
	struct scaler_common_drv_type *scaler =
		(struct scaler_common_drv_type *)client_data;

	if (is_vioc_intr_activatied(
		scaler->vioc_intr->id, scaler->vioc_intr->bits) == false)
		return IRQ_NONE;
...
static long scaler_drv_common_ioctl(
	...
	case TCC_SCALER_PATH:
		{
			...
			if (copy_from_user(&sc_drv_res,
				(struct tc_scaler_path_set *)arg,
				sizeof(struct tc_scaler_path_set))) {
				dprintk(KERN_ALERT
					"%s(): Error copy_from_user(%d)\n",
					__func__, cmd);
				ret = -EFAULT;
			}
			...
			if (ret >= 0) {
				tcc_scaler_drv_common_set_path_resource(
					scaler, &sc_drv_res);
				tcc_scaler_drv_common_register_interrupt(
					scaler, 1);
	...
	case TCC_SCALER_FILTER2D:
		{
			...
			if (copy_from_user(
				&sc_drv_filter_res,
				(struct tc_scaler_filter2D_set *)arg,
				sizeof(struct tc_scaler_filter2D_set))) {
				dprintk(KERN_ALERT
					"%s(): Error copy_from_user(%d)\n",
					__func__, cmd);
				ret = -EFAULT;
			...
			if (ret >= 0) {
				tcc_scaler_drv_common_set_filter2d(
					scaler, &sc_drv_filter_res);
				scaler->data->cmd_count = 0;
			}
			mutex_unlock(&scaler->data->io_mutex);
		}
	...
	case TCC_SCALER_IOCTRL_KERENL:
		...
		if (scaler->data->block_operating) {
			...
			if (ret <= 0) {
				scaler->data->block_operating = 0;
				dprintk(
					"%s(%d): timed_out block_operation(%d), cmd_count(%d)\n",
					__func__, ret,
		...
		if (cmd == TCC_SCALER_IOCTRL_KERENL) {
			memcpy(scaler->info, (SCALER_TYPE *)arg,
				sizeof(SCALER_TYPE));
		} else {
			if (copy_from_user(scaler->info,
				(SCALER_TYPE *)arg, sizeof(SCALER_TYPE))) {
		...
		if (ret >= 0) {
			if (scaler->data->block_operating >= 1) {
				pr_warn("[WAR][SCALER] %s(): block_operating(%d), block_waiting(%d), cmd_count(%d), poll_count(%d)\n",
					__func__, scaler->data->block_operating,
					scaler->data->block_waiting,
					scaler->data->cmd_count,
					scaler->data->poll_count);
...
#ifdef CONFIG_COMPAT
static long scaler_drv_common_compat_ioctl(
	struct file *file, unsigned int cmd, unsigned long arg)
{
	return scaler_drv_common_ioctl(
...
static int scaler_drv_common_release(struct inode *inode, struct file *filp)
{
	...
	if (scaler->data->dev_opened == 0) {
		...
		if (scaler->afbc_dec.reg) {
			VIOC_AFBCDec_TurnOFF(scaler->afbc_dec.reg);
			VIOC_CONFIG_AFBCDECPath(scaler->afbc_dec.id,
				scaler->rdma.id, 0);
			scaler->afbc_dec.reg = NULL;
		}
	#endif
...
static int scaler_drv_common_open(struct inode *inode, struct file *filp)
{
	...
	pr_info("[INF][SCALER] %s(): In -open(%d), block(%d), wait(%d), cmd(%d), irq(%d)\n",
		__func__, scaler->data->dev_opened,
		scaler->data->block_operating,
		scaler->data->block_waiting, scaler->data->cmd_count,
		scaler->data->irq_reged);
	...
	if (!scaler->data->irq_reged) {
		...
		ret = request_irq(scaler->irq, scaler_drv_common_handler,
			IRQF_SHARED, scaler->misc->name, scaler);
		if (ret) {
			if (scaler->clk)
				clk_disable_unprepare(scaler->clk);
			pr_err("[ERR][SCALER] (ret %d) to aquire %s request_irq(%d)\n",
	...
	pr_info("[INF][SCALER] %s(): Out - open(%d)\n",
		__func__, scaler->data->dev_opened);
	return ret;
}
...
static int scaler_drv_common_probe(struct platform_device *pdev)
{
	...
	pr_info("[INF][SCALER] %s: Scaler Driver Initialized\n", pdev->name);
...
err_misc_alloc:
	...
	pr_info("[INF][SCALER] %s: %s: err ret:%d\n",
...
{
	struct scaler_common_drv_type *scaler =
		(struct scaler_common_drv_type *)platform_get_drvdata(pdev);
...
static int scaler_drv_common_suspend(
