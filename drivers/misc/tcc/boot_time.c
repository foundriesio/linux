...
static void __iomem *timer_tc32mcnt_addr;
...
static long boot_time_ioctl(struct file *flip, unsigned int cmd,
			    unsigned long arg);
...
static int boot_time_open(struct inode *inode, struct file *file)
{
	pr_warn("Driver Open Function Called...!!!\n");
	return 0;
}

static int boot_time_release(struct inode *inode, struct file *file)
{
	pr_warn("Driver Release Function Called...!!!\n");
	return 0;
}
...
#define BOOT_STAMP_DESCRIPTION \
	_IOWR(BOOT_TIME_IOC_MAGIC, 1, struct boot_time)
#define BOOT_STAMP_DESCRIPTION_ALL \
	_IOWR(BOOT_TIME_IOC_MAGIC, 2, struct boot_time)
#define BOOT_TIME \
	_IOWR(BOOT_TIME_IOC_MAGIC, 3, struct boot_time)
#define GET_BOOT_STAMP_NUM \
	_IOR(BOOT_TIME_IOC_MAGIC, 4, struct boot_time)
#define GET_CURRENT_TIME_VAL \
	_IOR(BOOT_TIME_IOC_MAGIC, 5, struct boot_time)
#define GET_INITCALL_DONE \
	_IOR(BOOT_TIME_IOC_MAGIC, 6, struct boot_time)
			    ...
			    unsigned long arg)
{
	...
	case GET_BOOT_STAMP_NUM:
		...
		boot_time_data.boot_stamp_desc_num =
...
#else
		arm_smccc_smc(smc_time_val, (unsigned long)START_MP_INDEX, 0, 0,
			      0, 0, 0, 0, &res);
	...
	default:
		pr_err("ICTC:default boot time ioctl\n");
...
static int boot_time_show(struct seq_file *m, void *v)
{
	...
	for (i = 0; i < cnt; i++) {
		arm_smccc_smc(0x82007004, (unsigned long)i, 0, 0, 0, 0, 0, 0,
			      &res);
		seq_printf(m, "%s : %d\n", boot_stamp_desc[i],
			   (uint32_t) res.a1);
	}
...
static int boot_time_procfs_init(void)
{
	...
	ret = proc_create("boot_time", 0444, 0, &boot_time_fops);
	if (ret == NULL) {
		return -ENOMEM;
	}
...
static int boot_time_probe(struct platform_device *pdev)
{
	...
	property_temp =
	    of_get_property(pdev->dev.of_node, "smc-time-val", NULL);
	smc_time_val = be32_to_cpup(property_temp);
	property_temp =
	    of_get_property(pdev->dev.of_node, "smc-time-num", NULL);
	...
	boot_time_dev_class = class_create(THIS_MODULE, "Boot_Time_class");

	if (boot_time_dev_class == NULL) {
		pr_warn("Cannot create the struct class\n");
		goto r_class;
	}

	if ((device_create
	     (boot_time_dev_class, NULL, boot_time_devt, NULL,
	      "boot_time_device")) == NULL) {
		pr_warn("Cannot create the Device 1\n");
		goto r_device;
	}
	pr_warn("Device Driver Insert...Done!!!\n");
	return 0;
