...
dma_addr_t get_current_dma_period_base_addr(struct tcc_sdr_t *sdr, uint32_t sdr_port) 
{
	dma_addr_t addr;
	...
	addr = (sdr_port == 0) ? readl(sdr->adma_reg+ADMA_RxDaCdar) :
		...
		(sdr_port == 3) ? readl(sdr->adma_reg+ADMA_RxDaCar3) : (dma_addr_t)NULL;
...
static unsigned int tcc_i2s_get_mclk_fs(unsigned int freq, struct clk   *dai_pclk)
{
	...
	while((fs>=MIN_TCC_MCLK_FS)&&(fs<=MAX_TCC_MCLK_FS)){
		if((fs<MIN_TCC_MCLK_FS)||(fs>MAX_TCC_MCLK_FS))break;
		temp = fs*freq;
...
static unsigned int tcc_i2s_set_clock(struct tcc_sdr_t *sdr, unsigned int req_lrck)
{
	...
	if((sdr->dai_clk_rate[0] != req_lrck)
		...
		if(sdr->dai_pclk) {
			...
			SDR_DBG("CKC_CHECK: set_rate[%d], get_rate[%lu], fs[%d]\n", (req_lrck*temp), clk_get_rate(sdr->dai_pclk), temp);

			dai_damr &= ~(DAMR_BITCLK_DIV_MASK | DAMR_FRMCLK_DIV_MASK);
			switch(temp) {
				case 256:
					dai_damr |= (DAMR_FRMCLK_DIV_64);
					break;
				case 512:
					dai_damr |= (DAMR_BITCLK_DIV_8|DAMR_FRMCLK_DIV_64);
					break;
				case 1024:
					dai_damr |= (DAMR_BITCLK_DIV_16|DAMR_FRMCLK_DIV_64);
					break;
				default:
					break;
			}
...
int tcc_sdr_set_param(struct tcc_sdr_t *sdr, HS_I2S_PARAM *p)
{
	...
	if(p->eRadioMode) {
		...
		sdr->bitmode = (p->eBitMode == 16) ? DRMR_BITMODE_16BIT :
			...
			(p->eBitMode == 80) ? DRMR_BITMODE_80BIT : DRMR_BITMODE_20BIT;
		sdr->bit_polarity = (p->eBitPolarity == SDR_BIT_POLARITY_POSITIVE_EDGE) ? 0 : DAMR_BIT_POLARITY;
	} else {	//I2S Slave Mode
		sdr->radio_mode = 0;
		sdr->ports_num = p->eChannel;
		sdr->bitmode = (p->eBitMode == 16) ? DAMR_RXS_LSB_16BIT : DAMR_RXS_LSB_24BIT;
...
void tcc_sdr_rx_isr(struct tcc_sdr_t *sdr)
{
	...
	for (i=0; i<sdr->ports_num; i++) {
		if (Overrun[i]) { // Overrun
			if (sdr->opened && sdr->started) {
				SDR_DBG("%s - Overrun(%d), new_read_pos:0x%x, dma_sz:0x%x, valid_sz(%p):0x%x\n", 
						__func__, i, sdr->port[i].read_pos, sdr->port[i].dma_sz, &sdr->port[i].valid_sz, sdr->port[i].valid_sz);
...
static void set_radio_dma_inbuffer(struct tcc_sdr_t *sdr, uint32_t length, gfp_t gfp)
{
	...
	if(sdr->radio_mode) {
		...
		writel(((sdr->period_bytes*sdr->ports_num)>>6)-1, sdr->adma_reg+ADMA_RxDaTCnt);
	} else {	
		if(sdr->adrcnt_mode) {
		...
		writel(((sdr->period_bytes)>>5)-1, sdr->adma_reg+ADMA_RxDaTCnt);
	}
	...
	for (i=0; i<sdr->ports_num; i++) {
		...
		SDR_DBG("port(%d) vaddr: %p, paddr: %p\n", 
				i, (void*)sdr->port[i].dma_vaddr, (void*)sdr->port[i].dma_paddr);
	}

	SDR_DBG("%s - HwRxDaParam [0x%X]\n", __func__, readl(sdr->adma_reg+ADMA_RxDaParam));
	SDR_DBG("%s - HwRxDaTCnt [%d]\n", __func__, readl(sdr->adma_reg+ADMA_RxDaTCnt));
	...
	regval |= (ADMA_TRANS_DAI_RX_WSIZE_32 | ADMA_TRANS_RX_CONTINUOUS_MODE_EN);
...
int tcc_sdr_initialize(struct tcc_sdr_t *sdr, gfp_t gfp)
{
	...
	adma_chctrl &= ~(ADMA_CHCTRL_DAI_RX_MULTI_MASK|ADMA_CHCTRL_DAI_RX_MULTI_CH_EN);
	//ADMA Bit Mode Setting for I2S Mode
	if(!sdr->radio_mode) {
		sdr->bitmode = sdr->bitmode & DAMR_RXS_MASK;
		if((sdr->bitmode == DAMR_RXS_LSB_16BIT)||(sdr->bitmode == DAMR_RXS_MSB_16BIT)) {
			//16bit mode
	...
	if(sdr->radio_mode) {
		if(sdr->ports_num == 1) {  //1port mode
			...
			dai_mccr0 |= (MCCR0_DAO1_DISABLE|MCCR0_DAO2_DISABLE|MCCR0_DAO3_DISABLE);
		...
		if(sdr->ports_num == 1) {  //mono channel
			...
			dai_damr |= DAMR_MULTI_PORT_EN;
			dai_mccr0 &= ~MCCR0_DAO_DISABLE_MASK;
			adma_chctrl |= (ADMA_CHCTRL_DAI_RX_MULTI_CH_EN|ADMA_CHCTRL_DAI_RX_MULTI_7_1);
		} else {
...
#ifdef PREALLOCATE_DMA_BUFFER_MODE 
	sdr->dma_total_size = TCC_SDR_BUF_SZ_MAX * (SDR_MAX_PORT_NUM+1); // plus 1 for align 
	SDR_DBG("dma_total_size : %d\n", sdr->dma_total_size);
