...
const char *SiTraceConfiguration   (const char *config) {
    ...
    char *array[50];
    char *pBuffer;
	#if defined(LINUX_ST_SDK2_I2C) || defined(TCC_KERNEL_MODULE)
	char *config_string;
	#endif
...
STCHIP_Error_t SDK2_I2cRead(STCHIP_Handle_t hChip, u_int8_t ChipAddress, unsigned char *Data, int NbData)
{
	...
	do
	{
		...
		if(ret != MsgIndex && retry_count == 2)
		{
			...
			{
				pr_err("%s:WR msg[%d] addr = 0x%x len = %d Err= %d\n", __func__, msg_cnt, msg[msg_cnt].addr, msg[msg_cnt].len, ret);
			}
...
void    L0_Init              (L0_Context *i2c) {
  ...
  i2c->address             = 0;
  i2c->indexSize           = 0;
  i2c->connectionType      = CUSTOMER; /*Default: SIMU*/
  i2c->trackWrite          = 0;
  i2c->trackRead           = 0;
	...
	case CUSTOMER:
		{
			#ifdef TCC_KERNEL_MODULE
			{
				unsigned char uc_addr = i2c->address >> 1;
				
				if(tcc_i2c_read(uc_addr, iNbBytes, pucDataBuffer) < 0)
				{
					nbReadBytes = 0;
					break;
				}
	...
	case CUSTOMER:
		{
			...
			#ifdef TCC_KERNEL_MODULE
			{
				...
				if(tcc_i2c_write(uc_addr, iNbBytes + i2c->indexSize, pucBuffer) < 0)
				{
					write_error++;
					break;
				}
