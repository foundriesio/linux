...
void MxL532C_Tune(UINT32 u32TunerFreq, UINT8 u8BandWidth)
{
...
UINT32 MxL532C_GetSNR(void)
{
/*
	MXL_DEMOD_SNR_INFO_T snr;
	// Read back SNR
    MxLWare_API_GetDemodStatus(MXL_DEMOD_SNR_REQ, &snr);
  //  MXL_MSB(printf("MxL532C : Demod SNR = %d dB\n", snr.SNR)); */
	return 0;//snr.SNR/10000;
	
}

MXL_BOOL_E MxL532C_GetLock(void)
{
/*
	MXL_DEMOD_LOCK_STATUS_T rsLockStatus;
	// Read back RS Lock Status
    MxLWare_API_GetDemodStatus(MXL_DEMOD_RS_LOCK_REQ, &rsLockStatus);
    MXL_MSB(printf("MxL532C : RS Lock Status = 0x%x\n", rsLockStatus.Status)); */
	return 0; //rsLockStatus.Status;
}

UINT32 MxL532C_GetBER(void)
{
/*
	MXL_DEMOD_BER_INFO_T ber;
	// Read back BER
    MxLWare_API_GetDemodStatus(MXL_DEMOD_BER_REQ, &ber);
  //  MXL_MSB(printf("MxL532C : Demod BER = %d\n", ber.BER)); */
	return 0; //ber.BER;
}

SINT32 MxL532C_GetSigStrength(void)
{
/*
	MXL_SIGNAL_STATS_T sigStrength;
	// Read back Signal Strength
    MxLWare_API_GetTunerStatus(MXL_TUNER_SIGNAL_STRENGTH_REQ, &sigStrength);
 //   MXL_MSB(printf("MxL532C : Signal Strength = %d dBm\n", sigStrength.SignalStrength)); */
	return 0; //sigStrength.SignalStrength;
}

UINT16 MxL532C_GetTPSCellID(void)
{
/*
	MXL_DEMOD_CELL_ID_INFO_T tpsCellIdInfo;
    // Read back TPS Cell ID
    MxLWare_API_GetDemodStatus(MXL_DEMOD_TPS_CELL_ID_REQ, &tpsCellIdInfo);
    MXL_MSB(printf("MxL532C : TPS Cell ID = %04X\n", tpsCellIdInfo.TpsCellId));	*/
	return 0; //tpsCellIdInfo.TpsCellId;
}

void MxL532C_PowerOnOff(UINT8 u8PowerOn)
{
/*
	MXL_CMD_TYPE_E CmdType = MXL_DEV_532C_POWER_MODE_CFG;
	...
	if(u8PowerOn==0)
	{
		...
		{
			printf("MxL532C_PowerOff failed\n");
		}
		else
		{
			printf("MxL532C_PowerOff ok\n");
		}
	}
	*/
}
