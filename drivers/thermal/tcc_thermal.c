...
#endif
#if defined(CONFIG_ARCH_TCC805X)
static int temp_to_code(struct cal_thermal_data *data, unsigned int temp_trim1,
		unsigned int temp_trim2, int temp)
{
	...
	if (temp >= 25) {
		...
		temp_code = ((((temp - 25)*((570+(60))/65)*
			     ((int)temp_trim2 - (int)temp_trim1))/10) /
			     (85 - 25)) + (int)temp_trim1;
	} else {
	/**/
	/**/
...
static int code_to_temp(struct cal_thermal_data *data, unsigned int temp_trim1,
		unsigned int temp_trim2, int temp_code)
{
	/*Always apply two point calibration*/
	int temp;
	...
	if (temp_code >= 1596) { //temp_code < data->otp_data
		...
		temp = (((temp_code - (int)temp_trim1) * (85 - 25) /
			((int)temp_trim2 - (int)temp_trim1))*
			(65/(57+(6)))) + 25; //(65/57+d_otp_sl)
	} else {
	/**/
	/**/
	}
...
static int tcc_thermal_read(struct tcc_thermal_data *data)
{
	...
	switch (tag) {
	case 0:
		celsius_temp = code_to_temp(data->cal_data,
			data->temp_trim1,
			data->temp_trim2,
...
static ssize_t temp_mode_write(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t len)
{
	...
	if (kstrtoint(buf, 10, &value) != 0) {
		pr_err("[TSENSOR] Control mode change - Input Value False\n");
	} else {
		if ((value != (uint32_t)1) && (value != (uint32_t)0)) {
			pr_err("[TSENSOR] Control mode change - Input Value False\n");
		} else {
			writel(0, data->enable);
			control_data = readl_relaxed(data->control);
			writel(((control_data & ~((uint32_t)0x1)) | value),
			       data->control);
			writel(1, data->enable);
		}
	}

	if ((readl_relaxed(data->control) & (uint32_t)0x1) == (unsigned int)0) {
		pr_info("[TSENSOR] Control mode - One-shot mode\n");
		/**/
	} else {
		pr_info("[TSENSOR] Control mode - Continuous mode\n");
		/**/
	}
...
static ssize_t temp_irq_en_write(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t len)
{
	struct tcc_thermal_data *data = dev_get_drvdata(dev);
	...
	if (kstrtoint(buf, 16, &value) != 0) {
		pr_err("[TSENSOR] IRQ Enable setting fail\n");
		/**/
	} else {
		if (value > (unsigned int)0x777777) {
			pr_err("[TSENSOR] IRQ Enable setting fail\n");
		/**/
		} else {
			writel(0, data->enable);
...
static ssize_t temp_probe_write(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t len)
{
	struct tcc_thermal_data *data = dev_get_drvdata(dev);
	int value = 0;
	...
	if (kstrtoint(buf, 10, &value) != 0) {
		/**/
		pr_err("[TSENSOR] Probe select false\n");
	} else {
		if (value > 0x3F) {
			pr_err("[TSENSOR] Probe select false\n");
			/**/
 ...
 */
	...
	pr_info("irq occur 0x%x\n", reg);

	writel((1 << 20) | (1 << 16) | (1 << 12) | (1 << 8) | (1 << 4) | 1,
		data->interrupt_clear); // clear irq
	return IRQ_HANDLED;
}
#endif
#endif
static int tcc_thermal_init(const struct tcc_thermal_data *data)
{
	...
	threshold_high_temp[0] = temp_to_code(data->cal_data,
				data->temp_trim1,
				data->temp_trim2,
				data->pdata->threshold_high_temp);
	threshold_high_temp[1] = temp_to_code(data->cal_data,
				data->probe0_temp_trim1,
				data->probe0_temp_trim2,
				data->pdata->threshold_high_temp);
	threshold_high_temp[2] = temp_to_code(data->cal_data,
				data->probe1_temp_trim1,
				data->probe1_temp_trim2,
				data->pdata->threshold_high_temp);
	threshold_high_temp[3] = temp_to_code(data->cal_data,
				data->probe2_temp_trim1,
				data->probe2_temp_trim2,
				data->pdata->threshold_high_temp);
	threshold_high_temp[4] = temp_to_code(data->cal_data,
				data->probe3_temp_trim1,
				data->probe3_temp_trim2,
				data->pdata->threshold_high_temp);
	threshold_high_temp[5] = temp_to_code(data->cal_data,
				data->probe4_temp_trim1,
				data->probe4_temp_trim2,
				data->pdata->threshold_high_temp);
	...
	threshold_low_temp[0] = temp_to_code(data->cal_data,
				data->temp_trim1,
				data->temp_trim2,
				data->pdata->threshold_low_temp);
	threshold_low_temp[1] = temp_to_code(data->cal_data,
				data->probe0_temp_trim1,
				data->probe0_temp_trim2,
				data->pdata->threshold_low_temp);
	threshold_low_temp[2] = temp_to_code(data->cal_data,
				data->probe1_temp_trim1,
				data->probe1_temp_trim2,
				data->pdata->threshold_low_temp);
	threshold_low_temp[3] = temp_to_code(data->cal_data,
				data->probe2_temp_trim1,
				data->probe2_temp_trim2,
				data->pdata->threshold_low_temp);
	threshold_low_temp[4] = temp_to_code(data->cal_data,
				data->probe3_temp_trim1,
				data->probe3_temp_trim2,
				data->pdata->threshold_low_temp);
	threshold_low_temp[5] = temp_to_code(data->cal_data,
				data->probe4_temp_trim1,
				data->probe4_temp_trim2,
				data->pdata->threshold_low_temp);
...
static unsigned long thermal_otp_read(struct tcc_thermal_data *data, int probe)
{
	...
	switch (probe) {
	case 0:
		arm_smccc_smc((unsigned long)0x5004, (unsigned long)0x3210,
			(unsigned long)sizeof(unsigned int), buf,
			0, 0, 0, 0, &res);
		data->temp_trim1 =
			((unsigned int)buf & (unsigned int)0x3FF);
		data->temp_trim2 =
			((unsigned int)buf >> (unsigned int)16) &
						(unsigned int)0x3FF;
		break;
	case 1:
		arm_smccc_smc((unsigned long)0x5004, (unsigned long)0x3214,
			(unsigned long)sizeof(unsigned int), buf,
			0, 0, 0, 0, &res);
			data->probe0_temp_trim1 =
				((unsigned int)buf & (unsigned int)0x3FF);
		...
		break;
	case 2:
		arm_smccc_smc((unsigned long)0x5004, (unsigned long)0x3218,
			(unsigned long)sizeof(unsigned int), buf,
			0, 0, 0, 0, &res);
		data->probe1_temp_trim1 =
			((unsigned int)buf & (unsigned int)0x3FF);
		...
		break;
	case 3:
		arm_smccc_smc((unsigned long)0x5004, (unsigned long)0x321C,
			(unsigned long)sizeof(unsigned int), buf,
			0, 0, 0, 0, &res);
		data->probe2_temp_trim1 =
			((unsigned int)buf & (unsigned int)0x3FF);
		...
		break;
	case 4:
		arm_smccc_smc((unsigned long)0x5004, (unsigned long)0x3220,
			(unsigned long)sizeof(unsigned int), buf,
			0, 0, 0, 0, &res);
		...
		break;
	case 5:
		arm_smccc_smc((unsigned long)0x5004, (unsigned long)0x3224,
			(unsigned long)sizeof(unsigned int), buf,
			0, 0, 0, 0, &res);
		data->probe4_temp_trim1 =
			((unsigned int)buf & (unsigned int)0x3FF);
		...
		break;
	case 6:
		arm_smccc_smc((unsigned long)0x5004, (unsigned long)0x3228,
			(unsigned long)sizeof(unsigned int), buf,
			0, 0, 0, 0, &res);

		data->ts_test_info = ((unsigned int)buf	&
							(unsigned int)0xFFFFFF);
		break;
	case 7:
		arm_smccc_smc((unsigned long)0x5004, (unsigned long)0x322C,
			(unsigned long)sizeof(unsigned int), buf,
			0, 0, 0, 0, &res);
		data->buf_slope_sel_ts  = (((unsigned int)buf >>
					(unsigned int)28) & (unsigned int)0xF);
		data->d_otp_slope	= (((unsigned int)buf >>
					(unsigned int)24) & (unsigned int)0xF);
		data->calib_sel		= (((unsigned int)buf >>
					(unsigned int)14) & (unsigned int)0x3);
		data->buf_vref_sel_ts	= (((unsigned int)buf >>
					(unsigned int)12) & (unsigned int)0x3);
		data->trim_bgr_ts	= (((unsigned int)buf >>
					(unsigned int)8) & (unsigned int)0xF);
		data->trim_vlsb_ts	= (((unsigned int)buf >>
					(unsigned int)4) & (unsigned int)0xF);
		data->trim_bjt_cur_ts	= ((unsigned int)buf &
					(unsigned int)0xF);
		break;
	default:
...
static void tcc_thermal_get_otp(struct platform_device *pdev)
{
	...
	if (buf == (unsigned long)0) {
		...
		pr_info("[INFO][T-SENSOR] otp read data index 8 : %lx\n",
			buf);
	}
...
static DEVICE_ATTR(tsensor_mode, 0644,
				temp_mode_read, temp_mode_write);
static DEVICE_ATTR(tsensor_probe_sel, 0644,
				temp_probe_read, temp_probe_write);
static DEVICE_ATTR(tsensor_irq_set, 0644,
				temp_irq_en_read, temp_irq_en_write);
