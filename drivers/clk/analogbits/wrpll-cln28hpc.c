 ...
 * configurations that are restricted by software or manufacturer
 * guidance to a small, pre-determined set of performance points.
 *
 * References:
 * - Analog Bits "Wide Range PLL Datasheet", version 2015.10.01
 * - Analog Bits "28nm PLL Product Brief" (2011):
 *   http://www.analogbits.com/pdf/28nm_PLL_Product_Brief.pdf
 ...
 * The macro type suffix is required to prevent warnings from the call
 * to min() below.
 */
#define MAX_DIVR_DIVISOR		64U
...
/**
 * wrpll_calc_filter_range() - determine PLL loop filter bandwidth
 ...
 */
static int wrpll_calc_filter_range(unsigned long post_divr_freq)
{
	u8 range;
...
/**
 * wrpll_calc_fbdiv() - return feedback fixed divide value
 ...
 *         is enabled.
 */
static u8 wrpll_calc_fbdiv(const struct analogbits_wrpll_cfg *c)
{
	return (c->flags & WRPLL_FLAGS_INT_FEEDBACK_MASK) ? 2 : 1;
}

/**
 * wrpll_calc_divq() - determine DIVQ based on target PLL output clock rate
 ...
 *         upon success.
 */
static u8 wrpll_calc_divq(u32 target_rate, u64 *vco_rate)
{
	u64 s;
	u8 divq = 0;
...
/**
 * wrpll_update_parent_rate() - update PLL data when parent rate changes
 ...
 */
static int wrpll_update_parent_rate(struct analogbits_wrpll_cfg *c,
				    unsigned long parent_rate)
{
	unsigned int max_r_for_parent;
	...
	c->max_r = min(MAX_DIVR_DIVISOR, max_r_for_parent);
...
/**
 * analogbits_wrpll_configure_for_rate() - compute PLL config for a target rate
					...
					unsigned long parent_rate)
{
	...
	if (parent_rate != c->parent_rate) {
		if (wrpll_update_parent_rate(c, parent_rate)) {
	...
	divq = wrpll_calc_divq(target_rate, &target_vco_rate);
	if (divq == 0)
		return -ERANGE;
	c->divq = divq;
	...
	c->range = wrpll_calc_filter_range(post_divr_freq);
 ...
 */
unsigned int analogbits_wrpll_calc_max_lock_us(const struct analogbits_wrpll_cfg *c)
{
	return MAX_LOCK_US;
}
