...
static u32		stClockSource[MAX_CLK_SRC];
...
#define tcc_pll_write(reg,en,p,m,s,src) { \
	if (en) { \
		...
		ckc_writel(ckc_readl(reg) | ((u32)(en&1)<<PLL_EN_SHIFT), reg);	\
		while((ckc_readl(reg)&(1<<PLL_LOCKST_SHIFT))==0); \
	} else \
		ckc_writel(ckc_readl(reg) & ~((u32)1<<PLL_EN_SHIFT), reg);\
}
...
#define tcc_pll_write_cb(reg,p,m,s,src) { \
	...
	ckc_writel(ckc_readl(reg) & (PLL_CHGPUMP_MASK << PLL_CHGPUMP_SHIFT)  \
		|(1<<PLL_LOCKEN_SHIFT) \
		|((src&PLL_SRC_MASK)<<PLL_SRC_SHIFT)|((s&PLL_S_MASK)<<PLL_S_SHIFT) \
		|((m&PLL_M_MA SK)<<PLL_M_SHIFT)|((p&PLL_P_MASK)<<PLL_P_SHIFT), reg); \
	for (i=100; i; i--);	\
	ckc_writel(ckc_readl(reg) | ((u32)(en&1)<<PLL_EN_SHIFT), reg); \
	while((ckc_readl(reg)&(1<<PLL_LOCKST_SHIFT)) == 0);	\
}
...
#define tcc_cpu_write(reg,cnt,sel) { \
	ckc_writel((ckc_readl(reg) & (~((CPU_SEL_MASK<<CPU_SEL_SHIFT)|(1<<CPU_MODE_SHIFT)))) \
			| ((sel&CPU_SEL_MASK)<<CPU_SEL_SHIFT), reg);	\
	while(ckc_readl(reg) & (1<<CPU_CHGRQ_SHIFT)); \
	ckc_writel(ckc_readl(reg) &(~(CPU_CNT_MASK)<<CPU_CNT_SHIFT)	\
			| ((cnt&CPU_CNT_MASK)<<CPU_CNT_SHIFT), reg);	\
	while(ckc_readl(reg) & (1<<CPU_CFGRQ_SHIFT)); \
}
...
#define tcc_clkctrl_cpu_write(reg,en,config,sel) { \
	ckc_writel((ckc_readl(reg) & (~(CLKCTRL_SEL_MASK<<CLKCTRL_SEL_SHIFT))) | ((sel&CLKCTRL_SEL_MASK)<<CLKCTRL_SEL_SHIFT), reg);	\
	while(ckc_readl(reg) & (1<<CLKCTRL_CHGRQ_SHIFT));	\
	ckc_writel((ckc_readl(reg) & (~(CLKCTRL_CPU_MASK<<CLKCTRL_CONFIG_SHIFT))) | ((config&CLKCTRL_CPU_MASK)<<CLKCTRL_CONFIG_SHIFT), reg);	\
	while(ckc_readl(reg) & (1<<CLKCTRL_CFGRQ_SHIFT));	\
	ckc_writel((ckc_readl(reg) & (~(1<<CLKCTRL_EN_SHIFT))) | ((en&1)<<CLKCTRL_EN_SHIFT), reg);	\
	while(ckc_readl(reg) & (1<<CLKCTRL_CFGRQ_SHIFT)); \
}
...
#define tcc_pclkctrl_write(reg,md,en,sel,div,type) { \
	...
	ckc_writel((ckc_readl(reg) & ~(div_mask<<PCLKCTRL_DIV_SHIFT)) | ((div&div_mask)<<PCLKCTRL_DIV_SHIFT), reg);	\
	ckc_writel((ckc_readl(reg) & ~(PCLKCTRL_SEL_MASK<<PCLKCTRL_SEL_SHIFT)) | ((sel&PCLKCTRL_SEL_MASK)<<PCLKCTRL_SEL_SHIFT), reg);	\
	ckc_writel((ckc_readl(reg) & ~(1<<PCLKCTRL_MD_SHIFT)) | ((md&1)<<PCLKCTRL_MD_SHIFT), reg);	\
	ckc_writel((ckc_readl(reg) & ~(1<<PCLKCTRL_EN_SHIFT)) | ((en&1)<<PCLKCTRL_EN_SHIFT), reg);	\
}
...
static inline void tcc_clkctrl_write(void __iomem *reg, u32 en, 
				     u32 config, u32 sel)
{
	u32 cur_config;
...
static inline int tcc_find_pms(tPMS *PLL, u32 srcfreq)
{
	u64	u64_pll, u64_src, fvco, srch_p, srch_m, u64_tmp;
	u32	srch_pll, err, srch_err;
	int	srch_s;
	...
	err = 0xFFFFFFFF;
	srch_err = 0xFFFFFFFF;
	for (srch_s = PLL_S_MAX, fvco = (u64_pll << PLL_S_MAX) ; 
			(srch_s >= PLL_S_MIN) ; 
			fvco = (u64_pll << (--srch_s))) {
		if ((fvco >= PLL_VCO_MIN) && (fvco <= PLL_VCO_MAX)) {
			for (srch_p = PLL_P_MIN ; 
					srch_p <= PLL_P_MAX ; srch_p++) {
				srch_m = fvco * srch_p;
	...
	u64_tmp = u64_src * (u64)PLL->m;
	do_div(u64_tmp, PLL->p);
	PLL->fpll = (u32)(u64_tmp >> (PLL->s));
	PLL->en = 1;
	return 0;
}
...
tcc_ckc_setpll_failed:
	tcc_pll_write(reg, 0, PLL_P_MIN, ((PLL_P_MIN * PLL_VCO_MIN) + 
				XIN_CLK_RATE) / XIN_CLK_RATE, PLL_S_MIN, src);
	tcc_ckc_reset_clock_source(id);
	return -1;
}
...
static unsigned long tcc_ckc_pll_get_rate(int id)
{
	...
	return (u32)((u64_tmp) >> nPLLCFG.s);
}

#define PLLDIV_MASK             0xff
#define PLLDIV_EN               0x80
#define PLLDIV_DIV              0x3f
#define PLLDIV_DIV_SHIFT        8

static int tcc_ckc_pll_div_set(int id, u32 div)
{
        void __iomem	*reg;
        u32		reg_values;
        u32		offset = 0;
	...
	reg_values = ckc_readl(reg) & ~(PLLDIV_MASK << offset);
	ckc_writel(reg_values, reg);

	if (div != 0)
		reg_values |= (PLLDIV_EN | (div & PLLDIV_DIV)) << offset;
	else
		reg_values |= 1 << offset;
...
static unsigned long tcc_ckc_plldiv_get_rate(int id)
{
	...
	if (pdiv == 0)	/* should not be zero */
		return 0;

	fpll = tcc_ckc_pll_get_rate(id);
	return (u32)fpll / (pdiv + 1);
}
...
{
	u32	i, div[MAX_CLK_SRC], err[MAX_CLK_SRC], searchsrc, clk_rate;
...
{
	u32	i, div[MAX_CLK_SRC], err[MAX_CLK_SRC], searchsrc, clk_rate, unit;
	searchsrc = 0xFFFFFFFF;
	...
	CLKCTRL->freq = div[searchsrc] * (stClockSource[searchsrc]
			/(CLKCTRL_CONFIG_MAX + 1));
	CLKCTRL->config = 0;
...
static int tcc_ckc_clkctrl_enable(int id)
{
	...
	while ((ckc_readl(reg) & (1 << CLKCTRL_CFGRQ_SHIFT)) != 0);
...
static int tcc_ckc_clkctrl_set_rate(int id, unsigned long rate)
{
	...
	switch (id) {
		...
		case FBUS_CPU1:
			nCLKCTRL.freq = rate;
			if (tcc_find_clkctrl_for_cpu1(&nCLKCTRL) != 0)
				return -1;
			nCLKCTRL.en = (ckc_readl(reg) & (1 << CLKCTRL_EN_SHIFT)) ? 1 : 0;
	...
	nCLKCTRL.freq = rate;
	if (tcc_find_clkctrl(&nCLKCTRL) != 0)
		return -1;
...
static inline u32 tcc_pclk_support_below_freq(u32 periname)
{
...
static inline u32 tcc_ckc_is_audio_clk(u32 periname)
{
...
static inline u32 tcc_ckc_pclk_divider(tPCLKCTRL *PCLKCTRL, u32 *div,
	const u32 src_CLK, u32 div_min, u32 div_max)
{
	u32	clk_rate1, clk_rate2, err1, err2;
	...
	if (tcc_pclk_support_below_freq(PCLKCTRL->periname) != 0) {
...
static inline u32 tcc_ckc_pclk_dco(tPCLKCTRL *PCLKCTRL, u32 *div,
	const u32 src_CLK, u32 div_min, u32 div_max)
{
	u32	clk_rate1, clk_rate2, err1, err2;
	u64	u64_tmp;
	...
	u64_tmp = (u64)(PCLKCTRL->freq) * (u64)div_max;
	...
	*div = (u32)u64_tmp;
	...
	u64_tmp = (u64)src_CLK * (u64)(*div);
	do_div(u64_tmp, (div_max + 1));
	clk_rate1 = (u32)u64_tmp;

	u64_tmp = (u64)src_CLK * (u64)(*div + 1);
	do_div(u64_tmp, (div_max + 1));
	clk_rate2 = (u32)u64_tmp;
...
{
	int	 i;
	u32	div_max, searchsrc, err_dco, err_div, md;
	u32	div[MAX_CLK_SRC], err[MAX_CLK_SRC];
	u32	div_dco = PCLKCTRL_DIV_DCO_MIN;
	u32	div_div = PCLKCTRL_DIV_MIN;
...
#ifdef HDMIA_SRC_SHOULD_BE_SAME_WITH_AUDIO
				|| ((PCLKCTRL->periname == PERI_HDMIA) && 
					(stHDMIASrc == audio_pll_ch))
#endif
				) && audio_pll_used) {
	...
	for (i = (MAX_CLK_SRC - 1); i >= 0; i--) {
		...
		if ((adjust_audio_clk != false) && 
				(tcc_ckc_is_audio_clk(PCLKCTRL->periname) != 0))
			err_div = 0xFFFFFFFF;
	...
	if (PCLKCTRL->md == PCLKCTRL_MODE_DCO) {
		...
		if (PCLKCTRL->div > (div_max / 2))
			u64_tmp = (u64)stClockSource[searchsrc] * 
					(u64)(div_max - PCLKCTRL->div);
		else
			u64_tmp = (u64)stClockSource[searchsrc] *
				(u64)PCLKCTRL->div;
		
		do_div(u64_tmp, div_max);
		PCLKCTRL->freq = (u64)u64_tmp;

		if ((PCLKCTRL->div < PCLKCTRL_DIV_DCO_MIN) ||
		...
		PCLKCTRL->freq = stClockSource[searchsrc] /
			(PCLKCTRL->div + 1);
	}
	return 0;
}

static inline tPCLKTYPE tcc_check_pclk_type(u32 periname)
{
	switch (periname) {
		case PERI_LCD0:
		case PERI_LCD1:
...
static int tcc_ckc_peri_set_rate(int id, unsigned long rate)
{
	...
	if (((nPCLKCTRL.periname == PERI_LCD0) || 
			(nPCLKCTRL.periname == PERI_LCD1)) &&
		...
		if (tcc_find_pclk(&nPCLKCTRL, type) != 0)
	...
	if ((audio_clk.flag != 0) && (adjust_audio_clk != false)) {
...
static unsigned long tcc_ckc_peri_get_rate(int id)
{
	...
	if (nPCLKCTRL.md == PCLKCTRL_MODE_DIVIDER)
		...
		if (nPCLKCTRL.div > (div_mask + 1) / 2)
			u64_tmp = (u64)src_freq * (u64)((div_mask + 1) - nPCLKCTRL.div);
		else
			u64_tmp = (u64)src_freq * (u64)nPCLKCTRL.div;

		do_div(u64_tmp ,div_mask + 1);
		nPCLKCTRL.freq = (u32)u64_tmp;
	}
...
static inline int tcc_ckc_set_audio_pll(unsigned long rate)
{
	...
	u32		pll, real_pll, cmp_value, int_tmp, real_pclk, dco_err, div_err;
	u64		u64_tmp;
...
#else
	for (int_tmp = (PCLKCTRL_DIV_YYY_MAX + 1); (int_tmp > 0); int_tmp--)
#endif
	{
		...
		nPLL.fpll = (u32)u64_tmp;
		if (tcc_find_pms(&nPLL, XIN_CLK_RATE) != 0)
			continue;
	...
	/* dco mode */
#ifdef HDMIA_SRC_SHOULD_BE_SAME_WITH_AUDIO
	for (int_tmp = ((PCLKCTRL_DIV_YYY_MAX + 1) / 4); (int_tmp > 0); int_tmp--)
#else
	for (int_tmp = ((PCLKCTRL_DIV_YYY_MAX + 1) / 2); (int_tmp > 0); int_tmp--)
#endif
	{
...
tcc_set_audio_pll_found:
	(dco_err < div_err) ? memcpy(&nPLL, &dco_PLL, sizeof(tPMS)) :
		memcpy(&nPLL, &div_PLL, sizeof(tPMS));
...
tcc_set_audio_pll_finish:
	tcc_pll_write(reg, 0, PLL_P_MIN, ((PLL_P_MIN * PLL_VCO_MIN) +
			XIN_CLK_RATE)/XIN_CLK_RATE, PLL_S_MIN, PLLSRC_XIN);
	return 0;
}

static inline int tcc_find_audio_pclk(tPCLKCTRL *PCLKCTRL)
{
	u32	audio_pll, div_max;
	u32	err_dco = 0xFFFFFFFF;
	u32	err_div = 0xFFFFFFFF;
	u32	div_div = PCLKCTRL_DIV_MIN;
	u32	div_dco = PCLKCTRL_DIV_DCO_MIN;
...
#endif
	{
		...
		tcc_pclkctrl_write(reg, PCLKCTRL_MODE_DIVIDER , PCLKCTRL->en,
				PCLKCTRL_SEL_XIN, 0, PCLKCTRL_TYPE_YYY);
		audio_pll = tcc_ckc_set_audio_pll(PCLKCTRL->freq);
	}
	div_max = PCLKCTRL_DIV_YYY_MAX;

	/* dco mode */
	err_dco = tcc_ckc_pclk_dco(PCLKCTRL, &div_dco, audio_pll,
				   PCLKCTRL_DIV_DCO_MIN, div_max);

	/* divider mode */
	err_div = tcc_ckc_pclk_divider(PCLKCTRL, &div_div, audio_pll, 
				       PCLKCTRL_DIV_MIN + 1, div_max + 1);

	/* select mode/sel */
	PCLKCTRL->sel = (u32)audio_pll_ch;
	...
	if (err_dco < err_div) {
		...
		if (PCLKCTRL->div > (div_max + 1) / 2)
			u64_tmp = (u64)audio_pll * (u64)((div_max + 1) -
					PCLKCTRL->div);
		else
			u64_tmp = (u64)audio_pll * (u64)PCLKCTRL->div;

		do_div(u64_tmp, (div_max + 1));
		PCLKCTRL->freq = (u32)u64_tmp;
	}
	else {
		PCLKCTRL->md = PCLKCTRL_MODE_DIVIDER;
...
static inline void tcc_ckc_reset_clock_source(int id)
{
	...
	switch (id) {
 	...
 	if ((audio_pll_used != false) && ((int)audio_pll_ch == id)) {
		stClockSource[id] = 0;
		stClockSource[MAX_TCC_PLL + id] = 0;
		return;
	}
...
static int tcc_ckc_pmu_pwdn(int id, bool pwdn)
{
	...
	u32	ckcrst_mask = 0, pwrsts_mask = 0, pwrsts1_mask = 0, pwrsts2_mask = 0;
	u32	memhclk_mask = 0, memrst_mask = 0, memsts_mask = 0;
...
/*	case FBUS_HEVC_BPU:	*/
	...
	while ((ckc_readl(pmu_base + PMU_PWRSTS0) & (0x3F << 25)) != 0);
...
void tcc_ckc_backup_regs(u32 clk_down)
{
...
void tcc_ckc_restore_regs(void)
{
	...
	for (i = 0; i < PERI_MAX; i++) {
		...
		if ((do_hibernate_boot != 0) && ((i == PERI_LCD0) || (i == PERI_LCD1) || (i == PERI_ADAI1) || (i == PERI_ADAM1))) //In case of 8971 LCN
			continue;
		#else
		if ((do_hibernate_boot != 0) && ((i == PERI_LCD0) || (i == PERI_LCD1)))
			continue;
		#endif
#endif
		tcc_ckc_peri_set_rate(i, ckc_backup->peri[i].rate ? : XIN_CLK_RATE );
		
		if ((ckc_backup->peri[i].en) != 0)
...
int tcc_ckc_set_hdmi_audio_src(u32 src_id)
{
#ifdef HDMIA_SRC_SHOULD_BE_SAME_WITH_AUDIO
	volatile u32 *pPCLKCTRL = (volatile u32 *)(ckc_base + CKC_PCLKCTRL) + src_id;
	u32	sel;

	if (tcc_ckc_is_audio_clk(src_id) == 0) {
		stHDMIASrc = 0xFFFFFFFF;
		return -1;
	}

	sel = (*(volatile u32 *)pPCLKCTRL & (PCLKCTRL_SEL_MASK << PCLKCTRL_SEL_SHIFT)) >> PCLKCTRL_SEL_SHIFT;
	switch (sel) {
		case PCLKCTRL_SEL_PLL0:
		case PCLKCTRL_SEL_PLL1:
		case PCLKCTRL_SEL_PLL2:
...
int tcc_ckc_adjust_audio_clk(u32 aud_id, int value)
{
	void __iomem	*aud_pclk = ckc_base + CKC_PCLKCTRL + (aud_id * 0x4);
	u32	div, offset;
	u32	reg_value;

	if (adjust_audio_clk == 0)
		return -1;
	...
	if (tcc_ckc_is_audio_clk(aud_id) == 0)
		return -1;

	reg_value = ckc_readl(aud_pclk);

	/* return. if divider mode */
	if ((reg_value & (1 << PCLKCTRL_MD_SHIFT)) != 0)
		return -1;
...
void tcc_ckc_restore_audio_clk(u32 aud_id)
{
	if ((audio_clk.flag) != 0) {
		BUG_ON(aud_id != (audio_clk.aud_id));
		audio_clk.flag = 0;
 ...
 */
unsigned long tcc_ckc_get_nand_iobus_clk(void)
{
	return tcc_ckc_clkctrl_get_rate(FBUS_IO) / 100;
}
EXPORT_SYMBOL(tcc_ckc_get_nand_iobus_clk);
