...
static void __iomem	*vbusckc_base = NULL;
...
static inline int tcc_find_dithered_pms(tDPMS *PLL, unsigned int srcfreq)
{
	u64	u64_pll, u64_src, fvco, srch_p, srch_m, u64_tmp;
	unsigned int srch_pll, err, srch_err;
	int	srch_s;
...
static unsigned long tcc_ckc_clkctrl_get_rate(int id)
{
	...
	switch (id) {
		...
		case FBUS_G2D:
			return tcc_ckc_dedicated_pll_get_rate(gpu_2d_base + DCKC_PLLPMS);
		case FBUS_MEM_PHY:
			return tcc_ckc_dedicated_pll_get_rate(mem_ckc_base + DCKC_PLLPMS);
		case FBUS_VBUS:
		case FBUS_CHEVC:
		case FBUS_BHEVC:
		case FBUS_CODA:
