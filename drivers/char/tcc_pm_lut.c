...
void pm_lut_drv_set_last_frame(void)
{
	...
	if (pixelmapper_plug >= 0) {
		...
		if (backup_pm_lut_cmd.pm_lut_dev_num == 0)
			...
			last_frame_pm = 0;
		vioc_pm_set_lut_table(last_frame_pm,
				      (unsigned int *)&backup_pm_lut_cmd.table);
		ret =
		    VIOC_CONFIG_PlugIn(VIOC_PIXELMAP + last_frame_pm,
				       VIOC_RDMA01);
		if (ret < 0)
			pr_err
			    ("[ERR][PM_LUT] %s pixel_mapper_%d plug in fail\n",
			     __func__, get_vioc_index(VIOC_PIXELMAP0));
		dprintk("pixel mapper-%d plug in to RDMA :0x%x n",
...
void pm_lut_drv_last_frame_plugout(void)
{
	...
	if (VIOC_CONFIG_Device_PlugState(VIOC_PIXELMAP0, &VIOC_lastframe_PlugIn)
	    == VIOC_DEVICE_CONNECTED) {
		if (VIOC_lastframe_PlugIn.connect_device == 1
		    && (VIOC_lastframe_PlugIn.connect_statue ==
			VIOC_PATH_CONNECTED))
			component = VIOC_PIXELMAP0;
	}

	if (VIOC_CONFIG_Device_PlugState(VIOC_PIXELMAP1, &VIOC_lastframe_PlugIn)
	    == VIOC_DEVICE_CONNECTED) {
		if (VIOC_lastframe_PlugIn.connect_device == 1
		    && (VIOC_lastframe_PlugIn.connect_statue ==
			VIOC_PATH_CONNECTED))
	...
	if (component) {
		...
		if (ret < 0)
			pr_err
			    ("[ERR][PM_LUT] %s pixel_mapper_%d plug out fail\n",
			     __func__, get_vioc_index(VIOC_PIXELMAP0));
	}
}
EXPORT_SYMBOL(pm_lut_drv_last_frame_plugout);

static long pm_lut_drv_ioctl(struct file *filp, unsigned int cmd,
			     unsigned long arg)
{
	switch (cmd) {
	case TCC_PM_LUT_SET:
		{
			...
			pm_lut_cmd =
			    kmalloc(sizeof(struct VIOC_PM_LUT_VALUE_SET),
				    GFP_KERNEL);

			if (copy_from_user
			    ((void *)pm_lut_cmd, (const void *)arg,
			     sizeof(struct VIOC_PM_LUT_VALUE_SET))) {
				kfree(pm_lut_cmd);
	...
	case TCC_PM_LUT_ONOFF:
		{
			...
			if (copy_from_user
			    ((void *)&pm_lut_onoff, (const void *)arg,
			     sizeof(struct VIOC_PM_LUT_ONOFF_SET))) {
				return -EFAULT;
			}
			dprintk("%s pm_lut_dev_num:%d pm_lut_onoff:%d\n",
				__func__, pm_lut_onoff.pm_lut_dev_num,
				pm_lut_onoff.pm_lut_onoff);
	...
	case TCC_PM_LUT_PLUG_IN:
		{
			...
			if (copy_from_user
			    ((void *)&pm_lut_cmd, (const void *)arg,
			     sizeof(struct VIOC_PM_LUT_PLUG_IN_SET))) {
				return -EFAULT;
			}
			dprintk
			    (
				"%s enable:%d pm_lut dev_num:%d pm_lut_plug_in_ch:%d\n",
			     __func__, pm_lut_cmd.enable,
			     pm_lut_cmd.pm_lut_dev_num,
			     pm_lut_cmd.pm_lut_plug_in_ch);

			if (!pm_lut_cmd.enable) {
				VIOC_CONFIG_PlugOut(VIOC_PIXELMAP +
						    pm_lut_cmd.pm_lut_dev_num);
			} else {
				int support_plug =
				    CheckPixelMapPathSelection(
						pm_lut_cmd.pm_lut_plug_in_ch);
				dprintk
				    ("pixel mapper 0  plug in to RDMA :0x%x\n",
				     pm_lut_cmd.pm_lut_plug_in_ch);

				if (support_plug >= 0) {
					int ret =
					    VIOC_CONFIG_PlugIn(VIOC_PIXELMAP +
						pm_lut_cmd.pm_lut_dev_num,
						pm_lut_cmd.pm_lut_plug_in_ch);
					if (ret < 0)
						pr_err
						    (
							"[ERR][PM_LUT] %s pixelmapper plug in fail\n",
						     __func__);
				}
			}
		}
	...
	default:
		pr_err(
			"[ERR][PM_LUT] not supported PM_LUT IOCTL(0x%x).\n",
		       cmd);
		break;
	}
...
static const struct file_operations pm_lut_drv_fops = {
	.owner = THIS_MODULE,
...
static int pm_lut_drv_probe(struct platform_device *pdev)
{
	...
	pr_info
	    (
		"[INF][PM_LUT] %s: :%s, Driver Initialized  pm_lut set num :0x%x\n",
...
{
	struct pm_lut_drv_type *pm_lut =
	    (struct pm_lut_drv_type *)platform_get_drvdata(pdev);
...
static const struct of_device_id pm_lut_of_match[] = {
