...
static unsigned int gCpVersion =        0;
static unsigned int gCpI2cChannel =     0;
static unsigned int gCpState =          0;
static struct class *tcc_cp_class =     NULL;
static struct device *tcc_cp_dev =      NULL;
...
void tcc_cp_gpio_init(struct device *dev)
{
    ...
    {
        if (gCpVersion == 0x20B)
		    dev_err(dev, "Invalid GPIO Set in Device Tree: Reset GPIO[%d]\n", gCPResetGpio);
        else if (gCpVersion == 0x20C)
            dev_warn(dev, "I2C Slave Address Will Not Be Changed: GPIO[%d]\n", gCPResetGpio); 
        else
...
static void tcc_cp_read_device_tree(struct device *dev)
{
	...
	if (of_find_property(dev->of_node, "cp-type", 0)) 
    {
		...
		/* Auth CP type 2 = 3.0, Auth CP type 1 = 2.0C, Auth CP type 0 = 2.0B */
        switch (temp)
        {
...
static int tcc_cp_ctrl_probe(struct platform_device *pdev)
{
	...
	if((res = register_chrdev(CP_DEV_MAJOR_NUM, TCC_CP_DEV_NAME, &tcc_cp_fops)) >= TCC_CP_NOERR)
    {
	    if ((tcc_cp_class = class_create(THIS_MODULE, TCC_CP_DEV_NAME)) != NULL)
        {
        	if((tcc_cp_dev = device_create(tcc_cp_class, NULL, MKDEV(CP_DEV_MAJOR_NUM, CP_DEV_MINOR_NUM), NULL, TCC_CP_DEV_NAME)) != NULL)
            {
#ifdef CONFIG_OF
