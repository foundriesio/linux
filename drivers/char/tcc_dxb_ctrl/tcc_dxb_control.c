...
struct tcc_dxb_ctrl_t *gDxbCtrl;
...
static void GPIO_OUT_INIT(int pin)
{
	...
	if (!gpio_is_valid(pin)) {
		pr_err("[ERROR][TCC_DXB_CTRL] %s pin(0x%X) error\n",
		       __func__, pin);
		return;
	}
	rc = gpio_request(pin, NULL);
	if (rc) {
		pr_err("[ERROR][TCC_DXB_CTRL] %s pin(0x%X) error(%d)\n",
		       __func__, pin, rc);
		return;
	}
	pr_info("[INFO][TCC_DXB_CTRL] %s pin[0x%X] value[0]\n", __func__, pin);
...
static void GPIO_IN_INIT(int pin)
{
	...
	pr_info("[INFO][TCC_DXB_CTRL] %s pin[0x%X]\n", __func__, pin);
...
static void GPIO_SET_VALUE(int pin, int value)
{
	...
	pr_info("[INFO][TCC_DXB_CTRL] %s pin[0x%X] value[%d]\n", __func__, pin,
		value);
...
struct baseband {
	int type;
	int (*ioctl)(struct tcc_dxb_ctrl_t *ctrl, unsigned int cmd,
		      unsigned long arg);
...
static irqreturn_t isr_checking_ant_overload(int irq, void *dev_data)
{
	...
	pr_info("[INFO][TCC_DXB_CTRL] %s : ant overload = %d\n", __func__,
	       GPIO_GET_VALUE(ctrl->gpio_check_ant_overload));
...
static int tcc_dxb_ctrl_ant_on(struct tcc_dxb_ctrl_t *ctrl)
{
	...
	case PWRCTRL_AUTO:
		...
		if (GPIO_GET_VALUE(ctrl->gpio_check_ant_overload) == 0) {
			pr_info("[INFO][TCC_DXB_CTRL] ANT_OVERLOAD is low\n");
			GPIO_SET_VALUE(ctrl->gpio_ant_pwr, 0);
			return 0;
		}
		ctrl->irq_check_ant_overlaod =
		    gpio_to_irq(gDxbCtrl->gpio_check_ant_overload);
#if 1
		err =
...
#endif
		if (err) {
			pr_err("[ERROR][TCC_DXB_CTRL] ");
			pr_err("Unable to request ANT_OVERLOAD IRQ[%d]\n", err);
...
static long tcc_dxb_ctrl_ioctl(struct file *filp, unsigned int cmd,
			       unsigned long arg)
{
	struct baseband *bb;
	...
	case IOCTL_DXB_CTRL_SET_BOARD:
		{
			...
			if (copy_from_user
			    ((void *)&new_board_type, (const void *)arg,
			     sizeof(unsigned int)) != 0) {
				break;
			}
			...
			if (gDxbCtrl->bb_index == (-1)) {
				...
				pr_err("[ERROR][TCC_DXB_CTRL] %s cmd[0x%X] undefined board type %d\n",
				     __func__, cmd, new_board_type);
				return -1;
			}

			bb = &BB[gDxbCtrl->bb_index];
			pr_info("[INFO][TCC_DXB_CTRL] new_board_type=%d, bb_index=%d\n",
...
const struct file_operations tcc_dxb_ctrl_fops = {
	.owner = THIS_MODULE,
 ...
 ******************************************************************************/
static ssize_t tcc_dxb_ctrl_store(struct device *dev,
...
static ssize_t tcc_dxb_ctrl_show(struct device *dev,
...
static DEVICE_ATTR(state, 0644, tcc_dxb_ctrl_show, tcc_dxb_ctrl_store);
...
static int tcc_dxb_ctrl_probe(struct platform_device *pdev)
{
	...
	if (NULL ==
	    device_create(tcc_dxb_ctrl_class, NULL,
			  MKDEV(major_num, DXB_CTRL_DEV_MINOR), NULL,
			  DXB_CTRL_DEV_NAME)) {
		pr_err("[ERROR][TCC_DXB_CTRL] %s device_create failed\n",
		       __func__);
	}
	...
	if (device_create_file(dev, &dev_attr_state)) {
		pr_err("[ERROR][TCC_DXB_CTRL] Failed to create file.\n");
	}
	...
	gDxbCtrl->gpio_dxb_0_irq =
	    of_get_named_gpio(dev->of_node, "dxb0-gpios", 2);
	gDxbCtrl->gpio_dxb_0_sdo =
...
static int tcc_dxb_ctrl_remove(struct platform_device *pdev)
{
	...
	device_destroy(tcc_dxb_ctrl_class,
		       MKDEV(gDxbCtrl->device_major_number,
			     DXB_CTRL_DEV_MINOR));
