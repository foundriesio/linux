...
//#define dprintk(msg...) pr_info("dispman: " msg)
...
static int tcc_dispman_support_resolution_length;
...
static ssize_t tcc_hdmi_720p_fixed_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_TCC_HDMI_720P_FIXED]));
}

static ssize_t tcc_hdmi_720p_fixed_store(struct device *dev,
...
static ssize_t tcc_audio_hdmi_link_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_TCC_AUDIO_HDMI_LINK]));
}

static ssize_t tcc_audio_hdmi_link_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
...
static ssize_t tcc_audio_sampling_rate_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_TCC_AUDIO_SAMPLING_RATE]));
}

static ssize_t tcc_audio_sampling_rate_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
	...
	if (error)
		return error;
	//if (data > 1) data = 1;
	atomic_set(&tcc_dispman_attribute_data[MGEM_TCC_AUDIO_SAMPLING_RATE],
		data);
	return count;
}

static ssize_t tcc_audio_channels_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_TCC_AUDIO_CHANNELS]));
}

static ssize_t tcc_audio_channels_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
...
static ssize_t tcc_cec_imageview_on_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_TCC_CEC_IMAGEVIEW_ON]));
}

static ssize_t tcc_cec_imageview_on_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
	...
	//if (data > 1) data = 1;
	atomic_set(
		&tcc_dispman_attribute_data[MGEM_TCC_CEC_IMAGEVIEW_ON], data);
	return count;
}

static ssize_t tcc_cec_textview_on_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_TCC_CEC_TEXTVIEW_ON]));
}

static ssize_t tcc_cec_textview_on_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
...
static ssize_t tcc_cec_active_source_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_TCC_CEC_ACTIVE_SOURCE]));
}

static ssize_t tcc_cec_active_source_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
	...
	if (error)
		return error;
	//if (data > 1) data = 1;
	atomic_set(&tcc_dispman_attribute_data[MGEM_TCC_CEC_ACTIVE_SOURCE],
		data);
	return count;
}

static ssize_t tcc_cec_in_active_source_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_TCC_CEC_IN_ACTIVE_SOURCE]));
}

static ssize_t tcc_cec_in_active_source_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
	...
	if (error)
		return error;
	//if (data > 1) data = 1;
	atomic_set(&tcc_dispman_attribute_data[MGEM_TCC_CEC_IN_ACTIVE_SOURCE],
		data);
	return count;
}


static ssize_t tcc_cec_standby_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_TCC_CEC_STANDBY]));
}

static ssize_t tcc_cec_standby_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
...
static ssize_t tcc_cec_connection_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_TCC_CEC_CONNECTION]));
}

static ssize_t tcc_cec_connection_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
...
static ssize_t tcc_hdmi_audio_type_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_TCC_HDMI_AUDIO_TYPE]));
}

static ssize_t tcc_hdmi_audio_type_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
...
static ssize_t tcc_output_display_type_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_TCC_OUTPUT_DISPLAY_TYPE]));
}

static ssize_t tcc_output_display_type_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
	...
	atomic_set(&tcc_dispman_attribute_data[MGEM_TCC_OUTPUT_DISPLAY_TYPE],
		data);
	return count;
}

static ssize_t tcc_output_hdmi_3d_format_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_TCC_OUTPUT_HDMI_3D_FORMAT]));
}

static ssize_t tcc_output_hdmi_3d_format_store(
	...
	const char *buf, size_t count)
{
	...
	if (error)
		return error;
	//if (data > 1) data = 1;
	atomic_set(&tcc_dispman_attribute_data[MGEM_TCC_OUTPUT_HDMI_3D_FORMAT],
		data);
	return count;
}


static ssize_t tcc_output_hdmi_audio_onoff_show(
	struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_TCC_OUTPUT_HDMI_AUDIO_ONOFF]));
}

static ssize_t tcc_output_hdmi_audio_onoff_store(
	struct device *dev, struct device_attribute *attr,
	const char *buf, size_t count)
{
	unsigned long data;
...
static ssize_t tcc_output_hdmi_audio_disable_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_TCC_OUTPUT_HDMI_AUDIO_DISABLE])
		);
}

static ssize_t tcc_output_hdmi_audio_disable_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
...
static ssize_t tcc_output_hdmi_video_format_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n",
		atomic_read(
		&tcc_dispman_attribute_data[MGEM_TCC_OUTPUT_HDMI_VIDEO_FORMAT])
		);
}

static ssize_t tcc_output_hdmi_video_format_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
...
static ssize_t tcc_output_hdmi_structure_3d_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_TCC_OUTPUT_HDMI_STRUCTURE_3D])
		);
}

static ssize_t tcc_output_hdmi_structure_3d_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
...
static ssize_t tcc_output_hdmi_supported_resolution_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int tcc_dispman_supported_resolution_length = (int)atomic_read(
		&tcc_dispman_attribute_data
		[MGEM_TCC_OUTPUT_HDMI_SUPPORTED_RESOLUTION]);
	ssize_t ret_size = 0;

	mutex_lock(&tcc_dispman_mutex);
	if (tcc_dispman_supported_resolution_length > 0) {
		memcpy(buf, tcc_dispman_supported_resolution,
			tcc_dispman_supported_resolution_length);
...
static ssize_t tcc_output_hdmi_supported_resolution_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	mutex_lock(&tcc_dispman_mutex);
	...
	atomic_set(&tcc_dispman_attribute_data
		[MGEM_TCC_OUTPUT_HDMI_SUPPORTED_RESOLUTION], count);
	mutex_unlock(&tcc_dispman_mutex);
	return count;
}

static ssize_t tcc_output_hdmi_supported_3d_mode_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int tcc_dispman_supported_3d_mode_lenght = (int)atomic_read(
		&tcc_dispman_attribute_data
		[MGEM_TCC_OUTPUT_HDMI_SUPPORTED_3D_MODE]);
	ssize_t ret_size = 0;
	...
	if (tcc_dispman_supported_3d_mode_lenght > 0) {
		memcpy(buf, tcc_dispman_supported_3d_mode,
			tcc_dispman_supported_3d_mode_lenght);
...
static ssize_t tcc_output_hdmi_supported_3d_mode_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	mutex_lock(&tcc_dispman_mutex);
	...
	atomic_set(&tcc_dispman_attribute_data
		[MGEM_TCC_OUTPUT_HDMI_SUPPORTED_3D_MODE], count);
	mutex_unlock(&tcc_dispman_mutex);
	return count;
}

static ssize_t tcc_output_hdmi_supported_hdr_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	ssize_t ret_size = 0;
	int tcc_dispman_supported_hdr_lenght =
		(int)atomic_read(&tcc_dispman_attribute_data
			[MGEM_TCC_OUTPUT_HDMI_SUPPORTED_HDR]);

	mutex_lock(&tcc_dispman_mutex);
	if (tcc_dispman_supported_hdr_lenght > 0) {
		memcpy(buf, tcc_dispman_supported_hdr,
			tcc_dispman_supported_hdr_lenght);
...
static ssize_t tcc_output_hdmi_supported_hdr_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	mutex_lock(&tcc_dispman_mutex);
	...
	atomic_set(
		&tcc_dispman_attribute_data[MGEM_TCC_OUTPUT_HDMI_SUPPORTED_HDR],
		count);
	mutex_unlock(&tcc_dispman_mutex);
	return count;
}

static ssize_t tcc_video_hdmi_resolution_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_TCC_VIDEO_HDMI_RESOLUTION]));
}

static ssize_t tcc_video_hdmi_resolution_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
	...
	if (error)
		return error;
	//if (data > 1) data = 1;
	atomic_set(&tcc_dispman_attribute_data[MGEM_TCC_VIDEO_HDMI_RESOLUTION],
		data);
	return count;
}


static ssize_t tcc_output_mode_detected_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_TCC_OUTPUT_MODE_DETECTED]));
}

static ssize_t tcc_output_mode_detected_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
	...
	if (error)
		return error;
	//if (data > 1) data = 1;
	atomic_set(&tcc_dispman_attribute_data[MGEM_TCC_OUTPUT_MODE_DETECTED],
		data);
	return count;
}

static ssize_t tcc_output_panel_width_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
...
static ssize_t tcc_output_panel_height_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
#ifdef CONFIG_FB_VIOC
	ssize_t result_size;
	...
	struct device_attribute *attr, char *buf)
{
	...
	if ((ptccfb_info->pdata.Mdp_data.DispDeviceType == TCC_OUTPUT_HDMI)
		...
		pr_err("
		[ERR][DISP_MAN] %s Can't find output, Main:%d, Sub:%d\n",
		__func__,
		ptccfb_info->pdata.Mdp_data.DispDeviceType,
		ptccfb_info->pdata.Sdp_data.DispDeviceType
		);
...
static ssize_t tcc_output_mode_stb_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_TCC_OUTPUT_MODE_STB]));
}

static ssize_t tcc_output_mode_plugout_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_TCC_OUTPUT_MODE_PLUGOUT]));
}

static ssize_t tcc_output_mode_plugout_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
	...
	if (error)
		return error;
	//if (data > 1) data = 1;
	atomic_set(&tcc_dispman_attribute_data[MGEM_TCC_OUTPUT_MODE_PLUGOUT],
		data);
	return count;
}
...
{
	return atomic_read(
		&tcc_dispman_attribute_data[MGEM_TCC_2D_COMPRESSION]);
}
EXPORT_SYMBOL(tcc_2d_compression_read);

static ssize_t tcc_2d_compression_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", tcc_2d_compression_read());
}

static ssize_t tcc_2d_compression_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
...
//for hdcp
static ssize_t tcc_hdcp_hdmi_enable_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_TCC_HDCP_HDMI_ENABLE]));
}

static ssize_t tcc_hdcp_hdmi_enable_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
	...
	if (error)
		return error;
	//if (data > 1) data = 1;
	atomic_set(&tcc_dispman_attribute_data[MGEM_TCC_HDCP_HDMI_ENABLE],
		data);
	return count;
}
...
static ssize_t persist_hdcp1x_enable_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
...
static ssize_t persist_display_mode_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_DISPLAY_MODE]));
}

static ssize_t persist_extenddisplay_reset_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_EXTENDDISPLAY_RESET]));
}

static ssize_t persist_extenddisplay_reset_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
...
static ssize_t persist_output_attach_main_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_OUTPUT_ATTACH_MAIN]));
}

static ssize_t persist_output_attach_main_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
	...
	if (error)
		return error;
	//if (data > 1) data = 1;
	atomic_set(&tcc_dispman_attribute_data[MGEM_PERSIST_OUTPUT_ATTACH_MAIN],
		data);
	return count;
}

static ssize_t persist_output_attach_sub_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_OUTPUT_ATTACH_SUB]));
}

static ssize_t persist_output_attach_sub_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
...
static ssize_t persist_output_mode_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_OUTPUT_MODE]));
}

static ssize_t persist_output_mode_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
...
static ssize_t persist_auto_resolution_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_AUTO_RESOLUTION]));
}

static ssize_t persist_auto_resolution_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
	...
	if (error)
		return error;
	//if (data > 1) data = 1;
	atomic_set(&tcc_dispman_attribute_data[MGEM_PERSIST_AUTO_RESOLUTION],
		data);
	return count;
}


static ssize_t persist_spdif_setting_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_SPDIF_SETTING]));
}

static ssize_t persist_spdif_setting_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
	...
	if (error)
		return error;
	//if (data > 1) data = 1;
	atomic_set(&tcc_dispman_attribute_data[MGEM_PERSIST_SPDIF_SETTING],
		data);
	return count;
}
...
static ssize_t persist_hdmi_mode_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_MODE]));
}

static ssize_t persist_hdmi_mode_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
...
static ssize_t persist_hdmi_resize_up_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_RESIZE_UP]));
}

static ssize_t persist_hdmi_resize_up_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
	...
	if (error)
		return error;
	//if (data > 1) data = 1;
	atomic_set(&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_RESIZE_UP],
		data);
	return count;
}

static ssize_t persist_hdmi_resize_dn_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_RESIZE_DN]));
}

static ssize_t persist_hdmi_resize_dn_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
	...
	if (error)
		return error;
	//if (data > 1) data = 1;
	atomic_set(&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_RESIZE_DN],
		data);
	return count;
}


static ssize_t persist_hdmi_resize_lt_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_RESIZE_LT]));
}

static ssize_t persist_hdmi_resize_lt_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
	...
	if (error)
		return error;
	//if (data > 1) data = 1;
	atomic_set(&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_RESIZE_LT],
		data);
	return count;
}

static ssize_t persist_hdmi_resize_rt_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_RESIZE_RT]));
}

static ssize_t persist_hdmi_resize_rt_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
	...
	if (error)
		return error;
	//if (data > 1) data = 1;
	atomic_set(&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_RESIZE_RT],
		data);
	return count;
}


static ssize_t persist_hdmi_cec_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_CEC]));
}

static ssize_t persist_hdmi_cec_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
...
static ssize_t persist_hdmi_resolution_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_RESOLUTION]));
}

static ssize_t persist_hdmi_resolution_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
	...
	if (error)
		return error;
	//if (data > 1) data = 1;
	atomic_set(&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_RESOLUTION],
		data);
	return count;
}
...
static ssize_t persist_hdmi_detected_res_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_DETECTED_RES]));
}

static ssize_t persist_hdmi_detected_res_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
	...
	if (error)
		return error;
	//if (data > 1) data = 1;
	atomic_set(&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_DETECTED_RES],
		data);
	return count;
}


static ssize_t persist_hdmi_printlog_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_PRINTLOG]));
}

static ssize_t persist_hdmi_printlog_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
	...
	if (error)
		return error;
	//if (data > 1) data = 1;
	atomic_set(&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_PRINTLOG],
		data);
	return count;
}

static ssize_t persist_hdmi_color_depth_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_COLOR_DEPTH]));
}

static ssize_t persist_hdmi_color_depth_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
	...
	if (error)
		return error;
	//if (data > 1) data = 1;
	atomic_set(&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_COLOR_DEPTH],
		data);
	return count;
}
...
static ssize_t persist_hdmi_color_space_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_COLOR_SPACE]));
}

static ssize_t persist_hdmi_color_space_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
	...
	if (error)
		return error;
	//if (data > 1) data = 1;
	atomic_set(&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_COLOR_SPACE],
		data);
	return count;
}

static ssize_t persist_hdmi_colorimetry_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_COLORIMETRY]));
}

static ssize_t persist_hdmi_colorimetry_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
	...
	if (error)
		return error;
	//if (data > 1) data = 1;
	atomic_set(&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_COLORIMETRY],
		data);
	return count;
}

static ssize_t persist_hdmi_aspect_ratio_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_ASPECT_RATIO]));
}

static ssize_t persist_hdmi_aspect_ratio_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
	...
	if (error)
		return error;
	//if (data > 1) data = 1;
	atomic_set(&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_ASPECT_RATIO],
		data);
	return count;
}
...
static ssize_t persist_hdmi_detected_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_DETECTED]));
}

static ssize_t persist_hdmi_detected_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (error)
		return error;
	//if (data > 1) data = 1;
	prev_data = atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_DETECTED]);
	atomic_set(&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_DETECTED],
		data);
	return count;
}

static ssize_t persist_hdmi_detected_mode_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_DETECTED_MODE]));
}

static ssize_t persist_hdmi_detected_mode_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
	...
	if (error)
		return error;
	//if (data > 1) data = 1;
	atomic_set(&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_DETECTED_MODE],
		data);
	return count;
}

static ssize_t persist_hdmi_extra_mode_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_EXTRA_MODE]));
}

static ssize_t persist_hdmi_extra_mode_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
	...
	if (error)
		return error;
	//if (data > 1) data = 1;
	atomic_set(&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_EXTRA_MODE],
		data);
	return count;
}


static ssize_t persist_hdmi_refresh_mode_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_REFRESH_RATE]));
}

static ssize_t persist_hdmi_refresh_mode_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
	...
	if (error)
		return error;
	//if (data > 1) data = 1;
	atomic_set(&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_REFRESH_RATE],
		data);
	return count;
}
...
static ssize_t persist_composite_mode_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_COMPOSITE_MODE]));
}

static ssize_t persist_composite_mode_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
	...
	if (error)
		return error;
	//if (data > 1) data = 1;
	atomic_set(&tcc_dispman_attribute_data[MGEM_PERSIST_COMPOSITE_MODE],
		data);
	return count;
}


static ssize_t persist_composite_resize_up_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_COMPOSITE_RESIZE_UP]));
}

static ssize_t persist_composite_resize_up_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
...
static ssize_t persist_composite_resize_dn_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_COMPOSITE_RESIZE_DN]));
}

static ssize_t persist_composite_resize_dn_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
...
static ssize_t persist_composite_resize_lt_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_COMPOSITE_RESIZE_LT]));
}

static ssize_t persist_composite_resize_lt_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
...
static ssize_t persist_composite_resize_rt_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_COMPOSITE_RESIZE_RT]));
}

static ssize_t persist_composite_resize_rt_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
...
static ssize_t persist_composite_detected_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_COMPOSITE_DETECTED]));
}

static ssize_t persist_composite_detected_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data, prev_data;
	int error = kstrtoul(buf, 10, &data);
	...
	prev_data = atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_COMPOSITE_DETECTED]);
	atomic_set(&tcc_dispman_attribute_data[MGEM_PERSIST_COMPOSITE_DETECTED],
		data);
	return count;
}
...
//==COMPONENT==
static ssize_t persist_component_mode_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_COMPONENT_MODE]));
}

static ssize_t persist_component_mode_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
	...
	if (error)
		return error;
	//if (data > 1) data = 1;
	atomic_set(&tcc_dispman_attribute_data[MGEM_PERSIST_COMPONENT_MODE],
		data);
	return count;
}


static ssize_t persist_component_resize_up_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_COMPONENT_RESIZE_UP]));
}

static ssize_t persist_component_resize_up_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
...
static ssize_t persist_component_resize_dn_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_COMPONENT_RESIZE_DN]));
}

static ssize_t persist_component_resize_dn_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
...
static ssize_t persist_component_resize_lt_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_COMPONENT_RESIZE_LT]));
}

static ssize_t persist_component_resize_lt_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
	...
	//if (data > 1) data = 1;
	atomic_set(&tcc_dispman_attribute_data
		[MGEM_PERSIST_COMPONENT_RESIZE_LT], data);
	return count;
}

static ssize_t persist_component_resize_rt_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_COMPONENT_RESIZE_RT]));
}

static ssize_t persist_component_resize_rt_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
...
static ssize_t persist_component_detected_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_COMPONENT_DETECTED]));
}

static ssize_t persist_component_detected_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data, prev_data;
	int error = kstrtoul(buf, 10, &data);
	...
	prev_data = atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_COMPONENT_DETECTED]);
	atomic_set(&tcc_dispman_attribute_data[MGEM_PERSIST_COMPONENT_DETECTED],
		data);
	return count;
}
...
static ssize_t persist_supported_resolution_count_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(&tcc_dispman_attribute_data
		[MGEM_TCCDISPMAN_SUPPORT_RESOLUTION_COUNT]));
}

static ssize_t persist_supported_resolution_count_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
	...
	//if (data > 1) data = 1;
	atomic_set(&tcc_dispman_attribute_data
		[MGEM_TCCDISPMAN_SUPPORT_RESOLUTION_COUNT], data);
	return count;
}

static ssize_t persist_supported_resolution_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	ssize_t ret_size = 0;
	...
	if (tcc_dispman_support_resolution_length > 0) {
		memcpy(buf, tcc_dispman_support_resolution,
			sizeof(char) * tcc_dispman_support_resolution_length);
...
static ssize_t persist_supported_resolution_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	mutex_lock(&tcc_dispman_mutex);
	memset(tcc_dispman_support_resolution, 0,
		sizeof(tcc_dispman_support_resolution));
...
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_NATIVE_FIRST]));
}

static ssize_t hdmi_native_first_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
	...
	if (error)
		return error;
	//if (data > 1) data = 1;
	atomic_set(&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_NATIVE_FIRST],
		data);
	return count;
}
...
static ssize_t hdmi_hw_cts_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_HDMI_HW_CTS]));
}

static ssize_t hdmi_hw_cts_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
...
static ssize_t tcc_dispman_lock_file_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(&tcc_dispman_lock_file));
}

static ssize_t tcc_dispman_lock_file_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
...
{
	return sprintf(buf, "%d\n", atomic_read(
		&tcc_dispman_attribute_data[MGEM_TCC_SYS_OUTPUT_SECOND_ATTACH])
		);
}

static ssize_t tcc_sys_output_second_attach_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long data;
	int error = kstrtoul(buf, 10, &data);
...
{
	atomic_set(&tcc_dispman_attribute_data[MGEM_PERSIST_DISPLAY_MODE],
		(unsigned long)persist_display_mode);
	pr_info("[INFO][DISP_MAN] set persist_display_mode(%d)\n",
		atomic_read(
		&tcc_dispman_attribute_data[MGEM_PERSIST_DISPLAY_MODE]));
	return 0;
}
...
static int tcc_dispman_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	if (remap_pfn_range(vma, vma->vm_start, vma->vm_pgoff,
		...
		pr_err(
		"[ERR][DISP_MAN] %s(): Virtual address page port error\n",
		__func__);
		return -EAGAIN;
	}
...
static char banner[] __initdata = "dispman driver initializing\n";
static struct class *tcc_dispman_class;

void __exit tcc_dispman_cleanup(void)
{
	unregister_chrdev(tcc_dispman_major, DEVICE_NAME);
	device_destroy(tcc_dispman_class,
		MKDEV(tcc_dispman_major, MANAGER_MINOR_ID));
...
int __init tcc_dispman_init(void)
{
	...
	memset(tcc_dispman_attribute_data, 0,
		sizeof(tcc_dispman_attribute_data));

	tcc_dispman_major = MANAGER_MAJOR_ID;

	if (register_chrdev(tcc_dispman_major, DEVICE_NAME,
		&tcc_dispman_fops)) {
		tcc_dispman_major = register_chrdev(0, DEVICE_NAME,
			&tcc_dispman_fops);
		if (tcc_dispman_major <= 0) {
			pr_err(
		"[ERR][DISP_MAN] unable to register tcc_dispman device\n");
			return -EIO;
		}
		pr_err(
	"[ERR][DISP_MAN] unable to register major %d. Registered %d instead\n",
	...
	atomic_set(&tcc_dispman_attribute_data[MGEM_PERSIST_DISPLAY_MODE], 1);
	#elif defined(CONFIG_TCC_DISPLAY_MODE_DUAL_HDMI_CVBS)
	atomic_set(&tcc_dispman_attribute_data[MGEM_PERSIST_DISPLAY_MODE], 2);
	#elif defined(CONFIG_TCC_DISPLAY_MODE_DUAL_AUTO)
	atomic_set(&tcc_dispman_attribute_data[MGEM_PERSIST_DISPLAY_MODE], 3);
	#endif
#endif
	// Support Lock_F
	atomic_set(&tcc_dispman_lock_file, 0);
	...
	pr_info("[INFO][DISP_MAN] %s - STB Mode\n", __func__);
	// STB MODE
	atomic_set(&tcc_dispman_attribute_data[MGEM_TCC_OUTPUT_MODE_STB], 1);
	...
	// HDMI OUTPUT
	atomic_set(&tcc_dispman_attribute_data[MGEM_PERSIST_OUTPUT_MODE], 1);
	#endif
	...
	atomic_set(&tcc_dispman_attribute_data[MGEM_TCC_CEC_CONNECTION], 0);
	...
	if (default_component_resolution == 3/*STARTER_COMPONENT_1080I*/) {
		atomic_set(&tcc_dispman_attribute_data
			[MGEM_PERSIST_COMPONENT_MODE], 1);
	}
	#endif

	tcc_dispman_dev = device_create(tcc_dispman_class, NULL,
		MKDEV(tcc_dispman_major, MANAGER_MINOR_ID), NULL, DEVICE_NAME);

	ret = sysfs_create_group(&tcc_dispman_dev->kobj,
		&tcc_dispman_attribute_group);
	if (ret)
