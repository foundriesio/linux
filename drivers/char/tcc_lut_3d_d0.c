...
struct lut_3d_drv_type *lut_3d_api;
    ...
    case TCC_LUT_3D_SET_TABLE:
        {
            ...
            vioc_lut_3d_set_table(LUT_3D_DISP0,
                    lut_cmd->table);
            kfree(lut_cmd);
            ret = 0;
        }
    ...
    case TCC_LUT_3D_ONOFF:
        {
            ...
            ret =
                vioc_lut_3d_bypass(LUT_3D_DISP0, lut_cmd.lut_3d_onoff);
        }
        break;
...
static int lut_3d_drv_probe(struct platform_device *pdev)
{
    ...
    lut_3d->misc->name = "tcc_lut_3d_d0";
    ...
    /* Copy lut to lut_3d_api to support external APIs */
    lut_3d_api = lut_3d;
...
static const struct of_device_id lut_3d_of_match[] = {
    {.compatible = "telechips,vioc_lut_3d_d0"},
    {}
};

MODULE_DEVICE_TABLE(of, lut_3d_of_match);

static struct platform_driver lut_3d_driver_d0 = {
...
{
	return platform_driver_register(&lut_3d_driver_d0);
}

static void __exit lut_3d_drv_exit(void)
{
    platform_driver_unregister(&lut_3d_driver_d0);
}
...
MODULE_DESCRIPTION("Telechips 3D look up table Driver 0");
MODULE_LICENSE("GPL");
