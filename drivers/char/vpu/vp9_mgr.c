	...
	if (type < VPU_ENC)
	{
		...
		switch (cmd)
		{
			...
			case VPU_DEC_SEQ_HEADER_KERNEL:
			{
				...
				dprintk("Dec :: SEQ_HEADER out 0x%x \n res info. %d - %d - %d, %d - %d - %d \n", ret,
			...
			{
				vp9mgr_data.check_interrupt_detection = 1;
				ret = tcc_vp9_dec(cmd & ~VPU_BASE_OP_KERNEL, (codec_handle_t*)&pHandle, (void*)NULL, (void*)NULL);
			...
			{
				VP9_RINGBUF_GETINFO_t* arg = (VP9_RINGBUF_GETINFO_t*)args;
				vp9mgr_data.check_interrupt_detection = 1;
			...
			{
				VP9_RINGBUF_SETBUF_t* arg = (VP9_RINGBUF_SETBUF_t*)args;
				vp9mgr_data.check_interrupt_detection = 1;
			...
			{
				VP9_RINGBUF_SETBUF_PTRONLY_t* arg = (VP9_RINGBUF_SETBUF_PTRONLY_t*)args;
				vp9mgr_data.check_interrupt_detection = 1;
...
static long _vp9mgr_ioctl(struct file* file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd)
	{
		...
		case VPU_CHECK_CODEC_STATUS_KERNEL:
		{
			...
			{
				(void)memcpy((int*)arg, vp9mgr_data.closed, sizeof(vp9mgr_data .closed));
			}
			else
			{
