 ...
 */
/*
 *   drivers/char/vpu/vpu_hevc_enc.c
 *   Author:  <linux@telechips.com>
 *   Created: Apr 29, 2020
 *   Description: ioctl for TCC HEVC ENC
 */
...
#define dprintk(msg...) //printk( "TCC_VPU_HEVC_ENC: " msg);
#define detailk(msg...) //printk( "TCC_VPU_HEVC_ENC: " msg);
#define err(msg...) printk("TCC_VPU_HEVC_ENC[Err]: "msg);

static struct mutex add_mutex;
static void _vpu_hevc_enc_inter_add_list(vpu_encoder_data *vdata, int cmd, void* args)
{
...
static void _vpu_hevc_enc_init_list(vpu_encoder_data *vdata)
{
    int i = 0;
...
static int _vpu_hevc_enc_proc_exit(vpu_encoder_data *vdata, void *arg)
{
    void *pArgs;
...
static int _vpu_hevc_enc_result_general(vpu_encoder_data *vdata, int *arg)
{
...
static void _vpu_hevc_enc_force_close(vpu_encoder_data *vdata)
{
	int ret = 0;
...
static int _vpu_hevc_enc_dev_init(vpu_encoder_data *vdata, void *arg)
{
    if(vdata->vComm_data.dev_opened > 1)
	{
        ...
        _vpu_hevc_enc_force_close(vdata);
...
int vpu_hevc_enc_mmap(struct file *filp, struct vm_area_struct *vma)
{
...
unsigned int vpu_hevc_enc_poll(struct file *filp, poll_table *wait)
{
    struct miscdevice *misc = (struct miscdevice *)filp->private_data;
    vpu_encoder_data *vdata = dev_get_drvdata(misc->parent);
...
static int _vpu_hevc_enc_cmd_open(vpu_encoder_data *vdata, char *str)
{
...
static int _vpu_hevc_enc_cmd_release(vpu_encoder_data *vdata, char *str)
{
...
long vpu_hevc_enc_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    struct miscdevice *misc = (struct miscdevice *) filp->private_data;
    vpu_encoder_data *vdata = dev_get_drvdata(misc->parent);

    if(vmgr_hevc_enc_get_alive() == 0)
    {
    ...
    switch(cmd)
    {
		...
		case V_ENC_TRY_OPEN_DEV:
	            _vpu_hevc_enc_cmd_open(vdata, "cmd");
		    break;

		case V_ENC_TRY_CLOSE_DEV:
	            _vpu_hevc_enc_cmd_release(vdata, "cmd");
		    break;
#endif
        default:
...
int vpu_hevc_enc_open(struct inode *inode, struct file *filp)
{
    struct miscdevice *misc = (struct miscdevice *)filp->private_data;
    vpu_encoder_data *vdata = dev_get_drvdata(misc->parent);
...
#else
    _vpu_hevc_enc_cmd_open(vdata, "file");
#endif

    return 0;
}

int vpu_hevc_enc_release(struct inode *inode, struct file *filp)
{
    struct miscdevice *misc = (struct miscdevice *)filp->private_data;
    vpu_encoder_data *vdata = dev_get_drvdata(misc->parent);
...
#else
    _vpu_hevc_enc_cmd_release(vdata, "file");
#endif
...
static struct file_operations vdev_enc_fops = {
    .owner              = THIS_MODULE,
    .open               = vpu_hevc_enc_open,
    .release            = vpu_hevc_enc_release,
    .mmap               = vpu_hevc_enc_mmap,
    .unlocked_ioctl     = vpu_hevc_enc_ioctl,
#ifdef CONFIG_COMPAT
    .compat_ioctl       = vpu_hevc_enc_ioctl,
#endif
    .poll               = vpu_hevc_enc_poll,
};

int vpu_hevc_enc_probe(struct platform_device *pdev)
{
...
EXPORT_SYMBOL(vpu_hevc_enc_probe);

int vpu_hevc_enc_remove(struct platform_device *pdev)
{
...
EXPORT_SYMBOL(vpu_hevc_enc_remove);
#endif
