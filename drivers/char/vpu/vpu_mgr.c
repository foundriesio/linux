	...
	if (type < VPU_ENC)
	{
		...
		switch (cmd)
		{
			case VPU_DEC_INIT: case VPU_DEC_INIT_KERNEL:
			{
				...
				{
					dprintk("Dec-%d :: Init Done with ret(0x%x)\n", type, ret);
			...
			case VPU_DEC_FLUSH_OUTPUT_KERNEL:
			{
				...
				dprintk("Dec-%d :: VPU_DEC_FLUSH_OUTPUT !! \n", type);
				ret = tcc_vpu_dec(cmd & ~VPU_BASE_OP_KERNEL, (codec_handle_t*)&pHandle, (void*)(&arg->gsVpuDecInput), (void*)(&arg->gsVpuDecOutput));
			}
			break;
			...
			{
				VDEC_RINGBUF_GETINFO_t* arg = (VDEC_RINGBUF_GETINFO_t*)args;
				vmgr_data.check_interrupt_detection = 1;
			...
			{
				VDEC_RINGBUF_SETBUF_t* arg = (VDEC_RINGBUF_SETBUF_t*)args;
				vmgr_data.check_interrupt_detection = 1;
			...
			{
				VDEC_RINGBUF_SETBUF_PTRONLY_t* arg = (VDEC_RINGBUF_SETBUF_PTRONLY_t*)args;
				vmgr_data.check_interrupt_detection = 1;
			...
			{
				VDEC_SEQ_HEADER_t* arg = (VDEC_SEQ_HEADER_t*)args;
				vmgr_data.check_interrupt_detection = 1;
			...
			{
				VDEC_GET_VERSION_t* arg = (VDEC_GET_VERSION_t*)args;
				vmgr_data.check_interrupt_detection = 1;
...
static long _vmgr_ioctl(struct file* file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd)
	{
		...
		case VPU_CHECK_CODEC_STATUS_KERNEL:
		{
			...
			{
				(void)memcpy((int*)arg, vmgr_data.closed, sizeof(vmgr_data.closed));
			}
			else
			{
		...
		case VPU_SET_RENDERED_FRAMEBUFFER_KERNEL:
		{
			...
			{
				(void)memcpy(&vmgr_data.gsRender_fb_info,(void*)arg,sizeof(VDEC_RENDERED_BUFFER_t));
			}
			else
			{
		...
		case VPU_GET_RENDERED_FRAMEBUFFER_KERNEL:
		{
			...
			{
				(void)memcpy((void*)arg, &vmgr_data.gsRender_fb_info, sizeof(VDEC_RENDERED_BUFFER_t));
			}
			else
			{
...
void vmgr_waitlist_init_pending(int type, int force_clear)
{
	if (IsUseWaitList())
	{
		...
		{
			wait_entry_info.wait_dec_status = 0;
			dprintk("=====> [wait_entry(%d) vs. type(%d)] end waiting with closing VPU (by %s)\n",
