...
static void dwc_hdmi_tx_handler_thread(struct work_struct *work)
{
	uint8_t phy_decode;
	uint32_t decode;
	struct hdmi_tx_dev *dev = (work != NULL) ?
		container_of(work, struct hdmi_tx_dev, tx_handler) : NULL;

	if (dev != NULL) {
		if (dev->verbose >= VERBOSE_IRQ)
			pr_info("[INFO][HDMI_V20] hdmi_tx_handler_thread\r\n");
		...
		if ((decode & IH_DECODE_IH_PHY_MASK) ? 1 : 0) {
			...
			if (
				(phy_decode & IH_PHY_STAT0_TX_PHY_LOCK_MASK) ?
				1 : 0)
				hdmi_irq_clear_bit(
					dev, PHY,
					IH_PHY_STAT0_TX_PHY_LOCK_MASK);

			if ((phy_decode & IH_PHY_STAT0_HPD_MASK) ? 1 : 0) {
				dev->hotplug_real_status =
					hdmi_phy_hot_plug_detected(dev);
				hdmi_irq_clear_bit(
					dev, PHY, IH_PHY_STAT0_HPD_MASK);
				if (dev->verbose >= VERBOSE_IRQ)
					pr_info(
						"[INFO][HDMI_V20] >HPD :%d\r\n",
						dev->hotplug_real_status);
		...
		if ((decode & IH_DECODE_IH_I2CM_STAT0_MASK) ? 1 : 0) {
			...
			if (state & 0x3) {
				hdmi_irq_clear_bit(dev,
					I2C_DDC,
					IH_I2CM_STAT0_I2CMASTERDONE_MASK);
				pr_err(
					"[ERROR][HDMI_V20]%s:I2C DDC 0x%x",
					__func__, state);
			}
			// SCDC_READREQ
			else if (state & 0x4) {
...
static void dwc_hdmi_tx_hotplug_thread(struct work_struct *work)
{
	...
	if (dev != NULL) {
		...
		if (match >= 4) {
			pr_info("[INFO][HDMI_V20]\e[33mhpd=%d \e[0m\r\n",
								current_hpd);
...
dwc_hdmi_tx_hpd_irq_handler(int irq, void *dev_id)
{
	...
	if (dev == NULL) {
		pr_err("[ERROR][HDMI_V20]%s: irq_dev is NULL\r\n", __func__);
		goto end_handler;
	}
	/* disable hpd irq */
...
dwc_init_interrupts(struct hdmi_tx_dev *dev)
{
	...
	if (gpio_is_valid(dev->hotplug_gpio)) {
		if (devm_gpio_request(
			dev->parent_dev, dev->hotplug_gpio,
			"hdmi_hotplug") < 0) {
			pr_err("[ERROR][HDMI_V20]%s gpio request\r\n",
			__func__);
		} else {
			...
			if (dev->hotplug_irq < 0) {
				pr_err("[ERROR][HDMI_V20]%s gpio to irq\r\n",
				__func__);
				ret = -1;
			} else {
				pr_info("[INFO][HDMI_V20]%s gpio hpd (%d)\r\n",
				__func__, dev->hotplug_irq);
				dev->hotplug_status =
				dev->hotplug_real_status =
				gpio_get_value(dev->hotplug_gpio)?1:0;
				/* Disable IRQ auto enable */
				irq_set_status_flags(
					dev->hotplug_irq, IRQ_NOAUTOEN);
				flag = (
					dev->hotplug_real_status ?
					IRQ_TYPE_LEVEL_LOW :
					IRQ_TYPE_LEVEL_HIGH) | IRQF_ONESHOT;
				ret = devm_request_irq(
					dev->parent_dev, dev->hotplug_irq,
			...
			if (ret < 0)
				pr_err("[ERROR][HDMI_V20]%s hpd irq\r\n",
				__func__);
		}
	}
	...
	ret = devm_request_irq(
		dev->parent_dev, dev->irq[HDMI_IRQ_TX_CORE],
		dwc_hdmi_tx_handler, IRQF_SHARED,
		"dwc_hdmi_tx_handler", &dev->irq_dev[HDMI_IRQ_TX_CORE]);
	if (ret)
		pr_err(
			"[ERROR][HDMI_V20]%s link irq\n",
			__func__);

	#if defined(CONFIG_HDMI_USE_CEC_IRQ)
	dev->irq_dev[HDMI_IRQ_TX_CEC].dev = dev;
	ret = devm_request_irq(
			dev->parent_dev, dev->irq[HDMI_IRQ_TX_CEC],
			dwc_hdmi_tx_cec_handler, IRQF_SHARED,
			"dwc_hdmi_tx_cec_handler",
			&dev->irq_dev[HDMI_IRQ_TX_CEC]);
	if (ret)
		pr_err("[ERROR][HDMI_V20]%s cec irq\n",
			__func__);
	#endif
...
dwc_deinit_interrupts(struct hdmi_tx_dev *dev)
{
	...
	for (irq_loop = 0; irq_loop < HDMI_IRQ_TX_MAX; irq_loop++) {
		devm_free_irq(
			dev->parent_dev,
			dev->irq[irq_loop], &dev->irq_dev[irq_loop]);
	}
