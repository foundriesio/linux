...
static int scdc_force_error_stage;
...
{
	return hdmi_ddc_read(
		dev, SCDC_SLAVE_ADDRESS, 0, 0, address, size, data);
}
...
void scdc_set_rr_flag(struct hdmi_tx_dev *dev, u8 enable)
{
	if (hdmi_ddc_write(dev, SCDC_SLAVE_ADDRESS, SCDC_CONFIG_0, 1, &enable))
		LOGGER(
			SNPS_ERROR,
			"%s: SCDC addr 0x%x - 0x%x write failed ",
			__func__, SCDC_CONFIG_0, enable);
}
...
int scdc_get_rr_flag(struct hdmi_tx_dev *dev, u8 *flag)
{
	if (
		hdmi_ddc_read(
			dev,
			SCDC_SLAVE_ADDRESS, 0, 0, SCDC_CONFIG_0, 1, flag)) {
		LOGGER(
			SNPS_ERROR,
			"%s: SCDC addr 0x%x read failed ",
			__func__, SCDC_CONFIG_0);
		return -1;
	}
	return 0;
}
...
static unsigned int scdc_time_diff_ms(
	...
	struct hdmi_scdc_error_data *hdmi_scdc_error_data, int stage)
{
	...
	if (reg_value & 1)
		set_bit(
			SCDC_STATUS_SCRAMBLED,
			&hdmi_scdc_error_data->status);
	...
	if (reg_value & 1)
		set_bit(
			SCDC_STATUS_SET_SCRAMBLE,
			&hdmi_scdc_error_data->status);

	if (reg_value & 2)
		set_bit(
			SCDC_STATUS_SET_TMDS,
			&hdmi_scdc_error_data->status);
	...
	if (reg_value & (1 << 1) ||
		test_bit(
			HDMI_TX_STATUS_SCDC_FORCE_ERROR,
			&dev->status))
	...
	if (
		...
		if ((reg_value >> 0) & 1)
			set_bit(
				SCDC_STATUS_CLK_LOCK,
				&hdmi_scdc_error_data->status);
		/* Channel 0 Lock */
		if ((reg_value >> 1) & 1)
			set_bit(
				SCDC_CHANNEL_LOCK,
				&hdmi_scdc_error_data->ch0);
		/* Channel 2 Lock */
		if ((reg_value >> 2) & 1)
			set_bit(
				SCDC_CHANNEL_LOCK,
				&hdmi_scdc_error_data->ch1);
		/* Channel 2 Lock */
		if ((reg_value >> 3) & 1)
			set_bit(
				SCDC_CHANNEL_LOCK,
				&hdmi_scdc_error_data->ch2);
		...
		if (test_bit(HDMI_TX_STATUS_SCDC_CHECK, &dev->status)) {
			...
			memcpy(
				&prev_status_timeval,
				&current_timeval, sizeof(current_timeval));
			pr_info(
		"[INFO][SCDC]\x1b[33mC%d V%d%d V%d%d V%d%d %dms\x1b[0m\r\n",
				(unsigned int)
					test_bit(
						SCDC_STATUS_CLK_LOCK,
						&hdmi_scdc_error_data->status),
	...
	reg_value =
		(test_bit(
			SCDC_STATUS_SET_SCRAMBLE,
				&hdmi_scdc_error_data->status) << 2) |
		(test_bit(
			SCDC_STATUS_SET_TMDS,
			&hdmi_scdc_error_data->status) << 1) |
		test_bit(
			SCDC_STATUS_SET_SCRAMBLE,
			&hdmi_scdc_error_data->status);
	...
	if (reg_value != dev->prev_scdc_status) {
		...
		if (test_bit(HDMI_TX_STATUS_SCDC_CHECK, &dev->status))
			pr_info(
				"[INFO][SCDC]\x1b[33mCFG_%02d SC_%d\x1b[0m\r\n",
				(unsigned int)
				(test_bit(
					SCDC_STATUS_SET_TMDS,
					&hdmi_scdc_error_data->status) << 1) |
	...
	if (test_bit(SCDC_STATUS_CED, &hdmi_scdc_error_data->status)) {
		...
		if (scdc_read(dev, SCDC_ERR_DET_0_L, 8, scdc_errors) == 0) {
			...
			hdmi_scdc_error_data->ch0 =
			((regl_value | ((regh_value & 0x7F) << 8)) <<
						SCDC_CHANNEL_ERROR_SHIFT);
			if (regh_value >> 7)
				set_bit(
					SCDC_CHANNEL_VALID,
					&hdmi_scdc_error_data->ch0);
			...
			/* Channel 1 */
			regl_value =
				scdc_errors[2]; regh_value = scdc_errors[3];
			if (
				test_bit(
					HDMI_TX_STATUS_SCDC_FORCE_ERROR,
					&dev->status))
			...
			if (
				...
				memcpy(
					&prev_ced_timeval,
					&current_timeval,
					sizeof(current_timeval));
				pr_info(
	"[INFO][SCDC]\x1b[33m CH0_%05x CH1_%05x CH2_%05x %dms\x1b[0m\r\n",
					(unsigned int)((
						hdmi_scdc_error_data->ch0 >>
						SCDC_CHANNEL_ERROR_SHIFT) &
						SCDC_CHANNEL_ERROR_MASK),
					(unsigned int)((
						hdmi_scdc_error_data->ch1 >>
						SCDC_CHANNEL_ERROR_SHIFT) &
						SCDC_CHANNEL_ERROR_MASK),
	...
	if (test_bit(HDMI_TX_STATUS_SCDC_FORCE_ERROR, &dev->status))
		...
		case 1:
			...
			clear_bit(
				HDMI_TX_STATUS_SCDC_FORCE_ERROR, &dev->status);
			break;
		default:
			scdc_force_error_stage = 0;
			break;
	...
	struct hdmi_scdc_error_data *hdmi_scdc_error_data)
{
	...
	if (hdmi_dev_read_mask(dev, FC_GCP, FC_GCP_SET_AVMUTE_MASK))
		scdc_error_detection_core(
			dev, hdmi_scdc_error_data, SCDC_STAGE_MUTE);
	else
		scdc_error_detection_core(
			dev, hdmi_scdc_error_data, SCDC_STAGE_UNMUTE);
	return ret;
}


int scdc_read_sink_version(
	struct hdmi_tx_dev *dev, unsigned int *version)
{
	u8 read_value = 0;
