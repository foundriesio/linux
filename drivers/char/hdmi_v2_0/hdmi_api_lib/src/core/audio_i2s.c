...
void _audio_i2s_reset_fifo(struct hdmi_tx_dev *dev)
{
	LOG_TRACE();
	hdmi_dev_write_mask(
		dev, AUD_CONF0, AUD_CONF0_SW_AUDIO_FIFO_RST_MASK, 1);
}
...
void _audio_i2s_data_enable(struct hdmi_tx_dev *dev, u8 value)
{
	LOG_TRACE1(value);
	hdmi_dev_write_mask(
		dev, AUD_CONF0, AUD_CONF0_I2S_IN_EN_MASK, value);
}
...
void _audio_i2s_data_mode(struct hdmi_tx_dev *dev, u8 value)
{
	LOG_TRACE1(value);
	hdmi_dev_write_mask(
		dev, AUD_CONF1, AUD_CONF1_I2S_MODE_MASK, value);
}
...
void _audio_i2s_data_width(struct hdmi_tx_dev *dev, u8 value)
{
	LOG_TRACE1(value);
	hdmi_dev_write_mask(
		dev, AUD_CONF1, AUD_CONF1_I2S_WIDTH_MASK, value);
}
...
void audio_i2s_select(struct hdmi_tx_dev *dev, u8 bit)
{
	LOG_TRACE1(bit);
	hdmi_dev_write_mask(
		dev, AUD_CONF0, AUD_CONF0_I2S_SELECT_MASK, bit);
}
...
void audio_i2s_interrupt_mask(struct hdmi_tx_dev *dev, u8 value)
{
	LOG_TRACE1(value);
	hdmi_dev_write_mask(
		dev, AUD_INT, AUD_INT_FIFO_FULL_MASK_MASK |
...
void audio_i2s_configure(struct hdmi_tx_dev *dev, audioParams_t *audio)
{
	...
	pr_info(
		"[INFO][HDMI_V20] << [%s] : %s, %s, %s\r\n",
		__func__,
		hdmi_dev_read_mask(dev, AUD_CONF2, AUD_CONF2_HBR_MASK) ?
		"HBR Stream" : "Non HBR",
		hdmi_dev_read_mask(dev, AUD_CONF2, AUD_CONF2_NLPCM_MASK) ?
		"Bitstream Mode" : "PCM Mode",
		hdmi_dev_read_mask(dev, AUD_CONF2, AUD_CONF2_INSERT_PCUV) ?
		"BPCUV Auto" : "BPCUV Manual");
}
