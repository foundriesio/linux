...
void halAudioMultistream_MetaDataPacket_Descriptor_X(
	...
	u8 descrNr, audioMetaDataDescriptor_t *mAudioMetaDataDescriptor)
{
	...
	hdmi_dev_write(
		dev, (FC_AMP_PB + (((0x05 * descrNr) + 0x01)*4)), (u8)(pb));
	if (mAudioMetaDataDescriptor->mLC_Valid == 1) {
		hdmi_dev_write(
			dev, FC_AMP_PB + (((0x05 * descrNr) + 0x02)*4),
			((u8)(mAudioMetaDataDescriptor->mLanguage_Code[0])) &
			0xff);
		hdmi_dev_write(
			dev, FC_AMP_PB + (((0x05 * descrNr) + 0x03)*4),
			((u8)(mAudioMetaDataDescriptor->mLanguage_Code[1])) &
			0xff);
		hdmi_dev_write(
			dev, FC_AMP_PB + (((0x05 * descrNr) + 0x04)*4),
			((u8)(mAudioMetaDataDescriptor->mLanguage_Code[2])) &
			0xff);
	}
	LOGGER(
		SNPS_NOTICE,
		" body AMP descriptor 0 0x%x",
...
void halAudioMultistream_MetaDataPacket_Header(
	struct hdmi_tx_dev *dev, audioMetaDataPacket_t *mAudioMetaDataPckt)
{
	hdmi_dev_write(
		dev, FC_AMP_HB1,
	...
	hdmi_dev_write(
		...
		(mAudioMetaDataPckt->mAudioMetaDataHeader.mNumAudioStreams) <<
		2)));
...
void halAudioMultistream_MetaDataPacketBody(
	struct hdmi_tx_dev *dev, audioMetaDataPacket_t *mAudioMetaDataPacket)
{
	u8 cnt = 0;

	while (
		cnt <=
		(mAudioMetaDataPacket->mAudioMetaDataHeader.mNumAudioStreams +
		1)) {
		LOGGER(
			SNPS_NOTICE,
"(mAudioMetaDataPacket->mAudioMetaDataHeader.mNumAudioStreams + 1) = %d",
		(mAudioMetaDataPacket->mAudioMetaDataHeader.mNumAudioStreams +
		1));
		LOGGER(
			SNPS_NOTICE,
			" audiometadata packet descriptor %d", cnt);
		halAudioMultistream_MetaDataPacket_Descriptor_X(
			dev, 0,
