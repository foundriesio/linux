...
void fc_RgbYcc(struct hdmi_tx_dev *dev, u8 type)
{
	LOG_TRACE1(type);
	hdmi_dev_write_mask(
		dev, FC_AVICONF0, FC_AVICONF0_RGC_YCC_INDICATION_MASK, type);
}
...
void fc_ScanInfo(struct hdmi_tx_dev *dev, u8 left)
{
	LOG_TRACE1(left);
	hdmi_dev_write_mask(
		dev, FC_AVICONF0, FC_AVICONF0_SCAN_INFORMATION_MASK, left);
}

void fc_Colorimetry(struct hdmi_tx_dev *dev, unsigned int cscITU)
{
	LOG_TRACE1(cscITU);
	hdmi_dev_write_mask(
		dev, FC_AVICONF1, FC_AVICONF1_COLORIMETRY_MASK, cscITU);
}
...
void fc_PicAspectRatio(struct hdmi_tx_dev *dev, u8 ar)
{
	LOG_TRACE1(ar);
	hdmi_dev_write_mask(
		dev, FC_AVICONF1, FC_AVICONF1_PICTURE_ASPECT_RATIO_MASK, ar);
}
...
void fc_ActiveFormatAspectRatio(struct hdmi_tx_dev *dev, u8 left)
{
	LOG_TRACE1(left);
	hdmi_dev_write_mask(
		dev, FC_AVICONF1, FC_AVICONF1_ACTIVE_ASPECT_RATIO_MASK, left);
}
...
void fc_IsItContent(struct hdmi_tx_dev *dev, u8 it)
{
	...
	hdmi_dev_write_mask(
		dev, FC_AVICONF2,
		FC_AVICONF2_IT_CONTENT_MASK, ((it & 0xF) ? 1 : 0));
...
void fc_ExtendedColorimetry(struct hdmi_tx_dev *dev, u8 extColor)
{
	...
	hdmi_dev_write_mask(
		dev, FC_AVICONF1, FC_AVICONF1_COLORIMETRY_MASK, 0x3);
}
...
void fc_QuantizationRange(struct hdmi_tx_dev *dev, u8 rgb_range, u8 yuv_range)
{
	...
	hdmi_dev_write_mask(
		dev, FC_AVICONF2,
		FC_AVICONF2_QUANTIZATION_RANGE_MASK, rgb_range);
	if (yuv_range > 1)
		/*
		 * 0 : Limited Range
		 * 1 : Full Range
...
void fc_OutPixelRepetition(struct hdmi_tx_dev *dev, u8 pr)
{
	LOG_TRACE1(pr);
	hdmi_dev_write_mask(
		dev, FC_PRCONF, FC_PRCONF_OUTPUT_PR_FACTOR_MASK, pr);
}
...
int fc_colorimetry_config(struct hdmi_tx_dev *dev, videoParams_t *videoParams)
{
	if (videoParams->mColorimetry == EXTENDED_COLORIMETRY) {
		...
		case BT2020YCCBCR:
		case BT2020YCBCR:
			fc_ExtendedColorimetry(
				dev, videoParams->mExtColorimetry);
			fc_Colorimetry(
				dev, videoParams->mColorimetry); /* EXT-3 */
			break;
		default:
...
void fc_avi_config(struct hdmi_tx_dev *dev, videoParams_t *videoParams)
{
	...
	// Same as Picture Aspect Ratio - Table 10 AVI InfoFrame
	// Data Byte 2 (CEA-861-F)
	fc_ActiveFormatAspectRatio(dev, 0x8);

	LOGGER(
		SNPS_NOTICE,
		"%s:infoframe status %x", __func__, fc_GetInfoFrameSatus(dev));
	...
	fc_QuantizationRange(
		dev, videoParams->mRgbQuantizationRange,
		videoParams->mYuvQuantizationRange);
	fc_NonUniformPicScaling(dev, videoParams->mNonUniformScaling);
	if (dtd->mCode != (u8) (-1)) {
		if (videoParams->mHdmi20 == 1) {
			if (
				videoParams->mHdmiVideoFormat != 2 &&  // 3D
				(dtd->mCode == 93 || dtd->mCode == 94 ||
				dtd->mCode == 95 || dtd->mCode == 98))
				fc_VideoCode(dev, 0);
				// hdmi spec..!!
			else
	...
	if (videoParams->mActiveFormatAspectRatio != 0) {
		fc_ActiveFormatAspectRatio(
			dev, videoParams->mActiveFormatAspectRatio);
		fc_ActiveAspectRatioValid(dev, 1);
	} else {
		fc_ActiveAspectRatioValid(dev, 0);
	}
	if (
		videoParams->mEndTopBar != (u16) (-1) ||
	...
	fc_OutPixelRepetition(
		dev, (dtd->mPixelRepetitionInput + 1) *
