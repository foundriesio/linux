...
//Read only register that holds the "packing phase" output of the Video
//Packetizer block
...
//The Color depth configuration is described as the following, with the
//action stated corresponding to color_depth[3:0]: - 0000b: 24 bits per
//pixel video (8 bits per component)
...
//Controls the default phase packing machine used according to HDMI 1

//Video Packetizer YCC422 Remapping Register
#define VP_REMAP  0x0000200C
#define VP_REMAP_YCC422_SIZE_MASK  0x00000003
//YCC 422 remap input video size ycc422_size[1:0] 00b: YCC 422 16-bit input
//video (8 bits per component) 01b: YCC 422 20-bit input video (10 bits per
//component) 10b: YCC 422 24-bit input video (12 bits per component)
//11b: Reserved
...
//Video Packetizer output selection output_selector[1:0] 00b: Data from pixel
//packing block 01b: Data from YCC 422 remap block 10b: Data from 8-bit bypass
//block 11b: Data from 8-bit bypass block
#define VP_CONF_BYPASS_SELECT_MASK  0x00000004
//bypass_select 0b: Data from pixel repeater block 1b: Data from input of
//Video Packetizer block
...
//Mask bit for Video Packetizer 8-bit bypass FIFO empty
#define VP_MASK_OINTFULLBYP_MASK  0x00000002
//Mask bit for Video Packetizer 8-bit bypass FIFO full
#define VP_MASK_OINTEMPTYREMAP_MASK  0x00000004
//Mask bit for Video Packetizer pixel YCC 422 re-mapper FIFO empty
#define VP_MASK_OINTFULLREMAP_MASK  0x00000008
//Mask bit for Video Packetizer pixel YCC 422 re-mapper FIFO full
#define VP_MASK_OINTEMPTYPP_MASK  0x00000010
//Mask bit for Video Packetizer pixel packing FIFO empty
#define VP_MASK_OINTFULLPP_MASK  0x00000020
//Mask bit for Video Packetizer pixel packing FIFO full
#define VP_MASK_OINTEMPTYREPET_MASK  0x00000040
//Mask bit for Video Packetizer pixel repeater FIFO empty
#define VP_MASK_OINTFULLREPET_MASK  0x00000080
//Mask bit for Video Packetizer pixel repeater FIFO full
