...
void fc_gamut_Profile(struct hdmi_tx_dev *dev, u8 profile)
{
	LOG_TRACE1(profile);
	hdmi_dev_write_mask(
		dev, FC_GMD_HB, FC_GMD_HB_GMDGBD_PROFILE_MASK, profile);
}
...
void fc_gamut_AffectedSeqNo(struct hdmi_tx_dev *dev, u8 no)
{
	LOG_TRACE1(no);
	hdmi_dev_write_mask(
		dev, FC_GMD_HB, FC_GMD_HB_GMDAFFECTED_GAMUT_SEQ_NUM_MASK, no);
}
...
void fc_gamut_PacketsPerFrame(struct hdmi_tx_dev *dev, u8 packets)
{
	LOG_TRACE1(packets);
	hdmi_dev_write_mask(
		dev, FC_GMD_CONF, FC_GMD_CONF_GMDPACKETSINFRAME_MASK, packets);
}
	...
	struct hdmi_tx_dev *dev, const u8 *gbdContent, u8 length)
{
	...
	fc_gamut_AffectedSeqNo(
		dev, (fc_gamut_CurrentSeqNo(dev) + 1) % 16); /* sequential */
