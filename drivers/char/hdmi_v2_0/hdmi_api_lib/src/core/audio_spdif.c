...
void _audio_spdif_reset_fifo(struct hdmi_tx_dev *dev)
{
	LOG_TRACE();
	hdmi_dev_write_mask(
		dev, AUD_SPDIF0, AUD_SPDIF0_SW_AUDIO_FIFO_RST_MASK, 1);
}
...
void _audio_spdif_non_linear_pcm(struct hdmi_tx_dev *dev, u8 bit)
{
	...
	hdmi_dev_write_mask(
		dev, AUD_SPDIF1, AUD_SPDIF1_SPDIF_HBR_MODE_MASK, bit);
}
...
void _audio_spdif_data_width(struct hdmi_tx_dev *dev, u8 value)
{
	LOG_TRACE1(value);
	hdmi_dev_write_mask(
		dev, AUD_SPDIF1, AUD_SPDIF1_SPDIF_WIDTH_MASK, value);
}
...
void audio_spdif_interrupt_mask(struct hdmi_tx_dev *dev, u8 value)
{
	...
	hdmi_dev_write_mask(
		dev,
		AUD_SPDIFINT, AUD_SPDIFINT_SPDIF_FIFO_FULL_MASK_MASK |
