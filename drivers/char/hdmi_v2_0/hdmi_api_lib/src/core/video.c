...
int video_Configure(struct hdmi_tx_dev *dev, videoParams_t *video)
{
	...
	if ((video->mHdmi == DVI) &&
		...
		LOGGER(
			SNPS_ERROR,
			"DVI with pixel repetition: video not transmitted");
		return -1;
	}
	pr_err(
		"[INFO][HDMI_V20]  VIDEO VIC[%d] %s - %dBIT\r\n",
		video->mDtd.mCode, (video->mEncodingOut == RGB) ? "RGB" :
		(video->mEncodingOut == YCC444) ? "YCC444" :
		(video->mEncodingOut == YCC422) ? "YCC422" :
		(video->mEncodingOut == YCC420) ? "YCC420" :
		"UNKNOWN", (video->mColorResolution * 3));
...
int video_ColorSpaceConverter(struct hdmi_tx_dev *dev, videoParams_t *video)
{
	...
	if (videoParams_IsColorSpaceInterpolation(dev, video)) {
		if (video->mCscFilter > 1) {
			LOGGER(SNPS_ERROR,
				"invalid chroma interpolation filter: %d",
				video->mCscFilter);
			return -1;
		}
		...
		if (video->mCscFilter > 2) {
			LOGGER(
				SNPS_ERROR,
				"invalid chroma decimation filter: %d",
				video->mCscFilter);
			return -1;
		}
	...
	if ((video->mColorResolution == COLOR_DEPTH_8) ||
		...
		LOGGER(
			SNPS_ERROR,
			"invalid color depth: %d", video->mColorResolution);
	return -1;
	}
...
int video_VideoPacketizer(struct hdmi_tx_dev *dev, videoParams_t *video)
{
	...
	if (
		...
		LOGGER(
			SNPS_ERROR,
			"invalid output encoding type: %d",
			video->mEncodingOut);
		return -1;
	}
