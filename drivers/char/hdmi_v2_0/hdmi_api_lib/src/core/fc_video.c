...
void fc_video_hdcp_keepout(struct hdmi_tx_dev *dev, u8 bit)
{
	LOG_TRACE1(bit);
	hdmi_dev_write_mask(
		dev, FC_INVIDCONF, FC_INVIDCONF_HDCP_KEEPOUT_MASK, bit);
}
...
void fc_video_VSyncPolarity(struct hdmi_tx_dev *dev, u8 bit)
{
	LOG_TRACE1(bit);
	hdmi_dev_write_mask(
		dev, FC_INVIDCONF, FC_INVIDCONF_VSYNC_IN_POLARITY_MASK, bit);
}
...
void fc_video_HSyncPolarity(struct hdmi_tx_dev *dev, u8 bit)
{
	LOG_TRACE1(bit);
	hdmi_dev_write_mask(
		dev, FC_INVIDCONF, FC_INVIDCONF_HSYNC_IN_POLARITY_MASK, bit);
}
...
void fc_video_DataEnablePolarity(struct hdmi_tx_dev *dev, u8 bit)
{
	LOG_TRACE1(bit);
	hdmi_dev_write_mask(
		dev, FC_INVIDCONF, FC_INVIDCONF_DE_IN_POLARITY_MASK, bit);
}
...
void fc_video_DviOrHdmi(struct hdmi_tx_dev *dev, u8 bit)
{
	...
	/* 1: HDMI; 0: DVI */
	hdmi_dev_write_mask(
		dev, FC_INVIDCONF, FC_INVIDCONF_DVI_MODEZ_MASK, bit);
}
...
void fc_video_VBlankOsc(struct hdmi_tx_dev *dev, u8 bit)
{
	LOG_TRACE1(bit);
	hdmi_dev_write_mask(
		dev, FC_INVIDCONF, FC_INVIDCONF_R_V_BLANK_IN_OSC_MASK, bit);
}
...
void fc_video_HActive(struct hdmi_tx_dev *dev, u16 value)
{
	...
	hdmi_dev_write_mask(
		dev, FC_INHACTIV1, FC_INHACTIV1_H_IN_ACTIV_MASK |
...
void fc_video_HBlank(struct hdmi_tx_dev *dev, u16 value)
{
	...
	hdmi_dev_write_mask(
		dev, FC_INHBLANK1, FC_INHBLANK1_H_IN_BLANK_MASK |
...
void fc_video_VActive(struct hdmi_tx_dev *dev, u16 value)
{
	...
	hdmi_dev_write_mask(
		dev, FC_INVACTIV1, FC_INVACTIV1_V_IN_ACTIV_MASK |
...
void fc_video_HSyncEdgeDelay(struct hdmi_tx_dev *dev, u16 value)
{
	...
	hdmi_dev_write_mask(
		dev, FC_HSYNCINDELAY1, FC_HSYNCINDELAY1_H_IN_DELAY_MASK |
...
void fc_video_HSyncPulseWidth(struct hdmi_tx_dev *dev, u16 value)
{
	...
	hdmi_dev_write_mask(
		dev, FC_HSYNCINWIDTH1,
		FC_HSYNCINWIDTH1_H_IN_WIDTH_MASK, (u8)(value >> 8));
}
...
void fc_video_VSyncPulseWidth(struct hdmi_tx_dev *dev, u16 value)
{
	...
	hdmi_dev_write_mask(
		dev, FC_VSYNCINWIDTH,
		FC_VSYNCINWIDTH_V_IN_WIDTH_MASK, (u8)(value));
}
...
void fc_video_RefreshRate(struct hdmi_tx_dev *dev, u32 value)
{
	...
	hdmi_dev_write_mask(
		dev, FC_INFREQ2, FC_INFREQ2_INFREQ_MASK, (u8)(value >> 16));
}
...
void fc_video_ExtendedControlPeriodMinDuration(
			struct hdmi_tx_dev *dev, u8 value)
{
	LOG_TRACE1(value);
...
void fc_video_PixelRepetitionInput(struct hdmi_tx_dev *dev, u8 value)
{
	LOG_TRACE1(value);
	hdmi_dev_write_mask(
		dev, FC_PRCONF, FC_PRCONF_INCOMING_PR_FACTOR_MASK, value);
}
...
void fc_video_timing_dump(struct hdmi_tx_dev *dev)
{
	...
	pr_info("[INFO][HDMI_V20] Timing Dump\r\n");
	reg = hdmi_dev_read(dev, (FC_INVIDCONF));
	pr_info(
		"[INFO][HDMI_V20]  VSYNC(%s), HSYNC(%s), DE(%s) %smode\r\n",
		(reg & (1 << 6)) ? "High" : "Low",
		(reg & (1 << 5)) ? "High" : "Low",
		(reg & (1 << 4)) ? "High" : "Low",
		(reg & (1 << 3)) ? "HDMI" : "DVI");
	...
	reg &=
		(
			FC_HSYNCINDELAY1_H_IN_DELAY_MASK |
			FC_HSYNCINDELAY1_H_IN_DELAY_12_MASK);
	val1 |= (reg << 8);
	...
	pr_info(
		"[INFO][HDMI_V20] HA(%d), DELAY(%d), SYNC(%d), BLANK(%d)\r\n",
		val, val1, val2, val3);
	...
	pr_info(
		"[INFO][HDMI_V20] VA(%d), DELAY(%d), SYNC(%d), BLANK(%d)\r\n",
		val, val1, val2, val3);
}
...
int fc_video_config_timing(struct hdmi_tx_dev *dev, videoParams_t *video)
{
	...
	if ((dev == NULL) || (video == NULL)) {
		pr_err(
			"[ERROR][HDMI_V20]%s Invalid param: dev=%p; video=%p",
			__func__, dev, video);
		return FALSE;
	}
	...
	fc_video_DataEnablePolarity(
			dev, dev->hdmi_tx_ctrl.data_enable_polarity);
	fc_video_DviOrHdmi(dev, video->mHdmi);

	if (video->mHdmiVideoFormat == 2) {
		if (video->m3dStructure == 0) {
			/*
			 * 3d data frame packing is transmitted as a
			 * progressive format
			 */
