...
u8 product_SetProductName(
	struct hdmi_tx_dev *dev,
	productParams_t *product, const u8 *data, u8 length)
{
	u16 i;
...
u8 product_IsSourceProductValid(
	struct hdmi_tx_dev *dev, productParams_t *product)
{
	return (product->mSourceType != (u8)(-1)) &&
		 (product->mVendorNameLength != 0) &&
		  (product->mProductNameLength != 0);
}

u8 product_IsVendorSpecificValid(
	struct hdmi_tx_dev *dev, productParams_t *product)
{
