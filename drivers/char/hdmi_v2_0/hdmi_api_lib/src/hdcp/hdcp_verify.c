...
void sha_input(struct hdmi_tx_dev *dev, sha_t *sha, const u8 *data, size_t size)
{
	...
	int rc = TRUE;
	if (data == 0 || size == 0) {
		LOGGER(SNPS_ERROR, "invalid input data");
		return;
	}
	...
	while (size-- && sha->mCorrupted == FALSE) {
		...
		for (i = 0; i < 8; i++) {
			...
			sha->mCorrupted = (sha->mCorrupted == TRUE || rc == TRUE) ? TRUE : FALSE;
		}
...
int hdcp_compute_inv(struct hdmi_tx_dev *dev, u8 *out, const u8 *z, const u8 *a, size_t n)
{
	...
	if ((n > SIZE) || (hdcp_array_tst(dev, z, 0, n) == TRUE)
		|| (hdcp_array_tst(dev, a, 0, n) == TRUE) || (hdcp_array_cmp(dev, z, a, n) >= 0)) {
		LOGGER(SNPS_ERROR, "invalid input data");
		return FALSE;
	}
...
int hdcp_verify_ksv(struct hdmi_tx_dev *dev, const u8 *data, size_t size)
{
	...
	if (sha_result(dev, &sha) == FALSE) {
		LOGGER(SNPS_ERROR, "cannot process SHA digest");
		return FALSE;
	}

	for (i = 0; i < SHAMAX; i++) {
		if (data[size - SHAMAX + i] != (u8)(sha.mDigest[i / 4] >> ((i % 4) * 8))) {
			LOGGER(SNPS_ERROR, "SHA digest does not match");
			return FALSE;
		}
	}
	return TRUE;
...
int hdcp_verify_srm(struct hdmi_tx_dev *dev, const u8 *data, size_t size)
{
	...
	/* M, n, r, s */
	return hdcp_verify_dsa(
		dev, data, size - 2 * DSAMAX, &data[size - 2 * DSAMAX], &data[size - DSAMAX]);
}
...
int hdcp_verify_dsa(struct hdmi_tx_dev *dev, const u8 *M, size_t n, const u8 *r, const u8 *s)
{
	int i = 0;
	sha_t sha;
	static const u8 q[] = {0xE7, 0x08, 0xC7, 0xF9, 0x4D, 0x3F, 0xEF, 0x97, 0xE2, 0x14,
						   0x6D, 0xCD, 0x6A, 0xB5, 0x6D, 0x5E, 0xCE, 0xF2, 0x8A, 0xEE};
	static const u8 p[] = {
		};
	static const u8 g[] = {
		};
#ifndef FACSIMILE
	static const u8 y[] = {
		};
#else
	static const u8 y[] = {
		0x46, 0xB9, 0xC2, 0xE5, 0xBE, 0x57, 0x3B, 0xA6, 0x22, 0x7B, 0xAA, 0x83, 0x81, 0xA9, 0xD2,
		0x0F, 0x03, 0x2E, 0x0B, 0x70, 0xAC, 0x96, 0x42, 0x85, 0x4E, 0x78, 0x8A, 0xDF, 0x65, 0x35,
		0x97, 0x6D, 0xE1, 0x8D, 0xD1, 0x7E, 0xA3, 0x83, 0xCA, 0x0F, 0xB5, 0x8E, 0xA4, 0x11, 0xFA,
		0x14, 0x6D, 0xB1, 0x0A, 0xCC, 0x5D, 0xFF, 0xC0, 0x8C, 0xD8, 0xB1, 0xE6, 0x95, 0x72, 0x2E,
		0xBD, 0x7C, 0x85, 0xDE, 0xE8, 0x52, 0x69, 0x92, 0xA0, 0x22, 0xF7, 0x01, 0xCD, 0x79, 0xAF,
		0x94, 0x83, 0x2E, 0x01, 0x1C, 0xD7, 0xEF, 0x86, 0x97, 0xA3, 0xBB, 0xCB, 0x64, 0xA6, 0xC7,
		0x08, 0x5E, 0x8E, 0x5F, 0x11, 0x0B, 0xC0, 0xE8, 0xD8, 0xDE, 0x47, 0x2E, 0x75, 0xC7, 0xAA,
		0x8C, 0xDC, 0xB7, 0x02, 0xC4, 0xDF, 0x95, 0x31, 0x74, 0xB0, 0x3E, 0xEB, 0x95, 0xDB, 0xB0,
	...
	if (sha_result(dev, &sha) == TRUE) {
		...
		LOGGER(SNPS_ERROR, "cannot digest message");
		return FALSE;
	}
	...
	if (hdcp_compute_exp(dev, gu1, g, u1, p, sizeof(gu1), sizeof(u1)) == FALSE) {
		return FALSE;
	}
	if (hdcp_compute_exp(dev, yu2, y, u2, p, sizeof(yu2), sizeof(u2)) == FALSE) {
		return FALSE;
	}
