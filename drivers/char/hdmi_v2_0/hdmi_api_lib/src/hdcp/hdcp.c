...
{
	hdmi_dev_write_mask(dev, A_HDCPCFG0,
		A_HDCPCFG0_BYPENCRYPTION_MASK, bit);
}
...
{
	// Software reset signal, active by writing a zero and auto cleared to
	// 1 in the following cycle
	hdmi_dev_write_mask(dev, A_HDCPCFG1, A_HDCPCFG1_SWRESET_MASK, 0);
}

void _DisableEncryption(struct hdmi_tx_dev *dev, u8 bit)
{
	hdmi_dev_write_mask(dev, A_HDCPCFG1,
		A_HDCPCFG1_ENCRYPTIONDISABLE_MASK, bit);
}
...
{
	return hdmi_dev_read_mask(dev, A_HDCPOBS0,
		A_HDCPOBS0_SUBSTATEA_MASK | A_HDCPOBS0_STATEA_MASK);
}
...
{
	hdmi_dev_write_mask(dev, A_VIDPOLCFG,
		A_VIDPOLCFG_UNENCRYPTCONF_MASK, value);
}
...
{
	LOG_TRACE();
	hdmi_dev_write_mask(dev, A_KSVMEMCTRL,
		A_KSVMEMCTRL_KSVMEMREQUEST_MASK, bit);
}

u8 _MemoryAccessGranted(struct hdmi_tx_dev *dev)
{
	LOG_TRACE();
	return (u8)((hdmi_dev_read(dev, A_KSVMEMCTRL) &
...
void _AnWrite(struct hdmi_tx_dev *dev, u8 *data)
{
	...
	if (data != 0) {
		...
		for (i = 0; i <= (HDCPREG_AN7 - HDCPREG_AN0); i++) {
			hdmi_dev_write(dev, (HDCPREG_AN0 + (i * ADDR_JUMP)),
				data[i]);
		}
		hdmi_dev_write_mask(dev, HDCPREG_ANCONF,
			HDCPREG_ANCONF_OANBYPASS_MASK, 1);
	} else {
		hdmi_dev_write_mask(dev, HDCPREG_ANCONF,
			HDCPREG_ANCONF_OANBYPASS_MASK, 0);
	}
}
...
{
	hdmi_dev_write_mask(dev, HDCP22REG_CTRL,
		HDCP22REG_CTRL_HDCP22_SWITCH_LCK_MASK, 0);
	hdmi_dev_write_mask(dev, HDCP22REG_CTRL,
...
{
	hdmi_dev_write_mask(dev, HDCP22REG_CTRL,
		HDCP22REG_CTRL_HPD_OVR_EN_MASK, 1);
	hdmi_dev_write_mask(dev, HDCP22REG_CTRL,
		HDCP22REG_CTRL_HPD_OVR_VAL_MASK, 1);
	hdmi_dev_write_mask(dev, HDCP22REG_CTRL,
		HDCP22REG_CTRL_HDCP22_SWITCH_LCK_MASK, 0);
	hdmi_dev_write_mask(dev, HDCP22REG_CTRL,
		HDCP22REG_CTRL_HDCP22_OVR_EN_MASK, 1);
	hdmi_dev_write_mask(dev, HDCP22REG_CTRL,
		HDCP22REG_CTRL_HDCP22_OVR_VAL_MASK, 1);
	return 0;
}
...
int hdcp1p4Configure(struct hdmi_tx_dev *dev, hdcpParams_t *hdcp)
{
	...
	if (id_hdcp22_support(dev) == FALSE) {
		LOGGER(SNPS_DEBUG,
			"HDCP 2.2 SNPS is not present (HDCP 1.4 support only)");
		hdcp_2p2 = 0;
	} else {
		LOGGER(SNPS_DEBUG,
			"HDCP 2.2 SNPS is present (both HDCP 1.4 and HDCP 2.2 versions supported)");
		hdcp_2p2 = 1;
	}

	// 3 - Select DVI or HDMI mode
	LOGGER(SNPS_DEBUG, "Set HDCP %s", mode == HDMI ? "HDMI" : "DVI");
	hdmi_dev_write_mask(dev, A_HDCPCFG0, A_HDCPCFG0_HDMIDVI_MASK,
		(mode == HDMI) ? 1 : 0);
	...
	_DataEnablePolarity(dev,
		(dev->hdmi_tx_ctrl.data_enable_polarity > 0) ? 1 : 0);

	// a. If HDCP2Version is 0x00, write “0” to mc_opctrl.h22s_ovr_val
...
void hdcp1p4Start(struct hdmi_tx_dev *dev, hdcpParams_t *hdcp)
{
	...
	// 11 - Configure Device Private Keys (required when
	//	DWC_HDMI_HDCP_DPK_ROMLESS configuration parameter is set to True
	//	[1]), which is illustrated in Figure 3-7 on page 82.
	//	For required memory contents, refer to the “HDCP DPK 56-bit
	//	Memory Mapping” table in Chapter 2 of the DesignWare HDMI
...
void hdcp2p2Stop(struct hdmi_tx_dev *dev)
{
	...
	if (interrupt_status == 0) {
		...
		// pr_info("HDCP_IDLE - interrupt_status : %d\n",
		//	interrupt_status);
	}
}
...
u8 hdcp22_event_handler(struct hdmi_tx_dev *dev, int *param)
{
	...
	if (interrupt_status & HDCP22REG_STAT_ST_HDCP2_NOT_CAPABLE_MASK) {
		...
		pr_info("%s:%s\n", __func__,
			"HDCP22REG_STAT_ST_HDCP2_NOT_CAPABLE");
	}
	if (interrupt_status &
	    HDCP22REG_STAT_ST_HDCP_AUTHENTICATION_LOST_MASK) {
		dev->hdcp_status = HDCP2_AUTHENTICATION_LOST;
		pr_info("%s:%s\n", __func__,
			"HDCP22REG_STAT_ST_HDCP_AUTHENTICATION_LOST");
		if (dev->hotplug_status == 0 ||
		    hdmi_phy_get_rx_sense_status(dev) == 0) {
			pr_info("%s:%s, hpd : %d, rxsense : %d\n", __func__,
				"HDCP_IDLE", dev->hotplug_status, dev->rxsense);
			dev->hdcp_status = HDCP_IDLE;
		}
	}
	if (interrupt_status & HDCP22REG_STAT_ST_HDCP_AUTHENTICATED_MASK) {
		...
		pr_info("%s:%s\n", __func__,
			"HDCP22REG_STAT_ST_HDCP_AUTHENTICATED");
	}
	if (interrupt_status &
	    HDCP22REG_STAT_ST_HDCP_AUTHENTICATION_FAIL_MASK) {
		dev->hdcp_status = HDCP2_AUTHENTICATION_FAIL;
		pr_info("%s:%s\n", __func__,
			"HDCP22REG_STAT_ST_HDCP_AUTHENTICATION_FAIL");
		if (dev->hotplug_status == 0 ||
		    hdmi_phy_get_rx_sense_status(dev) == 0) {
			pr_info("%s:%s, hpd : %d, rxsense : %d\n", __func__,
				"HDCP_IDLE", dev->hotplug_status, dev->rxsense);
	...
	if (interrupt_status & HDCP22REG_STAT_ST_HDCP_DECRYPTED_CHG_MASK) {
		pr_info("%s:%s\n", __func__,
			"HDCP22REG_STAT_ST_HDCP_DECRYPTED_CHG");
...
u8 hdcp_event_handler(struct hdmi_tx_dev *dev, int *param)
{
	...
	if (dev->hotplug_status == 0 ||
		...
		if (interrupt_status & INT_KSV_SHA1_DONE) {
			pr_info("INT_KSV_SHA1_DONE - Status %d\n",
		...
		if ((interrupt_status & INT_HDCP_FAIL) != 0) {
			*param = 0;
			if (hdmi_dev_read_mask(dev, A_HDCPCFG1,
			    A_HDCPCFG1_ENCRYPTIONDISABLE_MASK) == 0) {
				_DisableEncryption(dev, TRUE);
			}
			pr_info("HDCP 1.4 Authentication process - %s\n",
				"HDCP_FAILED");
		...
		if ((interrupt_status & INT_HDCP_ENGAGED) != 0) {
			*param = 1;
			if (hdmi_dev_read_mask(dev, A_HDCPCFG1,
			    A_HDCPCFG1_ENCRYPTIONDISABLE_MASK) == 1) {
...
void hdcp_av_mute(struct hdmi_tx_dev *dev, int enable)
{
	...
	if (dev->hdmi_tx_ctrl.hdcp_on == 1) {
		...
		hdmi_dev_write_mask(dev, HDCP22REG_CTRL1,
			HDCP22REG_CTRL1_HDCP22_AVMUTE_OVR_EN_MASK, 1);
		hdmi_dev_write_mask(dev, HDCP22REG_CTRL1,
			HDCP22REG_CTRL1_HDCP22_AVMUTE_OVR_VAL_MASK, enable);
	}
#endif
}
