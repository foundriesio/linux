...
#if defined(DBG_I2C)
#define dpr_info(args...) pr_info("[INFO][HDMI_V20]" args)
#else
 ...
 */
static u16 _scl_calc(u16 sfrclk, u16 scl_min)
{
	/* temporary scl period */
	unsigned long tmp_priod = 0;

	if (((sfrclk * scl_min) % I2C_DIV_FACTOR) != 0)
		tmp_priod = (unsigned long)(
			(sfrclk * scl_min) + (
				I2C_DIV_FACTOR -
				((sfrclk * scl_min) % I2C_DIV_FACTOR))
			) / I2C_DIV_FACTOR;
	else
		tmp_priod = (unsigned long)(sfrclk * scl_min) / I2C_DIV_FACTOR;
	return (u16)(tmp_priod);
}
...
static int _write(struct hdmi_tx_dev *dev, u8 i2cAddr, u8 addr, u8 data)
{
	...
	hdmi_dev_write_mask(
		dev, I2CM_SLAVE, I2CM_SLAVE_SLAVEADDR_MASK, i2cAddr);
...
static int _read(
	...
	u8 i2cAddr, u8 segment, u8 pointer, u8 addr,   u8 *value)
{
	...
	hdmi_dev_write_mask(
		dev, I2CM_SLAVE,
		I2CM_SLAVE_SLAVEADDR_MASK, i2cAddr);
	hdmi_dev_write(dev, I2CM_ADDRESS, addr);
	...
	if (status & IH_I2CM_STAT0_I2CMASTERERROR_MASK) {
		LOGGER(
			SNPS_NOTICE,
			"%s Read i2cAddr 0x%x seg 0x%x pointer 0x%x addr 0x%x",
	...
	u8 i2cAddr, u8 segment, u8 pointer, u8 addr, u8 *value)
{
	...
	#if defined(DBG_I2C)
	pr_info(
		"[INFO][HDMI_V20]I2CM_SLAVE = 0x%x\r\n",
		hdmi_dev_read(dev, I2CM_SLAVE));
	pr_info(
		"[INFO][HDMI_V20]I2CM_SEGADDR = 0x%x\r\n",
		hdmi_dev_read(dev, I2CM_SEGADDR));
	pr_info(
		"[INFO][HDMI_V20]I2CM_SEGPTR = 0x%x\r\n",
		hdmi_dev_read(dev, I2CM_SEGPTR));
	pr_info(
		"[INFO][HDMI_V20]I2CM_ADDRESS = 0x%x\r\n",
		hdmi_dev_read(dev, I2CM_ADDRESS));
	pr_info(
		"[INFO][HDMI_V20]IH_I2CM_STAT0 = 0x%x\r\n",
		hdmi_dev_read(dev, IH_I2CM_STAT0));
	#endif

	if (pointer)
		hdmi_dev_write(
			dev, I2CM_OPERATION, I2CM_OPERATION_READ_SEQ_EXT);
	else
	...
	if (status & IH_I2CM_STAT0_I2CMASTERDONE_MASK) {
		...
		while (i < 8) { //read 8 bytes
			value[i] = (u8)hdmi_dev_read(
					dev, I2CM_READ_BUFF0 + (4 * i));
			i += 1;
		}
		result = 0;
	}
...
void hdmi_i2cddc_clk_config(
	struct hdmi_tx_dev *dev,
	u16 sfrclk, u16 ss_low_ckl,
	u16 ss_high_ckl, u16 fs_low_ckl, u16 fs_high_ckl)
{
	if (dev != NULL && !dev->ddc_disable) {
		_standard_speed_low_clk_ctrl(
			dev, _scl_calc(sfrclk, ss_low_ckl));
		_standard_speed_high_clk_ctrl(
			dev, _scl_calc(sfrclk, ss_high_ckl));
		_fast_speed_low_clk_ctrl(dev, _scl_calc(sfrclk, fs_low_ckl));
		_fast_speed_high_clk_ctrl(dev, _scl_calc(sfrclk, fs_high_ckl));
	}
}
...
void hdmi_i2cddc_fast_mode(struct hdmi_tx_dev *dev, u8 value)
{
	if (dev != NULL && !dev->ddc_disable)
		...
		hdmi_dev_write_mask(
			dev, I2CM_DIV, I2CM_DIV_FAST_STD_MODE_MASK, value);
}
...
void hdmi_i2cddc_sda_hold(struct hdmi_tx_dev *dev, int hold)
{
	if (dev != NULL && !dev->ddc_disable)
		hdmi_dev_write_mask(
			dev, I2CM_SDA_HOLD, I2CM_SDA_HOLD_OSDA_HOLD_MASK, hold);
}
...
int hdmi_ddc_read(
	struct hdmi_tx_dev *dev,
	u8 i2cAddr, u8 segment, u8 pointer, u8 addr, u8 len, u8 *data)
{
	int tries;
	int i, status = -1;
...
int hdmi_ddc_check(struct hdmi_tx_dev *dev, int addr, int len)
{
	...
	if (dev != NULL && !dev->ddc_disable) {
		...
		if (len > 0) {
			ret = hdmi_ddc_read(
				dev, (0xA0) >> 1, 0x0, 0x0, addr, len, data);

			pr_info("[INFO][HDMI_V20][0x%03x] ", addr);
			for (i = 0; i < len; i++)
				pr_info("[INFO][HDMI_V20]0x%02x ", data[i]);
			pr_info("[INFO][HDMI_V20]\r\n");
		}
	}
	return ret;
}
