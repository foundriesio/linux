...
int dwc_hdmi_is_suspended(struct hdmi_tx_dev *dev)
{
	...
	if (dev != NULL)
		suspend_status = (dev->status &
			(
				(1 << HDMI_TX_STATUS_SUSPEND_L1) |
				(1 << HDMI_TX_STATUS_SUSPEND_L0))) ? 1 : 0;
...
void dwc_hdmi_hw_reset(struct hdmi_tx_dev *dev, int reset_on)
{
	...
	if (test_bit(HDMI_TX_STATUS_SUSPEND_L1, &dev->status)) {
		pr_info("[INFO][HDMI_V20] %s line(%d) ## linke suspended\n",
			__func__, __LINE__);
		return;
	}
	...
	if (reset_on) {
		...
		if (!test_bit(HDMI_TX_STATUS_PHY_ALIVE, &dev->status)) {
			...
			pr_info(
				"[INFO][HDMI_V20]%s phy only alived\n",
				__func__);
		}
	} else {
		if (!test_bit(HDMI_TX_STATUS_PHY_ALIVE, &dev->status)) {
		...
		VIOC_DDICONFIG_Set_tmds_bit_order(
			dev->ddibus_io, HDMI_CTRL_TB_VAL);
		#endif
 ...
 * @details		This function controls blocks related with display
 *			device such as hdmi phy, dibus
 * @date		2018.05.14
 */
static void dwc_hdmi_phy_power_on(struct hdmi_tx_dev *dev)
{
	...
	if (++dev->display_clock_enable_count == 1 &&
		...
		if (dev->verbose >= VERBOSE_IO)
			pr_info(
				"[INFO][HDMI_V20] %s enable display clock\n",
				__func__);
		...
		if (dwc_hdmi_phy_use_peri_clock(dev)) {
			...
			if (!IS_ERR(dev->clk[HDMI_CLK_INDEX_PERI])) {
				clk_set_rate(
					dev->clk[HDMI_CLK_INDEX_PERI],
					HDMI_PHY_REF_CLK_RATE);
				clk_prepare_enable(
					dev->clk[HDMI_CLK_INDEX_PERI]);
				if (dev->verbose >= VERBOSE_IO)
					pr_info(
						"[INFO][HDMI_V20]%s PHY %dHz\n",
						__func__,
						(int)clk_get_rate(
						dev->clk[HDMI_CLK_INDEX_PERI]));
			}
		...
		if (!IS_ERR(dev->clk[HDMI_CLK_INDEX_APB])) {
			clk_set_rate(
				dev->clk[HDMI_CLK_INDEX_APB],
						dev->clk_freq_apb);
			clk_prepare_enable(dev->clk[HDMI_CLK_INDEX_APB]);
			if (dev->verbose >= VERBOSE_IO)
				pr_info(
					"[INFO][HDMI_V20]%s PCLK is %dHz\n",
					__func__,
					(int)clk_get_rate(
						dev->clk[HDMI_CLK_INDEX_APB]));
		}
	...
	if (dev->verbose >= VERBOSE_IO)
		pr_info(
			"[INFO][HDMI_V20] %s display_clock_enable_count=%d\n",
...
static void dwc_hdmi_phy_power_off(struct hdmi_tx_dev *dev)
{
	if (dev->display_clock_enable_count == 1) {
		...
		if (!test_bit(HDMI_TX_STATUS_SUSPEND_L1, &dev->status)) {
			if (dev->verbose >= VERBOSE_IO)
				pr_info(
					"[INFO][HDMI_V20] %s disable pclk\n",
					__func__);
			if (!IS_ERR(dev->clk[HDMI_CLK_INDEX_ISOIP]))
				clk_disable_unprepare(
					dev->clk[HDMI_CLK_INDEX_ISOIP]);
			if (dwc_hdmi_phy_use_peri_clock(dev))
				if (!IS_ERR(dev->clk[HDMI_CLK_INDEX_PERI]))
					clk_disable_unprepare(
						dev->clk[HDMI_CLK_INDEX_PERI]);
...
static void dwc_hdmi_link_power_on_core(
	struct hdmi_tx_dev *dev, int need_link_reset)
{
	#if defined(CONFIG_TCC_HDMI_TIME_PROFILE)
	struct timeval prev_val, curr_val;
	...
	if (++dev->hdmi_clock_enable_count == 1 &&
		...
		if (dev->verbose >= VERBOSE_IO)
			pr_info(
				"[INFO][HDMI_V20] %s enable hdmi clock\n",
				__func__);

		if (!IS_ERR(dev->clk[HDMI_CLK_INDEX_SPDIF])) {
			clk_set_rate(
				dev->clk[HDMI_CLK_INDEX_SPDIF],
				HDMI_SPDIF_REF_CLK_RATE);
			clk_prepare_enable(dev->clk[HDMI_CLK_INDEX_SPDIF]);
			if (dev->verbose >= VERBOSE_IO)
				pr_info(
					"[INFO][HDMI_V20]%s SPDIF is %dHz\n",
					__func__,
					(int)clk_get_rate(
					dev->clk[HDMI_CLK_INDEX_SPDIF]));
		}
		...
		if (!IS_ERR(dev->clk[HDMI_CLK_INDEX_HDCP14])) {
			clk_set_rate(
				dev->clk[HDMI_CLK_INDEX_HDCP14],
				HDMI_HDCP14_CLK_RATE);
			clk_prepare_enable(dev->clk[HDMI_CLK_INDEX_HDCP14]);
			if (dev->verbose >= VERBOSE_IO)
				pr_info(
					"[INFO][HDMI_V20]%s HDCP14 is %dHz\n",
					__func__,
					(int)clk_get_rate(
					dev->clk[HDMI_CLK_INDEX_HDCP14]));
		}
		...
		if (!IS_ERR(dev->clk[HDMI_CLK_INDEX_HDCP22])) {
			clk_set_rate(
				dev->clk[HDMI_CLK_INDEX_HDCP22],
				HDMI_HDCP22_CLK_RATE);
			clk_prepare_enable(dev->clk[HDMI_CLK_INDEX_HDCP22]);
			if (dev->verbose >= VERBOSE_IO)
				pr_info(
					"[INFO][HDMI_V20]%s HDCP22 is %dHz\n",
					__func__,
					(int)clk_get_rate(
					dev->clk[HDMI_CLK_INDEX_HDCP22]));
		}
		// Select REF - PHY 24MHz
		VIOC_DDICONFIG_Set_refclock(
			dev->ddibus_io,
			dev->hdmi_ref_src_clk << HDMI_CTRL_REF_SHIFT);
	...
	if (dev->verbose >= VERBOSE_IO)
		pr_info(
			"[INFO][HDMI_V20] %s hdmi_clock_enable_count=%d\n",
...
void dwc_hdmi_set_hdcp_keepout(struct hdmi_tx_dev *dev)
{
	...
	if (dev->hdmi_tx_ctrl.hdcp_on ||
		...
		/*
		 * When the HDMI 2.0 support is enabled
		 * (DWC_HDMI_TX_20 parameter is set), all
		 * output video modes with a TMDS frequency
		 * higher than 340 MHz require that the
		 * Scrambler feature is activated by setting
		 * the field fc_invidconf.HDCP_keepout register
		 * to 1`b1 and field
		 * fc_scramber_ctrl.scrambler_en register to
		 * 1`b1.
		 */
...
encoding_t hdmi_get_current_output_encoding(struct hdmi_tx_dev *dev)
{
	...
	type = hdmi_dev_read_mask(
		dev, FC_AVICONF0, FC_AVICONF0_RGC_YCC_INDICATION_MASK);
	switch (type) {
	case 1:
		encoding = YCC422;
		break;
...
color_depth_t hdmi_get_current_output_depth(
	struct hdmi_tx_dev *dev, encoding_t encoding)
{
	...
	reg_color_depth = hdmi_dev_read_mask(
				dev, VP_PR_CD, VP_PR_CD_COLOR_DEPTH_MASK);
	reg_remap_size = hdmi_dev_read_mask(
				dev, VP_REMAP, VP_REMAP_YCC422_SIZE_MASK);
	if (encoding == YCC422)
...
video_mode_t hdmi_get_current_output_mode(struct hdmi_tx_dev *dev)
{
	...
	if (dev != NULL) {
		video_mode =
		hdmi_dev_read_mask(
			dev, FC_INVIDCONF, FC_INVIDCONF_DVI_MODEZ_MASK);
	}
	return video_mode;
}
...
unsigned int hdmi_get_current_output_vic(struct hdmi_tx_dev *dev)
{
	...
	if (!test_bit(HDMI_TX_STATUS_SUSPEND_L1, &dev->status)) {
		if (test_bit(HDMI_TX_STATUS_POWER_ON, &dev->status)) {
			vic = hdmi_dev_read_mask(
				dev, FC_AVIVID, FC_AVIVID_FC_AVIVID_MASK);
			if (vic == 0) {
				hdmi_video_format =
					hdmi_dev_read(dev, FC_VSDPAYLOAD0);
				hdmi_vic =
					hdmi_dev_read(dev, FC_VSDPAYLOAD0 + 4);
				hdmi_video_format >>= 5;
				if (hdmi_video_format == 1)
					vic =
					videoParams_GetCeaVicCode(hdmi_vic);
			}
		} else {
			pr_err(
				"[ERROR][HDMI_V20]%s No power <%d>\n",
				__func__, __LINE__);
		}
	} else {
		pr_err(
			"[ERROR][HDMI_V20] %s line(%d) ## linke suspended\n",
			__func__, __LINE__);
	}
	...
	if (pdev != NULL) {
		...
		case FB_BLANK_UNBLANK:
			if (pdev->power.usage_count.counter != 1)
				/*
				 * usage_count = 1 ( resume ), blank_mode = 0
				 * ( FB_BLANK_UNBLANK ) means that this driver
				 * is stable state when booting. don't call
				 * runtime_suspend or resume state
				 */
...
dwc_hdmi_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (dev == NULL) {
		pr_err(
			"[ERROR][HDMI_V20]%s hdmi_tx_dev device is NULL\n",
			__func__);
		ret = -EINVAL;
		goto end_process;
	}
	...
	case FB_HDMI_CORE_READ:
	{
		...
		if (
			copy_to_user(
				(void __user *)arg, &hdmi_data,
				sizeof(dwc_hdmi_ioctl_data))) {
			pr_err(
			"[ERROR][HDMI_V20]%s READ: reg 0x%08x [-EIO]\n",
	...
	case FB_HDMI_CORE_WRITE:
	{
		...
		if (
			hdmi_dev_write(
				dev,
				hdmi_data.address, hdmi_data.value)) {
			ret = -EIO;
			pr_err(
			"[ERROR][HDMI_V20]%s WRITE reg 0x%08x [-EIO]\n",
			__func__, hdmi_data.address);
			break;
		}
		if (
			copy_to_user(
				...
				sizeof(dwc_hdmi_ioctl_data))) {
			pr_err(
			"[ERROR][HDMI_V20]%s copy_to_user line(%d)\n",
			__func__, __LINE__);
			break;
		}
		ret = 0;
	...
	case HDMI_TIME_PROFILE:
	{
		...
		if (time_log[0] == 0) {
			...
			/* 1 means stop */
			time_diff = dwc_hdmi_get_time(
					&dev->time_backup, &curr_val);
			pr_info("[INFO][HDMI_V20] >> [%d]ms %s\n",
	...
	case HDMI_DRV_LOG:
	{
		...
		if (
			copy_from_user(
				&log_data,
				(void __user *)arg, sizeof(log_data))) {
			pr_err(
				"[ERROR][HDMI_V20]%s copy_from_user line(%d)\n",
				__func__, __LINE__);
			break;
		...
		if (IS_ERR_OR_NULL(log_data.data)) {
			...
			pr_err(
			"[ERROR][HDMI_V20] memdup_user failed (ret=%d)\n",
			(int)ret);
			break;
		}
		pr_info("[INFO][HDMI_V20]%s\n", log_data.data);
	...
	case HDMI_SET_PWR_CONTROL:
	{
		...
		if (dev->verbose >= VERBOSE_IO)
			pr_info(
				"[INFO][HDMI_V20]HDMI_SET_PWR_CONTROL <%d>\n",
				data);
		...
		if (dev->verbose >= VERBOSE_IO)
			pr_info("[INFO][HDMI_V20]HDMI_SET_PWR_CONTROL done\n");
		ret = 0;
		mutex_unlock(&dev->mutex);
	}
	break;
	...
	case HDMI_GET_SOC_FEATURES:
	{
		...
		if (
			copy_to_user(
				(void __user *)arg,
				&soc_features, sizeof(soc_features))) {
			pr_err(
				"[ERROR][HDMI_V20]%s copy_to_user line(%d)\n",
				__func__, __LINE__);
			break;
	...
	case HDMI_GET_BOARD_FEATURES:
	{
		...
		if (
			copy_to_user(
				(void __user *)arg,
				&board_features, sizeof(board_features))) {
			pr_err(
				"[ERROR][HDMI_V20]%s copy_to_user line(%d)\n",
				__func__, __LINE__);
			break;
	...
	case HDMI_STORE_DOLBYVISION_VSIF_LIST:
	{
		...
		if (copy_from_user(
			&transfer_data,
			(void __user *)arg, sizeof(transfer_data))) {
			pr_err(
				"[ERROR][HDMI_V20]%s copy_from_user line(%d)\n",
				__func__, __LINE__);
			break;
		...
		dev->dolbyvision_visf_list =
			devm_kmalloc(
				dev->parent_dev,
				transfer_data.size, GFP_KERNEL);
		if (IS_ERR_OR_NULL(dev->dolbyvision_visf_list)) {
			ret = PTR_ERR(dev->dolbyvision_visf_list);
			dev->dolbyvision_visf_list = NULL;
			pr_err(
				"[ERROR][HDMI_V20]%s memdup_user line(%d)\n",
				__func__, __LINE__);
			break;
		}
		memcpy(
			dev->dolbyvision_visf_list,
			transfer_data.vsif_list, transfer_data.size);
	...
	case HDMI_GET_DTD_INFO:
	{
		...
		if (
			copy_from_user(
				&dtd_param,
				(void __user *)arg, sizeof(dtd_param))) {
			pr_err(
				"[ERROR][HDMI_V20]%s copy_from_user line(%d)\n",
				__func__, __LINE__);
			break;
		...
		if (
			hdmi_dtd_fill(
				&dtd_param.dtd,
				dtd_param.code, dtd_param.refreshRate)) {
			pr_err(
				"[ERROR][HDMI_V20]%s vic (%d) and (%d)hz\n",
				__func__,
				dtd_param.code, dtd_param.refreshRate);
			break;
		}
	...
	case HDMI_DDC_WRITE_DATA:
		...
		if (test_bit(HDMI_TX_STATUS_POWER_ON, &dev->status)) {
			...
			ret = hdmi_ddc_write(
				dev,
				transfer_data.i2cAddr, transfer_data.addr,
				transfer_data.len, transfer_data.data);
			kfree(data_ptrs);
		} else {
			pr_err(
			"[ERROR][HDMI_V20]%s No power <%d>\n",
	...
	case HDMI_DDC_READ_DATA:
	{
		...
		if (dwc_hdmi_is_suspended(dev)) {
			pr_err(
				"[ERROR][HDMI_V20] %s line(%d) ## suspended\n",
				__func__, __LINE__);
			mutex_unlock(&dev->mutex);
			break;
		}
		...
		ret =
			hdmi_ddc_read(
				dev,
				transfer_data.i2cAddr, transfer_data.segment,
				transfer_data.pointer, transfer_data.addr,
				transfer_data.len, data_ptrs);
		if (ret < 0) {
			switch (transfer_data.i2cAddr) {
			case 0x50:
				/* HDMI EDID */
				pr_err(
					"[ERROR][HDMI_V20]## READ EDID\n");
				break;
			case 0x74:
				/* HDMI HDCP */
				pr_err("[ERROR][HDMI_V20]## READ HDCP\n");
				break;
			default:
				pr_err(
					"[ERROR][HDMI_V20]## UNKNOWN(0x%x)\n",
	...
	case HDMI_SCDC_GET_SINK_VERSION:
	{
		...
		if (ret < 0) {
			pr_err(
				"[ERROR][HDMI_V20]%s sink_version line(%d)\n",
				__func__, __LINE__);
			mutex_unlock(&dev->mutex);
			break;
		}
	...
	case HDMI_SCDC_GET_SOURCE_VERSION:
	{
		...
		if (ret < 0) {
			pr_err(
				"[ERROR][HDMI_V20]%s source_version line(%d)\n",
				__func__, __LINE__);
			mutex_unlock(&dev->mutex);
			break;
		}
	...
	case HDMI_VIDEO_GET_SCRAMBLE_STATUS:
	{
		...
		enable = scdc_scrambling_status(dev);
	...
	case HDMI_VIDEO_GET_SCRAMBLE_WAIT_TIME:
	{
		...
		wait_time_ms = scrambling_get_wait_time();
		if (
			copy_to_user(
				(void __user *)arg,
				&wait_time_ms, sizeof(wait_time_ms))) {
			pr_err(
				"[ERROR][HDMI_V20]%s copy_to_user line(%d)\n",
				__func__, __LINE__);
			mutex_unlock(&dev->mutex);
	...
	case HDMI_VIDEO_GET_TMDS_CONFIG_STATUS:
	{
		...
		tmds_config_status = scdc_tmds_config_status(dev);
		if (
			copy_to_user(
				(void __user *)arg,
				&tmds_config_status,
				sizeof(tmds_config_status))) {
			pr_err(
				"[ERROR][HDMI_V20]%s copy_to_user line(%d)\n",
				__func__, __LINE__);
			mutex_unlock(&dev->mutex);
	...
	case HDMI_VIDEO_GET_ERROR_DETECTION:
	{
		...
		if (!ret && (void __user *)arg != NULL) {
			if (
				copy_to_user(
					(void __user *)arg,
					&hdmi_scdc_error_data,
					sizeof(struct hdmi_scdc_error_data))) {
				pr_err(
				"[ERROR][HDMI_V20]%s copy_to_user line(%d)\n",
				__func__, __LINE__);
				mutex_unlock(&dev->mutex);
	...
	case HDMI_AUDIO_INIT:
		...
		pr_info("[INFO][HDMI_V20]HDMI Audio Init!!!\n");
		mutex_unlock(&dev->mutex);
		break;
	...
	case HDMI_AUDIO_CONFIG:
	{
		...
		if (
			copy_from_user(
				&audioParam,
				(void __user *)arg, sizeof(audioParams_t))) {
			pr_err(
				"[ERROR][HDMI_V20]%s copy_from_user line(%d)\n",
				__func__, __LINE__);
			break;
	...
	case HDMI_SET_VENDOR_INFOFRAME:
	{
		...
		if (
			copy_from_user(
				&productParams,
				(void __user *)arg, sizeof(productParams_t))) {
			pr_err(
				"[ERROR][HDMI_V20]%s copy_from_user line(%d)\n",
				__func__, __LINE__);
			break;
	...
	case HDMI_API_GET_NEED_PRE_CONFIG:
	{
		...
		if (
			copy_to_user(
				(void __user *)arg, &pre_config, sizeof(int))) {
			pr_err(
				"[ERROR][HDMI_V20]%s copy_to_user line(%d)\n",
				__func__, __LINE__);
			mutex_unlock(&dev->mutex);
	...
	case HDMI_API_CONFIG_EX:
	{
		...
		memcpy(
			dev->videoParam,
			&api_data.videoParam, sizeof(videoParams_t));
		memcpy(
			dev->audioParam,
			&api_data.audioParam, sizeof(audioParams_t));
		memcpy(
			dev->productParam,
			&api_data.productParam, sizeof(productParams_t));
		...
		hdmi_phy_get_actual_tmds_bit_ratio_by_videoparam(
			dev, (videoParams_t *) dev->videoParam);
	...
	case HDMI_API_DISABLE:
	{
		...
		mutex_lock(&dev->mutex);
		pr_info("[INFO][HDMI_V20] >> HDMI_API_DISABLE\n");
	...
	case HDMI_API_PHY_MASK:
	{
		...
		BUG_ON(
			(magic & HDMI_API_PHY_MASK_MAGIC) !=
					HDMI_API_PHY_MASK_MAGIC);
	...
	case HDMI_API_DRM_CONFIG:
	{
		...
		if (
			copy_from_user(
				&DRMParm,
				(void __user *)arg, sizeof(DRM_Packet_t))) {
			pr_err(
				"[ERROR][HDMI_V20]%s copy_from_user line(%d)\n",
				__func__, __LINE__);
			break;
	...
	case HDMI_API_SET_VSIF_UPDATE_HDR_10P:
	{
		...
		if (
			copy_from_user(
				&update, (void __user *)arg, sizeof(int))) {
			pr_err(
				"[ERROR][HDMI_V20]%s copy_from_user line(%d)\n",
				__func__, __LINE__);
			break;
		...
		if (update == 0)
			clear_bit(
				HDMI_TX_VSIF_UPDATE_FOR_HDR_10P,
							&dev->status);
		else
	...
	case HDMI_API_GET_DRM_CONFIG:
		if (dev->drmParm == NULL) {
			pr_err(
				"[ERROR][HDMI_V20]%s invalid param line(%d)\n",
				__func__, __LINE__);
			break;
		}
		mutex_lock(&dev->mutex);
		if (
			copy_to_user(
				(void __user *)arg,
				dev->drmParm, sizeof(DRM_Packet_t))) {
			pr_err(
				"[ERROR][HDMI_V20]%s copy_to_user line(%d)\n",
				__func__, __LINE__);
			mutex_unlock(&dev->mutex);
	...
	case HDMI_GET_PHY_RX_SENSE_STATUS:
	{
		...
		rx_sense = hdmi_phy_get_rx_sense_status(dev);
		ret = copy_to_user(
			(void __user *)arg, &rx_sense, sizeof(rx_sense));
		mutex_unlock(&dev->mutex);
	}
	break;
	...
	case HDMI_GET_HDCP22_STATUS:
	{
		...
		hdcp22_status = _HDCP22RegStatusRead(dev);
		ret = copy_to_user(
			(void __user *)arg,
			&hdcp22_status, sizeof(hdcp22_status));
		mutex_unlock(&dev->mutex);
	}
	break;
...
dwc_hdmi_release(struct inode *inode, struct file *file)
{
	...
	if (dev->verbose)
		pr_info("[INFO][HDMI_V20]%s read function\n", __func__);
	...
	struct file *file, const char *buf, size_t count, loff_t *f_pos)
{
	...
	if (dev->verbose)
		pr_info("[INFO][HDMI_V20]%s write function\n", __func__);
	return count;
}
...
dwc_hdmi_poll(struct file *file, poll_table *wait)
{
	...
	if (dev->verbose)
		pr_info("[INFO][HDMI_V20]%s poll function\n", __func__);

	//pr_info("[INFO][HDMI_V20] +dwc_hdmi_poll\n");
