...
typedef enum {
	 ...
	 * selected by parsing E-EDID - It is used by user-level
	 * application only
	 */
	ENC_AUTO = 125,
} encoding_t;
...
typedef struct {
	 ...
	 * @note mEncodingOuput and mEncodingIn is should be same
	 * color space.
	 */
...
typedef struct {
	...
	/*ISO 639.2 alpha-3 code, examples: eng,fre,spa,por,jpn,chi */
	u8 mLanguage_Code[3];
...
typedef struct {
	...
	packet_t mPacketType; /*
			       * packet type. currently only Audio Sample
			       * (AUDS) and High Bit Rate (HBR) are supported
			       */

	u16 mClockFsFactor; /*
			     * Input audio clock Fs factor used at the audio
			     * packetizer to calculate the CTS value and ACR
			     * packet insertion rate
			     */

	dmaIncrement_t mDmaBeatIncrement; /*
					   * Incremental burst modes:
					   * unspecified lengths (upper limit
					   * is 1kB boundary) and INCR4, INCR8,
					   * and INCR16 fixed-beat burst
					   */

	u8 mDmaThreshold; /*
			   * When the number of samples in the Audio FIFO is
			   * lower than the threshold, the DMA engine requests
			   * a new burst request to the AHB master interface
			   */

	u8 mDmaHlock; /* Master burst lock mechanism */

	u8 mGpaInsertPucv;	/*
				 * discard incoming (Parity, Channel status,
				 * User bit, Valid and B bit) data and insert
				 * data configured in controller instead
				 */
	audioMetaDataPacket_t mAudioMetaDataPacket; /*
						     * Audio Multistream
						     * variables, to be written
						     *  to the Audio Metadata
						     * Packet
						     */
	u8 mAudioType;
} audioParams_t;
...
typedef struct {
	 ...
	 * (depending on instantiated KSV MEM RAM – Revocation Memory
	 * to support HDCP repeaters)
	 */
	u8 maxDevices;

	/* KSV List buffer
	 * Shall be dimensioned to accommodate 5[bytes] x No. of supported
	 * devices (depending on instantiated KSV MEM RAM – Revocation Memory
	 * to support HDCP repeaters)
...
/*
 * @brief This structure defines the header of Dynamic Range and Mastering
 * InfoFrame.
 */
typedef struct {
	u8 version;
...
/*
 * @brief This structure defines the body of Dynamic Range and Mastering
 * InfoFrame.
 */
typedef struct {
	u8 EOTF;
...
typedef struct hdmi_board_features {
	unsigned int support_feature_1;         // fixed video identification
						// code
	unsigned int support_feature_2;         // reserved for future use
	unsigned int support_feature_3;         // reserved for future use
	unsigned int support_feature_4;         // reserved for future use
 ...
 * fb_data->value -> value to write
 */
#define HDMI_GET_PWR_STATUS                     \
				_IOR(IOCTL_HDMI_MAGIC, 0x100, unsigned int)
#define HDMI_SET_PWR_CONTROL                    \
				_IOW(IOCTL_HDMI_MAGIC, 0x101, unsigned int)
#define HDMI_GET_SUSPEND_STATUS                 \
				_IOR(IOCTL_HDMI_MAGIC, 0x102, unsigned int)
#define HDMI_GET_OUTPUT_ENABLE_STATUS           \
				_IOR(IOCTL_HDMI_MAGIC, 0x103, unsigned int)

#define HDMI_IOC_BLANK                          \
				_IOW(IOCTL_HDMI_MAGIC, 0x110, unsigned int)



#define HDMI_GET_SOC_FEATURES                   \
				_IOR(IOCTL_HDMI_MAGIC, 0x121, \
				hdmi_soc_features)
#define HDMI_GET_BOARD_FEATURES                 \
				_IOR(IOCTL_HDMI_MAGIC, 0x122, \
				hdmi_board_features)

/*
 * @short IOCTL to stores Vendor Specific Info Frames
 */
#define HDMI_STORE_DOLBYVISION_VSIF_LIST        \
				_IOW(IOCTL_HDMI_MAGIC, 0x140, \
				hdmi_dolbyvision_vsif_transfer_data)
 ...
 */
#define HDMI_GET_DTD_INFO                       \
				_IOR(IOCTL_HDMI_MAGIC, 0x201, \
				dwc_hdmi_dtd_data)
 ...
 */
#define HDMI_TIME_PROFILE                       \
				_IOW(IOCTL_HDMI_MAGIC, 0x220, char*)

#define HDMI_DDC_READ_DATA                      \
				_IOR(IOCTL_HDMI_MAGIC, 0x222, \
				dwc_hdmi_ddc_transfer_data)
#define HDMI_DDC_WRITE_DATA                     \
				_IOW(IOCTL_HDMI_MAGIC, 0x223, \
				dwc_hdmi_ddc_transfer_data)
#define HDMI_SCDC_GET_SINK_VERSION              \
				_IOR(IOCTL_HDMI_MAGIC, 0x224, \
				unsigned int)
#define HDMI_SCDC_GET_SOURCE_VERSION            \
				_IOW(IOCTL_HDMI_MAGIC, 0x225, \
				unsigned int)
#define HDMI_SCDC_SET_SOURCE_VERSION            \
				_IOW(IOCTL_HDMI_MAGIC, 0x226, \
				unsigned int)
#define HDMI_DDC_BUS_CLEAR			\
				_IO(IOCTL_HDMI_MAGIC, 0x227)

#define HDMI_VIDEO_SET_TMDS_CONFIG_INIT         _IO(IOCTL_HDMI_MAGIC, 0x231)
#define HDMI_VIDEO_SET_SCRAMBLING               \
				_IOW(IOCTL_HDMI_MAGIC, 0x232, int)
#define HDMI_VIDEO_GET_ERROR_DETECTION          _IO(IOCTL_HDMI_MAGIC, 0x234)
#define HDMI_VIDEO_GET_SCRAMBLE_STATUS          \
				_IOR(IOCTL_HDMI_MAGIC, 0x235, int)
#define HDMI_VIDEO_GET_SCRAMBLE_WAIT_TIME       \
				_IOR(IOCTL_HDMI_MAGIC, 0x236, int)
#define HDMI_VIDEO_GET_TMDS_CONFIG_STATUS       \
				_IOR(IOCTL_HDMI_MAGIC, 0x237, int)
...
#define HDMI_AUDIO_INIT                         \
				_IO(IOCTL_HDMI_MAGIC, 0x240)
#define HDMI_AUDIO_CONFIG                       \
				_IOW(IOCTL_HDMI_MAGIC, 0x241, audioParams_t)
...
#define HDMI_HPD_GET_STATUS                     \
				_IOR(IOCTL_HDMI_MAGIC, 0x251, int)


#define HDMI_SET_VENDOR_INFOFRAME               \
				_IOW(IOCTL_HDMI_MAGIC, 0x261, productParams_t)


#define HDMI_PACKET_CONFIG                      _IO(IOCTL_HDMI_MAGIC, 0x270)
#define HDMI_AV_MUTE_MAGIC                      0xF0F0A0A0
#define HDMI_SET_AV_MUTE                        \
				_IOW(IOCTL_HDMI_MAGIC, 0x271, unsigned int)


#define HDMI_API_GET_NEED_PRE_CONFIG            \
				_IOR(IOCTL_HDMI_MAGIC, 0x278, int)
#define HDMI_API_PRE_CONFIG                     _IO(IOCTL_HDMI_MAGIC, 0x279)
#define HDMI_API_CONFIG                         \
				_IOW(IOCTL_HDMI_MAGIC, 0x280, \
				dwc_hdmi_api_data)
#define HDMI_API_CONFIG_EX                      \
				_IOW(IOCTL_HDMI_MAGIC, 0x280, \
				dwc_hdmi_api_ex_data)
#define HDMI_API_DISABLE_MAGIC                  0xA0500281
#define HDMI_API_DISABLE                        \
				_IOW(IOCTL_HDMI_MAGIC, 0x281, unsigned int)
#define HDMI_API_PHY_MASK_MAGIC			0xCC104770
#define HDMI_API_PHY_MASK			\
				_IOW(IOCTL_HDMI_MAGIC, 0x282, unsigned int)
 ...
 * @retval -1 otherwise
 */
#define HDMI_API_DRM_CONFIG			\
				_IOW(IOCTL_HDMI_MAGIC, 0x285, DRM_Packet_t)
 ...
 * @retval -1 otherwise
 */
#define HDMI_API_DRM_GET_VALID                  \
				_IOR(IOCTL_HDMI_MAGIC, 0x288, int)

/*
 * @short This ioctls sets HDMI_TX_VSIF_UPDATE_FOR_HDR_10P of device status
 *		bits. This bit is used to determins whether to runs
 *		hdmi_api_vsif_update_for_hdr_10p.
 ...
 */
#define HDMI_API_SET_VSIF_UPDATE_HDR_10P	\
				_IOW(IOCTL_HDMI_MAGIC, 0x289, int)

#define HDMI_API_GET_DRM_CONFIG			\
				_IOR(IOCTL_HDMI_MAGIC, 0x28A, DRM_Packet_t)


#if defined(HDCP_SUPPORT)
#define HDCP22_CTRL_REG_RESET			_IO(IOCTL_HDMI_MAGIC, 0x290)
#define HDMI_GET_PHY_RX_SENSE_STATUS		\
				_IOR(IOCTL_HDMI_MAGIC, 0x291, int)
#define HDMI_GET_HDCP22_STATUS			\
				_IOR(IOCTL_HDMI_MAGIC, 0x292, int)
#endif
