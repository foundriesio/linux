...
#define HDMI_DRV_VERSION        "2.1.0d"
#else
#define HDMI_DRV_VERSION        "2.1.0"
#endif
...
struct hdmi_tx_ctrl {
	...
	/*
	 * In general, HDCP Keepout is set to 1 when TMDS character rate is
	 * higherthan 340 MHz or when HDCP is enabled.
	 * If HDCP Keepout is set to 1 then the control period configuration is
	 *  changed in order to supports scramble and HDCP encryption. But some
	 * SINK needs this packet configuration always even if HDMI ouput is
	 * not scrambled or HDCP is not enabled.
	 */
...
struct hdmi_tx_dev {
	 ...
	 * In such cases, you should set this variable to 1 to disable
	 * ddc communication
	 */
	int ddc_disable;
