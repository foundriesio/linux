...
of_parse_hdmi_dt(struct hdmi_tx_dev *dev, struct device_node *node)
{
	...
	dev->hdmi_tx_phy_if_io = of_iomap(node, PROTO_HDMI_TX_PHY);
	if (dev->hdmi_tx_phy_if_io == NULL) {
		pr_err(
			"[ERROR][%s]%s map phy_if_base_addr resource\n",
			LOG_TAG, __func__);
		ret = -ENODEV;
		goto end_process;
	}
	...
	if (dev->io_bus == NULL) {
		pr_err(
			"[ERROR][%s]%s map io_bus base address resource\n",
			LOG_TAG, __func__);
		ret = -ENODEV;
		goto end_process;
	}
	...
	if (ddibus_np == NULL) {
		pr_err("[ERROR][%s]%s map ddibus resource\n",
			LOG_TAG, __func__);
		ret = -ENODEV;
		goto end_process;
	}
	...
	if (dev->ddibus_io == NULL) {
		pr_err("[ERROR][%s]%s map ddibus_io resource\n",
				LOG_TAG, __func__);
		ret = -ENODEV;
		goto end_process;
	}
	...
	// Parse Clock
	for (
		of_loop = HDMI_CLK_INDEX_PERI;
		of_loop < HDMI_CLK_INDEX_MAX; of_loop++) {
		dev->clk[of_loop] = of_clk_get(node, of_loop);
		if (IS_ERR(dev->clk[of_loop])) {
			pr_err(
				"[ERROR][%s]%s map hdmi clock (%d)\n",
				LOG_TAG, __func__, of_loop);
			ret = -ENODEV;
			goto end_process;
		}
	}
	...
	ret = of_property_read_u32(
		node, "clock-frequency", &dev->clk_freq_apb);
	if (ret < 0) {
		pr_err(
			"[ERROR][%s]%s map hdmi pclk frequency\n",
			LOG_TAG, __func__);
		ret = -ENODEV;
		goto end_process;
	}
	...
	if (
		of_property_read_u32(
			node, "audio_if_selection_ofst",
			&dev->hdmi_audio_if_sel_ofst) < 0) {
		dev->hdmi_audio_if_sel_ofst = 0xff;
		pr_err(
			"[ERROR][%s]%s map audio source offset\n",
			LOG_TAG, __func__);
	}

	if (
		of_property_read_u32(
			node, "audio_rx_tx_chmux", &dev->hdmi_rx_tx_chmux) < 0)
	...
	if (dev->verbose >= VERBOSE_BASIC)
		pr_info("[INFO][%s]%s  verbose level is %d\n",
		LOG_TAG, __func__, dev->verbose);
	...
	if (
		of_property_read_string(
			...
			&dt_string) == 0 && dt_string != NULL)
		strncpy(
			dev->product_description,
			dt_string, sizeof(dev->product_description));
	else
		strcpy(dev->product_description, "TCC8xxxx");

	if (
		of_property_read_u32(
			node, "source_information",
			&dev->source_information) < 0)
...
static int of_parse_i2c_mapping(struct hdmi_tx_dev *dev)
{
	...
	do {
		...
		if (dev == NULL) {
			pr_err(
				"[ERROR][%s]%s hdmi_tx_dev is NULL\r\n",
				LOG_TAG, __func__);
			break;
		}
		if (dev->parent_dev == NULL) {
			pr_err(
				"[ERROR][%s]%s parent_dev is NULL\r\n",
				LOG_TAG, __func__);
			break;
		}
		...
		if (
			of_property_read_u32(
				node,
				"hdmi_i2c_port_disable", &dev->ddc_disable) < 0)
			dev->ddc_disable = 0;

		if (dev->ddc_disable) {
			pr_info(
				"[INFO][%s]%s ddc is disabled.. \r\n",
				LOG_TAG, __func__);
			break;
		}
		...
		if (io_i2c_map == NULL) {
			pr_err(
				"[ERROR][%s]%s map i2c mapping\n",
				LOG_TAG, __func__);
			ret = -ENODEV;
			goto end_process;
		}
		ret = of_property_read_u32_index(
			node, "hdmi_i2c_port_mapping", 0, &i2c_mapping_offset);
		if (ret < 0) {
			pr_err(
				"[ERROR][%s]%s map i2c_mapping offset\n",
				LOG_TAG, __func__);
			ret = -ENODEV;
			goto end_process;
		}
		...
		ret = of_property_read_u32_index(
			node, "hdmi_i2c_port_mapping", 1, &i2c_mapping_mask);
		if (ret < 0) {
			pr_err(
				"[ERROR][%s]%s map i2c_mapping_mask\n",
				LOG_TAG, __func__);
			ret = -ENODEV;
			goto end_process;
		}
		...
		ret = of_property_read_u32_index(
			node, "hdmi_i2c_port_mapping", 2, &i2c_mapping_val);
		if (ret < 0) {
			pr_err(
				"[ERROR][%s]%s map i2c_mapping_val\n",
				LOG_TAG, __func__);
			ret = -ENODEV;
			goto end_process;
		}
 ...
 * @return Void
 */
void release_memory_blocks(struct hdmi_tx_dev *dev)
{
	if (dev->ddibus_io != NULL)
...
static void send_hdmi_output_event(struct work_struct *work)
{
	...
	struct hdmi_tx_dev *dev =
		container_of(work, struct hdmi_tx_dev, hdmi_output_event_work);

	if (dev != NULL) {
		snprintf(
			u_event_name, sizeof(u_event_name),
			"SWITCH_STATE=%d",
			test_bit(HDMI_TX_STATUS_OUTPUT_ON, &dev->status)?1:0);
		u_events[0] = u_event_name;
		u_events[1] = NULL;
		pr_info(
			"[INFO][%s]%s u_event(%s)\r\n",
			LOG_TAG, __func__, u_event_name);
		kobject_uevent_env(
			&dev->parent_dev->kobj, KOBJ_CHANGE, u_events);
	}
}
...
static void hdmi_tx_restore_hotpug_work(struct work_struct *work)
{
	struct hdmi_tx_dev *dev =
		container_of(
			...
			struct hdmi_tx_dev, hdmi_restore_hotpug_work);
	if (dev != NULL) {
		pr_info(
			"[INFO][%s]%s restore hotplug_status\r\n",
			LOG_TAG, __func__);
...
int hdmi_tx_suspend(struct device *dev)
{
	...
	struct hdmi_tx_dev *hdmi_tx_dev =
		(dev != NULL) ?
		(struct hdmi_tx_dev *)dev_get_drvdata(dev) : NULL;

	if (hdmi_tx_dev == NULL)
		goto out;
	pr_info("[INFO][%s]### %s \r\n", LOG_TAG, __func__);
	if (test_bit(HDMI_TX_STATUS_SUSPEND_L1, &hdmi_tx_dev->status)) {
		pr_err("[ERROR][%s]%s already suspended\r\n",
		LOG_TAG, __func__);
		goto out;
	}
...
int hdmi_tx_resume(struct device *dev)
{
	...
	if (gpio_is_valid(hdmi_tx_dev->hotplug_gpio)) {
		hdmi_tx_dev->hotplug_status =
		hdmi_tx_dev->hotplug_real_status =
		gpio_get_value(hdmi_tx_dev->hotplug_gpio);
...
hdmi_tx_init(struct platform_device *pdev)
{
	...
	pr_info(
		...
		LOG_TAG);
	pr_info(
		"[INFO][%s]%s HDMI driver %s\n",
		LOG_TAG, __func__, HDMI_DRV_VERSION);
	pr_info(
		"[INFO][%s]****************************************\n",
		LOG_TAG);
	...
	if (dev == NULL) {
		pr_err(
			"[ERROR][%s]%s Could not allocated hdmi_tx_dev\n",
			__func__, LOG_TAG);
		return -ENOMEM;
	}
	...
	pr_info(
		"[INFO][%s]%s Driver's name '%s'\n",
		LOG_TAG, __func__, dev->device_name);
	...
	#if defined(CONFIG_TCC_RUNTIME_TUNE_HDMI_PHY)
	INIT_DELAYED_WORK(
		&dev->hdmi_restore_hotpug_work, hdmi_tx_restore_hotpug_work);
	#endif

	dev->videoParam = (void *)devm_kzalloc(
		dev->parent_dev, sizeof(videoParams_t), GFP_KERNEL);
	dev->audioParam = (void *)devm_kzalloc(
		dev->parent_dev, sizeof(audioParams_t), GFP_KERNEL);
	dev->productParam = (void *)devm_kzalloc(
		dev->parent_dev, sizeof(productParams_t), GFP_KERNEL);
	dev->drmParm = (void *)devm_kzalloc(
		dev->parent_dev, sizeof(DRM_Packet_t), GFP_KERNEL);
	...
	if (of_parse_hdmi_dt(dev, pdev->dev.of_node) < 0) {
		pr_info(
			"[INFO][%s]%s Map memory blocks failed\n",
			LOG_TAG, __func__);
		ret = -ENOMEM;
		goto free_mem;
	}
	...
	if (of_parse_i2c_mapping(dev) < 0) {
		pr_info(
			"[INFO][%s]%s Map i2c mapping failed\n",
			LOG_TAG, __func__);
		ret = -ENOMEM;
		goto free_mem;
	}
	...
	// Proc file system
	pr_info(
		"[INFO][%s]%s Init proc file system @ /proc/hdmi_tx\n",
		LOG_TAG, __func__);
...
hdmi_tx_exit(struct platform_device *pdev)
{
	...
	pr_info("[INFO][%s]**************************************\n", LOG_TAG);
	pr_info("[INFO][%s]%s Removing HDMI module\n", LOG_TAG, __func__);
	pr_info("[INFO][%s]**************************************\n", LOG_TAG);
	...
	while (!list_empty(&devlist_global)) {
		...
		pr_info(
			"[INFO][%s]%s Remove proc file system\n",
			LOG_TAG, __func__);
		...
		pr_info(
			"[INFO][%s]%s Release memory blocks\n",
			LOG_TAG, __func__);
		release_memory_blocks(dev);
