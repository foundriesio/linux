...
int hdmi_dev_write(
	struct hdmi_tx_dev *hdmi_dev, uint32_t offset, uint32_t data)
{
	volatile void __iomem *dwc_hdmi_tx_core_io =
		(volatile void __iomem *)hdmi_dev->dwc_hdmi_tx_core_io;
	if (offset & 0x3)
		return -1;
...
unsigned int hdmi_dev_read(
	struct hdmi_tx_dev *hdmi_dev, uint32_t offset)
{
	volatile void __iomem *dwc_hdmi_tx_core_io =
		(volatile void __iomem *)hdmi_dev->dwc_hdmi_tx_core_io;
	if (offset & 0x3)
...
void hdmi_dev_write_mask(
	struct hdmi_tx_dev *hdmi_dev, u32 addr, u8 mask, u8 data)
{
	u8 temp = 0;
