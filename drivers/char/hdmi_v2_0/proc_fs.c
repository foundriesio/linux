	...
	struct file *filp, char __user *usr_buf, size_t cnt, loff_t *off_set)
{
	...
	case 0:
		size += sprintf(hdcp_status_buf + size, "HDCP idle.\n");
		break;
	case 1:
		size += sprintf(
				hdcp_status_buf + size,
	...
	size = simple_read_from_buffer(
		usr_buf, cnt, off_set, hdcp_status_buf, size);
	devm_kfree(dev->parent_dev, hdcp_status_buf);
	return size;
}

ssize_t proc_write_hdcp22(
	struct file *filp,
	const char __user *buffer, size_t cnt, loff_t *off_set)
{
	uint32_t hdcp22 = 0;
	struct hdmi_tx_dev *dev = PDE_DATA(file_inode(filp));
	...
	if (cnt != sizeof(uint32_t))
		...
		//	pr_err(
		//		"[ERROR][HDMI_V20]%s: Size miss [%d != %d]\n",
		//		__func__, (int)cnt, (int)sizeof(uint32_t));
		return -1;
	...
	if (copy_from_user(&hdcp22, buffer, cnt)) {
		pr_err(
			"[ERROR][HDMI_V20]%s copy_from_user line(%d)\r\n",
			__func__, __LINE__);
		return -EFAULT;
	}
	dev->hdcp22 = hdcp22;
	...
	const char __user *buffer, size_t cnt, loff_t *off_set)
{
	...
	hpd_lock_buf[cnt] = '\0';
	ret = kstrtou32(hpd_lock_buf, 0, &hpd_lock);
	devm_kfree(dev->parent_dev, hpd_lock_buf);
	if (ret < 0)
		return ret;
	...
	struct file *filp, char __user *usr_buf, size_t cnt, loff_t *off_set)
{
	...
	size = simple_read_from_buffer(
			usr_buf, cnt,  off_set, hpd_lock_buf, size);
	devm_kfree(dev->parent_dev, hpd_lock_buf);
	return size;
}
	...
	struct file *filp, char __user *usr_buf, size_t cnt, loff_t *off_set)
{
	...
	size = simple_read_from_buffer(
		usr_buf, cnt,  off_set, rxsense_buf, size);
	devm_kfree(dev->parent_dev, rxsense_buf);
	return size;
}

ssize_t proc_write_scdc_check(
	struct file *filp,
	const char __user *buffer, size_t cnt, loff_t *off_set)
{
	int ret;
	...
	scdc_buf[cnt] = '\0';
	ret = kstrtou32(scdc_buf, 0, &scdc_check);
	//pr_info("[INFO][HDMI_V20]scdc_check = 0x%x\r\n", scdc_check);
	devm_kfree(dev->parent_dev, scdc_buf);
	if (ret < 0)
		return ret;
	...
	struct file *filp, char __user *usr_buf, size_t cnt, loff_t *off_set)
{
	...
	mutex_lock(&dev->mutex);
	scdc_check = test_bit(
		HDMI_TX_STATUS_SCDC_CHECK, &dev->status) ? 0x1 : 0;
	scdc_check |= test_bit(
		HDMI_TX_STATUS_SCDC_IGNORE, &dev->status) ? 0x10 : 0;
	scdc_check |= test_bit(
		HDMI_TX_STATUS_SCDC_FORCE_ERROR, &dev->status) ? 0x100 : 0;
	mutex_unlock(&dev->mutex);

	size = sprintf(scdc_check_buf, "%x\n", scdc_check);
	size = simple_read_from_buffer(
		usr_buf, cnt,  off_set, scdc_check_buf, size);
	devm_kfree(dev->parent_dev, scdc_check_buf);
	return size;
}

ssize_t proc_write_ddc_check(
	struct file *filp,
	const char __user *buffer, size_t cnt, loff_t *off_set)
{
	int ret;
	...
	if (!test_bit(HDMI_TX_STATUS_SUSPEND_L1, &dev->status)) {
		if (test_bit(HDMI_TX_STATUS_POWER_ON, &dev->status))
			...
			pr_err(
				"[ERROR][HDMI_V20]%s HDMI not powred <%d>\r\n",
				__func__, __LINE__);
	} else {
		pr_err(
			"[ERROR][HDMI_V20]##%s hdmi linke was suspended\r\n",
			__func__);
	}
	mutex_unlock(&dev->mutex);
...
ssize_t proc_write_edid_machine_id(
	struct file *filp,
	const char __user *buffer, size_t cnt, loff_t *off_set)
{
	int ret;
	...
	struct hdmi_tx_dev *dev = PDE_DATA(file_inode(filp));
	char *edid_machine_id_buf =
		devm_kzalloc(dev->parent_dev, cnt + 1, GFP_KERNEL);

	if (edid_machine_id_buf == NULL)
		return -ENOMEM;
	ret = simple_write_to_buffer(
		edid_machine_id_buf, cnt, off_set, buffer, cnt);
	if (ret != cnt) {
	...
	ret = kstrtou32(edid_machine_id_buf, 0, &edid_machine_id);
	devm_kfree(dev->parent_dev, edid_machine_id_buf);
	if (ret < 0)
		return ret;
	...
	if (dev->verbose >= VERBOSE_IO)
		pr_info("[INFO][HDMI_V20][%s] edid_machine_id = %d\n",
						__func__, edid_machine_id);

	return cnt;
}

ssize_t proc_read_edid_machine_id(
	struct file *filp,
	char __user *usr_buf, size_t cnt, loff_t *off_set)
{
	ssize_t size;
	...
	size = simple_read_from_buffer(
			usr_buf, cnt,  off_set, edid_machine_id_buf, size);
	devm_kfree(dev->parent_dev, edid_machine_id_buf);
	return size;
}
#endif
	...
	const char __user *buffer, size_t cnt, loff_t *off_set)
{
	...
	ret = simple_write_to_buffer(
		drm_test_run_buf, cnt, off_set, buffer, cnt);
	if (ret != cnt) {
	...
	ret = kstrtou32(drm_test_run_buf, 0, &drm_test_run);
	...
	for (i = 0; i < 16; i++) {
		...
		hdmi_apply_drm(dev);
		pr_info(
			"[INFO][HDMI_V20]%s primaries_x[0] = %d\r\n",
			__func__,  i);
		msleep(1000);
	}
		break;
	...
	for (i = 0; i < 16; i++) {
		...
		hdmi_apply_drm(dev);
		pr_info(
			"[INFO][HDMI_V20]%s primaries_x[1] = %d\r\n",
			__func__,  i);
		msleep(1000);
	}
	break;
	...
	for (i = 0; i < 16; i++) {
		...
		hdmi_apply_drm(dev);
		pr_info(
			"[INFO][HDMI_V20]%s primaries_x[2] = %d\r\n",
			__func__,  i);
		msleep(1000);
	}
	break;
	...
	for (i = 0; i < 16; i++) {
		...
		hdmi_apply_drm(dev);
		pr_info(
			"[INFO][HDMI_V20]%s disp_primaries_y[0] = %d\r\n",
			__func__,  i);
		msleep(1000);
	}
	break;
	...
	for (i = 0; i < 16; i++) {
		...
		hdmi_apply_drm(dev);
		pr_info(
			"[INFO][HDMI_V20]%s disp_primaries_y[1] = %d\r\n",
			__func__,  i);
		msleep(1000);
	}
	break;
	...
	for (i = 0; i < 16; i++) {
		...
		hdmi_apply_drm(dev);
		pr_info(
			"[INFO][HDMI_V20]%s disp_primaries_y[2] = %d\r\n",
			__func__,  i);
		msleep(1000);
	}
	break;
	...
	for (i = 0; i < 16; i++) {
		...
		hdmi_apply_drm(dev);
		pr_info(
			"[INFO][HDMI_V20]%s white_point_x = %d\r\n",
			__func__,  i);
		msleep(1000);
	}
	break;
	...
	for (i = 0; i < 16; i++) {
		...
		hdmi_apply_drm(dev);
		pr_info(
			"[INFO][HDMI_V20]%s white_point_y = %d\r\n",
			__func__,  i);
		msleep(1000);
	}
	break;
	...
	drm_param.mDescriptor_type1.EOTF = 2;
	for (i = 0; i < 16; i++) {
		drm_param.mDescriptor_type1.max_disp_mastering_luminance =
		 (1 << i);
		hdmi_update_drm_configure(dev, &drm_param);
		hdmi_apply_drm(dev);
		pr_info(
		"[INFO][HDMI_V20]%s max_disp_mastering_luminance = %d\r\n",
			__func__,  i);
		msleep(1000);
	}
	break;
	...
	drm_param.mDescriptor_type1.EOTF = 2;
	for (i = 0; i < 16; i++) {
		drm_param.mDescriptor_type1.min_disp_mastering_luminance =
		(1 << i);
		hdmi_update_drm_configure(dev, &drm_param);
		hdmi_apply_drm(dev);
		pr_info(
		"[INFO][HDMI_V20]%s min_disp_mastering_luminance = %d\r\n",
			__func__,  i);
		msleep(1000);
	}
	break;
	...
	for (i = 0; i < 16; i++) {
		...
		hdmi_apply_drm(dev);
		pr_info(
			"[INFO][HDMI_V20]%s max_content_light_level = %d\r\n",
			__func__,  i);
		msleep(1000);
	}
	break;
	...
	drm_param.mDescriptor_type1.EOTF = 2;
	for (i = 0; i < 16; i++) {
		drm_param.mDescriptor_type1.max_frame_avr_light_level =
		(1 << i);
		hdmi_update_drm_configure(dev, &drm_param);
		hdmi_apply_drm(dev);
		pr_info(
			"[INFO][HDMI_V20]%s max_frame_avr_light_level = %d\r\n",
			__func__,  i);
		msleep(1000);
	}
	break;
	...
	const char __user *buffer, size_t cnt, loff_t *off_set)
{
	...
	size = simple_write_to_buffer(
		phy_regs_buf, cnt, off_set, buffer, cnt);
	if (size != cnt) {
		if (size >= 0) {
			size = -EIO;
			goto out;
	...
	if (dwc_hdmi_proc_write_phy_regs(dev, phy_regs_buf) == 0) {
		size = cnt;
		if (test_bit(HDMI_TX_HOTPLUG_STATUS_LOCK, &dev->status)) {
			pr_info(
				"[INFO][HDMI_V20]%s hpd is locked\r\n",
				__func__);
		} else {
			...
			pr_info(
				"[INFO][HDMI_V20]%s set hpd swing\r\n",
				__func__);
			schedule_delayed_work(
				&dev->hdmi_restore_hotpug_work,
	...
	struct file *filp, char __user *usr_buf, size_t cnt, loff_t *off_set)
{
	...
	if (phy_regs_buf != NULL) {
		size = dwc_hdmi_proc_read_phy_regs(
				dev, phy_regs_buf, DEBUGFS_BUF_SIZE);
		if (size > 0)
			size = simple_read_from_buffer(
				usr_buf, cnt,  off_set, phy_regs_buf, size);
		devm_kfree(dev->parent_dev, phy_regs_buf);
	}
	return size
}
	...
	const char __user *buffer, size_t cnt, loff_t *off_set)
{
	...
	ret = kstrtou32(vsif_vic_buf, 0, &vsif_vic);
	...
	if (vsif_vic > 0 && vsif_vic <= 4) {
		...
		productParams.mVendorPayload[
			productParams.mVendorPayloadLength++] = 1 << 5;
		/* PB[5] */
		productParams.mVendorPayload[
			productParams.mVendorPayloadLength++] = vsif_vic;
		/*
	...
	const char __user *buffer, size_t cnt, loff_t *off_set)
{
	...
	size = simple_write_to_buffer(
		dv_vsfi_buf, cnt, off_set, buffer, cnt);
	if (size != cnt) {
		if (size >= 0) {
			size = -EIO;
			goto out;
	...
	if (kstrtoint(dv_vsfi_buf, 0, &dv_vsif_index) < 0)
		pr_debug("[DEBUG][HDMI_V20]%s -EINTVAL\r\n", __func__);

	devm_kfree(dev->parent_dev, dv_vsfi_buf);

	if (dv_vsif_index < 0 || dv_vsif_index > 6) {
		pr_err(
			"[ERROR][HDMI_V20]invalid index %d\r\n",
			dv_vsif_index);
		goto out;
	}
...
ssize_t proc_write_audio_channel_mux(
	struct file *filp,
	const char __user *buffer, size_t cnt, loff_t *off_set)
{
	ssize_t size;
	...
	struct hdmi_tx_dev *dev = PDE_DATA(file_inode(filp));
	char *audio_channel_mux_buff =
		devm_kzalloc(dev->parent_dev, cnt + 1, GFP_KERNEL);
	int ret;

	pr_info(
		"[INFO][HDMI_V20]audio_channel_mux_buff=0x%x\r\n",
	...
	size = simple_write_to_buffer(
		audio_channel_mux_buff, cnt, off_set, buffer, cnt);
	if (size != cnt) {
		if (size >= 0) {
			size = -EIO;
			goto out;
	...
	pr_info(
		"[INFO][HDMI_V20]audio_channel_mux_buff=[%s]\r\n",
		audio_channel_mux_buff);

	ret = sscanf(
		audio_channel_mux_buff, "%u,%u,%u",
	...
	if (!test_bit(HDMI_TX_STATUS_SUSPEND_L1, &dev->status)) {
		if (test_bit(HDMI_TX_STATUS_POWER_ON, &dev->status)) {
			pr_info(
				"[INFO][AUDIO]i2s(%d) sport(%d) smux(%d)\r\n",
				i2s_channel_mux,
				spdif_input_port, spdif_channel_mux);
...
ssize_t proc_read_audio_channel_mux(
	struct file *filp, char __user *usr_buf, size_t cnt, loff_t *off_set)
{
	int i, j;
	ssize_t size = -EINVAL;
	unsigned int i2s_channel_mux = 0;
	unsigned int spdif_input_port = 0;
	unsigned int spdif_channel_mux = 0;
	struct hdmi_tx_dev *dev = PDE_DATA(file_inode(filp));
	char *audio_channel_mux_buff =
		devm_kzalloc(dev->parent_dev, DEBUGFS_BUF_SIZE, GFP_KERNEL);
	...
	const char __user *buffer, size_t cnt, loff_t *off_set)
{
	...
	do {
		...
		size = simple_write_to_buffer(
				debug_buffer, cnt, off_set, buffer, cnt);
		if (size != cnt) {
			if (size >= 0) {
				size = -EIO;
				break;
		...
		pr_info(
			"[INFO][HDMI_V20]HDMI Debug Param  <%d, %d>\r\n",
			debug_param[0], debug_param[1]);
...
{
	if (dev == NULL) {
		pr_err("[ERROR][HDMI_V20]%s:Device is null\n", __func__);
		return;
	}
	dev->hdmi_proc_dir = proc_mkdir("hdmi_tx", NULL);
	if (dev->hdmi_proc_dir == NULL)
		pr_err(
			"[ERROR][HDMI_V20]%s:failed  @/proc/hdmi_tx\n",
			__func__);
	// HPD
	//pr_info("[INFO][HDMI_V20]%s:Installing hpd file\n", __func__);
	dev->hdmi_proc_hpd =
		proc_create_data(
			"hpd", S_IFREG | 0444,
			dev->hdmi_proc_dir, &proc_fops_hpd, dev);
	if (dev->hdmi_proc_hpd == NULL)
		pr_err("[ERROR][HDMI_V20]%s:failed  @hpd\n", __func__);
	// HPD_LOCK
	dev->hdmi_proc_hpd_lock =
		proc_create_data(
			"hpd_lock", S_IFREG | 0444 | 0222,
			dev->hdmi_proc_dir, &proc_fops_hpd_lock, dev);
	if (dev->hdmi_proc_hpd_lock == NULL)
		pr_err("[ERROR][HDMI_V20]%s:failed  @hpd_lock\n", __func__);
	// RX_Sense
	dev->hdmi_proc_rxsense =
		proc_create_data(
			"rxsense", S_IFREG | 0444,
			dev->hdmi_proc_dir, &proc_fops_rxsense, dev);
	if (dev->hdmi_proc_rxsense == NULL)
		pr_err("[ERROR][HDMI_V20]%s:failed  @rxsense\n", __func__);
	// HDCP 2.2 status
	//pr_info(
		//"[INFO][HDMI_V20]%s:Installing hdcp2_status file\n",
		//__func__);
	dev->hdmi_proc_hdcp_status =
		proc_create_data(
			"hdcp_status", S_IFREG | 0444,
			dev->hdmi_proc_dir, &proc_fops_hdcp_status, dev);
	if (dev->hdmi_proc_hdcp_status == NULL)
		pr_err("[ERROR][HDMI_V20]%s:failed  @hdcp_status\n", __func__);
	// HDCP 2.2
	//pr_info("[INFO][HDMI_V20]%s:Installing hdcp22 file\n", __func__);
	dev->hdmi_proc_hdcp22 =
		proc_create_data(
			"hdcp22", S_IFREG | 0222,
			dev->hdmi_proc_dir, &proc_fops_hdcp22, dev);
	if (dev->hdmi_proc_hdcp22 == NULL)
		pr_err("[ERROR][HDMI_V20]%s:failed  @hdcp22\n", __func__);
	//pr_info("[INFO][HDMI_V20]%s:Installing scdc_check file\n", __func__);
	dev->hdmi_proc_scdc_check =
		proc_create_data(
			"scdc_check", S_IFREG | 0444 | 0222,
			dev->hdmi_proc_dir, &proc_fops_scdc_check, dev);
	if (dev->hdmi_proc_scdc_check == NULL)
		pr_err("[ERROR][HDMI_V20]%s:failed  @scdc_check\n", __func__);
	//pr_info("[INFO][HDMI_V20]%s:Installing ddc_check file\n", __func__);
	dev->hdmi_proc_ddc_check =
		proc_create_data(
			"ddc_check", S_IFREG | 0222,
			dev->hdmi_proc_dir, &proc_fops_ddc_check, dev);
	if (dev->hdmi_proc_ddc_check == NULL)
		pr_err("[ERROR][HDMI_V20]%s:failed  @ddc_check\n", __func__);
	#if defined(CONFIG_TCC_RUNTIME_GET_EDID_ID)
	//pr_info(
		//"[INFO][HDMI_V20]%s:Installing edid_machine_id file\n",
		//__func__);
	dev->hdmi_proc_edid_machine_id =
		proc_create_data(
			"edid_machine_id", S_IFREG | 0444 | 0222,
			dev->hdmi_proc_dir, &proc_fops_edid_machine_id, dev);
	if (dev->hdmi_proc_edid_machine_id == NULL)
		pr_err("[ERROR][HDMI_V20]%s:failed  @edid_machine_id\n",
		__func__);

	#endif

	#if defined(CONFIG_TCC_RUNTIME_DRM_TEST)
	//pr_info("[INFO][HDMI_V20]%s:Installing drm file\n", __func__);
	dev->hdmi_proc_drm =
		proc_create_data(
			"drm", S_IFREG | 0444 | 0222,
			dev->hdmi_proc_dir, &proc_fops_drm, dev);
	if (dev->hdmi_proc_drm == NULL)
		pr_err("[ERROR][HDMI_V20]%s:failed  @drm\n", __func__);
	#endif

	#if defined(CONFIG_TCC_RUNTIME_TUNE_HDMI_PHY)
	//pr_info("[INFO][HDMI_V20]%s:Installing phy_regs file\n", __func__);
	dev->hdmi_proc_phy_regs =
		proc_create_data(
			"phy_regs",  S_IFREG | 0444 | 0222,
			dev->hdmi_proc_dir, &proc_fops_phy_regs, dev);
	if (dev->hdmi_proc_phy_regs == NULL)
		pr_err("[ERROR][HDMI_V20]%s:failed  @phy_regs\n", __func__);
	#endif
	...
	if (dev->hdmi_proc_vsif == NULL) {
		pr_err("[ERROR][HDMI_V20]%s:failed  @vsif\n", __func__);
	#endif


	#if defined(CONFIG_TCC_RUNTIME_DV_VSIF)
	dev->hdmi_proc_dv_vsif =
		proc_create_data(
			"dv_vsif", S_IFREG | 0222,
			dev->hdmi_proc_dir, &proc_fops_dv_vsif, dev);
	if (dev->hdmi_proc_dv_vsif == NULL)
		pr_err("[ERROR][HDMI_V20]%s:failed  @dv_vsif\n", __func__);
	#endif

	#if defined(CONFIG_TCC_AUDIO_CHANNEL_MUX)
	dev->hdmi_proc_audio_channel_mux =
		proc_create_data(
			"audio_channel_mux",
			S_IFREG | 0444 | 0222,
	...
	if (dev->hdmi_proc_audio_channel_mux == NULL)
		pr_err(
			"[ERROR][HDMI_V20]%s:failed  @audio_channel_mux\n",
			__func__);
	#endif
	...
	if (dev->hdmi_proc_debug == NULL)
		pr_err("[ERROR][HDMI_V20]%s:failed  @debug\n",
		__func__);
}
