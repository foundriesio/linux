...
void hdmi_stop(void)
{
	...
	if (!test_bit(HDMI_TX_STATUS_SUSPEND_L1, &hdmi_apis.dev->status)) {
		if (
			test_bit(
				HDMI_TX_STATUS_OUTPUT_ON,
					&hdmi_apis.dev->status)) {
...
unsigned int hdmi_get_refreshrate(void)
{
	...
	videoParams_t *videoParams =
		(hdmi_apis.dev != NULL) ?
		(videoParams_t *)hdmi_apis.dev->videoParam : NULL;
...
void hdmi_activate_callback(void)
{
	...
	if (hdmi_apis.callback != NULL)
		hdmi_apis.callback(
			hdmi_apis.callback_hdmi_fbi,
			hdmi_apis.callback_hdmi_stage);
}
...
void hdmi_api_AvMute(int enable)
{
	...
	do {
		if (hdmi_apis.dev == NULL) {
			pr_err(
				"[ERROR][HDMI_V20]%s device is not ready\r\n",
				__func__);
			break;
		}
...
int hdmi_api_vsif_update(productParams_t *productParams)
{
	...
	if (productParams == NULL) {
		pr_err(
			"[ERROR][HDMI_V20]%s productParams is NULL\r\n",
			__func__);
		goto out;
	}
...
int hdmi_api_vsif_update_for_hdr_10p(productParams_t *productParams)
{
	...
	if (!dwc_hdmi_is_suspended(hdmi_apis.dev)) {
		...
		if (!test_bit(
			...
			&hdmi_apis.dev->status)) {
			dpr_info(
				"[INFO][HDMI_V20] %s has not permissions\r\n",
				__func__);
			mutex_unlock(&hdmi_apis.dev->mutex);
			goto out;
		}
...
int hdmi_api_vsif_update_by_index(int index)
{
	...
	if (videoParams == NULL) {
		pr_err(
			"[ERROR][HDMI_V20]%s videoParams is NULL\r\n",
			__func__);
		goto out;
	}
	...
	if (hdmi_apis.dev->dolbyvision_visf_list == NULL) {
		pr_err(
			"[ERROR][HDMI_V20]%s dolbyvision vsif is empty\r\n",
			__func__);
		goto out;
	}
	...
	if (index < 0 || index > 5) {
		pr_err(
			"[ERROR][HDMI_V20]%s index(%d) is out of range\r\n",
			__func__, index);
		goto out;
	}
	...
	if (!dwc_hdmi_is_suspended(hdmi_apis.dev)) {
		...
		productParams.mOUI =
			base_vsif[0] |
			(base_vsif[1] << 8) | (base_vsif[2] << 16);
		productParams.mVendorPayloadLength = base_vsif[3];
		memcpy(
			productParams.mVendorPayload,
			&base_vsif[4], productParams.mVendorPayloadLength);
...
static int hdmi_api_update_avi_infoframe(videoParams_t *videoParam)
{
	...
	if (videoParam == NULL) {
		pr_err(
			"[ERROR][HDMI_V20]%s videoParam is NULL\r\n", __func__);
		goto out;
	}
	...
	if (!dwc_hdmi_is_suspended(hdmi_apis.dev)) {
		if (
			!test_bit(
				HDMI_TX_STATUS_POWER_ON,
					&hdmi_apis.dev->status)) {
			pr_err("[ERROR][HDMI_V20]%s No power <%d>\r\n",
			__func__, __LINE__);
			goto out;
		}
		...
		if (
			packets_Configure(
				hdmi_apis.dev, videoParam, NULL) < 0) {
			pr_err("[ERROR][HDMI_V20]%s packet  <%d>\r\n",
			__func__, __LINE__);
			goto out;
		}
		/* Update Video parameters */
		memcpy(
			hdmi_apis.dev->videoParam,
			videoParam, sizeof(videoParams_t));
		...
		do {
			usleep_range(10, 20);
			mc_reg_val =
			hdmi_dev_read(hdmi_apis.dev, MC_SWRSTZREQ);
		} while (mc_timeout-- && mc_reg_val != 0xDF);
		if (mc_timeout < 1) {
			pr_info(
				"[INFO][HDMI_V20]%s MC timeout \r\n",
				__func__);
		}

		fc_video_VSyncPulseWidth(
			hdmi_apis.dev,
			videoParam->mDtd.mVSyncPulseWidth);
		ret = 0;
	} else {
		pr_err(
			"[ERROR][HDMI_V20] %s line(%d) ## linke suspended\r\n",
	...
	ext_colorimetry_t ext_colorimetry)
{
	...
	if (hdmi_apis.dev->videoParam == NULL) {
		pr_err(
			"[ERROR][HDMI_V20]%s videoParam of dev is NULL\r\n",
			__func__);
		goto out;
	}
	mutex_lock(&hdmi_apis.dev->mutex);
	if (!dwc_hdmi_is_suspended(hdmi_apis.dev)) {
		...
		memcpy(
			&videoParam,
			hdmi_apis.dev->videoParam, sizeof(videoParams_t));
 ...
 *	[0] For RGB, This value means that the quantization
 *		range corresponds to the default RGB quantization range.
 *		For YUV, this value means that the quantization range
 *		corresponds to the  limited quantization range
 *	[1] For RGB, This value means that the quantization range
 *		corresponds to the limited quantization range.
 *		For YUV, this value means that the quantization range
 *		corresponds to the  limited quantization range
 *	[2] For RGB, This value means that the quantization range
 *		corresponds to the full quantization range.
 *		For YUV, this value means that the quantization range
 *		corresponds to the full quantization range
 * @return Return the result of this function
 * @retval 0 if this function succeeded \n
	...
	if (!dwc_hdmi_is_suspended(hdmi_apis.dev)) {
		...
		for (i = 0x00000400; i <= 0x000007FC; i += 4) {
			...
			if ((i & 0xF) == 0)
				...
				pr_info("[INFO][HDMI_V20] %02x ", reg_val);
		}
		pr_info("[INFO][HDMI_V20]\r\nDUMP Video Sampler\r\n");
