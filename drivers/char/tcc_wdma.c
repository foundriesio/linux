...
//#define WDMA_DEBUG
#ifdef WDMA_DEBUG
#define dprintk(msg...) pr_info("[DBG][WDMA] " msg)
#else
#define dprintk(msg...)
#endif

/* To debug wdma image
 *  image buffer set to 0x12 before writing the image data by WDMA block.
 */
...
unsigned int wdma_queue_list_init(struct wdma_queue_list *frame_list,
	struct vioc_wdma_frame_info *cmd_data)
{
	...
	*frame_list->data = *cmd_data;
	...
	for (i = 0; i < frame_list->q_max_cnt; i++) {
		...
		tccxxx_GetAddress(cmd_data->frame_fmt, base_addr,
...
unsigned int wdma_get_writable_buffer_index(struct wdma_queue_list *frame_list)
{
	...
	for (i = 0; i < frame_list->q_max_cnt; i++) {
		...
		if ((frame_list->wbuf_list[cur_index].status == PREPARED_S)
		|| (frame_list->wbuf_list[cur_index].status == WRITED_S)) {
			break;
		}
	}
...
int wdma_find_index_of_status(struct wdma_queue_list *frame_list,
	unsigned int wdma_s)
{
	int i, find_index = 0;
...
char tccxxx_wdma_ctrl(unsigned long argp, struct tcc_wdma_dev *pwdma_data)
{
	...
	if (copy_from_user((void *)&ImageCfg, (const void __user *)argp,
		sizeof(ImageCfg))) {
	...
	if (pwdma_data->sc.reg) {
		VIOC_SC_SetBypass(pwdma_data->sc.reg, 0);
		VIOC_SC_SetDstSize(pwdma_data->sc.reg, ImageCfg.TargetWidth,
			ImageCfg.TargetHeight);
		VIOC_SC_SetOutSize(pwdma_data->sc.reg, ImageCfg.TargetWidth,
			ImageCfg.TargetHeight);
		VIOC_SC_SetOutPosition(pwdma_data->sc.reg, 0, 0);
		VIOC_CONFIG_PlugIn(pwdma_data->sc.id, pwdma_data->wdma.id);
		VIOC_SC_SetUpdate(pwdma_data->sc.reg);
	...
	vioc_intr_enable(pwdma_data->irq, pwdma_data->vioc_intr->id,
		pwdma_data->vioc_intr->bits);
	ret = wait_event_interruptible_timeout(pwdma_data->poll_wq,
		pwdma_data->block_operating == 0, msecs_to_jiffies(100));
	if (ret <= 0) {
		pwdma_data->block_operating = 0;
...
static unsigned int tccxxx_wdma_poll(struct file *filp, poll_table *wait)
{
	...
	if (wdma_data->block_operating == 0)
...
static irqreturn_t tccxxx_wdma_handler(int irq, void *client_data)
{
	...
	if (wdma_data->wdma_contiuous) {
		...
		cur_id = wdma_find_index_of_status(&wdma_data->frame_list,
			WRITING_S);
		next_id = wdma_get_writable_buffer_index(
			&wdma_data->frame_list);
		tc_wdrv_wdma_addr_set(wdma_data,
		...
		wdma_set_index_of_status(&wdma_data->frame_list, next_id,
			WRITING_S);
		wdma_set_index_of_status(&wdma_data->frame_list, cur_id,
			WRITED_S);
...
#if 0
		if (wdma_data->block_waiting) {
			pr_info("%s C:%d N:%d, Y:0x%08x U:0x%08x V:0x%08x\n",
...
long tccxxx_wdma_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case 0x100:
	{
		...
		cmd_data.buffer_num = wdma_queue_list_init(
			&wdma_data->frame_list, &cmd_data);
		wdma_data->wdma_contiuous = 1;

		if (wdma_data->sc.reg) {
			tc_wdrv_scaler_set(wdma_data, cmd_data.frame_x,
				cmd_data.frame_y);
		}

		wdma_data->frame_list.q_index = 0;
		tc_wdrv_wdma_path_set(wdma_data, &cmd_data,
	...
	case 0x101:
	{
		...
		before_id = wdma_find_index_of_status(
			&wdma_data->frame_list, UPLOAD_S);
		...
		wdma_data->frame_list.data->frame_fmt;
			...
			wdma_set_index_of_status(&wdma_data->frame_list,
				wbuffer.index, UPLOAD_S);
		}
	...
	case 0x102:
	{
		...
		before_id = wdma_find_index_of_status(
			&wdma_data->frame_list, UPLOAD_S);
		cur_id = wdma_find_index_of_status(
			&wdma_data->frame_list, WRITED_S);
		pr_info("[INF][WDMA] index : %d  before_id : %d\n",
			cur_id, before_id);
	}
	...
	case TC_WDRV_COUNT_START:
	{
		...
		if (copy_from_user(&cmd_data, (const void __user *)arg,
		...
		vioc_intr_clear(wdma_data->vioc_intr->id,
			wdma_data->vioc_intr->bits);
		vioc_intr_enable(wdma_data->irq, wdma_data->vioc_intr->id,
			wdma_data->vioc_intr->bits);
		tc_wdrv_wdma_path_set(wdma_data, &cmd_data,
	(struct wdma_buffer_list *)&wdma_data->frame_list.wbuf_list[0]);
		if (copy_to_user((struct vioc_wdma_frame_info *)arg,
	...
	#ifdef WDMA_IMAGE_DEBUG
	{
		...
		if (checking_loop >= CHECKING_AREA(
			...
			wdma_data->frame_list.data->frame_y)) {
			pr_err(
				"[ERR][WDMA] get buffer image is same with CHECKING_NUM checking_area : %d\n",
			CHECKING_AREA(
				wdma_data->frame_list.data->frame_x,
				wdma_data->frame_list.data->frame_y));
		}
	...
	case TC_WDRV_GET_CUR_DATA:
	{
		...
		if (ret <= 0) {
			wdma_data->block_operating = 0;
			pr_info(
				"[INF][WDMA] ret: %d : wdma 0 timed_out block_operation:%d!! cmd_count:%d\n",
				ret, wdma_data->block_waiting,
				wdma_data->cmd_count);
		}
		wdma_data->block_waiting = 0;
	...
	case TC_WDRV_COUNT_END:
		...
		if (ret <= 0)	{
			pr_info(
				"[INF][WDMA] [%d]  block_operation:%d!! cmd_count:%d\n",
				ret, wdma_data->block_waiting,
				wdma_data->cmd_count);
		}
		wdma_data->block_waiting = 0;
		...
		vioc_intr_clear(wdma_data->vioc_intr->id,
			wdma_data->vioc_intr->bits);
		vioc_intr_disable(wdma_data->irq,
...
int tccxxx_wdma_release(struct inode *inode, struct file *filp)
{
	...
	if (pwdma_data->dev_opened == 0) {
		...
		if (ret <= 0) {
			pr_info(
				"[INF][WDMA] [%d]: wdma timed_out block_operation: %d, block_waiting:%d  cmd_count: %d.\n",
...
int tccxxx_wdma_open(struct inode *inode, struct file *filp)
{
	...
	if (!pwdma_data->irq_reged) {
		dprintk("wdma irq id:0x%x bits :0x%x\n",
			...
			pwdma_data->vioc_intr->bits);
		vioc_intr_clear(pwdma_data->vioc_intr->id,
			pwdma_data->vioc_intr->bits);
		vioc_intr_enable(pwdma_data->irq, pwdma_data->vioc_intr->id,
			pwdma_data->vioc_intr->bits);
	...
	struct tcc_wdma_dev *pwdma_data)
{
	...
	if (vioc_node) {
		if (of_property_read_u32(vioc_node, "telechips,fbdisplay_num",
...
static int wdma_screen_capture(char *Capture)
{
	...
	// Open wdma device
	filp = filp_open("/dev/wdma_drv@0", O_RDWR, 0600/*S_IRUSR|S_IWUSR*/);
	if (IS_ERR(filp)) {
	...
	image_virt_addr =
		dma_alloc_coherent(
			misc->parent, image_size, &handle, GFP_KERNEL);
	if (!image_virt_addr) {
	...
	if (ret >= 0) {
		if (wdma_data->block_operating >= 1) {
			pr_info(
				"[INF][WDMA] wdma driver :: block_operating(%d) - block_waiting(%d) - cmd_count(%d) - poll_count(%d)!!!\n",
				wdma_data->block_operating,
				wdma_data->block_waiting,
				wdma_data->cmd_count,
				wdma_data->poll_count);
		...
		tccxxx_GetAddress(
			ImageCfg.ImgFormat, (unsigned int)base_addr,
			ImageCfg.TargetWidth, ImageCfg.TargetHeight, 0, 0,
			&addr_Y, &addr_U, &addr_V);

		if (ImageCfg.ImgFormat == TCC_LCDC_IMG_FMT_YUV420SP ||
			ImageCfg.ImgFormat == TCC_LCDC_IMG_FMT_RGB888) {
			addr_U = GET_ADDR_YUV42X_spU(
				base_addr, ImageCfg.TargetWidth,
				ImageCfg.TargetHeight);

			if (ImageCfg.ImgFormat == TCC_LCDC_IMG_FMT_YUV420SP) {
				addr_V = GET_ADDR_YUV420_spV(
					addr_U, ImageCfg.TargetWidth,
					ImageCfg.TargetHeight);
			} else {
				addr_V = GET_ADDR_YUV422_spV(
					addr_U, ImageCfg.TargetWidth,
					ImageCfg.TargetHeight);
			}
		}
		...
		VIOC_WMIX_GetSize(
			wdma_data->wmix.reg, &Wmix_Width, &Wmix_Height);

		DDevice =
			VIOC_DISP_Get_TurnOnOff(
				wdma_data->disp_info.virt_addr);
		if (
			(Wmix_Width == 0) || (Wmix_Height == 0) ||
			(DDevice == 0)) {
			wdma_data->block_operating = 0;
			pr_err(
				"[ERR][WDMA] %s W:%d H:%d DD-Power:%d\n",
				__func__,
				Wmix_Width, Wmix_Height, DDevice);
			return -1;
		}
		...
		dprintk(
			"src  w:%d h:%d base:0x%08x\n", ImageCfg.ImgSizeWidth,
			ImageCfg.ImgSizeHeight, ImageCfg.BaseAddress);
		dprintk(
			"dest w:%d h:%d fb_dd_num:%d\n", ImageCfg.TargetWidth,
			ImageCfg.TargetHeight, wdma_data->fb_dd_num);
		dprintk(
			"wmixer size %d %d :base1:0x%08x base2:0x%08x\n",
			Wmix_Width, Wmix_Height, ImageCfg.BaseAddress1,
			ImageCfg.BaseAddress2);
		dprintk(
			"ImgFormat:%d  set effect : hue:%d, bright:%d contrast:%d\n",
			ImageCfg.ImgFormat, ImageCfg.Hue, ImageCfg.Bright,
			ImageCfg.Contrast);
		...
		if (wdma_data->sc.reg) {
			...
			VIOC_SC_SetDstSize(
				wdma_data->sc.reg, ImageCfg.TargetWidth,
				ImageCfg.TargetHeight);
			VIOC_SC_SetOutSize(
				wdma_data->sc.reg, ImageCfg.TargetWidth,
				ImageCfg.TargetHeight);
			...
			VIOC_CONFIG_PlugIn(
				wdma_data->sc.id, wdma_data->wdma.id);
		...
		VIOC_DISP_GetDisplayBlock_Info(
			wdma_data->disp_info.virt_addr, &DDinfo);
		dd_rgb = VIOC_DISP_FMT_isRGB(DDinfo.pCtrlParam.pxdw);

		if (ImageCfg.ImgFormat <= TCC_LCDC_IMG_FMT_ARGB6666_3) {
			if (dd_rgb) {
				VIOC_WDMA_SetImageY2RMode(
					wdma_data->wdma.reg, 0x2);
				VIOC_WDMA_SetImageY2REnable(
					wdma_data->wdma.reg, 0);
				...
				VIOC_WDMA_SetImageY2RMode(
					wdma_data->wdma.reg, 0x2);
				VIOC_WDMA_SetImageY2REnable(
					wdma_data->wdma.reg, 1);
				VIOC_WDMA_SetImageR2YEnable(
					wdma_data->wdma.reg, 0);
			}
		} else {
			if (dd_rgb) {
				VIOC_WDMA_SetImageR2YMode(
					wdma_data->wdma.reg, 0x2);
				VIOC_WDMA_SetImageR2YEnable(
					wdma_data->wdma.reg, 1);
				VIOC_WDMA_SetImageY2REnable(
		...
		VIOC_WDMA_SetImageSize(
			wdma_data->wdma.reg, ImageCfg.TargetWidth,
			ImageCfg.TargetHeight);
		VIOC_WDMA_SetImageFormat(
			wdma_data->wdma.reg, ImageCfg.ImgFormat);
		VIOC_WDMA_SetImageOffset(
			wdma_data->wdma.reg, ImageCfg.ImgFormat,
			ImageCfg.TargetWidth);
		VIOC_WDMA_SetImageBase(
			wdma_data->wdma.reg, ImageCfg.BaseAddress,
			ImageCfg.BaseAddress1, ImageCfg.BaseAddress2);
		VIOC_WDMA_SetImageRGBSwapMode(wdma_data->wdma.reg, 0);
		#if defined(CONFIG_ARCH_TCC899X) || \
			defined(CONFIG_ARCH_TCC901X)
		VIOC_WDMA_SetImageEnhancer(
			wdma_data->wdma.reg, ImageCfg.Contrast,
			ImageCfg.Bright, ImageCfg.Hue);
	#endif
		VIOC_WDMA_SetImageEnable(
			wdma_data->wdma.reg, ImageCfg.ContinuousMode);

		spin_unlock_irq(&(wdma_data->cmd_lock));
		wdma_data->vioc_intr->bits = (1 << VIOC_WDMA_INTR_EOFR);
		vioc_intr_clear(
			wdma_data->vioc_intr->id, wdma_data->vioc_intr->bits);
		vioc_intr_enable(wdma_data->irq, wdma_data->vioc_intr->id,
			wdma_data->vioc_intr->bits);
	...
	filp_mem = filp_open(
		pName, O_CREAT | O_WRONLY, 0600/*S_IRUSR|S_IWUSR*/);
	if (IS_ERR(filp_mem)) {
	...
	ret = vfs_write(
		filp_mem,  (char __user *)image_virt_addr, image_size,
		&filp_mem->f_pos);

	vfs_fsync(filp_mem, 0);
	set_fs(oldfs);
	dma_free_coherent(
		misc->parent, image_size, image_virt_addr, handle);
...
DEVICE_ATTR(
	screen_capture, 0644/*S_IRUGO | S_IWUSR*/, wdma_screen_capture_show,
	wdma_screen_capture_store);
...
static int  tcc_wdma_probe(struct platform_device *pdev)
{
	...
	pr_info("TCC WDMA Driver Initializing\n");
	...
	if (dev_np) {
		of_property_read_u32_index(pdev->dev.of_node, "scalers", 1,
			&wdma_data->sc.id);
	...
	if (dev_np) {
		of_property_read_u32_index(pdev->dev.of_node, "wmixs", 1,
			&wdma_data->wmix.id);
	...
	if (dev_np) {
		of_property_read_u32_index(pdev->dev.of_node, "wdmas", 1,
			&wdma_data->wdma.id);
		wdma_data->wdma.reg = VIOC_WDMA_GetAddress(wdma_data->wdma.id);
		wdma_data->irq = irq_of_parse_and_map(dev_np,
			get_vioc_index(wdma_data->wdma.id));
		wdma_data->vioc_intr->id = VIOC_INTR_WD0
			+ get_vioc_index(wdma_data->wdma.id);
