...
int32_t tcc_hsm_sp_cmd_get_version(uint32_t *major, uint32_t *minor)
{
	...
	mbox_result_size = \
        sp_sendrecv_cmd(TCCHSM_CMD_GET_VERSION, mbox_data, sizeof(mbox_data), mbox_result, sizeof(mbox_result));
    ...
    uint32_t keyIndex, uint32_t algorithm, uint32_t opMode, uint32_t residual, uint32_t sMsg)
{
	...
	mbox_result_size = \
        sp_sendrecv_cmd(TCCHSM_CMD_SET_MODE, mbox_data, sizeof(mbox_data), &mbox_result, sizeof(mbox_result));
	if (mbox_result_size < 0) {
		dprintk("[%s:%d] sp_sendrecv_cmd error(%d)\n", __func__, __LINE__, mbox_result_size);
		result = -EBADR;
		goto out;
    ...
    uint32_t keyIndex, uint32_t keyType, uint32_t keyMode, uint8_t *key, uint32_t keySize)
{
	...
	mbox_result_size = \
        sp_sendrecv_cmd(TCCHSM_CMD_SET_KEY, mbox_data, sizeof(mbox_data), &mbox_result, sizeof(mbox_result));
	if (mbox_result_size < 0) {
		dprintk("[%s:%d] sp_sendrecv_cmd error(%d)\n", __func__, __LINE__, mbox_result_size);
		result = -EBADR;
		goto out;
...
int32_t tcc_hsm_sp_cmd_set_iv(uint32_t keyIndex, uint8_t *iv, uint32_t ivSize)
{
	...
	mbox_result_size = \
        sp_sendrecv_cmd(TCCHSM_CMD_SET_IV, mbox_data, sizeof(mbox_data), &mbox_result, sizeof(mbox_result));
	if (mbox_result_size < 0) {
		dprintk("[%s:%d] sp_sendrecv_cmd error(%d)\n", __func__, __LINE__, mbox_result_size);
		result = -EBADR;
		goto out;
    ...
    uint32_t keyIndex, uintptr_t *klData, uint32_t klDataSize)
{
	...
	mbox_result_size = \
        sp_sendrecv_cmd(TCCHSM_CMD_SET_KLDATA, mbox_data, sizeof(mbox_data), &mbox_result, sizeof(mbox_result));
	if (mbox_result_size < 0) {
		dprintk("[%s:%d] sp_sendrecv_cmd error(%d)\n", __func__, __LINE__, mbox_result_size);
		result = -EBADR;
		goto out;
...
int32_t tcc_hsm_sp_cmd_write_otp(uint32_t otpAddr, uint8_t *otpBuf, uint32_t otpSize)
{
	...
	mbox_result_size =  \
        sp_sendrecv_cmd(TCCHSM_CMD_WRITE_OTP, mbox_data, sizeof(mbox_data), &mbox_result, sizeof(mbox_result));
	if (mbox_result_size < 0) {
		dprintk("[%s:%d] sp_sendrecv_cmd error(%d)\n", __func__, __LINE__, mbox_result_size);
		result = -EBADR;
		goto out;
...
int32_t tcc_hsm_sp_cmd_get_rand(uint8_t *rng, int32_t rngSize)
{
	...
	mbox_result_size =  \
        sp_sendrecv_cmd(TCCHSM_CMD_GET_RNG, &mbox_data, sizeof(mbox_data), mbox_result, sizeof(mbox_result));
	if (mbox_result_size < 0) {
		dprintk("[%s:%d] sp_sendrecv_cmd error(%d)\n", __func__, __LINE__, mbox_result_size);
		result = -EBADR;
		goto out;
