...
int send_update_start(struct snor_updater_device *updater_dev)
{
	...
	if (ret == SNOR_UPDATER_SUCCESS) {
		if (receiveMsg.cmd[0] ==
			...
			if (receiveMsg.cmd[1] !=
				(unsigned int)SNOR_UPDATE_ACK)	{

				if (receiveMsg.cmd[2]  ==
					(unsigned int)NACK_FIRMWARE_LOAD_FAIL) {
					...
					(unsigned int)NACK_SNOR_INIT_FAIL) {
...
int send_update_done(struct snor_updater_device *updater_dev)
{
	...
	sendMsg.cmd[0] = (unsigned int)UPDATE_DONE;
	sendMsg.data_len = 0;
	...
	if (ret == SNOR_UPDATER_SUCCESS) {
		if (receiveMsg.cmd[0] ==
			...
			if (receiveMsg.cmd[1] !=
				...
				if (receiveMsg.cmd[2]  ==
					(unsigned int)NACK_SNOR_ACCESS_FAIL) {
					ret = SNOR_UPDATER_ERR_SNOR_ACCESS_FAIL;

				} else if (receiveMsg.cmd[2]  ==
					(unsigned int)NACK_CRC_ERROR) {
					ret =
	...
	unsigned int fwDataSize, unsigned int fwdataCRC)
{
	...
	if (ret == SNOR_UPDATER_SUCCESS) {
		ret = snor_updater_wait_event_timeout(
			updater_dev,
			(unsigned int)UPDATE_FW_SEND_ACK,
			&receiveMsg,
			(unsigned int)ACK_TIMEOUT);
		if (ret != SNOR_UPDATER_SUCCESS) {
			eprintk(updater_dev->dev,
...
int send_fw_done(struct snor_updater_device *updater_dev)
{
	...
	if (ret == SNOR_UPDATER_SUCCESS) {
		...
		ret = snor_updater_wait_event_timeout(
			updater_dev,
			(unsigned int)UPDATE_FW_COMPLETE,
			&receiveMsg,
			(unsigned int)ACK_TIMEOUT);
