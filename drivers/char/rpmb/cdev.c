...
static int rpmb_ioctl_info_cmd(struct rpmb_dev *rdev,
		struct rpmb_info_cmd __user *ptr)
{
	struct rpmb_info_cmd cmd;
	...
	rpmbd = kmalloc(sizeof(struct rpmb_info_data), GFP_KERNEL);

	rpmbd->req_type = cmd.req;

	rpmbd->ufs_info = memdup_user(u64_to_ptr(cmd.desc_info_ptr), sizeof(struct ufs_desc_info));
	if (IS_ERR(rpmbd->ufs_info)) {
		ret = PTR_ERR(rpmbd->ufs_info);
		rpmbd->ufs_info = NULL;
		dev_err(&rdev->dev, "Failed to get desc info ptr = %d.\n", ret);
		goto out;
	}

	ret  = rpmb_send_info_req(rdev, rpmbd);
	if (ret) {
		dev_err(&rdev->dev, "Failed to process request = %d.\n", ret);
		goto out;
	}

	if (copy_to_user(u64_to_ptr(cmd.desc_info_ptr), rpmbd->ufs_info, sizeof(struct ufs_desc_info))) {
		ret = -EFAULT;
		goto out;
	}
	if (copy_to_user(ptr, &cmd, sizeof(cmd))) {
	...
	dev_dbg(&rdev->dev, "%s : %s\n", __func__, rpmbd->ufs_info->model);
	dev_dbg(&rdev->dev, "%s : 0x%x\n", __func__, rpmbd->ufs_info->bRPMB_ReadWriteSize);
	dev_dbg(&rdev->dev, "%s : 0x%x\n", __func__, rpmbd->ufs_info->bRPMBRegion0Size);
...
static long rpmb_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	} else if (cmd == RPMB_INFO_REQ) {
		struct rpmb_info_cmd __user *req = (void __user *)arg;
		return rpmb_ioctl_info_cmd(rdev, req);
	} else {
