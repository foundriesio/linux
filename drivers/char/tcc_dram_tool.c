...
{
	struct dram_tool_platform_data *pd = container_of(work, struct dram_tool_platform_data, monitor_work.work);
	struct arm_smccc_res res;


	if(pd->monitor_enable){
		//printk("[%s] monitor status = 0x%08x\n", __func__, readl(pd->omc_base + 0x798));

		if(readl(pd->omc_base + 0x798) == 0){
			if(pd->cycle_time > 10){
				arm_smccc_smc(SIP_DRAM_TOOL_TM_SET, msec_to_cycle(pd->cycle_time), 0, 0, 0, 0, 0, 0, &res);
				schedule_delayed_work(&pd->monitor_work, msecs_to_jiffies(pd->cycle_time-1));		
			}
			else{
				printk("Bandwidth monitor error : measure time should be over 10 ms!\n");
				schedule_delayed_work(&pd->monitor_work, msecs_to_jiffies(MONITOR_IDLE_TIME));		
			}
		}
		else if((readl(pd->omc_base + 0x798)&(1<<30)) != 0){
		// print cycle, read_num, write_num
			printk("CH - 0, Cycle = %d, Read_num = %d, Write_num = %d", readl(pd->omc_base + 0x79C), readl(pd->omc_base + 0x7AC), readl(pd->omc_base + 0x7B0));
			arm_smccc_smc(SIP_CHIP_NAME, 0, 0, 0, 0, 0, 0, 0, &res);
			if(res.a0 == 0x8990)
				printk(" CH - 1, Cycle = %d, Read_num = %d, Write_num = %d\n", readl(pd->omc_base2 + 0x79C), readl(pd->omc_base2 + 0x7AC), readl(pd->omc_base2 + 0x7B0));	
			else
				printk("\n");
			...
			printk("[%s] Monitor check out time\n", __func__);			
			schedule_delayed_work(&pd->monitor_work, msecs_to_jiffies(MONITOR_CHECK_TIME));	
		}
...
static ssize_t monitor_enable_read(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static ssize_t monitor_enable_write(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct dram_tool_platform_data *pdata = dev_get_drvdata(dev);
	int ret = 0;
	bool val;
...
static ssize_t cycle_time_read(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct dram_tool_platform_data *pdata = dev_get_drvdata(dev);
...
static ssize_t cycle_time_write(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct dram_tool_platform_data *pdata = dev_get_drvdata(dev);
	int ret = 0;
...
static DEVICE_ATTR(monitor_enable, 0644, monitor_enable_read, monitor_enable_write);
...
static int dram_tool_probe(struct platform_device *pdev)
{
	...
	pdata = devm_kzalloc(&pdev->dev, sizeof(struct dram_tool_platform_data), GFP_KERNEL);
	...
	if(ret)
		printk("[DRAM_TOOL] error creating sysfs entries\n");
	...
	printk("[DRAM_TOOL] probe is done\n");


	schedule_delayed_work(&pdata->monitor_work, msecs_to_jiffies(MONITOR_START_TIME));
