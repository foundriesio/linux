...
//#define VIOMG_DEBUG
#ifdef VIOMG_DEBUG
#define dprintk(msg...) pr_info("VIOCMG: " msg)
#else
#define dprintk(msg...)
#endif
...
static struct viocmg_soc *viocmg;

struct viocmg_soc *viocmg_get_soc(void)
{
	if (viocmg == NULL) {
		pr_err("%s Error.. VIOCMG is NULL..!!!\n", __func__);
	}
	return viocmg;
}
...
int viocmg_lock_viqe(unsigned int caller_id)
{
	...
	struct viocmg_soc *viocmg = viocmg_get_soc();
	VIOC_IREQ_CONFIG *pVIOCConfig =
		(VIOC_IREQ_CONFIG *)viocmg->reg.camera_config;

	mutex_lock(&viocmg->mutex_viqe);

	if (caller_id != VIOCMG_CALLERID_REAR_CAM
		&& viocmg->viocmg_dt_info->feature_rear_cam_use_viqe
		&& viocmg->viocmg_dt_info->rear_cam_mode > 0) {
		dprintk("warring rear camera using viqe\n");
		goto END_PROCESS;
	}

	if (viocmg->components.viqe.owner != VIOCMG_CALLERID_NONE
	...
	if (caller_id == VIOCMG_CALLERID_REAR_CAM) {
		...
		&& (vioc_plug_in.connect_statue != VIOC_PATH_DISCONNECTED)) {
			...
			VIOC_CONFIG_SWReset(pVIOCConfig, VIOC_CONFIG_VIQE,
				viocmg->viocmg_dt_info->rear_cam_cam_config,
				VIOC_CONFIG_RESET);
			VIOC_CONFIG_SWReset(pVIOCConfig, VIOC_CONFIG_VIQE,
				viocmg->viocmg_dt_info->rear_cam_cam_config,
				VIOC_CONFIG_CLEAR);
...
static int tcc_viocmg_parse_fbdisplay(struct platform_device *pdev)
{
	...
	// main_display_ovp
	if (of_property_read_u32(np_fbdisplay, "telechips,fbdisplay_num",
		&val)) {
	...
	// fbdisplay_port
	if (of_property_read_u32(np_fbdisplay, "telechips,fbdisplay_port",
		&val)) {
	...
	// main_display_ovp
	if (of_property_read_u32(np_fbdisplay, "telechips,overlay_priority",
		&val)) {
...
static int tcc_viocmg_parse_camera_common(struct platform_device *pdev)
{
	...
	result = of_property_read_u32_index(np_camera, "camera_wmixer", 1,
		&val);
	if (result) {
	...
	result = of_property_read_u32_index(np_camera, "camera_videoin", 2,
		&val);
	if (result) {
	...
	result = of_property_read_u32_index(np_camera, "camera_scaler", 1,
		&val);
	if (result) {
	...
	result = of_property_read_u32_index(np_camera, "camera_config", 1,
		&val);
	if (result) {
		pr_err("could not find index from camera_scaler\n");
		goto END_PROCESS;
...
static int tcc_viocmg_parse_viocmg_common(struct platform_device *pdev)
{
	...
	if (result) {
		pr_err("cann't parse telechips,disp_rdma\n");
		goto END_PROCESS;
	}
	...
	if (result) {
		pr_err("cann't parse telechips,rear_cam_enable\n");
		goto END_PROCESS;
	}
	...
	if (result) {
		pr_err("cann't parse telechips,rear_cam_use_viqe\n");
		goto END_PROCESS;
	}
	...
	if (result) {
		pr_err("cann't parse telechips,rear_cam_viqe_mode\n");
		goto END_PROCESS;
	}
	viocmg->viocmg_dt_info->feature_rear_cam_viqe_mode = val;

	result = of_property_read_u32(np, "telechips,rear_cam_use_parking_line",
		&val);
	if (result) {
		pr_err("cann't parse telechips,rear_cam_use_parking_line\n");
		goto END_PROCESS;
	}
	...
	if (result) {
		pr_err("cann't parse telechips,rear_cam_mode\n");
		goto END_PROCESS;
	}
	...
	if (result) {
		pr_err("cann't parse telechips,rear_cam_ovp\n");
		goto END_PROCESS;
	}
	pr_info("rear_cam_ovp = %d\n", val);
...
static int tcc_viocmg_parse_hw_reg(struct platform_device *pdev)
{
	...
	viocmg->reg.display_wmix = of_iomap(np_phandle,
		viocmg->viocmg_dt_info->main_display_id);
	...
	viocmg->reg.camera_config = of_iomap(np_phandle,
		viocmg->viocmg_dt_info->rear_cam_cam_config);
...
static int tcc_viocmg_probe(struct platform_device *pdev)
{
	...
	viocmg = devm_kzalloc(&pdev->dev, sizeof(struct viocmg_soc),
		GFP_KERNEL);
	if (!viocmg) {
		pr_err("viocmg: alloc failed\n");
		result = -ENOMEM;
		goto END_PROCESS;
	}

	viocmg->viocmg_dt_info =
		(struct viocmg_dt_info *)devm_kzalloc(&pdev->dev,
			sizeof(struct viocmg_dt_info), GFP_KERNEL);
