...
//#define VIOMG_DEBUG
#ifdef VIOMG_DEBUG
#define dprintk(msg...) pr_info("REAR_CAM: " msg)
#else
#define dprintk(msg...)
#endif
...
static void viocmg_rear_cam_save_wmix(void)
{
	...
	memcpy(&viocmg->rear_cam_backup_components.wmix[0],
		(void *)(viocmg->reg.display_wmix +
		(0x100 * viocmg->viocmg_dt_info->main_display_id)), 0x20);
	memcpy(&viocmg->rear_cam_backup_components.wmix[1],
		(void *)(viocmg->reg.display_wmix +
		(0x100 * viocmg->viocmg_dt_info->rear_cam_vin_wmix)), 0x20);
}
...
static void viocmg_rear_cam_restore_wmix(void)
{
	...
	source_wmix =
	viocmg->rear_cam_backup_components.wmix[REARCAM_BK_WMIX_DISPLAY];

	dest_wmix = viocmg->reg.display_wmix +
		(0x100 * viocmg->viocmg_dt_info->main_display_id);
	...
	dest_positon[viocmg->viocmg_dt_info->rear_cam_display_rdma%4].nREG =
	source_position[viocmg->viocmg_dt_info->rear_cam_display_rdma%4].nREG;

	dprintk("saved wmix position[%d] = 0x%x\n",
		viocmg->viocmg_dt_info->rear_cam_display_rdma%4,
	...
	memcpy((void *)(viocmg->reg.display_wmix +
		(0x100 * viocmg->viocmg_dt_info->rear_cam_vin_wmix)),
		&viocmg->rear_cam_backup_components.wmix[REARCAM_BK_WMIX_VIN],
		sizeof(VIOC_WMIX));
...
static void viocmg_rear_cam_set_rearovp(void)
{
	...
	VIOC_WMIX_SetOverlayPriority(
		viocmg->reg.display_wmix +
			(0x100 * viocmg->viocmg_dt_info->main_display_id),
		viocmg->viocmg_dt_info->rear_cam_ovp);
	VIOC_WMIX_SetUpdate(viocmg->reg.display_wmix +
		(0x100 * viocmg->viocmg_dt_info->main_display_id));
#endif
}
...
void viocmg_save_wmix_default(void)
{
	...
	VIOC_WMIX_SetOverlayPriority(
		&viocmg->rear_cam_backup_components.wmix[0],
		...
		case VIOCMG_REAR_MODE_PREPARE:
			...
			if (viocmg->viocmg_dt_info->feature_rear_cam_use_viqe) {
				...
				wait_for_completion_interruptible_timeout(
					&viocmg->components.viqe.completion,
					(HZ/10));
...
unsigned int viocmg_get_rear_cam_mode(void)
{
	...
	if (viocmg->viocmg_dt_info->feature_rear_cam_enable) {
		if (viocmg->reardrv_suspend) {
			...
			dprintk("%s rear driver is suspend\n", __func__);

			VIOC_WMIX_GetOverlayPriority(viocmg->reg.display_wmix +
			(0x100 * viocmg->viocmg_dt_info->main_display_id),
			&wmix_ovp);
			...
			if (wmix_ovp == viocmg->viocmg_dt_info->rear_cam_ovp) {
				// && viocmg->reardrv_get_dmastatus())
				viocmg->viocmg_dt_info->rear_cam_mode =
					VIOCMG_REAR_MODE_RUN;
			} else {
				viocmg->viocmg_dt_info->rear_cam_mode =
