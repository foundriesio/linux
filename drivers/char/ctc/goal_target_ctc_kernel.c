...
#define CTC_PAGE_COUNT (1 << CTC_PAGE_COUNT_EXPONENT) /**< number of pages */
#define CTC_PAGE_SIZE (1 << CTC_PAGE_SIZE_EXPONENT) /**< size of a page */

#define CTC_MEM_SIZE (CTC_PAGE_COUNT * CTC_PAGE_SIZE * CTC_CHANNEL_NUM) /**< size of shm for one direction */
#define CTC_MEM_OFFSET_RX (CTC_MEM_SIZE)        /**< offset to the receive part on the shm */

/**< types of a mailbox message */
#define CTC_MBOX_MSG_REQ 0                      /**< request message */
#define CTC_MBOX_MSG_RES 1                      /**< response message */
...
struct ctcDeviceData {
    ...
    DECLARE_KFIFO(fifoRead, struct ctcDataEntry *, CTC_FIFO_SIZE); /**< read fifo */
...
static void ctc_mBoxRxRes(
    int minor,                                  /**< minor number */
    __u16 size                                  /**< buffer length */
);

static void ctcCalcPages(
    size_t len,                                 /**< data length */
    int ctcIdx,                                 /**< index of mailbox fifo */
    int *pReqPages,                             /**< required pages */
    int *pIdxOffset                             /**< offset to write index */
);
...
static __u32 *pCtcMboxDataTx;                   /**< transmit data for mailbox */
static __u32 *pCtcMboxDataRx;                   /**< receive data for mailbox */
...
)
{
    ...
    if (!pRes) {
        /* Error */
        printk(KERN_ALERT "CTC: %s - unable to allocate resource for shared memory initialization.\n", __func__);
        return -EFAULT;
    }
    ...
    ctc_sramSize = resource_size(pRes);
...
)
{
    ...
    for (idx = 0; idx < CTC_CHANNEL_NUM; idx++) {
        ...
        INIT_KFIFO(ctcDevices[idx].fifoRead);

        ctcDevices[idx].pWrData = (char *) (pVirtKernMem + (CTC_PAGE_COUNT * CTC_PAGE_SIZE * idx));

        ctcDevices[idx].idxWrite = 0;
        ctcDevices[idx].idxRead = 0;
        ...
        /* init the mutex */
        mutex_init(&(ctcDevices[idx].mtxCtcDev));
    }
...
)
{
    ...
    pCtcMboxDataTx = (__u32 *) kmalloc(CTC_MBOX_LEN * sizeof(__u32), GFP_ATOMIC);
    if (!pCtcMboxDataTx) {
        /* Error */
        printk(KERN_ALERT "CTC: %s - unable to allocate memory for transmitting mailbox buffer.\n", __func__);
        return -EFAULT;
    }
    pCtcMboxDataRx = (__u32 *) kmalloc(CTC_MBOX_LEN * sizeof(__u32), GFP_ATOMIC);
    if (!pCtcMboxDataRx) {
        /* Error */
        printk(KERN_ALERT "CTC: %s - unable to allocate memory for receiving mailbox buffer.\n", __func__);
        return -EFAULT;
    }
...
)
{
    ...
    if ((CTC_CHANNEL_NUM > minor) && (CTC_PAGE_COUNT > page) && ((CTC_PAGE_COUNT * CTC_PAGE_SIZE) > len)) {
        ...
        printk(KERN_NOTICE "CTC: %s - unable to evaluate a mailbox message. [minor: %u, page: %lu, len: %u]\n", __func__, minor, page, len);
    }
    return 0;
}
...
)
{
    ...
    int idxOffset;                              /* offset to write index */

    /* calculate the required pages and the offset */
    ctcCalcPages((size_t) size, ctcDevices[minor].idxRead, &reqPages, &idxOffset);

    /* increase the read index and consider the overflow */
    ctcDevices[minor].idxRead += (reqPages + idxOffset);
    ctcDevices[minor].idxRead &= (CTC_PAGE_COUNT - 1);
}
...
)
{
    ...
    pFifo = (struct kfifo *) &(ctcDevices[minor].fifoRead);
...
)
{
    ...
    reqPages = len / CTC_PAGE_SIZE;
    if (len % CTC_PAGE_SIZE) {
        reqPages++;
    }
    ...
    if (CTC_PAGE_COUNT < (ctcIdx + reqPages)) {
        /* calculate offset */
        *pIdxOffset = (int) (CTC_PAGE_COUNT - ctcIdx);
    } else {
        *pIdxOffset = 0;
    }
...
)
{
    ...
    kfree(pCtcMboxDataRx);
    kfree(pCtcMboxDataTx);
    ...
    if (0 == res) {
        ...
        if (1 == fifoDataLen) {
            ...
            if (len >= pEntry->size) {
                ...
                res = copy_to_user(pBuf, pEntry->pBuf, pEntry->size);
                if (!res) {
                    ...
                    /* Free the buffer. */
                    kfree(pEntry);
                    printk(KERN_NOTICE "CTC: %s - Unable to copy data to user space.\n", __func__);
                    return (0 > res) ? (res) : (-EMSGSIZE);
                }
            }
            /* set the mailbox message */
            ctcSetMboxMsg(pCtcMboxDataRx, minor, 0, pEntry->size, CTC_MBOX_MSG_RES);

            /* get the current time stamp */
            getnstimeofday(&mboxTimeout);
            do {
                /* send the mailbox message */
                resMbox = pl320_ipc_transmit(pCtcMboxDataRx);
...
)
{
    ...
    unsigned int idxOffset;                     /* offset to first element of cyclic buffer */
    ...
    mutex_lock(&(ctcDevices[minor].mtxCtcDev));

    if (ctcDevices[minor].flgCyclic) {
        if (ctcDevices[minor].idxRead != ctcDevices[minor].idxWrite) {
            /* ERROR */
            mutex_unlock(&(ctcDevices[minor].mtxCtcDev));
    ...
    /* The index is 0 <= idxWrite < CTC_PAGE_COUNT */
    ctcPageWrIdx = ctcDevices[minor].idxWrite;

    /* calculate the required pages and offset */
    ctcCalcPages(len, ctcPageWrIdx, &reqPages, &idxOffset);

    /* calculate the remaining pages on the buffer */
    remPages = (CTC_PAGE_COUNT - 1) - (ctcPageWrIdx) + ctcDevices[minor].idxRead;
    ...
    /* prepare the mailbox message */
    ctcSetMboxMsg(pCtcMboxDataTx, minor, ctcPageWrIdx, (uint16_t) len, CTC_MBOX_MSG_REQ);

    /* get the current time stamp */
    getnstimeofday(&mboxTimeout);
    do {
        /* send the mailbox message */
        resMbox = pl320_ipc_transmit(pCtcMboxDataTx);
        getnstimeofday(&mboxTime);
        if (0 == resMbox) {
            /* update the write index according to the required pages and consider the overflow */
            ctcDevices[minor].idxWrite = (ctcPageWrIdx + reqPages) & (CTC_PAGE_COUNT - 1);
            mutex_unlock(&(ctcDevices[minor].mtxCtcDev));
            return len;
        }
    } while ((CTC_GET_TIME_MS(mboxTimeout) + ctcMboxTimeoutMs) > CTC_GET_TIME_MS(mboxTime));
    ...
    switch (cmd) {
        ...
        case CTC_IOCTL_SETUP_CYCLIC:
            ...
            if ((0 != ctcDevices[minor].pid)
                    && (ctcDevices[minor].idxWrite == ctcDevices[minor].idxRead)
                    && (kfifo_is_empty(pFifo))) {
                /* clear the write and read index */
                ctcDevices[minor].idxWrite = 0;
                ctcDevices[minor].idxRead = 0;
                /* set the cyclic flag */
