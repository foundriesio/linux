	...
	{	
		...
		while( true ) 
		{
			...
			*/
			...
			if( !( pstDptx->stAuxParams.uiAuxStatus & DPTX_AUX_STS_REPLY_RECEIVED ) )		/* 1: Indicates an active AUX transfer,  0: AUX_REPLY has been received and status/data is valid */
			{
				break;
			}
...
bool Dptx_Aux_Read_Bytes_From_DPCD( struct Dptx_Params *pstDptx, u32 uiAddr, u8 *pucBuffer, u32 uiLength )
{
	bool		bRetVal;
	bool		bRead_Mode = true, bI2C_Mode = false, bEnableMot_Mode = true, bAdressOnly = false;
