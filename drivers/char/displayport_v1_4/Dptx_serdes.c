...
static int of_parse_serdes_dt(struct Max968xx_dev *pstDev)
{
	...
	pstSerDes_DN = of_find_compatible_node(
						NULL,
						NULL,
...
static int Dptx_Max968XX_I2C_Write(
			...
			unsigned char ucValue)
{
	...
	aucWBuf[0] = (u8)(usRegAdd >> 8);
	aucWBuf[1] = (u8)(usRegAdd & 0xFF);
	aucWBuf[2] = ucValue;

	iRW_Len = i2c_master_send(
			(const struct i2c_client *)client,
			(const char *)aucWBuf,
			(int)(SER_DES_I2C_REG_ADD_LEN + SER_DES_I2C_DATA_LEN));
	if (iRW_Len != (SER_DES_I2C_REG_ADD_LEN + SER_DES_I2C_DATA_LEN)) {
		dptx_err("i2c device %s: error to write address 0x%x.. len %d",
					client->name,
					client->addr,
					iRW_Len);
	...
	dptx_dbg("Write I2C Dev 0x%x: Reg add 0x%x -> 0x%x",
				(client->addr << 1),
				usRegAdd,
				ucValue);
...
static unsigned char Dptx_Max968XX_Convert_DevAdd_To_Index(
						unsigned char ucI2C_DevAdd)
{
	u8 ucElements;
	struct Max968xx_dev *pstMax968xx_dev;

	for (ucElements = 0;
			ucElements < SER_DES_INPUT_INDEX_MAX;
			ucElements++) {
		pstMax968xx_dev = &stMax968xx_dev[ucElements];
		if ((pstMax968xx_dev->pstClient != NULL) &&
			(pstMax968xx_dev->pstClient->addr << 1) == ucI2C_DevAdd)
			break;
	}
...
int32_t Dptx_Max968XX_Get_TopologyState(u8 *pucNumOfPluggedPorts)
{
	...
	for (ucElements = (u8)DES_INPUT_INDEX_0;
			ucElements < SER_DES_INPUT_INDEX_MAX;
			ucElements++) {
		pstMax968xx_dev = &stMax968xx_dev[ucElements];
		if (pstMax968xx_dev->pstClient == NULL)
...
int32_t Dptx_Max968XX_Reset(struct Dptx_Params *pstDptx)
{
	...
	unsigned char ucSER_Revision_Num = 0, ucDES_Revision_Num = 0;
	unsigned char ucRW_Data, ucEVB_Tpye;
	...
	for (uiElements = 0;
			!(pstSERDES_Reg_Info[uiElements].uiDev_Addr == 0 &&
			pstSERDES_Reg_Info[uiElements].uiReg_Addr == 0 &&
			pstSERDES_Reg_Info[uiElements].uiReg_Val == 0);
			uiElements++) {
		if (pstSERDES_Reg_Info[uiElements].uiDev_Addr ==
				DP_SER_DES_DELAY_DEV_ADDR) {
			mdelay(pstSERDES_Reg_Info[uiElements].uiReg_Val);
			continue;
		}

		ucSerDes_Index =
			Dptx_Max968XX_Convert_DevAdd_To_Index(
				(u8)pstSERDES_Reg_Info[uiElements].uiDev_Addr);
		...
		if ((pstSERDES_Reg_Info[uiElements].ucDeviceType !=
			TCC_ALL_EVB_TYPE) &&
			(ucEVB_Tpye !=
		...
		if ((pstSERDES_Reg_Info[uiElements].ucSER_Revision !=
			SER_REV_ALL) &&
			(ucSER_Revision_Num !=
		...
		if ((pstDptx->bMultStreamTransport == false) &&
			((pstSERDES_Reg_Info[uiElements].uiReg_Addr ==
			DES_DROP_VIDEO) ||
			(pstSERDES_Reg_Info[uiElements].uiReg_Addr ==
			DES_STREAM_SELECT)))
			continue;
		...
		ucRW_Data =
			(unsigned char)pstSERDES_Reg_Info[uiElements].uiReg_Val;
		...
		if (pstSERDES_Reg_Info[uiElements].uiReg_Addr ==
			DES_STREAM_SELECT) {
			ucRW_Data =
				(pstDptx->aucVCP_Id[(ucSerDes_Index - 1)] - 1);
		...
		iRetVal = Dptx_Max968XX_I2C_Write(
				pstMax968xx_dev->pstClient,
				pstSERDES_Reg_Info[uiElements].uiReg_Addr,
				ucRW_Data);
	...
	dptx_info("\n[%s:%d]SerDes %d Resisters update is successfully done!!!\n",
				__func__,
				__LINE__,
				uiElements);
...
static int Dptx_Max968XX_probe(
			struct i2c_client *client,
			const struct i2c_device_id *id)
{
	...
	if (ucElements == SER_DES_INPUT_INDEX_MAX) {
		dptx_err("It's limited to 4 SerDes ports connected.. Dev add 0x%x",
					client->addr << 1);
		return -ENOMEM;
	}
	...
	dptx_info("[%d]%s: %s I2C name '%s', address 0x%x : Rev %d..SerDes Lane %s",
	ucElements,
	(pstMax968xx_dev->ucEVB_Type == TCC8059_EVB_01) ?
	"TCC8059 EVB" :
	(pstMax968xx_dev->ucEVB_Type == TCC8050_SV_01) ?
	"TCC8050/3 sv0.1" :
	(pstMax968xx_dev->ucEVB_Type == TCC8050_SV_10) ?
	"TCC8050/3 sv1.0" : "Unknown",
...
{
	memset(&stMax968xx_dev[SER_INPUT_INDEX_0], 0,
		(sizeof(struct Max968xx_dev) * SER_DES_INPUT_INDEX_MAX));
