...
int32_t Dptx_Platform_Init_Params(
			...
			struct device *pstParentDev)
{
	...
	for (ucStream_Index = 0;
			ucStream_Index < PHY_INPUT_STREAM_MAX;
			ucStream_Index++) {
		pstDptx->aucVCP_Id[ucStream_Index] = (ucStream_Index + 1);

		pstDptx->paucEdidBuf_Entry[ucStream_Index] =
					kzalloc(DPTX_EDID_BUFLEN, GFP_KERNEL);
		...
		memset(pstDptx->paucEdidBuf_Entry[ucStream_Index],
				0,
				DPTX_EDID_BUFLEN);
	}
...
//		dptx_err("Can't allocate Edid Buf");
...
int32_t Dptx_Platform_Init(struct Dptx_Params *pstDptx)
{
	...
	iRetVal = Dptx_Platform_Get_PLLLock_Status(pstDptx,
							&bPLL_LockStatus);
	if (iRetVal !=  DPTX_RETURN_NO_ERROR) {
	...
	iRetVal = Dptx_Platform_Set_PLL_ClockSource(pstDptx,
					(u8)DPTX_CLKCTRL0_PLL_DIVIDER_OUTPUT);
	if (iRetVal !=  DPTX_RETURN_NO_ERROR) {
		dptx_err("from Dptx_Platform_Set_PLL_ClockSource()");
		return iRetVal;
	}

	iRetVal = Dptx_Platform_Set_RegisterBank(pstDptx,
				(enum PHY_LINK_RATE)pstDptx->ucMax_Rate);
	if (iRetVal !=  DPTX_RETURN_NO_ERROR) {
...
int32_t Dptx_Platform_Set_ProtectRegister_PW(
			struct Dptx_Params *pstDptx,
			u32 uiProtect_Cfg_PW)
{
	Dptx_Reg_Writel(pstDptx,
			(pstDptx->uiProtect_RegAddr_Offset +
			DP_PORTECT_CFG_PW_OK),
			(u32)uiProtect_Cfg_PW);

	return DPTX_RETURN_NO_ERROR;
}

int32_t Dptx_Platform_Set_ProtectRegister_CfgAccess(
			struct Dptx_Params *pstDptx,
			bool bAccessable)
{
	if (bAccessable)
		Dptx_Reg_Writel(pstDptx,
			(u32)(pstDptx->uiProtect_RegAddr_Offset +
			DP_PORTECT_CFG_ACCESS),
			(u32)DP_PORTECT_CFG_ACCESSABLE);
	else
		Dptx_Reg_Writel(pstDptx,
			(u32)(pstDptx->uiProtect_RegAddr_Offset +
			DP_PORTECT_CFG_ACCESS),
			(u32)DP_PORTECT_CFG_NOT_ACCESSABLE);

	return DPTX_RETURN_NO_ERROR;
}

int32_t Dptx_Platform_Set_ProtectRegister_CfgLock(
			struct Dptx_Params *pstDptx,
			bool bAccessable)
{
	if (bAccessable)
		Dptx_Reg_Writel(pstDptx,
			(u32)(pstDptx->uiProtect_RegAddr_Offset +
			DP_PORTECT_CFG_LOCK),
			(u32)DP_PORTECT_CFG_UNLOCKED);
	else
		Dptx_Reg_Writel(pstDptx,
			(u32)(pstDptx->uiProtect_RegAddr_Offset +
			DP_PORTECT_CFG_LOCK),
			(u32)DP_PORTECT_CFG_LOCKED);
...
int32_t Dptx_Platform_Set_PLL_Divisor(struct Dptx_Params *pstDptx)
{
	...
	Dptx_Reg_Writel(pstDptx,
				(pstDptx->uiCKC_RegAddr_Offset +
				DPTX_CKC_CFG_CLKDIVC0),
				DIV_CFG_CLK_200HMZ);
	Dptx_Reg_Writel(pstDptx,
				(pstDptx->uiCKC_RegAddr_Offset +
				DPTX_CKC_CFG_CLKDIVC1),
				DIV_CFG_CLK_160HMZ);
	Dptx_Reg_Writel(pstDptx,
				(pstDptx->uiCKC_RegAddr_Offset +
				DPTX_CKC_CFG_CLKDIVC2),
				DIV_CFG_CLK_100HMZ);
	Dptx_Reg_Writel(pstDptx,
				(pstDptx->uiCKC_RegAddr_Offset +
				DPTX_CKC_CFG_CLKDIVC3),
				DIV_CFG_CLK_40HMZ);
			...
			u8 ucClockSource)
{
	...
	Dptx_Reg_Writel(pstDptx,
				(pstDptx->uiCKC_RegAddr_Offset +
				DPTX_CKC_CFG_CLKCTRL0),
				uiRegMap_Val);
	Dptx_Reg_Writel(pstDptx,
				(pstDptx->uiCKC_RegAddr_Offset +
				DPTX_CKC_CFG_CLKCTRL1),
				uiRegMap_Val);
	Dptx_Reg_Writel(pstDptx,
				(pstDptx->uiCKC_RegAddr_Offset +
				DPTX_CKC_CFG_CLKCTRL2),
				uiRegMap_Val);
	Dptx_Reg_Writel(pstDptx,
				(pstDptx->uiCKC_RegAddr_Offset +
				DPTX_CKC_CFG_CLKCTRL3),
				uiRegMap_Val);
			...
			bool *pbPll_Locked)
{
	...
	do {
		...
		bPllLock =
			(uiRegMap_PllPMS & DPTX_PLLPMS_LOCK_MASK) ?
			true : false;
			...
			enum PHY_LINK_RATE eLinkRate)
{
	...
	dptx_dbg("Writing REG_22[0x12480058]: 0x%x -> 0x%x",
				uiReg_R_data,
				uiReg_W_data);
	...
	case LINK_RATE_RBR:
		Dptx_Reg_Writel(pstDptx,
			(u32)(pstDptx->uiRegBank_RegAddr_Offset +
			DP_REGISTER_BANK_REG_0),
			(u32)0x004D0000);
		Dptx_Reg_Writel(pstDptx,
			(u32)(pstDptx->uiRegBank_RegAddr_Offset +
			DP_REGISTER_BANK_REG_20),
			(u32)0x00002501);
		Dptx_Reg_Writel(pstDptx,
			(u32)(pstDptx->uiRegBank_RegAddr_Offset +
			DP_REGISTER_BANK_REG_11),
			(u32)0x00000401);
		Dptx_Reg_Writel(pstDptx,
			(u32)(pstDptx->uiRegBank_RegAddr_Offset +
			DP_REGISTER_BANK_REG_12),
			(u32)0x00000000);
		Dptx_Reg_Writel(pstDptx,
			(u32)(pstDptx->uiRegBank_RegAddr_Offset +
			DP_REGISTER_BANK_REG_13),
			(u32)0x00000000);
		Dptx_Reg_Writel(pstDptx,
			(u32)(pstDptx->uiRegBank_RegAddr_Offset +
			DP_REGISTER_BANK_REG_14),
			(u32)0x00000000);
		Dptx_Reg_Writel(pstDptx,
			(u32)(pstDptx->uiRegBank_RegAddr_Offset +
			DP_REGISTER_BANK_REG_16),
			(u32)0x00000000);
		Dptx_Reg_Writel(pstDptx,
			(u32)(pstDptx->uiRegBank_RegAddr_Offset +
			DP_REGISTER_BANK_REG_15),
			(u32)0x08080808);
		Dptx_Reg_Writel(pstDptx,
			(u32)(pstDptx->uiRegBank_RegAddr_Offset +
			DP_REGISTER_BANK_REG_17),
			(u32)0x10000000);
		Dptx_Reg_Writel(pstDptx,
			(u32)(pstDptx->uiRegBank_RegAddr_Offset +
			DP_REGISTER_BANK_REG_20),
		...
		Dptx_Reg_Writel(pstDptx,
			(u32)(pstDptx->uiRegBank_RegAddr_Offset +
			DP_REGISTER_BANK_REG_21),
			(u32)0x00000000);
		Dptx_Reg_Writel(pstDptx,
			(u32)(pstDptx->uiRegBank_RegAddr_Offset +
			DP_REGISTER_BANK_REG_23),
			(u32)0x00000008);
		Dptx_Reg_Writel(pstDptx,
			(u32)(pstDptx->uiRegBank_RegAddr_Offset +
			DP_REGISTER_BANK_REG_22),
	...
	case LINK_RATE_HBR2:
		...
		Dptx_Reg_Writel(pstDptx,
			(u32)(pstDptx->uiRegBank_RegAddr_Offset +
			DP_REGISTER_BANK_REG_1),
			(u32)0xA00F0001);
		Dptx_Reg_Writel(pstDptx,
			(u32)(pstDptx->uiRegBank_RegAddr_Offset +
			DP_REGISTER_BANK_REG_2),
			(u32)0x00A80122);
		Dptx_Reg_Writel(pstDptx,
			(u32)(pstDptx->uiRegBank_RegAddr_Offset +
			DP_REGISTER_BANK_REG_3),
			(u32)0x001E8A00);
		Dptx_Reg_Writel(pstDptx,
			(u32)(pstDptx->uiRegBank_RegAddr_Offset +
			DP_REGISTER_BANK_REG_4),
			(u32)0x00000004);
		Dptx_Reg_Writel(pstDptx,
			(u32)(pstDptx->uiRegBank_RegAddr_Offset +
			DP_REGISTER_BANK_REG_7),
			(u32)0x00000C0C);
		Dptx_Reg_Writel(pstDptx,
			(u32)(pstDptx->uiRegBank_RegAddr_Offset +
			DP_REGISTER_BANK_REG_8),
			(u32)0x05460546);
		Dptx_Reg_Writel(pstDptx,
			(u32)(pstDptx->uiRegBank_RegAddr_Offset +
			DP_REGISTER_BANK_REG_9),
			(u32)0x00000011);
		Dptx_Reg_Writel(pstDptx,
			(u32)(pstDptx->uiRegBank_RegAddr_Offset +
			DP_REGISTER_BANK_REG_10),
			(u32)0x00700000);
		Dptx_Reg_Writel(pstDptx,
			(u32)(pstDptx->uiRegBank_RegAddr_Offset +
			DP_REGISTER_BANK_REG_11),
...
int32_t Dptx_Platform_Set_APAccess_Mode(struct Dptx_Params *pstDptx)
{
	...
	uiRegMap_R_ApbSel = Dptx_Reg_Direct_Read(
			pstDptx->pioMIC_SubSystem_BaseAddr + DPTX_APB_SEL_REG);
	...
	dptx_dbg("Register access...Reg[0x%x]:0x%08x -> 0x%08x",
			(pstDptx->pioMIC_SubSystem_BaseAddr + DPTX_APB_SEL_REG),
			uiRegMap_R_ApbSel,
			uiRegMap_W_ApbSel);
...
int32_t Dptx_Platform_Set_PMU_ColdReset_Release(struct Dptx_Params *pstDptx)
{
	...
	uiRegMap_R_HsmRst_Msk = Dptx_Reg_Direct_Read(
				pstDptx->pioPMU_BaseAddr + PMU_HSM_RSTN_MASK);

	if (uiRegMap_R_HsmRst_Msk & PMU_COLD_RESET_MASK) {
		uiRegMap_W_HsmRst_Msk =
			(uiRegMap_R_HsmRst_Msk & ~PMU_COLD_RESET_MASK);
		Dptx_Reg_Direct_Write(
			(pstDptx->pioPMU_BaseAddr + PMU_HSM_RSTN_MASK),
			uiRegMap_W_HsmRst_Msk);

		dptx_dbg("DP Cold reset mask release...0x%08x -> 0x%08x",
					uiRegMap_R_HsmRst_Msk,
					uiRegMap_W_HsmRst_Msk);
	}
...
int32_t Dptx_Platform_Set_PHY_StandardLane_PinConfig(
			struct Dptx_Params *pstDptx)
{
	u32 uiRegMap_R_Reg24, uiRegMap_W_Reg24;

	uiRegMap_R_Reg24 = Dptx_Reg_Readl(pstDptx,
		(u32)(pstDptx->uiRegBank_RegAddr_Offset +
				DP_REGISTER_BANK_REG_24));

	uiRegMap_W_Reg24 = (pstDptx->bPHY_Lane_Reswap) ?
					(uiRegMap_R_Reg24 | STD_EN_MASK) :
					(uiRegMap_R_Reg24 & ~STD_EN_MASK);
...
int32_t Dptx_Platform_Get_PHY_StandardLane_PinConfig(
			struct Dptx_Params *pstDptx)
{
	...
	pstDptx->bPHY_Lane_Reswap =
		(uiRegMap_R_Reg24 & STD_EN_MASK) ? true : false;
...
int32_t Dptx_Platform_Set_SDMBypass_Ctrl(struct Dptx_Params *pstDptx)
{
	...
	uiRegMap_W_Reg24 = (pstDptx->bSDM_Bypass) ?
				(uiRegMap_R_Reg24 | SDM_BYPASS_MASK) :
				(uiRegMap_R_Reg24 & ~SDM_BYPASS_MASK);
...
int32_t Dptx_Platform_Get_SDMBypass_Ctrl(struct Dptx_Params *pstDptx)
{
	...
	pstDptx->bSDM_Bypass =
		(uiRegMap_R_Reg24 & SDM_BYPASS_MASK) ? true : false;
...
int32_t Dptx_Platform_Set_SRVCBypass_Ctrl(struct Dptx_Params *pstDptx)
{
	...
	uiRegMap_W_Reg24 = (pstDptx->bSRVC_Bypass) ?
				(uiRegMap_R_Reg24 | SRVC_BYPASS_MASK) :
				(uiRegMap_R_Reg24 & ~SRVC_BYPASS_MASK);
...
int32_t Dptx_Platform_Get_SRVCBypass_Ctrl(struct Dptx_Params *pstDptx)
{
	...
	pstDptx->bSRVC_Bypass =
		(uiRegMap_R_Reg24 & SRVC_BYPASS_MASK) ? true : false;
...
int32_t Dptx_Platform_Set_MuxSelect(struct Dptx_Params *pstDptx)
{
	...
	if (pstDptx->ucNumOfPorts >= PHY_INPUT_STREAM_MAX) {
		dptx_err("The number of ports( %d ) is larger than maximum(%d)",
...
int32_t Dptx_Platform_Get_MuxSelect(struct Dptx_Params *pstDptx)
{
	...
	for (ucDP_Index = 0; ucDP_Index < PHY_INPUT_STREAM_MAX; ucDP_Index++) {
		...
		pstDptx->aucMuxInput_Index[ucDP_Index] =
		((uiRegMap_R_Reg24 & uiRegMap_MuxSel_Mask) >>
		ucRegMap_MuxSel_Shift);
...
int32_t Dptx_Platform_Set_ClkPath_To_XIN(struct Dptx_Params *pstDptx)
{
	...
	Dptx_Platform_Set_ProtectRegister_PW(
				pstDptx,
				(u32)DP_PORTECT_CFG_PW_VAL);
	Dptx_Platform_Set_ProtectRegister_CfgLock(pstDptx, true);
	Dptx_Platform_Set_ProtectRegister_CfgAccess(pstDptx, true);

	Dptx_Reg_Writel(pstDptx,
		(u32)(pstDptx->uiCKC_RegAddr_Offset + DP_REGISTER_BANK_REG_0),
		(u32)0x00);
	Dptx_Reg_Writel(pstDptx,
		(u32)(pstDptx->uiCKC_RegAddr_Offset + DP_REGISTER_BANK_REG_1),
		(u32)0x00);
	Dptx_Reg_Writel(pstDptx,
		(u32)(pstDptx->uiCKC_RegAddr_Offset + DP_REGISTER_BANK_REG_2),
		(u32)0x00);
	Dptx_Reg_Writel(pstDptx,
		(u32)(pstDptx->uiCKC_RegAddr_Offset + DP_REGISTER_BANK_REG_3),
		(u32)0x00);

	uiRegMap_PLLPMS = Dptx_Reg_Readl(pstDptx,
		(u32)(pstDptx->uiCKC_RegAddr_Offset + DP_REGISTER_BANK_REG_4));
	...
	dptx_dbg("Clk path to XIN...Reg[0x%x] -> 0x%08x",
		(u32)(pstDptx->uiCKC_RegAddr_Offset + DP_REGISTER_BANK_REG_4),
		uiRegMap_PLLPMS);
...
int32_t Dptx_Platform_Free_Handle(struct Dptx_Params *pstDptx_Handle)
{
	...
	for (ucStreamIdx = 0;
			ucStreamIdx < PHY_INPUT_STREAM_MAX;
			ucStreamIdx++) {
		pstDptx_Handle->paucEdidBuf_Entry[ucStreamIdx] =
			kzalloc(DPTX_EDID_BUFLEN, GFP_KERNEL);
