...
int32_t Dptx_Edid_Verify_BaseBlk_Data( u8 *pucEDID_Buf )
{
	...
	iCompCmp = memcmp( pucEDID_Buf, aucEdid_Header, sizeof(aucEdid_Header));
	if (iCompCmp) {
		dptx_err( "Invalid EDID header( 0x%x, 0x%x, 0x%x, 0x%x, 0x%x ) ", pucEDID_Buf[0], pucEDID_Buf[1], pucEDID_Buf[2], pucEDID_Buf[3], pucEDID_Buf[4] );
