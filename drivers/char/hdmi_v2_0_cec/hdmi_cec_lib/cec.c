...
static int cec_CfgTxBuf(struct cec_device * dev, const char *buf, unsigned size)
{
	...
	if (size > CEC_TX_DATA_SIZE) {
		pr_info("[INFO][HDMI_CEC] %s: invalid parameter\n",__func__);
		return -1;
	}
...
static int cec_CfgRxBuf(struct cec_device * dev, char *buf, unsigned size)
{
	...
	if (cnt > CEC_RX_DATA_SIZE) {
		pr_info("[INFO][HDMI_CEC] %s: wrong byte count\n",__func__);
		return -1;
	}
        		...
        		if (cec_CfgTxBuf(dev, buf, size) == (int)size) {
        			for (i = 0; i < (int)retry; i++) {
        				...
        				while (cec_GetSend(dev) != 0);
                                        ...
                                        #if 0 // for debuging
        				pr_info("[INFO][HDMI_CEC] CEC interrupt status = 0x%08x \r\n",cec_dev_read(dev,IO_IH_CEC_STAT0));
                                        #endif
...
int cec_check_wake_up_interrupt(struct cec_device * dev)
{
	...
	pr_info("[INFO][HDMI_CEC] [%s] IH_CEC_STAT0(Offset=0x106) = 0x%08x, %s \n",__func__,cec_dev_read(dev,IO_IH_CEC_STAT0),
...
int cec_Disable(struct cec_device * dev, int wakeup)
{
	...
	pr_info("[INFO][HDMI_CEC] %s: Wakeup status : %d \n",__func__,wakeup);
	return 0;
}
...
int cec_CfgLogicAddr(struct cec_device * dev, unsigned addr, int enable)
{
	...
	if (addr > BCST_ADDR) {
		pr_info("[INFO][HDMI_CEC] %s: invalid parameter",__func__);
		return -1;
	}
	if (addr == BCST_ADDR) {
		if (enable) {
			...
			pr_info("[INFO][HDMI_CEC] %s: cannot de-allocate broadcast logical address",__func__);
			return -1;
		}
	}
...
{
	pr_info("[INFO][HDMI_CEC] [%s] cec_ctrl(link offset = 0x7d00, IO offset = 0x%04x) : 0x%08x\n",__func__,CEC_CTRL,cec_dev_read(dev,CEC_CTRL));
	pr_info("[INFO][HDMI_CEC] [%s] cec_mask(link offset = 0x7d02, IO offset = 0x%04x) : 0x%08x\n",__func__,CEC_MASK,cec_dev_read(dev,CEC_MASK));
	pr_info("[INFO][HDMI_CEC] [%s] cec_addr_l(link offset = 0x7d05, IO offset = 0x%04x) : 0x%08x\n",__func__,CEC_ADDR_L,cec_dev_read(dev,CEC_ADDR_L));
	pr_info("[INFO][HDMI_CEC] [%s] cec_addr_h(link offset = 0x7d06, IO offset = 0x%04x) : 0x%08x\n",__func__,CEC_ADDR_H,cec_dev_read(dev,CEC_ADDR_H));
	pr_info("[INFO][HDMI_CEC] [%s] cec_lock(link offset = 0x7d30, IO offset = 0x%04x) : 0x%08x\n",__func__,CEC_LOCK,cec_dev_read(dev,CEC_LOCK));
	pr_info("[INFO][HDMI_CEC] [%s] cec_wakeupctrl(link offset = 0x7d31, IO offset = 0x%04x) : 0x%08x\n",__func__,CEC_WAKEUPCTRL,cec_dev_read(dev,CEC_WAKEUPCTRL));
	pr_info("[INFO][HDMI_CEC] [%s] ih_cec_stat0(link offset = 0x106, IO offset = 0x%04x) : 0x%08x\n",__func__,IO_IH_CEC_STAT0,cec_dev_read(dev,IO_IH_CEC_STAT0));
	pr_info("[INFO][HDMI_CEC] [%s] ih_mute_cec_stat0(link offset = 0x186, IO offset = 0x%04x) : 0x%08x\n",__func__,IO_IH_MUTE_CEC_STAT0,cec_dev_read(dev,IO_IH_MUTE_CEC_STAT0));
}