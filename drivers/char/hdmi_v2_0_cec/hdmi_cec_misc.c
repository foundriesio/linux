...
#define dpr_info(fmt,...) pr_info("[INFO][HDMI_CEC] "fmt, ##__VA_ARGS__))
#else
#define dpr_info
#endif
...
static int hdmi_cec_blank(struct cec_device *dev, int blank_mode)
{
	...
	pr_info("[INFO][HDMI_CEC] %s : blank(mode=%d)\n", __func__, blank_mode);
		...
		case CEC_IOC_STOP:
        		{
        			int wakeup;
                                ...
                                if(!wakeup && dev->cec_wakeup_enable > 1) {
                                        pr_info("[INFO][HDMI_CEC] (%s CEC_IOC_STOP : clean wakeup\r\n", __func__);
		...
		case CEC_IOC_SETLADDR:
        		{
        			...
        			pr_info("[INFO][HDMI_CEC] %s: ioctl(CEC_IOC_SETLADDR)\n",__func__);
        			pr_info("[INFO][HDMI_CEC] %s: logical address = 0x%02x\n", __func__, laddr);
        		        #endif
        			dev->l_address = laddr;
		...
		case CEC_IOC_CLEARLADDR:
        		{
        			unsigned int laddr;
        		        ...
        		        #ifdef CEC_KERNEL_DEBUG
        			pr_info("[INFO][HDMI_CEC] %s: ioctl(CEC_IOC_CLEARLADDR)\n",__func__);
        			pr_info("[INFO][HDMI_CEC] %s: logical address = 0x%02x\n", __func__, laddr);
        		        #endif
		...
		case CEC_IOC_SENDDATA:
        		{
        			...
        			memset(dev->buf.send_buf,0,CEC_TX_DATA_SIZE);
        		        ...
        		        #ifdef CEC_KERNEL_DEBUG
        			pr_info("[INFO][HDMI_CEC] \nCEC Tx(%d): ",dev->buf.size);
        			for(i = 0; i < dev->buf.size; i++)
        			        pr_info("[INFO][HDMI_CEC] %02x ", dev->buf.send_buf[i]);
        			pr_info("[INFO][HDMI_CEC] \n");
        		        #endif
		...
		case CEC_IOC_RECVDATA:
        		{
        			...
        			if(ret > 0)
        			{
        				...
        				pr_info("[INFO][HDMI_CEC] \nCEC Rx(%d): ", (int)ret);
        				for(i = 0; i < ret; i++)
        				        pr_info("[INFO][HDMI_CEC] %02x ", dev->buf.recv_buf[i]);
        				pr_info("[INFO][HDMI_CEC] \n");
        		                #endif
                ...
                case CEC_IOC_SENDDATA_EX:
                        {
                                ...
                                pr_info("[INFO][HDMI_CEC] (%s CEC_IOC_SENDDATA_EX write size=%d\r\n", __func__, cec_tx_data.size );
                                ...
                                for(i = 0; i < cec_tx_data.size; i++)
                                        pr_info("[INFO][HDMI_CEC] %02x ", cec_tx_data.buff[i]);
                                pr_info("[INFO][HDMI_CEC] \n");
                                #endif
                ...
                case CEC_IOC_RECVDATA_EX:
        		{
        			...
        			if(cec_rx_data.size >= 0) {
                				...
                				for(i = 0; i < cec_rx_data.size; i++)
                				        pr_info("[INFO][HDMI_CEC] %02x ", cec_rx_data.buff[i]);
                				pr_info("[INFO][HDMI_CEC] \n");
                                        }
        		                #endif
        ...
        if(dev !=NULL) {
                if(dev->clk_enable_count == 1) {
                        if(dev->cec_wakeup_enable) {
                                ...
                                pr_info("[INFO][HDMI_CEC] (%s is skipped because cec_wakeup_enable(%d) is setted\r\n", __func__, dev->cec_wakeup_enable);
                        } else {
                                pr_info("[INFO][HDMI_CEC] (%s cound = %d\r\n", __func__, dev->clk_enable_count);
