...
int32_t tcc_thsm_cmd_init(uint32_t device_id)
{
	...
	rdata_size = sec_sendrecv_cmd(
		device_id, TCCTHSM_EVT_INIT, NULL, 0, &rdata, sizeof(rdata));
	if (rdata_size < 0) {
...
int32_t tcc_thsm_cmd_finalize(uint32_t device_id)
{
	...
	rdata_size = sec_sendrecv_cmd(
		device_id, TCCTHSM_EVT_FINALIZE, NULL, 0, &rdata,
		sizeof(rdata));
	if (rdata_size < 0) {
		ELOG("sec_sendrecv_cmd error(%d)\n", rdata_size);
		return -EBADR;
...
int32_t
tcc_thsm_cmd_get_version(uint32_t device_id, uint32_t *major, uint32_t *minor)
{
	int32_t rdata[4] = {0};
	int32_t rdata_size = 0;
	int32_t result = 0;
	...
	rdata_size = sec_sendrecv_cmd(
		device_id, TCCTHSM_EVT_GET_VERSION, NULL, 0, rdata,
		sizeof(rdata));
	if (rdata_size < 0) {
		ELOG("sec_sendrecv_cmd error(%d)\n", rdata_size);
		return -EBADR;
...
int32_t tcc_thsm_cmd_set_mode(
	uint32_t device_id, uint32_t key_index, uint32_t algorithm,
	uint32_t op_mode, uint32_t key_size)
{
	uint32_t data[4] = {0};
	int32_t data_size = 0;
	...
	rdata_size = sec_sendrecv_cmd(
		device_id, TCCTHSM_EVT_SET_MODE, data, data_size, &rdata,
		sizeof(rdata));
	if (rdata_size < 0) {
		ELOG("sec_sendrecv_cmd error(%d)\n", rdata_size);
		return -EBADR;
...
int32_t tcc_thsm_cmd_set_key(
	uint32_t device_id, uint32_t key_index, uint32_t *key1,
	uint32_t key1_size, uint32_t *key2, uint32_t key2_size, uint32_t *key3,
	uint32_t key3_size)
{
	uint32_t data[128] = {0};
	int32_t rdata = 0;
	...
	rdata_size = sec_sendrecv_cmd(
		device_id, TCCTHSM_EVT_SET_KEY, data, data_size, &rdata,
		sizeof(rdata));
	if (rdata_size < 0) {
		ELOG("sec_sendrecv_cmd error(%d)\n", rdata_size);
		return -EBADR;
...
int32_t tcc_thsm_cmd_set_key_from_storage(
	uint32_t device_id, uint32_t key_index, uint8_t *obj_id,
	uint32_t obj_id_len)
{
	uint32_t data[64] = {0};
	int32_t rdata = 0;
	...
	rdata_size = sec_sendrecv_cmd(
		device_id, TCCTHSM_EVT_SET_KEY_FROM_STORAGE, data, data_size,
		&rdata, sizeof(rdata));
	if (rdata_size < 0) {
		ELOG("sec_sendrecv_cmd error(%d)\n", rdata_size);
		return -EBADR;
...
int32_t tcc_thsm_cmd_set_key_from_otp(
	uint32_t device_id, uint32_t key_index, uint32_t otp_addr,
	uint32_t key_size)
{
	uint32_t data[64] = {0};
	int32_t rdata = 0;
	...
	rdata_size = sec_sendrecv_cmd(
		device_id, TCCTHSM_EVT_SET_KEY_FROM_OTP, data, data_size,
		&rdata, sizeof(rdata));
	if (rdata_size < 0) {
		ELOG("sec_sendrecv_cmd error(%d)\n", rdata_size);
		return -EBADR;
...
int32_t tcc_thsm_cmd_free_mode(uint32_t device_id, uint32_t key_index)
{
	...
	rdata_size = sec_sendrecv_cmd(
		device_id, TCCTHSM_EVT_FREE_MODE, &data, data_size, &rdata,
		sizeof(rdata));
	if (rdata_size < 0) {
		ELOG("sec_sendrecv_cmd error(%d)\n", rdata_size);
		return -EBADR;
	...
	uint32_t device_id, uint32_t key_index, uint32_t *iv, uint32_t iv_size)
{
	...
	rdata_size = sec_sendrecv_cmd(
		device_id, TCCTHSM_EVT_SET_IV_SYMMETRIC, data, data_size,
		&rdata, sizeof(rdata));
	if (rdata_size < 0) {
...
int32_t tcc_thsm_cmd_run_cipher_by_dma(
	uint32_t device_id, uint32_t key_index, uint32_t src_addr,
	uint32_t src_size, uint32_t dst_addr, uint32_t *dst_size, uint32_t flag)
{
	uint32_t data[6] = {0};
	int32_t rdata[2] = {0};
	int32_t data_size = 0, rdata_size = 0;
	...
	rdata_size = sec_sendrecv_cmd(
		device_id, TCCTHSM_EVT_RUN_CIPHER_BY_DMA, data, data_size,
		rdata, DMA_MAX_RSIZE);
	if (rdata_size < 0) {
		DLOG("sec_sendrecv_cmd error(%d)\n", rdata_size);
		return -EBADR;
...
int32_t tcc_thsm_cmd_run_digest_by_dma(
	uint32_t device_id, uint32_t key_index, uint32_t chunk_addr,
	uint32_t chunk_len, uint8_t *hash, uint32_t *hash_len, uint32_t flag)
{
	uint32_t data[8] = {0};
	int32_t rdata[32] = {0};
	int32_t data_size = 0, rdata_size = 0;
	...
	rdata_size = sec_sendrecv_cmd(
		device_id, TCCTHSM_EVT_RUN_DIGEST_BY_DMA, data, data_size,
		rdata, DMA_MAX_RSIZE);
	if (rdata_size < 0) {
		DLOG("sec_sendrecv_cmd error(%d)\n", rdata_size);
		return -EBADR;
...
int32_t tcc_thsm_cmd_set_iv_mac(
	uint32_t device_id, uint32_t key_index, uint32_t *iv, uint32_t iv_size)
{
	uint32_t data[32] = {0};
	int32_t rdata = 0;
	...
	rdata_size = sec_sendrecv_cmd(
		device_id, TCCTHSM_EVT_SET_IV_MAC, data, data_size, &rdata,
		sizeof(rdata));
	if (rdata_size < 0) {
		DLOG("sec_sendrecv_cmd error(%d)\n", rdata_size);
		return -EBADR;
...
int32_t tcc_thsm_cmd_compute_mac_by_dma(
	uint32_t device_id, uint32_t key_index, uint32_t message,
	uint32_t message_len, uint8_t *mac, uint32_t *mac_len, uint32_t flag)
{
	uint32_t data[8] = {0};
	int32_t rdata[32] = {0};
	int32_t data_size = 0, rdata_size = 0;
	...
	rdata_size = sec_sendrecv_cmd(
		device_id, TCCTHSM_EVT_COMPUTE_MAC_BY_DMA, data, data_size,
		rdata, DMA_MAX_RSIZE);
	if (rdata_size < 0) {
		DLOG("sec_sendrecv_cmd error(%d)\n", rdata_size);
		return -EBADR;
	...
	uint32_t message_len, uint8_t *mac, uint32_t mac_len, uint32_t flag)
{
	...
	rdata_size = sec_sendrecv_cmd(
		device_id, TCCTHSM_EVT_COMPARE_MAC_BY_DMA, data, data_size,
		&rdata, sizeof(rdata));
	if (rdata_size < 0) {
		DLOG("sec_sendrecv_cmd error(%d)\n", rdata_size);
		return -EBADR;
...
int32_t
tcc_thsm_cmd_get_rand(uint32_t device_id, uint32_t *rng, uint32_t rng_size)
{
	uint32_t data = 0;
	int32_t rdata[16] = {0};
	int32_t data_size = 0, rdata_size = 0;
	...
	rdata_size = sec_sendrecv_cmd(
		device_id, TCCTHSM_EVT_GET_RAND, &data, data_size, &rdata,
		sizeof(rdata));
	if (rdata_size < 0) {
		DLOG("sec_sendrecv_cmd error(%d)\n", rdata_size);
		return -EBADR;
...
int32_t tcc_thsm_cmd_gen_key_ss(
	uint32_t device_id, int8_t *obj_id, uint32_t obj_len,
	uint32_t algorithm, uint32_t key_size)
{
	...
	rdata_size = sec_sendrecv_cmd(
		device_id, TCCTHSM_EVT_GEN_KEY_SS, data, data_size, &rdata,
		sizeof(rdata));
	if (rdata_size < 0) {
		DLOG("sec_sendrecv_cmd error(%d)\n", rdata_size);
		return -EBADR;
...
int32_t
tcc_thsm_cmd_del_key_ss(uint32_t device_id, int8_t *obj_id, uint32_t obj_len)
{
	uint32_t data[64] = {0};
	int32_t rdata = 0;
	int32_t data_size = 0, rdata_size = 0;
	...
	rdata_size = sec_sendrecv_cmd(
		device_id, TCCTHSM_EVT_DEL_KEY_SS, data, data_size, &rdata,
		sizeof(rdata));
	if (rdata_size < 0) {
		DLOG("sec_sendrecv_cmd error(%d)\n", rdata_size);
		return -EBADR;
...
int32_t tcc_thsm_cmd_write_key_ss(
	uint32_t device_id, int8_t *obj_id, uint32_t obj_len, uint8_t *buffer,
	uint32_t buffer_size)
{
	...
	rdata_size = sec_sendrecv_cmd(
		device_id, TCCTHSM_EVT_WRITE_KEY_SS, data, data_size, &rdata,
		sizeof(rdata));
	if (rdata_size < 0) {
		DLOG("sec_sendrecv_cmd error(%d)\n", rdata_size);
		return -EBADR;
...
int32_t tcc_thsm_cmd_write_otp(
	uint32_t device_id, uint32_t otp_addr, uint8_t *otp_buf,
	uint32_t otp_size)
{
	uint32_t data[32] = {0};
	int32_t rdata = 0;
	...
	rdata_size = sec_sendrecv_cmd(
		device_id, TCCTHSM_EVT_WRITE_OTP, data, data_size, &rdata,
		sizeof(rdata));
	if (rdata_size < 0) {
		DLOG("sec_sendrecv_cmd error(%d)\n", rdata_size);
		return -EBADR;
...
int32_t tcc_thsm_cmd_write_otpimage(
	uint32_t device_id, uint32_t otp_addr, uint32_t otp_size)
{
	uint32_t data[2] = {0};
	int32_t rdata = 0;
	int32_t data_size = 0, rdata_size = 0;
	...
	rdata_size = sec_sendrecv_cmd(
		device_id, TCCTHSM_EVT_WRITE_OTP_IMAGE, data, data_size, &rdata,
		sizeof(rdata));
	if (rdata_size < 0) {
		DLOG("sec_sendrecv_cmd error(%d)\n", rdata_size);
		return -EBADR;
...
int32_t tcc_thsm_cmd_asym_enc_dec_by_dma(
	uint32_t device_id, uint32_t key_index, uint32_t src_addr,
	uint32_t src_size, uint32_t dst_addr, uint32_t *dst_size, uint32_t enc)
{
	uint32_t data[8] = {0};
	int32_t rdata[2] = {0};
	int32_t data_size = 0, rdata_size = 0;
	...
	rdata_size = sec_sendrecv_cmd(
		device_id, cmd, data, data_size, rdata, DMA_MAX_RSIZE);
	if (rdata_size < 0) {
		DLOG("sec_sendrecv_cmd error(%d)\n", rdata_size);
		return -EBADR;
	}
...
int32_t tcc_thsm_cmd_asym_sign_digest(
	uint32_t device_id, uint32_t key_index, uint8_t *dig, uint32_t dig_size,
	uint8_t *sig, uint32_t *sig_size)
{
	...
	rdata_size = sec_sendrecv_cmd(
		device_id, TCCTHSM_EVT_ASYMMETRIC_SIGN, data, data_size, rdata,
		sizeof(rdata));
	if (rdata_size < 0) {
		DLOG("sec_sendrecv_cmd error(%d)\n", rdata_size);
		return -EBADR;
	...
	uint8_t *sig, uint32_t sig_size)
{
	...
	rdata_size = sec_sendrecv_cmd(
		device_id, TCCTHSM_EVT_ASYMMETRIC_VERIFY, data, data_size,
		&rdata, sizeof(rdata));
	if (rdata_size < 0) {
		DLOG("sec_sendrecv_cmd error(%d)\n", rdata_size);
		return -EBADR;
