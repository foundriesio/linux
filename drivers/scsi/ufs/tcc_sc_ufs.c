...
static void tcc_sc_ufs_queuecommand(struct work_struct *work)
{
	...
	sc_cmd.lun = tcc_sc_ufs_scsi_to_upiu_lun(scsi_cmd->device->lun);
	sc_cmd.tag = tag;
...
static int tcc_sc_ufs_queuecommand_sc(struct Scsi_Host *host, struct scsi_cmnd *cmd)
{
	...
	/* queue work */
	queue_work(sc_host->ufs_tcc_wq, &sc_host->request_work);

	return 0;
}

static int tcc_sc_ufs_set_dma_mask(struct tcc_sc_ufs_host *host)
{
...
{
	struct tcc_sc_ufs_host *sc_host = shost_priv(sdev->host);
...
static int tcc_sc_ufs_probe(struct platform_device *pdev)
{
	...
	scsi_host->cmd_per_lun = host->nutrs;
	...
	async_schedule(tcc_sc_ufs_async_scan, host);
	return ret;
}
