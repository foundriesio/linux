...
static int tcc_pwm_enable(struct pwm_chip *chip, struct pwm_device *pwm)
{
	...
	pwm_writel(pwm_readl(tcc->pwm_base + PWMEN) | ((unsigned int)(0x00010UL) << (pwm->hwpwm)),
			tcc->pwm_base + PWMEN);
	pwm_writel(pwm_readl(tcc->pwm_base + PWMEN) | ((unsigned int)(0x00011UL) << (pwm->hwpwm)),
			tcc->pwm_base + PWMEN);
	pwm_writel(pwm_readl(tcc->pwm_base + PWMEN) | ((unsigned int)(0x10011UL) << (pwm->hwpwm)),
			tcc->pwm_base + PWMEN);
...
static void tcc_pwm_wait(struct pwm_chip *chip, struct pwm_device *pwm)
{
	...
	while(delay_cnt != 0UL)
	{
		...
		if((busy  & ((unsigned int)0x1 << pwm->hwpwm)) == (unsigned int)0)
			break;
		delay_cnt--;
	}
...
static void tcc_pwm_register_mode_set(struct pwm_chip *chip, struct pwm_device *pwm, uint regist_value)
{
	...
	reg = ((reg & ~((unsigned int)0xF << bit_shift)) | ((unsigned int)REGISTER_OUT_MODE << bit_shift)) & (unsigned int)0xFFFFFFFF;
	pwm_writel(reg, tcc->pwm_base + PWMMODE);  //phase mode

	reg = pwm_readl(tcc->pwm_base + PWMMODE);
	bit_shift = ((unsigned int)2 * pwm->hwpwm) + (unsigned int)24;
	reg = (reg & ~((unsigned int)0x3 << bit_shift));		//divide by 2 : default value
		...
		int duty_ns, int period_ns_)
{
	...
	dprintk("%s clk_freq:%lu  npwn:%d duty_ns:%d period_ns:%llu hwpwm:%d \n", __func__, clk_freq, chip->npwm, duty_ns, period_ns,  pwm->hwpwm);
	
#ifdef TCC_USE_GFB_PORT
	gfb_port_value = pwm_readl(tcc->io_pwm_port_base);
	gfb_port_value = ((gfb_port_value & ~((unsigned int)0xFF << ((unsigned int)8 * pwm->hwpwm)))
					| ((tcc->gfb_port[pwm->hwpwm] & (unsigned int)0xFF) << ((unsigned int)8 * pwm->hwpwm)));
	...
	for(divide = 1; divide < 0xFFFFFFFF; divide++)
	{
		...
		if((div_u64(ULLONG_MAX, (unsigned int)duty_ns)) > (div_u64(total_cnt, (unsigned int)divide)))
			break;
	}
...
//pwm_result:
	...
	reg = (reg & ~((unsigned int)0xF << bit_shift)) | ((unsigned int)PHASE_MODE << bit_shift);  //phase mode
	pwm_writel(reg, tcc->pwm_base + PWMMODE); 
	...
	reg = (reg & ~((unsigned int)0x1 << bit_shift)) | ((unsigned int)0x0 << bit_shift);//signal inverse clear
	pwm_writel(reg, tcc->pwm_base + PWMMODE);  
...
static int tcc_pwm_probe(struct platform_device *pdev)
{
	...
	if (ret < 0) 	{
		...
		dev_info(&pdev->dev, "pwm default clock :%lu init", PWM_PERI_CLOCK);
	}
	else {
...
static int tcc_pwm_resume(struct device *dev)
{
	...
	for (i = 0; i < tcc->chip.npwm; i++) {
		...
		if (state.enabled) {
			tcc_pwm_config(&tcc->chip, pwm, (int)state.duty_cycle, (int)state.period);
