...
static void tcc_hsio_write_reg(struct tcc_dwmac *gmac,
...
static struct tcc_dwmac *tcc_config_dt(struct platform_device *pdev)
{
	...
	of_property_read_u32(pdev->dev.of_node, "txclk-o-dly",
			&gmac->txclk_o_dly);
	of_property_read_u32(pdev->dev.of_node, "txclk-o-inv",
			&gmac->txclk_o_inv);
	of_property_read_u32(pdev->dev.of_node, "rxclk-i-dly",
			&gmac->rxclk_i_dly);
	of_property_read_u32(pdev->dev.of_node, "rxclk-i-inv",
			&gmac->rxclk_i_inv);
	of_property_read_u32(pdev->dev.of_node, "txclk-i-dly",
			&gmac->txclk_i_dly);
	of_property_read_u32(pdev->dev.of_node, "txclk-i-inv",
			&gmac->txclk_i_inv);
