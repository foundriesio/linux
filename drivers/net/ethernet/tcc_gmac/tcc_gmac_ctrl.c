...
static int tcc_gmac_dma_interrupt(void __iomem *dma_base,
				  struct tcc_gmac_extra_stats *x)
{
	...
	if ((bool)
	    (unlikely
	     ((bool)
	((unsigned int)(intr_status & DMA_STATUS_AIS) != (unsigned int)0)))) {
		CTRL_DBG("CSR5[15] DMA ABNORMAL IRQ: ");
		pr_info("CSR5[15] DMA ABNORMAL IRQ: \n");
		if ((bool)
		    unlikely((unsigned int)(intr_status & DMA_STATUS_UNF) !=
			     (unsigned int)0)) {
		...
		if ((bool)
		    unlikely((unsigned int)(intr_status & DMA_STATUS_TJT) !=
			     (unsigned int)0)) {
			CTRL_DBG("transmit jabber\n");
			x->tx_jabber_irq++;
		}
		if ((bool)
		    unlikely((unsigned int)(intr_status & DMA_STATUS_OVF) !=
			     (unsigned int)0)) {
			CTRL_DBG("recv overflow\n");
			x->rx_overflow_irq++;
		}
		if ((bool)
		    unlikely((unsigned int)(intr_status & DMA_STATUS_RU) !=
			     (unsigned int)0)) {
			CTRL_DBG("receive buffer unavailable\n");
			x->rx_buf_unav_irq++;
		}
		if ((bool)
		    unlikely((unsigned int)(intr_status & DMA_STATUS_RPS) !=
			     (unsigned int)0)) {
			CTRL_DBG("receive process stopped\n");
			x->rx_process_stopped_irq++;
		}
		if ((bool)
		    unlikely((unsigned int)(intr_status & DMA_STATUS_RWT) !=
			     (unsigned int)0)) {
			CTRL_DBG("receive watchdog\n");
			x->rx_watchdog_irq++;
		}
		if ((bool)
		    unlikely((unsigned int)(intr_status & DMA_STATUS_ETI) !=
			     (unsigned int)0)) {
			CTRL_DBG("transmit early interrupt\n");
			x->tx_early_irq++;
		}
		if ((bool)
		    unlikely((unsigned int)(intr_status & DMA_STATUS_TPS) !=
			     (unsigned int)0)) {
		...
		if ((bool)
		    unlikely((unsigned int)(intr_status & DMA_STATUS_FBI) !=
			     (unsigned int)0)) {
	...
	if ((unsigned int)(intr_status & DMA_STATUS_NIS) != (unsigned int)0) {
		x->normal_irq_n++;
		if ((bool)
		    likely(((unsigned int)(intr_status & DMA_STATUS_RI) !=
			    (unsigned int)0)
			   || ((unsigned int)(intr_status & (DMA_STATUS_TI)) !=
			       (unsigned int)0)))
			ret = (int)handle_tx_rx;

	}
	/* Optional hardware blocks, interrupts should be disabled */
	if ((bool) unlikely((unsigned int)(intr_status &
				       (DMA_STATUS_GPI | DMA_STATUS_GMI |
					DMA_STATUS_GLI)) != (unsigned int)0))
		pr_info("%s: unexpected status %08x\n", __func__, intr_status);
	/* Clear the interrupt by writing a logic 1 to the CSR5[15-0] */
	writel(((unsigned int)intr_status & (unsigned int)0x1ffff),
	       dma_base + DMA_STATUS_OFFSET);

	CTRL_DBG("\n\n");
	return ret;
}

static int tcc_gmac_dma_ch0_interrupt(void __iomem *ioaddr,
				      struct tcc_gmac_extra_stats *x)
{
	return tcc_gmac_dma_interrupt(ioaddr + DMA_CH0_OFFSET, x);
}

static int tcc_gmac_dma_ch1_interrupt(void __iomem *ioaddr,
				      struct tcc_gmac_extra_stats *x)
{
	return tcc_gmac_dma_interrupt(ioaddr + DMA_CH1_OFFSET, x);
}

static int tcc_gmac_dma_ch2_interrupt(void __iomem *ioaddr,
				      struct tcc_gmac_extra_stats *x)
{
...
static void tcc_gmac_dma_slot_control(void __iomem *dma_base, bool enable,
				      bool advance)
{
...
static void tcc_gmac_dma_ch1_slot_control(void __iomem *ioaddr, bool enable,
					  bool advance)
{
	tcc_gmac_dma_slot_control(ioaddr + DMA_CH1_OFFSET, enable, advance);
}

static void tcc_gmac_dma_ch2_slot_control(void __iomem *ioaddr, bool enable,
					  bool advance)
{
...
static void tcc_gmac_dma_ch1_cbs_control(void __iomem *ioaddr,
					 ...
					 unsigned int slot_count, bool abpssie)
{
	tcc_gmac_dma_cbs_control(ioaddr + DMA_CH1_OFFSET, enable,
				 credit_control, slot_count, abpssie);
}
...
static void tcc_gmac_dma_ch2_cbs_control(void __iomem *ioaddr,
					 ...
					 unsigned int slot_count, bool abpssie)
{
	tcc_gmac_dma_cbs_control(ioaddr + DMA_CH2_OFFSET, enable,
				 credit_control, slot_count, abpssie);
}
...
static void tcc_gmac_dma_ch1_idle_slope_credit(void __iomem *ioaddr,
					       unsigned int bits_per_cycle)
{
	writel((unsigned int)bits_per_cycle & (unsigned int)0x3fff,
	       ioaddr + DMA_CH1_IDLE_SLOPE_CREDIT);
}

static void tcc_gmac_dma_ch2_idle_slope_credit(void __iomem *ioaddr,
					       unsigned int bits_per_cycle)
{
	writel((unsigned int)bits_per_cycle & (unsigned int)0x3fff,
	       ioaddr + DMA_CH2_IDLE_SLOPE_CREDIT);
}

//////////////////////////////////////////////////////////

static void tcc_gmac_dma_ch1_send_slope_credit(void __iomem *ioaddr,
					       unsigned int bits_per_cycle)
{
	writel((unsigned int)bits_per_cycle & (unsigned int)0x3fff,
	       ioaddr + DMA_CH1_SEND_SLOPE_CREDIT);
}

static void tcc_gmac_dma_ch2_send_slope_credit(void __iomem *ioaddr,
					       unsigned int bits_per_cycle)
{
	writel((unsigned int)bits_per_cycle & (unsigned int)0x3fff,
	       ioaddr + DMA_CH2_SEND_SLOPE_CREDIT);
}

//////////////////////////////////////////////////////////

static void tcc_gmac_dma_ch1_hi_credit(void __iomem *ioaddr,
				       unsigned int credit)
{
	writel((unsigned int)credit & (unsigned int)0x1fffffff,
	       ioaddr + DMA_CH1_HI_CREDIT);
}

static void tcc_gmac_dma_ch2_hi_credit(void __iomem *ioaddr,
				       unsigned int credit)
{
	writel((unsigned int)credit & (unsigned int)0x1fffffff,
	       ioaddr + DMA_CH2_HI_CREDIT);
}

//////////////////////////////////////////////////////////

static void tcc_gmac_dma_ch1_lo_credit(void __iomem *ioaddr,
				       unsigned int credit)
{
	writel((unsigned int)credit & (unsigned int)0x1fffffff,
	       ioaddr + DMA_CH1_LO_CREDIT);
}

static void tcc_gmac_dma_ch2_lo_credit(void __iomem *ioaddr,
				       unsigned int credit)
{
	writel((unsigned int)credit & (unsigned int)0x1fffffff,
	       ioaddr + DMA_CH2_LO_CREDIT);
}
...
data = ((unsigned int)addr[5] << (unsigned int)8) | (unsigned int)addr[4];
writel(data, ioaddr + high);
data =
((unsigned int)addr[3] << (unsigned int)24) | ((unsigned int)addr[2] <<
				  ...
				  unsigned int high, unsigned int low)
{
	...
	addr[1] =
	    (unsigned char)((lo_addr >> (unsigned char)8) & (unsigned char)
			    0xff);
	addr[2] =
	    (unsigned char)((lo_addr >> (unsigned char)16) & (unsigned char)
			    0xff);
	addr[3] =
	    (unsigned char)((lo_addr >> (unsigned char)24) & (unsigned char)
			    0xff);
	addr[4] = (unsigned char)(hi_addr & (unsigned char)0xff);
	addr[5] =
	    (unsigned char)((hi_addr >> (unsigned char)8) & (unsigned char)
			    0xff);
			     ...
			     u32 dma_rx)
{
	...
	do {
		...
		// if((timeout--)<(int)0){
		if ((unsigned int)timeout == (unsigned int)0) {
			value &= ~DMA_BUS_MODE_SFT_RESET;

			writel(value, dma_base + DMA_BUS_MODE_OFFSET);
			pr_info("#ERR# [GMAC] Failed DMA Soft Reset!! \n");
			break;
		} else
			timeout--;

	} while ((unsigned int)
		 (readl(dma_base + DMA_BUS_MODE_OFFSET) &
		  DMA_BUS_MODE_SFT_RESET) != (unsigned int)0);

	value = /*DMA_BUS_MODE_FB | */ DMA_BUS_MODE_8PBL |
#ifdef CONFIG_TCC_GMAC_PTP
	    DMA_BUS_MODE_ATDS |
#endif
	    (((unsigned int)pbl << DMA_BUS_MODE_PBL_SHIFT) |
	     ((unsigned int)pbl << DMA_BUS_MODE_RPBL_SHIFT));
...
static void tcc_gmac_dma_set_bus_mode(void __iomem *dma_base, u32 prwg,
				      u32 txpr, u32 pr, u32 da)
{
	u32 bus_mode;

	bus_mode = readl(dma_base + DMA_BUS_MODE_OFFSET);
	bus_mode =
	    (bus_mode & ~DMA_BUS_MODE_PRWG_MASK) | (prwg <<
						    DMA_BUS_MODE_PRWG_SHIFT);

	if ((unsigned int)txpr != (unsigned int)0)
	...
	if ((unsigned int)da != (unsigned int)0)
...
static void tcc_gmac_dma_ch0_set_bus_mode(void __iomem *ioaddr, u32 prwg,
					  u32 txpr, u32 pr, u32 da)
{
	tcc_gmac_dma_set_bus_mode(ioaddr + DMA_CH0_OFFSET, prwg, txpr, pr, da);
}

static void tcc_gmac_dma_ch1_set_bus_mode(void __iomem *ioaddr, u32 prwg,
					  u32 txpr, u32 pr, u32 da)
{
	tcc_gmac_dma_set_bus_mode(ioaddr + DMA_CH1_OFFSET, prwg, txpr, pr, da);
}

static void tcc_gmac_dma_ch2_set_bus_mode(void __iomem *ioaddr, u32 prwg,
					  u32 txpr, u32 pr, u32 da)
{
...
do {
} while ((unsigned int)(readl(ioaddr + DMA_CH0_CONTROL) & DMA_CONTROL_FTF)
!= (unsigned int)0);
}
					...
					int rxmode)
{
	...
	if (txmode == SF_DMA_MODE) {
		CTRL_DBG(KERN_DEBUG
			 "GMAC: enabling TX store and forward mode\n");
		...
		if (txmode <= 32)
			csr6 |= (unsigned int)DMA_CONTROL_TTC_32;
		else if (txmode <= 64)
			csr6 |= (unsigned int)DMA_CONTROL_TTC_64;
		else if (txmode <= 128)
			csr6 |= (unsigned int)DMA_CONTROL_TTC_128;
		else if (txmode <= 192)
			csr6 |= (unsigned int)DMA_CONTROL_TTC_192;
		else
			csr6 |= (unsigned int)DMA_CONTROL_TTC_256;
	}

	if (rxmode == SF_DMA_MODE) {
		CTRL_DBG(KERN_DEBUG
			 "GMAC: enabling RX store and forward mode\n");
		csr6 |= (unsigned int)DMA_CONTROL_RSF;
	} else {
		CTRL_DBG(KERN_DEBUG "GMAC: disabling RX store and forward mode"
			 " (threshold = %d)\n", rxmode);
		csr6 &= (unsigned int)(~DMA_CONTROL_RSF);
		csr6 &= (unsigned int)DMA_CONTROL_TC_RX_MASK;
		if (rxmode <= 32)
			csr6 |= (unsigned int)DMA_CONTROL_RTC_32;
		else if (rxmode <= 64)
			csr6 |= (unsigned int)DMA_CONTROL_RTC_64;
		else if (rxmode <= 96)
			csr6 |= (unsigned int)DMA_CONTROL_RTC_96;
		else
			csr6 |= (unsigned int)DMA_CONTROL_RTC_128;
	}
...
(unsigned int)i,
(unsigned int)(DMA_CH0_BUS_MODE + (unsigned int)offset),
(unsigned int)readl(ioaddr + DMA_CH0_BUS_MODE +
(unsigned int)offset));
		}
	}
}
...
static void tcc_gmac_dump_dma_ch1_regs(void __iomem *ioaddr)
{
	...
	for (i = 64; i < 94; i++) {
		...
		default:
			...
			pr_err("\t Reg No. %d (offset 0x%x): 0x%08x\n",
(unsigned int)i,
(unsigned int)(DMA_CH1_BUS_MODE + (unsigned int)offset),
(unsigned int)readl(ioaddr + DMA_CH1_BUS_MODE +
(unsigned int)offset));
			break;
		}
	}
...
static void tcc_gmac_dump_dma_ch2_regs(void __iomem *ioaddr)
{
	...
	for (i = 128; i < 158; i++) {
		...
		default:
			...
			pr_err("\t Reg No. %d (offset 0x%x): 0x%08x\n",
(unsigned int)i,
(unsigned int)(DMA_CH2_BUS_MODE + (unsigned int)offset),
(unsigned int)readl(ioaddr + DMA_CH2_BUS_MODE +
offset));
			break;
		}
	}
					...
					void __iomem *ioaddr)
{
	...
	if (((unsigned int)
	(unlikely(((unsigned int)(p->des01.etx.error_summary)
		   != (unsigned int)0)))
	     != (unsigned int)0)) {
		CTRL_DBG(KERN_ERR "GMAC TX error... 0x%08x\n", p->des01.etx);
		if ((unsigned int)
		    (unlikely
		     ((unsigned int)(p->des01.etx.jabber_timeout) !=
		...
		if ((unsigned int)
		    (unlikely
	     ((unsigned int)(p->des01.etx.frame_flushed) != (unsigned int)0)) !=
		    (unsigned int)0) {
		...
		if ((unsigned int)
		    (unlikely
	     ((unsigned int)(p->des01.etx.loss_carrier) != (unsigned int)0)) !=
		    (unsigned int)0) {
		...
		if ((unsigned int)
		    (unlikely
	     ((unsigned int)(p->des01.etx.no_carrier) != (unsigned int)0)) !=
		    (unsigned int)0) {
		...
		if ((unsigned int)
		    (unlikely
		     ((unsigned int)(p->des01.etx.late_collision) !=
		      (unsigned int)0)) != (unsigned int)0) {
		...
		if ((unsigned int)
		    (unlikely
		     ((unsigned int)(p->des01.etx.excessive_collisions) !=
		      (unsigned int)0)) != (unsigned int)0) {
		...
		if ((unsigned int)
		    (unlikely
		     ((unsigned int)(p->des01.etx.excessive_deferral) !=
		      (unsigned int)0)) != (unsigned int)0) {
		...
		if ((unsigned int)
		    (unlikely
		     ((unsigned int)(p->des01.etx.underflow_error) !=
		      (unsigned int)0)) != (unsigned int)0) {
		...
		if ((unsigned int)
		    (unlikely
		     ((unsigned int)(p->des01.etx.ip_header_error) !=
		      (unsigned int)0)) != (unsigned int)0) {
		...
		if ((unsigned int)
		    (unlikely
	     ((unsigned int)(p->des01.etx.payload_error) != (unsigned int)0)) !=
		    (unsigned int)0) {
	...
	if ((unsigned int)
	(unlikely((unsigned int)(p->des01.etx.deferred) != (unsigned int)0)) !=
	    (unsigned int)0) {
		CTRL_DBG(KERN_INFO "GMAC TX status: tx deferred\n");
		x->tx_deferred++;
	}
#ifdef TCC_GMAC_VLAN_TAG_USED
	if ((unsigned int)(p->des01.etx.vlan_frame) != (unsigned int)0) {
...
static int tcc_gmac_coe_rdes0(int ipc_err, int type, int payload_err)
{
	...
	u32 status =
	    (((unsigned int)type << (unsigned int)2) |
	((unsigned int)ipc_err << (unsigned int)1) |
	((unsigned int)payload_err &
						   (unsigned int)0x7));
	...
	if ((unsigned int)status == (unsigned int)0x0) {
		...
		ret = (int)csum_none;
	} else if ((unsigned int)status == (unsigned int)0x6) {
		CTRL_DBG(KERN_ERR "RX Des0 status: IPv4/6 Header Error.\n");
		ret = (int)csum_none;
	} else if ((unsigned int)status == (unsigned int)0x7) {
		CTRL_DBG(KERN_ERR
			 "RX Des0 status: IPv4/6 Header and Payload Error.\n");
		ret = (int)csum_none;
	} else if ((unsigned int)status == (unsigned int)0x1) {
		CTRL_DBG(KERN_ERR
					...
					struct dma_desc *p)
{
	...
	if ((unsigned int)
	(unlikely((unsigned int)(p->des01.erx.error_summary)
		  != (unsigned int)0)) !=
	    (unsigned int)0) {
		...
		if ((unsigned int)
		    (unlikely
		     ((unsigned int)(p->des01.erx.descriptor_error) !=
		      (unsigned int)0)) != (unsigned int)0) {
		...
		if ((unsigned int)
		    (unlikely
		     ((unsigned int)(p->des01.erx.overflow_error) !=
		      (unsigned int)0)) != (unsigned int)0) {
		...
		if ((unsigned int)
		    (unlikely
		     ((unsigned int)(p->des01.erx.ipc_csum_error) !=
		      (unsigned int)0)) != (unsigned int)0)
			CTRL_DBG(KERN_ERR "\tIPC Csum Error/Giant frame\n");

		if ((unsigned int)
		    (unlikely
		     ((unsigned int)(p->des01.erx.late_collision) !=
		      (unsigned int)0)) != (unsigned int)0) {
			CTRL_DBG(KERN_ERR "\tlate_collision error\n");
			stats->collisions++;
			//stats->collisions++;
		...
		if ((unsigned int)
		    (unlikely
		     ((unsigned int)(p->des01.erx.receive_watchdog) !=
		      (unsigned int)0)) != (unsigned int)0) {
		...
		if ((unsigned int)
		    (unlikely
	     ((unsigned int)(p->des01.erx.error_gmii) != (unsigned int)0)) !=
		    (unsigned int)0) {
		...
		if ((unsigned int)
		    (unlikely
	     ((unsigned int)(p->des01.erx.crc_error) != (unsigned int)0)) !=
		    (unsigned int)0) {
	...
	ret = tcc_gmac_coe_rdes0((int)(p->des01.erx.ipc_csum_error),
				 (int)(p->des01.erx.frame_type),
				 (int)(p->des01.erx.payload_csum_error));

	if ((unsigned int)
	(unlikely((unsigned int)(p->des01.erx.dribbling) != (unsigned int)0)) !=
	    (unsigned int)0) {
		CTRL_DBG(KERN_ERR "GMAC RX: dribbling error\n");
		ret = (int)discard_frame;
	}
	if ((unsigned int)
	(unlikely((unsigned int)(p->des01.erx.sa_filter_fail)
		  != (unsigned int)0))
	    != (unsigned int)0) {
	...
	if ((unsigned int)
	(unlikely((unsigned int)(p->des01.erx.da_filter_fail)
		  != (unsigned int)0))
	    != (unsigned int)0) {
		CTRL_DBG(KERN_ERR
			 "GMAC RX : Destination Address filter fail\n");
		x->da_rx_filter_fail++;
		ret = (int)discard_frame;
	}
	if ((unsigned int)
	(unlikely((unsigned int)(p->des01.erx.length_error)
		  != (unsigned int)0)) !=
	    (unsigned int)0) {
	...
	if ((unsigned int)
	(unlikely((unsigned int)(p->des01.erx.vlan_tag) != (unsigned int)0)) !=
	    (unsigned int)0) {
				  ...
				  int disable_rx_ic)
{
	...
	pr_info("[INFO][GMAC]  %s : ring_size %d \n ", __func__,
	       ring_size);
for (i = 0; (unsigned int)i < (unsigned int)ring_size; (unsigned int)i++) {
#ifdef CONFIG_TCC_GMAC_PTP
		...
		if ((unsigned int)i ==
		    (unsigned int)((unsigned int)ring_size - (unsigned int)1))
			p->des01.erx.end_ring = 1;
		if ((unsigned int)disable_rx_ic != (unsigned int)0)
...
static void tcc_gmac_init_tx_desc(struct dma_desc *p, unsigned int ring_size)
{
	...
	pr_info("[INFO][GMAC] %s : ring_size %d \n ", __func__,
	       ring_size);
	for (i = 0; (unsigned int)i < (unsigned int)ring_size; i++) {
		p->des01.etx.own = 0;
	if ((unsigned int)i == (unsigned int)(ring_size - (unsigned int)1))
...
static void tcc_gmac_release_tx_desc(struct dma_desc *p)
{
	...
	p->des01.etx.end_ring = (unsigned int)ter;
...
#endif
	p->des01.etx.first_segment = (unsigned int)is_fs;
if ((unsigned int)(unlikely((int)len > (int)BUF_SIZE_4KiB))
		!= (unsigned int)0) {
		p->des01.etx.buffer1_size = (unsigned int)BUF_SIZE_4KiB;
		p->des01.etx.buffer2_size =
		    (unsigned int)((unsigned int)len -
				    (unsigned int)BUF_SIZE_4KiB);
	} else {
		p->des01.etx.buffer1_size = (unsigned int)len;
	}
	if ((unsigned int)(likely(
				(unsigned int)csum_flag != (unsigned int)0)) !=
	    (unsigned int)0)
		p->des01.etx.checksum_insertion = (unsigned int)cic_full;
}
...
static void tcc_gmac_timestamp_irq_enable(void __iomem *ioaddr)
{
	...
	intr_mask &= (unsigned int)(~(unsigned int)time_stamp_irq);
...
static void tcc_gmac_timestamp_irq_disable(void __iomem *ioaddr)
{
	...
	intr_mask = readl(ioaddr + GMAC_INT_MASK);
	intr_mask |= (unsigned int)time_stamp_irq;
	writel(intr_mask, ioaddr + GMAC_INT_MASK);
}
...
static void tcc_gmac_set_av_ethertype(void __iomem *ioaddr,
				      unsigned int ethertype)
{
	...
	av_mac_control =
	    (av_mac_control & ~GMAC_AV_MAC_CONTROL_ETHER_TYPE_MASK) |
	    ((ethertype << GMAC_AV_MAC_CONTROL_ETHER_TYPE_SHIFT) &
	     GMAC_AV_MAC_CONTROL_ETHER_TYPE_MASK);
	writel(av_mac_control, ioaddr + GMAC_AV_MAC_CONTROL);
}

static void tcc_gmac_set_av_priority(void __iomem *ioaddr,
				     unsigned int priority)
{
	...
	av_mac_control = (av_mac_control & ~GMAC_AV_MAC_CONTROL_AVP_MASK) |
	    ((priority << GMAC_AV_MAC_CONTROL_AVP_SHIFT) &
	     GMAC_AV_MAC_CONTROL_AVP_MASK);
	writel(av_mac_control, ioaddr + GMAC_AV_MAC_CONTROL);
}
...
static void tcc_gmac_set_av_avch(void __iomem *ioaddr, unsigned int avch)
{
	...
	av_mac_control = (av_mac_control & ~GMAC_AV_MAC_CONTROL_AVCH_MASK) |
	    ((avch << GMAC_AV_MAC_CONTROL_AVCH_SHIFT) &
	     GMAC_AV_MAC_CONTROL_AVCH_MASK);
	writel(av_mac_control, ioaddr + GMAC_AV_MAC_CONTROL);
}
...
static void tcc_gmac_set_av_ptpch(void __iomem *ioaddr, unsigned int ptpch)
{
	...
	av_mac_control = (av_mac_control & ~GMAC_AV_MAC_CONTROL_PTPCH_MASK) |
	    ((ptpch << GMAC_AV_MAC_CONTROL_PTPCH_SHIFT) &
	     GMAC_AV_MAC_CONTROL_PTPCH_MASK);
	writel(av_mac_control, ioaddr + GMAC_AV_MAC_CONTROL);
}
...
static void tcc_gmac_ptp_pps0_trig_enable(void __iomem *ioaddr)
{
	...
	pps_control |=
	    ((unsigned int)trgtmodesel_intr <<
	     GMAC_PPS_CONTROL_TRGTMODESEL0_SHIFT);
...
static void tcc_gmac_ptp_pps0_trig_disable(void __iomem *ioaddr)
{
	...
	pps_control &= ~GMAC_PPS_CONTROL_TRGTMODESEL0_MASK;
	pps_control |=
	    ((unsigned int)trgtmodesel_reserved <<
	     GMAC_PPS_CONTROL_TRGTMODESEL0_SHIFT);
	writel(pps_control, ioaddr + GMAC_PPS_CONTROL);
}
...
static void tcc_gmac_ptp_pps0_set_time(void __iomem *ioaddr, u32 sec, u32 nsec)
{
	while ((unsigned int)
		...
		GMAC_TARGET_TIME_NANO_SECONDS_TSTRBUSY) != (unsigned int)0)
		;
					   ...
					   u32 sec, u32 nsec, bool addsub)
{
	...
	for (i = 0; i < TS_UPDT_LOOP_MAX; i++) {
		if ((unsigned int)
		    (readl(ioaddr + GMAC_TIMESTAMP_CONTROL) &
		     GMAC_TIMESTAMP_CONTROL_TSUPDT) == (unsigned int)0) {
			break;
		}
	...
	if (i < TS_INIT_LOOP_MAX) {
		...
		pr_info("The TSUPDT bit is not getting cleared\n");
		return -1;
	}
...
static int tcc_gmac_get_system_time(void __iomem *ioaddr,
...
static int tcc_gmac_set_system_time(void __iomem *ioaddr, u32 sec, u32 nsec)
{
	...
	for (i = 0; i < TS_INIT_LOOP_MAX; i++) {
		if ((unsigned int)
		    (readl(ioaddr + GMAC_TIMESTAMP_CONTROL) &
		     GMAC_TIMESTAMP_CONTROL_TSINIT) == (unsigned int)0) {
			break;
		}
	...
	if (i < TS_INIT_LOOP_MAX) {
		...
		pr_info("The TSINIT bit is not getting cleared\n");
		return -1;
	}
	return 0;
}
...
static int tcc_gmac_ptp_snapshot_mode(void __iomem *ioaddr, u32 rx_filter)
{
	...
	switch (rx_filter) {
	case (unsigned int)HWTSTAMP_FILTER_NONE:
		ret = 0;
		break;
	case (unsigned int)HWTSTAMP_FILTER_ALL:
	case (unsigned int)HWTSTAMP_FILTER_SOME:
		ts_control |= GMAC_TIMESTAMP_CONTROL_TSENALL;
		ret = 0;
		break;

	case (unsigned int)HWTSTAMP_FILTER_PTP_V1_L4_EVENT:
		// (SNAPTYPE:01, TSMSTRENA:x, TSEVNTENA:0)
		ts_control |= ((unsigned int)0x1 <<
				GMAC_TIMESTAMP_CONTROL_SNAPTYPSEL_SHIFT) |
			(GMAC_TIMESTAMP_CONTROL_TSIPV4ENA) |	//PTP over IPv4
		    (GMAC_TIMESTAMP_CONTROL_TSIPV6ENA);	//PTP over IPv6
		ret = 0;
		break;
	case (unsigned int)HWTSTAMP_FILTER_PTP_V1_L4_SYNC:
		// (SNAPTYPE:00, TSMSTRENA:0, TSEVNTENA:1)
		ts_control |= ((unsigned int)0x0 <<
				GMAC_TIMESTAMP_CONTROL_SNAPTYPSEL_SHIFT) |
			(GMAC_TIMESTAMP_CONTROL_TSEVNTENA) |
			(GMAC_TIMESTAMP_CONTROL_TSIPV4ENA) |	//PTP over IPv4
		    (GMAC_TIMESTAMP_CONTROL_TSIPV6ENA);	//PTP over IPv6
		ret = 0;
		break;
	case (unsigned int)HWTSTAMP_FILTER_PTP_V1_L4_DELAY_REQ:
	...
	case (unsigned int)HWTSTAMP_FILTER_PTP_V2_L4_EVENT:
		// (SNAPTYPE:01, TSMSTRENA:x, TSEVNTENA:0)
		ts_control |= ((unsigned int)0x1 <<
				GMAC_TIMESTAMP_CONTROL_SNAPTYPSEL_SHIFT) |
			(GMAC_TIMESTAMP_CONTROL_TSIPV4ENA) |	//PTP over IPv4
		...
		ret = 0;
		break;
	case (unsigned int)HWTSTAMP_FILTER_PTP_V2_L4_SYNC:
		// (SNAPTYPE:00, TSMSTRENA:0, TSEVNTENA:1)
		ts_control |= ((unsigned int)0x0 <<
				GMAC_TIMESTAMP_CONTROL_SNAPTYPSEL_SHIFT) |
			(GMAC_TIMESTAMP_CONTROL_TSEVNTENA) |
		...
		ret = 0;
		break;
	case (unsigned int)HWTSTAMP_FILTER_PTP_V2_L4_DELAY_REQ:
		// (SNAPTYPE:00, TSMSTRENA:1, TSEVNTENA:1)
		ts_control |= ((unsigned int)0x0 <<
				GMAC_TIMESTAMP_CONTROL_SNAPTYPSEL_SHIFT) |
			(GMAC_TIMESTAMP_CONTROL_TSMSTRENA) |
	...
	case (unsigned int)HWTSTAMP_FILTER_PTP_V2_L2_EVENT:
		// (SNAPTYPE:01, TSMSTRENA:x, TSEVNTENA:0)
		ts_control |= ((unsigned int)0x1 <<
				GMAC_TIMESTAMP_CONTROL_SNAPTYPSEL_SHIFT) |
			(GMAC_TIMESTAMP_CONTROL_TSIPENA) |
		...
		ret = 0;
		break;
	case (unsigned int)HWTSTAMP_FILTER_PTP_V2_L2_SYNC:
		// (SNAPTYPE:00, TSMSTRENA:0, TSEVNTENA:1)
		ts_control |= ((unsigned int)0x0 <<
				GMAC_TIMESTAMP_CONTROL_SNAPTYPSEL_SHIFT) |
			(GMAC_TIMESTAMP_CONTROL_TSEVNTENA) |
		...
		ret = 0;
		break;
	case (unsigned int)HWTSTAMP_FILTER_PTP_V2_L2_DELAY_REQ:
		// (SNAPTYPE:00, TSMSTRENA:1, TSEVNTENA:1)
		ts_control |= ((unsigned int)0x0 <<
				GMAC_TIMESTAMP_CONTROL_SNAPTYPSEL_SHIFT) |
			(GMAC_TIMESTAMP_CONTROL_TSMSTRENA) |
	...
	case (unsigned int)HWTSTAMP_FILTER_PTP_V2_EVENT:
		// (SNAPTYPE:01, TSMSTRENA:x, TSEVNTENA:0)
		ts_control |= ((unsigned int)0x1 <<
				GMAC_TIMESTAMP_CONTROL_SNAPTYPSEL_SHIFT) |
			(GMAC_TIMESTAMP_CONTROL_TSIPENA) |
		...
		ret = 0;
		break;
	case (unsigned int)HWTSTAMP_FILTER_PTP_V2_SYNC:
		// (SNAPTYPE:00, TSMSTRENA:0, TSEVNTENA:1)
		ts_control |= ((unsigned int)0x0 <<
				GMAC_TIMESTAMP_CONTROL_SNAPTYPSEL_SHIFT) |
			(GMAC_TIMESTAMP_CONTROL_TSEVNTENA) |
		...
		ret = 0;
		break;
	case (unsigned int)HWTSTAMP_FILTER_PTP_V2_DELAY_REQ:
		// (SNAPTYPE:00, TSMSTRENA:1, TSEVNTENA:1)
		ts_control |= ((unsigned int)0x0 <<
				GMAC_TIMESTAMP_CONTROL_SNAPTYPSEL_SHIFT) |
			(GMAC_TIMESTAMP_CONTROL_TSMSTRENA) |
...
static int tcc_gmac_ptp_timestamp_init(void __iomem *ioaddr, u32 sec, u32 nsec)
{
	...
	pps_control =
	    ((unsigned int)trgtmodesel_reserved <<
	     GMAC_PPS_CONTROL_TRGTMODESEL0_SHIFT) | ((unsigned int)
			     trgtmodesel_reserved <<
			     GMAC_PPS_CONTROL_TRGTMODESEL1_SHIFT)
| ((unsigned int)trgtmodesel_reserved <<
GMAC_PPS_CONTROL_TRGTMODESEL2_SHIFT) | ((unsigned int)
			       trgtmodesel_reserved <<
				       GMAC_PPS_CONTROL_TRGTMODESEL3_SHIFT);
...
static void tcc_gmac_set_filter(struct net_device *dev)
{
	...
	if (((unsigned int)(dev->flags) &
				(unsigned int)IFF_PROMISC) != (unsigned int)0)
		...
		netdev_for_each_mc_addr((ha), (dev)) {
			...
			u32 bit_nr =
			    (unsigned int)(bitrev32
				       ((~
					 ((unsigned int)
				  crc32_le((~((unsigned int)0)), (ha->addr),
						   (6)))))) >> 24;
			/* The most significant bit determines the register to
			 * use (H/L) while the other 5 bits determine the bit
			 * within the register. */
			mc_filter[bit_nr >> (unsigned int)5] |=
			    (unsigned int)1 << (bit_nr & (unsigned int)0x1f);
		}
...
if ((unsigned int)(dev->uc.count) > (unsigned int)GMAC_MAX_UNICAST_ADDRESSES)
		...
		tcc_gmac_set_umac_addr(ioaddr, ha->addr, (unsigned int)reg);
			reg++;
		}
	}
	...
	CTRL_DBG("\tFrame Filter reg: 0x%08x\n\tHash regs: "
		 ...
		 readl(ioaddr + GMAC_HASH_TABLE_0),
		 readl(ioaddr + GMAC_HASH_TABLE_1),
		 readl(ioaddr + GMAC_HASH_TABLE_2),
		 readl(ioaddr + GMAC_HASH_TABLE_3),
		 readl(ioaddr + GMAC_HASH_TABLE_4),
		 readl(ioaddr + GMAC_HASH_TABLE_5),
		 readl(ioaddr + GMAC_HASH_TABLE_6),
		 readl(ioaddr + GMAC_HASH_TABLE_7));
			       ...
			       unsigned int fc, unsigned int pause_time)
{
	...
	if (((unsigned int)fc & (unsigned int)FLOW_RX) != (unsigned int)0) {
		CTRL_DBG(KERN_DEBUG "\tReceive Flow-Control ON\n");
		flow |= GMAC_FLOW_CTRL_RFE;
	}
	if (((unsigned int)fc & (unsigned int)FLOW_TX) != (unsigned int)0) {
		CTRL_DBG(KERN_DEBUG "\tTransmit Flow-Control ON\n");
		flow |= GMAC_FLOW_CTRL_TFE;
	}

	if ((unsigned int)duplex != (unsigned int)0) {
...
static void tcc_gmac_pmt(void __iomem *ioaddr, unsigned long mode)
{
	...
	if ((unsigned int)mode == (unsigned int)WAKE_MAGIC) {
		CTRL_DBG(KERN_DEBUG "GMAC: WOL Magic frame\n");
		pr_info("GMAC: WOL Magic frame\n");
		pmt |= (unsigned int)power_down | (unsigned int)magic_pkt_en;
	} else if ((unsigned int)mode == (unsigned int)WAKE_UCAST) {
		CTRL_DBG(KERN_DEBUG "GMAC: WOL on global unicast\n");
		pmt |= (unsigned int)global_unicast;
	}
...
if (((unsigned int)intr_status & (unsigned int)mmc_tx_irq) != (unsigned int)0) {
		CTRL_DBG(KERN_DEBUG "GMAC: MMC tx interrupt: 0x%08x\n",
			 readl(ioaddr + GMAC_MMC_TX_INTR));
	}
	if ((unsigned int)
	    (unlikely
	(((unsigned int)intr_status & (unsigned int)mmc_rx_irq)
	!= (unsigned int)0)) !=
	    (unsigned int)0) {
	...
	if ((unsigned int)
	    (unlikely
	     (((unsigned int)intr_status &
	       (unsigned int)mmc_rx_csum_offload_irq) !=
	      (unsigned int)0)) != (unsigned int)0) {
	...
	if ((unsigned int)
	    (unlikely
	     (((unsigned int)intr_status &
	       (unsigned int)pmt_irq) != (unsigned int)0)) !=
	    (unsigned int)0) {
	...
	if ((unsigned int)
	    (unlikely
	     (((unsigned int)intr_status &
	       (unsigned int)rgmii_irq) != (unsigned int)0)) !=
	    (unsigned int)0) {
	...
	if ((unsigned int)
	    (unlikely
	     (((unsigned int)intr_status & (unsigned int)time_stamp_irq) !=
	      (unsigned int)0)) != (unsigned int)0) {
		u32 ts_status;
	...
	if ((unsigned int)
	    (unlikely
	     (((unsigned int)intr_status &
	       (unsigned int)lpiis_irq) != (unsigned int)0)) !=
	    (unsigned int)0) {
...
unsigned int calc_mdio_clk_rate(unsigned int bus_clk_rate)
{
	...
	if (bus_clk_rate > (unsigned int)300000000)
		pr_warn
		    ("tcc_gmac warning : bus_clk_rate is out \
		     of range!! mdc must be under 2.5Mhz\n");

clk_rate = (bus_clk_rate < (unsigned int)35000000) ? (GMII_CLK_RANGE_20_35M) :
	// div 16
((bus_clk_rate < (unsigned int)60000000) ? (GMII_CLK_RANGE_35_60M) :
 // div 26
((bus_clk_rate < (unsigned int)100000000) ? (GMII_CLK_RANGE_60_100M) :
 // div 42
((bus_clk_rate < (unsigned int)150000000) ? (GMII_CLK_RANGE_100_150M) :
 // div 62
((bus_clk_rate < (unsigned int)250000000) ? (GMII_CLK_RANGE_150_250M) :
 // div 102
