...
static DEFINE_MUTEX(contexts_lock);
...
static struct arm_smmu_ctx_desc *arm_smmu_share_asid(u16 asid)
{
	 ...
	 */
	ret = __xa_alloc(&asid_xa, &cd->asid, cd,
			 XA_LIMIT(1, 1 << smmu->asid_bits),
			 GFP_ATOMIC);
	if (ret)
	 ...
	 * TLB.
	 */
	arm_smmu_write_ctx_desc(smmu_domain->parent ?: smmu_domain,
				smmu_domain->ssid, cd);

	/* Invalidate TLB entries previously associated with that context */
	arm_smmu_tlb_inv_asid(smmu, old_asid);

	__xa_erase(&asid_xa, old_asid);
...
static struct arm_smmu_ctx_desc *arm_smmu_alloc_shared_cd(struct mm_struct *mm)
{
	...
	if (!old_cd) {
		old_cd = __xa_store(&asid_xa, asid, cd, GFP_KERNEL);
		ret = xa_err(old_cd);
		BUG_ON(!ret && old_cd);
		cd->asid = asid;
	}
	...
	/* FIXME: these are defined in io-pgtable-arm.c */
...
static int arm_smmu_atc_inv_master_all(struct arm_smmu_master *master, int ssid)
{
	struct arm_smmu_cmdq_ent cmd;
...
static bool arm_smmu_ats_supported(struct arm_smmu_master *master)
{
	...
	return to_pci_dev(master->dev)->ats_cap;
}
#else
static bool arm_smmu_ats_supported(struct arm_smmu_master *master)
{
...
static int arm_smmu_mm_attach(struct device *dev, int pasid, void *entry)
{
	...
	/* TODO: multiple devices per domain! */
...
static void arm_smmu_aux_detach_dev(struct iommu_domain *domain, struct device *dev)
{
	...
	if (!--smmu_domain->aux_nr_devs) {
		...
		arm_smmu_atc_inv_master_all(master, smmu_domain->ssid);
	}
out_unlock:
	mutex_unlock(&smmu_domain->init_mutex);
}
