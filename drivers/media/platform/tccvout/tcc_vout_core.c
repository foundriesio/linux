		...
		unsigned int *base0, unsigned int *base1, unsigned int *base2)
{
	...
	if ((viocfmt >= VIOC_IMG_FMT_RGB332)
		&& (viocfmt <= VIOC_IMG_FMT_ARGB6666_3)) {
		int bpp;

		if (viocfmt == VIOC_IMG_FMT_RGB332)
			bpp = 1;
		else if ((viocfmt >= VIOC_IMG_FMT_ARGB4444)
			&& (viocfmt <= VIOC_IMG_FMT_ARGB1555))
			bpp = 2;
		else if ((viocfmt >= VIOC_IMG_FMT_ARGB8888)
			&& (viocfmt <= VIOC_IMG_FMT_ARGB6666_3))
			bpp = 4;
		else
			bpp = 2;
...
void vout_pop_all_buffer(struct tcc_vout_device *vout)
{
	...
	atomic_add(atomic_read(&vout->readable_buff_count),
		&vout->displayed_buff_count);
...
int vout_set_m2m_path(int deintl_default, struct tcc_vout_device *vout)
{
	...
	memcpy(&vout->deintl_pmap.name, kasprintf(GFP_KERNEL,
		"v4l2_vout%d", vout->id), sizeof(VOUT_MEM_PATH_PMAP_NAME));
	...
	if (deintl_default) {
		...
		if (dev_np) {
			/* swreser bit */
			of_property_read_u32_index(vout->v4l2_np,
				"m2m_rdma", 1, &vioc->m2m_rdma.id);
			vioc->m2m_rdma.addr = VIOC_RDMA_GetAddress(
		...
		if (dev_np) {
			...
			of_property_read_u32_index(
				vout->v4l2_np, "scaler", 1, &vioc->sc.id);
			vioc->sc.addr = VIOC_SC_GetAddress(
	...
	case VIOC_RDMA12:
		vioc->m2m_wmix.pos = 0;
		vioc->m2m_subplane_wmix.pos = 1;
		vioc->m2m_wmix.ovp = vioc->m2m_subplane_wmix.ovp = 5;
		break;
	case VIOC_RDMA13:
		vioc->m2m_wmix.pos = 1;
	...
	if (dev_np) {
		/* swreser bit */
		of_property_read_u32_index(
			vout->v4l2_np, "m2m_wmix", 1, &vioc->m2m_wmix.id);
		vioc->m2m_wmix.addr = VIOC_WMIX_GetAddress(
	...
	if (dev_np) {
		/* swreser bit */
		of_property_read_u32_index(
			vout->v4l2_np, "m2m_wdma", 1, &vioc->m2m_wdma.id);
		vioc->m2m_wdma.addr = VIOC_WDMA_GetAddress(
			get_vioc_index(vioc->m2m_wdma.id));
		vioc->m2m_wdma.irq = irq_of_parse_and_map(
			dev_np, get_vioc_index(vioc->m2m_wdma.id));

		vioc->m2m_wdma.vioc_intr =
			kzalloc(sizeof(struct vioc_intr_type), GFP_KERNEL);
		if (vioc->m2m_wdma.vioc_intr == 0) {
			pr_err("[ERR][VOUT] memory allocation fail(vioc->m2m_wdma)\n");
			return -ENOMEM;
		}
		vioc->m2m_wdma.vioc_intr->id =
			VIOC_INTR_WD0 + get_vioc_index(vioc->m2m_wdma.id);
		vioc->m2m_wdma.vioc_intr->bits = VIOC_WDMA_IREQ_EOFR_MASK;
		dprintk("[M2M] WDMA < vir_addr = 0x%p , id = %d, irq = %d\n",
			vioc->m2m_wdma.addr, get_vioc_index(vioc->m2m_wdma.id),
			vioc->m2m_wdma.irq);
	} else {
	...
	if (dev_np) {
		/* swreser bit */
		of_property_read_u32_index(
			vout->v4l2_np, "m2m_subplane_rdma", 1,
			&vioc->m2m_subplane_rdma.id);
		vioc->m2m_subplane_rdma.addr =
	...
	if (!of_property_read_u32_index(
		vout->v4l2_np, "deintl_block", 1, &deinterlacer)) {
		if (get_vioc_type(deinterlacer)
			== get_vioc_type(VIOC_DEINTLS)) {
			vout->deinterlace = VOUT_DEINTL_S;

			/* path plugin */
			vioc->deintl_s.id = deinterlacer;
			dprintk("DEINTL_S < id = %d\n", vioc->deintl_s.id);
		} else if (get_vioc_type(deinterlacer) ==
			get_vioc_type(VIOC_VIQE)) {
			dev_np = of_parse_phandle(vout->v4l2_np,
				"deintl_block", 0);
			if (dev_np) {
				vout->deinterlace = VOUT_DEINTL_VIQE_3D;

				vioc->viqe.id = deinterlacer;
				vioc->viqe.addr = VIOC_VIQE_GetAddress(
					get_vioc_index(vioc->viqe.id));
	...
	dprintk("RDMA%d%s - WMIX%d - SC%d - WDMA%d\n",
		get_vioc_index(vioc->m2m_rdma.id),
		vout->deinterlace == VOUT_DEINTL_VIQE_3D ?
		" - VIQE" : vout->deinterlace ==
		VOUT_DEINTL_S ? " - DEINTL" : "",
		get_vioc_index(vioc->m2m_wmix.id),
		get_vioc_index(vioc->sc.id),
		get_vioc_index(vioc->m2m_wdma.id));
...
int vout_set_m2m_dual_path(struct tcc_vout_device *vout)
{
	...
	memcpy(&vout->m2m_dual_pmap.name,
		kasprintf(GFP_KERNEL, "dual_display"),
	...
	for (i = M2M_DUAL_0; i < M2M_DUAL_MAX; i++) {
		...
		if (dev_np) {
			/* swreser bit */
			of_property_read_u32_index(
				vout->v4l2_np, "m2m_dual_rdma", i+1,
		...
		if (dev_np) {
			...
			vioc->m2m_dual_sc[i].addr =
				VIOC_SC_GetAddress(get_vioc_index(
					vioc->m2m_dual_sc[i].id));
		...
		case VIOC_RDMA07:
			vioc->m2m_dual_wmix[i].pos = 3;
			vioc->m2m_subplane_wmix.pos = 1;
			vioc->m2m_dual_wmix[i].ovp =
		...
		case VIOC_RDMA16:
			...
			break;
		default:
			pr_err("[ERR][VOUT] invalid m2m_dual_rdma(%d) index\n",
				vioc->m2m_dual_rdma[i].id);
			return -1;
		}
		...
		if (dev_np) {
			...
			of_property_read_u32_index(vout->v4l2_np,
				"m2m_dual_wmix", i+1,
				&vioc->m2m_dual_wmix[i].id);
			vioc->m2m_dual_wmix[i].addr =
				VIOC_WMIX_GetAddress(
					get_vioc_index(
						vioc->m2m_dual_wmix[i].id));
			dprintk("[EXT] WMIX < vir_addr = 0x%p , id = %d\n",
				vioc->m2m_dual_wmix[i].addr,
				get_vioc_index(vioc->m2m_dual_wmix[i].id));
		} else {
		...
		if (dev_np) {
			...
			of_property_read_u32_index(vout->v4l2_np,
				"m2m_dual_wdma", i+1,
				&vioc->m2m_dual_wdma[i].id);
			vioc->m2m_dual_wdma[i].addr =
				VIOC_WDMA_GetAddress(
					get_vioc_index(
						vioc->m2m_dual_wdma[i].id));
			vioc->m2m_dual_wdma[i].irq = irq_of_parse_and_map(
				dev_np, get_vioc_index(
					vioc->m2m_dual_wdma[i].id));

			vioc->m2m_dual_wdma[i].vioc_intr =
				kzalloc(sizeof(struct vioc_intr_type),
				GFP_KERNEL);
			if (vioc->m2m_dual_wdma[i].vioc_intr == 0) {
				pr_err("[ERR][VOUT] memory allocation fail (vioc->m2m_dual_wdma[i])\n");
				return -ENOMEM;
			}
			vioc->m2m_dual_wdma[i].vioc_intr->id =
				VIOC_INTR_WD0 + get_vioc_index(
					vioc->m2m_dual_wdma[i].id);
			vioc->m2m_dual_wdma[i].vioc_intr->bits =
				VIOC_WDMA_IREQ_EOFR_MASK;
			dprintk("[M2M] WDMA < vir_addr = 0x%p, id = %d, irq = %d\n",
				vioc->m2m_dual_wdma[i].addr,
				get_vioc_index(vioc->m2m_dual_wdma[i].id),
				vioc->m2m_dual_wdma[i].irq);
		} else {
			dprintk("[M2M] could not find wdma node of vout driver.\n");
		}
		...
		dprintk("RDMA%d%s - WMIX%d - SC%d - WDMA%d\n",
			get_vioc_index(vioc->m2m_dual_rdma[i].id),
			vout->deinterlace == VOUT_DEINTL_VIQE_3D ? " - VIQE"
			: vout->deinterlace == VOUT_DEINTL_S ? " - DEINTL" : "",
			get_vioc_index(vioc->m2m_dual_wmix[i].id),
			get_vioc_index(vioc->m2m_dual_sc[i].id),
			get_vioc_index(vioc->m2m_dual_wdma[i].id));
	}
...
int vout_set_vout_dual_path(struct tcc_vout_device *vout)
{
	...
	if (dev_np) {
		/* swreser bit */
		of_property_read_u32_index(vout->v4l2_np, "rdma", 2,
			&vioc->rdma_dual.id);
		vioc->rdma_dual.addr =
			VIOC_RDMA_GetAddress(
	...
	if (dev_np) {
		/* swreser bit */
		of_property_read_u32_index(vout->v4l2_np, "wmix", 2,
			&vioc->wmix_dual.id);
		vioc->wmix_dual.addr = VIOC_WMIX_GetAddress(
			get_vioc_index(vioc->wmix_dual.id));
	...
	case VIOC_RDMA11:
		if (get_vioc_index(vioc->wmix_dual.id))
			...
			- (0x4 * get_vioc_index(vioc->wmix_dual.id));
		else
			vioc->wmix_dual.pos =
			get_vioc_index(vioc->rdma_dual.id);
		break;
	...
	if (dev_np) {
		of_property_read_u32_index(vout->v4l2_np, "disp", 2,
			&vioc->disp_dual.id);
		vioc->disp_dual.addr = VIOC_DISP_GetAddress(
			get_vioc_index(vioc->disp_dual.id));
...
int vout_set_vout_path(struct tcc_vout_device *vout)
{
	...
	if (dev_np) {
		...
		of_property_read_u32_index(vout->v4l2_np,
			"rdma", 1, &vioc->rdma.id);
		vioc->rdma.addr = VIOC_RDMA_GetAddress(
	...
	if (dev_np) {
		/* swreser bit */
		of_property_read_u32_index(vout->v4l2_np, "wmix", 1,
			&vioc->wmix.id);
		vioc->wmix.addr = VIOC_WMIX_GetAddress(
	...
	case VIOC_RDMA11:
		...
		if (get_vioc_index(vioc->wmix.id))
			...
			- (0x4 * get_vioc_index(vioc->wmix.id));
		else
			vioc->wmix.pos = get_vioc_index(vioc->rdma.id);
		break;
	default:
	...
	if (dev_np) {
		of_property_read_u32_index(vout->v4l2_np,
			"disp", 1, &vioc->disp.id);
		vioc->disp.addr = VIOC_DISP_GetAddress(
			get_vioc_index(vioc->disp.id));
		#ifdef CONFIG_VOUT_USE_VSYNC_INT
		vioc->disp.vioc_intr = kzalloc(
			sizeof(struct vioc_intr_type), GFP_KERNEL);
		if (vioc->disp.vioc_intr == 0) {
			pr_err("[ERR][VOUT] memory alloc fail(vioc->disp)\n");
			return -ENOMEM;
		}
		vioc->disp.irq = irq_of_parse_and_map(dev_np,
			get_vioc_index(vioc->disp.id));
	...
	if (dev_np) {
		/* swreser bit */
		of_property_read_u32_index(vout->v4l2_np, "lastframe_rdma", 1,
			vioc->lastframe_rdma.id);
		vioc->lastframe_rdma.addr = VIOC_RDMA_GetAddress(
			get_vioc_index(vioc->lastframe_rdma.id));
...
int vout_vioc_set_default(struct tcc_vout_device *vout)
{
	...
	memcpy(&vout->pmap.name, VOUT_DISP_PATH_PMAP_NAME,
		sizeof(VOUT_DISP_PATH_PMAP_NAME));
	...
	if (vout->id == VOUT_MAIN) {
		...
		ret |= vout_set_m2m_dual_path(vout);
	} else {
		ret = vout_set_vout_path(vout);
		ret |= vout_set_m2m_path(1, vout);
	}
	...
	if (vout->vout_timer == NULL) {
		vout->vout_timer = tcc_register_timer(
			NULL, 1000/*msec*/, NULL);
...
void vout_disp_ctrl(struct tcc_vout_device *vout, int enable)
{
	struct tcc_vout_vioc *vioc = vout->vioc;
...
void vout_rdma_setup(struct tcc_vout_device *vout)
{
	...
	VIOC_RDMA_SetImageSize(disp_rdma->addr,
		disp_rdma->width, disp_rdma->height);
	VIOC_RDMA_SetImageFormat(disp_rdma->addr, disp_rdma->fmt);
	VIOC_RDMA_SetImageOffset(disp_rdma->addr,
		disp_rdma->fmt, disp_rdma->width);
	...
	if (vout->id == VOUT_MAIN) {
		...
		VIOC_RDMA_SetImageY2REnable(vioc->rdma_dual.addr,
			disp_rdma->y2r);
		VIOC_RDMA_SetImageDisable(vioc->rdma_dual.addr);
	}
#endif
...
void vout_wmix_setup(struct tcc_vout_device *vout)
{
	...
	#ifdef CONFIG_TCC_VIOCMG
	viocmg_set_wmix_ovp(VIOCMG_CALLERID_VOUT,
		get_vioc_index(vioc->wmix.id), vioc->wmix.ovp);
	#endif
...
void vout_wmix_getsize(
	struct tcc_vout_device *vout, unsigned int *w, unsigned int *h)
{
	struct tcc_vout_vioc *vioc = vout->vioc;
...
void m2m_dual_path_reset(struct tcc_vout_vioc *vioc, int m2m_dual_index)
{
	/* reset state */
	VIOC_CONFIG_SWReset(
		vioc->m2m_dual_wdma[m2m_dual_index].id, VIOC_CONFIG_RESET);
	VIOC_CONFIG_SWReset(
		vioc->m2m_dual_sc[m2m_dual_index].id, VIOC_CONFIG_RESET);
	VIOC_CONFIG_SWReset(
		vioc->m2m_dual_wmix[m2m_dual_index].id, VIOC_CONFIG_RESET);
	VIOC_CONFIG_SWReset(
		vioc->m2m_dual_rdma[m2m_dual_index].id, VIOC_CONFIG_RESET);

	/* normal state */
	VIOC_CONFIG_SWReset(
		vioc->m2m_dual_rdma[m2m_dual_index].id, VIOC_CONFIG_CLEAR);
	VIOC_CONFIG_SWReset(
		vioc->m2m_dual_wmix[m2m_dual_index].id, VIOC_CONFIG_CLEAR);
	VIOC_CONFIG_SWReset(
		vioc->m2m_dual_sc[m2m_dual_index].id, VIOC_CONFIG_CLEAR);
	VIOC_CONFIG_SWReset(
		vioc->m2m_dual_wdma[m2m_dual_index].id, VIOC_CONFIG_CLEAR);
}
#endif
...
void m2m_rdma_setup(struct vioc_rdma *rdma)
{
	...
	VIOC_RDMA_SetImageBase(rdma->addr, rdma->img.base0,
		rdma->img.base1, rdma->img.base2);
	VIOC_RDMA_SetImageSize(rdma->addr, rdma->width, rdma->height);
	VIOC_RDMA_SetImageFormat(rdma->addr, rdma->fmt);

	if (rdma->y_stride) {
		if (rdma->fmt > VIOC_IMG_FMT_ARGB6666_3)
			VIOC_RDMA_SetImageOffset(rdma->addr,
				rdma->fmt, rdma->y_stride);
		else
			VIOC_RDMA_SetImageOffset(rdma->addr,
				TCC_LCDC_IMG_FMT_RGB332, rdma->y_stride);
	} else {
		VIOC_RDMA_SetImageOffset(rdma->addr, rdma->fmt, rdma->width);
	}
}
...
static void m2m_wmix_setup(struct vioc_wmix *wmix)
{
	#ifdef CONFIG_TCC_VIOCMG
	viocmg_set_wmix_ovp(VIOCMG_CALLERID_VOUT,
		get_vioc_index(wmix->id), wmix->ovp);
	#else
...
static void m2m_scaler_setup(struct tcc_vout_device *vout, int m2m_dual_index)
{
	...
	VIOC_SC_SetDstSize(vioc->m2m_dual_sc[m2m_dual_index].addr,
		...
		vout->disp_rect.top < 0 ? dh + abs(vout->disp_rect.top) : dh);
	...
	VIOC_SC_SetOutSize(vioc->m2m_dual_sc[m2m_dual_index].addr, dw, dh);

	VIOC_CONFIG_PlugIn(vioc->m2m_dual_sc[m2m_dual_index].id,
		vioc->m2m_dual_rdma[m2m_dual_index].id);
...
static void scaler_setup(struct tcc_vout_device *vout)
{
	...
	sw = vout->src_pix.width == vout->crop_src.width ?
		 vout->src_pix.width : vout->crop_src.width;
	sh = vout->src_pix.height == vout->crop_src.height ?
		 vout->src_pix.height : vout->crop_src.height;
	dw = vout->disp_rect.width;
	dh = vout->disp_rect.height;
	...
	VIOC_SC_SetOutPosition(vioc->sc.addr,
		vout->disp_rect.left < 0 ? abs(vout->disp_rect.left) : 0,
		vout->disp_rect.top < 0 ? abs(vout->disp_rect.top) : 0);
	VIOC_SC_SetOutSize(vioc->sc.addr, dw, dh);

	if (vout->onthefly_mode)
		VIOC_CONFIG_PlugIn(vioc->sc.id, vioc->rdma.id);
	else
		VIOC_CONFIG_PlugIn(vioc->sc.id, vioc->m2m_wdma.id);
...
static void m2m_wdma_setup(struct vioc_wdma *wdma)
{
	...
	VIOC_WDMA_SetImageBase(wdma->addr, wdma->img.base0,
		wdma->img.base1, wdma->img.base2);
...
static int deintl_viqe_setup(
	struct tcc_vout_device *vout, enum deintl_type deinterlace, int plugin)
{
	struct tcc_vout_vioc *vioc = vout->vioc;
	...
	int vmisc_tsdu = 0;
	// 0: viqe size is get from vioc module
	VIOC_VIQE_DEINTL_MODE di_mode;
	VIOC_VIQE_FMT_TYPE di_fmt = VIOC_VIQE_FMT_YUV420;
	// DI_DECn_MISC.FMT, DI_FMT.F422 register
	...
	if (deinterlace == VOUT_DEINTL_VIQE_3D) {
		...
		dprintk("[PMAP] %s: 0x%08x ~ 0x%08x (0x%08x)\n",
			pmap_viqe.name, pmap_viqe.base,
			pmap_viqe.base + pmap_viqe.size, pmap_viqe.size);
		...
		if ((viqe_deintl_base[3] + img_size) >
	...
	VIOC_VIQE_SetControlRegister(vioc->viqe.addr,
		framebufferWidth, framebufferHeight, di_fmt);
	VIOC_VIQE_SetDeintlRegister(vioc->viqe.addr,
		di_fmt, vmisc_tsdu, framebufferWidth,
		framebufferHeight, di_mode,
		viqe_deintl_base[0], viqe_deintl_base[1],
		viqe_deintl_base[2], viqe_deintl_base[3]);
	...
	if (plugin) {
		if (vout->onthefly_mode)
			...
			ret = VIOC_CONFIG_PlugIn(vioc->viqe.id,
				vioc->m2m_rdma.id);
	}
...
static int vout_check_syncTime(
	struct tcc_vout_device *vout,
	struct v4l2_buffer *buf, unsigned long base_time)
{
	...
	current_time = ((buf->timestamp.tv_sec*1000)+
	...
	struct tcc_vout_device *vout, struct v4l2_buffer *buf)
{
	if (VIOC_CONFIG_DV_GET_EDR_PATH()) {
		...
		buf->m.planes[MPLANE_VID].reserved[VID_DOLBY_REG_OUT_TYPE]) {
		vioc_v_dv_prog(
		buf->m.planes[MPLANE_VID].reserved[VID_DOLBY_MD_HDMI_ADDR],
		...
		1);
			...
			VIOC_V_DV_SetSize(pDisp_DV, NULL, vout->disp_rect.left,
				vout->disp_rect.top, Hactive, Vactive);
			VIOC_V_DV_Turnon(pDisp_DV, NULL);
		} else {
			pr_err("[ERR][VOUT] %s-%d type mismatch(%d != %d)\n",
				__func__, __LINE__,
				vioc_get_out_type(),
				buf->m.planes[MPLANE_VID].reserved[74]);
				//VID_DOLBY_REG_OUT_TYPE = 74
		}
	}
}

void vout_onthefly_convert_video_info(
	struct tcc_vout_device *vout, struct v4l2_buffer *buf,
	struct tcc_lcdc_image_update *ImageInfo)
{
	...
	ImageInfo->Frame_width	=
		buf->m.planes[MPLANE_VID].reserved[VID_WIDTH];
	ImageInfo->Frame_height	=
		buf->m.planes[MPLANE_VID].reserved[VID_HEIGHT];
	...
	if ((ImageInfo->crop_left !=
		buf->m.planes[MPLANE_VID].reserved[VID_CROP_LEFT]
		|| ImageInfo->crop_top !=
		buf->m.planes[MPLANE_VID].reserved[VID_CROP_TOP]
		|| (ImageInfo->crop_right - ImageInfo->crop_left) !=
		buf->m.planes[MPLANE_VID].reserved[VID_CROP_WIDTH]
		|| (ImageInfo->crop_bottom - ImageInfo->crop_top) !=
		 buf->m.planes[MPLANE_VID].reserved[VID_CROP_HEIGHT])
		&& (buf->m.planes[MPLANE_VID].reserved[VID_CROP_WIDTH] &&
		buf->m.planes[MPLANE_VID].reserved[VID_CROP_HEIGHT])
		&& (ImageInfo->Frame_width >=
		buf->m.planes[MPLANE_VID].reserved[VID_CROP_WIDTH] &&
	...
	ImageInfo->private_data.optional_info[VID_OPT_HAVE_MC_INFO] =
		buf->m.planes[MPLANE_VID].reserved[VID_CONVERTER_EN];
	if (buf->m.planes[MPLANE_VID].reserved[VID_CONVERTER_EN] == 1) {
{
	...
	ImageInfo->private_data.mapConv_info.m_uiLumaStride =
		buf->m.planes[MPLANE_VID].reserved[VID_HEVC_LUMA_STRIDE];
	ImageInfo->private_data.mapConv_info.m_uiChromaStride =
		buf->m.planes[MPLANE_VID].reserved[VID_HEVC_CHROMA_STRIDE];
	ImageInfo->private_data.mapConv_info.m_uiLumaBitDepth =
		buf->m.planes[MPLANE_VID].reserved[VID_HEVC_LUMA_BIT_DEPTH];
	ImageInfo->private_data.mapConv_info.m_uiChromaBitDepth =
		buf->m.planes[MPLANE_VID].reserved[VID_HEVC_CHROMA_BIT_DEPTH];
	ImageInfo->private_data.mapConv_info.m_uiFrameEndian =
		buf->m.planes[MPLANE_VID].reserved[VID_HEVC_FRMAE_ENDIAN];

	ImageInfo->private_data.mapConv_info.m_CompressedY[0] =
		buf->m.planes[MPLANE_VID].reserved[VID_HEVC_FRAME_BASE_Y0];
	ImageInfo->private_data.mapConv_info.m_CompressedY[1] =
		buf->m.planes[MPLANE_VID].reserved[VID_HEVC_FRAME_BASE_Y1];
	ImageInfo->private_data.mapConv_info.m_CompressedCb[0] =
		buf->m.planes[MPLANE_VID].reserved[VID_HEVC_FRAME_BASE_C0];
	ImageInfo->private_data.mapConv_info.m_CompressedCb[1] =
		buf->m.planes[MPLANE_VID].reserved[VID_HEVC_FRAME_BASE_C1];

	ImageInfo->private_data.mapConv_info.m_FbcYOffsetAddr[0] =
		buf->m.planes[MPLANE_VID].reserved[VID_HEVC_OFFSET_BASE_Y0];
	ImageInfo->private_data.mapConv_info.m_FbcYOffsetAddr[1] =
		buf->m.planes[MPLANE_VID].reserved[VID_HEVC_OFFSET_BASE_Y1];
	ImageInfo->private_data.mapConv_info.m_FbcCOffsetAddr[0] =
		buf->m.planes[MPLANE_VID].reserved[VID_HEVC_OFFSET_BASE_C0];
	ImageInfo->private_data.mapConv_info.m_FbcCOffsetAddr[1] =
		buf->m.planes[MPLANE_VID].reserved[VID_HEVC_OFFSET_BASE_C1];
}
	...
	dvprintk("^%s:%d^^^^^^ Disp-Region(%d,%d ~ %dx%d) ^^^^^^^ @@@ EL Info(0x%x-0x%x-0x%x, %dx%d (%dx%d),%d bpp) #Reg(0x%x) #Meta(0x%x)\n",
		...
		buf->m.planes[MPLANE_VID].reserved[VID_DOLBY_EL_OFFSET0],
		buf->m.planes[MPLANE_VID].reserved[VID_DOLBY_EL_OFFSET1],
		buf->m.planes[MPLANE_VID].reserved[VID_DOLBY_EL_OFFSET2],
		buf->m.planes[MPLANE_VID].reserved[VID_DOLBY_EL_FRAME_WIDTH],
		buf->m.planes[MPLANE_VID].reserved[VID_DOLBY_EL_FRAME_HEIGHT],
		buf->m.planes[MPLANE_VID].reserved[VID_DOLBY_EL_BUFF_WIDTH],
		buf->m.planes[MPLANE_VID].reserved[VID_DOLBY_EL_BUFF_HEIGHT],
		ImageInfo->private_data.optional_info[VID_OPT_BIT_DEPTH],
		buf->m.planes[MPLANE_VID].reserved[VID_DOLBY_REG_ADDR],
		buf->m.planes[MPLANE_VID].reserved[VID_DOLBY_MD_HDMI_ADDR]);

	ImageInfo->fmt =
		ImageInfo->private_data.format =
		buf->m.planes[MPLANE_VID].reserved[VID_MJPEG_FORMAT] ==
		DEC_FMT_420IL0 ? TCC_LCDC_IMG_FMT_YUV420ITL0 :
		TCC_LCDC_IMG_FMT_YUV420SP;//vioc->rdma.fmt;
	ImageInfo->addr0 = ImageInfo->private_data.offset[0]		=
		buf->m.planes[MPLANE_VID].m.userptr;
	ImageInfo->addr1 = ImageInfo->private_data.offset[1]		=
		buf->m.planes[MPLANE_VID].reserved[VID_BASE1];
	ImageInfo->addr2 = ImageInfo->private_data.offset[2]		=
		buf->m.planes[MPLANE_VID].reserved[VID_BASE2];
	...
	ImageInfo->private_data.optional_info[VID_OPT_HAVE_DOLBYVISION_INFO] =
		buf->m.planes[MPLANE_VID].reserved[VID_DOLBY_EN];
...
void vout_edr_certification_display(
	struct tcc_vout_device *vout, struct v4l2_buffer *buf)
{
	struct tcc_lcdc_image_update ImageInfo;
	struct tcc_vout_vioc *vioc = vout->vioc;
	...
	unsigned long current_time =
		((buf->timestamp.tv_sec*1000)+(buf->timestamp.tv_usec/1000));
	...
	if (ImageInfo.private_data.optional_info[VID_OPT_FRAME_WIDTH] == 0 ||
	ImageInfo.private_data.optional_info[VID_OPT_FRAME_HEIGHT] == 0) {
		ImageInfo.private_data.optional_info[VID_OPT_FRAME_WIDTH]  =
			bl_w;
		ImageInfo.private_data.optional_info[VID_OPT_FRAME_HEIGHT] =
			bl_h;
	...
	if (ImageInfo.private_data.dolbyVision_info.el_frame_width == 0 ||
		ImageInfo.private_data.dolbyVision_info.el_frame_height == 0) {
	...
	ImageInfo.private_data.dolbyVision_info.osd_addr[0] =
		buf->m.planes[MPLANE_VID].reserved[VID_DOLBY_OSD_OFFSET0];
	...
	if (nIdx_copy < 120) {//nTS_Prev != current_time) {
		...
		ImageInfo.private_data.optional_info[VID_OPT_PLAYER_IDX] =
			atomic_read(&vout->readable_buff_count);
		memcpy(&nCopy_ImageInfo[nIdx_copy], &ImageInfo,
...
void vout_onthefly_display_update(
	struct tcc_vout_device *vout, struct v4l2_buffer *buf)
{
	struct tcc_vout_vioc *vioc = vout->vioc;
	...
	if (buf->m.planes[MPLANE_VID].reserved[VID_NUM] == 2) {
		memcpy(&vioc->subplane_alpha,
			buf->m.planes[MPLANE_SUB].reserved,
			sizeof(struct vioc_alpha));
		vioc->subplane_rdma.img.base0 =
			buf->m.planes[MPLANE_SUB].m.mem_offset;
...

{
	 ...
	 * WARNING: line over 80 characters
	 */
	drmparm.mInfoFrame.version =
		buf->m.planes[MPLANE_VID].reserved[VID_HDR_VERSION];
	drmparm.mInfoFrame.length =
		buf->m.planes[MPLANE_VID].reserved[VID_HDR_STRUCT_SIZE];

	drmparm.mDescriptor_type1.EOTF = 2;
	//buf->m.planes[MPLANE_VID].reserved[VID_HDR_EOTF];
	drmparm.mDescriptor_type1.Descriptor_ID = 0;
	//buf->m.planes[MPLANE_VID].reserved[VID_HDR_DESCRIPTOR_ID];
	drmparm.mDescriptor_type1.disp_primaries_x[0] =
		buf->m.planes[MPLANE_VID].reserved[VID_HDR_PRIMARY_X0];
	drmparm.mDescriptor_type1.disp_primaries_x[1] =
		buf->m.planes[MPLANE_VID].reserved[VID_HDR_PRIMARY_X1];
	drmparm.mDescriptor_type1.disp_primaries_x[2] =
		buf->m.planes[MPLANE_VID].reserved[VID_HDR_PRIMARY_X2];
	drmparm.mDescriptor_type1.disp_primaries_y[0] =
		buf->m.planes[MPLANE_VID].reserved[VID_HDR_PRIMARY_Y0];
	drmparm.mDescriptor_type1.disp_primaries_y[1] =
		buf->m.planes[MPLANE_VID].reserved[VID_HDR_PRIMARY_Y1];
	drmparm.mDescriptor_type1.disp_primaries_y[2] =
		buf->m.planes[MPLANE_VID].reserved[VID_HDR_PRIMARY_Y2];
	drmparm.mDescriptor_type1.white_point_x =
		buf->m.planes[MPLANE_VID].reserved[VID_HDR_WPOINT_X];
	drmparm.mDescriptor_type1.white_point_y =
		buf->m.planes[MPLANE_VID].reserved[VID_HDR_WPOINT_Y];
	drmparm.mDescriptor_type1.max_disp_mastering_luminance =
		buf->m.planes[MPLANE_VID].reserved[VID_HDR_MAX_LUMINANCE];
	drmparm.mDescriptor_type1.min_disp_mastering_luminance =
		buf->m.planes[MPLANE_VID].reserved[VID_HDR_MIN_LUMINANCE];
	drmparm.mDescriptor_type1.max_content_light_level =
		buf->m.planes[MPLANE_VID].reserved[VID_HDR_MAX_CONTENT];
	drmparm.mDescriptor_type1.max_frame_avr_light_level =
		buf->m.planes[MPLANE_VID].reserved[VID_HDR_MAX_PIC_AVR];
}
	...
	if (((vout->src_pix.width !=
		...
		vout->src_pix.bytesperline =
			vout->src_pix.width * tcc_vout_try_bpp(
				vout->src_pix.pixelformat,
		...
		vioc->rdma.y_stride = buf->m.planes[MPLANE_VID].bytesused ?
			buf->m.planes[MPLANE_VID].bytesused :
			vout->src_pix.bytesperline && 0x0000FFFF;
		VIOC_RDMA_SetImageSize(
			vioc->rdma.addr, vioc->rdma.width, vioc->rdma.height);
		VIOC_RDMA_SetImageOffset(
			vioc->rdma.addr, vioc->rdma.fmt, vioc->rdma.y_stride);
	 ...
	 *	Change crop info
	 */
	if ((vout->crop_src.left !=
		buf->m.planes[MPLANE_VID].reserved[VID_CROP_LEFT]
		|| vout->crop_src.top !=
		buf->m.planes[MPLANE_VID].reserved[VID_CROP_TOP]
		|| vout->crop_src.width !=
		buf->m.planes[MPLANE_VID].reserved[VID_CROP_WIDTH]
		|| vout->crop_src.height !=
		buf->m.planes[MPLANE_VID].reserved[VID_CROP_HEIGHT])
		&& (buf->m.planes[MPLANE_VID].reserved[VID_CROP_WIDTH]
		&& buf->m.planes[MPLANE_VID].reserved[VID_CROP_HEIGHT])
		&& ((vout->src_pix.width >=
		...
		tcc_get_base_address(vioc->rdma.fmt, base0,
			vioc->rdma.y_stride ? vioc->rdma.y_stride :
			vioc->rdma.width,
			vioc->rdma.height,
		...
		VIOC_RDMA_SetImageSize(vioc->rdma.addr,
			vioc->rdma.width, vioc->rdma.height);
		res_change = ON;
	} else if ((vout->src_pix.width != vout->crop_src.width)
		|| (vout->src_pix.height != vout->crop_src.height)) {
	...
	/* rdma stride */
	if (buf->m.planes[MPLANE_VID].bytesused
		&& (vioc->rdma.y_stride !=
		buf->m.planes[MPLANE_VID].bytesused)) {
	...
	if (buf->m.planes[MPLANE_VID].reserved[VID_CONVERTER_EN] == 1) {
		...
		if ((vout->deinterlace == VOUT_DEINTL_VIQE_3D)
			...
			VIOC_CONFIG_SWReset(
				vioc->deintl_s.id, VIOC_CONFIG_RESET);
			VIOC_CONFIG_SWReset(
				vioc->deintl_s.id, VIOC_CONFIG_CLEAR);
		}
		...
		if (VIOC_CONFIG_DMAPath_Support()) {
			...
			if (VIOC_CONFIG_MCPath(vioc->wmix.id, VIOC_MC0) < 0) {
				pr_err("[ERR][VOUT] %s[%d]:
				HW Decompresser can not be connected on %s\n",
		...
		/* update map_converter info */
{
	 ...
	 * WARNING: line over 80 characters
	 */
	vioc->map_converter.mapConv_info.m_uiLumaStride =
		buf->m.planes[MPLANE_VID].reserved[VID_HEVC_LUMA_STRIDE];
	vioc->map_converter.mapConv_info.m_uiChromaStride =
		buf->m.planes[MPLANE_VID].reserved[VID_HEVC_CHROMA_STRIDE];
	vioc->map_converter.mapConv_info.m_uiLumaBitDepth =
		buf->m.planes[MPLANE_VID].reserved[VID_HEVC_LUMA_BIT_DEPTH];
	vioc->map_converter.mapConv_info.m_uiChromaBitDepth =
		buf->m.planes[MPLANE_VID].reserved[VID_HEVC_CHROMA_BIT_DEPTH];
	vioc->map_converter.mapConv_info.m_uiFrameEndian =
		buf->m.planes[MPLANE_VID].reserved[VID_HEVC_FRMAE_ENDIAN];

	vioc->map_converter.mapConv_info.m_CompressedY[0] =
		buf->m.planes[MPLANE_VID].reserved[VID_HEVC_FRAME_BASE_Y0];
	vioc->map_converter.mapConv_info.m_CompressedY[1] =
		buf->m.planes[MPLANE_VID].reserved[VID_HEVC_FRAME_BASE_Y1];
	vioc->map_converter.mapConv_info.m_CompressedCb[0] =
		buf->m.planes[MPLANE_VID].reserved[VID_HEVC_FRAME_BASE_C0];
	vioc->map_converter.mapConv_info.m_CompressedCb[1] =
		buf->m.planes[MPLANE_VID].reserved[VID_HEVC_FRAME_BASE_C1];

	vioc->map_converter.mapConv_info.m_FbcYOffsetAddr[0] =
		buf->m.planes[MPLANE_VID].reserved[VID_HEVC_OFFSET_BASE_Y0];
	vioc->map_converter.mapConv_info.m_FbcYOffsetAddr[1] =
		buf->m.planes[MPLANE_VID].reserved[VID_HEVC_OFFSET_BASE_Y1];
	vioc->map_converter.mapConv_info.m_FbcCOffsetAddr[0] =
		buf->m.planes[MPLANE_VID].reserved[VID_HEVC_OFFSET_BASE_C0];
	vioc->map_converter.mapConv_info.m_FbcCOffsetAddr[1] =
		buf->m.planes[MPLANE_VID].reserved[VID_HEVC_OFFSET_BASE_C1];
}

		if (VIOC_CONFIG_GetScaler_PluginToRDMA(vioc->rdma.id) !=
			vioc->sc.id)
			scaler_setup(vout);
		...
		#if defined(CONFIG_ARCH_TCC803X) || defined(CONFIG_ARCH_TCC805X)
		tca_map_convter_driver_set(VIOC_MC0, vout->src_pix.width,
			vout->src_pix.height, vout->crop_src.left,
			vout->crop_src.top,
			vout->crop_src.width, vout->crop_src.height,
			bY2R, &vioc->map_converter.mapConv_info);
		tca_map_convter_onoff(VIOC_MC0, 1, 0);
		#else
		tca_map_convter_driver_set(vioc->map_converter.id,
			vout->src_pix.width, vout->src_pix.height,
			vout->crop_src.left, vout->crop_src.top,
			vout->crop_src.width, vout->crop_src.height,
			bY2R, &vioc->map_converter.mapConv_info);
		...
		if (VIOC_CONFIG_DMAPath_Support()) {
			if (VIOC_CONFIG_DMAPath_Select(vioc->rdma.id) !=
				...
				|| (vout->deinterlace == VOUT_DEINTL_VIQE_2D)) {
					VIOC_CONFIG_PlugOut(vioc->viqe.id);
					VIOC_CONFIG_SWReset(vioc->viqe.id,
						VIOC_CONFIG_RESET);
					VIOC_CONFIG_SWReset(vioc->viqe.id,
						VIOC_CONFIG_CLEAR);
				} else if (vout->deinterlace == VOUT_DEINTL_S) {
					VIOC_CONFIG_PlugOut(vioc->deintl_s.id);
					VIOC_CONFIG_SWReset(vioc->deintl_s.id,
						VIOC_CONFIG_RESET);
					VIOC_CONFIG_SWReset(vioc->deintl_s.id,
						VIOC_CONFIG_CLEAR);
				}
				...
				VIOC_CONFIG_PlugOut(vioc->sc.id);
				VIOC_CONFIG_SWReset(vioc->sc.id,
					VIOC_CONFIG_RESET);
				VIOC_CONFIG_SWReset(vioc->sc.id,
					VIOC_CONFIG_CLEAR);

				dprintk("___dtrc converter connect to RDMA%d\n",
					VIOC_RDMA_03);
				VIOC_CONFIG_DMAPath_UnSet(vioc->rdma.id);
				tca_dtrc_convter_swreset(vioc->dtrc.id);
				VIOC_CONFIG_DMAPath_Set(vioc->rdma.id,
					vioc->dtrc.id);
			}

{
	...
	/* update dtrc info */
	vioc->dtrc.dtrcConv_info.m_CompressedY[0] =
		buf->m.planes[MPLANE_VID].reserved[VID_DTRC_FRAME_BASE_Y0];
	vioc->dtrc.dtrcConv_info.m_CompressedY[1] =
		buf->m.planes[MPLANE_VID].reserved[VID_DTRC_FRAME_BASE_Y1];
	vioc->dtrc.dtrcConv_info.m_CompressedCb[0] =
		buf->m.planes[MPLANE_VID].reserved[VID_DTRC_FRAME_BASE_C0];
	vioc->dtrc.dtrcConv_info.m_CompressedCb[1] =
		buf->m.planes[MPLANE_VID].reserved[VID_DTRC_FRAME_BASE_C1];
	vioc->dtrc.dtrcConv_info.m_CompressionTableLuma[0] =
		buf->m.planes[MPLANE_VID].reserved[VID_DTRC_TABLE_LUMA0];
	vioc->dtrc.dtrcConv_info.m_CompressionTableLuma[1] =
		buf->m.planes[MPLANE_VID].reserved[VID_DTRC_TABLE_LUMA1];
	vioc->dtrc.dtrcConv_info.m_CompressionTableChroma[0] =
		buf->m.planes[MPLANE_VID].reserved[VID_DTRC_TABLE_CHROMA0];
	vioc->dtrc.dtrcConv_info.m_CompressionTableChroma[1] =
		buf->m.planes[MPLANE_VID].reserved[VID_DTRC_TABLE_CHROMA1];
	vioc->dtrc.dtrcConv_info.m_iCompressionTableLumaSize =
		buf->m.planes[MPLANE_VID].reserved[VID_DTRC_LUMA_TABLE_SIZE];
	vioc->dtrc.dtrcConv_info.m_iCompressionTableChromaSize =
		buf->m.planes[MPLANE_VID].reserved[VID_DTRC_CHROMA_TABLE_SIZE];
	vioc->dtrc.dtrcConv_info.m_iBitDepthLuma =
		buf->m.planes[MPLANE_VID].reserved[VID_DTRC_LUMA_BIT_DEPTH];
	vioc->dtrc.dtrcConv_info.m_iBitDepthChroma =
		buf->m.planes[MPLANE_VID].reserved[VID_DTRC_CHROMA_BIT_DEPTH];
	vioc->dtrc.dtrcConv_info.m_iHeight =
		buf->m.planes[MPLANE_VID].reserved[VID_DTRC_FRAME_HEIGHT];
	vioc->dtrc.dtrcConv_info.m_iWidth =
		buf->m.planes[MPLANE_VID].reserved[VID_DTRC_FRAME_WIDTH];
	vioc->dtrc.dtrcConv_info.m_iPicStride =
		buf->m.planes[MPLANE_VID].reserved[VID_DTRC_FRAME_STRIDE];
}

			if (VIOC_CONFIG_GetScaler_PluginToRDMA(
				vioc->rdma.id) != vioc->sc.id)
				scaler_setup(vout);
			...
			VIOC_SC_SetUpdate(vioc->sc.addr);
			tca_dtrc_convter_driver_set(vioc->dtrc.id,
				vout->crop_src.width, vout->crop_src.height,
				vout->crop_src.left, vout->crop_src.top, bY2R,
				&vioc->dtrc.dtrcConv_info);
			...
			pr_err("[ERR][VOUT] Not support for
				DTRC on this chipset type!!\n");
		}
		#endif
	} else {
		...
		if (VIOC_CONFIG_DMAPath_Support()) {
			int component_num =
				VIOC_CONFIG_DMAPath_Select(vioc->rdma.id);
		...
		/* dec output format */
		if (vout->memory == V4L2_MEMORY_USERPTR) {
			vout_check_format(&vioc->rdma,
			buf->m.planes[MPLANE_VID].reserved[VID_MJPEG_FORMAT]);
		}

		#ifdef CONFIG_VIOC_10BIT
		if (buf->flags & V4L2_BUFFER_FLAG_YUV_10BIT_MODE0) {
			VIOC_RDMA_SetDataFormat(vioc->rdma.addr, 0x1, 0x1);
			/* YUV 10bit support(16bit format type) */
		} else if (buf->flags & V4L2_BUFFER_FLAG_YUV_10BIT_MODE1) {
			VIOC_RDMA_SetDataFormat(vioc->rdma.addr, 0x3, 0x0);
			/* YUV 10bit support(10bit format type) */
			VIOC_RDMA_SetImageOffset(vioc->rdma.addr,
				vioc->rdma.fmt, (vioc->rdma.width*125)/100);
		} else {
			VIOC_RDMA_SetDataFormat(vioc->rdma.addr, 0x0, 0x0);
			/* YUV 8bit support */
		}
		#endif
		...
		if (res_change) {
			...
			if (vioc->wmix.width == vioc->rdma.width
				&& vioc->wmix.height == vioc->rdma.height) {
				dprintk("BYPASS   src(%dx%d) -> dst(%dx%d)\n",
					vioc->rdma.width, vioc->rdma.height,
					vioc->wmix.width, vioc->wmix.height);
				VIOC_SC_SetBypass(vioc->sc.addr, ON);
				if (VIOC_CONFIG_GetScaler_PluginToRDMA(
					vioc->rdma.id) != vioc->sc.id)
					VIOC_CONFIG_PlugIn(
						vioc->sc.id, vioc->rdma.id);
				VIOC_SC_SetUpdate(vioc->sc.addr);
			} else {
				...
				VIOC_SC_SetDstSize(vioc->sc.addr,
					vioc->wmix.width, vioc->wmix.height);
				VIOC_SC_SetOutSize(vioc->sc.addr,
					vioc->wmix.width, vioc->wmix.height);
				if (VIOC_CONFIG_GetScaler_PluginToRDMA(
					vioc->rdma.id) != vioc->sc.id)
					VIOC_CONFIG_PlugIn(
		...
		if (vout->deinterlace == VOUT_DEINTL_VIQE_3D
			...
			if (viocmg_lock_viqe(VIOCMG_CALLERID_VOUT) < 0) {
				...
				VIOC_CONFIG_Device_PlugState(
					vioc->viqe.id, &plugin_state);
				if (!plugin_state.enable ||
					plugin_state.connect_statue !=
					VIOC_PATH_CONNECTED) {
					if (deintl_viqe_setup(vout,
						vout->deinterlace, 1) < 0)
		...
		#ifdef CONFIG_TCC_VIOCMG
		if (res_change || force_process || vout->previous_field !=
			vout->src_pix.field)
		#else
			...
			|| vout->src_pix.field == V4L2_FIELD_INTERLACED) {
				...
				case VOUT_DEINTL_VIQE_2D:
					if (res_change) {
						...
						VIOC_VIQE_SetDeintlMode(
						vioc->viqe.addr,
						VIOC_VIQE_DEINTL_MODE_2D);
						VIOC_VIQE_SetControlMode(
							vioc->viqe.addr,
							OFF, OFF, OFF, OFF, ON);
					}

					if (vout->src_pix.colorspace ==
				...
				case VOUT_DEINTL_VIQE_2D:
					VIOC_VIQE_SetDeintlMode(
						vioc->viqe.addr,
						VIOC_VIQE_DEINTL_MODE_BYPASS);
					VIOC_VIQE_SetControlMode(
						vioc->viqe.addr, OFF, OFF,
						OFF, OFF, OFF);
					vout->frame_count = 0;
	...
	if (vout->src_pix.field == V4L2_FIELD_INTERLACED_TB
		...
		if ((vout->deinterlace == VOUT_DEINTL_VIQE_3D)
			...
			if (vout->frame_count >= 3) {
				...
				int deintl_judder_cnt =
					(vioc->rdma.width + 64) / 64 - 1;

				VIOC_VIQE_SetDeintlJudderCnt(
					vioc->viqe.addr,
					deintl_judder_cnt);
			} else {
				VIOC_VIQE_SetDeintlMode(
	...
	struct tcc_vout_device *vout, struct v4l2_buffer *buf)
{
	...
	if (buf->m.planes[MPLANE_VID].reserved[VID_NUM] == 2) {
		...
		buf->m.planes[MPLANE_SUB].reserved,
		sizeof(struct vioc_alpha));
		vioc->m2m_subplane_rdma.img.base0 =
			buf->m.planes[MPLANE_SUB].m.mem_offset;
	...
	if (vout->memory  == V4L2_MEMORY_USERPTR) {
		if (buf->timecode.type != STILL_IMGAGE) {
			...
			if (vout->pfmt != TCC_PFMT_RGB) {
				/*
				 * Re-store src_pix.height from
				 * "The VIQE need 4-line align"
				 */
				if (vioc->m2m_rdma.height !=
					vout->src_pix.height) {
	...
	if ((vout->src_pix.width !=
		...
		if (vout->src_pix.height % 4) {
			vioc->m2m_rdma.height =
				ROUND_UP_4(vout->src_pix.height);
			dprintk(" 4-line align: %d -> %d\n",
				vout->src_pix.height, vioc->m2m_rdma.height);
		} else {
			vioc->m2m_rdma.height = vout->src_pix.height;
		}
		vioc->m2m_rdma.y_stride =
			buf->m.planes[MPLANE_VID].bytesused ?
			buf->m.planes[MPLANE_VID].bytesused :
			vout->src_pix.bytesperline && 0x0000FFFF;
		VIOC_RDMA_SetImageSize(vioc->m2m_rdma.addr,
			vioc->m2m_rdma.width, vioc->m2m_rdma.height);
		VIOC_RDMA_SetImageOffset(vioc->m2m_rdma.addr,
			vioc->m2m_rdma.fmt, vioc->m2m_rdma.y_stride);

		vioc->m2m_wmix.width = vioc->m2m_rdma.width;
		vioc->m2m_wmix.height = vioc->m2m_rdma.height;
		VIOC_WMIX_SetSize(vioc->m2m_wmix.addr,
			vioc->m2m_wmix.width, vioc->m2m_wmix.height);
	...
	if ((vout->crop_src.left !=
		...
		tcc_get_base_address(vioc->m2m_rdma.fmt, base0,
			vioc->m2m_rdma.y_stride ? vioc->m2m_rdma.y_stride :
			vioc->m2m_rdma.width,
			vioc->m2m_rdma.height,
			vout->crop_src.left, vout->crop_src.top,
			&base0, &base1, &base2);
		...
		if (vout->crop_src.height % 4) {
			vioc->m2m_rdma.height =
				ROUND_UP_4(vout->crop_src.height);
			dprintk(" 4-line align: %d -> %d\n",
	...
	if (res_change) {
		...
		if (vioc->m2m_wmix.width == vioc->m2m_wdma.width
			&& vioc->m2m_wmix.height == vioc->m2m_wdma.height) {
			dprintk("BYPASS         src(%dx%d) -> dst(%dx%d)\n",
				vioc->m2m_wmix.width, vioc->m2m_wmix.height,
				vioc->m2m_wdma.width, vioc->m2m_wdma.height);
			VIOC_SC_SetBypass(vioc->sc.addr, ON);
			VIOC_SC_SetUpdate(vioc->sc.addr);
		} else {
			dprintk("SCALING        src(%dx%d) -> dst(%dx%d)\n",
	...
	if (vout->id == VOUT_MAIN) {
		...
		VIOC_WDMA_SetImageSize(vioc->m2m_wdma.addr,
			vioc->m2m_wdma.width, vioc->m2m_wdma.height);
		VIOC_WDMA_SetImageOffset(vioc->m2m_wdma.addr,
		vioc->m2m_wdma.fmt, vioc->m2m_wdma.width);
	...
	/* dec output format */
	if (vout->memory == V4L2_MEMORY_USERPTR)
		vout_check_format(&vioc->m2m_rdma,
		buf->m.planes[MPLANE_VID].reserved[VID_MJPEG_FORMAT]);

	#ifdef CONFIG_VIOC_10BIT
	if (buf->flags & V4L2_BUFFER_FLAG_YUV_10BIT_MODE0) {
		VIOC_RDMA_SetDataFormat(vioc->m2m_rdma.addr, 0x1, 0x1);
		/* YUV 10bit support(16bit format type) */
	} else if (buf->flags & V4L2_BUFFER_FLAG_YUV_10BIT_MODE1) {
		VIOC_RDMA_SetDataFormat(vioc->m2m_rdma.addr, 0x3, 0x0);
		/* YUV 10bit support(10bit format type) */
		VIOC_RDMA_SetImageOffset(vioc->m2m_rdma.addr,
			vioc->m2m_rdma.fmt, (vioc->m2m_rdma.width*125)/100);
	} else {
		VIOC_RDMA_SetDataFormat(vioc->m2m_rdma.addr, 0x0, 0x0);
		/* YUV 8bit support */
	}
	#endif
	...
	if (vout->id == VOUT_MAIN) {
		vioc->m2m_dual_wdma[M2M_DUAL_0].img.base0 =
			vout->m2m_dual_bufs[index].img_base0;
		vioc->m2m_dual_wdma[M2M_DUAL_0].img.base1 =
			vout->m2m_dual_bufs[index].img_base1;
		vioc->m2m_dual_wdma[M2M_DUAL_0].img.base2 =
			vout->m2m_dual_bufs[index].img_base2;
		vioc->m2m_dual_wdma[M2M_DUAL_1].img.base0 =
			vout->m2m_dual_bufs_hdmi[index].img_base0;
		vioc->m2m_dual_wdma[M2M_DUAL_1].img.base1 =
			vout->m2m_dual_bufs_hdmi[index].img_base1;
		vioc->m2m_dual_wdma[M2M_DUAL_1].img.base2 =
			vout->m2m_dual_bufs_hdmi[index].img_base2;
	}
#endif
	if (buf->m.planes[MPLANE_VID].reserved[VID_CONVERTER_EN] == 1) {
		#ifdef CONFIG_VIOC_MAP_DECOMP
		...
		VIOC_CONFIG_SWReset(vioc->sc.id, VIOC_CONFIG_CLEAR);
		if (VIOC_CONFIG_DMAPath_Support()) {
			if (VIOC_CONFIG_DMAPath_Select(
				vioc->m2m_rdma.id) != vioc->map_converter.id) {
			...
			if (VIOC_CONFIG_MCPath(
				vioc->m2m_wmix.id, VIOC_MC1) < 0) {
...

{
	...
	vioc->map_converter.mapConv_info.m_FbcCOffsetAddr[1] =
		...
		tca_map_convter_driver_set(
			VIOC_MC1, vout->src_pix.width, vout->src_pix.height,
			vout->crop_src.left, vout->crop_src.top,
		...
		VIOC_WDMA_SetImageBase(vioc->m2m_wdma.addr,
			vioc->m2m_wdma.img.base0,
			vioc->m2m_wdma.img.base1, vioc->m2m_wdma.img.base2);
		...
		if (VIOC_CONFIG_DMAPath_Support()) {
			if (VIOC_CONFIG_DMAPath_Select(
				...
				VIOC_CONFIG_DMAPath_UnSet(vioc->m2m_rdma.id);
				tca_dtrc_convter_swreset(vioc->dtrc.id);
				VIOC_CONFIG_DMAPath_Set(vioc->m2m_rdma.id,
					vioc->dtrc.id);
			}
	...
	vioc->dtrc.dtrcConv_info.m_iPicStride =
		buf->m.planes[MPLANE_VID].reserved[VID_DTRC_FRAME_STRIDE];
			...
			tca_dtrc_convter_driver_set(vioc->dtrc.id,
				vout->crop_src.width, vout->crop_src.height,
					vout->crop_src.left, vout->crop_src.top,
					ON, &vioc->dtrc.dtrcConv_info);
			tca_dtrc_convter_onoff(vioc->dtrc.id, ON, 0);

			if (VIOC_CONFIG_GetScaler_PluginToRDMA(
				vioc->m2m_rdma.id) != vioc->sc.id)
				scaler_setup(vout);

			/* update wdma base address */
			VIOC_WDMA_SetImageBase(vioc->m2m_wdma.addr,
				vioc->m2m_wdma.img.base0,
				vioc->m2m_wdma.img.base1,
				vioc->m2m_wdma.img.base2);
			vout_m2m_ctrl(vioc, 1);
		} else {
		...
		if (VIOC_CONFIG_DMAPath_Support()) {
			int component_num =
				VIOC_CONFIG_DMAPath_Select(vioc->m2m_rdma.id);
			...
			#if defined(CONFIG_ARCH_TCC803X) || defined(CONFIG_ARCH_TCC805X)
			VIOC_CONFIG_MCPath(
				vioc->m2m_wmix.id, vioc->m2m_rdma.id);
			#endif
		}
		#endif
		...
		if (vout->deinterlace == VOUT_DEINTL_VIQE_3D
			...
			if (viocmg_lock_viqe(VIOCMG_CALLERID_VOUT) < 0) {
				...
				== V4L2_FIELD_INTERLACED) {
					...
					previous_deinterlace =
						vout->deinterlace;
					vout->deinterlace = VOUT_DEINTL_S;

					// force disable wdma r2y
					VIOC_WDMA_SetImageR2YEnable(
						vioc->m2m_wdma.addr, OFF);
				...
				#ifndef CONFIG_VOUT_USE_VSYNC_INT
				vout_clear_buffer(vout,
					vout->last_cleared_buffer);
				#endif
				return;
				#endif
			} else {
		...
		VIOC_RDMA_SetImageBfield(
			vioc->m2m_rdma.addr, vioc->m2m_rdma.bf);
		VIOC_RDMA_SetImageBase(vioc->m2m_rdma.addr,
			vioc->m2m_rdma.img.base0,
			vioc->m2m_rdma.img.base1,
			vioc->m2m_rdma.img.base2);
...
next_field:
		if (wait_event_interruptible_timeout(
			vout->frame_wait, vout->wakeup_int == 1,
		...
		if (vout->firstFieldFlag) {
			...
			/* wdma base address */
			vioc->m2m_wdma.img.base0 =
				vout->deintl_bufs[index].img_base0;
			vioc->m2m_wdma.img.base1 =
				vout->deintl_bufs[index].img_base1;
			vioc->m2m_wdma.img.base2 =
				vout->deintl_bufs[index].img_base2;
			VIOC_WDMA_SetImageBase(
				vioc->m2m_wdma.addr, vioc->m2m_wdma.img.base0,
				vioc->m2m_wdma.img.base1,
				vioc->m2m_wdma.img.base2);

			/* change rdma bfield */
			VIOC_RDMA_SetImageBfield(vioc->m2m_rdma.addr,
				!(vioc->m2m_rdma.bf));
	...
	if (vout->id == VOUT_MAIN) {
		...
		VIOC_SC_SetDstSize(vioc->m2m_dual_sc[M2M_DUAL_0].addr,
			vout->disp_rect.left < 0 ? vout->disp_rect.width
			+ abs(vout->disp_rect.left) : dw,
			vout->disp_rect.top < 0 ? vout->disp_rect.height
			+ abs(vout->disp_rect.top) : dh);
		VIOC_SC_SetOutPosition(vioc->m2m_dual_sc[M2M_DUAL_0].addr,
			vout->disp_rect.left < 0 ? abs(vout->disp_rect.left) : 0,
			vout->disp_rect.top < 0 ? abs(vout->disp_rect.top) : 0);
		VIOC_SC_SetOutSize(vioc->m2m_dual_sc[M2M_DUAL_0].addr, dw, dh);
		...
		VIOC_WDMA_SetImageFormat(vioc->m2m_dual_wdma[M2M_DUAL_0].addr,
			vioc->m2m_wdma.fmt);
		VIOC_WDMA_SetImageOffset(vioc->m2m_dual_wdma[M2M_DUAL_0].addr,
			vioc->m2m_wdma.fmt,
			vioc->m2m_dual_wmix[M2M_DUAL_0].width);

		VIOC_WDMA_SetImageSize(vioc->m2m_dual_wdma[M2M_DUAL_0].addr,
			vioc->m2m_dual_wmix[M2M_DUAL_0].width,
			vioc->m2m_dual_wmix[M2M_DUAL_0].height);
		vout_m2m_dual_ctrl(vioc, 1, M2M_DUAL_0);

		vout->ext_flag = 1;

		if (wait_event_interruptible_timeout(vout->ext_frame_wait,
		...
		udelay(200);	//do not delete

		#if defined(CONFIG_TCC_DUAL_DISPLAY_LUT)
		if ((vout->id == VOUT_MAIN)
			&& lut_drv_api_get_plugin(vout->lut_number))
			lut_drv_api_set_plugin(vout->lut_number, OFF,
				vout->lut_plugin_ch);
		#endif
		...
		VIOC_SC_SetOutPosition(vioc->m2m_dual_sc[M2M_DUAL_1].addr,
			...
			abs(vout->disp_rect.left) : 0,
			vout->disp_rect.top < 0 ? abs(vout->disp_rect.top) : 0);
		VIOC_SC_SetOutSize(vioc->m2m_dual_sc[M2M_DUAL_1].addr, dw, dh);
		...
		VIOC_WDMA_SetImageFormat(vioc->m2m_dual_wdma[M2M_DUAL_1].addr,
			vioc->m2m_wdma.fmt);
		VIOC_WDMA_SetImageOffset(vioc->m2m_dual_wdma[M2M_DUAL_1].addr,
		vioc->m2m_wdma.fmt, vioc->m2m_dual_wmix[M2M_DUAL_1].width);

		VIOC_WDMA_SetImageSize(vioc->m2m_dual_wdma[M2M_DUAL_1].addr,
			vioc->m2m_dual_wmix[M2M_DUAL_1].width,
			vioc->m2m_dual_wmix[M2M_DUAL_1].height);
	...
	if (vout->is_viqe_shared) {
		...
		if (vout->src_pix.field == V4L2_FIELD_INTERLACED_TB
			...
			// restore wdma r2y
			VIOC_WDMA_SetImageR2YEnable(
				vioc->m2m_wdma.addr, vioc->m2m_wdma.r2y);
		}
	}
	#endif
	if (viqe_locked)
...
search_nextfrm:
	...
	#ifdef CONFIG_VIOC_DOLBY_VISION_CERTIFICATION_TEST
	if (vout->last_cleared_buffer == NULL
		&& (HDMI_video_width == 720 && HDMI_video_height == 480)
		&& atomic_read(&vout->readable_buff_count) < 7)
		return;
	#endif

	if ((atomic_read(&vout->readable_buff_count) > 0)
		|| vout->firstFieldFlag) {
		pNextBuffer = vout_get_buffer(vout);
		/* get next buffer */
		if (vout->firstFieldFlag == 0) {
			...
			if (pNextBuffer->reserved2 || vout->force_sync) {
				if (vout->force_sync)
					vout->force_sync = OFF;
				tcc_vout_set_time(vout,
				((pNextBuffer->timestamp.tv_sec*1000)
				+(pNextBuffer->timestamp.tv_usec/1000)));
			}

			/* check video time */
			ret = vout_check_syncTime(vout,
				pNextBuffer, tcc_vout_get_time(vout));
			if (ret == VOUT_DRV_ERR_DROPFRM) {
				if ((atomic_read(
					&vout->readable_buff_count)-1) == 0)
					goto force_disp;
				...
				if (nFrame > 1) {
					nFrame_t1++;
					if (vout->onthefly_mode)
						vout_onthefly_display_update(
						vout,
						vout->last_cleared_buffer);
					else
						vout_m2m_display_update(vout,
						vout->last_cleared_buffer);
				}
			#endif
				return;
...
force_disp:
			dbprintk("d(%d %d)(%d %d)\n",
				vout->popIdx,
				atomic_read(&vout->readable_buff_count),
				pNextBuffer->index, pNextBuffer->reserved2);
		...
		if (!vout->firstFieldFlag && nFrame > 1
			&& vout->last_cleared_buffer != NULL) {
			nFrame_t2++;
			if (vout->onthefly_mode)
				vout_onthefly_display_update(
					vout, vout->last_cleared_buffer);
			else
				vout_m2m_display_update(vout,
					vout->last_cleared_buffer);
		}
...
static irqreturn_t vsync_irq_handler(int irq, void *client_data)
{
...
#endif
	{
		if (!is_vioc_intr_activatied(
			vioc->disp.vioc_intr->id, vioc->disp.vioc_intr->bits))
			return IRQ_NONE;
		vioc_intr_clear(vioc->disp.vioc_intr->id,
			vioc->disp.vioc_intr->bits);
	}
	...
	if (status & VIOC_WDMA_IREQ_EOFR_MASK) {
		...
		if (VIOC_DISP_Get_TurnOnOff(vioc->disp.addr)
			...
			if (vout->disp_mode == 0 || vout->disp_mode == 1) {
				if (vout->vout_mute == VOUT_MUTE_OFF) {
					...
					vioc->m2m_wdma.fmt,
					vioc->m2m_dual_wmix[M2M_DUAL_0].width);
					VIOC_RDMA_SetImageBase(vioc->rdma.addr,
						vioc->m2m_dual_wdma[M2M_DUAL_0].img.base0,
						vioc->m2m_dual_wdma[M2M_DUAL_0].img.base1,
						vioc->m2m_dual_wdma[M2M_DUAL_0].img.base2);
					...
					vout->ext_flag = 0;
				} else if (vout->vout_mute == VOUT_MUTE_ON) {
					VIOC_RDMA_SetImageDisable(
						vioc->rdma.addr);
				}
			} else if (vout->disp_mode == 2 ||
				vout->disp_mode == 3) {
				VIOC_RDMA_SetImageDisable(vioc->rdma.addr);
			}
		...
		if (VIOC_DISP_Get_TurnOnOff(vioc->disp_dual.addr)
			...
			if (vout->disp_mode == 0 || vout->disp_mode == 2) {
				VIOC_WMIX_SetPosition(vioc->wmix_dual.addr,
					vioc->wmix_dual.pos,
					vout->dual_disp_rect.left,
					vout->dual_disp_rect.top);
				VIOC_WMIX_SetUpdate(vioc->wmix_dual.addr);

				VIOC_RDMA_SetImageSize(vioc->rdma_dual.addr,
					vioc->m2m_dual_wmix[M2M_DUAL_1].width,
					vioc->m2m_dual_wmix[M2M_DUAL_1].height);
				VIOC_RDMA_SetImageOffset(vioc->rdma_dual.addr,
					vioc->m2m_wdma.fmt,
					vioc->m2m_dual_wmix[M2M_DUAL_1].width);
				VIOC_RDMA_SetImageBase(vioc->rdma_dual.addr,
				vioc->m2m_dual_wdma[M2M_DUAL_1].img.base0,
				vioc->m2m_dual_wdma[M2M_DUAL_1].img.base1,
				vioc->m2m_dual_wdma[M2M_DUAL_1].img.base2);
...
static irqreturn_t wdma_irq_handler(int irq, void *client_data)
{
	...
	if (!is_vioc_intr_activatied(
		vioc->m2m_wdma.vioc_intr->id, vioc->m2m_wdma.vioc_intr->bits))
		return IRQ_NONE;

	VIOC_WDMA_GetStatus(vioc->m2m_wdma.addr, &status);
	if (status & VIOC_WDMA_IREQ_EOFR_MASK) {
		/* clear wdma interrupt status */
		vioc_intr_clear(vioc->m2m_wdma.vioc_intr->id,
			vioc->m2m_wdma.vioc_intr->bits);
		...
		if (vout->id == VOUT_MAIN) {
			...
			if (VIOC_DISP_Get_TurnOnOff(vioc->disp.addr)) {
				VIOC_WMIX_SetPosition(
					vioc->m2m_dual_wmix[M2M_DUAL_1].addr,
					vioc->m2m_dual_wmix[M2M_DUAL_0].pos,
					vioc->m2m_dual_wmix[M2M_DUAL_0].left,
					vioc->m2m_dual_wmix[M2M_DUAL_0].top);
				VIOC_WMIX_SetUpdate(
					vioc->m2m_dual_wmix[M2M_DUAL_1].addr);

				VIOC_RDMA_SetImageSize(
					vioc->m2m_dual_rdma[M2M_DUAL_1].addr,
					vioc->m2m_wdma.width,
					vioc->m2m_wdma.height);
				VIOC_RDMA_SetImageOffset(
					vioc->m2m_dual_rdma[M2M_DUAL_1].addr,
					vioc->m2m_wdma.fmt,
					vioc->m2m_wdma.width);
		...
		if (vout->src_pix.field == V4L2_FIELD_INTERLACED_TB
			...
			if ((vout->deinterlace == VOUT_DEINTL_VIQE_3D)
				&& (vout->frame_count != -1)) {
				if (vout->frame_count >= 3) {
					VIOC_VIQE_SetDeintlMode(vioc->viqe.addr,
						VIOC_VIQE_DEINTL_MODE_3D);
					vout->frame_count = -1;
					dprintk("Change VIQE_3D mode\n");
...
void vout_intr_onoff(char on, struct tcc_vout_device *vout)
{
	...
	if (VIOC_CONFIG_DV_GET_EDR_PATH()) {
		if (on) {
			vioc_intr_enable(VIOC_INTR_TYPE0,
				VIOC_INTR_V_DV, 1<<FALL_HDMITX_VS);
		} else {
			void __iomem *pDV_Cfg =
				VIOC_DV_VEDR_GetAddress(VDV_CFG);

			vioc_intr_disable(vioc->disp.irq,
				VIOC_INTR_V_DV, 1<<FALL_HDMITX_VS);
			VIOC_V_DV_ClearInterrupt(pDV_Cfg,
				INT_CLR_F_TX_VS_MASK);
		}
	} else
#endif
	{
		if (on)
			vioc_intr_enable(vioc->disp.irq,
				vioc->disp.vioc_intr->id,
				vioc->disp.vioc_intr->bits);
		else
			vioc_intr_disable(vioc->disp.irq,
				vioc->disp.vioc_intr->id,
...
static int vout_video_display_enable(struct tcc_vout_device *vout)
{
	...
	if (vioc->disp.irq_enable == 0) {
		...
		ret = request_irq(vioc->disp.irq,
			vsync_irq_handler, IRQF_SHARED,
			vout->vdev->name, vout);
		if (ret)
...
#if defined(CONFIG_TCC_DUAL_DISPLAY)
void vout_m2m_dual_ctrl(
	struct tcc_vout_vioc *vioc, int enable, int m2m_dual_index)
{
	if (enable) {
		VIOC_SC_SetUpdate(vioc->m2m_dual_sc[m2m_dual_index].addr);
		VIOC_WDMA_SetImageEnable(
			vioc->m2m_dual_wdma[m2m_dual_index].addr,
			vioc->m2m_dual_wdma[m2m_dual_index].cont);
	} else {
...
void vout_m2m_ctrl(struct tcc_vout_vioc *vioc, int enable)
{
	if (enable) {
		VIOC_SC_SetUpdate(vioc->sc.addr);
		VIOC_WDMA_SetImageEnable(
			vioc->m2m_wdma.addr, vioc->m2m_wdma.cont);
	} else {
...
void vout_video_overlay(struct tcc_vout_device *vout)
{
	 ...
	 */
	/* sc */
	sw = vout->onthefly_mode ?
		vioc->rdma.width : vioc->m2m_rdma.width;
	sh = vout->onthefly_mode ?
		vioc->rdma.height : vioc->m2m_rdma.height;
	dw = vout->disp_rect.width;
	dh = vout->disp_rect.height;
	...
	if (vout->onthefly_mode) {	// onthefly_mode
		...
		VIOC_SC_SetOutPosition(vioc->sc.addr,
			...
			abs(vout->disp_rect.left) : 0,
			vout->disp_rect.top < 0 ?
			abs(vout->disp_rect.top) : 0);
		VIOC_SC_SetOutSize(vioc->sc.addr, dw, dh);
		VIOC_SC_SetUpdate(vioc->sc.addr);

		// wmix
		vioc->wmix.left = vout->disp_rect.left < 0 ?
		0 : vout->disp_rect.left;
		vioc->wmix.top = vout->disp_rect.top < 0 ?
		0 : vout->disp_rect.top;
		VIOC_WMIX_SetPosition(vioc->wmix.addr,
			vioc->wmix.pos, vioc->wmix.left, vioc->wmix.top);
		VIOC_WMIX_SetUpdate(vioc->wmix.addr);
	} else {	// m2m
	#if defined(CONFIG_TCC_DUAL_DISPLAY)
		if (vout->id == VOUT_MAIN) {
			VIOC_SC_SetDstSize(
				vioc->m2m_dual_sc[M2M_DUAL_0].addr,
				vout->disp_rect.left < 0 ?
				vout->disp_rect.width +
				abs(vout->disp_rect.left) : dw,
				vout->disp_rect.top < 0 ?
			...
			VIOC_SC_SetOutPosition(
				...
				vout->disp_rect.top < 0 ?
				abs(vout->disp_rect.top) : 0);
			VIOC_SC_SetOutSize(vioc->m2m_dual_sc[M2M_DUAL_0].addr,
				dw, dh);
			VIOC_SC_SetUpdate(vioc->m2m_dual_sc[M2M_DUAL_0].addr);
			...
			VIOC_WDMA_SetImageSize(
				vioc->m2m_dual_wdma[M2M_DUAL_0].addr,
				vioc->m2m_dual_wdma[M2M_DUAL_0].width,
				vioc->m2m_dual_wdma[M2M_DUAL_0].height);
			VIOC_WDMA_SetImageOffset(
				vioc->m2m_dual_wdma[M2M_DUAL_0].addr,
				vioc->m2m_dual_wdma[M2M_DUAL_0].fmt,
				vioc->m2m_dual_wdma[M2M_DUAL_0].width);
			VIOC_WDMA_SetImageUpdate(
				vioc->m2m_dual_wdma[M2M_DUAL_0].addr);
			...
			VIOC_SC_SetDstSize(vioc->m2m_dual_sc[M2M_DUAL_1].addr,
				...
				abs(vout->disp_rect.top) : dh);
			VIOC_SC_SetOutPosition(
				vioc->m2m_dual_sc[M2M_DUAL_1].addr,
				vout->disp_rect.left < 0 ?
				abs(vout->disp_rect.left) : 0,
				vout->disp_rect.top < 0 ?
				abs(vout->disp_rect.top) : 0);
			VIOC_SC_SetOutSize(
				vioc->m2m_dual_sc[M2M_DUAL_1].addr, dw, dh);
			VIOC_SC_SetUpdate(vioc->m2m_dual_sc[M2M_DUAL_1].addr);

			VIOC_WDMA_SetImageSize(
				vioc->m2m_dual_wdma[M2M_DUAL_1].addr,
				vioc->m2m_dual_wdma[M2M_DUAL_1].width,
				vioc->m2m_dual_wdma[M2M_DUAL_1].height);
			VIOC_WDMA_SetImageOffset(
				vioc->m2m_dual_wdma[M2M_DUAL_1].addr,
				vioc->m2m_dual_wdma[M2M_DUAL_1].fmt,
				vioc->m2m_dual_wdma[M2M_DUAL_1].width);
			VIOC_WDMA_SetImageUpdate(
				vioc->m2m_dual_wdma[M2M_DUAL_1].addr);
			...
			vioc->wmix_dual.left = vout->disp_rect.left < 0 ?
				0 : vout->disp_rect.left;
			vioc->wmix_dual.top = vout->disp_rect.top < 0 ?
				0 : vout->disp_rect.top;
		} else {
			VIOC_SC_SetDstSize(vioc->sc.addr,
				vout->disp_rect.left < 0 ?
		...
		// wmix
		vioc->wmix.left =
			vout->disp_rect.left < 0 ? 0 : vout->disp_rect.left;
		vioc->wmix.top =
			vout->disp_rect.top < 0 ? 0 : vout->disp_rect.top;
	#endif
	}
}
...
int vout_m2m_dual_init(struct tcc_vout_device *vout)
{
	...
	for (i = M2M_DUAL_0; i < M2M_DUAL_1; i++) {
		...
		/* 3. wmixer */
		vioc->m2m_dual_wmix[i].left = 0;
		// default: to assume sub-plane size same video size
		...
		/* 5. wdma */
		wdma->width = vout->disp_rect.width;
		...
		if (vioc->m2m_dual_wdma[i].irq_enable == 0) {
			vioc->m2m_dual_wdma[i].irq_enable++;
			synchronize_irq(vioc->m2m_dual_wdma[i].irq);
			vioc_intr_clear(vioc->m2m_dual_wdma[i].vioc_intr->id,
				vioc->m2m_dual_wdma[i].vioc_intr->bits);

			if (i == M2M_DUAL_0)
				request_irq(vioc->m2m_dual_wdma[i].irq,
					ext_wdma_irq_handler, IRQF_SHARED,
					"ext_wdma_interrupt", vout);

			if (ret)
				pr_err("[ERR][VOUT] wdma_irq_handler failed(%d)\n", ret);
			vioc_intr_enable(vioc->m2m_dual_wdma[i].irq,
				vioc->m2m_dual_wdma[i].vioc_intr->id,
				vioc->m2m_dual_wdma[i].vioc_intr->bits);
		}
	}
...
int vout_m2m_init(struct tcc_vout_device *vout)
{
	...
	if (vioc->m2m_wdma.irq_enable == 0) {
		...
		vioc_intr_clear(vioc->m2m_wdma.vioc_intr->id,
			vioc->m2m_wdma.vioc_intr->bits);
		ret = request_irq(vioc->m2m_wdma.irq,
			wdma_irq_handler, IRQF_SHARED,
			vout->vdev->name, vout);
		if (ret)
			pr_err("[ERR][VOUT] wdma_irq_handler failed(%d)\n",
				ret);
		vioc_intr_enable(vioc->m2m_wdma.irq,
			vioc->m2m_wdma.vioc_intr->id,
			vioc->m2m_wdma.vioc_intr->bits);
	}
...
void vout_otf_deinit(struct tcc_vout_device *vout)
{
	...
	if (VIOC_CONFIG_DV_GET_EDR_PATH()) {
		...
		struct tcc_dp_device *pdp_data =
			tca_get_displayType(TCC_OUTPUT_HDMI);
	...
	if (VIOC_CONFIG_DMAPath_Support()) {
		...
		case VIOC_MC1:
			...
			VIOC_CONFIG_SWReset(
				vioc->map_converter.id, VIOC_CONFIG_RESET);
			VIOC_CONFIG_SWReset(
				vioc->map_converter.id, VIOC_CONFIG_CLEAR);
			break;
		#endif
		#ifdef CONFIG_VIOC_DTRC_DECOMP
		case VIOC_DTRC0:
...
void vout_m2m_dual_deinit(struct tcc_vout_device *vout)
{
	...
	if (vioc->m2m_dual_wdma[M2M_DUAL_0].irq_enable) {
		...
		vioc_intr_disable(vioc->m2m_dual_wdma[M2M_DUAL_0].irq,
			vioc->m2m_dual_wdma[M2M_DUAL_0].vioc_intr->id,
			vioc->m2m_dual_wdma[M2M_DUAL_0].vioc_intr->bits);
...
void vout_m2m_deinit(struct tcc_vout_device *vout)
{
	...
	if (vioc->m2m_wdma.irq_enable) {
		vioc->m2m_wdma.irq_enable--;
		vioc_intr_disable(vioc->m2m_wdma.irq,
			vioc->m2m_wdma.vioc_intr->id,
			vioc->m2m_wdma.vioc_intr->bits);
...
static void sub_rdma_setup(struct vioc_rdma *sub_rdma)
{
	...
	VIOC_RDMA_SetImageSize(sub_rdma->addr, sub_rdma->width,
		sub_rdma->height);
	VIOC_RDMA_SetImageFormat(sub_rdma->addr, sub_rdma->fmt);
	VIOC_RDMA_SetImageOffset(sub_rdma->addr, sub_rdma->fmt,
		sub_rdma->width);
	...
	struct tcc_vout_device *vout, struct v4l2_buffer *buf)
{
	...
	if (buf) {
		...
		fbmixer.rsp_type		=
			WMIXER_POLLING;
		fbmixer.src_img_width	=
			buf->m.planes[MPLANE_VID].reserved[VID_WIDTH];
		fbmixer.src_img_height	=
			buf->m.planes[MPLANE_VID].reserved[VID_HEIGHT];

		fbmixer.src_win_left    =
			buf->m.planes[MPLANE_VID].reserved[VID_CROP_LEFT];
		fbmixer.src_win_top     =
			buf->m.planes[MPLANE_VID].reserved[VID_CROP_TOP];
		fbmixer.src_win_right   =
			buf->m.planes[MPLANE_VID].reserved[VID_CROP_LEFT]
			+ buf->m.planes[MPLANE_VID].reserved[VID_CROP_WIDTH];
		fbmixer.src_win_bottom  =
			buf->m.planes[MPLANE_VID].reserved[VID_CROP_TOP]
			+ buf->m.planes[MPLANE_VID].reserved[VID_CROP_HEIGHT];
		...
		fbmixer.src_fmt         =
			vout->onthefly_mode ? vioc->rdma.fmt :
			vioc->m2m_rdma.fmt;
		...
		if (lastframe_pbuf.size <
			(fbmixer.dst_img_width*fbmixer.dst_img_height*2)) {
		...
		dprintk("src(%d,%d %dx%d)(%d) (0x%08x 0x%08x 0x%08x)-> dst(%dx%d)(%d) (0x%08x)\n",
			fbmixer.src_win_left, fbmixer.src_win_top,
			fbmixer.src_img_width, fbmixer.src_img_height,
			fbmixer.src_fmt, fbmixer.src_y_addr, fbmixer.src_u_addr,
			fbmixer.src_v_addr, fbmixer.dst_img_width,
			fbmixer.dst_img_height, fbmixer.dst_fmt,
			fbmixer.dst_y_addr);
		...
		ret = file->f_op->unlocked_ioctl(
			file, TCC_WMIXER_ALPHA_SCALING_KERNEL,
...
void vout_disp_last_frame(struct tcc_vout_device *vout)
{
	...
	if (enable_LastFrame) {
		if (vioc->lastframe_rdma.addr) {
			VIOC_RDMA_SetImageSize(
				vioc->lastframe_rdma.addr,
				fbmixer.dst_img_width,
				fbmixer.dst_img_height);
			VIOC_RDMA_SetImageFormat(
				vioc->lastframe_rdma.addr,
				fbmixer.dst_fmt);
			VIOC_RDMA_SetImageOffset(
				vioc->lastframe_rdma.addr,
				fbmixer.dst_fmt, fbmixer.dst_img_width);
			VIOC_RDMA_SetImageBase(
				vioc->lastframe_rdma.addr,
				fbmixer.dst_y_addr,
				fbmixer.dst_u_addr, fbmixer.dst_v_addr);
			#ifdef CONFIG_TCC_VIOC_DISP_PATH_INTERNAL_CS_YUV
			VIOC_RDMA_SetImageR2YMode(
				vioc->lastframe_rdma.addr, 0x2);
			VIOC_RDMA_SetImageR2YEnable(
				vioc->lastframe_rdma.addr, ON);
			#endif

			VIOC_WMIX_SetPosition(vioc->wmix.addr,
				vioc->lastframe_rdma.id > VIOC_RDMA03 ?
				get_vioc_index(
					(vioc->lastframe_rdma.id - VIOC_RDMA03))
				: get_vioc_index(vioc->lastframe_rdma.id),
					vout->disp_rect.left,
...
void vout_subplane_onthefly_init(struct tcc_vout_device *vout)
{
	...
	VIOC_RDMA_SetImageSize(vioc->subplane_rdma.addr,
		vioc->subplane_rdma.width, vioc->subplane_rdma.height);
	VIOC_RDMA_SetImageFormat(vioc->subplane_rdma.addr,
		vioc->subplane_rdma.fmt);
	VIOC_RDMA_SetImageOffset(vioc->subplane_rdma.addr,
		vioc->subplane_rdma.fmt, vioc->subplane_rdma.width);
	VIOC_RDMA_SetImageAlphaSelect(vioc->subplane_rdma.addr,
		RDMA_ALPHA_ASEL_PIXEL);
	VIOC_RDMA_SetImageAlphaEnable(vioc->subplane_rdma.addr, ON);

	VIOC_WMIX_GetOverlayPriority(
		vioc->subplane_wmix.addr, &vioc->subplane_wmix.ovp);
}
...
int vout_subplane_onthefly_qbuf(struct tcc_vout_device *vout)
{
	...
	/* check subtitle size */
	if ((vioc->subplane_rdma.width != vioc->subplane_alpha.width)
		|| (vioc->subplane_rdma.height !=
		vioc->subplane_alpha.height)) {
		vioc->subplane_rdma.width = vioc->subplane_alpha.width;
		vioc->subplane_rdma.height = vioc->subplane_alpha.height;

		/* change rdma base address */
		VIOC_RDMA_SetImageSize(vioc->subplane_rdma.addr,
			((vioc->subplane_rdma.width >> 1) << 1),
			vioc->subplane_rdma.height);
		VIOC_RDMA_SetImageOffset(vioc->subplane_rdma.addr,
			vioc->subplane_rdma.fmt, vioc->subplane_rdma.width);
	}

	VIOC_RDMA_SetImageBase(vioc->subplane_rdma.addr,
		vioc->subplane_rdma.img.base0,
		vioc->subplane_rdma.img.base1,
		vioc->subplane_rdma.img.base2);

	/* check subtitle start position (x,y) */
	if ((vioc->subplane_wmix.left != vioc->subplane_alpha.offset_x)
		|| (vioc->subplane_wmix.top != vioc->subplane_alpha.offset_y)) {
		vioc->subplane_wmix.left = vioc->subplane_alpha.offset_x;
		vioc->subplane_wmix.top = vioc->subplane_alpha.offset_y;
		VIOC_WMIX_SetPosition(vioc->subplane_wmix.addr,
			vioc->subplane_wmix.pos, vioc->subplane_wmix.left,
			vioc->subplane_wmix.top);
		VIOC_WMIX_SetUpdate(vioc->subplane_wmix.addr);
	}
	dprintk("(%d, %d)(%dx%d)\n",
		vioc->subplane_wmix.left, vioc->subplane_wmix.top,
		vioc->subplane_rdma.width, vioc->subplane_rdma.height);
...
void vout_subplane_m2m_init(struct tcc_vout_device *vout)
{
	...
	/* 1. rdma */
	vioc->m2m_subplane_rdma.fmt = VIOC_IMG_FMT_ARGB8888;
	vioc->m2m_subplane_rdma.width = vout->src_pix.width;
	vioc->m2m_subplane_rdma.height = vout->src_pix.height;
	vioc->m2m_subplane_rdma.swap = 5;
	sub_rdma_setup(&vioc->m2m_subplane_rdma);

	/* 2. wmix */
	vioc->m2m_subplane_wmix.left = 0;
...
int vout_subplane_m2m_qbuf(
	struct tcc_vout_device *vout, struct vioc_alpha *alpha)
{
	struct tcc_vout_vioc *vioc = vout->vioc;
	...
	/* check subtitle size */
	if ((vioc->m2m_subplane_rdma.width != alpha->width)
		|| (vioc->m2m_subplane_rdma.height != alpha->height)) {
		vioc->m2m_subplane_rdma.width = alpha->width;
		vioc->m2m_subplane_rdma.height = alpha->height;

		/* change rdma base address */
		VIOC_RDMA_SetImageSize(vioc->m2m_subplane_rdma.addr,
			((vioc->m2m_subplane_rdma.width >> 1) << 1),
			vioc->m2m_subplane_rdma.height);
		VIOC_RDMA_SetImageOffset(vioc->m2m_subplane_rdma.addr,
			vioc->m2m_subplane_rdma.fmt,
			vioc->m2m_subplane_rdma.width);
	}
	/* change rdma base address */
	VIOC_RDMA_SetImageBase(vioc->m2m_subplane_rdma.addr,
		vioc->m2m_subplane_rdma.img.base0,
		vioc->m2m_subplane_rdma.img.base1,
		vioc->m2m_subplane_rdma.img.base2);

	/* check subtitle start position (x,y) */
	if ((vioc->m2m_subplane_wmix.left != alpha->offset_x)
	...
	dprintk("(%d, %d)(%dx%d)\n",
		vioc->m2m_subplane_wmix.left, vioc->m2m_subplane_wmix.top,
		vioc->m2m_subplane_rdma.width, vioc->m2m_subplane_rdma.height);
...
void vout_subplane_deinit(struct tcc_vout_device *vout)
{
	...
	VIOC_RDMA_SetImageDisable(vout->onthefly_mode ?
		vioc->subplane_rdma.addr : vioc->m2m_subplane_rdma.addr);
...
void vout_subplane_ctrl(struct tcc_vout_device *vout, int enable)
{
	...
	if (enable)
		VIOC_RDMA_SetImageEnable(
			vout->onthefly_mode ? vioc->subplane_rdma.addr
			: vioc->m2m_subplane_rdma.addr);
	else
		VIOC_RDMA_SetImageDisable(
			vout->onthefly_mode ? vioc->subplane_rdma.addr
