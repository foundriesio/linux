...
static int tcc_vout_buffer_copy(struct v4l2_buffer *dst, struct v4l2_buffer *src, int mode)
{
	int ret = 0;
	...
	if(mode)
		ret = copy_from_user((void *)dst->m.planes, (const void *)src->m.planes, sizeof(struct v4l2_plane) * MPLANE_NUM);
	else
		ret = copy_to_user((void *)dst->m.planes, (const void *)src->m.planes, sizeof(struct v4l2_plane) * MPLANE_NUM);
	return ret;
}
...
static int tcc_v4l2_buffer_set(struct tcc_vout_device *vout, struct v4l2_requestbuffers *req)
{
	...
	for (index = vout->nr_qbufs; index < (vout->nr_qbufs + req->count); index++) {
		...
		case V4L2_PIX_FMT_YUV422P:	/* YCbCr 4:2:2 separated (VIOC_IMG_FMT_YUV422SEP) */
			b[index].img_base0 = vout->pmap.base + (total_offset * index);
		...
		case V4L2_PIX_FMT_YVYU:		/* YCbCr 4:2:2 YVYU Sequential (VIOC_IMG_FMT_YVYU) */
			...
			break;
		default:					/* RGB and etc... */
			b[index].img_base0 = vout->pmap.base + (vout->src_pix.sizeimage * index);
			b[index].img_base1 = 0;
			b[index].img_base2 = 0;
		}
...
static int tcc_deintl_buffer_set(struct tcc_vout_device *vout)
{
	...
	/*
	 * CAUTION!!!: deintl_buf (m2m_wdma buf) is always YUV420 Interleaved Type0.
	 */
...
static int vidioc_s_fmt_vid_out(struct file *file, void *fh, struct v4l2_format *f)
{
	...
	case TCC_PFMT_RGB:
	default:
		vout->src_pix.sizeimage = PAGE_ALIGN(vout->src_pix.width * vout->src_pix.height * vout->bpp);
		break;
	}
	 ...
	 *
	 * | type of input frame | m2m_rdma | deinterlacer | m2m_wdma | (disp) rdma |
	 * |---------------------|-------------|--------------|-------------|-------------|
	 ...
	 * Therefore, 'm2m_rdma.y2r changing codes' existed in the vout_deintl_qbuf().
	 *
	 * Known-bug:
	...
	if(vout->onthefly_mode)
	{
		...
		dprintk("rdma.y2r(%d)-viqe.y2r(%d)", vioc->rdma.y2r, vioc->viqe.y2r);

		vout->first_frame = 1;
		ret = vout_otf_init(vout);
		if (ret) {
			printk(KERN_ERR VOUT_NAME ": [error] onthefly setup\n");
			goto out_exit;
		}
	}
	else
	{
		...
		dprintk("rdma.y2r(%d)-viqe.y2r(%d)-wdma.r2y(%d)-rdma.y2r(%d)", vioc->m2m_rdma.y2r, vioc->viqe.y2r, vioc->m2m_wdma.r2y, vioc->rdma.y2r);
		...
		if(vout->id == VOUT_MAIN) {
			...
			vioc->m2m_wdma.fmt = VIOC_IMG_FMT_YUYV;		// for GRDMA support
		}
...
static int vidioc_s_fmt_vid_out_overlay(struct file *file, void *fh, struct v4l2_format *f)
{
	...
	if (vout->disp_rect.left < 0) {
		vout->disp_rect.width = vout->disp_rect.width + vout->disp_rect.left;
	...
	if (vout->disp_rect.top < 0) {
		vout->disp_rect.height = vout->disp_rect.height + vout->disp_rect.top;
...
static int vidioc_reqbufs(struct file *file, void *fh, struct v4l2_requestbuffers *req)
{
	...
	if(!vout->mapped) {
		...
		if (V4L2_MEMORY_MMAP == vout->memory) {
			...
			if (ret) {
				...
				if(tcc_vout_get_status(VOUT_SUB) >= 0) {
					...
					vout->pmap.base = (vout->id == VOUT_MAIN) ? vout->pmap.base : (vout->pmap.base + pmap_size);
				} else {
			...
			if ((req->count > actual_bufs) || ((__s32)req->count < 0))
				req->count = actual_bufs;
		...
		/* init driver data */
		vout->qbufs = kzalloc(sizeof(struct tcc_v4l2_buffer) * req->count, GFP_KERNEL);
		if(!vout->qbufs)
			return -ENOMEM;
...
static int vidioc_querybuf(struct file *file, void *fh, struct v4l2_buffer *buf)
{
	...
	copy_to_user(buf->m.planes, qbuf->buf.m.planes, sizeof(struct v4l2_buffer) * MPLANE_NUM);
...
static int vidioc_qbuf(struct file *file, void *fh, struct v4l2_buffer *buf)
{
	...
	dbprintk("q(%d %d)(%d %d)(%ldmsec)\n", vout->pushIdx, atomic_read(&vout->readable_buff_count), buf->index, buf->reserved2, (buf->timestamp.tv_sec*1000)+(buf->timestamp.tv_usec/1000));
...
force_dqbuf:
	mutex_lock(&vout->lock);
	dbprintk("dq(%d %d)(%d %d)\n", vout->clearIdx, atomic_read(&vout->displayed_buff_count), buf->index, buf->reserved2);
...
static long vout_param_handler(struct file *file, void *fh, bool valid_prio, unsigned int cmd, void *arg)
{
	...
	case VIDIOC_USER_CLEAR_FRAME:
		if(!vout->first_frame) {
			dprintk("Flushing all buffers [R:%d D:%d] \n", atomic_read(&vout->readable_buff_count), atomic_read(&vout->displayed_buff_count));
	...
	case VIDIOC_USER_CAPTURE_LASTFRAME:
		if(!vout->first_frame) {
			tcc_vout_buffer_copy(&vout->lastframe, (struct v4l2_buffer *)arg, TO_KERNEL_SPACE);
...
{
	struct tcc_vout_device *vout = (struct tcc_vout_device *)platform_get_drvdata(pdev);
#ifdef CONFIG_VOUT_DISPLAY_LASTFRAME
