...
#define DS90UB964_WIDTH			1280
#else
#define DS90UB964_WIDTH			5120
#endif
#define DS90UB964_HEIGHT		720

extern volatile void __iomem *  ddicfg_base;
static unsigned int des_irq_num;

struct capture_size sensor_sizes[] = {
	{DS90UB964_WIDTH, DS90UB964_HEIGHT},
	{DS90UB964_WIDTH, DS90UB964_HEIGHT}
};
...
static int write_regs_ds90ub964(const struct videosource_reg * list, unsigned int mode) {
...
void ds90ub964_read_line(void) {
    unsigned char PORT_ISR_LO = 0, PORT_ISR_HI = 0, RX_PORT_STS1 = 0, RX_PORT_STS2 = 0, CSI_TX_ISR = 0, CSI_STS = 0, \
                LINE_COUNT_1 = 0, LINE_COUNT_0 = 0, LINE_LEN_1 = 0, LINE_LEN_0 = 0;

    unsigned int val = 0;

    //################################################
    //    LINE_LEN_1 = ReadI2C(0x75)
        DDI_I2C_Read(0x75, 1, &LINE_LEN_1, 1);
    
        val = LINE_LEN_1;
        val <<= 8;
    
    //################################################
    //    LINE_LEN_1 = ReadI2C(0x76)
        DDI_I2C_Read(0x76, 1, &LINE_LEN_0, 1);
    ...
    //    LINE_COUNT_1 = ReadI2C(0x73)
        DDI_I2C_Read(0x73, 1, &LINE_COUNT_1, 1);
    
        val = LINE_COUNT_1;
        val <<= 8;
    
    //################################################
    //    LINE_COUNT_0 = ReadI2C(0x74)
        DDI_I2C_Read(0x74, 1, &LINE_COUNT_0, 1);
...
EXPORT_SYMBOL(ds90ub964_read_line);

void ds90ub964_check_interrupt_status(void) 
{
    unsigned char PORT_ISR_LO = 0, PORT_ISR_HI = 0, RX_PORT_STS1 = 0, RX_PORT_STS2 = 0, CSI_STS = 0, \
                LINE_COUNT_1 = 0, LINE_COUNT_0 = 0, LINE_LEN_1 = 0, LINE_LEN_0 = 0;
...
//  printk("line count high : 0x%x , line count low : 0x%x (%d) \n", LINE_COUNT_1, LINE_COUNT_0, val);
     ...
     */
    DDI_I2C_Read(0xDB, 1, &PORT_ISR_LO, 1);
...
//    printk("0xDB PORT_ISR_LO : 0x%x \n", PORT_ISR_LO); //# readout; cleared by RX_PORT_STS2
     ...
     */
    DDI_I2C_Read(0xDA, 1, &PORT_ISR_HI, 1);
...
//    printk("0xDA PORT_ISR_HI : 0x%x \n", PORT_ISR_HI); //# readout; cleared by RX_PORT_STS2
     ...
     */
    DDI_I2C_Read(0x4D, 1, &RX_PORT_STS1, 1);
...
//  printk("0x4D RX_PORT_STS1 : 0x%x \n", RX_PORT_STS1);
     ...
     */ 
    DDI_I2C_Read(0x4E, 1, &RX_PORT_STS2, 1);
...
void ds90ub964_process_interrupt(void) {
    ...
    if(DDI_I2C_Read(0x24 , 1, &interrupt_sts, 1)) {
       printk("ERROR: Sensor I2C !!!! \n"); 
       return IRQ_HANDLED;
    }
    ...
    if ((interrupt_sts & 0x10) >> 4) {
        ...
        DDI_I2C_Write(data, 1, 1);

        DDI_I2C_Read(0x37, 1, &retVal, 1);
    ...
    if ((interrupt_sts & 0x08) >> 3) {
        ...
        ds90ub964_check_interrupt_status();
    }
...
extern unsigned int tccvin_set_skip_frame(unsigned int num);

irqreturn_t ds90ub964_irq_handler(int irq, void *client_data)
{
	printk("%s in\n", __func__);

    tccvin_set_skip_frame(3);
...
irqreturn_t ds90ub964_irq_thread_handler(int irq, void * client_data) {
	printk("%s in \n", __func__);
    ds90ub964_process_interrupt();
...
int ds90ub964_set_irq_handler(TCC_SENSOR_INFO_TYPE * sensor_info, unsigned int enable) {
    if(enable) {
...
//				if(request_irq(des_irq_num, ds90ub964_irq_handler, IRQF_TRIGGER_FALLING, "ds90ub964", NULL)) {
//            if(request_threaded_irq(des_irq_num, ds90ub964_irq_handler, ds90ub964_irq_thread_handler, IRQF_SHARED | IRQF_TRIGGER_FALLING, "ds90ub964", "ds90ub964")) {
            if(request_threaded_irq(des_irq_num, ds90ub964_irq_handler, ds90ub964_irq_thread_handler, IRQF_TRIGGER_FALLING, "ds90ub964", NULL)) {
...
//        free_irq(des_irq_num, "ds90ub964");
...
int ds90ub964_change_mode(int mode) {
	...
	ret = write_regs_ds90ub964(videosource_reg_table_list[mode], mode);
...
void sensor_info_init(TCC_SENSOR_INFO_TYPE * sensor_info) {
	sensor_info->width			    = DS90UB964_WIDTH;
	sensor_info->height			    = DS90UB964_HEIGHT;// - 1;        
	...
	sensor_info->capture_w			= DS90UB964_WIDTH;
	sensor_info->capture_h			= DS90UB964_HEIGHT;// - 1;

	sensor_info->capture_zoom_offset_x	= 0;
	sensor_info->capture_zoom_offset_y	= 0;
	sensor_info->cam_capchg_width		= DS90UB964_WIDTH;
...
void sensor_init_fnc(SENSOR_FUNC_TYPE * sensor_func) {
	sensor_func->open			    = ds90ub964_open;
	sensor_func->close			    = ds90ub964_close;
	sensor_func->change_mode	    = ds90ub964_change_mode;
    sensor_func->set_irq_handler    = ds90ub964_set_irq_handler;
}
