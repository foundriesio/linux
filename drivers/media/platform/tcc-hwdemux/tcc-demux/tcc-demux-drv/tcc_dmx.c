...
#ifdef TS_PACKET_CHK_MODE
static struct ts_packet_chk_info_t* ts_packet_chk_info = NULL;
#endif//TS_PACKET_CHK_MODE
...
static void tcc_dmx_ts_cc_check(unsigned char* buf)
{
	...
	if(ts_packet_chk_info != NULL)
	{
		...
		if(ts_packet_chk_info->packet == NULL)
		{
			tmp = (struct ts_packet_chk_t*)kmalloc(sizeof(struct ts_packet_chk_t), GFP_ATOMIC);
			if(tmp == NULL)
			{
	...
	for (i = 0; i < inst->dev_num; i++)
	{
		...
		if(demux->users > 0)
		{
			if(p1 != NULL && p1_size > 0)
			{
				if(p1[0] != 0x47)
				{
					pr_err("[ERROR][HWDMX]packet error 1... [%x]\n", p1[0]);
					return -1;
				}
				else
				{
			...
			if(p2 != NULL && p2_size > 0)
			{
				if(p2[0] != 0x47)
				{
					pr_err("[ERROR][HWDMX]packet error 2... [%x]\n", p2[0]);
					return -1;
				}
				else
				{
            ...
            if (dmxfilter->state == DMX_STATE_GO)
            {
                ...
                if (feed->type == DMX_TYPE_SEC && feed->state == DMX_STATE_GO)
                {
                    int section_syntax_indicator=0;
                     ...
                     if(crc_err)
                     {
                        pr_err("[ERROR][HWDMX][check section]should be checked crc result! : hwdemux:size[%d]crc_result[%d]\n", size, crc_err);
                        hexdump(p, 32);
                     }
#endif
...
#ifdef      TS_SECTION_CHK_MODE
                              hexdump(p, 64);
                              pr_err("[ERROR][HWDMX][check section] section crc checking error !.section size[%d]crc_result[%d]\n", size, crc_err);
#endif
                          }
                     }
                    ...
                    if (!feed->feed.sec.check_crc || !crc_err)
                    {
                        ...
                        if(secsize != size)
                        {
                            pr_err("[ERROR][HWDMX][check section] section size mismatch!-[%d][%d]\n", secsize, size);
                            hexdump(p, 64);
                        ...
                        if(feed->feed.sec.check_crc && section_syntax_indicator)
                        {
                            ...
                            if( demux->check_crc32(feed, p, size) )
                            {
                                pr_err("[ERROR][HWDMX][check section] section crc checking error !.section size[%d]\n", size);
                                pr_err("[ERROR][HWDMX][check section] filter values [0x%X][0x%X][0x%X][0x%X][0x%X][0x%X].\n",  secfilter->filter_value[0], secfilter->filter_value[1], secfilter->filter_value[2], \
                        ...
                        if(has_error)
                        {
                            pr_err("[ERROR][HWDMX][check section] section matching error!.section size[%d] crc[%d] doneq[%d]\n",size, feed->feed.sec.check_crc, dmxfilter->doneq);
                            pr_err("[ERROR][HWDMX][check section] filter values [0x%X][0x%X][0x%X][0x%X][0x%X][0x%X].\n",  secfilter->filter_value[0], secfilter->filter_value[1], secfilter->filter_value[2], \
                                        secfilter->filter_value[3], secfilter->filter_value[4], secfilter->filter_value[5]);
                            hexdump(p, 64);
                        }
...
static int tcc_dmx_start_feed(struct dvb_demux_feed *feed)
{
	...
	{
#ifdef TS_PACKET_CHK_MODE
        ts_packet_chk_info = (struct ts_packet_chk_info_t*)kmalloc(sizeof(struct ts_packet_chk_info_t), GFP_ATOMIC);
        if(ts_packet_chk_info == NULL)
        {
            pr_err("[ERROR][HWDMX]\t ts_packet_chk_info_t mem alloc err..\n");
        }
    	memset(ts_packet_chk_info, 0x0, sizeof(struct ts_packet_chk_info_t));
#endif
	}
...
int tcc_dmx_init(tcc_dmx_inst_t *inst)
{
	...
	for (i = 0; i < inst->dev_num; i++)
	{
    	...
    	{
	        pr_err("[ERROR][HWDMX]%s(fail - dvb_dmx_init)\n", __FUNCTION__);
        	return -1;
    	}
	    ...
	    {
        	pr_err("[ERROR][HWDMX]%s(fail - dvb_dmxdev_init)\n", __FUNCTION__);
        	return -1;
    	}
	    ...
	    for (j = 0; j < 4; j++)
	    {
        	...
        	{
	            pr_err("[ERROR][HWDMX]%s(fail - add_frontend fe_hw[%d])\n", __FUNCTION__, j);
            	return -1;
        	}
    	}
    	...
    	{
	        pr_err("[ERROR][HWDMX]%s(fail - add_frontend fe_mem)\n", __FUNCTION__);
        	return -1;
    	}

	    if (inst->dmx[i].demux.dmx.connect_frontend(&inst->dmx[i].demux.dmx, &inst->dmx[i].fe_hw[0]) < 0)
	    {
        	pr_err("[ERROR][HWDMX]%s(fail - connect_frontend)\n", __FUNCTION__);
        	return -1;
    	}
    }
...
//	pr_info("[INFO][HWDMX]%s", __FUNCTION__);
