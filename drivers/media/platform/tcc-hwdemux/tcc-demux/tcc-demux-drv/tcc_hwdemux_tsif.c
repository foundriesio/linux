...
#define TSIF_MAJOR_NUMBER 0255
#define TSIF_MINOR_NUMBER 0
...
static int tsif_major_num = 0;
static int hwdmx_tsif_num = 0;
	...
	if(ts_packet_chk_info != NULL) {
		if(ts_packet_chk_info->packet != NULL) {
			...
			if(mod) {
				...
				do {
					pr_info("[INFO][HWDMX]\t\tpid:0x%04x => cnt:%llu err:%d\n", tmp->pid, tmp->cnt, tmp->err);
					tmp = tmp->next;
				} while(tmp != NULL);
			}
		}
...
static void itv_ts_cc_check(unsigned char* buf)
{
	...
	if(ts_packet_chk_info != NULL) {
		...
		if(ts_packet_chk_info->packet == NULL) {
			...
			if(tmp == NULL) {
				pr_info("[INFO][HWDMX]\t ts_packet_chk_t mem alloc err..\n");
			}
    ...
    case IOCTL_TSIF_DMA_START:
        {
            ...
            if (tcc_hwdmx_tsif_copy_from_user(demux, &param, (void *)arg, sizeof(struct tcc_tsif_param))) {
                pr_err("[ERROR][HWDMX]cannot copy from user tcc_tsif_param in IOCTL_TSIF_DMA_START !!! \n");
                return -EFAULT;
            }
            if (demux->state == 1) {
                demux->state = 0;
    ...
    case IOCTL_TSIF_GET_STC:
        {
            ...
            if (tcc_hwdmx_tsif_copy_to_user(demux, (void *)arg, (void *)&uiSTC, sizeof(int))) {
                pr_err("[ERROR][HWDMX]cannot copy to user tcc_tsif_param in IOCTL_TSIF_GET_PCR !!! \n");
                return -EFAULT;
            }
        }
        break;
