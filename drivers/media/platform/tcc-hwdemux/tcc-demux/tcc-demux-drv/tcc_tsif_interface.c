 ...
 * Functions
 ******************************************************************************/
static void tcc_tsif_running_feed(unsigned long id)
{
	...
	for (index = 0; index < TS_PACKET_LIST; index++) {
		...
		if (type == TSIF_FILTER_TYPE_SECTION) {
			if (p2_size > 0) {
				tcc_hwdmx_tsif_buffer_flush(&pHandle->filp,
							    (unsigned long)p2,
							    p2_size);
			}
...
int tcc_tsif_interface_open(unsigned int devid)
{
	...
	if (pHandle->users == 1) {
		...
		if (err != 0) {
			...
			pr_err(
			"[ERROR][HWDMX]Fail open driver [ID=%d, Err=%d]\n",
			devid, err);
			return err;
		}
		...
		tcc_hwdmx_tsif_set_external_tsdemux(&pHandle->filp,
						    tcc_tsif_parse_packet, 1,
						    0);
	}
...
int tcc_tsif_interface_close(unsigned int devid)
{
	...
	if (pHandle->users == 0) {
		...
		if (err != 0) {
			...
			pr_err(
			"[ERROR][HWDMX]Fail close driver [ID=%d, Err=%d]\n",
			devid, err);
			return err;
		}
	}
...
static int tcc_tsif_set_pid_for_hwdmx(struct tcc_tsif_priv_t *pHandle,
				      struct dvb_demux_feed *feed)
{
		...
		if (i == MAX_TS_CNT) {
			...
			for (i = 0; i < MAX_SEC_CNT; i++) {
				if (pHandle->sec_table.iPID[i] ==
					...
					param.f_size = 0;
					if (tcc_hwdmx_tsif_ioctl_ex
					    (&pHandle->filp,
					     IOCTL_TSIF_REMOVE_PID,
					     (unsigned long)&param) !=
					    0) {
						pr_err(
						"[ERROR][HWDMX]%s:%d\n",
			...
			param.f_size = 0;
			if (tcc_hwdmx_tsif_ioctl_ex
			    (&pHandle->filp, IOCTL_TSIF_ADD_PID,
	...
	case DMX_TYPE_SEC:
	{
		...
		for (i = 0; i < demux->filternum; i++) {
			...
			pHandle->sec_table.iUseSW[
				demux->filter[i].index] = useSW;
...
static int tcc_tsif_set_pid(struct tcc_tsif_priv_t *pHandle,
			    struct dvb_demux_feed *feed)
{
	...
	for (i = 0; i < MAX_TS_CNT; i++) {
		if (pHandle->ts_table.iPID[i] < PID_NULL) {
			...
			pid_param.pid_data[pid_param.valid_data_cnt] =
			    pHandle->ts_table.iPID[i];
	...
	if (tcc_hwdmx_tsif_ioctl_ex
	    (&pHandle->filp, IOCTL_TSIF_SET_PID,
	     (unsigned long)&pid_param) != 0) {
		pr_err("[ERROR][HWDMX]%s:%d\n",
			   __func__, __LINE__);
		return -1;
	}
	if (pHandle->is_start_dma == 0) {
		...
		dma_param.mode = 0x04;
		if (tcc_hwdmx_tsif_ioctl_ex
		    (&pHandle->filp, IOCTL_TSIF_DMA_START,
		     (unsigned long)&dma_param) != 0) {
			pr_err("[ERROR][HWDMX]%s:%d\n",
			       __func__, __LINE__);
			return -1;
		}
				  ...
				  unsigned int devid)
{
	...
	mutex_lock(&pHandle->mutex);
	ret =
	    tcc_hwdmx_tsif_ioctl_ex(&pHandle->filp, IOCTL_TSIF_SET_PCRPID,
				    (unsigned long)&param);
	mutex_unlock(&pHandle->mutex);
...
static int tcc_tsif_remove_pid_for_hwdmx(struct tcc_tsif_priv_t *pHandle,
					 struct dvb_demux_feed *feed)
{
	struct tcc_tsif_filter param;
	struct dvb_demux *demux = feed->demux;
	...
	case DMX_TYPE_TS:
	{
		...
		if (i == MAX_TS_CNT) {
			pr_err(
			"[ERROR][HWDMX]Failed remove pid = %d]\n",
			feed->pid);
			return -1;
		}
		if (pHandle->ts_table.iUsers[i] == 0) {
			...
			param.f_size = 0;
			if (tcc_hwdmx_tsif_ioctl_ex
			    (&pHandle->filp, IOCTL_TSIF_REMOVE_PID,
			     (unsigned long)&param) != 0
			    && param.f_pid != 0x2000) {
				pr_err(
				"[ERROR][HWDMX]%s:%d\n",
			...
			for (i = 0; i < demux->filternum; i++) {
				...
				if (pHandle->sec_table.iPID[
					...
					param.f_comp =
					    demux->filter[
					    i].filter.filter_value;
					param.f_mask =
					    demux->filter[
					    i].filter.filter_mask;
					param.f_mode =
					    demux->filter[
					    i].filter.filter_mode;
					check_j = 1;
				}

				for (j = DMX_MAX_FILTER_SIZE;
				     j > 0 && check_j == 1; j--) {
					if (param.f_mask[j-1] != 0x0)
						break;
				}
		...
		for (i = 0; i < demux->filternum; i++) {
			if (demux->filter[i].state == DMX_STATE_GO
				...
				demux->filter[i].state =
				    DMX_STATE_READY;
				pHandle->sec_table.iPID[demux->filter[
					i].index] = PID_NULL;
...
static int tcc_tsif_remove_pid(struct tcc_tsif_priv_t *pHandle,
			       struct dvb_demux_feed *feed)
{
	struct tcc_tsif_pid_param pid_param;
	int i;
	...
	if (tcc_hwdmx_tsif_ioctl_ex
	    (&pHandle->filp, IOCTL_TSIF_SET_PID,
	     (unsigned long)&pid_param) != 0) {
		pr_err("[ERROR][HWDMX]%s : pid error !!!\n", __func__);
		return -1;
	}
	...
	if (pHandle->is_start_dma == 0) {
		if (tcc_hwdmx_tsif_ioctl_ex
		    (&pHandle->filp, IOCTL_TSIF_DMA_STOP, 0) != 0) {
			pr_err("[ERROR][HWDMX]%s : dma error !!!\n",
			       __func__);
			return -1;
		}
	}
...
unsigned int tcc_tsif_interface_get_stc(unsigned int index, unsigned int devid)
{
	...
	mutex_lock(&pHandle->mutex);
	tcc_hwdmx_tsif_ioctl_ex(&pHandle->filp, IOCTL_TSIF_GET_STC,
				(unsigned long)&stc);
	mutex_unlock(&pHandle->mutex);
