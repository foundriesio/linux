...
struct tcc_tsif_regs {
	volatile unsigned long TSDI, TSRXCR, TSPID[16], TSIC, TSIS, TSISP,
	    TSTXC;
};
#pragma pack(pop)
...
struct tcc_tsif_handle {
	...
	unsigned int working_mode;//1:tsif mode for isdbt
	struct clk *cm3_clk;
...
extern int tca_tsif_register_pids(struct tcc_tsif_handle *h, unsigned int *pids,
				  unsigned int count);
extern int tca_tsif_can_support(void);
extern int tca_tsif_buffer_updated_callback(struct tcc_tsif_handle *h,
...
extern int tca_tsif_add_filter(struct tcc_tsif_handle *h,
			       struct tcc_tsif_filter *feed);
extern int tca_tsif_remove_filter(struct tcc_tsif_handle *h,
				  struct tcc_tsif_filter *feed);
extern int tca_tsif_input_internal(unsigned int dmx_id, unsigned int phy_addr,
				   unsigned int size);

extern int tsdmx_lock_on(void);
extern int tsdmx_lock_off(void);

extern int tca_hwdemux_cipher_Set_Algorithm(
		struct stHWDEMUXCIPHER_ALGORITHM *pARG);
extern int tca_hwdemux_cipher_Set_Key(struct stHWDEMUXCIPHER_KEY *pARG);
extern int tca_hwdemux_cipher_Set_IV(struct stHWDEMUXCIPHER_VECTOR *pARG);
extern int tca_hwdemux_cipher_Cipher_Run(struct stHWDEMUXCIPHER_EXECUTE *pARG);
