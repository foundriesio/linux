...
struct ts_packet_chk_info_t *ts_packet_chk_info;
...
static void itv_ts_cc_check(unsigned char *buf)
{
	...
	pid =
	    ((*(unsigned char *)(buf + 1) & 0x1f) << 8) | *(unsigned char *)(buf
									     +
									     2);
	...
	if (ts_packet_chk_info->packet == NULL) {
		tmp = kmalloc(sizeof(struct ts_packet_chk_t), GFP_ATOMIC);
		if (tmp == NULL)
			return;
		...
		if (new) {
			...
			if (tmp == NULL) {
				pr_err(
				"[ERROR][HWDMX] ts_packet_chk_t mem alloc err..\n");
			}
...
{
	pr_info(
	"[INFO][HWDMX] debug_log_chk_time:%d(sec)\n",
	     chk_time);
...
static int __maybe_unused rx_dma_buffer_alloc(int devid,
					      struct tea_dma_buf *dma)
{
...
static int rx_dma_alloc_buffer(int devid)
{
	...
	if (os + size > tsif_ex_pri.pmap_tsif.size) {
		pr_err(
		"[ERROR][HWDMX] %s:%d dma buffer is not enough\n",
		__func__, __LINE__);
		ret = -ENOMEM;
		goto err;
	}
	...
	tsif_ex_pri.static_dma_buffer[devid]->v_addr =
	...
	tsif_ex_pri.static_dma_buffer[devid]->dma_sec_addr =
	    tsif_ex_pri.pmap_tsif.base;
#else
	if (rx_dma_buffer_alloc(devid, tsif_ex_pri.static_dma_buffer[devid]) !=
	    0) {
		pr_err(
		"%s:%d dma alloc(%d) failed\n", __func__, __LINE__, devid);
		ret = -ENOMEM;
		goto err;
	}
...
static int
rx_update_stc(struct tcc_hwdmx_tsif_rx_handle *demux, char *p1, int p1_size,
	      char *p2, int p2_size)
{
	int i;
	...
	int updated_buff_offset, int end_buff_offset)
{
	...
	if (++demux->ts_demux_feed_handle.index >= packet_th) {
		...
		if (g_smpcb[dmx_id] != NULL) {
			if (g_smpcb[dmx_id](dmx_id, (uintptr_t)p_addr1,
					size1, (uintptr_t)p_addr2, size2)
					== 0) {
				demux->write_buff_offset = updated_buff_offset;
				demux->ts_demux_feed_handle.index = 0;
			}
...
static int rx_updated_callback(unsigned int dmxid, unsigned int ftype,
			       ...
			       unsigned int value2, unsigned int bErrCRC)
{
	...
	if (demux->ts_demux_feed_handle.is_active != 0) {
		...
		case 1:
			...
			demux,
			demux->tsif_ex_handle.dma_buffer->v_secure_addr,
			demux->tsif_ex_handle.dma_buffer->secure_dma_addr,
			value1, value2);
			break;
		}
		}
...
#if defined(DEMUX_SYSFS)
static ssize_t tcc_hwdmx_tsif_sysfs_show(struct kobject *kobj,
					 struct kobj_attribute *attr, char *buf)
{
	...
	for (i = 0; i < HWDMX_NUM; i++) {
		count +=
		    sprintf(&buf[count], "%10d ", tsif_ex_pri.demux[i].state);
	}
	count += sprintf(&buf[count], "\n");

	for (i = 0; i < HWDMX_NUM; i++) {
		count +=
		    sprintf(&buf[count], "%10d ",
			    atomic_read(&tsif_ex_pri.stat[i].pidnum));
	}
	count += sprintf(&buf[count], "\n");
	...
	for (i = 0; i < HWDMX_NUM; i++) {
		count +=
		    sprintf(&buf[count], "%10d ",
			    atomic_read(&tsif_ex_pri.stat[i].secnum));
	}
	count += sprintf(&buf[count], "\n");

	for (i = 0; i < HWDMX_NUM; i++) {
		count +=
		    sprintf(&buf[count], "%10lu ", tsif_ex_pri.stat[i].tsevt);
	}
	count += sprintf(&buf[count], "\n");

	for (i = 0; i < HWDMX_NUM; i++) {
		count +=
		    sprintf(&buf[count], "%10lu ", tsif_ex_pri.stat[i].secevt);
	}
	count += sprintf(&buf[count], "\n");

	for (i = 0; i < HWDMX_NUM; i++) {
		count +=
		    sprintf(&buf[count], "%10lu ", tsif_ex_pri.stat[i].pcrevt);
	}
	count += sprintf(&buf[count], "\n");
...
static ssize_t tcc_hwdmx_tsif_sysfs_store(struct kobject *kobj,
...
__ATTR(stat, 0200 | 0444, tcc_hwdmx_tsif_sysfs_show,
tcc_hwdmx_tsif_sysfs_store);
#endif
...
struct tcc_hwdmx_tsif_rx_handle *tcc_hwdmx_tsif_rx_start(unsigned int devid)
{
	...
	memcpy((void *)&demux->tsif_ex_handle.port_cfg,
	       (void *)&tsif_ex_pri.port_cfg[devid],
	...
	if (tsif_ex_pri.dev[demux->tsif_ex_handle.dmx_id] != NULL) {
		pinctrl =
		pinctrl_get_select(
		tsif_ex_pri.dev[demux->tsif_ex_handle.dmx_id],
		"active");
	}
#if 0				// don't care
	if (IS_ERR(pinctrl))
	...
	hwdmx_buffer_updated_callback(&demux->tsif_ex_handle,
				      rx_updated_callback);
...
int tcc_hwdmx_tsif_rx_stop(struct tcc_hwdmx_tsif_rx_handle *demux,
			   unsigned int devid)
{
	struct tea_dma_buf *dma_buffer;
	...
	if (tsif_ex_pri.open_cnt > 0) {
		if (demux->state == 1) {
			...
			if (tsif_ex_pri.dev[demux->tsif_ex_handle.dmx_id] !=
			    NULL) {
				pinctrl =
				pinctrl_get_select(
				tsif_ex_pri.dev[demux->tsif_ex_handle.dmx_id],
...
int __maybe_unused
tcc_hwdmx_tsif_rx_buffer_flush(struct tcc_hwdmx_tsif_rx_handle *demux,
			       unsigned long addr, int len)
{
	...
	if ((unsigned long)demux->tsif_ex_handle.dma_buffer->v_addr <= addr) {
		offset =
		    addr -
		    (unsigned long)demux->tsif_ex_handle.dma_buffer->v_addr;
		if (offset < demux->tsif_ex_handle.dma_buffer->buf_size) {
			addr = demux->tsif_ex_handle.dma_buffer->dma_addr;
			dma_sync_single_for_cpu(0, addr + offset, len,
						DMA_FROM_DEVICE);
			return 0;
		}
	}
	if ((unsigned long)demux->tsif_ex_handle.dma_buffer->v_sec_addr
		<= addr) {
		offset =
		    addr -
...
int tcc_hwdmx_tsif_rx_set_external_tsdemux(struct tcc_hwdmx_tsif_rx_handle
					   *demux, int (*decoder) (char *p1,
								   int p1_size,
								   char *p2,
								   int p2_size,
								   int devid))
...
#endif
	return hwdmx_add_filter_cmd(&demux->tsif_ex_handle,
...
#endif
	return hwdmx_remove_filter_cmd(&demux->tsif_ex_handle,
				       (struct tcc_tsif_filter *)param);
}
				  ...
				  unsigned int index, unsigned int pcr_pid)
{
	...
	if (index == 0) {
		return hwdmx_set_pcrpid_cmd(&demux->tsif_ex_handle,
					    demux->pcr_pid[index]);
	}
	return 0;
}
...
int tcc_hwdmx_tsif_rx_set_cipher_dec_pid(struct tcc_hwdmx_tsif_rx_handle *demux,
					 ...
					 unsigned short *pids)
{
	hwdmx_set_cipher_dec_pid_cmd(&demux->tsif_ex_handle, numOfPids,
				     delete_option, pids);
	return 0;
}

int tcc_hwdmx_tsif_rx_set_mode(struct tcc_hwdmx_tsif_rx_handle *demux, int algo,
			       int opmode, int residual, int smsg,
			       unsigned int numOfPids, unsigned short *pids)
{
	hwdmx_set_algo_cmd(&demux->tsif_ex_handle, algo, opmode, residual, smsg,
			   numOfPids, pids);
	return 0;
}
...
int tcc_hwdmx_tsif_rx_register(int devid, struct device *dev)
{
#ifdef CONFIG_OF
	if (dev != NULL) {
		of_property_read_u32(dev->of_node, "tsif-port",
				     &tsif_ex_pri.port_cfg[devid].tsif_port);
		pr_info(
		"[INFO][HWDMX] tsif_port: %d\n",
		tsif_ex_pri.port_cfg[devid].tsif_port);
	} else {
	...
	if (rx_dma_alloc_buffer(devid) != 0) {
		pr_err(
		"[ERROR][HWDMX] rx_dma_alloc buffer failed\n");
		return -EFAULT;
	}
...
int tcc_hwdmx_tsif_rx_init(struct device *dev)
{
	 ...
	 * and 0 on failure. Weird function design.
	 */
	...
	tsif_ex_pri.mem_base =
	    ioremap_nocache(tsif_ex_pri.pmap_tsif.base,
			    tsif_ex_pri.pmap_tsif.size);
	if (tsif_ex_pri.mem_base == NULL) {
		ret = -EFAULT;
		goto out;
	}
	...
	tsif_ex_pri.pmap_tsif.base, tsif_ex_pri.mem_base,
	tsif_ex_pri.pmap_tsif.size);
	...
	tsif_ex_pri.virt_secure_tsif =
		ioremap_nocache(tsif_ex_pri.pmap_secure_tsif.base,
