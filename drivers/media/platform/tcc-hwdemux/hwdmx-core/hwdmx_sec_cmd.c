...
long long hwdmx_get_stc(struct tcc_tsif_handle *h)
{
	...
	mbox_data[2] = 0;
	mbox_data[3] = 0;
	sec_sendrecv_cmd(MBOX_DEV_M4, HWDMX_GET_STC_CMD, mbox_data, mbox_data, sizeof(mbox_data));
...
int hwdmx_input_stream_cmd(unsigned int dmx_id, unsigned int phy_addr, unsigned int size)
{
	...
	mutex_lock(&input_stream_mutex); // for secple demux, it is critical section
	/* Atomic operation */
...
int hwdmx_set_cipher_dec_pid_cmd(
	struct tcc_tsif_handle *h, unsigned int numOfPids, unsigned int delete_option,
	unsigned short *pids)
{
	int result = 0, rsize;
...
int hwdmx_set_algo_cmd(
	struct tcc_tsif_handle *h, int algo, int opmode, int residual, int smsg, unsigned int numOfPids,
	unsigned short *pids)
{
	int result = 0, rsize;
...
int hwdmx_setCw(
	struct tcc_tsif_handle *h /*unsigned int _tunerId*/, unsigned int _descramblingAlgorithm,
	unsigned int _opt, unsigned char *_evenKey, unsigned char *_oddKey, unsigned char *_iv,
