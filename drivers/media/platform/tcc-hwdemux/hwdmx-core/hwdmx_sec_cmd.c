...
#define MAGIC_NUM						0
#define HWDMX_START_CMD				SEC_IPC_CMD(MAGIC_NUM, 0x001)
#define HWDMX_STOP_CMD				SEC_IPC_CMD(MAGIC_NUM, 0x002)
#define HWDMX_ADD_FILTER_CMD		SEC_IPC_CMD(MAGIC_NUM, 0x003)
#define HWDMX_DELETE_FILTER_CMD		SEC_IPC_CMD(MAGIC_NUM, 0x004)
#define HWDMX_GET_STC_CMD			SEC_IPC_CMD(MAGIC_NUM, 0x005)
#define HWDMX_SET_PCRPID_CMD		SEC_IPC_CMD(MAGIC_NUM, 0x006)
#define HWDMX_GET_VERSION_CMD		SEC_IPC_CMD(MAGIC_NUM, 0x007)
#define HWDMX_INPUT_STREAM_CMD		SEC_IPC_CMD(MAGIC_NUM, 0x008)
#define HWDMX_CHANGE_INTERFACE_CMD	SEC_IPC_CMD(MAGIC_NUM, 0x009)
#define HWDMX_SET_MODE_CMD			SEC_IPC_CMD(MAGIC_NUM, 0x00A)
#define HWDMX_SET_KEY_CMD			SEC_IPC_CMD(MAGIC_NUM, 0x00B)
#define HWDMX_SET_IV_CMD			SEC_IPC_CMD(MAGIC_NUM, 0x00C)
#define HWDMX_SET_DATA_CMD			SEC_IPC_CMD(MAGIC_NUM, 0x00D)
#define HWDMX_SET_RUN_CMD			SEC_IPC_CMD(MAGIC_NUM, 0x00E)
#define HWDMX_SET_KDFDATA_CMD		SEC_IPC_CMD(MAGIC_NUM, 0x00F)
#define HWDMX_SET_KLDATA_CMD		SEC_IPC_CMD(MAGIC_NUM, 0x010)
#define HWDMX_GET_KLNRESP_CMD		SEC_IPC_CMD(MAGIC_NUM, 0x011)
#define HWDMX_SET_MODE_ADDPID_CMD	SEC_IPC_CMD(MAGIC_NUM, 0x012)
/* Up to 16 of events can be assigned. */
#define HWDMX_WBUF_UPDATED_EVT		SEC_IPC_EVENT(MAGIC_NUM, 1)
#define HWDMX_STC_RECV_EVT			SEC_IPC_EVENT(MAGIC_NUM, 2)
#define HWDMX_RBUF_EMPTY_EVT		SEC_IPC_EVENT(MAGIC_NUM, 3)
/* ... */
#define HWDMX_DEBUG_DATA_EVT		SEC_IPC_EVENT(MAGIC_NUM, 16)
// clang-format on
...
static FN_UPDATECALLBACK buf_updated_cb[8] = {
...
static unsigned long empty_bufevt_received;
...
static int hwdmx_evt_handler(int cmd, void *rdata, int size)
{
	...
	case HWDMX_WBUF_UPDATED_EVT:{
			...
			// ts: end point of updated buffer(offset),
			// sec: start point of updated
			// buffer(offset)
			value1 = ((int *)rdata)[3];

			// ts: end point of buffer(offset),
			// sec: end point of updated buffer(offset)
			...
			if (buf_updated_cb[dmxid] != NULL) {
				buf_updated_cb[dmxid] (dmxid, filter_type,
						       filter_id, value1,
						       value2, err_crc);
			}
	...
	case HWDMX_STC_RECV_EVT:{
			...
			if (buf_updated_cb[dmxid] != NULL) {
				buf_updated_cb[dmxid] (dmxid, filter_type, fid,
						       stc_base, stc_ext,
						       ((int *)rdata)[3]);
			}
			break;
	...
	if (iDMXID < 0) {
		for (i = 0; i < 8; i++) {
			if (interface[i] != tsif) {
				...
				mbox_data[1] = tsif;
				mbox_data[2] = 0;
				sec_sendrecv_cmd(MBOX_DEV_M4,
						 HWDMX_CHANGE_INTERFACE_CMD,
		...
		if (interface[iDMXID] != tsif) {
			...
			mbox_data[2] = 0;
			sec_sendrecv_cmd(MBOX_DEV_M4,
					 HWDMX_CHANGE_INTERFACE_CMD, mbox_data,
					 sizeof(mbox_data), NULL, 0);
		}
	}
}
...
int hwdmx_start_cmd(struct tcc_tsif_handle *h)
{
	...
	mbox_data[11] = 0;
	rsize =
	    sec_sendrecv_cmd(MBOX_DEV_M4, HWDMX_START_CMD, mbox_data,
...
int hwdmx_stop_cmd(struct tcc_tsif_handle *h)
{
	...
	rsize =
	    sec_sendrecv_cmd(MBOX_DEV_M4, HWDMX_STOP_CMD, &mbox_data,
			     sizeof(mbox_data), &mbox_result,
			     sizeof(mbox_result));
	if (rsize < 0) {
		pr_err("[ERROR][HWDMX] [%s:%d] sp_sendrecv_cmd error\n",
...
int hwdmx_buffer_updated_callback(struct tcc_tsif_handle *h,
...
int hwdmx_set_pcrpid_cmd(struct tcc_tsif_handle *h, unsigned int pid)
{
	...
	rsize =
	    sec_sendrecv_cmd(MBOX_DEV_M4, HWDMX_SET_PCRPID_CMD, mbox_data,
			     sizeof(mbox_data), &mbox_result,
			     sizeof(mbox_result));
	if (rsize < 0) {
		pr_err("[ERROR][HWDMX] [%s:%d] sp_sendrecv_cmd error\n",
...
long long hwdmx_get_stc(struct tcc_tsif_handle *h)
{
	...
	mbox_data[2] = 0;
	mbox_data[3] = 0;
	sec_sendrecv_cmd(MBOX_DEV_M4, HWDMX_GET_STC_CMD, mbox_data, mbox_data,
			 sizeof(mbox_data));
...
int hwdmx_add_filter_cmd(struct tcc_tsif_handle *h,
			 struct tcc_tsif_filter *feed)
{
	int result = 0, rsize;
	int mbox_data[5 + (16 * 3 / 4)], mbox_result;
	...
	case SECTION:
		...
		memcpy((int8_t *) (mbox_data + 5) + feed->f_size, feed->f_mask,
		       feed->f_size);
		memcpy((int8_t *) (mbox_data + 5) + feed->f_size * 2,
		       feed->f_mode, feed->f_size);

		rsize =
		    sec_sendrecv_cmd(MBOX_DEV_M4, HWDMX_ADD_FILTER_CMD,
				     mbox_data, 20 + feed->f_size * 3,
	...
	case PES:
		rsize =
		    sec_sendrecv_cmd(MBOX_DEV_M4, HWDMX_ADD_FILTER_CMD,
				     mbox_data, sizeof(int) * 3, &mbox_result,
				     sizeof(mbox_result));
		break;
...
int hwdmx_remove_filter_cmd(struct tcc_tsif_handle *h,
			    struct tcc_tsif_filter *feed)
{
	int result = 0, rsize;
	int mbox_data[4], mbox_result;
	...
	rsize =
	    sec_sendrecv_cmd(MBOX_DEV_M4, HWDMX_DELETE_FILTER_CMD, mbox_data,
			     sizeof(mbox_data), &mbox_result,
			     sizeof(mbox_result));
	if (rsize < 0) {
		pr_err("[ERROR][HWDMX] [%s:%d] sp_sendrecv_cmd error\n",
...
int hwdmx_input_stream_cmd(unsigned int dmx_id, unsigned int phy_addr,
			   unsigned int size)
{
	int result = 0, rsize;
	...
	mbox_data[2] = size;
	rsize =
	    sec_sendrecv_cmd(MBOX_DEV_M4, HWDMX_INPUT_STREAM_CMD, mbox_data,
			     sizeof(mbox_data), &mbox_result,
			     sizeof(mbox_result));
	if (rsize < 0) {
		pr_err("[ERROR][HWDMX] [%s:%d] sp_sendrecv_cmd error\n",
	...
	result =
	    wait_event_timeout(waitq_empty_bufevt, empty_bufevt_received == 1,
			       msecs_to_jiffies(2000));
	if (result == 0 && (empty_bufevt_received != 1)) {
		pr_info("[INFO][HWDMX] Timeout\n");
	} else {
		/* The condition (empty_bufevt_received == 1)
		 * is met before timeout
		 */
		result = 0;
	}

out:
	mutex_unlock(&input_stream_mutex);	// for secple demux
	return result;
}

int hwdmx_set_cipher_dec_pid_cmd(struct tcc_tsif_handle *h,
				 unsigned int numOfPids,
				 unsigned int delete_option,
				 unsigned short *pids)
{
	int result = 0, rsize;
	...
	rsize =
	    sec_sendrecv_cmd(MBOX_DEV_M4, HWDMX_SET_MODE_ADDPID_CMD, mbox_data,
			     sizeof(mbox_data), &mbox_result,
			     sizeof(mbox_result));
	if (rsize < 0) {
		pr_err("[ERROR][HWDMX] [%s:%d] sp_sendrecv_cmd error\n",
...
int hwdmx_set_algo_cmd(struct tcc_tsif_handle *h, int algo, int opmode,
		       ...
		       unsigned short *pids)
{
	...
	rsize =
	    sec_sendrecv_cmd(MBOX_DEV_M4, HWDMX_SET_MODE_CMD, mbox_data,
			     sizeof(mbox_data), &mbox_result,
			     sizeof(mbox_result));
	if (rsize < 0) {
		pr_err("[ERROR][HWDMX] [%s:%d] sp_sendrecv_cmd error\n",
...
int hwdmx_set_key_cmd(struct tcc_tsif_handle *h, int keytype, int keymode,
		      int size, void *key)
{
	int result = 0, rsize;
	int mbox_data[4 + 32], mbox_result;
	...
	rsize =
	    sec_sendrecv_cmd(MBOX_DEV_M4, HWDMX_SET_KEY_CMD, mbox_data,
			     sizeof(mbox_data), &mbox_result,
			     sizeof(mbox_result));
	if (rsize < 0) {
		pr_err("[ERROR][HWDMX] [%s:%d] sp_sendrecv_cmd error\n",
...
{
	int result = 0, rsize;
	int mbox_data[3 + 16], mbox_result;
	...
	rsize =
	    sec_sendrecv_cmd(MBOX_DEV_M4, HWDMX_SET_IV_CMD, mbox_data,
			     sizeof(mbox_data), &mbox_result,
			     sizeof(mbox_result));
	if (rsize < 0) {
		pr_err("[ERROR][HWDMX] [%s:%d] sp_sendrecv_cmd error\n",
...
int hwdmx_setCw(struct tcc_tsif_handle *h,
		unsigned int _descramblingAlgorithm,
		unsigned int _opt, unsigned char *_evenKey,
		unsigned char *_oddKey, unsigned char *_iv,
		unsigned int _numOfPids, unsigned short *_pids)
{
	hwdmx_set_algo_cmd(h, _descramblingAlgorithm, _opt, 0, 0, _numOfPids,
			   _pids);
...
int hwdmx_set_data_cmd(unsigned int dmxid, unsigned char *srcaddr,
		       unsigned char *destaddr, unsigned int srclen)
{
	int result = 0, rsize;
	int mbox_data[4], mbox_result;
	...
	mbox_data[3] = srclen;
	rsize =
	    sec_sendrecv_cmd(MBOX_DEV_M4, HWDMX_SET_DATA_CMD, mbox_data,
			     sizeof(mbox_data), &mbox_result,
			     sizeof(mbox_result));
	if (rsize < 0) {
		pr_err("[ERROR][HWDMX] [%s:%d] sp_sendrecv_cmd error\n",
...
int hwdmx_run_cipher_cmd(int dmxid, int encmode, int cwsel, int klidx,
			 int keymode)
{
	int result = 0, rsize;
