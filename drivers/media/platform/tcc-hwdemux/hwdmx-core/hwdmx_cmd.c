	...
	if (iDMXID < 0) {
		for (i = 0; i < 8; i++) {
			if (interface[i] != tsif) {
				...
				mbox_data[1] = tsif;
				mbox_data[2] = 0;
				sp_sendrecv_cmd(HWDMX_CHANGE_INTERFACE_CMD,
		...
		if (interface[iDMXID] != tsif) {
			...
			mbox_data[1] = tsif;
			mbox_data[2] = 0;
			sp_sendrecv_cmd(HWDMX_CHANGE_INTERFACE_CMD, mbox_data,
...
int hwdmx_start_cmd(struct tcc_tsif_handle *h)
{
	...
	rsize = sp_sendrecv_cmd(
		HWDMX_START_CMD, mbox_data, sizeof(mbox_data),
		&mbox_result, sizeof(mbox_result));
	if (rsize < 0) {
		pr_err(
		"[ERROR][HWDMX] [%s:%d] sp_sendrecv_cmd error\n",
		__func__, __LINE__);
		result = -EBADR;
		goto out;
	}
	...
	if (mbox_result == -EINVAL) {
		rsize = sp_sendrecv_cmd(
			HWDMX_START_CMD, mbox_data, 12 * sizeof(int),
			&mbox_result, sizeof(mbox_result));
		if (rsize < 0) {
			pr_err(
			"[ERROR][HWDMX] [%s:%d] sp_sendrecv_cmd error\n",
	...
	if (result != 0) {
		pr_err(
		"[ERROR][HWDMX] [%s:%d] SP returned an error: %d\n",
...
int hwdmx_stop_cmd(struct tcc_tsif_handle *h)
{
	...
	rsize = sp_sendrecv_cmd(
		HWDMX_STOP_CMD, &mbox_data, sizeof(mbox_data),
		&mbox_result, sizeof(mbox_result));
	if (rsize < 0) {
...
int hwdmx_set_pcrpid_cmd(struct tcc_tsif_handle *h, unsigned int pid)
{
	...
	rsize =
	    sp_sendrecv_cmd(HWDMX_SET_PCRPID_CMD, mbox_data, sizeof(mbox_data),
			    &mbox_result, sizeof(mbox_result));
	if (rsize < 0) {
		pr_err("[ERROR][HWDMX] [%s:%d] sp_sendrecv_cmd error\n",
		       __func__, __LINE__);
...
{
	// pr_info("[INFO][HWDMX] [DEMUX #%d]hwdmx_get_stc\n", h->dmx_id);
	...
	mbox_data[2] = 0;
	mbox_data[3] = 0;
	sp_sendrecv_cmd(HWDMX_GET_STC_CMD, mbox_data, mbox_data,
			sizeof(mbox_data));
			 ...
			 struct tcc_tsif_filter *feed)
{
	...
	case FILTER_TYPE_SECTION:
		if (feed->f_size > 16) {
			pr_err("[ERROR][HWDMX] filter size is over 16\n");
			feed->f_size = 16;
		}
...
#if 0
		print_hex_dump_bytes("FltComp: ", DUMP_PREFIX_ADDRESS,
				     feed->f_comp, feed->f_size);
		print_hex_dump_bytes("FltMask: ", DUMP_PREFIX_ADDRESS,
				     feed->f_mask, feed->f_size);
		print_hex_dump_bytes("FltMode: ", DUMP_PREFIX_ADDRESS,
				     feed->f_mode, feed->f_size);
		pr_info("[INFO][HWDMX] %s:%d mbox_size: %d\n", __func__,
			__LINE__, 20 + feed->f_size * 3);
#endif
		rsize =
		    sp_sendrecv_cmd(HWDMX_ADD_FILTER_CMD, mbox_data,
				    20 + feed->f_size * 3, &mbox_result,
				    sizeof(mbox_result));
		break;
			    ...
			    struct tcc_tsif_filter *feed)
{
	...
	rsize =
	    sp_sendrecv_cmd(HWDMX_DELETE_FILTER_CMD, mbox_data,
			   ...
			   unsigned int size)
{
	...
	mbox_data[2] = size;
	rsize =
	    sp_sendrecv_cmd(HWDMX_INPUT_STREAM_CMD, mbox_data,
			    sizeof(mbox_data), &mbox_result,
			    sizeof(mbox_result));
	if (rsize < 0) {
		pr_err("[ERROR][HWDMX] [%s:%d] sp_sendrecv_cmd error\n",
	...
	if (result == 0 && (empty_bufevt_received != 1)) {
		...
		/* The condition is met before timeout */
		result = 0;
	}
				 ...
				 unsigned short *pids)
{
	...
	rsize =
	    sp_sendrecv_cmd(HWDMX_SET_MODE_ADDPID_CMD, mbox_data,
			    sizeof(mbox_data), &mbox_result,
			    sizeof(mbox_result));
	if (rsize < 0) {
		pr_err("[ERROR][HWDMX] [%s:%d] sp_sendrecv_cmd error\n",
...
int hwdmx_set_algo_cmd(struct tcc_tsif_handle *h, int algo,
		       int opmode, int residual, int smsg,
		       unsigned int numOfPids, unsigned short *pids)
{
	int result = 0, rsize;
	...
	rsize =
	    sp_sendrecv_cmd(HWDMX_SET_MODE_CMD, mbox_data, sizeof(mbox_data),
			    &mbox_result, sizeof(mbox_result));
	if (rsize < 0) {
		pr_err("[ERROR][HWDMX] [%s:%d] sp_sendrecv_cmd error\n",
		       __func__, __LINE__);
		      ...
		      int size, void *key)
{
	...
	rsize =
	    sp_sendrecv_cmd(HWDMX_SET_KEY_CMD, mbox_data, sizeof(mbox_data),
			    &mbox_result, sizeof(mbox_result));
	if (rsize < 0) {
		pr_err("[ERROR][HWDMX] [%s:%d] sp_sendrecv_cmd error\n",
		       __func__, __LINE__);
...
int hwdmx_set_iv_cmd(struct tcc_tsif_handle *h, int ividx, int size, void *iv)
{
	...
	rsize =
	    sp_sendrecv_cmd(HWDMX_SET_IV_CMD, mbox_data, sizeof(mbox_data),
			    &mbox_result, sizeof(mbox_result));
	if (rsize < 0) {
		pr_err("[ERROR][HWDMX] [%s:%d] sp_sendrecv_cmd error\n",
		       __func__, __LINE__);
		       ...
		       unsigned char *destaddr, unsigned int srclen)
{
	...
	mbox_data[3] = srclen;
	rsize =
	    sp_sendrecv_cmd(HWDMX_SET_DATA_CMD, mbox_data, sizeof(mbox_data),
			    &mbox_result, sizeof(mbox_result));
	if (rsize < 0) {
		pr_err("[ERROR][HWDMX] [%s:%d] sp_sendrecv_cmd error\n",
		__func__, __LINE__);
...
int hwdmx_set_smp(int dmxch, int enabled)
{
	...
	result = sp_sendrecv_cmd(
		HWDMX_SET_SMP_CMD, mbox_data, sizeof(mbox_data),
		&mbox_result, sizeof(mbox_result));
	if (result < 0) {
