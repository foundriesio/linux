...
int tccvin_check_cif_pinctrl(struct device * dev, const char * funcname) {
...
static int tccvin_check_cif_port(struct v4l2_subdev *sd)
{
	...
	/* check pinctrl */
	ret = tccvin_check_cif_pinctrl(&client->dev, PINCTRL_STATE_DEFAULT);
	if (ret != 0) {
		/* failure of tccvin_check_cif_pinctrl */
		loge("tccvin_check_cif_pinctrl, ret: %d\n", ret);
	}
