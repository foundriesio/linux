...
static irqreturn_t tccvin_vin_isr(int irq, void *data)
{
	...
	if (status & VIN_INT_VS_MASK) {
		...
		logd(dev_ptr,
			...
			stream->next_buf->buf.vb2_buf.memory);
		logd(dev_ptr,
			"bufidx: %d, addr: 0x%08lx, 0x%08lx, 0x%08lx\n",
			stream->next_buf->buf.vb2_buf.index,
			addr[0], addr[1], addr[2]);
		VIOC_WDMA_SetImageBase(wdma, addr[0], addr[1], addr[2]);
wdma_update:
...
int32_t tccvin_video_subdevs_set_fmt(struct tccvin_streaming *stream)
{
	...
	int32_t				idx		= 0;
	struct v4l2_subdev		*subdev		= NULL;
	struct tccvin_subdev		*tc_sd		= NULL;
	...
	tc_sd = &dev->linked_subdevs[dev->bounded_subdevs - 1];
	dev_ptr = tccvin_streaming_to_devptr(stream);
	...
	if (tc_sd->channel != -1) {
		logi(dev_ptr, "call %s channel(%d) get format\n",
			subdev->name,
			tc_sd->channel);
	} else {
	...
	for (idx = dev->bounded_subdevs-2; idx >= 0; idx--) {
		subdev = dev->linked_subdevs[idx].sd;
		tc_sd = &dev->linked_subdevs[idx];

		if (tc_sd->channel != -1) {
			logi(dev_ptr, "call %s channel(%d) set_fmt\n",
				subdev->name,
				tc_sd->channel);
		} else {
