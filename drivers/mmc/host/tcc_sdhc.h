...
#define  TCCSDHC_SPACE_AVAILABLE	0x00000400 // Buffer Write Available
#define  TCCSDHC_DATA_AVAILABLE		0x00000800 // Buffer Read Available
...
// Delay value for CLK, CMD and DATA
#define TCCSDHC_TAPDLY_TUNE_CNT(x)		((x & 0x3F) << 16)
#define TCCSDHC_TAPDLY_ITAP_SEL(x)		((x & 0x1F) << 0)
#define TCCSDHC_TAPDLY_ITAP_SEL_MASK	(TCCSDHC_TAPDLY_ITAP_SEL(0x1F))
#define TCCSDHC_TAPDLY_ITAP_EN(x)		((x & 0x1) << 5)
#define TCCSDHC_TAPDLY_ITAP_CHGWIN(x)	((x & 0x1) << 6)
#define TCCSDHC_TAPDLY_OTAP_SEL(x)		((x & 0x1F) << 8)
#define TCCSDHC_TAPDLY_OTAP_SEL_MASK	(TCCSDHC_TAPDLY_OTAP_SEL(0x1F))
#define TCCSDHC_TAPDLY_OTAP_EN(x)		((x & 0x1) << 13)
#define TCCSDHC_TAPDLY_ASYNCWKUP_EN(x)	((x & 0x1) << 14)

#define TCCSDHC_CMDDLY_IN(x)			((x & 0xF) << 16)
#define TCCSDHC_CMDDLY_OUT(x)			((x & 0xF) << 20)
#define TCCSDHC_CMDDLY_EN(x)			((x & 0xF) << 24)

#define TCCSDHC_DATADLY_IN(n, x)		((x & 0xF) << ((n & 0x1) * 12))
...
#define TCCSDHC_TAPDLY_DEF				(TCCSDHC_TAPDLY_TUNE_CNT(0x20) \
	...
	| TCCSDHC_TAPDLY_ASYNCWKUP_EN(1) )

#define TCCSDHC_CMDDLY_DEF_TAP		(8)
#define TCCSDHC_DATADLY_DEF_TAP		(8)

#define TCCSDHC_MK_CMDDLY(x)			(TCCSDHC_CMDDLY_IN(x) \
	| TCCSDHC_CMDDLY_OUT(x) \
	| TCCSDHC_CMDDLY_EN(x) )

#define TCCSDHC_MK_DATADLY(x)				(TCCSDHC_DATADLY_IN(0, x) \
	| TCCSDHC_DATADLY_OUT(0, x) \
	| TCCSDHC_DATADLY_EN(0, x) \
	| TCCSDHC_DATADLY_IN(1, x) \
	| TCCSDHC_DATADLY_OUT(1, x) \
	| TCCSDHC_DATADLY_EN(1, x) )
...
struct tcc_mmc_tap_delays {
	...
	u32	data45_tap_delay;
	u32	data67_tap_delay;
};
...
#define TCC_MMC_REQ_USE_DMA	(1<<2)	/* Use DMA for this req. */
	...
	struct tcc_mmc_tap_delays tcc_tap_delays;
};
