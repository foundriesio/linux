...
static struct platform_device *tcc_drm_ipp_pdev;
 ...
 * @event: ipp event.
 */
struct drm_tcc_ipp_send_event {
	struct drm_pending_event	base;
	struct drm_tcc_ipp_event	event;
};
 ...
 */
struct drm_tcc_ipp_mem_node {
	struct list_head	list;
	enum drm_tcc_ops_id	ops_id;
	u32	prop_id;
	u32	buf_id;
	struct drm_tcc_ipp_buf_info	buf_info;
};
...
static LIST_HEAD(tcc_drm_ippdrv_list);
static DEFINE_MUTEX(tcc_drm_ippdrv_lock);
static BLOCKING_NOTIFIER_HEAD(tcc_drm_ippnb_list);

int tcc_platform_device_ipp_register(void)
{
	...
	pdev = platform_device_register_simple("tcc-drm-ipp", -1, NULL, 0);
	if (IS_ERR(pdev))
		return PTR_ERR(pdev);

	tcc_drm_ipp_pdev = pdev;

	return 0;
}

void tcc_platform_device_ipp_unregister(void)
{
	if (tcc_drm_ipp_pdev) {
		platform_device_unregister(tcc_drm_ipp_pdev);
		tcc_drm_ipp_pdev = NULL;
	}
}

int tcc_drm_ippdrv_register(struct tcc_drm_ippdrv *ippdrv)
{
	mutex_lock(&tcc_drm_ippdrv_lock);
	list_add_tail(&ippdrv->drv_list, &tcc_drm_ippdrv_list);
	mutex_unlock(&tcc_drm_ippdrv_lock);

	return 0;
}

int tcc_drm_ippdrv_unregister(struct tcc_drm_ippdrv *ippdrv)
{
	mutex_lock(&tcc_drm_ippdrv_lock);
...
static int ipp_check_driver(struct tcc_drm_ippdrv *ippdrv,
...
static struct tcc_drm_ippdrv *ipp_find_driver(struct ipp_context *ctx,
		struct drm_tcc_ipp_property *property)
{
	...
	if (ipp_id) {
		...
		list_for_each_entry(ippdrv, &tcc_drm_ippdrv_list, drv_list) {
			ret = ipp_check_driver(ippdrv, property);
			if (ret == 0)
				return ippdrv;
		}
...
static struct tcc_drm_ippdrv *ipp_find_drv_by_handle(u32 prop_id)
{
	struct tcc_drm_ippdrv *ippdrv;
...
int tcc_drm_ipp_get_property(struct drm_device *drm_dev, void *data,
		struct drm_file *file)
{
	struct drm_tcc_file_private *file_priv = file->driver_priv;
	struct device *dev = file_priv->ipp_dev;
	struct ipp_context *ctx = get_ipp_context(dev);
	struct drm_tcc_ipp_prop_list *prop_list = data;
	struct tcc_drm_ippdrv *ippdrv;
	int count = 0;
...
static void ipp_print_property(struct drm_tcc_ipp_property *property,
		int idx)
{
	struct drm_tcc_ipp_config *config = &property->config[idx];
	struct drm_tcc_pos *pos = &config->pos;
	struct drm_tcc_sz *sz = &config->sz;
...
static struct drm_tcc_ipp_cmd_work *ipp_create_cmd_work(void)
{
	struct drm_tcc_ipp_cmd_work *cmd_work;
...
static struct drm_tcc_ipp_event_work *ipp_create_event_work(void)
{
...
int tcc_drm_ipp_set_property(struct drm_device *drm_dev, void *data,
		struct drm_file *file)
{
	struct drm_tcc_file_private *file_priv = file->driver_priv;
	struct device *dev = file_priv->ipp_dev;
	struct ipp_context *ctx = get_ipp_context(dev);
	struct drm_tcc_ipp_property *property = data;
	struct tcc_drm_ippdrv *ippdrv;
	struct drm_tcc_ipp_cmd_node *c_node;
	u32 prop_id;
	int ret, i;
...
static int ipp_validate_mem_node(struct drm_device *drm_dev,
				 struct drm_tcc_ipp_mem_node *m_node,
				 struct drm_tcc_ipp_cmd_node *c_node)
{
	struct drm_tcc_ipp_config *ipp_cfg;
	...
	for (i = 0; i < num_plane; ++i) {
		...
		size = tcc_drm_gem_get_size(drm_dev,
		...
		struct drm_tcc_ipp_mem_node *m_node)
{
	...
	for_each_ipp_planar(i) {
		...
		if (handle)
			tcc_drm_gem_put_dma_addr(drm_dev, handle,
							c_node->filp);
	}
...
static struct drm_tcc_ipp_mem_node
		*ipp_get_mem_node(struct drm_device *drm_dev,
		struct drm_tcc_ipp_cmd_node *c_node,
		struct drm_tcc_ipp_queue_buf *qbuf)
{
	struct drm_tcc_ipp_mem_node *m_node;
	struct drm_tcc_ipp_buf_info *buf_info;
	int i;
	...
	for_each_ipp_planar(i) {
		...
		if (qbuf->handle[i]) {
			...
			addr = tcc_drm_gem_get_dma_addr(drm_dev,
...
static void ipp_clean_mem_nodes(struct drm_device *drm_dev,
			       struct drm_tcc_ipp_cmd_node *c_node, int ops)
{
	struct drm_tcc_ipp_mem_node *m_node, *tm_node;
...
static int ipp_get_event(struct drm_device *drm_dev,
		...
		struct drm_tcc_ipp_queue_buf *qbuf)
{
	struct drm_tcc_ipp_send_event *e;
	unsigned long flags;
	...
	/* make event */
	e->event.base.type = DRM_TCC_IPP_EVENT;
	e->event.base.length = sizeof(e->event);
	e->event.user_data = qbuf->user_data;
	e->event.prop_id = qbuf->prop_id;
	e->event.buf_id[TCC_DRM_OPS_DST] = qbuf->buf_id;
...
static void ipp_put_event(struct drm_tcc_ipp_cmd_node *c_node,
		struct drm_tcc_ipp_queue_buf *qbuf)
{
	struct drm_tcc_ipp_send_event *e, *te;
	int count = 0;
	...
	list_for_each_entry_safe(e, te, &c_node->event_list, base.link) {
		...
		/* compare buffer id */
		if (qbuf && (qbuf->buf_id ==
		    e->event.buf_id[TCC_DRM_OPS_DST])) {
			/* delete list */
			list_del(&e->base.link);
			kfree(e);
			goto out_unlock;
...
static bool ipp_check_mem_list(struct drm_tcc_ipp_cmd_node *c_node)
{
	switch (c_node->property.cmd) {
	case IPP_CMD_WB:
		return !list_empty(&c_node->mem_list[TCC_DRM_OPS_DST]);
	case IPP_CMD_OUTPUT:
		return !list_empty(&c_node->mem_list[TCC_DRM_OPS_SRC]);
	case IPP_CMD_M2M:
	default:
		return !list_empty(&c_node->mem_list[TCC_DRM_OPS_SRC]) &&
		       !list_empty(&c_node->mem_list[TCC_DRM_OPS_DST]);
	}
}

static struct drm_tcc_ipp_mem_node
		*ipp_find_mem_node(struct drm_tcc_ipp_cmd_node *c_node,
		struct drm_tcc_ipp_queue_buf *qbuf)
{
	struct drm_tcc_ipp_mem_node *m_node;
...
static int ipp_set_mem_node(struct tcc_drm_ippdrv *ippdrv,
		struct drm_tcc_ipp_cmd_node *c_node,
		struct drm_tcc_ipp_mem_node *m_node)
{
	struct tcc_drm_ipp_ops *ops = NULL;
	int ret = 0;
		...
		struct drm_tcc_ipp_queue_buf *qbuf)
{
	 ...
	 */
	if (ipp_is_m2m_cmd(property->cmd)) {
		struct drm_tcc_ipp_cmd_work *cmd_work = c_node->start_work;
...
int tcc_drm_ipp_queue_buf(struct drm_device *drm_dev, void *data,
		struct drm_file *file)
{
	struct drm_tcc_file_private *file_priv = file->driver_priv;
	struct device *dev = file_priv->ipp_dev;
	struct ipp_context *ctx = get_ipp_context(dev);
	struct drm_tcc_ipp_queue_buf *qbuf = data;
	struct drm_tcc_ipp_cmd_node *c_node;
	struct drm_tcc_ipp_mem_node *m_node;
	int ret;
	...
	if (qbuf->ops_id >= TCC_DRM_OPS_MAX) {
	...
	case IPP_BUF_ENQUEUE:
		 ...
		 * if needed.
		 */
		if (qbuf->ops_id == TCC_DRM_OPS_DST) {
...
static bool tcc_drm_ipp_check_valid(struct device *dev,
		enum drm_tcc_ipp_ctrl ctrl, enum drm_tcc_ipp_state state)
{
...
int tcc_drm_ipp_cmd_ctrl(struct drm_device *drm_dev, void *data,
		struct drm_file *file)
{
	struct drm_tcc_file_private *file_priv = file->driver_priv;
	struct tcc_drm_ippdrv *ippdrv = NULL;
	struct device *dev = file_priv->ipp_dev;
	struct ipp_context *ctx = get_ipp_context(dev);
	struct drm_tcc_ipp_cmd_ctrl *cmd_ctrl = data;
	struct drm_tcc_ipp_cmd_work *cmd_work;
	struct drm_tcc_ipp_cmd_node *c_node;
	...
	if (!tcc_drm_ipp_check_valid(ippdrv->dev, cmd_ctrl->ctrl,
	    c_node->state)) {
		DRM_ERROR("invalid state.\n");
		return -EINVAL;
	}
...
int tcc_drm_ippnb_register(struct notifier_block *nb)
{
	return blocking_notifier_chain_register(
		&tcc_drm_ippnb_list, nb);
}

int tcc_drm_ippnb_unregister(struct notifier_block *nb)
{
	return blocking_notifier_chain_unregister(
		&tcc_drm_ippnb_list, nb);
}

int tcc_drm_ippnb_send_event(unsigned long val, void *v)
{
	return blocking_notifier_call_chain(
		&tcc_drm_ippnb_list, val, v);
}

static int ipp_set_property(struct tcc_drm_ippdrv *ippdrv,
		struct drm_tcc_ipp_property *property)
{
	struct tcc_drm_ipp_ops *ops = NULL;
	bool swap = false;
...
static int ipp_start_property(struct tcc_drm_ippdrv *ippdrv,
		struct drm_tcc_ipp_cmd_node *c_node)
{
	struct drm_tcc_ipp_mem_node *m_node;
	struct drm_tcc_ipp_property *property = &c_node->property;
	struct list_head *head;
	int ret, i;
	...
	case IPP_CMD_M2M:
		for_each_ipp_ops(i) {
			...
			m_node = list_first_entry(head,
				struct drm_tcc_ipp_mem_node, list);
	...
	case IPP_CMD_WB:
		...
		head = &c_node->mem_list[TCC_DRM_OPS_DST];
	...
	case IPP_CMD_OUTPUT:
		/* source memory list */
		head = &c_node->mem_list[TCC_DRM_OPS_SRC];
		...
		struct drm_tcc_ipp_cmd_node *c_node)
{
	...
	case IPP_CMD_M2M:
		...
		break;
	case IPP_CMD_WB:
		ipp_clean_mem_nodes(drm_dev, c_node, TCC_DRM_OPS_DST);
		break;
	case IPP_CMD_OUTPUT:
		ipp_clean_mem_nodes(drm_dev, c_node, TCC_DRM_OPS_SRC);
		break;
	default:
		DRM_ERROR("invalid operations.\n");
		return -EINVAL;
...
{
	struct drm_tcc_ipp_cmd_work *cmd_work =
		container_of(work, struct drm_tcc_ipp_cmd_work, work);
	struct tcc_drm_ippdrv *ippdrv;
	struct drm_tcc_ipp_cmd_node *c_node;
	struct drm_tcc_ipp_property *property;
	int ret;
...
static int ipp_send_event(struct tcc_drm_ippdrv *ippdrv,
		struct drm_tcc_ipp_cmd_node *c_node, int *buf_id)
{
	...
	case IPP_CMD_WB:
		...
		qbuf.ops_id = TCC_DRM_OPS_DST;
		qbuf.buf_id = buf_id[TCC_DRM_OPS_DST];
		...
		tbuf_id[TCC_DRM_OPS_DST] = m_node->buf_id;
	...
	case IPP_CMD_OUTPUT:
		...
		tbuf_id[TCC_DRM_OPS_SRC] = m_node->buf_id;
	...
	if (tbuf_id[TCC_DRM_OPS_DST] != buf_id[TCC_DRM_OPS_DST])
	...
	e = list_first_entry(&c_node->event_list,
		struct drm_tcc_ipp_send_event, base.link);
	...
	DRM_DEBUG_KMS("done cmd[%d]prop_id[%d]buf_id[%d]\n",
		property->cmd, property->prop_id, tbuf_id[TCC_DRM_OPS_DST]);
...
{
	struct drm_tcc_ipp_event_work *event_work =
		container_of(work, struct drm_tcc_ipp_event_work, work);
	struct tcc_drm_ippdrv *ippdrv;
	struct drm_tcc_ipp_cmd_node *c_node;
	int ret;
	...
	DRM_DEBUG_KMS("buf_id[%d]\n", event_work->buf_id[TCC_DRM_OPS_DST]);
...
err:
	/* get ipp driver entry */
	list_for_each_entry_continue_reverse(ippdrv, &tcc_drm_ippdrv_list, drv_list) {
#if 0
		if (is_drm_iommu_supported(drm_dev))
...
{
	struct tcc_drm_ippdrv *ippdrv, *t;
	struct ipp_context *ctx = get_ipp_context(dev);

	/* get ipp driver entry */
	list_for_each_entry_safe(ippdrv, t, &tcc_drm_ippdrv_list, drv_list) {
#if 0
		if (is_drm_iommu_supported(drm_dev))
			drm_iommu_detach_device(drm_dev, ippdrv->dev);
#endif
		...
		struct drm_file *file)
{
	struct tcc_drm_ippdrv *ippdrv = NULL;
	struct ipp_context *ctx = get_ipp_context(dev);
	struct drm_tcc_ipp_cmd_node *c_node, *tc_node;
	int count = 0;
...
static int ipp_probe(struct platform_device *pdev)
{
	...
	ret = tcc_drm_subdrv_register(subdrv);
	if (ret < 0) {
...
static int ipp_remove(struct platform_device *pdev)
{
	...
	tcc_drm_subdrv_unregister(&ctx->subdrv);
