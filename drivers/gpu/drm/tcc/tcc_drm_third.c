...
struct third_context {
	...
	struct third_driver_data *driver_data;
...
static inline struct third_driver_data *drm_third_get_driver_data(
	struct platform_device *pdev)
{
	const struct of_device_id *of_id =
			of_match_device(third_driver_dt_match, &pdev->dev);

	return (struct third_driver_data *)of_id->data;
}
...
static void third_win_set_colkey(struct third_context *ctx, unsigned int win)
{
 ...
 */
static void third_shadow_protect_win(struct third_context *ctx,
			       ...
			       struct tcc_drm_plane *plane)
{
	...
	third_shadow_protect_win(ctx, plane->zpos, true);
}
			       ...
			       struct tcc_drm_plane *plane)
{
	...
	third_shadow_protect_win(ctx, plane->zpos, false);
}
			      ...
			      struct tcc_drm_plane *plane)
{
	...
	third_win_set_pixfmt(ctx, win, state->fb);
...
static const struct tcc_drm_crtc_ops third_crtc_ops = {
	...
	.wait_for_vblank = third_wait_for_vblank,
...
static int third_bind(struct device *dev, struct device *master, void *data)
{
	...
	for (zpos = 0; zpos < WINDOWS_NR; zpos++) {
		...
		ret = tcc_plane_init(drm_dev, &ctx->planes[zpos],
					1 << ctx->pipe, type, third_formats,
					ARRAY_SIZE(third_formats), zpos);
		if (ret)
			return ret;
	}
	...
	ctx->crtc = tcc_drm_crtc_create(drm_dev, &tcc_plane->base,
					   ctx->pipe, TCC_DISPLAY_TYPE_THIRD,
					   &third_crtc_ops, ctx);
	if (IS_ERR(ctx->crtc))
...
static int third_probe(struct platform_device *pdev)
{
	...
	ctx->dev = dev;
	ctx->suspended = true;
	ctx->driver_data = drm_third_get_driver_data(pdev);
	...
	ctx->irq = res->start;
	ret = devm_request_irq(dev, ctx->irq, third_irq_handler,
