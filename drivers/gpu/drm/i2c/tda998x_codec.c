			...
			struct snd_soc_dai *dai)
{
	...
	if (dai->id == priv->audio_params.format &&
	    params_format(params) == priv->audio_params.format) {
	...
	priv->audio_params.config =
		priv->audio_port[dai->id == (int)AFMT_I2S ? 0 : 1].config;
...
static int tda_probe(struct snd_soc_codec *codec)
{
	...
	for (i = 0; i < 2; i++) {
		u32 port_val, port_id;

		ret = of_property_read_u32_index(np, "audio-ports", 2 * i + 1,
						&port_val);
		if (ret) {
			if (i == 0)
				dev_err(codec->dev,
		...
		if (ret) {
			dev_err(codec->dev,
				"missing audio-port-ID[%d]\n", i);
			break;
		}
		if (port_id == TDA998x_I2S) {
			j = 0;
		} else if (port_id == TDA998x_SPDIF) {
			j = 1;
		} else {
			dev_err(codec->dev,
				"bad audio-port-ID '%d'\n", port_id);
		...
		priv->audio_port[j].config = (u8)port_val;
		priv->audio_port[j].format = (u8)port_id;
	}
	return 0;
}
