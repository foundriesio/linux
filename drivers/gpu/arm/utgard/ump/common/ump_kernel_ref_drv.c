...
_mali_osk_errcode_t _ump_ukk_allocate(_ump_uk_allocate_s *user_interaction)
{
	...
	if((UMP_REF_DRV_UK_CONSTRAINT_PHYSICALLY_LINEAR & user_interaction->constraints) 
		|| (UMP_REF_DRV_UK_CONSTRAINT_PHYSICALLY_LINEAR_AF & user_interaction->constraints))
	{
		if(UMP_REF_DRV_UK_CONSTRAINT_PHYSICALLY_LINEAR_AF & user_interaction->constraints)
			new_allocation->autofree_enable = 1;
		else
			new_allocation->autofree_enable = 0;
