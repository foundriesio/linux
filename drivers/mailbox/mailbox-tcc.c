...
static int tcc_mbox_send_data(struct mbox_chan *chan, void *data)
{
	...
	writel_relaxed(readl_relaxed(mdev->mbox_base + MBOX_CTR) & ~(1 << 5),
		mdev->mbox_base + MBOX_CTR);
	...
	if (msg->trans_type == DATA_MBOX && msg->msg_len > 0) {
		...
		for (idx = 0; idx < (msg->msg_len + 3) / 4; idx++) {
			writel_relaxed(
				message[idx], mdev->mbox_base + MBOX_TXD);
		}

		while ((readl_relaxed(mdev->mbox_base + MBOX_TXD_STS) & 0xFFFF)
			!= idx)
			;
	}
...
static int tcc_mbox_startup(struct mbox_chan *chan)
{
	...
	spin_lock_irqsave(&mdev->lock, flags);
	writel_relaxed(readl_relaxed(mdev->mbox_base + MBOX_CTR) | 0x10,
		mdev->mbox_base + MBOX_CTR);
	spin_unlock_irqrestore(&mdev->lock, flags);
...
static irqreturn_t tcc_mbox_irq(int irq, void *dev_id)
{
	...
	if (!(status & (1 << 16)) && (irq == mdev->irq)) {
		...
		spin_lock_irqsave(&mdev->lock, flags);
		writel_relaxed(
			readl_relaxed(mdev->mbox_base + MBOX_CTR) & ~(1 << 4),
			mdev->mbox_base + MBOX_CTR);
		spin_unlock_irqrestore(&mdev->lock, flags);
...
static irqreturn_t tcc_mbox_isr(int irq, void *dev_id)
{
	...
	if (((readl_relaxed(mdev->mbox_base + MBOX_CMD_STS) >> 20) & 0xF)
	    == 0) {
	...
	if ((mdev->msg->trans_type == DATA_MBOX) && mdev->msg->msg_len > 0) {
		...
		for (idx = 0; idx < (mdev->msg->msg_len + 3) / 4; idx++) {
			message[idx] =
				readl_relaxed(mdev->mbox_base + MBOX_RXD);
		}

#if (0)
		/* Disable this statement because of mailbox hang. */
		while ((readl_relaxed(mdev->mbox_base + MBOX_RXD_STS)
			& (1 << 31)) == 0)
			;
#endif
	}
	...
	if (mdev->mbox.chans[0].cl != NULL) {
		mbox_chan_received_data(
			&mdev->mbox.chans[0], (void *)mdev->msg);
	}

	/* Set mbox received interrupt bit */
	spin_lock_irqsave(&mdev->lock, flags);
	writel_relaxed(readl_relaxed(mdev->mbox_base + MBOX_CTR) | (1 << 4),
		mdev->mbox_base + MBOX_CTR);
	spin_unlock_irqrestore(&mdev->lock, flags);
...
static int tcc_mbox_thread(void *arg)
{
	...
	while (!kthread_should_stop()) {
		wait_event_interruptible_timeout(
			mdev->wait, mdev->received == 1, msecs_to_jiffies(500));
...
static int tcc_mbox_probe(struct platform_device *pdev)
{
	...
	mdev = devm_kzalloc(
		&pdev->dev, sizeof(struct tcc_mbox_device), GFP_KERNEL);
	if (!mdev)
		return -ENOMEM;
	...
	mdev->msg = devm_kzalloc(
		&pdev->dev, sizeof(struct tcc_mbox_msg), GFP_KERNEL);
	if (!mdev->msg)
		return -ENOMEM;
	...
	ret = devm_request_threaded_irq(
		&pdev->dev, mdev->irq, tcc_mbox_irq, tcc_mbox_isr, IRQF_ONESHOT,
		dev_name(&pdev->dev), mdev);
#else
	ret = request_irq(
		mdev->irq, tcc_mbox_irq, 0, dev_name(&pdev->dev), mdev);
#endif
	if (ret < 0)
		return ret;
